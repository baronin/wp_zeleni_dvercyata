...
final class _WP_Editors
{
    ...
    public static function parse_settings($editor_id, $settings)
    {
        ...
        if (
            "content" === $editor_id &&
            empty($set["tinymce"]["wp_autoresize_on"])
        ) {
    ...
    public static function editor($content, $editor_id, $settings = [])
    {
        ...
        if (self::$this_tinymce) {
            ...
            if (self::$this_quicktags) {
                $default_editor = $set["default_editor"]
                    ? $set["default_editor"]
                    : wp_default_editor();
                ...
                $tmce_active =
                    "html" === $default_editor ? ' aria-pressed="true"' : "";
                $html_active =
                    "html" === $default_editor ? "" : ' aria-pressed="true"';
        ...
        $switch_class =
            "html" === $default_editor ? "html-active" : "tmce-active";
    ...
    public static function editor_settings($editor_id, $set)
    {
        if (empty(self::$first_init)) {
            if (is_admin()) {
                ...
                add_action(
                    "admin_print_footer_scripts",
                    [__CLASS__, "force_uncompressed_tinymce"],
                    1
                );
                add_action(
                    "admin_print_footer_scripts",
        ...
        if (self::$this_quicktags) {
            ...
            self::$qt_buttons = array_merge(
                self::$qt_buttons,
                explode(",", $qt_init["buttons"])
            );
        }
        ...
        if (self::$this_tinymce) {
            if (empty(self::$first_init)) {
                ...
                if ($set["teeny"]) {
                     ...
                     *                                 when called from block editor's Classic block.
                     */
                    $mce_external_plugins = apply_filters(
                    ...
                    if (!empty($mce_external_plugins)) {
                         ...
                         * @param string $editor_id    Unique editor identifier, e.g. 'content'.
                         */
                        $mce_external_languages = apply_filters(
                ...
                if (!empty($mce_external_plugins)) {
                    $settings["external_plugins"] = wp_json_encode(
                        $mce_external_plugins
                    );
                }
            ...
            if ($post) {
                $body_class .=
                    " post-type-" .
                    sanitize_html_class($post->post_type) .
                    " post-status-" .
                    sanitize_html_class($post->post_status);

                if (post_type_supports($post->post_type, "post-formats")) {
                    $post_format = get_post_format($post);
                    if ($post_format && !is_wp_error($post_format)) {
                        $body_class .=
                            " post-format-" . sanitize_html_class($post_format);
                    } else {
                ...
                if (false !== $page_template) {
                    $page_template = empty($page_template)
                        ? "default"
                        : str_replace(
                            ".",
                            "-",
    ...
    public static function enqueue_scripts($default_scripts = false)
    {
        ...
        if (
            $default_scripts ||
            in_array("wplink", self::$plugins, true) ||
            in_array("link", self::$qt_buttons, true)
        ) {
    ...
    public static function enqueue_default_editor()
    {
        ...
        if (is_admin()) {
            ...
            add_action(
                "admin_print_footer_scripts",
                [__CLASS__, "print_default_editor_scripts"],
                45
            );
        } else {
            add_action(
  ...
  self::wp_link_dialog();
    ...
    public static function get_mce_locale()
    {
        if (empty(self::$mce_locale)) {
            ...
            self::$mce_locale = empty($mce_locale)
                ? "en"
                : strtolower(substr($mce_locale, 0, 2)); // ISO 639-1.
        }
    ...
    private static function get_translation()
    {
        if (empty(self::$translation)) {
            self::$translation = [
                ...
                "Id should start with a letter, followed only by letters, numbers, dashes, dots, colons or underscores." => __(
                ...
                "Paste your embed code below:" => __(
                ...
                "Url" => __("URL"),
                "The URL you entered seems to be an email address. Do you want to add the required mailto: prefix?" => __(
                    "The URL you entered seems to be an email address. Do you want to add the required mailto: prefix?"
                ),
                "The URL you entered seems to be an external link. Do you want to add the required http:// prefix?" => __(
                ...
                "Could not find the specified string." => __(
                ...
                "Rich Text Area. Press ALT-F9 for menu. Press ALT-F10 for toolbar. Press ALT-0 for help" => __(
                    "Rich Text Area. Press Alt-Shift-H for help."
                ),
                "Rich Text Area. Press Control-Option-H for help." => __(
                    "Rich Text Area. Press Control-Option-H for help."
                ),
                "You have unsaved changes are you sure you want to navigate away?" => __(
                    "The changes you made will be lost if you navigate away from this page."
                ),
                'Your browser doesn\'t support direct access to the clipboard. Please use the Ctrl+X/C/V keyboard shortcuts instead.' => __(
                ...
                "Paste URL or type to search" => __(
                ...
                "Code|tab" => _x(
                    "Code",
                    "Name for the Code editor tab (formerly Text)"
                ), // Editor switch tab label.
                "Add Media" => [__("Add Media"), "accessM"], // Tooltip for the 'Add Media' button in the block editor Classic block.

                // Shortcuts help modal.
                "Keyboard Shortcuts" => [__("Keyboard Shortcuts"), "accessH"],
                "Classic Block Keyboard Shortcuts" => __(
                ...
                "Inline toolbar (when an image, link or preview is selected)" => __(
                    "Inline toolbar (when an image, link or preview is selected)"
                ),
                "Editor menu (when enabled)" => __(
                ...
                "Letter" => __("Letter"),
                "Action" => __("Action"),
                "Warning: the link has been inserted but may have errors. Please test it." => __(
                    "Warning: the link has been inserted but may have errors. Please test it."
                ),
                "To move focus to other buttons use Tab or the arrow keys. To return focus to the editor press Escape or use one of the buttons." => __(
                    "To move focus to other buttons use Tab or the arrow keys. To return focus to the editor press Escape or use one of the buttons."
                ),
                "When starting a new paragraph with one of these formatting shortcuts followed by a space, the formatting will be applied automatically. Press Backspace or Escape to undo." => __(
                    "When starting a new paragraph with one of these formatting shortcuts followed by a space, the formatting will be applied automatically. Press Backspace or Escape to undo."
                ),
                "The following formatting shortcuts are replaced when pressing Enter. Press Escape or the Undo button to undo." => __(
                    "The following formatting shortcuts are replaced when pressing Enter. Press Escape or the Undo button to undo."
                ),
                "The next group of formatting shortcuts are applied as you type or when you insert them around plain text in the same paragraph. Press Escape or the Undo button to undo." => __(
    ...
    public static function wp_mce_translation(
        ...
        foreach ($mce_translation as $key => $value) {
            ...
            if (str_contains($value, "&")) {
                $mce_translation[$key] = html_entity_decode(
                    $value,
                    ENT_QUOTES,
                    "UTF-8"
                );
        ...
        return "tinymce.addI18n( '$mce_locale', " .
  ...
  do_action("after_wp_tiny_mce", self::$mce_settings);
   ...
   _e("Close"); ?>
		...
		<div id="link-selector">
			<div id="link-options">
				<p class="howto" id="wplink-enter-url"><?php _e(
        "Enter the destination URL"
				...
				<div class="link-target">
					<label><span></span>
					<input type="checkbox" id="wp-link-target" /> <?php _e(
         "Open link in a new tab"
     ); ?></label>
				</div>
			</div>
			<p class="howto" id="wplink-link-existing-content"><?php _e(
			...
			<div id="search-panel">
				...
				<div id="most-recent-results" class="query-results" tabindex="0">
					<div class="query-notice" id="query-notice-message">
						<em class="query-notice-default"><?php _e(
