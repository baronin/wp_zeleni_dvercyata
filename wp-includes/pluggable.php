                    ...
                    switch (strtolower($name)) {
                        ...
                        case "content-type":
                            if (str_contains($content, ";")) {
                                list($type, $charset_content) = explode(
                                    ";",
                                    $content
                                );
                                ...
                                if (
                                    false !==
                                    stripos($charset_content, "charset=")
                                ) {
                                    $charset = trim(
                                        str_replace(
                                            ["charset=", '"'],
                                    ...
                                    false !==
                                    stripos($charset_content, "boundary=")
                                ) {
                                    $boundary = trim(
                                        str_replace(
                                            ["BOUNDARY=", "boundary=", '"'],
...
if (!function_exists("wp_authenticate")):
    ...
    function wp_authenticate($username, #[\SensitiveParameter] $password)
    {
        ...
        if (
            is_wp_error($user) &&
            !in_array($user->get_error_code(), $ignore_codes, true)
        ) {
            $error = $user;
...
if (!function_exists("wp_validate_auth_cookie")):
    ...
    function wp_validate_auth_cookie($cookie = "", $scheme = "")
    {
        ...
        if (
            str_starts_with($user->user_pass, '$P$') ||
            str_starts_with($user->user_pass, '$2y$')
        ) {
...
if (!function_exists("wp_generate_auth_cookie")):
    ...
    function wp_generate_auth_cookie(
        ...
        $cookie =
            $user->user_login . "|" . $expiration . "|" . $token . "|" . $hash;
...
if (!function_exists("wp_set_auth_cookie")):
    ...
    function wp_set_auth_cookie(
        ...
        $auth_cookie = wp_generate_auth_cookie(
            $user_id,
            $expiration,
            $scheme,
            $token
...
if (!function_exists("auth_redirect")):
    ...
    function auth_redirect()
    {
        ...
        if ($user_id) {
            ...
            if (
                !$secure &&
                get_user_option("use_ssl", $user_id) &&
                str_contains($_SERVER["REQUEST_URI"], "wp-admin")
            ) {
                if (str_starts_with($_SERVER["REQUEST_URI"], "http")) {
                    wp_redirect(
                        set_url_scheme($_SERVER["REQUEST_URI"], "https")
                    );
                    exit();
                } else {
                    wp_redirect(
        ...
        if (
            str_contains($_SERVER["REQUEST_URI"], "/options.php") &&
            wp_get_referer()
        ) {
            $redirect = wp_get_referer();
        } else {
            $redirect = set_url_scheme(
...
if (!function_exists("check_admin_referer")):
    ...
    function check_admin_referer($action = -1, $query_arg = "_wpnonce")
    {
        ...
        if (
            !$result &&
            !(-1 === $action && str_starts_with($referer, $adminurl))
        ) {
            wp_nonce_ays($action);
            die();
        }
...
if (!function_exists("wp_redirect")):
    ...
    function wp_redirect($location, $status = 302, $x_redirect_by = "WordPress")
    {
        ...
        if ($status < 300 || 399 < $status) {
            wp_die(
                __("HTTP redirect status code must be a redirection code, 3xx.")
            );
        }
...
if (!function_exists("wp_validate_redirect")):
    ...
    function wp_validate_redirect($location, $fallback_url = "")
    {
        ...
        if (
            isset($lp["scheme"]) &&
            !("http" === $lp["scheme"] || "https" === $lp["scheme"])
        ) {
            return $fallback_url;
        }
        ...
        if (
            ...
            if (!empty($_SERVER["REQUEST_URI"])) {
                $path = dirname(
                    parse_url(
                        "http://placeholder" . $_SERVER["REQUEST_URI"],
                        PHP_URL_PATH
                    ) . "?"
                );
        ...
        if (
            isset($lp["host"]) &&
            (!in_array($lp["host"], $allowed_hosts, true) &&
                strtolower($wpp["host"]) !== $lp["host"])
        ) {
...
if (!function_exists("wp_notify_postauthor")):
    ...
    function wp_notify_postauthor($comment_id, $deprecated = null)
    {
        ...
        if (
            $author &&
            !$notify_author &&
            !user_can($post->post_author, "read_post", $post->ID)
        ) {
            unset($emails[$author->user_email]);
        }
        ...
        foreach ($emails as $email) {
            ...
            switch ($comment->comment_type) {
                case "trackback":
                    ...
                    $notify_message =
                        sprintf(
                            __('New trackback on your post "%s"'),
                            $post->post_title
                        ) . "\r\n";
                    /* translators: 1: Trackback/pingback website name, 2: Website IP address, 3: Website hostname. */
                    $notify_message .=
                        sprintf(
                            __('Website: %1$s (IP address: %2$s, %3$s)'),
                    ...
                    $notify_message .=
                        sprintf(__("URL: %s"), $comment->comment_author_url) .
                        "\r\n";
                    ...
                    $notify_message .=
                        ...
                        "\r\n\r\n";
                    $notify_message .=
                        __("You can see all trackbacks on this post here:") .
                        "\r\n";
                    /* translators: Trackback notification email subject. 1: Site title, 2: Post title. */
                    $subject = sprintf(
                        __('[%1$s] Trackback: "%2$s"'),
                        $blogname,
                        $post->post_title
                    );
                    break;
                ...
                case "pingback":
                    ...
                    $notify_message =
                        sprintf(
                            __('New pingback on your post "%s"'),
                            $post->post_title
                        ) . "\r\n";
                    /* translators: 1: Trackback/pingback website name, 2: Website IP address, 3: Website hostname. */
                    $notify_message .=
                    ...
                    $notify_message .=
                        ...
                        "\r\n\r\n";
                    $notify_message .=
                        __("You can see all pingbacks on this post here:") .
                        "\r\n";
                    ...
                    if (
                        ...
                        $notify_message .=
                            sprintf(
                                __("In reply to: %s"),
                                admin_url(
                                    "comment.php?action=editcomment&c={$comment->comment_parent}#wpbody-content"
                                )
                            ) . "\r\n";
                    }
            ...
            $notify_message .=
                sprintf(__("Permalink: %s"), get_comment_link($comment)) .
                "\r\n";
                ...
                if (EMPTY_TRASH_DAYS) {
                    ...
                    $notify_message .=
                        sprintf(
                            __("Trash it: %s"),
                            admin_url(
                                "comment.php?action=trash&c={$comment->comment_ID}#wpbody-content"
                            )
                        ) . "\r\n";
                } else {
                    /* translators: Comment moderation. %s: Comment action URL. */
                    $notify_message .=
                        sprintf(
                            __("Delete it: %s"),
                            admin_url(
                                "comment.php?action=delete&c={$comment->comment_ID}#wpbody-content"
                            )
                        ) . "\r\n";
                }
                /* translators: Comment moderation. %s: Comment action URL. */
        ...
        foreach ($emails as $email) {
            ...
            $notify_message .=
                sprintf(
                    __("Approve it: %s"),
                    admin_url(
                        "comment.php?action=approve&c={$comment_id}#wpbody-content"
                    )
                ) . "\r\n";
            ...
            if (EMPTY_TRASH_DAYS) {
                ...
                $notify_message .=
                    sprintf(
                        __("Trash it: %s"),
                        admin_url(
                            "comment.php?action=trash&c={$comment_id}#wpbody-content"
                        )
                    ) . "\r\n";
            } else {
                /* translators: Comment moderation. %s: Comment action URL. */
                $notify_message .=
                    sprintf(
                        __("Delete it: %s"),
                        admin_url(
                            "comment.php?action=delete&c={$comment_id}#wpbody-content"
                        )
                    ) . "\r\n";
            }
            ...
            $notify_message .=
                sprintf(
                    __("Spam it: %s"),
                    admin_url(
                        "comment.php?action=spam&c={$comment_id}#wpbody-content"
                    )
                ) . "\r\n";
            ...
            $notify_message .=
                admin_url(
                    "edit-comments.php?comment_status=moderated#wpbody-content"
                ) . "\r\n";
...
if (!function_exists("wp_password_change_notification")):
    ...
    function wp_password_change_notification($user)
    {
        ...
        if (0 !== strcasecmp($user->user_email, get_option("admin_email"))) {
            ...
            $message =
                sprintf(
                    __("Password changed for user: %s"),
                    $user->user_login
                ) . "\r\n";
            /*
             * The blogname option is escaped with esc_html() on the way into the database in sanitize_option().
             ...
             */
            $wp_password_change_notification_email = apply_filters(
                "wp_password_change_notification_email",
                $wp_password_change_notification_email,
                $user,
                $blogname
            ...
            wp_mail(
                ...
                wp_specialchars_decode(
                    sprintf(
                        $wp_password_change_notification_email["subject"],
                        $blogname
                    )
                ),
...
if (!function_exists("wp_new_user_notification")):
    ...
    function wp_new_user_notification(
        ...
        if ("user" !== $notify && true === $send_notification_to_admin) {
            ...
            $message =
                sprintf(
                    __("New user registration on your site %s:"),
                    $blogname
                ) . "\r\n\r\n";
             ...
             * @param string  $blogname The site title.
             */
            $wp_new_user_notification_email_admin = apply_filters(
                "wp_new_user_notification_email_admin",
                $wp_new_user_notification_email_admin,
                $user,
                $blogname
            ...
            wp_mail(
                ...
                wp_specialchars_decode(
                    sprintf(
                        $wp_new_user_notification_email_admin["subject"],
                        $blogname
                    )
                ),
         ...
         * @param WP_User $user User object for new user.
         */
        $send_notification_to_user = apply_filters(
        ...
        if (
            "admin" === $notify ||
            true !== $send_notification_to_user ||
        ...
        wp_mail(
            ...
            wp_specialchars_decode(
                sprintf($wp_new_user_notification_email["subject"], $blogname)
            ),
...
if (!function_exists("wp_salt")):
    ...
    function wp_salt($scheme = "auth")
    {
        ...
        if (null === $duplicated_keys) {
            ...
            foreach (
                ["AUTH", "SECURE_AUTH", "LOGGED_IN", "NONCE", "SECRET"]
                as $first
            ) {
        ...
        foreach (["auth", "secure_auth", "logged_in", "nonce"] as $key) {
            foreach (["key", "salt"] as $second) {
                ...
                if (
                    !defined($const) ||
                    true === $duplicated_keys[constant($const)]
                ) {
        ...
        if (
            defined("SECRET_KEY") &&
            SECRET_KEY &&
            empty($duplicated_keys[SECRET_KEY])
        ) {
        ...
        if (
            ...
            defined("SECRET_SALT") &&
            SECRET_SALT &&
            empty($duplicated_keys[SECRET_SALT])
        ) {
        ...
        if (
            ...
            foreach (["key", "salt"] as $type) {
                ...
                if (
                    defined($const) &&
                    constant($const) &&
                    empty($duplicated_keys[constant($const)])
                ) {
...
if (!function_exists("wp_hash_password")):
    ...
    function wp_hash_password(#[\SensitiveParameter] $password)
    {
         ...
         */
        $options = apply_filters("wp_hash_password_options", [], $algorithm);
...
if (!function_exists("wp_check_password")):
    ...
    function wp_check_password(
        ...
        if (strlen($hash) <= 32) {
            ...
            $password_to_verify = base64_encode(
            ...
            $check = (new PasswordHash(8, true))->CheckPassword(
                $password,
                $hash
            );
        } else {
...
if (!function_exists("wp_rand")):
    ...
    function wp_rand($min = null, $max = null)
    {
         ...
         */
        $max_random_number =
            3000000000 === 2147483647 ? (float) "4294967295" : 4294967295; // 4294967295 = 0xffffffff
...
if (!function_exists("get_avatar")):
    ...
    function get_avatar(
        ...
        $url2x = get_avatar_url(
            $id_or_email,
            array_merge($args, ["size" => $args["size"] * 2])
        );
        ...
        if (
            in_array($args["loading"], ["lazy", "eager"], true) &&
        ...
        if (
            in_array($args["fetchpriority"], ["high", "low", "auto"], true) &&
        ...
        if (
            in_array($args["decoding"], ["async", "sync", "auto"], true) &&
