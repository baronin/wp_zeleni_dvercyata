...
final class WP_Font_Collection
{
    ...
    public function __construct(string $slug, array $args)
    {
        ...
        if (
            isset($args["font_families"]) &&
            is_string($args["font_families"])
        ) {
        ...
        $this->data = $this->sanitize_and_validate_data(
            $args,
            $required_properties
        );
    }
    ...
    private function load_from_json($file_or_url)
    {
        ...
        $file = file_exists($file_or_url)
            ? wp_normalize_path(realpath($file_or_url))
            : false;
