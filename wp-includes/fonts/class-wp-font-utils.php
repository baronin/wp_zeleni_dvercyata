...
class WP_Font_Utils
{
    ...
    public static function get_font_face_slug($settings)
    {
        ...
        $font_stretch = str_replace(
            array_keys($font_stretch_map),
            array_values($font_stretch_map),
            $font_stretch
        );
        ...
        $slug_elements = array_map(function ($elem) {
    ...
    public static function sanitize_from_schema($tree, $schema)
    {
        ...
        foreach ($tree as $key => $value) {
            ...
            $is_schema_array =
                is_array($schema[$key]) && !is_callable($schema[$key]);
            ...
            if ($is_value_array && $is_schema_array) {
                if (wp_is_numeric_array($value)) {
                    ...
                    foreach ($value as $item_key => $item_value) {
                        $tree[$key][$item_key] =
                            ...
                            is_array($schema[$key][0])
                                ? self::sanitize_from_schema(
                                    $item_value,
                                    $schema[$key][0]
                                )
                                : self::apply_sanitizer(
                                    $item_value,
                                    $schema[$key][0]
                                );
                    }
                    ...
                    $tree[$key] = self::sanitize_from_schema(
                        $value,
                        $schema[$key]
                    );
                }
    ...
    public static function get_allowed_font_mime_types()
    {
        ...
        return [
            ...
            "ttf" =>
                PHP_VERSION_ID >= 70400 ? "font/sfnt" : $php_7_ttf_mime_type,
            "woff" =>
                PHP_VERSION_ID >= 80112 ? "font/woff" : "application/font-woff",
            "woff2" =>
