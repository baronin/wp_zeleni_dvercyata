...
class WP_Font_Face_Resolver
{
    ...
    private static function parse_settings(array $settings)
    {
        ...
        foreach ($settings["typography"]["fontFamilies"] as $font_families) {
            foreach ($font_families as $definition) {
                ...
                $font_family_name = static::maybe_parse_name_from_comma_separated_list(
                ...
                $fonts[] = static::convert_font_face_properties(
                    $definition["fontFace"],
                    $font_family_name
                );
            }
     ...
     */
    private static function maybe_parse_name_from_comma_separated_list(
        $font_family
    ) {
    ...
    private static function convert_font_face_properties(
        ...
        foreach ($font_face_definition as $font_face) {
            ...
            if (!empty($font_face["src"])) {
                $font_face["src"] = static::to_theme_file_uri(
                    (array) $font_face["src"]
                );
            }
