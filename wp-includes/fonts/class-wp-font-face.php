...
class WP_Font_Face
{
    ...
    private function validate_font_face_declarations(array $font_face)
    {
        ...
        if (
            empty($font_face["font-family"]) ||
            !is_string($font_face["font-family"])
        ) {
        ...
        if (
            empty($font_face["src"]) ||
            (!is_string($font_face["src"]) && !is_array($font_face["src"]))
        ) {
            // @todo replace with `wp_trigger_error()`.
            _doing_it_wrong(
                __METHOD__,
        ...
        if (
            !is_string($font_face["font-weight"]) &&
            !is_int($font_face["font-weight"])
        ) {
            // @todo replace with `wp_trigger_error()`.
            _doing_it_wrong(
                __METHOD__,
        ...
        if (
            !in_array(
                $font_face["font-display"],
                $this->valid_font_display,
                true
            )
        ) {
            $font_face["font-display"] =
                $this->font_face_property_defaults["font-display"];
        }
    ...
    private function get_css($font_faces)
    {
        ...
        foreach ($font_faces as $font_face) {
            ...
            $css .=
                "@font-face{" .
                $this->build_font_face_css($font_face) .
                "}" .
                "\n";
        }
