...
final class WP_Block_Templates_Registry
{
    ...
    public function register($template_name, $args = [])
    {
        ...
        if (!is_string($template_name)) {
            ...
            !preg_match('/^[a-z0-9_\-]+\/\/[a-z0-9_\-]+$/', $template_name)
        ) {
            $error_message = __(
            ...
            $error_message = sprintf(
                __('Template "%s" is already registered.'),
                $template_name
            );
        ...
        if (!$template) {
            ...
            $template->content = isset($args["content"])
                ? $args["content"]
                : "";
            $template->source = "plugin";
            $template->slug = $slug;
            $template->type = "wp_template";
            $template->title = isset($args["title"])
                ? $args["title"]
                : $template_name;
            $template->description = isset($args["description"])
                ? $args["description"]
                : "";
            $template->status = "publish";
            $template->origin = "plugin";
            $template->is_custom = !isset(
                $default_template_types[$template_name]
            );
            $template->post_types = isset($args["post_types"])
                ? $args["post_types"]
                : [];
        }
    ...
    public function get_by_query($query = [])
    {
        ...
        foreach ($all_templates as $template_name => $template) {
            if (
                $slugs_to_include &&
                !in_array($template->slug, $slugs_to_include, true)
            ) {
                continue;
            }
            ...
            if (
                $slugs_to_skip &&
                in_array($template->slug, $slugs_to_skip, true)
            ) {
                continue;
            }
    ...
    public function unregister($template_name)
    {
        if (!$this->is_registered($template_name)) {
            ...
            return new WP_Error(
                ...
                __('Template "%s" is not registered.')
            );
        }
