...
class wpdb
{
    ...
    public function prepare($query, ...$args)
    {
        ...
        $passed_as_array =
            isset($args[0]) && is_array($args[0]) && 1 === count($args);
        if ($passed_as_array) {
            $args = $args[0];
        }
        ...
        while ($key < $split_query_count) {
            ...
            if (
                ...
                if ("i" === $type) {
                    ...
                    if (false !== $argnum_pos) {
                        ...
                        $arg_identifiers[] =
                            ((int) substr($format, 0, $argnum_pos)) - 1;
                    } else {
            ...
            $new_query .=
                $split_query[$key - 2] . $split_query[$key - 1] . $placeholder;
    ...
    public function check_connection($allow_bail = true)
    {
        ...
        $message =
            "<h1>" . __("Error reconnecting to the database") . "</h1>\n";
    ...
    public function query($query)
    {
        ...
        if ($this->last_error) {
            ...
            if (
                $this->insert_id &&
                preg_match("/^\s*(insert|replace)\s/i", $query)
            ) {
                $this->insert_id = 0;
            }
    ...
    public function placeholder_escape()
    {
        ...
        if (!$placeholder) {
            ...
            $salt =
                defined("AUTH_SALT") && AUTH_SALT ? AUTH_SALT : (string) rand();

            $placeholder =
                "{" . hash_hmac("sha256", uniqid($salt, true), $salt) . "}";
        }
         ...
         */
        if (
            false === has_filter("query", [$this, "remove_placeholder_escape"])
        ) {
     ...
     * @return int|false The number of rows affected, or false on error.
     */
    public function _insert_replace_helper(
        $table,
        $data,
        $format = null,
        $type = "INSERT"
    ...
    protected function get_table_charset($table)
    {
        ...
        if (!$results) {
            return new WP_Error(
                ...
                __("Could not retrieve table charset.")
            );
        }
    ...
    public function get_col_charset($table, $column)
    {
        ...
        list($charset) = explode(
            "_",
            $this->col_meta[$tablekey][$columnkey]->Collation
        );
        return $charset;
    }
    ...
    protected function strip_invalid_text($data)
    {
        ...
        foreach ($data as &$value) {
            ...
            if (
                ...
                if (
                    false !== $length &&
                    mb_strlen($value["value"], "UTF-8") > $length
                ) {
                    $value["value"] = mb_substr(
                        $value["value"],
                        0,
                        $length,
                        "UTF-8"
        ...
        if ($db_check_string) {
            ...
            foreach ($data as $col => $value) {
                if (!empty($value["db"])) {
                    ...
                    if (
                        isset($value["length"]["type"]) &&
                        "byte" === $value["length"]["type"]
                    ) {
                    ...
                    if ($this->charset) {
                        ...
                        $connection_charset = mysqli_character_set_name(
                            $this->dbh
                        );
                    }
            ...
            if (!$row) {
                return new WP_Error(
                    "wpdb_strip_invalid_text_failure",
                    __("Could not strip invalid text.")
                );
            }
    ...
    protected function strip_invalid_text_from_query($query)
    {
        ...
        if (
            preg_match(
                "/^(?:SHOW|DESCRIBE|DESC|EXPLAIN|CREATE)\s/i",
                $trimmed_query
            )
        ) {
            return $query;
    ...
    public function check_database_version()
    {
        ...
        if (
            version_compare($this->db_version(), $required_mysql_version, "<")
        ) {
