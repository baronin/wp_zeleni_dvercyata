...
function wp_register_dimensions_support($block_type)
{
    ...
    $has_dimensions_support = block_has_support(
        $block_type,
        "dimensions",
        false
    );
...
function wp_apply_dimensions_support($block_type, $block_attributes)
{
    ...
    $has_min_height_support = block_has_support(
        $block_type,
        ["dimensions", "minHeight"],
        false
    );
    $block_styles = isset($block_attributes["style"])
    ...
    $skip_min_height = wp_should_skip_block_supports_serialization(
        $block_type,
        "dimensions",
        "minHeight"
    );
    ...
    if ($has_min_height_support && !$skip_min_height) {
        $dimensions_block_styles["minHeight"] = isset(
    ...
    $styles = wp_style_engine_get_styles([
        "dimensions" => $dimensions_block_styles,
    ]);
...
function wp_render_dimensions_support($block_content, $block)
{
    ...
    $block_attributes =
        isset($block["attrs"]) && is_array($block["attrs"])
            ? $block["attrs"]
            : [];
    $has_aspect_ratio_support = block_has_support(
        $block_type,
    ...
    $dimensions_block_styles = [];
    $dimensions_block_styles["aspectRatio"] =
        $block_attributes["style"]["dimensions"]["aspectRatio"] ?? null;
            ...
            if (!empty($styles["classnames"])) {
                foreach (explode(" ", $styles["classnames"]) as $class_name) {
                    if (
                        ...
                        !isset(
                            $block_attributes["style"]["dimensions"][
                                "aspectRatio"
                            ]
                        )
                    ) {
