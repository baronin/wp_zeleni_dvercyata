    ...
    if ("default" === $layout_type) {
        if ($has_block_gap_support) {
            ...
            if (null !== $gap_value && !$should_skip_gap_serialization) {
                ...
                if (
                    is_string($gap_value) &&
                    str_contains($gap_value, "var:preset|spacing|")
                ) {
                    $index_to_splice = strrpos($gap_value, "|") + 1;
                    $slug = _wp_to_kebab_case(
                        substr($gap_value, $index_to_splice)
                    );
        ...
        $content_size = isset($layout["contentSize"])
            ? $layout["contentSize"]
            : "";
        $wide_size = isset($layout["wideSize"]) ? $layout["wideSize"] : "";
        $justify_content = isset($layout["justifyContent"])
        ...
        $wide_max_width_value = safecss_filter_attr(
            explode(";", $wide_max_width_value)[0]
        );

        $margin_left =
            "left" === $justify_content ? "0 !important" : "auto !important";
        $margin_right =
            "right" === $justify_content ? "0 !important" : "auto !important";
        ...
        if (isset($block_spacing)) {
            ...
            if (isset($block_spacing_values["declarations"]["padding-right"])) {
                $padding_right =
                    $block_spacing_values["declarations"]["padding-right"];
                // Add unit if 0.
                if ("0" === $padding_right) {
                    $padding_right = "0px";
                }
                $layout_styles[] = [
                    "selector" => "$selector > .alignfull",
                    "declarations" => [
                        "margin-right" => "calc($padding_right * -1)",
                    ],
                ];
            }
            if (isset($block_spacing_values["declarations"]["padding-left"])) {
                ...
                $layout_styles[] = [
                    "selector" => "$selector > .alignfull",
                    "declarations" => [
                        "margin-left" => "calc($padding_left * -1)",
                    ],
                ];
            }
        }
        ...
        $layout_orientation = isset($layout["orientation"])
            ? $layout["orientation"]
            : "horizontal";
        ...
        if ($has_block_gap_support && isset($gap_value)) {
            ...
            foreach ($gap_sides as $gap_side) {
                ...
                if (
                    ...
                    $slug = _wp_to_kebab_case(
                        substr($process_value, $index_to_splice)
                    );
        ...
        if ("horizontal" === $layout_orientation) {
            ...
            if (
                ...
                $layout_styles[] = [
                    ...
                    "declarations" => [
                        "justify-content" =>
                            $justify_content_options[$layout["justifyContent"]],
                    ],
                ];
            }

            if (
                !empty($layout["verticalAlignment"]) &&
                array_key_exists(
        ...
        if (!empty($layout["columnCount"])) {
            ...
            $minimum_column_width = !empty($layout["minimumColumnWidth"])
...
function wp_render_layout_support_flag($block_content, $block)
{
    ...
    $block_supports_layout =
        block_has_support($block_type, "layout", false) ||
        block_has_support($block_type, "__experimentalLayout", false);
    $child_layout = isset($block["attrs"]["style"]["layout"])
    ...
    if ($child_layout) {
        ...
        $container_content_class = wp_unique_id_from_values(
            [
                ...
                "parentLayout" => array_intersect_key(
                    $block["parentLayout"] ?? [],
                    array_flip(["minimumColumnWidth", "columnCount"])
                ),
            ],
        ...
        $self_stretch = isset($child_layout["selfStretch"])
            ? $child_layout["selfStretch"]
            : null;

        if ("fixed" === $self_stretch && isset($child_layout["flexSize"])) {
            $child_layout_declarations["flex-basis"] =
                $child_layout["flexSize"];
        ...
        if (
            ...
            $container_query_value =
                $column_span_number * $parent_column_value +
                ($column_span_number - 1) * $default_gap_value;
            $container_query_value =
                $container_query_value . $parent_column_unit;
    ...
    if (empty($fallback_layout)) {
        $fallback_layout = isset(
            $block_type->supports["__experimentalLayout"]["default"]
        )
            ? $block_type->supports["__experimentalLayout"]["default"]
            : [];
    }
    $used_layout = isset($block["attrs"]["layout"])
        ? $block["attrs"]["layout"]
        : $fallback_layout;
    ...
    if (
        (isset($used_layout["inherit"]) && $used_layout["inherit"]) ||
        (isset($used_layout["contentSize"]) && $used_layout["contentSize"])
    ) {
        $used_layout["type"] = "constrained";
    }

    $root_padding_aware_alignments = isset(
    ...
    if (!empty($block["attrs"]["layout"]["orientation"])) {
        $class_names[] =
            "is-" . sanitize_title($block["attrs"]["layout"]["orientation"]);
    }

    if (!empty($block["attrs"]["layout"]["justifyContent"])) {
        $class_names[] =
            "is-content-justification-" .
            sanitize_title($block["attrs"]["layout"]["justifyContent"]);
    }

    if (
        !empty($block["attrs"]["layout"]["flexWrap"]) &&
        "nowrap" === $block["attrs"]["layout"]["flexWrap"]
    ) {
    ...
    if (!current_theme_supports("disable-layout-styles")) {
        ...
        if (is_array($gap_value)) {
            ...
            $gap_value =
                $gap_value && preg_match("%[\\\(&=}]|/\*%", $gap_value)
                    ? null
                    : $gap_value;
        }

        $fallback_gap_value = isset(
         ...
         */
        $should_skip_gap_serialization = wp_should_skip_block_supports_serialization(
            $block_type,
            "spacing",
            "blockGap"
        );
    ...
    $first_chunk = isset($block["innerContent"][0])
    ...
    do {
        ...
        if (
            ...
            str_contains($class_attribute, $inner_block_wrapper_classes)
        ) {
            break;
        }
 ...
 */
function wp_add_parent_layout_to_parsed_block(
    $parsed_block,
    $source_block,
    $parent_block
) {
    if (
        $parent_block &&
        isset($parent_block->parsed_block["attrs"]["layout"])
    ) {
        $parsed_block["parentLayout"] =
            $parent_block->parsed_block["attrs"]["layout"];
    }
    return $parsed_block;
}
...
{
    $tag_name = isset($block["attrs"]["tagName"])
        ? $block["attrs"]["tagName"]
        : "div";
    ...
    if (!empty($layout_classes)) {
        ...
        if (
            $processor->next_tag([
                "class_name" => "wp-block-group__inner-container",
            ])
        ) {
...
(
    ...
    $filtered_content_classnames = array_diff(
        $content_classnames,
        $wrapper_classnames
    );

    return '<div class="' .
        implode(" ", $wrapper_classnames) .
        '">' .
        $matches[1] .
        implode(" ", $filtered_content_classnames) .
        $matches[3] .
        "</div>";
}
