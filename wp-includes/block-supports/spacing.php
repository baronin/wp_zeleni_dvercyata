...
function wp_apply_spacing_support($block_type, $block_attributes)
{
    ...
    $attributes = [];
    $has_padding_support = block_has_support(
        $block_type,
        ["spacing", "padding"],
        false
    );
    $has_margin_support = block_has_support(
        $block_type,
        ["spacing", "margin"],
        false
    );
    ...
    $skip_padding = wp_should_skip_block_supports_serialization(
        $block_type,
        "spacing",
        "padding"
    );
    $skip_margin = wp_should_skip_block_supports_serialization(
        $block_type,
        "spacing",
        "margin"
    );
    ...
    if ($has_padding_support && !$skip_padding) {
        $spacing_block_styles["padding"] = isset(
    ...
    if ($has_margin_support && !$skip_margin) {
        $spacing_block_styles["margin"] = isset(
