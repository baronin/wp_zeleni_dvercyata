 ...
 */
function wp_resolve_block_style_variation_ref_values(
    &$variation_data,
    $theme_json
) {
...
function wp_render_block_style_variation_support_styles($parsed_block)
{
    ...
    foreach ($variations as $variation) {
        $variation_data =
            $theme_json["styles"]["blocks"][$parsed_block["blockName"]][
                "variations"
            ][$variation] ?? [];
     ...
     */
    $elements_data = $variation_data["elements"] ?? [];
    $blocks_data = $variation_data["blocks"] ?? [];
    unset($variation_data["elements"]);
    unset($variation_data["blocks"]);

    _wp_array_set(
        $blocks_data,
        [$parsed_block["blockName"], "variations", $variation_instance],
        $variation_data
    );
    ...
    $styles_registry->register($parsed_block["blockName"], [
        "name" => $variation_instance,
    ]);
...
WP_Block_Supports::get_instance()->register("block-style-variation", []);
...
function wp_register_block_style_variations_from_theme_json_partials(
    ...
    foreach ($variations as $variation) {
        ...
        foreach ($variation["blockTypes"] as $block_type) {
            $registered_styles = $registry->get_registered_styles_for_block(
                $block_type
            );
