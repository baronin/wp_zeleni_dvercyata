...
function wp_should_add_elements_class_name($block, $options)
{
    ...
    $element_color_properties = [
        "button" => [
            "skip" => isset($options["button"]["skip"])
                ? $options["button"]["skip"]
                : false,
            "paths" => [
                ["button", "color", "text"],
        ...
        "heading" => [
            "skip" => isset($options["heading"]["skip"])
                ? $options["heading"]["skip"]
                : false,
            "paths" => [
                ["heading", "color", "text"],
    ...
    foreach ($element_color_properties as $element_config) {
        ...
        foreach ($element_config["paths"] as $path) {
            if (
                null !== _wp_array_get($elements_style_attributes, $path, null)
            ) {
                return true;
            }
        }
...
function wp_render_elements_support_styles($parsed_block)
{
    ...
    $element_block_styles = isset($parsed_block["attrs"]["style"]["elements"])
        ? $parsed_block["attrs"]["style"]["elements"]
        : null;
    ...
    $skip_link_color_serialization = wp_should_skip_block_supports_serialization(
        $block_type,
        "color",
        "link"
    );
    $skip_heading_color_serialization = wp_should_skip_block_supports_serialization(
        $block_type,
        "color",
        "heading"
    );
    $skip_button_color_serialization = wp_should_skip_block_supports_serialization(
        $block_type,
        "color",
        "button"
    );
    ...
    foreach ($element_types as $element_type => $element_config) {
        ...
        $element_style_object = isset($element_block_styles[$element_type])
            ? $element_block_styles[$element_type]
            : null;
        ...
        if ($element_style_object) {
            ...
            if (isset($element_style_object[":hover"])) {
                wp_style_engine_get_styles($element_style_object[":hover"], [
