...
function wp_register_typography_support($block_type)
{
    ...
    $has_font_family_support = isset(
        $typography_supports["__experimentalFontFamily"]
    )
        ? $typography_supports["__experimentalFontFamily"]
        : false;
    $has_font_size_support = isset($typography_supports["fontSize"])
        ? $typography_supports["fontSize"]
        : false;
    $has_font_style_support = isset(
        $typography_supports["__experimentalFontStyle"]
    )
        ? $typography_supports["__experimentalFontStyle"]
        : false;
    $has_font_weight_support = isset(
        $typography_supports["__experimentalFontWeight"]
    )
        ? $typography_supports["__experimentalFontWeight"]
        : false;
    $has_letter_spacing_support = isset(
        $typography_supports["__experimentalLetterSpacing"]
    )
        ? $typography_supports["__experimentalLetterSpacing"]
        : false;
    $has_line_height_support = isset($typography_supports["lineHeight"])
        ? $typography_supports["lineHeight"]
        : false;
    $has_text_align_support = isset($typography_supports["textAlign"])
        ? $typography_supports["textAlign"]
        : false;
    $has_text_columns_support = isset($typography_supports["textColumns"])
        ? $typography_supports["textColumns"]
        : false;
    $has_text_decoration_support = isset(
        $typography_supports["__experimentalTextDecoration"]
    )
        ? $typography_supports["__experimentalTextDecoration"]
        : false;
    $has_text_transform_support = isset(
        $typography_supports["__experimentalTextTransform"]
    )
        ? $typography_supports["__experimentalTextTransform"]
        : false;
    $has_writing_mode_support = isset(
        $typography_supports["__experimentalWritingMode"]
    )
        ? $typography_supports["__experimentalWritingMode"]
        : false;
    ...
    if (
        $has_font_size_support &&
        !array_key_exists("fontSize", $block_type->attributes)
    ) {
    ...
    if (
        $has_font_family_support &&
        !array_key_exists("fontFamily", $block_type->attributes)
    ) {
...
function wp_apply_typography_support($block_type, $block_attributes)
{
    ...
    $should_skip_font_family = wp_should_skip_block_supports_serialization(
        $block_type,
        "typography",
        "fontFamily"
    );
    $should_skip_font_style = wp_should_skip_block_supports_serialization(
        $block_type,
        "typography",
        "fontStyle"
    );
    $should_skip_font_weight = wp_should_skip_block_supports_serialization(
        $block_type,
        "typography",
        "fontWeight"
    );
    $should_skip_line_height = wp_should_skip_block_supports_serialization(
        $block_type,
        "typography",
        "lineHeight"
    );
    $should_skip_text_align = wp_should_skip_block_supports_serialization(
        $block_type,
        "typography",
        "textAlign"
    );
    $should_skip_text_columns = wp_should_skip_block_supports_serialization(
        $block_type,
        "typography",
        "textColumns"
    );
    $should_skip_text_decoration = wp_should_skip_block_supports_serialization(
        $block_type,
        "typography",
        "textDecoration"
    );
    $should_skip_text_transform = wp_should_skip_block_supports_serialization(
        $block_type,
        "typography",
        "textTransform"
    );
    $should_skip_letter_spacing = wp_should_skip_block_supports_serialization(
        $block_type,
        "typography",
        "letterSpacing"
    );
    $should_skip_writing_mode = wp_should_skip_block_supports_serialization(
        $block_type,
        "typography",
        "writingMode"
    );
    ...
    if ($has_font_size_support && !$should_skip_font_size) {
        $preset_font_size = array_key_exists("fontSize", $block_attributes)
            ...
            : null;
        $custom_font_size = isset(
            $block_attributes["style"]["typography"]["fontSize"]
        )
            ? $block_attributes["style"]["typography"]["fontSize"]
            : null;
        $typography_block_styles["fontSize"] = $preset_font_size
            ? $preset_font_size
    ...
    if ($has_font_family_support && !$should_skip_font_family) {
        $preset_font_family = array_key_exists("fontFamily", $block_attributes)
            ...
            : null;
        $custom_font_family = isset(
            $block_attributes["style"]["typography"]["fontFamily"]
        )
            ...
            : null;
        $typography_block_styles["fontFamily"] = $preset_font_family
            ? $preset_font_family
            : $custom_font_family;
    }
    ...
    if ($has_line_height_support && !$should_skip_line_height) {
        $typography_block_styles["lineHeight"] = isset(
    ...
    if ($has_text_align_support && !$should_skip_text_align) {
        $typography_block_styles["textAlign"] = isset(
    ...
    if (
        ...
        isset($block_attributes["style"]["typography"]["textColumns"])
    ) {
        $typography_block_styles["textColumns"] = isset(
    ...
    if (
        ...
        $typography_block_styles["writingMode"] = isset(
    ...
    if (
        $has_text_align_support &&
        !$should_skip_text_align &&
        isset($block_attributes["style"]["typography"]["textAlign"])
    ) {
        $classnames[] =
            "has-text-align-" .
            $block_attributes["style"]["typography"]["textAlign"];
...
function wp_typography_get_preset_inline_style_value(
    ...
    if (
        empty($style_value) ||
        !str_contains($style_value, "var:preset|{$css_property}|")
    ) {
        return $style_value;
    }
...
function wp_render_typography_support($block_content, $block)
{
    ...
    $fluid_font_size = wp_get_typography_font_size_value([
        "size" => $custom_font_size,
    ]);
...
{
    $maximum_viewport_width_raw = isset($args["maximum_viewport_width"])
        ? $args["maximum_viewport_width"]
        : null;
    $minimum_viewport_width_raw = isset($args["minimum_viewport_width"])
        ? $args["minimum_viewport_width"]
        : null;
    $maximum_font_size_raw = isset($args["maximum_font_size"])
        ? $args["maximum_font_size"]
        : null;
    $minimum_font_size_raw = isset($args["minimum_font_size"])
        ? $args["minimum_font_size"]
        : null;
    $scale_factor = isset($args["scale_factor"]) ? $args["scale_factor"] : null;

    // Normalizes the minimum font size in order to use the value for calculations.
    $minimum_font_size = wp_get_typography_value_and_unit(
     ...
     */
    $font_size_unit = isset($minimum_font_size["unit"])
        ? $minimum_font_size["unit"]
        : "rem";
    ...
    $linear_factor_denominator =
        $maximum_viewport_width["value"] - $minimum_viewport_width["value"];
    ...
    $view_port_width_offset =
        round($minimum_viewport_width["value"] / 100, 3) . $font_size_unit;
    $linear_factor =
        100 *
        (($maximum_font_size["value"] - $minimum_font_size["value"]) /
            $linear_factor_denominator);
    $linear_factor_scaled = round($linear_factor * $scale_factor, 3);
    $linear_factor_scaled = empty($linear_factor_scaled)
        ? 1
...
function wp_get_typography_font_size_value($preset, $settings = [])
{
     ...
     */
    if (
        empty($typography_settings["fluid"]) &&
        empty($fluid_font_size_settings)
    ) {
        return $preset["size"];
    }

    $fluid_settings = isset($typography_settings["fluid"])
    ...
    // Defaults overrides.
    $minimum_viewport_width = isset($fluid_settings["minViewportWidth"])
        ? $fluid_settings["minViewportWidth"]
        : $default_minimum_viewport_width;
    $maximum_viewport_width =
        isset($layout_settings["wideSize"]) &&
        !empty(wp_get_typography_value_and_unit($layout_settings["wideSize"]))
    ...
    $has_min_font_size =
        isset($fluid_settings["minFontSize"]) &&
        !empty(
            wp_get_typography_value_and_unit($fluid_settings["minFontSize"])
        );
    $minimum_font_size_limit = $has_min_font_size
        ? $fluid_settings["minFontSize"]
        : $default_minimum_font_size_limit;

    // Try to grab explicit min and max fluid font sizes.
    $minimum_font_size_raw = isset($fluid_font_size_settings["min"])
        ? $fluid_font_size_settings["min"]
        : null;
    $maximum_font_size_raw = isset($fluid_font_size_settings["max"])
        ? $fluid_font_size_settings["max"]
        : null;
    ...
    if (!$maximum_font_size_raw) {
        $maximum_font_size_raw =
            $preferred_size["value"] . $preferred_size["unit"];
    }
    ...
    if (!$minimum_font_size_raw) {
        $preferred_font_size_in_px =
            "px" === $preferred_size["unit"]
                ? $preferred_size["value"]
                : $preferred_size["value"] * 16;
         ...
         */
        $minimum_font_size_factor = min(
            max(
                1 - 0.075 * log($preferred_font_size_in_px, 2),
                $default_minimum_font_size_factor_min
            ),
            $default_minimum_font_size_factor_max
        );
        $calculated_minimum_font_size = round(
            $preferred_size["value"] * $minimum_font_size_factor,
            3
        );
        ...
        if (
            !empty($minimum_font_size_limit) &&
            $calculated_minimum_font_size <= $minimum_font_size_limit["value"]
        ) {
            $minimum_font_size_raw =
                $minimum_font_size_limit["value"] .
                $minimum_font_size_limit["unit"];
        } else {
            $minimum_font_size_raw =
