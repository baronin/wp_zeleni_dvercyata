...
function get_block_categories($post_or_block_editor_context)
{
    ...
    $block_editor_context =
        $post_or_block_editor_context instanceof WP_Post
    ...
    if (!empty($block_editor_context->post)) {
         ...
         */
        $block_categories = apply_filters_deprecated(
            "block_categories",
            [$block_categories, $post],
            "5.8.0",
            "block_categories_all"
        );
    }
...
function get_allowed_block_types($block_editor_context)
{
    ...
    if (!empty($block_editor_context->post)) {
         ...
         */
        $allowed_block_types = apply_filters_deprecated(
            "allowed_block_types",
            [$allowed_block_types, $post],
            "5.8.0",
            "allowed_block_types_all"
        );
    }
...
function get_default_block_editor_settings()
{
    ...
    $image_default_size = in_array(
        $default_size,
        array_keys($image_size_names),
        true
    )
        ? $default_size
        : "large";
    ...
    if (
        ...
        $default_editor_styles_file_contents = file_get_contents(
            $default_editor_styles_file
        );
    }
...
function wp_get_first_block($blocks, $block_name)
{
    foreach ($blocks as $block) {
        ...
        if (!empty($block["innerBlocks"])) {
            $found_block = wp_get_first_block(
                $block["innerBlocks"],
                $block_name
            );
...
function wp_get_post_content_block_attributes()
{
    ...
    if (!empty($current_template)) {
        ...
        $post_content_block = wp_get_first_block(
            $template_blocks,
            "core/post-content"
        );
...
function get_block_editor_settings(
    ...
    $editor_settings = array_merge(
        ...
        [
            "allowedBlockTypes" => get_allowed_block_types(
                $block_editor_context
            ),
    ...
    if (
        isset($editor_settings["__experimentalFeatures"]["color"]["gradients"])
    ) {
        $gradients_by_origin =
    ...
    if (isset($editor_settings["__experimentalFeatures"]["color"]["custom"])) {
        $editor_settings["disableCustomColors"] = !$editor_settings[
    ...
    if (
        isset(
            $editor_settings["__experimentalFeatures"]["color"][
                "customGradient"
            ]
        )
    ) {
        $editor_settings["disableCustomGradients"] = !$editor_settings[
            "__experimentalFeatures"
    ...
    if (
        ...
        $editor_settings["disableCustomFontSizes"] = !$editor_settings[
            "__experimentalFeatures"
    ...
    if (
        ...
        $editor_settings["enableCustomLineHeight"] =
            $editor_settings["__experimentalFeatures"]["typography"][
                "lineHeight"
            ];
        unset(
    ...
    if (isset($editor_settings["__experimentalFeatures"]["spacing"]["units"])) {
        $editor_settings["enableCustomUnits"] =
            $editor_settings["__experimentalFeatures"]["spacing"]["units"];
        unset($editor_settings["__experimentalFeatures"]["spacing"]["units"]);
    }
    if (
        isset($editor_settings["__experimentalFeatures"]["spacing"]["padding"])
    ) {
        $editor_settings["enableCustomSpacing"] =
    ...
    if (
        ...
        $editor_settings["disableCustomSpacingSizes"] = !$editor_settings[
            "__experimentalFeatures"
    ...
    if (
        ...
        $editor_settings["spacingSizes"] = isset(
    ...
    $editor_settings["disableLayoutStyles"] = current_theme_supports(
    ...
    $editor_settings["canUpdateBlockBindings"] = current_user_can(
        "edit_block_binding",
        $block_editor_context
    );
    ...
    if (!empty($block_editor_context->post)) {
         ...
         */
        $editor_settings = apply_filters_deprecated(
            "block_editor_settings",
            [$editor_settings, $post],
            "5.8.0",
            "block_editor_settings_all"
        );
    }
...
function block_editor_rest_api_preload(
    ...
    if (!empty($block_editor_context->post)) {
         ...
         */
        $preload_paths = apply_filters_deprecated(
            "block_editor_preload_paths",
            [$preload_paths, $selected_post],
            "5.8.0",
            "block_editor_rest_api_preload_paths"
        );
    }
...
function get_classic_theme_supports_block_editor_settings()
{
    $theme_settings = [
        ...
        "disableCustomFontSizes" => get_theme_support(
            "disable-custom-font-sizes"
        ),
        "disableCustomGradients" => get_theme_support(
