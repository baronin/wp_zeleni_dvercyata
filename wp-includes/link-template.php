...
function get_permalink($post = 0, $leavename = false)
{
    ...
    if ("page" === $post->post_type) {
        ...
        in_array($post->post_type, get_post_types(["_builtin" => false]), true)
    ) {
...
function get_post_permalink($post = 0, $leavename = false, $sample = false)
{
    ...
    if (!empty($post_link) && (!$force_plain_link || $sample)) {
        ...
        if (
            $post_type->query_var &&
            (isset($post->post_status) && !$force_plain_link)
        ) {
...
function _get_page_link($post = false, $leavename = false, $sample = false)
{
    ...
    if (
        !empty($link) &&
        ((isset($post->post_status) && !$force_plain_link) || $sample)
    ) {
        if (!$leavename) {
...
function get_day_link($year, $month, $day)
{
    ...
    if (!empty($daylink)) {
        ...
        $daylink = home_url(
            "?m=" . $year . zeroise($month, 2) . zeroise($day, 2)
        );
    }
...
{
    $link =
        '<a href="' . esc_url(get_feed_link($feed)) . '">' . $anchor . "</a>";
...
function get_post_comments_feed_link($post_id = 0, $feed = "")
{
    ...
    $unattached =
        "attachment" === $post->post_type && 0 === (int) $post->post_parent;
...
function get_author_feed_link($author_id, $feed = "")
{
    ...
    if (!$permalink_structure) {
        ...
        $link =
            trailingslashit($link) . user_trailingslashit($feed_link, "feed");
    }
...
function get_post_type_archive_link($post_type)
{
    ...
    if (
        ...
        $struct =
            true === $post_type_obj->has_archive
...
function get_edit_post_link($post = 0, $context = "display")
{
    ...
    if (
        ...
        $link = admin_url(
            sprintf($post_type_object->_edit_link, $post->post_type, $slug)
        );
    } elseif ("wp_navigation" === $post->post_type) {
        $link = admin_url(
            sprintf($post_type_object->_edit_link, (string) $post->ID)
        );
...
function edit_bookmark_link(
    ...
    $link =
        '<a href="' .
        esc_url(get_edit_bookmark_link($bookmark)) .
        '">' .
        $link .
        "</a>";
...
function get_adjacent_post(
    ...
    if ($in_same_term || !empty($excluded_terms)) {
        if ($in_same_term) {
            ...
            $term_array = wp_get_object_terms($post->ID, $taxonomy, [
                "fields" => "ids",
            ]);
        ...
        if (!empty($excluded_terms)) {
            $where .=
                " AND p.ID NOT IN ( SELECT tr.object_id FROM $wpdb->term_relationships tr LEFT JOIN $wpdb->term_taxonomy tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE tt.term_id IN (" .
                implode(",", array_map("intval", $excluded_terms)) .
                ") )";
        }
    }
    ...
    if (is_user_logged_in()) {
        ...
        foreach ($private_states as $state) {
            if (current_user_can($read_private_cap)) {
                ...
                $where .= $wpdb->prepare(
                    " OR (p.post_author = %d AND p.post_status = %s)",
                    $user_id,
                    $state
                );
            }
    ...
    $where = apply_filters(
        ...
        $wpdb->prepare(
            "WHERE p.post_date $op %s AND p.post_type = %s $where",
            $current_post_date,
            $post->post_type
        ),
        $in_same_term,
...
function adjacent_posts_rel_link(
    ...
    echo get_adjacent_post_rel_link(
        $title,
        $in_same_term,
        $excluded_terms,
        true,
...
function _navigation_markup(
    $links,
    $css_class = "posts-navigation",
    $screen_reader_text = "",
    $aria_label = ""
) {
    if (empty($screen_reader_text)) {
        $screen_reader_text = /* translators: Hidden accessibility text. */ __(
            "Posts navigation"
        );
    }
    if (empty($aria_label)) {
...
function network_site_url($path = "", $scheme = null)
{
    ...
    if ("relative" === $scheme) {
        ...
        $url = set_url_scheme(
            "http://" . $current_network->domain . $current_network->path,
            $scheme
        );
    }
...
function is_avatar_comment_type($comment_type)
{
     ...
     */
    $allowed_comment_types = apply_filters("get_avatar_comment_types", [
        "comment",
    ]);
...
function get_theme_file_uri($file = "")
{
    ...
    if (empty($file)) {
        ...
        get_template_directory() !== $stylesheet_directory &&
...
function get_privacy_policy_url()
{
    ...
    if (
        !empty($policy_page_id) &&
        get_post_status($policy_page_id) === "publish"
    ) {
...
function wp_is_internal_link($link)
{
    ...
    if (
        in_array(
            wp_parse_url($link, PHP_URL_SCHEME),
            wp_allowed_protocols(),
            true
        )
    ) {
        return in_array(
            wp_parse_url($link, PHP_URL_HOST),
            wp_internal_hosts(),
            true
        );
    }
