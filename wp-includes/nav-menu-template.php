...
function wp_nav_menu($args = [])
{
    ...
    if ($args->container) {
        ...
        if (
            is_string($args->container) &&
            in_array($args->container, $allowed_tags, true)
        ) {
            $show_container = true;
            ...
            $id = $args->container_id
                ...
                : "";
            $aria_label =
                "nav" === $args->container && $args->container_aria_label
                    ? ' aria-label="' .
                        esc_attr($args->container_aria_label) .
                        '"'
                    : "";
            $nav_menu .=
                "<" . $args->container . $id . $class . $aria_label . ">";
        }
    }
...
function _wp_menu_item_classes_by_context(&$menu_items)
{
    ...
    if (
        ...
        foreach (
            (array) get_object_taxonomies($queried_object->post_type)
            as $taxonomy
        ) {
            if (is_taxonomy_hierarchical($taxonomy)) {
                ...
                if (is_array($terms)) {
                    $possible_object_parents = array_merge(
                        $possible_object_parents,
                        $terms
                    );
                    $term_to_ancestor = [];
                    foreach (
                        (array) $term_hierarchy
                        as $ancestor => $descendents
                    ) {
    ...
    foreach ((array) $menu_items as $key => $menu_item) {
        ...
        if (
            ...
            $front_page_id === (int) $menu_item->object_id
        ) {
        ...
        if (
            "post_type" === $menu_item->type &&
            $privacy_policy_page_id === (int) $menu_item->object_id
        ) {
        ...
        if (
            ...
            ((!empty($home_page_id) &&
                "post_type" === $menu_item->type &&
                $wp_query->is_home &&
                $home_page_id === (int) $menu_item->object_id) ||
            ...
            $_root_relative_current = untrailingslashit(
                $_SERVER["REQUEST_URI"]
            );

            // If it's the customize page then it will strip the query var off the URL before entering the comparison block.
            if (is_customize_preview()) {
                $_root_relative_current = strtok(
            ...
            $current_url = set_url_scheme(
                "http://" . $_SERVER["HTTP_HOST"] . $_root_relative_current
            );
    ...
    $active_ancestor_item_ids = array_filter(
        array_unique($active_ancestor_item_ids)
    );
    $active_parent_item_ids = array_filter(
        array_unique($active_parent_item_ids)
    );
    $active_parent_object_ids = array_filter(
        array_unique($active_parent_object_ids)
    );
    ...
    foreach ((array) $menu_items as $key => $parent_item) {
        ...
        if (
            ...
            (("post_type" === $parent_item->type &&
                ...
                ("taxonomy" === $parent_item->type &&
                    ...
                    in_array(
                        (int) $parent_item->object_id,
                        $possible_taxonomy_ancestors[$parent_item->object],
                        true
                    ) &&
            ...
            if (!empty($queried_object->taxonomy)) {
                $classes[] =
                    "current-" . $queried_object->taxonomy . "-ancestor";
            } else {
                $classes[] =
                    "current-" . $queried_object->post_type . "-ancestor";
            }
        }

        if (
            in_array((int) $parent_item->db_id, $active_ancestor_item_ids, true)
        ) {
        ...
        if (
            in_array((int) $parent_item->db_id, $active_parent_item_ids, true)
        ) {
