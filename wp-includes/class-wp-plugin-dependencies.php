...
class WP_Plugin_Dependencies
{
    ...
    {
        return in_array(
            self::convert_to_slug($plugin_file),
            (array) self::$dependency_slugs,
            true
        );
    }
    ...
    public static function has_unmet_dependencies($plugin_file)
    {
        ...
        foreach (self::$dependencies[$plugin_file] as $dependency) {
            ...
            if (
                ...
                is_plugin_inactive($dependency_filepath)
            ) {
                return true;
            }
        }
    ...
    public static function get_dependency_names($plugin_file)
    {
        ...
        foreach ($dependencies as $dependency) {
            ...
            if (isset($dependency_api_data[$dependency]["name"])) {
                ...
                $dependency_filepath = self::get_dependency_filepath(
                    $dependency
                );
    ...
    public static function display_admin_notice_for_circular_dependencies()
    {
        ...
        if (
            !empty($circular_dependencies) &&
            count($circular_dependencies) > 1
        ) {
            $circular_dependencies = array_unique(
    ...
    public static function check_plugin_dependencies_during_ajax()
    {
        ...
        if (
            ...
            is_plugin_inactive($plugin_file)
        ) {
        ...
        foreach ($dependencies as $dependency) {
            if (
                false === self::$plugin_dirnames[$dependency] ||
        ...
        if (!empty($inactive_dependencies)) {
            $inactive_dependency_names = array_map(function ($dependency) {
            ...
            $status["errorData"] = array_combine(
                $inactive_dependencies,
    ...
    protected static function read_dependencies_from_plugin_headers()
    {
        ...
        foreach ($plugins as $plugin => $header) {
            ...
            $dependency_slugs = self::sanitize_dependency_slugs(
    ...
    protected static function get_dependency_api_data()
    {
        ...
        if (
            !is_admin() ||
            ("plugins.php" !== $pagenow && "plugin-install.php" !== $pagenow)
        ) {
            return;
        }
        ...
        $plugins = self::get_plugins();
        self::$dependency_api_data = (array) get_site_transient(
        ...
        foreach (self::$dependency_slugs as $slug) {
            ...
            if (isset(self::$dependency_api_data[$slug])) {
                if (false === self::$dependency_api_data[$slug]) {
                    ...
                    if (false === $dependency_file) {
                        self::$dependency_api_data[$slug] = ["Name" => $slug];
                    } else {
                        self::$dependency_api_data[$slug] = [
                            "Name" => $plugins[$dependency_file]["Name"],
                        ];
                    }
                    continue;
                }
            ...
            self::$dependency_api_data[$slug]["Name"] =
                self::$dependency_api_data[$slug]["name"];
            set_site_transient(
                "wp_plugin_dependencies_plugin_data",
    ...
    protected static function check_for_circular_dependencies(
        ...
        $dependents_location_in_its_own_dependencies = array_intersect(
            $dependents,
            $dependencies
        );
        if (!empty($dependents_location_in_its_own_dependencies)) {
            foreach (
                ...
                unset(
                    $dependencies[
                        array_search($self_dependency, $dependencies, true)
                    ]
                );
            }
        }
        ...
        foreach ($dependencies as $dependency) {
            ...
            $dependency_location_in_dependents = array_search(
                $dependency,
                self::$dependent_slugs,
                true
            );
            ...
            if (false !== $dependency_location_in_dependents) {
                $dependencies_of_the_dependency =
                    self::$dependencies[$dependency_location_in_dependents];
