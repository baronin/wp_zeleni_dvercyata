...
class WP_URL_Pattern_Prefixer
{
    ...
    {
        if (count($contexts) > 0) {
            $this->contexts = array_map(static function (string $str): string {
     ...
     */
    public function prefix_path_pattern(
        string $path_pattern,
        string $context = "home"
    ): string {
    ...
    {
        return [
            "home" => self::escape_pattern_string(
                trailingslashit(
                    (string) wp_parse_url(home_url("/"), PHP_URL_PATH)
                )
            ),
            "site" => self::escape_pattern_string(
                trailingslashit(
                    (string) wp_parse_url(site_url("/"), PHP_URL_PATH)
                )
            ),
            "uploads" => self::escape_pattern_string(
                trailingslashit(
                    (string) wp_parse_url(
                        wp_upload_dir(null, false)["baseurl"],
                        PHP_URL_PATH
                    )
                )
            ),
            "content" => self::escape_pattern_string(
                trailingslashit(
                    (string) wp_parse_url(content_url(), PHP_URL_PATH)
                )
            ),
            "plugins" => self::escape_pattern_string(
                trailingslashit(
                    (string) wp_parse_url(plugins_url(), PHP_URL_PATH)
                )
            ),
            "template" => self::escape_pattern_string(
                trailingslashit(
                    (string) wp_parse_url(
            ...
            "stylesheet" => self::escape_pattern_string(
                trailingslashit(
                    (string) wp_parse_url(
