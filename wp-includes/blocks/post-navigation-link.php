...
function render_block_core_post_navigation_link($attributes, $content)
{
    ...
    $navigation_type = isset($attributes["type"])
        ? $attributes["type"]
        : "next";
    ...
    $format = "%link";
    $link =
        "next" === $navigation_type
            ? _x("Next", "label for next post link")
            : _x("Previous", "label for previous post link");
    $label = "";
    ...
    if (isset($attributes["showTitle"]) && $attributes["showTitle"]) {
        ...
        if (!$attributes["linkLabel"]) {
            if ($label) {
                $format =
                    '<span class="post-navigation-link__label">' .
                    wp_kses_post($label) .
                    "</span> %link";
            }
            $link = "%title";
            ...
            if ($label) {
                ...
                $label =
                    "next" === $navigation_type
                        ? _x("Next:", "label before the title of the next post")
                        : _x(
                            "Previous:",
                            "label before the title of the previous post"
                        );
                $link = sprintf(
    ...
    // Display arrows.
    if (
        isset($attributes["arrow"]) &&
        "none" !== $attributes["arrow"] &&
        isset($arrow_map[$attributes["arrow"]])
    ) {
