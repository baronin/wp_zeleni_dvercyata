    ...
    if ("core/gallery" === $parsed_block["blockName"]) {
        foreach ($parsed_block["innerBlocks"] as $key => $inner_block) {
            if ("core/image" === $inner_block["blockName"]) {
                if (
                    !isset(
                        $parsed_block["innerBlocks"][$key]["attrs"]["data-id"]
                    ) &&
                    isset($inner_block["attrs"]["id"])
                ) {
                    $parsed_block["innerBlocks"][$key]["attrs"][
                        "data-id"
                    ] = esc_attr($inner_block["attrs"]["id"]);
                }
            }
        }
    }
...
function block_core_gallery_render($attributes, $content)
{
    ...
    if (is_array($gap)) {
        foreach ($gap as $key => $value) {
            ...
            $value =
                $value && preg_match("%[\\\(&=}]|/\*%", $value) ? null : $value;
    ...
    if (is_array($gap_value)) {
        ...
        $gap_column = isset($gap_value["left"])
            ? $gap_value["left"]
            : $fallback_gap;
        $gap_value =
            $gap_row === $gap_column ? $gap_row : $gap_row . " " . $gap_column;
    }
