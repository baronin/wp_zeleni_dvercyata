...
function render_block_core_post_featured_image($attributes, $content, $block)
{
    ...
    $size_slug = isset($attributes["sizeSlug"])
        ? $attributes["sizeSlug"]
        : "post-thumbnail";
    $attr = get_block_core_post_featured_image_border_attributes($attributes);
    $overlay_markup = get_block_core_post_featured_image_overlay_element_markup(
        $attributes
    );
    ...
    if (!empty($attributes["style"]["shadow"])) {
        $shadow_styles = wp_style_engine_get_styles([
    ...
    if (!empty($extra_styles)) {
        $attr["style"] = empty($attr["style"])
            ? $extra_styles
            : $attr["style"] . $extra_styles;
    }
    ...
    if ($is_link) {
        ...
        $rel = !empty($attributes["rel"])
            ? 'rel="' . esc_attr($attributes["rel"]) . '"'
            : "";
        $height = !empty($attributes["height"])
            ? 'style="' .
                esc_attr(
                    safecss_filter_attr("height:" . $attributes["height"])
                ) .
                '"'
            : "";
...
{
    $has_dim_background =
        isset($attributes["dimRatio"]) && $attributes["dimRatio"];
    $has_gradient = isset($attributes["gradient"]) && $attributes["gradient"];
    $has_custom_gradient =
        isset($attributes["customGradient"]) && $attributes["customGradient"];
    $has_solid_overlay =
        isset($attributes["overlayColor"]) && $attributes["overlayColor"];
    $has_custom_overlay =
        isset($attributes["customOverlayColor"]) &&
    ...
    $border_attributes = get_block_core_post_featured_image_border_attributes(
        $attributes
    );
