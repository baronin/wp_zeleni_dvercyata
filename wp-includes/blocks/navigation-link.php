...
function block_core_navigation_link_maybe_urldecode($url)
{
    ...
    foreach ($query_params as $query_param) {
        $can_query_param_be_encoded =
            is_string($query_param) && !empty($query_param);
...
function render_block_core_navigation_link($attributes, $content, $block)
{
    ...
    if ($is_post_type && $navigation_link_has_id) {
        ...
        if (
            !$post ||
            !in_array($post->post_status, $allowed_post_status, true)
        ) {
            return "";
        }
    }
    ...
    $font_sizes = block_core_navigation_link_build_css_font_sizes(
        $block->context
    );
    ...
    if (isset($attributes["url"])) {
        $html .=
            ' href="' .
            esc_url(
                block_core_navigation_link_maybe_urldecode($attributes["url"])
            ) .
            '"';
    }
