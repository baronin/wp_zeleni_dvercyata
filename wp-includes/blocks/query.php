...
{
    $is_interactive =
        isset($attributes["enhancedPagination"]) &&
    ...
    if (!wp_style_is($style_asset)) {
        ...
        if (!$is_interactive && in_array($style_asset, $style_handles, true)) {
            $block->block_type->style_handles = array_diff($style_handles, [
                $style_asset,
            ]);
        }
        // If the styles are needed, but they were previously removed, add them again.
        if ($is_interactive && !in_array($style_asset, $style_handles, true)) {
            $block->block_type->style_handles = array_merge($style_handles, [
                $style_asset,
            ]);
        }
    }
...
function block_core_query_disable_enhanced_pagination($parsed_block)
{
    ...
    $supports_client_navigation =
        (isset($block_type->supports["interactivity"]["clientNavigation"]) &&
            true ===
                $block_type->supports["interactivity"]["clientNavigation"]) ||
        (isset($block_type->supports["interactivity"]) &&
            true === $block_type->supports["interactivity"]);
    ...
    if ("core/query" === $block_name && $has_enhanced_pagination) {
        ...
        if (!isset($render_query_callback)) {
             ...
             * @return string Returns the modified output of the query block.
             */
            $render_query_callback = static function ($content, $block) use (
                ...
                if (
                    isset($dirty_enhanced_queries[$block["attrs"]["queryId"]])
                ) {
