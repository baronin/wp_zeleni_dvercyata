...
function render_block_core_image($attributes, $content, $block)
{
    ...
    $has_id_binding =
        isset($attributes["metadata"]["bindings"]["id"]) &&
        isset($attributes["id"]);
    ...
    if ($has_id_binding) {
        ...
        if (
            is_string($image_classnames) &&
            !str_contains($image_classnames, $class_with_binding_value)
        ) {
            $image_classnames = preg_replace(
                "/wp-image-(\d+)/",
                $class_with_binding_value,
                $image_classnames
            );
    ...
    $link_destination = isset($attributes["linkDestination"])
...
function block_core_image_get_lightbox_settings($block)
{
    ...
    if (!isset($lightbox_settings)) {
        $lightbox_settings = wp_get_global_settings(
            ["lightbox"],
...
function block_core_image_render_lightbox($block_content, $block)
{
    ...
    wp_interactivity_state("core/image", [
        "metadata" => [
            $unique_image_id => [
