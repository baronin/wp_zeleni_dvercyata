...
function render_block_core_query_title($attributes)
{
    ...
    $title = "";
    if ($is_archive) {
        $show_prefix = isset($attributes["showPrefix"])
    ...
    if ($is_search) {
        ...
        if (
            isset($attributes["showSearchTerm"]) &&
    ...
    $tag_name = isset($attributes["level"])
        ? "h" . (int) $attributes["level"]
        : "h1";
    $align_class_name = empty($attributes["textAlign"])
        ? ""
