...
function block_core_page_list_build_css_colors($attributes, $context)
{
    ...
    if ($has_named_text_color) {
        ...
        $colors["css_classes"][] = sprintf(
            "has-%s-color",
            _wp_to_kebab_case($context["textColor"])
        );
    ...
    $has_picked_background_color = array_key_exists(
        "customBackgroundColor",
        $context
    );
    $has_custom_background_color = isset(
    ...
    if ($has_named_background_color) {
        ...
        $colors["css_classes"][] = sprintf(
            ...
            _wp_to_kebab_case($context["backgroundColor"])
        );
    ...
    $has_picked_overlay_text_color = array_key_exists(
    ...
    if ($has_named_overlay_text_color) {
        $colors["overlay_css_classes"][] = sprintf(
            "has-%s-color",
            _wp_to_kebab_case($context["overlayTextColor"])
        );
    ...
    $has_picked_overlay_background_color = array_key_exists(
    ...
    if ($has_named_overlay_background_color) {
        $colors["overlay_css_classes"][] = sprintf(
            "has-%s-background-color",
            _wp_to_kebab_case($context["overlayBackgroundColor"])
        );
...
function block_core_page_list_render_nested_page_list(
    ...
    foreach ((array) $nested_pages as $page) {
        ...
        $navigation_child_content_class = $is_navigation_child
            ? " wp-block-navigation-item__content"
            : "";

        // If this is the first level of submenus, include the overlay colors.
        if (
            ((0 < $depth && !$is_nested) || $is_nested) &&
            isset(
                $colors["overlay_css_classes"],
                $colors["overlay_inline_styles"]
            )
        ) {
            $css_class .=
                " " . trim(implode(" ", $colors["overlay_css_classes"]));
        ...
        if (
            ...
            $markup .=
                ...
                esc_attr($aria_label) .
                '" class="' .
                esc_attr($navigation_child_content_class) .
            ...
            $markup .=
                '<a class="wp-block-pages-list__item__link' .
                esc_attr($navigation_child_content_class) .
                '" href="' .
                esc_url($page["link"]) .
                '"' .
        ...
        if (isset($page["children"])) {
            ...
            $markup .= block_core_page_list_render_nested_page_list(
                $open_submenus_on_click,
                $show_submenu_icons,
                $is_navigation_child,
                $page["children"],
...
function block_core_page_list_nest_pages($current_level, $children)
{
    ...
    foreach ((array) $current_level as $key => $current) {
        if (isset($children[$key])) {
            $current_level[$key]["children"] = block_core_page_list_nest_pages(
                $children[$key],
                $children
            );
        }
...
function render_block_core_page_list($attributes, $content, $block)
{
    ...
    $open_submenus_on_click = array_key_exists(
        "openSubmenusOnClick",
        $block->context
    )
        ? $block->context["openSubmenusOnClick"]
        : false;

    $show_submenu_icons = array_key_exists("showSubmenuIcon", $block->context)
        ? $block->context["showSubmenuIcon"]
        : false;
