...
function render_block_core_template_part($attributes)
{
    ...
    if (isset($attributes["slug"]) && get_stylesheet() === $theme) {
        ...
        $template_part_post = $template_part_query->have_posts()
            ? $template_part_query->next_post()
            : null;
        if ($template_part_post) {
            // A published post might already exist if this template part was customized elsewhere
            // or if it's part of a customized template.
            $block_template = _build_block_template_result_from_post(
                $template_part_post
            );
            ...
            if (0 === validate_file($attributes["slug"])) {
                $block_template = get_block_file_template(
                    $template_part_id,
                    "wp_template_part"
                );
                ...
                $block_template_file = _get_block_template_file(
                    "wp_template_part",
                    $attributes["slug"]
                );
...
function build_template_part_block_instance_variations()
{
    ...
    $defined_areas = get_allowed_block_template_part_areas();
    $icon_by_area = array_combine(
        array_column($defined_areas, "area"),
        array_column($defined_areas, "icon")
    );
    ...
    foreach ($template_parts as $template_part) {
        $variations[] = [
            ...
            "scope" => ["inserter"],
            "icon" => isset($icon_by_area[$template_part->area])
                ? $icon_by_area[$template_part->area]
                : null,
            "example" => [
                "attributes" => [
                    "slug" => $template_part->slug,
