...
function render_block_core_post_date($attributes, $content, $block)
{
    ...
    if (
        ...
        if ($post_timestamp > time()) {
            ...
            $formatted_date = sprintf(
                __("%s from now"),
                human_time_diff($post_timestamp)
            );
        } else {
            // translators: %s: human-readable time difference.
            $formatted_date = sprintf(
        ...
        $formatted_date = get_the_date(
            empty($attributes["format"]) ? "" : $attributes["format"],
            $post_ID
        );
    }
     ...
     */
    if (
        isset($attributes["displayType"]) &&
        "modified" === $attributes["displayType"]
    ) {
        if (
            get_the_modified_date("Ymdhi", $post_ID) >
            get_the_date("Ymdhi", $post_ID)
        ) {
            if (
                isset($attributes["format"]) &&
                "human-diff" === $attributes["format"]
                ...
                $formatted_date = sprintf(
                    __("%s ago"),
                    human_time_diff(get_post_timestamp($post_ID, "modified"))
                );
            } else {
                $formatted_date = get_the_modified_date(
                    empty($attributes["format"]) ? "" : $attributes["format"],
                    $post_ID
                );
            }
