...
function render_block_core_query_pagination_previous(
    ...
    $page_key = isset($block->context["queryId"])
        ...
        : "query-page";
    $enhanced_pagination =
        isset($block->context["enhancedPagination"]) &&
        $block->context["enhancedPagination"];
    $max_page = isset($block->context["query"]["pages"])
        ? (int) $block->context["query"]["pages"]
        : 0;
    $page = empty($_GET[$page_key]) ? 1 : (int) $_GET[$page_key];
    $wrapper_attributes = get_block_wrapper_attributes();
    $show_label = isset($block->context["showLabel"])
        ? (bool) $block->context["showLabel"]
        : true;
    $default_label = __("Previous Page");
    $label_text =
        isset($attributes["label"]) && !empty($attributes["label"])
    ...
    if (
        ...
        $total =
            !$max_page || $max_page > $block_max_pages
                ? $block_max_pages
                : $max_page;
        wp_reset_postdata();
    ...
    if ($enhanced_pagination && isset($content)) {
        ...
        if (
            ...
            $p->set_attribute(
                "data-wp-on--click",
                "core/query::actions.navigate"
            );
            $p->set_attribute(
                "data-wp-on-async--mouseenter",
                "core/query::actions.prefetch"
            );
            $p->set_attribute(
                "data-wp-watch",
                "core/query::callbacks.prefetch"
            );
