...
class WP_Navigation_Block_Renderer
{
     ...
     * @var array
     */
    private static $needs_list_item_wrapper = [
        "core/site-title",
        "core/site-logo",
        "core/social-links",
    ];
     ...
     * @var array
     */
    private static $seen_menu_names = [];
    ...
    private static function is_responsive($attributes)
    {
        ...
        $has_old_responsive_attribute =
            !empty($attributes["isResponsive"]) && $attributes["isResponsive"];
        return (isset($attributes["overlayMenu"]) &&
    ...
    private static function does_block_need_a_list_item_wrapper($block)
    {
         ...
         */
        $needs_list_item_wrapper = apply_filters(
            "block_core_navigation_listable_blocks",
            static::$needs_list_item_wrapper
        );
    ...
    private static function get_inner_blocks_html($attributes, $inner_blocks)
    {
        ...
        foreach ($inner_blocks as $inner_block) {
            $inner_block_markup = static::get_markup_for_inner_block(
                $inner_block
            );
        ...
        if ($has_submenus && $is_interactive) {
            ...
            $inner_blocks_html = block_core_navigation_add_directives_to_submenu(
                $tags,
                $attributes
            );
        }
    ...
    private static function get_inner_blocks_from_navigation_post($attributes)
    {
        ...
        if (!isset($navigation_post)) {
            return new WP_Block_List([], $attributes);
        }
        ...
        if ("publish" === $navigation_post->post_status) {
            ...
            $blocks = block_core_navigation_filter_out_empty_blocks(
                $parsed_blocks
            );
    ...
    private static function get_inner_blocks($attributes, $block)
    {
        ...
        if (
            ...
            $inner_blocks = block_core_navigation_get_inner_blocks_from_unstable_location(
                $attributes
            );
        }
        ...
        if (array_key_exists("ref", $attributes)) {
            $inner_blocks = static::get_inner_blocks_from_navigation_post(
                $attributes
            );
        }
    ...
    private static function get_layout_class($attributes)
    {
        ...
        if (
            ...
            isset(
                $layout_justification[$attributes["layout"]["justifyContent"]]
            )
        ) {
            $layout_class .=
                $layout_justification[$attributes["layout"]["justifyContent"]];
        ...
        if (
            isset($attributes["layout"]["orientation"]) &&
            "vertical" === $attributes["layout"]["orientation"]
        ) {
            $layout_class .= " is-vertical";
        }

        if (
            isset($attributes["layout"]["flexWrap"]) &&
            "nowrap" === $attributes["layout"]["flexWrap"]
        ) {
    ...
    private static function get_classes($attributes)
    {
        ...
        $text_decoration =
            $attributes["style"]["typography"]["textDecoration"] ?? null;
        $text_decoration_class = sprintf(
            "has-text-decoration-%s",
            $text_decoration
        );
        ...
        $classes = array_merge(
            ...
            $text_decoration ? [$text_decoration_class] : []
        );
    ...
    private static function get_styles($attributes)
    {
        $colors = block_core_navigation_build_css_colors($attributes);
        $font_sizes = block_core_navigation_build_css_font_sizes($attributes);
        $block_styles = isset($attributes["styles"])
            ? $attributes["styles"]
            : "";
        return $block_styles .
     ...
     */
    private static function get_responsive_container_markup(
        $attributes,
        $inner_blocks,
        $inner_blocks_html
    ) {
        ...
        $should_display_icon_label =
            isset($attributes["hasIcon"]) && true === $attributes["hasIcon"];
        $toggle_button_icon =
        ...
        $toggle_button_content = $should_display_icon_label
            ? $toggle_button_icon
            : __("Menu");
        ...
        $toggle_close_button_content = $should_display_icon_label
            ? $toggle_close_button_icon
            : __("Close");
        $toggle_aria_label_open = $should_display_icon_label
            ? 'aria-label="' . __("Open menu") . '"'
            : ""; // Open button label.
        $toggle_aria_label_close = $should_display_icon_label
        ...
        return sprintf(
            ...
            !empty($overlay_inline_styles)
                ? "style=\"$overlay_inline_styles\""
                : "",
     ...
     */
    private static function get_nav_wrapper_attributes(
        $attributes,
        $inner_blocks
    ) {
        ...
        if ($is_responsive_menu) {
            $nav_element_directives = static::get_nav_element_directives(
                $is_interactive
            );
    ...
    private static function get_nav_element_directives($is_interactive)
    {
        ...
        $nav_element_directives =
            '
		 data-wp-interactive="core/navigation" ' . $nav_element_context;
     ...
     * @param WP_Block_List $inner_blocks The list of inner blocks.
     */
    private static function handle_view_script_module_loading(
        $attributes,
        $block,
        $inner_blocks
    ) {
        if (static::is_interactive($attributes, $inner_blocks)) {
            wp_enqueue_script_module(
                "@wordpress/block-library/navigation/view"
            );
        }
    }
    ...
    {
        $inner_blocks_html = static::get_inner_blocks_html(
            $attributes,
            $inner_blocks
        );
        if (static::is_responsive($attributes)) {
            return static::get_responsive_container_markup(
                $attributes,
                $inner_blocks,
                $inner_blocks_html
            );
    ...
    private static function get_unique_navigation_name($attributes)
    {
        ...
        if (
            isset(static::$seen_menu_names[$nav_menu_name]) &&
    ...
    public static function render($attributes, $content, $block)
    {
        ...
        if (
            isset($attributes["rgbBackgroundColor"]) &&
...
if (defined("IS_GUTENBERG_PLUGIN") && IS_GUTENBERG_PLUGIN) {
    ...
    function block_core_navigation_get_inner_blocks_from_unstable_location(
        ...
        $menu_items_by_parent_id = block_core_navigation_sort_menu_items_by_parent_id(
            $menu_items
        );
...
function block_core_navigation_add_directives_to_submenu(
    ...
    while (
        ...
        $tags->set_attribute(
            "data-wp-context",
            '{ "submenuOpenedBy": { "click": false, "hover": false, "focus": false }, "type": "submenu", "modal": null }'
        );
...
function block_core_navigation_build_css_colors($attributes)
{
    ...
    if ($has_named_text_color) {
        // Add the color class.
        $colors["css_classes"][] = sprintf(
            "has-%s-color",
            $attributes["textColor"]
        );
    } elseif ($has_custom_text_color) {
        // Add the custom color inline style.
        $colors["inline_styles"] .= sprintf(
            "color: %s;",
    ...
    // Background color.
    $has_named_background_color = array_key_exists(
        "backgroundColor",
        $attributes
    );
    $has_custom_background_color = array_key_exists(
    ...
    // Overlay text color.
    $has_named_overlay_text_color = array_key_exists(
        "overlayTextColor",
        $attributes
    );
    $has_custom_overlay_text_color = array_key_exists(
    ...
    $has_named_overlay_background_color = array_key_exists(
        "overlayBackgroundColor",
        $attributes
    );
    $has_custom_overlay_background_color = array_key_exists(
...
function block_core_navigation_build_css_font_sizes($attributes)
{
    ...
    if ($has_named_font_size) {
        ...
        $font_sizes["css_classes"][] = sprintf(
            "has-%s-font-size",
            $attributes["fontSize"]
        );
...
function block_core_navigation_get_fallback_blocks()
{
    ...
    $fallback_blocks = $registry->is_registered("core/page-list")
        ? $page_list_fallback
        : [];
    ...
    if ($navigation_post) {
        $parsed_blocks = parse_blocks($navigation_post->post_content);
        $maybe_fallback = block_core_navigation_filter_out_empty_blocks(
            $parsed_blocks
        );

        // Normalizing blocks may result in an empty array of blocks if they were all `null` blocks.
        // In this case default to the (Page List) fallback.
        $fallback_blocks = !empty($maybe_fallback)
            ? $maybe_fallback
            : $fallback_blocks;
...
function block_core_navigation_from_block_get_post_ids($block)
{
    ...
    if (
        "core/navigation-link" === $block->name ||
        "core/navigation-submenu" === $block->name
    ) {
        if (
            $block->attributes &&
            isset($block->attributes["kind"]) &&
            "post-type" === $block->attributes["kind"] &&
...
function block_core_navigation_typographic_presets_backcompatibility(
    ...
    if ("core/navigation" === $parsed_block["blockName"]) {
        ...
        foreach ($attribute_to_prefix_map as $style_attribute => $prefix) {
            if (
                !empty(
                    $parsed_block["attrs"]["style"]["typography"][
                        $style_attribute
                    ]
                )
            ) {
                ...
                if (
                    "textDecoration" === $style_attribute &&
                    "strikethrough" === $attribute_value
                ) {
...
function block_core_navigation_parse_blocks_from_menu_items(
    ...
    foreach ($menu_items as $menu_item) {
        $class_name = !empty($menu_item->classes)
            ? implode(" ", (array) $menu_item->classes)
            : null;
        $id =
            null !== $menu_item->object_id && "custom" !== $menu_item->object
                ? $menu_item->object_id
                : null;
        $opens_in_new_tab =
            null !== $menu_item->target && "_blank" === $menu_item->target;
        $rel =
            null !== $menu_item->xfn && "" !== $menu_item->xfn
                ? $menu_item->xfn
                : null;
        $kind =
            null !== $menu_item->type
                ? str_replace("_", "-", $menu_item->type)
                : "custom";

        $block = [
            "blockName" => isset($menu_items_by_parent_id[$menu_item->ID])
...
function block_core_navigation_get_classic_menu_fallback_blocks(
    ...
    $inner_blocks = block_core_navigation_parse_blocks_from_menu_items(
        isset($menu_items_by_parent_id[0]) ? $menu_items_by_parent_id[0] : [],
        $menu_items_by_parent_id
    );
...
function block_core_navigation_maybe_use_classic_menu_fallback()
{
    ...
    $classic_nav_menu_blocks = block_core_navigation_get_classic_menu_fallback_blocks(
        $classic_nav_menu
    );
