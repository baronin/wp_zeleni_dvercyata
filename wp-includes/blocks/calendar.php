...
function render_block_core_calendar($attributes)
{
    ...
    // Text color.
    $preset_text_color = array_key_exists("textColor", $attributes)
    ...
    $inline_styles = empty($styles["css"])
        ? ""
        : sprintf(' style="%s"', esc_attr($styles["css"]));
    $classnames = empty($styles["classnames"])
        ? ""
        : " " . esc_attr($styles["classnames"]);
...
function block_core_calendar_update_has_published_posts()
{
    ...
    $has_published_posts = (bool) $wpdb->get_var(
        "SELECT 1 as test FROM {$wpdb->posts} WHERE post_type = 'post' AND post_status = 'publish' LIMIT 1"
    );
    update_option(
        "wp_calendar_block_has_published_posts",
        $has_published_posts
