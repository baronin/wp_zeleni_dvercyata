...
function render_block_core_categories($attributes, $content, $block)
{
    ...
    if (
        !empty($attributes["showOnlyTopLevel"]) &&
    ...
    if (!empty($attributes["displayAsDropdown"])) {
        ...
        $label_text = !empty($attributes["label"])
            ? wp_kses_post($attributes["label"])
            : $default_label;
        $wrapper_markup =
            '<div %1$s><label class="wp-block-categories__label' .
            $show_label .
            '" for="' .
            esc_attr($id) .
            '">' .
	...
	( function() {
		...
		function onCatChange() {
			if ( dropdown.options[ dropdown.selectedIndex ].value !== -1 ) {
				location.href = "<?php echo esc_url(
        home_url()
    ); ?>/?" + dropdown.name + '=' + dropdown.options[ dropdown.selectedIndex ].value;
			}
		}
		dropdown.onchange = onCatChange;
	})();
	</script>
	<?php return wp_get_inline_script_tag(
