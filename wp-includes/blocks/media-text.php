...
function render_block_core_media_text($attributes, $content)
{
    ...
    $has_media_on_right =
        isset($attributes["mediaPosition"]) &&
        "right" === $attributes["mediaPosition"];
    ...
    $media_size_slug = isset($attributes["mediaSizeSlug"])
        ? $attributes["mediaSizeSlug"]
        : "full";
    $image_tag = '<img class="wp-block-media-text__featured_image">';
    $content = preg_replace(
        '/(<figure\s+id="' .
            preg_quote($unique_id, "/") .
    ...
    if (
        ...
        if (
            ...
            $image_tag_processor->set_attribute(
                "src",
...
{
    register_block_type_from_metadata(__DIR__ . "/media-text", [
