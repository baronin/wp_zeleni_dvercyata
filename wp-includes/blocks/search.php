...
function render_block_core_search($attributes)
{
    ...
    $typography_classes = get_typography_classes_for_block_core_search(
        $attributes
    );
    $is_button_inside =
        !empty($attributes["buttonPosition"]) &&
    ...
    $border_color_classes = get_border_color_classes_for_block_core_search(
        $attributes
    );
    ...
    $label_inner_html = empty($attributes["label"])
        ? __("Search")
        : wp_kses_post($attributes["label"]);
    $label = new WP_HTML_Tag_Processor(
        sprintf(
            '<label %1$s>%2$s</label>',
            $inline_styles["label"],
            $label_inner_html
    ...
    $input = new WP_HTML_Tag_Processor(
        sprintf(
            '<input type="search" name="s" required %s/>',
            $inline_styles["input"]
        )
    );
    ...
    if ($show_button) {
        ...
        if (!$use_icon_button) {
            if (!empty($attributes["buttonText"])) {
                $button_internal_markup = wp_kses_post(
        ...
        $button = new WP_HTML_Tag_Processor(
            sprintf(
        ...
        if ($button->next_tag()) {
            ...
            if ("button-only" === $attributes["buttonPosition"]) {
                ...
                $button->set_attribute(
                    "aria-controls",
                    "wp-block-search__input-" . $input_id
                );
                ...
                $button->set_attribute(
                    "aria-label",
                    wp_strip_all_tags($attributes["buttonText"])
                );
            }
        }
    }
...
function apply_block_core_search_border_style(
    ...
    $is_button_inside =
        isset($attributes["buttonPosition"]) &&
        "button-inside" === $attributes["buttonPosition"];
...
function styles_for_block_core_search($attributes)
{
    ...
    $is_button_inside =
        ...
        "button-inside" === $attributes["buttonPosition"];
    $show_label =
        isset($attributes["showLabel"]) && false !== $attributes["showLabel"];

    // Add width styles.
    $has_width =
        !empty($attributes["width"]) && !empty($attributes["widthUnit"]);
    ...
    if ($has_border_radius) {
        ...
        if (is_array($border_radius)) {
            ...
            $border_radius = is_numeric($border_radius)
                ? $border_radius . "px"
                : $border_radius;
            $border_style = sprintf(
                "border-radius: %s;",
...
function get_typography_classes_for_block_core_search($attributes)
{
    ...
    if ($has_named_font_family) {
        $typography_classes[] = sprintf(
            "has-%s-font-family",
            esc_attr($attributes["fontFamily"])
        );
    }
...
function get_color_classes_for_block_core_search($attributes)
{
    ...
    $has_custom_background_color = !empty(
        $attributes["style"]["color"]["background"]
    );
