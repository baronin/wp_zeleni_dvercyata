...
function render_block_core_navigation_submenu($attributes, $content, $block)
{
    ...
    $is_post_type =
        isset($attributes["kind"]) && "post-type" === $attributes["kind"];
    $is_post_type =
        $is_post_type ||
    ...
    if (
        $is_post_type &&
        $navigation_link_has_id &&
        "publish" !== get_post_status($attributes["id"])
    ) {
        return "";
    }
    ...
    $font_sizes = block_core_navigation_submenu_build_css_font_sizes(
        $block->context
    );
    $style_attribute = $font_sizes["inline_styles"];

    $has_submenu = count($block->inner_blocks) > 0;
    $kind = empty($attributes["kind"])
        ? "post_type"
        : str_replace("-", "_", $attributes["kind"]);
    $is_active =
        !empty($attributes["id"]) &&
    ...
    $show_submenu_indicators =
        isset($block->context["showSubmenuIcon"]) &&
        $block->context["showSubmenuIcon"];
    $open_on_click =
        isset($block->context["openSubmenusOnClick"]) &&
    ...
    $classes = array_merge($classes, $font_sizes["css_classes"]);
    if ($has_submenu) {
        $classes[] = "has-child";
    }
    if ($open_on_click) {
    ...
    if ($has_submenu) {
        ...
        if (array_key_exists("overlayBackgroundColor", $block->context)) {
            $attributes["backgroundColor"] =
                $block->context["overlayBackgroundColor"];
        }
        if (array_key_exists("customOverlayTextColor", $block->context)) {
            $attributes["style"]["color"]["text"] =
                $block->context["customOverlayTextColor"];
        }
        if (array_key_exists("customOverlayBackgroundColor", $block->context)) {
            $attributes["style"]["color"]["background"] =
                $block->context["customOverlayBackgroundColor"];
        }

        // This allows us to be able to get a response from wp_apply_colors_support.
        $block->block_type->supports["color"] = true;
        $colors_supports = wp_apply_colors_support(
            $block->block_type,
            $attributes
        );
