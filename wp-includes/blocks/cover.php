...
function render_block_core_cover($attributes, $content)
{
    if (
        ...
        false === $attributes["useFeaturedImage"]
    ) {
        return $content;
    }
    ...
    if (!($attributes["hasParallax"] || $attributes["isRepeated"])) {
        ...
        $image = get_the_post_thumbnail(
            null,
            $attributes["sizeSlug"] ?? "post-thumbnail",
            $attr
        );
    } else {
        if (in_the_loop()) {
        ...
        $styles =
            "background-position:" . ($object_position ?? "50% 50%") . ";";
        $styles .=
            "background-image:url(" . esc_url($current_featured_image) . ");";
    ...
    $inner_container_start =
        '/<div\b[^>]+wp-block-cover__inner-container[\s|"][^>]*>/U';
    if (
        1 ===
        preg_match(
            $inner_container_start,
        ...
        $offset = $matches[0][1];
        $content =
            substr($content, 0, $offset) . $image . substr($content, $offset);
    }
