...
function render_block_core_post_template($attributes, $content, $block)
{
    ...
    $classnames = "";
    if (
        isset($block->context["displayLayout"]) &&
        isset($block->context["query"])
    ) {
        if (
            isset($block->context["displayLayout"]["type"]) &&
    ...
    if (
        ...
        $classnames .=
            " " .
            sanitize_title("columns-" . $attributes["layout"]["columnCount"]);
    }
    ...
    while ($query->have_posts()) {
        ...
        $inner_block_directives = $enhanced_pagination
            ? ' data-wp-key="post-template-item-' . $post_id . '"'
            : "";
