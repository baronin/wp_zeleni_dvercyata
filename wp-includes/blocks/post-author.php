...
function render_block_core_post_author($attributes, $content, $block)
{
    ...
    if (
        ...
        !post_type_supports($block->context["postType"], "author")
    ) {
        return "";
    }

    $avatar = !empty($attributes["avatarSize"])
        ? get_avatar($author_id, $attributes["avatarSize"])
        : null;
    ...
    return sprintf('<div %1$s>', $wrapper_attributes) .
        (!empty($attributes["showAvatar"])
            ? '<div class="wp-block-post-author__avatar">' . $avatar . "</div>"
            : "") .
