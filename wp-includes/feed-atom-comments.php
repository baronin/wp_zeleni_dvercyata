...
>
	...
	<?php if (is_singular()) {
     ...
     printf(
         ent2ncr(__('Comments for %1$s searching on %2$s')),
...
<?php if (is_singular()): ?>
	<link rel="alternate" type="<?php bloginfo_rss(
     "html_type"
 ); ?>" href="<?php comments_link_feed(); ?>" />
	<link rel="self" type="application/atom+xml" href="<?php echo esc_url(
     get_post_comments_feed_link("", "atom")
 ); ?>" />
...
<?php elseif (is_search()): ?>
	...
	<link rel="self" type="application/atom+xml" href="<?php echo get_search_comments_feed_link(
     "",
     "atom"
 ); ?>" />
...
<?php else: ?>
	<link rel="alternate" type="<?php bloginfo_rss(
     "html_type"
 ); ?>" href="<?php bloginfo_rss("url"); ?>" />
	<link rel="self" type="application/atom+xml" href="<?php bloginfo_rss(
     "comments_atom_url"
 ); ?>" />
...
<?php while (have_comments()):
	...
	<entry>
      ...
      printf(
          ent2ncr(__('Comment on %1$s by %2$s')),
          $title,
          get_comment_author_rss()
      );
  } else {
      /* translators: Comment author title. %s: Comment author name. */ printf(
          ent2ncr(__("By: %s")),
          get_comment_author_rss()
      );
  } ?>
		</title>
		<link rel="alternate" href="<?php comment_link(); ?>" type="<?php bloginfo_rss(
    "html_type"
); ?>" />
    ...
    ?>
		...
		<?php // Return comment threading information (https://www.ietf.org/rfc/rfc4685.txt).


  if ("0" === $comment->comment_parent):// This comment is top-level.
       ?>
			<thr:in-reply-to ref="<?php the_guid(); ?>" href="<?php the_permalink_rss(); ?>" type="<?php bloginfo_rss(
    "html_type"
); ?>" />
			<?php // This comment is in reply to another comment.
      /*
      ...
      /*
			 ...
			 */
      ?>
			<thr:in-reply-to ref="<?php comment_guid(
       $parent_comment
   ); ?>" href="<?php echo get_comment_link(
    $parent_comment
