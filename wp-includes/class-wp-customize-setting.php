...
class WP_Customize_Setting
{
    ...
    protected function aggregate_multidimensional()
    {
        ...
        if (
            !isset(self::$aggregated_multidimensionals[$this->type][$id_base])
        ) {
    ...
    public function preview()
    {
        ...
        if (!$needs_preview) {
            if ($this->is_multidimensional_aggregated) {
                ...
                $value = $this->multidimensional_get(
                    $root,
                    $this->id_data["keys"],
                    $undefined
                );
        ...
        if (!$needs_preview) {
            if (
                !has_action("customize_post_value_set_{$this->id}", [
                    $this,
                    "preview",
                ])
            ) {
    ...
    final public function _clear_aggregated_multidimensional_preview_applied_flag()
    {
        unset(
            self::$aggregated_multidimensionals[$this->type][
                $this->id_data["base"]
            ]["preview_applied_instances"][$this->id]
        );
    }
    ...
    final public function _multidimensional_preview_filter($original)
    {
        ...
        foreach (
            ...
            $value = $previewed_setting->post_value(
                $previewed_setting->default
            );
            $root =
                self::$aggregated_multidimensionals[$previewed_setting->type][
                    $id_base
                ]["root_value"];
            $root = $previewed_setting->multidimensional_replace(
                $root,
                $previewed_setting->id_data["keys"],
                $value
            );
            self::$aggregated_multidimensionals[$previewed_setting->type][
                $id_base
            ...
            self::$aggregated_multidimensionals[$previewed_setting->type][
                $id_base
            ]["preview_applied_instances"][$previewed_setting->id] = true;
        }

        return self
            ::$aggregated_multidimensionals[$this->type][$id_base]["root_value"];
    }
    ...
    final public function check_capabilities()
    {
        ...
        if (
            $this->theme_supports &&
            !current_theme_supports(...(array) $this->theme_supports)
        ) {
            return false;
        }
     ...
     */
    final protected function multidimensional_get(
        $root,
        $keys,
        $default_value = null
    ) {
