...
class WP_Customize_Themes_Section extends WP_Customize_Section
{
       ...
       is_multisite()
        ...
        "Theme Details"
         ...
         "No themes found. Try a different search."
          ...
          sprintf(
              '<button type="button" class="button-link search-dotorg-themes">%s</button>',
    ...
    {
        ?>
		<button type="button" class="button button-primary customize-section-back customize-themes-mobile-back"><?php _e(
      "Go to theme sources"
  ); ?></button>
		<# if ( 'wporg' === data.action ) { #>
			<div class="themes-filter-container">
				<label for="wp-filter-search-input-{{ data.id }}"><?php _e(
        "Search themes"
    ); ?></label>
      ...
      _e("The search results will be updated as you type."); ?>
		...
		<div class="filter-themes-wrapper">
			...
			<button type="button" class="button feature-filter-toggle">
				<span class="filter-count-0"><?php _e(
        "Filter themes"
    ); ?></span><span class="filter-count-filters">
					<?php /* translators: %s: Number of filters selected. */
     printf(
         __("Filter themes (%s)"),
         '<span class="theme-filter-count">0</span>'
     ); ?>
				</span>
			</button>
			<# } #>
			<div class="filter-themes-count">
				<span class="themes-displayed">
					<?php /* translators: %s: Number of themes displayed. */
					...
					<fieldset class="filter-group">
						...
						<div class="filter-group-feature">
							<?php foreach ($features as $feature => $feature_name): ?>
								<input type="checkbox" id="filter-id-<?php echo esc_attr(
            $feature
        ); ?>" value="<?php echo esc_attr($feature); ?>" />
								<label for="filter-id-<?php echo esc_attr($feature); ?>"><?php echo esc_html(
    $feature_name
); ?></label>
							<?php endforeach; ?>
						</div>
