		...
		<div class="menu-item-bar">
			<div class="menu-item-handle">
				...
				<span class="item-controls">
					...
					<# if ( 0 === data.depth ) { #>
						<?php /* translators: 1: Title of a menu item, 2: Type of a menu item. 3: Item index, 4: Total items. */
      printf(
          __('Edit %1$s (%2$s, %3$d of %4$d)'),
          "{{ data.title || data.original_title || wp.customize.Menus.data.l10n.untitled }}",
          "{{ data.item_type_label }}",
          "",
          ""
      ); ?>
					<# } else if ( 1 === data.depth ) { #>
						<?php /* translators: 1: Title of a menu item, 2: Type of a menu item, 3, Item index, 4, Total items, 5: Item parent. */
      printf(
          __('Edit %1$s (%2$s, sub-item %3$d of %4$d under %5$s)'),
          "{{ data.title || data.original_title || wp.customize.Menus.data.l10n.untitled }}",
          "{{ data.item_type_label }}",
          "",
          "",
          ""
					...
					<# } else { #>
						<?php /* translators: 1: Title of a menu item, 2: Type of a menu item, 3, Item index, 4, Total items, 5: Item parent, 6: Item depth. */
      printf(
          __('Edit %1$s (%2$s, sub-item %3$d of %4$d under %5$s, level %6$s)'),
          "{{ data.title || data.original_title || wp.customize.Menus.data.l10n.untitled }}",
          "{{ data.item_type_label }}",
          "",
          "",
          ...
          "{{data.depth}}"
					...
					<?php /* translators: 1: Title of a menu item, 2: Type of a menu item. */
     printf(
   ...
   do_action("wp_nav_menu_item_custom_fields_customize_template"); ?>
			...
			<div class="menu-item-actions description-thin submitbox">
				...
				<p class="link-to-original">
					<?php /* translators: Nav menu item original title. %s: Original title. */
     printf(
         __("Original: %s"),
         '<a class="original-link" href="{{ data.url }}">{{ data.original_title }}</a>'
				...
				<button type="button" class="button-link button-link-delete item-delete submitdelete deletion"><?php _e(
        "Remove"
    ); ?></button>
				<span class="spinner"></span>
			</div>
