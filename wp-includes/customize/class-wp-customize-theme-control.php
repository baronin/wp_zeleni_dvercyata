...
class WP_Customize_Theme_Control extends WP_Customize_Control
{
    ...
    public function content_template()
    {
        ...
        $details_label = sprintf(
            __("Details for theme: %s"),
            "{{ data.theme.name }}"
        );
        /* translators: %s: Theme name. */
        $customize_label = sprintf(
            __("Customize theme: %s"),
            "{{ data.theme.name }}"
        );
        /* translators: %s: Theme name. */
        $preview_label = sprintf(
            __("Live preview theme: %s"),
            "{{ data.theme.name }}"
        );
        /* translators: %s: Theme name. */
        $install_label = sprintf(
            __("Install and preview theme: %s"),
            "{{ data.theme.name }}"
        );
        ?>
		...
		<# } #>
			...
			<span class="more-details theme-details" id="{{ data.section }}-{{ data.theme.id }}-action" aria-label="<?php echo esc_attr(
       $details_label
   ); ?>"><?php _e("Theme Details"); ?></span>

			<div class="theme-author">
			<?php /* translators: Theme author name. */
   printf(_x("By %s", "theme author"), "{{ data.theme.author }}"); ?>
      ...
      ?>
			...
			<# if ( data.theme.active ) { #>
				<div class="theme-id-container">
					...
					<div class="theme-actions">
						<button type="button" class="button button-primary customize-theme" aria-label="<?php echo esc_attr(
          $customize_label
      ); ?>"><?php _e("Customize"); ?></button>
					</div>
				</div>
        ...
        $aria_label = sprintf(_x("Activate %s", "theme"), "{{ data.name }}"); ?>
								<a href="{{{ data.theme.actions.activate }}}" class="button button-primary activate" aria-label="<?php echo esc_attr(
            $aria_label
        ); ?>"><?php _e("Activate"); ?></a>
							<# } #>
						</div>
					</div>
					<?php $customizer_not_supported_message = __(
         'This theme doesn\'t support Customizer.'
     ); ?>
					<# if ( data.theme.actions.activate ) { #>
						<?php $customizer_not_supported_message .=
          " " .
          sprintf(
              ...
              "{{{ data.theme.actions.activate }}}"
					...
					<?php wp_admin_notice($customizer_not_supported_message, [
         "type" => "error",
         "additional_classes" => ["notice-alt"],
     ]); ?>
				<# } else { #>
					<div class="theme-id-container">
						...
						<div class="theme-actions">
							<# if ( data.theme.compatibleWP && data.theme.compatiblePHP ) { #>
								<button type="button" class="button button-primary preview-theme" aria-label="<?php echo esc_attr(
            $preview_label
        ); ?>" data-slug="{{ data.theme.id }}"><?php _e(
    "Live Preview"
); ?></button>
							<# } else { #>
								<button type="button" class="button button-primary disabled" aria-label="<?php echo esc_attr(
            $preview_label
        ); ?>"><?php _e("Live Preview"); ?></button>
							<# } #>
						</div>
					</div>
					<?php wp_admin_notice(_x("Installed", "theme"), [
         ...
         "additional_classes" => ["notice-alt"],
			...
			<# } else { #>
				<div class="theme-id-container">
					...
					<div class="theme-actions">
						<# if ( data.theme.compatibleWP && data.theme.compatiblePHP ) { #>
							<button type="button" class="button button-primary theme-install preview" aria-label="<?php echo esc_attr(
           $install_label
       ); ?>" data-slug="{{ data.theme.id }}" data-name="{{ data.theme.name }}"><?php _e(
    "Install &amp; Preview"
); ?></button>
						<# } else { #>
							<button type="button" class="button button-primary disabled" aria-label="<?php echo esc_attr(
