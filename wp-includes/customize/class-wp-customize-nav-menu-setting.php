...
class WP_Customize_Nav_Menu_Setting extends WP_Customize_Setting
{
        ...
        if (
            ...
            if ($this->term_id > 0) {
                ...
                if ($term) {
                    ...
                    $nav_menu_options = (array) get_option(
                        "nav_menu_options",
                        []
                    );
                    ...
                    if (
                        isset($nav_menu_options["auto_add"]) &&
                        is_array($nav_menu_options["auto_add"])
                    ) {
                        $value["auto_add"] = in_array(
                            $term->term_id,
    ...
    public function preview()
    {
        ...
        add_filter(
            ...
            [$this, "filter_wp_get_nav_menu_object"],
            10,
            2
        );
        ...
        add_filter("option_nav_menu_options", [
            $this,
            "filter_nav_menu_options",
        ]);
    ...
    public function filter_wp_get_nav_menus($menus, $args)
    {
        ...
        foreach ($menus as $i => $menu) {
            if (
                (int) $this->term_id === (int) $menu->term_id ||
                (int) $this->previous_term_id === (int) $menu->term_id
            ) {
                $index = $i;
                break;
            }
    ...
    protected function update($value)
    {
        ...
        if ($is_delete) {
            ...
            $menu_data = wp_array_slice_assoc($value, [
                "description",
                "parent",
            ]);
        ...
        if ("inserted" === $this->update_status) {
            ...
            foreach ($this->manager->settings() as $setting) {
                ...
                if (
                    !is_null($post_value) &&
                    (int) $post_value === $this->previous_term_id
                ) {
            ...
            foreach (
                ...
                $nav_menu_widget_setting = $this->manager->get_setting(
                    $setting_id
                );
                if (
                ...
                if (
                    empty($widget_instance["nav_menu"]) ||
                    (int) $widget_instance["nav_menu"] !==
                ...
                $updated_widget_instance = $this->manager->widgets->sanitize_widget_js_instance(
                    $widget_instance
                );
                $this->manager->set_post_value(
                    $nav_menu_widget_setting->id,
    ...
    public function amend_customize_save_response($data)
    {
        ...
        $data["nav_menu_updates"][] = [
            ...
            "saved_value" =>
                "deleted" === $this->update_status ? null : $this->value(),
        ];
