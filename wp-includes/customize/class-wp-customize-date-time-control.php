...
class WP_Customize_Date_Time_Control extends WP_Customize_Control
{
    ...
    public function content_template()
    {
        ...
        if (
            1 !== substr_count($date_format, '%1$s') ||
            1 !== substr_count($date_format, '%2$s') ||
            1 !== substr_count($date_format, '%3$s')
        ) {
		...
		<div class="date-time-fields {{ data.includeTime ? 'includes-time' : '' }}">
			<fieldset class="day-row">
				<legend class="title-day {{ ! data.includeTime ? 'screen-reader-text' : '' }}"><?php esc_html_e(
        "Date"
    ); ?></legend>
    ...
    public function get_month_choices()
    {
        ...
        $months = [];
        for ($i = 1; $i < 13; $i++) {
            $month_text = $wp_locale->get_month_abbrev(
    ...
    public function get_timezone_info()
    {
        ...
        if ($tz_string) {
            ...
            if ($tz) {
                ...
                $formatted_gmt_offset = $this->format_gmt_offset(
                    $tz->getOffset($now) / HOUR_IN_SECONDS
                );
