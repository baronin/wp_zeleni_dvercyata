					...
					<div class="site-icon-preview customizer">
						<div class="direction-wrap">
							<img src="{{ data.attachment.sizes.full ? data.attachment.sizes.full.url : data.attachment.url }}" class="app-icon-preview" alt="{{
								data.attachment.alt ?
									wp.i18n.sprintf(
										<?php /* translators: %s: The selected image alt text. */
          echo wp_json_encode(__("App icon preview: Current image: %s")); ?>
										,
										data.attachment.alt
									) :
									wp.i18n.sprintf(
										<?php /* translators: %s: The selected image filename. */
          echo wp_json_encode(
              __(
            ...
            echo wp_json_encode(
             ...
             get_bloginfo("name")
				...
				<div class="actions">
					<# if ( data.canUpload ) { #>
						<button type="button" class="button remove-button"><?php echo $this
          ->button_labels["remove"]; ?></button>
						<button type="button" class="button upload-button"><?php echo $this
          ->button_labels["change"]; ?></button>
					<# } #>
				</div>
			</div>
		<# } else { #>
			<div class="attachment-media-view">
				<# if ( data.canUpload ) { #>
					<button type="button" class="upload-button button-add-media"><?php echo $this
         ->button_labels["site_icon"]; ?></button>
				<# } #>
				<div class="actions">
					<# if ( data.defaultAttachment ) { #>
						<button type="button" class="button default-button"><?php echo $this
          ->button_labels["default"]; ?></button>
					<# } #>
				</div>
			</div>
		<# } #>
