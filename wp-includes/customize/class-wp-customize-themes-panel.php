    ...
    protected function render_template()
    {
		...
		<li id="accordion-section-{{ data.id }}" class="accordion-section control-panel-themes">
			<h3 class="accordion-section-title">
				<?php if ($this->manager->is_theme_active()) {
        echo '<span class="customize-action">' .
            __("Active theme") .
            "</span> {{ data.title }}";
    } else {
        echo '<span class="customize-action">' .
            ...
            "</span> {{ data.title }}";
				...
				<?php if (current_user_can("switch_themes")): ?>
					<button type="button" class="button change-theme" aria-label="<?php esc_attr_e(
         "Change theme"
     ); ?>"><?php _ex("Change", "theme"); ?></button>
				<?php endif; ?>
			</h3>
