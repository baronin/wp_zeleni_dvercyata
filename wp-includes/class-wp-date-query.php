...
class WP_Date_Query
{
    ...
    public function validate_date_values($date_query = [])
    {
        ...
        if (array_key_exists("year", $date_query)) {
            ...
            $max_days_of_year =
                (int) gmdate("z", mktime(0, 0, 0, 12, 31, $_year)) + 1;
        } else {
        ...
        foreach ($min_max_checks as $key => $check) {
            ...
            foreach ((array) $date_query[$key] as $_value) {
                $is_between =
                    $_value >= $check["min"] && $_value <= $check["max"];
        ...
        $day_exists =
            array_key_exists("day", $date_query) &&
            is_numeric($date_query["day"]);
        $month_exists =
            array_key_exists("month", $date_query) &&
            is_numeric($date_query["month"]);
        $year_exists =
            array_key_exists("year", $date_query) &&
            is_numeric($date_query["year"]);
    ...
    public function validate_column($column)
    {
        ...
        if (!str_contains($column, ".")) {
            ...
            $known_columns = [
                ...
                $wpdb->comments => ["comment_date", "comment_date_gmt"],
                $wpdb->users => ["user_registered"],
                $wpdb->blogs => ["registered", "last_updated"],
            ];
    ...
    protected function get_sql_for_clause($query, $parent_query)
    {
        ...
        if (
            ...
            $time_query = $this->build_time_query(
                $column,
                $compare,
                $query["hour"],
                $query["minute"],
    ...
    public function build_time_query(
        ...
        if (
            in_array($compare, ["IN", "NOT IN", "BETWEEN", "NOT BETWEEN"], true)
        ) {
            $return = [];
        ...
        return $wpdb->prepare(
            "DATE_FORMAT( $column, %s ) $compare %f",
            $format,
            $time
        );
    }
