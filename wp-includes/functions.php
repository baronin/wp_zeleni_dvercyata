...
function current_time($type, $gmt = 0)
{
    ...
    if ("timestamp" === $type || "U" === $type) {
        return $gmt
            ? time()
            : time() +
                    (int) ((float) get_option("gmt_offset") * HOUR_IN_SECONDS);
    }
    ...
    if (empty($wp_locale->month) || empty($wp_locale->weekday)) {
        ...
        for ($i = 0; $i < $format_length; $i++) {
            switch ($format[$i]) {
                ...
                case "a":
                    $new_format .= addcslashes(
                        $wp_locale->get_meridiem($datetime->format("a")),
                        "\\A..Za..z"
                    );
                    break;
                case "A":
...
function human_readable_duration($duration = "")
{
    ...
    if (3 === $duration_count) {
        ...
        if (
            !((bool) preg_match('/^([0-5]?[0-9]):([0-5]?[0-9])$/', $duration))
        ) {
            return false;
        }
    ...
    if (is_numeric($hour)) {
        ...
        $human_readable_duration[] = sprintf(
...
function do_enclose($content, $post)
{
    ...
    foreach ((array) $post_links as $url) {
        ...
        if (
            ...
            if ($headers) {
                $len = isset($headers["Content-Length"])
...
function do_feed()
{
    ...
    if (!has_action("do_feed_{$feed}")) {
        wp_die(
            __("<strong>Error:</strong> This is not a valid feed template."),
            "",
            ["response" => 404]
        );
    }
...
function do_favicon()
{
    ...
    wp_redirect(
        get_site_icon_url(32, includes_url("images/w-logo-blue-white-bg.png"))
    );
    exit();
}
...
function wp_get_raw_referer()
{
    if (
        ...
        is_string($_REQUEST["_wp_http_referer"])
    ) {
...
function wp_get_original_referer()
{
    ...
    if (!empty($_REQUEST["_wp_original_http_referer"])) {
        return wp_validate_redirect(
            wp_unslash($_REQUEST["_wp_original_http_referer"]),
            false
        );
    }
...
function wp_mkdir_p($target)
{
    ...
    if (
        str_contains($target, "../") ||
        str_contains($target, ".." . DIRECTORY_SEPARATOR)
    ) {
        return false;
    }
    ...
    if (@mkdir($target, $dir_perms, true)) {
        ...
        if (($dir_perms & ~umask()) !== $dir_perms) {
            $folder_parts = explode(
                "/",
                substr($target, strlen($target_parent) + 1)
            );
        ...
        if (array_key_exists($path, $tested_paths)) {
            ...
            if (!wp_mkdir_p($path)) {
                if (str_starts_with($uploads["basedir"], ABSPATH)) {
                    $error_path =
                        str_replace(ABSPATH, "", $uploads["basedir"]) .
                        $uploads["subdir"];
                } else {
                    $error_path =
                        wp_basename($uploads["basedir"]) . $uploads["subdir"];
                }
...
function wp_unique_filename($dir, $filename, $unique_filename_callback = null)
{
    ...
    if ($unique_filename_callback && is_callable($unique_filename_callback)) {
        ...
        if (
            ...
            $ext &&
            @is_dir($dir) &&
            str_contains($dir, $upload_dir["basedir"])
        ) {
            /**
         ...
         */
        if ($is_image) {
            $output_formats = wp_get_image_editor_output_format(
                $_dir . $filename,
                $mime_type
            );
            $alt_types = [];
            ...
            if (!empty($output_formats[$mime_type])) {
                ...
                $alt_types = array_keys(
                    array_intersect($output_formats, [
                        $mime_type,
                        $alt_mime_type,
                    ])
                );
                ...
                $alt_types = array_keys(
                    array_intersect($output_formats, [$mime_type])
                );
            }
...
function wp_upload_bits($name, $deprecated, $bits, $time = null)
{
    ...
    if (!wp_mkdir_p(dirname($new_file))) {
        if (str_starts_with($upload["basedir"], ABSPATH)) {
            $error_path =
                str_replace(ABSPATH, "", $upload["basedir"]) .
                $upload["subdir"];
        } else {
...
function wp_get_default_extension_for_mime_type($mime_type)
{
    $extensions = explode(
        "|",
        array_search($mime_type, wp_get_mime_types(), true)
    );
...
function wp_check_filetype_and_ext($file, $filename, $mimes = null)
{
    ...
    if ($type && str_starts_with($type, "image/")) {
        ...
        $heic_images_extensions = ["heif", "heics", "heifs"];

        if (
            $real_mime &&
            ($real_mime !== $type ||
                in_array($ext, $heic_images_extensions, true))
        ) {
            /**
    ...
    if ($type && !$real_mime && extension_loaded("fileinfo")) {
        ...
        if (in_array($real_mime, $nonspecific_types, true)) {
             ...
             */
            if (
                substr($real_mime, 0, strcspn($real_mime, "/")) !==
                substr($type, 0, strcspn($type, "/"))
            ) {
                $type = false;
                $ext = false;
            ...
            if (
                !in_array(
                    $type,
                    ["text/rtf", "text/plain", "application/rtf"],
                    true
                )
            ) {
                $type = false;
...
function wp_get_image_mime($file)
{
    ...
    try {
        ...
        if (
            ...
            if (
                "avif" === hex2bin($magic[2]) ||
                "avis" === hex2bin($magic[2])
            ) {
                $mime = "image/avif";
...
function wp_nonce_ays($action)
{
    ...
    if ("log-out" === $action) {
        ...
        if (wp_get_referer()) {
            ...
            $wp_http_referer = wp_validate_redirect(
                sanitize_url($wp_http_referer)
            );
...
{
    list($message, $title, $parsed_args) = _wp_die_process_input(
        $message,
        $title,
        $args
    );
    ...
    if (!did_action("admin_head")):
        ...
        if (!headers_sent()) {
            header(
                "Content-Type: text/html; charset={$parsed_args["charset"]}"
            );
         ...
         */
        if (
            empty($args["text_direction"]) &&
            function_exists("language_attributes") &&
...
<head>
	<meta http-equiv="Content-Type" content="text/html; charset=<?php echo $parsed_args[
     "charset"
 ]; ?>" />
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
		<?php if (
      function_exists("wp_robots") &&
      function_exists("wp_robots_no_robots") &&
...
function _json_wp_die_handler($message, $title = "", $args = [])
{
    ...
    if (!headers_sent()) {
        header(
            "Content-Type: application/json; charset={$parsed_args["charset"]}"
        );
...
function _jsonp_wp_die_handler($message, $title = "", $args = [])
{
    ...
    if (!headers_sent()) {
        header(
            "Content-Type: application/javascript; charset={$parsed_args["charset"]}"
        );
...
function _wp_die_process_input($message, $title = "", $args = [])
{
    ...
    if (
        empty($args["text_direction"]) ||
        !in_array($args["text_direction"], ["ltr", "rtl"], true)
    ) {
...
function _mce_set_direction($mce_init)
{
    if (is_rtl()) {
        ...
        if (
            !empty($mce_init["plugins"]) &&
            !str_contains($mce_init["plugins"], "directionality")
        ) {
            $mce_init["plugins"] .= ",directionality";
        }

        if (
            !empty($mce_init["toolbar1"]) &&
            !preg_match("/\bltr\b/", $mce_init["toolbar1"])
        ) {
...
function _wp_to_kebab_case($input_string)
{
    ...
    $rsMisc =
        "[^" . $rsBreakRange . $rsDigits . $rsLowerRange . $rsUpperRange . "]";
    ...
    $regexp =
        ...
        implode("|", [
            ...
            "+" .
            "(?=" .
            implode("|", [$rsBreak, $rsUpper, '$']) .
            ")",
            $rsMiscUpper .
            "+" .
            "(?=" .
            implode("|", [$rsBreak, $rsUpper . $rsMiscLower, '$']) .
            ")",
            $rsUpper . "?" . $rsMiscLower . "+",
            $rsUpper . "+",
            $rsOrdUpper,
...
function dead_db()
{
    ...
    // Otherwise, be terse.
    wp_die(
        "<h1>" . __("Error establishing a database connection") . "</h1>",
        __("Database Error")
    );
}
...
function _deprecated_constructor($class_name, $version, $parent_class = "")
{
    ...
    if (
        WP_DEBUG &&
        apply_filters("deprecated_constructor_trigger_error", true)
    ) {
        if (function_exists("__")) {
            if ($parent_class) {
                $message = sprintf(
    ...
    if (
        ...
        if (function_exists("__")) {
            if ($version) {
                ...
                $version = sprintf(
                    __("(This message was added in version %s.)"),
                    $version
                );
            }
...
{
    $server_parts = explode(
        "/",
        isset($_SERVER["SERVER_SOFTWARE"]) ? $_SERVER["SERVER_SOFTWARE"] : ""
    );
    $server_parts[1] = isset($server_parts[1]) ? $server_parts[1] : "";

    return "lighttpd" === $server_parts[0] &&
        -1 === version_compare($server_parts[1], "1.5.0");
}
    ...
    if (defined("WP_SITEURL") && "" !== WP_SITEURL) {
        ...
        if (
            ...
            if (str_contains($_SERVER["SCRIPT_FILENAME"], $abspath_fix)) {
                ...
                $subdirectory = substr(
                    $abspath_fix,
                    strpos($abspath_fix, $script_filename_dir) +
                        strlen($script_filename_dir)
                );
                // Strip off any file/query params from the path, appending the subdirectory to the installation.
                $path =
                    preg_replace('#/[^/]*$#i', "", $_SERVER["REQUEST_URI"]) .
                    $subdirectory;
            } else {
...
function is_site_meta_supported()
{
    ...
    if (false === $supported) {
        $supported = $wpdb->get_var("SHOW TABLES LIKE '{$wpdb->blogmeta}'")
            ? 1
            : 0;
...
function wp_timezone_override_offset()
{
    ...
    return round(
        timezone_offset_get($timezone_object, $datetime_object) /
            HOUR_IN_SECONDS,
        2
    );
}
...
function _wp_timezone_choice_usort_callback($a, $b)
{
    ...
    if ("Etc" === $a["continent"] && "Etc" === $b["continent"]) {
        ...
        if (
            str_starts_with($a["city"], "GMT+") &&
            str_starts_with($b["city"], "GMT+")
        ) {
...
function wp_timezone_choice($selected_zone, $locale = null)
{
    ...
    foreach ($tz_identifiers as $zone) {
        ...
        $zonen[] = [
            ...
            "t_continent" => $exists[3]
                ? translate(
                    str_replace("_", " ", $zone[0]),
                    "continents-cities"
                )
                : "",
            "t_city" => $exists[4]
                ? translate(
                    str_replace("_", " ", $zone[1]),
                    "continents-cities"
                )
                : "",
            "t_subcity" => $exists[5]
                ? translate(
                    str_replace("_", " ", $zone[2]),
                    "continents-cities"
                )
                : "",
        ];
    ...
    if (
        in_array($selected_zone, $tz_identifiers, true) === false &&
        in_array(
            $selected_zone,
        ...
        $structure[] =
            '<option selected="selected" value="' .
            esc_attr($selected_zone) .
            '">' .
            esc_html($selected_zone) .
            "</option>";
    }
...
function get_file_data($file, $default_headers, $context = "")
{
    ...
    $extra_headers = $context
        ? apply_filters("extra_{$context}_headers", [])
        : [];
    if ($extra_headers) {
...
function wp_find_hierarchy_loop(
    ...
    $arbitrary_loop_member = wp_find_hierarchy_loop_tortoise_hare(
        $callback,
        $start,
        $override,
        $callback_args
    ...
    while (
        ...
        ($evanescent_hare = isset($override[$hare])
            ...
            : call_user_func_array(
                $callback,
                array_merge([$hare], $callback_args)
            )) &&
        ($hare = isset($override[$evanescent_hare])
            ? $override[$evanescent_hare]
            : call_user_func_array(
                $callback,
                array_merge([$evanescent_hare], $callback_args)
            ))
    ) {
        if ($_return_loop) {
        ...
        $tortoise = isset($override[$tortoise])
            ...
            : call_user_func_array(
                $callback,
                array_merge([$tortoise], $callback_args)
            );
    }
...
function wp_allowed_protocols()
{
    ...
    if (!did_action("wp_loaded")) {
         ...
         */
        $protocols = array_unique(
            (array) apply_filters("kses_allowed_protocols", $protocols)
        );
    }
...
function _validate_cache_id($object_id)
{
    ...
    $message = sprintf(
        __("Object ID must be an integer, %s given."),
        gettype($object_id)
    );
...
function _device_can_upload()
{
    ...
    if (
        ...
        return preg_match("#OS ([\d_]+) like Mac OS X#", $ua, $version) &&
            version_compare($version[1], "6", ">=");
    }
  ...
  _e("Close dialog"); ?>
	...
	<?php if ($same_domain) {
     $login_src = add_query_arg(
		...
		<div id="wp-auth-check-form" class="loading" data-src="<?php echo esc_url(
      $login_src
  ); ?>"></div>
		<?php
 } ?>
	<div class="wp-auth-fallback">
		<p><b class="wp-auth-fallback-expired" tabindex="0"><?php _e(
      "Session expired"
  ); ?></b></p>
		<p><a href="<?php echo esc_url($login_url); ?>" target="_blank"><?php _e(
    "Please log in again."
); ?></a>
		<?php _e(
...
{
    $response["wp-auth-check"] =
        is_user_logged_in() && empty($GLOBALS["login_grace_period"]);
    return $response;
}
...
function wp_raise_memory_limit($context = "admin")
{
    ...
    if (
        -1 === $filtered_limit_int ||
        ($filtered_limit_int > $wp_max_limit_int &&
            $filtered_limit_int > $current_limit_int)
    ) {
        ...
        -1 === $wp_max_limit_int ||
        $wp_max_limit_int > $current_limit_int
    ) {
...
function wp_is_uuid($uuid, $version = null)
{
    ...
    if (is_numeric($version)) {
        if (4 !== (int) $version) {
            _doing_it_wrong(
                __FUNCTION__,
                __("Only UUID V4 is supported at this time."),
                "4.9.0"
            );
            return false;
        }
...
function wp_unique_prefixed_id($prefix = "")
{
    ...
    if (!is_string($prefix)) {
        wp_trigger_error(
            __FUNCTION__,
            sprintf(
                'The prefix must be a string. "%s" data type given.',
                gettype($prefix)
            )
        );
        $prefix = "";
...
###SITEURL###'
     ...
     * @param string $new_email The new site admin email address.
     */
    $email_change_email = apply_filters(
...
function wp_privacy_exports_dir()
{
    ...
    $exports_dir =
        trailingslashit($upload_dir["basedir"]) . "wp-personal-data-exports/";
...
function wp_privacy_exports_url()
{
    $upload_dir = wp_upload_dir();
    $exports_url =
        trailingslashit($upload_dir["baseurl"]) . "wp-personal-data-exports/";
...
function wp_get_direct_php_update_url()
{
     ...
     */
    $direct_update_url = apply_filters(
...
function recurse_dirsize(
    ...
    if (
        isset($directory_cache[$directory]) &&
        is_int($directory_cache[$directory])
    ) {
...
function wp_get_admin_notice($message, $args = [])
{
    ...
    if (
        is_array($args["additional_classes"]) &&
...
function wp_fast_hash(#[\SensitiveParameter] string $message): string
{
    ...
    return '$generic$' .
        sodium_bin2base64($hashed, SODIUM_BASE64_VARIANT_URLSAFE_NO_PADDING);
}
