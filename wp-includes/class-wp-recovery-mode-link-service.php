...
class WP_Recovery_Mode_Link_Service
{
    ...
    public function handle_begin_link($ttl)
    {
        ...
        if (
            !isset($_GET["action"], $_GET["rm_token"], $_GET["rm_key"]) ||
            self::LOGIN_ACTION_ENTER !== $_GET["action"]
        ) {
            return;
        }
        ...
        $validated = $this->key_service->validate_recovery_mode_key(
            $_GET["rm_token"],
            $_GET["rm_key"],
            $ttl
        );
