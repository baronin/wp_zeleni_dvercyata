...
function image_downsize($id, $size = "medium")
{
    ...
    if ($intermediate) {
        ...
        $thumbfile = str_replace(
            wp_basename($imagefile),
            wp_basename($meta["thumb"]),
            $imagefile
        );
...
function wp_constrain_dimensions(
    ...
    if (
        (int) round($current_width * $larger_ratio) > $max_width ||
        (int) round($current_height * $larger_ratio) > $max_height
    ) {
...
function image_resize_dimensions(
    ...
    if (
        wp_fuzzy_number_match($new_w, $orig_w) &&
        wp_fuzzy_number_match($new_h, $orig_h)
    ) {
...
function image_make_intermediate_size($file, $width, $height, $crop = false)
{
    if ($width || $height) {
        ...
        if (
            is_wp_error($editor) ||
            is_wp_error($editor->resize($width, $height, $crop))
        ) {
            return false;
        }
...
function wp_image_matches_ratio(
    ...
    $matched =
        wp_fuzzy_number_match($constrained_size[0], $expected_size[0]) &&
...
function image_get_intermediate_size($post_id, $size = "thumbnail")
{
    ...
    if (is_array($size)) {
        ...
        foreach ($imagedata["sizes"] as $_size => $data) {
            ...
            if (
                (int) $data["width"] === (int) $size[0] &&
                (int) $data["height"] === (int) $size[1]
            ) {
        ...
        if (!empty($candidates)) {
            ...
            $imagedata["sizes"]["thumbnail"]["width"] >= $size[0] &&
            $imagedata["sizes"]["thumbnail"]["width"] >= $size[1]
        ) {
        ...
        list($data["width"], $data["height"]) = image_constrain_size_for_editor(
            $data["width"],
            $data["height"],
            $size
        );
...
function wp_get_registered_image_subsizes()
{
    ...
    foreach (get_intermediate_image_sizes() as $size_name) {
        ...
        if (isset($additional_sizes[$size_name]["height"])) {
            $size_data["height"] =
                (int) $additional_sizes[$size_name]["height"];
        } else {
...
function wp_get_attachment_image(
    ...
    if ($image) {
        ...
        if (
            empty($attr["decoding"]) ||
            !in_array($attr["decoding"], ["async", "sync", "auto"], true)
        ) {
            unset($attr["decoding"]);
        }
...
function _wp_get_attachment_relative_path($file)
{
    ...
    if (str_contains($dirname, "wp-content/uploads")) {
        ...
        $dirname = substr(
            $dirname,
            strpos($dirname, "wp-content/uploads") + 18
        );
...
function wp_get_attachment_image_srcset(
    ...
    return wp_calculate_image_srcset(
        $size_array,
        $image_src,
        $image_meta,
        $attachment_id
...
function wp_calculate_image_srcset(
     ...
     */
    if (
        !isset($image_sizes["thumbnail"]["mime-type"]) ||
     ...
     */
    $image_edited = preg_match(
        "/-e[0-9]{13}/",
        wp_basename($image_src),
        $image_edit_hash
    );
    ...
    foreach ($image_sizes as $image) {
        ...
        if (
            !$src_matched &&
            str_contains($image_src, $dirname . $image["file"])
        ) {
            $src_matched = true;
            $is_src = true;
        }
        ...
        if (
            $max_srcset_image_width &&
            $image["width"] > $max_srcset_image_width &&
            !$is_src
        ) {
            continue;
        }
...
function wp_get_attachment_image_sizes(
    ...
    return wp_calculate_image_sizes(
        $size_array,
        $image_src,
        $image_meta,
        $attachment_id
...
function wp_image_file_matches_image_meta(
    ...
    if (isset($image_meta["file"]) && strlen($image_meta["file"]) > 4) {
        ...
        if (
            strrpos($image_location, $image_meta["file"]) ===
            strlen($image_location) - strlen($image_meta["file"])
        ) {
            $match = true;
        } else {
            // Retrieve the uploads sub-directory from the full size image.
            ...
            if (!empty($image_meta["original_image"])) {
                ...
                if (
                    strrpos($image_location, $relative_path) ===
                    strlen($image_location) - strlen($relative_path)
                ) {
                    $match = true;
                }
            }
...
function wp_image_add_srcset_and_sizes($image, $image_meta, $attachment_id)
{
    ...
    if ($width && $height) {
        ...
        $size_array = wp_image_src_get_dimensions(
            $image_src,
            $image_meta,
            $attachment_id
        );
...
function wp_filter_content_tags($content, $context = null)
{
    ...
    foreach ($matches as $match) {
        ...
        if (isset($images[$match[0]])) {
            ...
            if (
                $attachment_id > 0 &&
                !str_contains($filtered_image, " width=") &&
                !str_contains($filtered_image, " height=")
            ) {
                $filtered_image = wp_img_tag_add_width_and_height_attr(
                    $filtered_image,
                    $context,
                    $attachment_id
                );
            ...
            if (
                $attachment_id > 0 &&
                !str_contains($filtered_image, " srcset=")
            ) {
                $filtered_image = wp_img_tag_add_srcset_and_sizes_attr(
                    $filtered_image,
                    $context,
                    $attachment_id
                );
            ...
            $filtered_image = wp_img_tag_add_loading_optimization_attrs(
                $filtered_image,
                $context
            );
        ...
        if (isset($iframes[$match[0]])) {
            ...
            if (
                ...
                !str_contains($filtered_iframe, " loading=")
            ) {
                $filtered_iframe = wp_iframe_tag_add_loading_attr(
                    $filtered_iframe,
                    $context
                );
            }
...
function wp_img_tag_add_auto_sizes(string $image): string
{
    ...
    if (
        !is_string($loading) ||
        "lazy" !== strtolower(trim($loading, " \t\f\r\n"))
    ) {
        return $image;
    }
...
{
    $src = preg_match('/ src=["\']?([^"\']*)/i', $image, $matche_src)
        ? $matche_src[1]
        : null;
    $width = preg_match('/ width=["\']([0-9]+)["\']/', $image, $match_width)
        ? (int) $match_width[1]
    ...
    $loading_val = preg_match(
        ...
        ? $match_loading[1]
        : null;
    $fetchpriority_val = preg_match(
        '/ fetchpriority=["\']([A-Za-z]+)["\']/',
        $image,
        $match_fetchpriority
    )
        ? $match_fetchpriority[1]
        : null;
    $decoding_val = preg_match(
        '/ decoding=["\']([A-Za-z]+)["\']/',
        $image,
        $match_decoding
    )
    ...
    if (empty($decoding_val)) {
        ...
        $filtered_decoding_attr = apply_filters(
            ...
            isset($optimization_attrs["decoding"])
                ? $optimization_attrs["decoding"]
                : false,
            $image,
            $context
        );
        ...
        if (
            ...
            in_array($filtered_decoding_attr, ["async", "sync", "auto"], true)
        ) {
        ...
        if (!empty($optimization_attrs["decoding"])) {
            $image = str_replace(
                "<img",
                '<img decoding="' .
                    esc_attr($optimization_attrs["decoding"]) .
                    '"',
                $image
            );
        }
    ...
    if (
        !str_contains($image, ' width="') ||
        !str_contains($image, ' height="')
    ) {
        return $image;
    }
    ...
    if (empty($loading_val) && $loading_attrs_enabled) {
        ...
        $filtered_loading_attr = apply_filters(
            ...
            isset($optimization_attrs["loading"])
                ? $optimization_attrs["loading"]
                : false,
            $image,
            $context
        );
        ...
        if (isset($optimization_attrs["loading"]) && !$filtered_loading_attr) {
             ...
             */
            if (
                isset($optimization_attrs["fetchpriority"]) &&
                "high" === $optimization_attrs["fetchpriority"] &&
                (isset($optimization_attrs["loading"])
                    ? $optimization_attrs["loading"]
                    : false) !== $filtered_loading_attr &&
        ...
        if (!empty($optimization_attrs["loading"])) {
            $image = str_replace(
                "<img",
                '<img loading="' .
                    esc_attr($optimization_attrs["loading"]) .
                    '"',
                $image
            );
        }
    ...
    if (
        empty($fetchpriority_val) &&
        !empty($optimization_attrs["fetchpriority"])
    ) {
        $image = str_replace(
            "<img",
            '<img fetchpriority="' .
                esc_attr($optimization_attrs["fetchpriority"]) .
                '"',
            $image
        );
    }
    ...
    if (true === $add) {
        ...
        if ($size_array && $size_array[0] && $size_array[1]) {
            ...
            $style_width = preg_match(
                ...
                : 0;
            if ($style_width) {
                $size_array[1] = (int) round(
                    ($size_array[1] * $style_width) / $size_array[0]
                );
...
function wp_img_tag_add_srcset_and_sizes_attr($image, $context, $attachment_id)
{
    ...
    if (true === $add) {
        $image_meta = wp_get_attachment_metadata($attachment_id);
        return wp_image_add_srcset_and_sizes(
            $image,
            $image_meta,
            $attachment_id
        );
...
function wp_iframe_tag_add_loading_attr($iframe, $context)
{
    ...
    if (
        !str_contains($iframe, ' src="') ||
        !str_contains($iframe, ' width="') ||
        !str_contains($iframe, ' height="')
    ) {
        return $iframe;
    }

    $value = isset($optimization_attrs["loading"])
        ? $optimization_attrs["loading"]
        : false;
...
function img_caption_shortcode($attr, $content = "")
{
    ...
    if ($atts["caption_id"]) {
        ...
        $describedby =
            'aria-describedby="' . esc_attr($atts["caption_id"]) . '" ';
    }
...
function gallery_shortcode($attr)
{
    ...
    if (!empty($post_parent_id)) {
        ...
        if (
            (!is_post_publicly_viewable($post_parent->ID) &&
                !current_user_can("read_post", $post_parent->ID)) ||
    ...
    $size_class = sanitize_html_class(
        is_array($atts["size"]) ? implode("x", $atts["size"]) : $atts["size"]
    );
    ...
    foreach ($attachments as $id => $attachment) {
        ...
        if (isset($image_meta["height"], $image_meta["width"])) {
            $orientation =
                $image_meta["height"] > $image_meta["width"]
                    ? "portrait"
                    : "landscape";
        }
	...
	<div class="wp-playlist-caption">
		<span class="wp-playlist-item-meta wp-playlist-item-title">
			<# if ( data.meta.album || data.meta.artist ) { #>
				<?php /* translators: %s: Playlist item title. */
    printf(
        _x("&#8220;%s&#8221;", "playlist item title"),
        "{{ data.title }}"
    ); ?>
			<# } else { #>
				{{ data.title }}
...
function wp_playlist_scripts($type)
{
    ...
    ?>
<!--[if lt IE 9]><script>document.createElement('<?php echo esc_js(
    $type
); ?>');</script><![endif]-->
	<?php
...
function wp_playlist_shortcode($attr)
{
    ...
    foreach ($attachments as $attachment) {
        ...
        if (!empty($meta)) {
            foreach (
                wp_get_attachment_id3_keys($attachment)
                as $key => $label
            ) {
                if (!empty($meta[$key])) {
...
<div class="wp-playlist wp-<?php echo $safe_type; ?>-playlist wp-playlist-<?php echo $safe_style; ?>">
	...
	<script type="application/json" class="wp-playlist-script"><?php echo wp_json_encode(
     $data
 ); ?></script>
</div>
...
function wp_get_audio_extensions()
{
     ...
     */
    return apply_filters("wp_audio_extensions", [
        "mp3",
        "ogg",
        "flac",
        "m4a",
...
function wp_audio_shortcode($attr, $content = "")
{
    ...
    foreach (["loop", "autoplay", "preload", "muted"] as $a) {
    ...
    foreach ($html_atts as $attribute_name => $attribute_value) {
        if (
            in_array($attribute_name, ["loop", "autoplay", "muted"], true) &&
            ...
            $allowed_preload_values = ["none", "metadata", "auto"];
...
function wp_get_video_extensions()
{
     ...
     */
    return apply_filters("wp_video_extensions", [
        "mp4",
        "m4v",
        "webm",
        "ogv",
...
function wp_video_shortcode($attr, $content = "")
{
    ...
    if (is_admin()) {
        ...
        if ($atts["width"] > $defaults_atts["width"]) {
            $atts["height"] = round(
                ($atts["height"] * $defaults_atts["width"]) / $atts["width"]
            );
        ...
        if (!empty($content_width) && $atts["width"] > $content_width) {
            $atts["height"] = round(
                ($atts["height"] * $content_width) / $atts["width"]
            );
    ...
    $output = sprintf(
        '<div style="%s" class="wp-video">%s</div>',
        $width_rule,
        $html
    );
...
function get_attachment_taxonomies($attachment, $output = "names")
{
    ...
    if (str_contains($filename, ".")) {
        $objects[] =
            "attachment:" . substr($filename, strrpos($filename, ".") + 1);
    }
    ...
    foreach (get_taxonomies([], "objects") as $taxonomy) {
        foreach ($taxonomy->object_type as $object_type) {
            if (
                ...
                str_starts_with($object_type, "attachment:")
            ) {
                if ("names" === $output) {
...
function wp_expand_dimensions(
    ...
    return wp_constrain_dimensions(
        $example_width * 1000000,
        $example_height * 1000000,
        $max_width,
        $max_height
    );
}
...
function wp_get_image_editor($path, $args = [])
{
    ...
    if (isset($args["mime_type"])) {
        $output_format = wp_get_image_editor_output_format(
            $path,
            $args["mime_type"]
        );
...
function _wp_image_editor_choose($args = [])
{
    ...
    $cache_key = md5(serialize([$args, $implementations]));
    ...
    foreach ($implementations as $implementation) {
        ...
        if (
            ...
            !call_user_func(
                [$implementation, "supports_mime_type"],
                $args["mime_type"]
            )
        ) {
            continue;
...
function wp_plupload_default_settings()
{
    ...
    if (!wp_image_editor_supports(["mime_type" => "image/heic"])) {
...
function wp_prepare_attachment_for_js($attachment)
{
    ...
    if ($author->exists()) {
        $author_name = $author->display_name
            ? $author->display_name
            : $author->nickname;
        $response["authorName"] = html_entity_decode(
            $author_name,
            ENT_QUOTES,
    ...
    if (current_user_can("edit_post", $attachment->ID)) {
        ...
        $response["nonces"]["edit"] = wp_create_nonce(
            "image_editor-" . $attachment->ID
        );
        $response["editLink"] = get_edit_post_link($attachment->ID, "raw");
    }

    if (current_user_can("delete_post", $attachment->ID)) {
        $response["nonces"]["delete"] = wp_create_nonce(
            "delete-post_" . $attachment->ID
        );
    }
    ...
    if ($meta && ("image" === $type || !empty($meta["sizes"]))) {
        ...
        foreach ($possible_sizes as $size => $label) {
            ...
            if ($downsize) {
                ...
                $sizes[$size] = [
                    ...
                    "url" => $downsize[0],
                    "orientation" =>
                        $downsize[2] > $downsize[1] ? "portrait" : "landscape",
                ];
        ...
        if ("image" === $type) {
            if (!empty($meta["original_image"])) {
                $response["originalImageURL"] = wp_get_original_image_url(
                    $attachment->ID
                );
                $response["originalImageName"] = wp_basename(
                    wp_get_original_image_path($attachment->ID)
                );
            }
            ...
            $sizes["full"] = [
                ...
                "width" => $meta["sizes"]["full"]["width"],
                "orientation" =>
                    $meta["sizes"]["full"]["height"] >
                    $meta["sizes"]["full"]["width"]
                        ? "portrait"
                        : "landscape",
            ];
        }
    ...
    if ($meta && ("audio" === $type || "video" === $type)) {
        ...
        $response["meta"] = [];
        foreach (
            wp_get_attachment_id3_keys($attachment, "js")
            as $key => $label
        ) {
...
function wp_enqueue_media($args = [])
{
     ...
     */
    $show_audio_playlist = apply_filters(
     ...
     */
    $show_video_playlist = apply_filters(
     ...
     */
    $infinite_scrolling = apply_filters(
    ...
    $settings = [
        ...
        "nonce" => [
            ...
            "setAttachmentThumbnail" => wp_create_nonce(
    ...
    if (isset($args["post"])) {
        ...
        $thumbnail_support =
            current_theme_supports("post-thumbnails", $post->post_type) &&
        ...
        if ($thumbnail_support) {
            $featured_image_id = get_post_meta(
                $post->ID,
                "_thumbnail_id",
                true
            );
            $settings["post"]["featuredImageId"] = $featured_image_id
                ? $featured_image_id
                : -1;
        }
    }
    ...
    $strings = [
        ...
        "trash" => _x("Trash", "noun"),
        "uploadedToThisPost" =>
            $post_type_object->labels->uploaded_to_this_item,
        "warnDelete" => __(
...
function get_media_embedded_in_content($content, $types = null)
{
    ...
    $allowed_media_types = apply_filters(
        ...
        ["audio", "video", "object", "embed", "iframe"]
    );
    ...
    if (has_block("gallery", $post->post_content)) {
        ...
        while ($block = array_shift($post_blocks)) {
            ...
            if ($has_inner_blocks && $html) {
                ...
                $galleries[] =
                    "<figure>" . implode(" ", $block_html) . "</figure>";
                continue;
            }
...
function wp_maybe_generate_attachment_metadata($attachment)
{
    ...
    if (empty($meta) && file_exists($file)) {
        ...
        if (
            !array_key_exists("_wp_attachment_metadata", $_meta) &&
            !get_transient($_lock)
        ) {
...
function attachment_url_to_postid($url)
{
    ...
    if (
        isset($image_path["scheme"]) &&
        $image_path["scheme"] !== $site_url["scheme"]
    ) {
...
function wpview_media_sandbox_styles()
{
    ...
    $mediaelement = includes_url(
        "js/mediaelement/mediaelementplayer-legacy.min.css?$version"
    );
    $wpmediaelement = includes_url(
        "js/mediaelement/wp-mediaelement.css?$version"
    );
...
function wp_get_loading_optimization_attributes($tag_name, $attr, $context)
{
    ...
    if (
        ...
        ("widget_text_content" !== $context &&
            doing_filter("widget_text_content")) ||
        ("widget_block_content" !== $context &&
    ...
    if (null === $maybe_in_viewport) {
         ...
         */
        $header_enforced_contexts = apply_filters(
    ...
    if ($maybe_in_viewport) {
        $loading_attrs = wp_maybe_add_fetchpriority_high_attr(
            $loading_attrs,
            $tag_name,
            $attr
        );
    ...
    if ($increase_count) {
        ...
        /** This filter is documented in wp-includes/media.php */
        $wp_min_priority_img_pixels = apply_filters(
