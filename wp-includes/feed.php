...
function get_wp_title_rss($deprecated = "&#8211;")
{
    if ("&#8211;" !== $deprecated) {
        ...
        _deprecated_argument(
            ...
            "4.4.0",
            sprintf(
                __("Use the %s filter instead."),
                "<code>document_title_separator</code>"
            )
        );
    }
        ...
        if ("enclosure" === $key) {
            foreach ((array) $val as $enc) {
                ...
                echo apply_filters(
                    "rss_enclosure",
                    '<enclosure url="' .
                        esc_url(trim($enclosure[0])) .
                        '" length="' .
                        absint(trim($enclosure[1])) .
                        '" type="' .
                        esc_attr($type) .
                        '" />' .
                        "\n"
...
function get_self_link()
{
    ...
    return set_url_scheme(
        "http://" . $domain . wp_unslash($_SERVER["REQUEST_URI"])
    );
}
