...
function wp_style_engine_get_styles($block_styles, $options = [])
{
    ...
    $parsed_styles = WP_Style_Engine::parse_block_styles(
        $block_styles,
        $options
    );
    ...
    if (!empty($parsed_styles["declarations"])) {
        $styles_output["css"] = WP_Style_Engine::compile_css(
    ...
    if (!empty($parsed_styles["classnames"])) {
        $styles_output["classnames"] = implode(
            " ",
...
{
    return WP_Style_Engine::compile_stylesheet_from_css_rules(
        WP_Style_Engine::get_store($context)->get_all_rules(),
        $options
    );
}
