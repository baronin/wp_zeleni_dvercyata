...
function wp_get_speculation_rules(): ?WP_Speculation_Rules
{
    ...
    if (get_option("permalink_structure")) {
        $base_href_exclude_paths[] = $prefixer->prefix_path_pattern(
            "/*\\?(.+)",
            "home"
        );
    } else {
     ...
     */
    $href_exclude_paths = (array) apply_filters(
    ...
    $href_exclude_paths = array_values(
        array_unique(
            array_merge(
                $base_href_exclude_paths,
                array_map(static function (string $href_exclude_path) use (
                    $prefixer
                ): string {
    ...
    $speculation_rules->add_rule($mode, "main", [
        "source" => "document",
        "where" => [
...
function wp_print_speculation_rules(): void
{
    ...
    wp_print_inline_script_tag((string) wp_json_encode($speculation_rules), [
