...
function register_taxonomy($taxonomy, $object_type, $args = [])
{
    ...
    if (!empty($taxonomy_object->default_term)) {
        ...
        if ($term) {
            update_option(
                "default_term_" . $taxonomy_object->name,
                $term["term_id"]
            );
        } else {
            $term = wp_insert_term(
                ...
                [
                    "slug" => sanitize_title(
                        $taxonomy_object->default_term["slug"]
                    ),
                    "description" =>
                        $taxonomy_object->default_term["description"],
                ]
            );
...
function unregister_taxonomy($taxonomy)
{
    ...
    if ($taxonomy_object->_builtin) {
        return new WP_Error(
            "invalid_taxonomy",
            __("Unregistering a built-in taxonomy is not allowed.")
        );
    }
...
function get_taxonomy_labels($tax)
{
    ...
    if (!isset($tax->labels->template_name) && isset($labels->singular_name)) {
        ...
        $labels->template_name = sprintf(
            _x("%s Archives", "taxonomy template name"),
...
function register_taxonomy_for_object_type($taxonomy, $object_type)
{
    ...
    // Filter out empties.
    $wp_taxonomies[$taxonomy]->object_type = array_filter(
    ...
    foreach ((array) $terms[$term_id] as $child) {
        ...
        if (isset($terms[$child])) {
            $children = array_merge(
                $children,
                get_term_children($child, $taxonomy)
            );
        }
    }
...
function get_terms($args = [], $deprecated = "")
{
    ...
    $key_intersect = array_intersect_key(
        $term_query->query_var_defaults,
        (array) $_args
    );
...
function has_term_meta($term_id)
{
    ...
    return $wpdb->get_results(
        $wpdb->prepare(
            "SELECT meta_key, meta_value, meta_id, term_id FROM $wpdb->termmeta WHERE term_id = %d ORDER BY meta_key,meta_id",
            $term_id
        ),
        ARRAY_A
    );
...
function term_is_ancestor_of($term1, $term2, $taxonomy)
{
    ...
    return term_is_ancestor_of(
        $term1,
        get_term($term2->parent, $taxonomy),
        $taxonomy
    );
}
...
function sanitize_term($term, $taxonomy, $context = "display")
{
    ...
    foreach ((array) $fields as $field) {
        if ($do_object) {
            if (isset($term->$field)) {
                $term->$field = sanitize_term_field(
                    $field,
                    $term->$field,
                    $term_id,
                    $taxonomy,
...
function wp_delete_object_term_relationships($object_id, $taxonomies)
{
    ...
    foreach ((array) $taxonomies as $taxonomy) {
        $term_ids = wp_get_object_terms($object_id, $taxonomy, [
            "fields" => "ids",
        ]);
        $term_ids = array_map("intval", $term_ids);
...
function wp_delete_term($term, $taxonomy, $args = [])
{
    ...
    if (is_taxonomy_hierarchical($taxonomy)) {
        ...
        $wpdb->update(
            $wpdb->term_taxonomy,
            compact("parent"),
            ["parent" => $term_obj->term_id] + compact("taxonomy")
        );
...
function wp_get_object_terms($object_ids, $taxonomies, $args = [])
{
    ...
    if (!empty($taxonomies)) {
        ...
        if (
            !empty($args["fields"]) &&
            str_starts_with($args["fields"], "id=>")
        ) {
...
function wp_insert_term($term, $taxonomy, $args = [])
{
    ...
    if ($args["alias_of"]) {
        ...
        if (!empty($alias->term_group)) {
             ...
             */
            $term_group =
                $wpdb->get_var("SELECT MAX(term_group) FROM $wpdb->terms") + 1;
    ...
    if ($name_match) {
        ...
        if (!$slug_provided || $name_match->slug === $slug || $slug_match) {
            if (is_taxonomy_hierarchical($taxonomy)) {
                ...
                if (
                    (!$slug_provided || $name_match->slug === $slug) &&
                    in_array($name, $sibling_names, true)
                ) {
                    $existing_term = $name_match;
                } elseif (
                    $slug_match &&
                    in_array($slug, $sibling_slugs, true)
                ) {
    ...
    $tt_id = $wpdb->get_var(
        $wpdb->prepare(
            "SELECT tt.term_taxonomy_id FROM $wpdb->term_taxonomy AS tt INNER JOIN $wpdb->terms AS t ON tt.term_id = t.term_id WHERE tt.taxonomy = %s AND t.term_id = %d",
            $taxonomy,
            $term_id
        )
    );
    ...
    if (
        ...
        return new WP_Error(
            "db_insert_error",
            __("Could not insert term taxonomy into the database."),
            $wpdb->last_error
        );
    }
    ...
    $duplicate_term = $wpdb->get_row(
        $wpdb->prepare(
            "SELECT t.term_id, t.slug, tt.term_taxonomy_id, tt.taxonomy FROM $wpdb->terms AS t INNER JOIN $wpdb->term_taxonomy AS tt ON ( tt.term_id = t.term_id ) WHERE t.slug = %s AND tt.parent = %d AND tt.taxonomy = %s AND t.term_id < %d AND tt.term_taxonomy_id != %d",
            $slug,
            $parent,
            $taxonomy,
            $term_id,
    ...
    if (!$append) {
        ...
        if ($delete_tt_ids) {
            ...
            $delete_term_ids = $wpdb->get_col(
                $wpdb->prepare(
                    "SELECT tt.term_id FROM $wpdb->term_taxonomy AS tt WHERE tt.taxonomy = %s AND tt.term_taxonomy_id IN ($in_delete_tt_ids)",
                    $taxonomy
                )
            );
            $delete_term_ids = array_map("intval", $delete_term_ids);

            $remove = wp_remove_object_terms(
                $object_id,
                $delete_term_ids,
                $taxonomy
            );
    ...
    if (!$append && isset($t->sort) && $t->sort) {
        ...
        if ($values) {
            if (
                false ===
                $wpdb->query(
                    "INSERT INTO $wpdb->term_relationships (object_id, term_taxonomy_id, term_order) VALUES " .
                        implode(",", $values) .
...
function wp_unique_term_slug($slug, $term)
{
    ...
    if (
        !term_exists($slug) ||
        (get_option("db_version") >= 30133 &&
            !get_term_by("slug", $slug, $term->taxonomy))
    ) {
        $needs_suffix = false;
    }
...
function wp_update_term($term_id, $taxonomy, $args = [])
{
    ...
    if (
        (int) $parsed_args["parent"] > 0 &&
        !term_exists((int) $parsed_args["parent"])
    ) {
        return new WP_Error(
            "missing_parent",
            __("Parent term does not exist.")
...
function wp_update_term_count($terms, $taxonomy, $do_deferred = false)
{
    ...
    if (wp_defer_term_counting()) {
        ...
        $_deferred[$taxonomy] = array_unique(
            array_merge($_deferred[$taxonomy], $terms)
        );
        return true;
    }
...
function update_object_term_cache($object_ids, $object_type)
{
    ...
    foreach ($taxonomies as $taxonomy) {
        $cache_values = wp_cache_get_multiple(
            (array) $object_ids,
...
function _pad_term_counts(&$terms, $taxonomy)
{
    ...
    $results = $wpdb->get_results(
        "SELECT object_id, term_taxonomy_id FROM $wpdb->term_relationships INNER JOIN $wpdb->posts ON object_id = ID WHERE term_taxonomy_id IN (" .
            implode(",", array_keys($term_ids)) .
            ") AND post_type IN ('" .
            implode("', '", $object_types) .
    ...
    foreach ($results as $row) {
        ...
        $term_items[$id][$row->object_id] = isset(
        ...
        while (
            ...
            if (!empty($term_items[$term_id])) {
                foreach ($term_items[$term_id] as $item_id => $touches) {
                    $term_items[$parent][$item_id] = isset(
...
function _prime_term_caches($term_ids, $update_meta_cache = true)
{
    ...
    if (!empty($non_cached_ids)) {
        $fresh_terms = $wpdb->get_results(
            sprintf(
                "SELECT t.*, tt.* FROM $wpdb->terms AS t INNER JOIN $wpdb->term_taxonomy AS tt ON t.term_id = tt.term_id WHERE t.term_id IN (%s)",
...
function _update_post_term_count($terms, $taxonomy)
{
    ...
    if ($object_types) {
        $object_types = esc_sql(
            array_filter($object_types, "post_type_exists")
        );
    }
    ...
    foreach ((array) $terms as $term) {
        ...
        if ($check_attachments) {
            ...
            $count += (int) $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(*) FROM $wpdb->term_relationships, $wpdb->posts p1 WHERE p1.ID = $wpdb->term_relationships.object_id AND ( post_status IN ('" .
                        implode("', '", $post_statuses) .
                        "') OR ( post_status = 'inherit' AND post_parent > 0 AND ( SELECT post_status FROM $wpdb->posts WHERE ID = p1.post_parent ) IN ('" .
                        implode("', '", $post_statuses) .
                        "') ) ) AND post_type = 'attachment' AND term_taxonomy_id = %d",
                    $term
                )
            );
        }
        ...
        if ($object_types) {
            ...
            $count += (int) $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(*) FROM $wpdb->term_relationships, $wpdb->posts WHERE $wpdb->posts.ID = $wpdb->term_relationships.object_id AND post_status IN ('" .
                        implode("', '", $post_statuses) .
                        "') AND post_type IN ('" .
                        implode("', '", $object_types) .
...
function _split_shared_term($term_id, $term_taxonomy_id, $record = true)
{
    ...
    $shared_tt_count = (int) $wpdb->get_var(
        $wpdb->prepare(
     ...
     */
    $check_term_id = (int) $wpdb->get_var(
        $wpdb->prepare(
    ...
    if (empty($shared_term)) {
        $shared_term = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT t.* FROM $wpdb->terms t WHERE t.term_id = %d",
                $term_id
            )
        );
    }
    ...
    if (false === $wpdb->insert($wpdb->terms, $new_term_data)) {
        return new WP_Error(
            "db_insert_error",
            __("Could not split shared term."),
            $wpdb->last_error
        );
    }
    ...
    $shared_term_taxonomies = $wpdb->get_col(
        $wpdb->prepare(
            "SELECT taxonomy FROM $wpdb->term_taxonomy WHERE term_id = %d",
            $term_id
        )
    );
    $taxonomies_to_clean = array_merge(
        $taxonomies_to_clean,
...
function _wp_batch_split_terms()
{
    ...
    foreach ($shared_tts as $shared_tt) {
        ...
        // Split the term.
        $split_term_data[$term_id][$shared_tt->taxonomy] = _split_shared_term(
            $shared_terms[$term_id],
            $shared_tt,
            false
        );
    }
...
{
    if (
        !get_option("finished_splitting_shared_terms") &&
        !wp_next_scheduled("wp_split_shared_term_batch")
    ) {
        wp_schedule_single_event(
 ...
 * @param string $taxonomy         Taxonomy for the split term.
 */
function _wp_check_split_default_terms(
    $term_id,
    $new_term_id,
    $term_taxonomy_id,
    $taxonomy
...
function the_taxonomies($args = [])
{
    ...
    echo $parsed_args["before"] .
        implode(
            $parsed_args["sep"],
            get_the_taxonomies($parsed_args["post"], $parsed_args)
        ) .
        $parsed_args["after"];
}
...
function get_the_taxonomies($post = 0, $args = [])
{
    ...
    foreach (get_object_taxonomies($post) as $taxonomy) {
        ...
        if ($links) {
            $taxonomies[$taxonomy] = wp_sprintf(
                $t["template"],
                $t["label"],
                $links,
                $terms
...
function is_object_in_term($object_id, $taxonomy, $terms = null)
{
    ...
    if (false === $object_terms) {
        ...
        wp_cache_set(
            $object_id,
            wp_list_pluck($object_terms, "term_id"),
            "{$taxonomy}_relationships"
        );
    }
    ...
    foreach ($object_terms as $object_term) {
        ...
        if ($strs) {
            ...
            $numeric_strs = array_map(
                "intval",
                array_filter($strs, "is_numeric")
            );
...
function get_ancestors($object_id = 0, $object_type = "", $resource_type = "")
{
    ...
    if ("taxonomy" === $resource_type) {
        ...
        while (
            !is_wp_error($term) &&
            !empty($term->parent) &&
            !in_array($term->parent, $ancestors, true)
        ) {
