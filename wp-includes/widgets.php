...
function register_sidebar($args = [])
{
     ...
     */
    $sidebar = wp_parse_args(
        $args,
        apply_filters("register_sidebar_defaults", $defaults)
    );
...
function wp_register_sidebar_widget(
    ...
    if (
        in_array($output_callback, $_wp_deprecated_widgets_callbacks, true) &&
    ...
    if (
        is_callable($output_callback) &&
        (!isset($wp_registered_widgets[$id]) || did_action("widgets_init"))
    ) {
        /**
...
function wp_register_widget_control(
    ...
    if (
        in_array($control_callback, $_wp_deprecated_widgets_callbacks, true) &&
    ...
    if (
        isset($wp_registered_widget_controls[$id]) &&
...
function dynamic_sidebar($index = 1)
{
    ...
    $sidebar["before_sidebar"] = sprintf(
        $sidebar["before_sidebar"],
        $sidebar["id"],
        $sidebar["class"]
    );
...
function is_active_widget(
    ...
    if (is_array($sidebars_widgets)) {
        foreach ($sidebars_widgets as $sidebar => $widgets) {
            if (
                $skip_inactive &&
                ("wp_inactive_widgets" === $sidebar ||
                    str_starts_with($sidebar, "orphaned_widgets"))
            ) {
                continue;
            }
            ...
            if (is_array($widgets)) {
                foreach ($widgets as $widget) {
                    if (
                        ($callback &&
                            isset(
                                $wp_registered_widgets[$widget]["callback"]
                            ) &&
                            $wp_registered_widgets[$widget]["callback"] ===
                                $callback) ||
...
function wp_get_sidebars_widgets($deprecated = true)
{
    ...
    if (
        ...
        isset($sidebars_widgets["array_version"])
    ) {
    ...
    if ($single) {
        ...
        if (is_admin()) {
            ...
            if (empty($GLOBALS["_wp_sidebars_widgets"])) {
                $GLOBALS["_wp_sidebars_widgets"] = get_option(
                    "sidebars_widgets",
                    []
                );
            }
...
function retrieve_widgets($theme_changed = false)
{
    ...
    if (!is_array(get_theme_mod("sidebars_widgets"))) {
        ...
        if ($sidebars_widgets_keys === $registered_sidebars_keys) {
            $sidebars_widgets = _wp_remove_unregistered_widgets(
                $sidebars_widgets,
...
function wp_map_sidebars_widgets($existing_sidebars_widgets)
{
    ...
    if (
        !is_array($existing_sidebars_widgets) ||
    ...
    foreach ($existing_sidebars_widgets as $sidebar => $widgets) {
        if (
            ...
            $new_sidebars_widgets["wp_inactive_widgets"] = array_merge(
                $new_sidebars_widgets["wp_inactive_widgets"],
                (array) $widgets
            );
    ...
    if (
        1 === count($existing_sidebars_widgets) &&
        1 === count($wp_registered_sidebars)
    ) {
        $new_sidebars_widgets[key($wp_registered_sidebars)] = array_pop(
        ...
        foreach ($common_slug_groups as $slug_group) {
            ...
            foreach ($slug_group as $slug) {
                ...
                foreach ($wp_registered_sidebars as $new_sidebar => $args) {
                    // ...actually match!
                    if (
                        false === stripos($new_sidebar, $slug) &&
                        false === stripos($slug, $new_sidebar)
                    ) {
                        continue;
                    }
                    ...
                    foreach (
                        ...
                        foreach ($slug_group as $slug) {
                            ...
                            if (
                                false === stripos($sidebar, $slug) &&
                                false === stripos($slug, $sidebar)
                            ) {
                                continue;
                            }
                            ...
                            if (!empty($existing_sidebars_widgets[$sidebar])) {
                                ...
                                $new_sidebars_widgets[
                                    ...
                                    $new_sidebars_widgets[$new_sidebar],
                                    $existing_sidebars_widgets[$sidebar]
                                );
    ...
    $old_sidebars_widgets = isset($old_sidebars_widgets["data"])
        ? $old_sidebars_widgets["data"]
        : false;
    ...
    if (is_array($old_sidebars_widgets)) {
        ...
        foreach ($new_sidebars_widgets as $new_sidebar => $new_widgets) {
            if (
                array_key_exists($new_sidebar, $old_sidebars_widgets) &&
                !empty($new_widgets)
            ) {
        ...
        $old_sidebars_widgets = _wp_remove_unregistered_widgets(
        ...
        if (!empty($old_sidebars_widgets)) {
            ...
            foreach ($old_sidebars_widgets as $old_sidebar => $old_widgets) {
                ...
                foreach (
                    $new_sidebars_widgets
                    as $new_sidebar => $new_widgets
                ) {
                    // ...for every widget we're trying to revive.
                    foreach ($old_widgets as $key => $widget_id) {
                        $active_key = array_search(
                            $widget_id,
                            $new_widgets,
                            true
                        );
                        ...
                        if (false !== $active_key) {
                            ...
                            if ("wp_inactive_widgets" === $new_sidebar) {
                                ...
                                unset(
                                    $old_sidebars_widgets[$old_sidebar][$key]
                                );
                            }
                        } // End if ( $active_key ).
        ...
        $new_sidebars_widgets = array_merge(
            $new_sidebars_widgets,
            $old_sidebars_widgets
        );
    }
...
function _wp_remove_unregistered_widgets(
    ...
    foreach ($sidebars_widgets as $sidebar => $widgets) {
        if (is_array($widgets)) {
            $sidebars_widgets[$sidebar] = array_intersect(
                $widgets,
                $allowed_widget_ids
            );
        }
...
function wp_widget_rss_output($rss, $args = [])
{
    ...
    foreach ($rss->get_items(0, $items) as $item) {
        ...
        if ($show_summary) {
            ...
            $summary =
                '<div class="rssSummary">' . esc_html($summary) . "</div>";
        }
...
function wp_widget_rss_form($args, $inputs = null)
{
    ...
    $args["show_summary"] = isset($args["show_summary"])
        ? (int) $args["show_summary"]
        : (int) $inputs["show_summary"];
    $args["show_author"] = isset($args["show_author"])
        ? (int) $args["show_author"]
        : (int) $inputs["show_author"];
    $args["show_date"] = isset($args["show_date"])
        ? (int) $args["show_date"]
        : (int) $inputs["show_date"];

    if (!empty($args["error"])) {
        echo '<p class="widget-error"><strong>' .
            __("RSS Error:") .
            "</strong> " .
            esc_html($args["error"]) .
            "</p>";
    ...
    if ($inputs["url"]): ?>
	<p><label for="rss-url-<?php echo $esc_number; ?>"><?php _e(
    "Enter the RSS feed URL here:"
); ?></label>
	<input class="widefat" id="rss-url-<?php echo $esc_number; ?>" name="widget-rss[<?php echo $esc_number; ?>][url]" type="text" value="<?php echo esc_url(
    $args["url"]
); ?>" /></p>
<?php endif;
    if ($inputs["title"]): ?>
	<p><label for="rss-title-<?php echo $esc_number; ?>"><?php _e(
    "Give the feed a title (optional):"
); ?></label>
	<input class="widefat" id="rss-title-<?php echo $esc_number; ?>" name="widget-rss[<?php echo $esc_number; ?>][title]" type="text" value="<?php echo esc_attr(
    $args["title"]
); ?>" /></p>
<?php endif;
    if ($inputs["items"]): ?>
	<p><label for="rss-items-<?php echo $esc_number; ?>"><?php _e(
...
<?php endif;
 ...
 if ($inputs["show_summary"]): ?>
		<input id="rss-show-summary-<?php echo $esc_number; ?>" name="widget-rss[<?php echo $esc_number; ?>][show_summary]" type="checkbox" value="1" <?php checked(
    $args["show_summary"]
); ?> />
		<label for="rss-show-summary-<?php echo $esc_number; ?>"><?php _e(
    "Display item content?"
); ?></label><br />
	<?php endif;
 if ($inputs["show_author"]): ?>
		<input id="rss-show-author-<?php echo $esc_number; ?>" name="widget-rss[<?php echo $esc_number; ?>][show_author]" type="checkbox" value="1" <?php checked(
    $args["show_author"]
); ?> />
		<label for="rss-show-author-<?php echo $esc_number; ?>"><?php _e(
    "Display item author if available?"
); ?></label><br />
	<?php endif;
 if ($inputs["show_date"]): ?>
		<input id="rss-show-date-<?php echo $esc_number; ?>" name="widget-rss[<?php echo $esc_number; ?>][show_date]" type="checkbox" value="1" <?php checked(
    $args["show_date"]
); ?> />
		<label for="rss-show-date-<?php echo $esc_number; ?>"><?php _e(
    "Display item date?"
); ?></label><br />
	<?php endif;
 ?>
	</p>
	<?php endif; // End of display options.
    foreach (array_keys($default_inputs) as $input):
        if ("hidden" === $inputs[$input]):
            $id = str_replace("_", "-", $input); ?>
<input type="hidden" id="rss-<?php echo esc_attr(
    $id
); ?>-<?php echo $esc_number; ?>" name="widget-rss[<?php echo $esc_number; ?>][<?php echo esc_attr(
    $input
...
function wp_widget_rss_process($widget_rss, $check_feed = true)
{
    ...
    $title = isset($widget_rss["title"])
        ? trim(strip_tags($widget_rss["title"]))
        : "";
    $show_summary = isset($widget_rss["show_summary"])
        ? (int) $widget_rss["show_summary"]
        : 0;
    $show_author = isset($widget_rss["show_author"])
        ? (int) $widget_rss["show_author"]
        : 0;
    $show_date = isset($widget_rss["show_date"])
        ? (int) $widget_rss["show_date"]
        : 0;
    $error = false;
    $link = "";
...
function wp_assign_widget_to_sidebar($widget_id, $sidebar_id)
{
    ...
    foreach ($sidebars as $maybe_sidebar_id => $widgets) {
        foreach ($widgets as $i => $maybe_widget_id) {
            if (
                $widget_id === $maybe_widget_id &&
                $sidebar_id !== $maybe_sidebar_id
            ) {
