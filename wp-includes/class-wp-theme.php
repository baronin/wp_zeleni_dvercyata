...
final class WP_Theme implements ArrayAccess
{
    ...
    public function __construct($theme_dir, $theme_root, $_child = null)
    {
        ...
        if (!isset(self::$persistently_cache)) {
            ...
            self::$persistently_cache = apply_filters(
        ...
        if (
            !in_array($theme_root, (array) $wp_theme_directories, true) &&
        ...
        if (is_array($cache)) {
             ...
             */
            $default_theme_slug = array_search(
                $this->headers["Name"],
                self::$default_themes,
                true
            );
        ...
        if (
            !$this->template &&
            $this->stylesheet === $this->headers["Template"]
        ) {
            $this->errors = new WP_Error(
                "theme_child_invalid",
                sprintf(
        ...
        if (
            ...
            $this->template !== $this->stylesheet &&
            !file_exists(
                $this->theme_root . "/" . $this->template . "/index.php"
            )
        ) {
            /*
            ...
            if (
                 ...
                 */
                $theme_root_template =
                    $directories[$this->template]["theme_root"];
            } else {
        ...
        if ($this->template !== $this->stylesheet) {
            ...
            if (
                $_child instanceof WP_Theme &&
                $_child->template === $this->stylesheet
            ) {
                $_child->parent = null;
        ...
        if (
            wp_paused_themes()->get($this->stylesheet) &&
    ...
    public function get($header)
    {
        ...
        if (self::$persistently_cache) {
            foreach (array_keys($this->headers) as $_header) {
                $this->headers_sanitized[$_header] = $this->sanitize_header(
                    $_header,
            ...
            $this->headers_sanitized[$header] = $this->sanitize_header(
                $header,
                $this->headers[$header]
            );
        }
    ...
    private function sanitize_header($header, $value)
    {
        switch ($header) {
            ...
            case "Tags":
                $value = array_filter(
                    array_map("trim", explode(",", strip_tags($value)))
                );
                break;
            case "Version":
            case "RequiresWP":
        ...
        switch ($header) {
            ...
            case "Author":
                if ($this->get("AuthorURI")) {
                    $value = sprintf(
                        '<a href="%1$s">%2$s</a>',
                        $this->display("AuthorURI", true, $translate),
                        $value
                    );
                } elseif (!$value) {
                    $value = __("Anonymous");
    ...
    private function translate_header($header, $value)
    {
        switch ($header) {
            case "Name":
                ...
                $this->name_translated = translate(
                    $value,
                    $this->get("TextDomain")
                );
            ...
            case "Tags":
                if (
                    empty($value) ||
                    !function_exists("get_theme_feature_list")
                ) {
                    return $value;
                }
    ...
    public function get_stylesheet_directory_uri()
    {
        return $this->get_theme_root_uri() .
            "/" .
            str_replace("%2F", "/", rawurlencode($this->stylesheet));
    }
    ...
    public function get_template_directory_uri()
    {
        ...
        return $theme_root_uri .
            "/" .
            str_replace("%2F", "/", rawurlencode($this->template));
    }
    ...
    public function get_screenshot($uri = "uri")
    {
        ...
        foreach (["png", "gif", "jpg", "jpeg", "webp", "avif"] as $ext) {
            if (
                file_exists(
                    $this->get_stylesheet_directory() . "/screenshot.$ext"
                )
            ) {
                ...
                return $this->get_stylesheet_directory_uri() .
                    "/" .
                    "screenshot." .
                    $ext;
            }
    ...
    public function get_post_templates()
    {
        ...
        if (
            $this->errors() &&
            $this->errors()->get_error_codes() !== ["theme_parent_invalid"]
        ) {
            return [];
        }
        ...
        if (!is_array($post_templates)) {
            ...
            foreach ($files as $file => $full_path) {
                ...
                foreach ($types as $type) {
                    ...
                    $post_templates[$type][$file] = _cleanup_header_comment(
                        $header[1]
                    );
                }
            }
            ...
            foreach (get_post_types(["public" => true]) as $type) {
                foreach ($block_templates as $block_template) {
                    ...
                    if (
                        ...
                        !in_array($type, $block_template->post_types, true)
                    ) {
                        continue;
                    }

                    $post_templates[$type][$block_template->slug] =
        ...
        if ($this->load_textdomain()) {
            foreach ($post_templates as &$post_type) {
                foreach ($post_type as &$post_template) {
                    $post_template = $this->translate_header(
                        "Template Name",
                        $post_template
                    );
                }
    ...
    public function get_page_templates($post = null, $post_type = "page")
    {
         ...
         */
        $post_templates = (array) apply_filters(
            "theme_templates",
            $post_templates,
            $this,
            $post,
    ...
    public function is_block_theme()
    {
        ...
        foreach (
            $paths_to_index_block_template
            as $path_to_index_block_template
        ) {
            if (
                is_file($path_to_index_block_template) &&
    ...
    public function get_file_path($file = "")
    {
        ...
        if (empty($file)) {
            ...
            file_exists($stylesheet_directory . "/" . $file)
        ) {
    ...
    public static function get_allowed_on_site($blog_id = null)
    {
        ...
        if (false === $allowed_themes[$blog_id]) {
            ...
            if (
                !is_array($allowed_themes[$blog_id]) ||
                empty($allowed_themes[$blog_id])
            ) {
                $allowed_themes[$blog_id] = [];
            } else {
                $converted = [];
                ...
                foreach ($themes as $stylesheet => $theme_data) {
                    if (
                        isset(
                            $allowed_themes[$blog_id][$theme_data->get("Name")]
                        )
                    ) {
    ...
    public function get_block_template_folders()
    {
        ...
        if (
            file_exists($stylesheet_directory . "/block-templates") ||
    ...
    private function get_pattern_cache()
    {
        ...
        $pattern_data = get_site_transient(
            "wp_theme_files_patterns-" . $this->cache_hash
        );

        if (
            is_array($pattern_data) &&
            $pattern_data["version"] === $this->get("Version")
        ) {
    ...
    private function set_pattern_cache(array $patterns)
    {
         ...
         */
        $cache_expiration = (int) apply_filters(
            "wp_theme_files_cache_ttl",
            self::$cache_expiration,
            "theme_block_patterns"
        );
