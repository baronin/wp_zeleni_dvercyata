...
function get_default_block_template_types()
{
    ...
    foreach ($post_formats as $post_format_slug => $post_format_name) {
        $default_template_types[
            "taxonomy-post_format-post-format-" . $post_format_slug
        ] = [
            "title" => sprintf(
...
function _get_block_templates_paths($base_directory)
{
    ...
    if (is_dir($base_directory)) {
        $nested_files = new RecursiveIteratorIterator(
            new RecursiveDirectoryIterator($base_directory)
        );
        $nested_html_files = new RegexIterator(
            $nested_files,
            '/^.+\.html$/i',
...
function _get_block_templates_files($template_type, $query = [])
{
    ...
    $slugs_to_skip = isset($query["slug__not_in"])
        ? $query["slug__not_in"]
        : [];
    ...
    foreach ($themes as $theme_slug => $theme_dir) {
        $template_base_paths = get_block_theme_folders($theme_slug);
        $theme_template_files = _get_block_templates_paths(
            $theme_dir . "/" . $template_base_paths[$template_type]
        );
        foreach ($theme_template_files as $template_file) {
            ...
            $template_slug = substr(
                ...
                strpos(
                    $template_file,
                    $template_base_path . DIRECTORY_SEPARATOR
                ) +
                    1 +
                    strlen($template_base_path),
            ...
            if (
                !empty($slugs_to_include) &&
                !in_array($template_slug, $slugs_to_include, true)
            ) {
                continue;
            }
            ...
            if (
                !empty($slugs_to_skip) &&
                in_array($template_slug, $slugs_to_skip, true)
            ) {
                continue;
            }
            ...
            if ("wp_template_part" === $template_type) {
                $candidate = _add_block_template_part_area_info(
                    $new_template_item
                );
                if (
                    !isset($area) ||
                    (isset($area) && $area === $candidate["area"])
                ) {
            ...
            if ("wp_template" === $template_type) {
                ...
                $is_custom = !isset(
                    $default_template_types[$candidate["slug"]]
                );

                if (
                    !$post_type ||
                    ($post_type &&
                        isset($candidate["postTypes"]) &&
                        in_array($post_type, $candidate["postTypes"], true))
                ) {
                    $template_files[$template_slug] = $candidate;
                }
...
function _add_block_template_info($template_item)
{
    ...
    if (isset($theme_data[$template_item["slug"]])) {
        ...
        $template_item["postTypes"] =
            $theme_data[$template_item["slug"]]["postTypes"];
    }
...
function _add_block_template_part_area_info($template_info)
{
    ...
    if (isset($theme_data[$template_info["slug"]]["area"])) {
        ...
        $template_info["area"] = _filter_block_template_part_area(
...
function _build_block_template_result_from_file($template_file, $template_type)
{
    ...
    $template->title = !empty($template_file["title"])
        ? $template_file["title"]
        : $template_file["slug"];
    ...
    if ("wp_template" === $template_type) {
        $registered_template = WP_Block_Templates_Registry::get_instance()->get_by_slug(
            $template_file["slug"]
        );
        if ($registered_template) {
            $template->plugin = $registered_template->plugin;
            $template->title =
                empty($template->title) || $template->title === $template->slug
                    ? $registered_template->title
                    : $template->title;
            $template->description = empty($template->description)
                ? $registered_template->description
    ...
    if (
        "wp_template" === $template_type &&
        isset($default_template_types[$template_file["slug"]])
    ) {
        $template->description =
            $default_template_types[$template_file["slug"]]["description"];
        $template->title =
            $default_template_types[$template_file["slug"]]["title"];
 ...
 */
function _wp_build_title_and_description_for_taxonomy_block_template(
    $taxonomy,
    $slug,
    WP_Block_Template $template
) {
 ...
 */
function _build_block_template_object_from_post_object(
    $post,
    $terms = [],
    $meta = []
) {
    if (empty($terms["wp_theme"])) {
        return new WP_Error(
            ...
            __("No theme is defined for this template.")
        );
    }
    ...
    $template_file = _get_block_template_file(
        $post->post_type,
        $post->post_name
    );
    ...
    if (
        "wp_template" === $post->post_type &&
        isset($default_template_types[$template->slug])
    ) {
        $template->is_custom = false;
    }
    ...
    if (
        ...
        isset($terms["wp_template_part_area"])
    ) {
...
function _build_block_template_result_from_post($post)
{
    ...
    $meta = [
        ...
        "is_wp_suggestion" => get_post_meta(
            $parent_post->ID,
            "is_wp_suggestion",
            true
        ),
    ...
    $template = _build_block_template_object_from_post_object(
        $post,
        $terms,
        $meta
    );
    ...
    if (
        "wp_template" === $parent_post->post_type &&
        ...
        if (
            preg_match(
                "/(author|page|single|tag|category|taxonomy)-(.+)/",
                $template->slug,
                $matches
            )
        ) {
                    ...
                    foreach ($post_types as $post_type) {
                        ...
                        if (
                            ...
                            $slug = substr(
                                $slug_remaining,
                                $post_type_length,
                                strlen($slug_remaining)
                            );
                            $found = _wp_build_title_and_description_for_single_post_type_block_template(
                                $post_type,
                                $slug,
                                $template
                            );
                ...
                case "taxonomy":
                    ...
                    foreach ($taxonomies as $taxonomy) {
                        ...
                        if (
                            ...
                            $found = _wp_build_title_and_description_for_taxonomy_block_template(
                                $taxonomy,
                                $slug,
                                $template
                            );
    ...
    if ("wp_template_part" === $template->type) {
        ...
        $attributes = !empty($existing_ignored_hooked_blocks)
            ? [
                "metadata" => [
                    "ignoredHookedBlocks" => json_decode(
...
function get_block_templates($query = [], $template_type = "wp_template")
{
    ...
    if (!empty($query["slug__in"])) {
        ...
        $wp_query_args["posts_per_page"] = count(
            array_unique($query["slug__in"])
        );
    }
    ...
    if (!isset($query["wp_id"])) {
        ...
        $template_files = _get_block_templates_files(
            $template_type,
            $template_files_query
        );
        foreach ($template_files as $template_file) {
            ...
            if (
                ...
                (isset($template_file["postTypes"]) &&
                    in_array(
                        $query["post_type"],
                        $template_file["postTypes"],
                        true
                    ))
            ) {
                $query_result[] = _build_block_template_result_from_file(
                    $template_file,
                    $template_type
                );
                ...
                $candidate = _build_block_template_result_from_file(
                    $template_file,
                    $template_type
                );
                $default_template_types = get_default_block_template_types();
        ...
        if ("wp_template" === $template_type) {
            ...
            $registered_templates = WP_Block_Templates_Registry::get_instance()->get_by_query(
                $query
            );
...
function get_block_file_template($id, $template_type = "wp_template")
{
    ...
    if (get_stylesheet() === $theme) {
        ...
        if (null !== $template_file) {
            $block_template = _build_block_template_result_from_file(
                $template_file,
                $template_type
            );
    ...
    $block_template = WP_Block_Templates_Registry::get_instance()->get_by_slug(
        $slug
    );
...
function wp_generate_block_templates_export_file()
{
    ...
    $tree = WP_Theme_JSON_Resolver::get_theme_data(
        [],
        ["with_supports" => false]
    );
    ...
    if ($theme_json_raw["version"]) {
        ...
        $schema = [
            '$schema' =>
                "https://schemas.wp.org/" . $theme_json_version . "/theme.json",
        ];
        $theme_json_raw = array_merge($schema, $theme_json_raw);
    }

    // Convert to a string.
    $theme_json_encoded = wp_json_encode(
        $theme_json_raw,
        JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE
    );

    // Replace 4 spaces with a tab.
    $theme_json_tabbed = preg_replace(
        "~(?:^|\G)\h{4}~m",
        "\t",
        $theme_json_encoded
    );
...
function get_template_hierarchy(
    ...
    if ($is_custom) {
        /** This filter is documented in wp-includes/template.php */
        return apply_filters("page_template_hierarchy", [
            "page",
            "singular",
            "index",
        ]);
    ...
    if ("front-page" === $slug) {
        /** This filter is documented in wp-includes/template.php */
        return apply_filters("frontpage_template_hierarchy", [
            "front-page",
            "home",
            "index",
        ]);
    ...
    if (!empty($template_prefix)) {
        ...
        preg_match('/^(author|category|archive|tag|page)-.+$/', $slug, $matches)
    ) {
...
function inject_ignored_hooked_blocks_metadata_attributes(
    ...
    if (empty($changes->ID)) {
        ...
        $terms["wp_theme"] =
            !is_wp_error($type_terms) && !empty($type_terms)
                ? $type_terms[0]->name
                : null;
    }
    ...
    if (
        ...
        $terms["wp_template_part_area"] =
            !is_wp_error($area_terms) && !empty($area_terms)
                ? $area_terms[0]->name
                : null;
    }
    ...
    if ("wp_template_part" === $post->post_type) {
        $attributes = [];
        $existing_ignored_hooked_blocks = isset($post->ID)
            ? get_post_meta($post->ID, "_wp_ignored_hooked_blocks", true)
            : "";
        ...
        $ignored_hooked_blocks =
            $wrapper_block["attrs"]["metadata"]["ignoredHookedBlocks"] ?? [];
        if (!empty($ignored_hooked_blocks)) {
            if (!isset($changes->meta_input)) {
                $changes->meta_input = [];
            }
            $changes->meta_input["_wp_ignored_hooked_blocks"] = wp_json_encode(
                $ignored_hooked_blocks
            );
        }
    } else {
        $changes->post_content = apply_block_hooks_to_content(
            $changes->post_content,
            $template,
            "set_ignored_hooked_blocks_metadata"
        );
