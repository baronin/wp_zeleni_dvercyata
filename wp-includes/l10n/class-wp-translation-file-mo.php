...
class WP_Translation_File_MO extends WP_Translation_File
{
    ...
    protected function parse_file(): bool
    {
        ...
        $offsets["originals_length"] =
            $offsets["translations_addr"] - $offsets["originals_addr"];
        $offsets["translations_length"] =
            $offsets["hash_addr"] - $offsets["translations_addr"];
        ...
        // Load the Originals.
        $original_data = str_split(
            substr(
                $file_contents,
                $offsets["originals_addr"],
                $offsets["originals_length"]
            ),
            8
        );
        $translations_data = str_split(
            substr(
                $file_contents,
                $offsets["translations_addr"],
                $offsets["translations_length"]
            ),
            8
        );
        ...
        foreach (array_keys($original_data) as $i) {
            $o = unpack(
                "{$this->uint32}length/{$this->uint32}pos",
                $original_data[$i]
            );
            $t = unpack(
                "{$this->uint32}length/{$this->uint32}pos",
