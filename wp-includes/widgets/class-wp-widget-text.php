...
class WP_Widget_Text extends WP_Widget
{
    ...
    public function is_legacy_instance($instance)
    {
        ...
        foreach ($body->getElementsByTagName("*") as $element) {
            ...
            if (
                !in_array($tag_name, $safe_empty_elements, true) &&
            ...
            foreach ($element->attributes as $attribute) {
                ...
                if (
                    !isset(
                        $safe_elements_attributes[$tag_name][$attribute_name]
                    )
                ) {
                    return true;
                }
    ...
    public function widget($args, $instance)
    {
        ...
        $is_visual_text_widget =
            !empty($instance["visual"]) && !empty($instance["filter"]);

        // In 4.8.0 only, visual Text widgets get filter=content, without visual prop; upgrade instance props just-in-time.
        if (!$is_visual_text_widget) {
            $is_visual_text_widget =
                isset($instance["filter"]) && "content" === $instance["filter"];
        }
        if ($is_visual_text_widget) {
         ...
         */
        $widget_text_do_shortcode_priority = has_filter(
            "widget_text",
            "do_shortcode"
        );
        $should_suspend_legacy_shortcode_support =
            $is_visual_text_widget &&
            false !== $widget_text_do_shortcode_priority;
        ...
        if ($is_visual_text_widget) {
             ...
             */
            if (
                has_filter("widget_text_content", "do_shortcode") &&
                !$widget_text_do_shortcode_priority
            ) {
                if (!empty($instance["filter"])) {
    ...
    public function update($new_instance, $old_instance)
    {
        ...
        if (
            isset($old_instance["filter"]) &&
            "content" === $old_instance["filter"]
        ) {
            $instance["visual"] = true;
        }
   ...
   ?>
    ...
    "title"
); ?>" class="title sync-input" type="hidden" value="<?php echo esc_attr(
    $instance["title"]
); ?>">
   ...
   wp_admin_notice($widget_info_message, [
       "type" => "info",
       "additional_classes" => ["notice-alt", "inline"],
   ]);
   ?>
			<p>
				<label for="<?php echo $this->get_field_id("text"); ?>"><?php _e(
    "Content:"
); ?></label>
				<textarea class="widefat" rows="16" cols="20" id="<?php echo $this->get_field_id(
        "text"
    ); ?>" name="<?php echo $this->get_field_name(
    "text"
    ...
    public static function render_control_template_scripts()
    {
		...
		<script type="text/html" id="tmpl-widget-text-control-fields">
          ...
          "Hey there, looks like you just pasted HTML into the &#8220;Visual&#8221; tab of the Text widget. You may want to paste your code into the &#8220;Code&#8221; tab instead. Alternately, try out the new &#8220;Custom HTML&#8221; widget!"
			...
			<p>
				<label for="{{ elementIdPrefix }}text" class="screen-reader-text"><?php /* translators: Hidden accessibility text. */ esc_html_e(
        "Content:"
    ); ?></label>
