...
class WP_Widget_Media_Video extends WP_Widget_Media
{
    ...
    public function __construct()
    {
        ...
        $this->l10n = array_merge($this->l10n, [
            ...
            "unsupported_file_type" => sprintf(
                ...
                "<code>." .
                    implode("</code>, <code>.", wp_get_video_extensions()) .
                    "</code>"
            ),
        ]);
    }
    ...
    public function get_instance_schema()
    {
        ...
        foreach (wp_get_video_extensions() as $video_extension) {
            $schema[$video_extension] = [
                ...
                "description" => sprintf(
                    __("URL to the %s video source file"),
                    $video_extension
                ),
            ];
        }
    ...
    public function render_media($instance)
    {
        ...
        if (
            $attachment ||
            preg_match($youtube_pattern, $src) ||
            preg_match($vimeo_pattern, $src)
        ) {
            add_filter("wp_video_shortcode", [
                $this,
                "inject_video_max_width_style",
            ]);

            echo wp_video_shortcode(
                array_merge($instance, compact("src")),
                $instance["content"]
            );

            remove_filter("wp_video_shortcode", [
                $this,
                "inject_video_max_width_style",
            ]);
        } else {
    ...
    public function render_control_template_scripts()
    {
        ...
        "additional_classes" => ["notice-alt", "notice-missing-attachment"],
			...
			<# } else if ( data.error && 'unsupported_file_type' === data.error ) { #>
				<?php wp_admin_notice($this->l10n["unsupported_file_type"], [
        "type" => "error",
        "additional_classes" => ["notice-alt", "notice-missing-attachment"],
    ]); ?>
			<# } else if ( data.error ) { #>
