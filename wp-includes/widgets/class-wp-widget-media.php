...
abstract class WP_Widget_Media extends WP_Widget
{
    ...
    public function __construct(
        ...
        $this->l10n = array_merge(
            self::get_l10n_defaults(),
            array_filter($this->l10n)
        );
    ...
    public function update($new_instance, $old_instance)
    {
        ...
        foreach ($schema as $field => $field_schema) {
            ...
            if (
                true !==
                rest_validate_value_from_schema($value, $field_schema, $field)
            ) {
                continue;
            }
            ...
            if (isset($field_schema["sanitize_callback"])) {
                $value = call_user_func(
                    $field_schema["sanitize_callback"],
                    $value
                );
            }
            if (is_wp_error($value)) {
    ...
    final public function form($instance)
    {
        ...
        $instance = wp_array_slice_assoc(
            wp_parse_args(
                (array) $instance,
                wp_list_pluck($instance_schema, "default")
            ),
    ...
    public function display_media_state($states, $post = null)
    {
        ...
        foreach ($this->get_settings() as $instance) {
            if (
                isset($instance["attachment_id"]) &&
                $instance["attachment_id"] === $post->ID
            ) {
                ++$use_count;
            }
        }
        ...
        if (1 === $use_count) {
            ...
            $states[] = sprintf(
                translate_nooped_plural(
                    $this->l10n["media_library_state_multi"],
                    $use_count
                ),
    ...
    {
        ?>
		<script type="text/html" id="tmpl-widget-media-<?php echo esc_attr(
      $this->id_base
  ); ?>-control">
    ...
    {
        return ($instance["attachment_id"] &&
            "attachment" === get_post_type($instance["attachment_id"])) ||
            $instance["url"];
    }
