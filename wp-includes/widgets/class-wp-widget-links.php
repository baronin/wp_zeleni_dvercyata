...
class WP_Widget_Links extends WP_Widget
{
    ...
    {
        $show_description = isset($instance["description"])
            ? $instance["description"]
            : false;
        $show_name = isset($instance["name"]) ? $instance["name"] : false;
        $show_rating = isset($instance["rating"]) ? $instance["rating"] : false;
        $show_images = isset($instance["images"]) ? $instance["images"] : true;
        $category = isset($instance["category"])
            ? $instance["category"]
            : false;
    ...
    $link_cat->term_id
			...
			<label for="<?php echo $this->get_field_id(
       "orderby"
   ); ?>"><?php _e("Sort by:"); ?></label>
			<select name="<?php echo $this->get_field_name(
       "orderby"
   ); ?>" id="<?php echo $this->get_field_id("orderby"); ?>" class="widefat">
				<option value="name"<?php selected(
        $instance["orderby"],
        "name"
    ); ?>><?php _e("Link title"); ?></option>
				<option value="rating"<?php selected(
        $instance["orderby"],
        "rating"
    ); ?>><?php _e("Link rating"); ?></option>
				<option value="id"<?php selected(
        $instance["orderby"],
        "id"
    ); ?>><?php _e("Link ID"); ?></option>
				<option value="rand"<?php selected(
        $instance["orderby"],
        "rand"
    ); ?>><?php _ex("Random", "Links widget"); ?></option>
			</select>
		</p>
       ...
       $instance["images"],
       true
   ); ?> id="<?php echo $this->get_field_id("images"); ?>" name="<?php echo $this->get_field_name("images"); ?>" />
			<label for="<?php echo $this->get_field_id(
       "images"
   ); ?>"><?php _e("Show Link Image"); ?></label>
			<br />
       ...
       true
			...
			<label for="<?php echo $this->get_field_id(
       "name"
   ); ?>"><?php _e("Show Link Name"); ?></label>
			<br />

			<input class="checkbox" type="checkbox"<?php checked(
       $instance["description"],
       true
   ); ?> id="<?php echo $this->get_field_id("description"); ?>" name="<?php echo $this->get_field_name("description"); ?>" />
			<label for="<?php echo $this->get_field_id(
       "description"
   ); ?>"><?php _e("Show Link Description"); ?></label>
			<br />

			<input class="checkbox" type="checkbox"<?php checked(
       $instance["rating"],
       true
   ); ?> id="<?php echo $this->get_field_id("rating"); ?>" name="<?php echo $this->get_field_name("rating"); ?>" />
			<label for="<?php echo $this->get_field_id(
       "rating"
   ); ?>"><?php _e("Show Link Rating"); ?></label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id(
       "limit"
   ); ?>"><?php _e("Number of links to show:"); ?></label>
			<input id="<?php echo $this->get_field_id(
       "limit"
   ); ?>" name="<?php echo $this->get_field_name("limit"); ?>" type="text" value="<?php echo -1 !== $limit ? (int) $limit : ""; ?>" size="3" />
		</p>
		<?php
    }
}
