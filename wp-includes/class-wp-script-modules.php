...
class WP_Script_Modules
{
    ...
    public function print_script_module_preloads()
    {
        foreach (
            $this->get_dependencies(
                array_keys($this->get_marked_for_enqueue()),
                ["static"]
            )
            as $id => $script_module
        ) {
    ...
    private function get_import_map(): array
    {
        $imports = [];
        foreach (
            $this->get_dependencies(array_keys($this->get_marked_for_enqueue()))
            as $id => $script_module
        ) {
    ...
    private function get_dependencies(
        array $ids,
        array $import_types = ["static", "dynamic"]
    ) {
        return array_reduce(
            $ids,
            function ($dependency_script_modules, $id) use ($import_types) {
                $dependencies = [];
                foreach (
                    $this->registered[$id]["dependencies"]
                    as $dependency
                ) {
                    if (
