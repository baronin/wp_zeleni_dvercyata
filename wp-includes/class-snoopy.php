            ...
            if ($this->fetch($URI)) {
                ...
                if (is_array($this->results)) {
                    for ($x = 0; $x < count($this->results); $x++) {
                        $this->results[$x] = $this->_striplinks(
                            $this->results[$x]
                        );
                    }
                } else {
            ...
            if ($this->submit($URI, $formvars, $formfiles)) {
                ...
                if (is_array($this->results)) {
                    for ($x = 0; $x < count($this->results); $x++) {
                        ...
                        if ($this->expandlinks) {
                            $this->results[$x] = $this->_expandlinks(
                                $this->results[$x],
                                $URI
                            );
                        }
...
\*======================================================================*/
        ...
        function _httprequest(
            ...
            $headers =
                $http_method . " " . $url . " " . $this->_httpversion . "\r\n";
            if (!empty($this->agent)) {
...
\*======================================================================*/
        ...
        function _httpsrequest(
            ...
            for (
                $currentHeader = 0;
                $currentHeader < count($result_headers);
                $currentHeader++
            ) {
                // if a header begins with Location: or URI:, set the redirect
                if (
                    ...
                    preg_match(
                        "/^(Location: |URI:)\s+(.*)/",
                        chop($result_headers[$currentHeader]),
                        $matches
                    );
                    // look for :// in the Location header to see if hostname is included
                    if (!preg_match("|\:\/\/|", $matches[2])) {
