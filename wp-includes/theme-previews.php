...
function wp_get_theme_preview_path($current_stylesheet = null)
{
    ...
    $preview_stylesheet = !empty($_GET["wp_theme_preview"])
        ? sanitize_text_field(wp_unslash($_GET["wp_theme_preview"]))
        : null;
...
function wp_block_theme_activate_nonce()
{
	...
	<script type="text/javascript">
		window.WP_BLOCK_THEME_ACTIVATE_NONCE = <?php echo wp_json_encode(
      wp_create_nonce($nonce_handle)
  ); ?>;
	</script>
	<?php
}
