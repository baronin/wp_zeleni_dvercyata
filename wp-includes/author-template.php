...
function get_the_author($deprecated = "")
{
    ...
    return apply_filters(
        "the_author",
        is_object($authordata) ? $authordata->display_name : ""
    );
}
...
function get_the_modified_author()
{
    ...
    if ($last_id) {
        ...
        return apply_filters(
            "the_modified_author",
            $last_user ? $last_user->display_name : ""
        );
    }
}
...
function get_the_author_link()
{
    if (get_the_author_meta("url")) {
        ...
        $link = sprintf(
            ...
            esc_attr(
                sprintf(__("Visit %s&#8217;s website"), $author_display_name)
            ),
            $author_display_name
        );
...
function wp_list_authors($args = "")
{
    ...
    foreach ($authors as $author_id) {
        ...
        if (
            ...
            $link .=
                '<a href="' .
                get_author_feed_link($author->ID, $parsed_args["feed_type"]) .
                '"';
...
function is_multi_author()
{
    ...
    if (false === $is_multi_author) {
        $rows = (array) $wpdb->get_col(
            "SELECT DISTINCT post_author FROM $wpdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2"
        );
