...
function comment_author_email_link(
    ...
    $link = get_comment_author_email_link(
        $link_text,
        $before,
        $after,
        $comment
...
function comment_class(
    ...
    $css_class =
        'class="' .
        implode(" ", get_comment_class($css_class, $comment, $post)) .
        '"';
...
function get_comment_class($css_class = "", $comment_id = null, $post = null)
{
    ...
    $classes[] = empty($comment->comment_type)
        ? "comment"
...
function get_comment_excerpt($comment_id = 0)
{
     ...
     */
    $comment_excerpt_length = apply_filters(
        "comment_excerpt_length",
        $comment_excerpt_length
    );

    $comment_excerpt = wp_trim_words(
        $comment_text,
        $comment_excerpt_length,
        "&hellip;"
    );
...
function get_comments_number_text(
    ...
    if ($comments_number > 1) {
        if (false === $more) {
            ...
            $comments_number_text = str_replace(
                "%",
                number_format_i18n($comments_number),
                $more
            );
...
function get_trackback_url()
{
    if (get_option("permalink_structure")) {
        ...
        $trackback_url =
            get_option("siteurl") . "/wp-trackback.php?p=" . get_the_ID();
    }
...
function wp_comment_form_unfiltered_html_nonce()
{
    ...
    if (current_user_can("unfiltered_html")) {
        wp_nonce_field(
            "unfiltered-html-comment_" . $post_id,
...
function comments_template($file = "/comments.php", $separate_comments = false)
{
    ...
    if (get_option("page_comments")) {
        ...
        if ($page) {
            ...
            if (isset($comment_args["include_unapproved"])) {
                $top_level_args["include_unapproved"] =
                    $comment_args["include_unapproved"];
            }
            ...
            $comment_args["offset"] =
                ((int) ceil($top_level_count / $per_page) - 1) * $per_page;
        }
    }
     ...
     * @param int   $post_id  Post ID.
     */
    $wp_query->comments = apply_filters(
        "comments_array",
        $comments_flat,
        $post->ID
    );
...
function comments_popup_link(
    ...
    if (false === $none) {
        /* translators: %s: Post title. */
        $none = sprintf(
            __('Comments Off<span class="screen-reader-text"> on %s</span>'),
            $post_title
        );
    }
...
function get_comment_reply_link($args = [], $comment = null, $post = null)
{
    ...
    if (get_option("comment_registration") && !is_user_logged_in()) {
        ...
        $aria_label = $args["show_reply_to_text"]
            ? ""
            : sprintf($args["reply_to_text"], get_comment_author($comment));
...
function get_cancel_comment_reply_link($link_text = "", $post = null)
{
    ...
    $link_url =
        esc_url(
            remove_query_arg(["replytocom", "unapproved", "moderation-hash"])
        ) . "#respond";
...
function wp_list_comments($args = [], $comments = null)
{
    ...
    if (null !== $comments) {
        ...
        if ($parsed_args["page"] || $parsed_args["per_page"]) {
            ...
            if (
                (int) $parsed_args["page"] !== $current_cpage ||
                (int) $parsed_args["per_page"] !== $current_per_page
            ) {
                $comment_args = [
                    "post_id" => get_the_ID(),
                    "orderby" => "comment_date_gmt",
            ...
            if ("all" !== $parsed_args["type"]) {
                if (empty($wp_query->comments_by_type)) {
                    $wp_query->comments_by_type = separate_comments(
                        $wp_query->comments
                    );
                }
    ...
    if (null === $parsed_args["reverse_top_level"]) {
        $parsed_args["reverse_top_level"] =
            "desc" === get_option("comment_order");
    }
...
function comment_form($args = [], $post = null)
{
    ...
    if (!isset($args["format"])) {
        $args["format"] = current_theme_supports("html5", "comment-form")
            ? "html5"
            : "xhtml";
    }
    ...
    if (
        ...
        !str_contains($args["comment_notes_before"], 'id="email-notes"')
    ) {
  ...
  if (get_option("comment_registration") && !is_user_logged_in()):
      ...
      $comment_fields =
          ["comment" => $args["comment_field"]] + (array) $args["fields"];

      $comment_fields = apply_filters("comment_form_fields", $comment_fields);

      $comment_field_keys = array_diff(array_keys($comment_fields), [
          "comment",
      ]);
