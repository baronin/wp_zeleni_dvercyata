...
class WP_Text_Diff_Renderer_Table extends Text_Diff_Renderer
{
    ...
    public function _changed($orig, $final)
    {
        ...
        list(
            ...
            $orig_rows,
            $final_rows,
        ) = $this->interleave_changed_lines($orig, $final);
        ...
        foreach ($orig_matches as $o => $f) {
            if (is_numeric($o) && is_numeric($f)) {
                ...
                if (
                    ...
                    $stripped_matches = strlen(
                        strip_tags(implode(" ", $diff_matches[0]))
                    );
                    /*
                     ...
                     */
                    $stripped_diff =
                        strlen(strip_tags($diff)) * 2 - $stripped_matches;
    ...
    public function interleave_changed_lines($orig, $final)
    {
        ...
        foreach (array_keys($orig) as $o) {
            foreach (array_keys($final) as $f) {
                $matches["$o,$f"] = $this->compute_string_distance(
                    $orig[$o],
                    $final[$f]
                );
            }
        ...
        foreach ($orig_rows_copy as $orig_row) {
            $final_pos = array_search(
                $orig_matches[$orig_row],
                $final_rows,
                true
            );
