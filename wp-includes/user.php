...
function wp_authenticate_application_password(
    ...
    $hashed_passwords = WP_Application_Passwords::get_user_application_passwords(
        $user->ID
    );
...
function wp_list_users($args = [])
{
    ...
    foreach ($users as $user_id) {
        ...
        if (
            !empty($parsed_args["feed_image"]) ||
            !empty($parsed_args["feed"])
        ) {
            $row .= " ";
            ...
            $row .=
                '<a href="' .
                get_author_feed_link($user->ID, $parsed_args["feed_type"]) .
                '"';
            ...
            if (!empty($parsed_args["feed_image"])) {
                $row .=
                    '<img src="' .
                    esc_url($parsed_args["feed_image"]) .
                    '" style="border: none;"' .
                    $alt .
                    " />";
            } else {
...
function get_blogs_of_user($user_id, $all = false)
{
    ...
    if (
        isset($keys[$wpdb->base_prefix . "capabilities"]) &&
        defined("MULTISITE")
    ) {
        $site_ids[] = 1;
...
function wp_dropdown_users($args = "")
{
    ...
    if (
        !empty($users) &&
        (empty($parsed_args["hide_if_only_one_author"]) || count($users) > 1)
    ) {
        ...
        if ($show_option_none) {
            ...
            $output .=
                ...
                "'$_selected>$show_option_none</option>\n";
        }
        ...
        foreach ((array) $users as $user) {
            ...
            $output .=
                "\t<option value='$user->ID'$_selected>" .
                esc_html($display) .
                "</option>\n";
        }
...
function wp_insert_user($userdata)
{
    ...
    if (mb_strlen($user_nicename) > 50) {
        return new WP_Error(
            ...
            __("Nicename may not be longer than 50 characters.")
        );
    }
    ...
    if ($user_nicename_check) {
        ...
        while ($user_nicename_check) {
            ...
            $alt_user_nicename =
                mb_substr($user_nicename, 0, $base_length) . "-$suffix";
            $user_nicename_check = $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT ID FROM $wpdb->users WHERE user_nicename = %s AND user_login != %s LIMIT 1",
                    $alt_user_nicename,
    ...
    if (mb_strlen($user_url) > 100) {
        return new WP_Error(
            "user_url_too_long",
            __("User URL may not be longer than 100 characters.")
        );
    }

    $user_registered = empty($userdata["user_registered"])
        ? gmdate("Y-m-d H:i:s")
        : $userdata["user_registered"];

    $user_activation_key = empty($userdata["user_activation_key"])
        ? ""
    ...
    if (!empty($userdata["spam"]) && !is_multisite()) {
        return new WP_Error(
            "no_spam",
            __("Sorry, marking a user as spam is only supported on Multisite.")
        );
    }
    ...
    $nickname = empty($userdata["nickname"])
        ? $user_login
        : $userdata["nickname"];
    ...
    $description = empty($userdata["description"])
        ? ""
    ...
    $meta["rich_editing"] = empty($userdata["rich_editing"])
        ? "true"
        : $userdata["rich_editing"];

    $meta["syntax_highlighting"] = empty($userdata["syntax_highlighting"])
        ? "true"
        : $userdata["syntax_highlighting"];

    $meta["comment_shortcuts"] =
        empty($userdata["comment_shortcuts"]) ||
    ...
    $meta["show_admin_bar_front"] = empty($userdata["show_admin_bar_front"])
        ? "true"
    ...
    if (empty($data) || !is_array($data)) {
        return new WP_Error(
            "empty_data",
            __("Not enough data to create this user.")
        );
    }
    ...
    $custom_meta = [];
    if (
        array_key_exists("meta_input", $userdata) &&
        is_array($userdata["meta_input"]) &&
        !empty($userdata["meta_input"])
    ) {
    ...
    if ($update) {
        ...
        if (
            isset($userdata["spam"]) &&
            $userdata["spam"] !== $old_user_data->spam
        ) {
            if ("1" === $userdata["spam"]) {
                /**
...
function wp_update_user($userdata)
{
    ...
    if (
         ...
         * @param array $userdata The updated user array.
         */
        $send_email_change_email = apply_filters(
...
###SITEURL###'
         ...
         * @param array $userdata The updated user array.
         */
        $pass_change_email = apply_filters(
...
###SITEURL###'
    ...
    if ($current_user->ID === $user_id) {
        if (isset($plaintext_pass)) {
            ...
            $default_cookie_life = apply_filters(
            ...
            if (
                false !== $logged_in_cookie &&
                (int) $logged_in_cookie["expiration"] - time() >
                    $default_cookie_life
            ) {
                $remember = true;
            }
...
function check_password_reset_key(#[\SensitiveParameter] $key, $login)
{
     ...
     */
    $expiration_duration = apply_filters(
        "password_reset_expiration",
        DAY_IN_SECONDS
    );

    if (str_contains($user->user_activation_key, ":")) {
        list($pass_request_time, $pass_key) = explode(
            ":",
            $user->user_activation_key,
            2
        );
    ...
    if (
        hash_equals($user->user_activation_key, $key) ||
        ($hash_is_correct && !$expiration_time)
    ) {
        $return = new WP_Error("expired_key", __("Invalid key."));
        $user_id = $user->ID;
...
function register_new_user($user_login, $user_email)
{
    ...
    if ("" === $sanitized_user_login) {
        ...
        if (
            in_array(
                strtolower($sanitized_user_login),
                array_map("strtolower", $illegal_user_logins),
                true
            )
        ) {
            $errors->add(
...
###SITEURL###'
        ...
        wp_mail(
            $_POST["email"],
            sprintf(__("[%s] Email Change Request"), $sitename),
            $content
        );
...
function wp_user_personal_data_exporter($email_address)
{
    ...
    if (is_array($_extra_data) && !empty($_extra_data)) {
        ...
        $extra_data = array_filter($_extra_data, static function ($item) use (
            $reserved_names
        ) {
        ...
        if (!empty($extra_data)) {
            $user_data_to_export = array_merge(
                $user_data_to_export,
                $extra_data
            );
        }
    ...
    if (isset($user_meta["community-events-location"])) {
        $location = maybe_unserialize(
            $user_meta["community-events-location"][0]
        );
...
function _wp_privacy_account_request_confirmed($request_id)
{
    ...
    if (
        !in_array($request->status, ["request-pending", "request-failed"], true)
    ) {
        return;
    }
...
function _wp_privacy_send_request_confirmation_notification($request_id)
{
    ...
    if (
        ...
        "request-confirmed" !== $request->status
    ) {
        return;
    }

    $already_notified = (bool) get_post_meta(
        $request_id,
        "_wp_admin_notified",
        true
    );
    ...
    $action_description = wp_user_request_action_description(
        $request->action_name
    );
...
function _wp_privacy_account_request_confirmed_message($request_id)
{
    ...
    $message =
        '<p class="success">' . __("Action has been confirmed.") . "</p>";
    $message .=
        "<p>" .
        __(
...
function wp_send_user_request($request_id)
{
    ...
    if (!$request) {
        return new WP_Error(
            "invalid_request",
            __("Invalid personal data request.")
        );
    }
...
###SITEURL###'
    ...
    $content = str_replace(
        "###CONFIRM_URL###",
        sanitize_url($email_data["confirm_url"]),
        $content
    );
    ...
    if (!$email_sent) {
        return new WP_Error(
            "privacy_email_error",
            __("Unable to send personal data export confirmation email.")
        );
    }
...
function wp_validate_user_request_key($request_id, #[\SensitiveParameter] $key)
{
    ...
    if (
        ...
        return new WP_Error(
            "expired_request",
            __("This personal data request has expired.")
        );
    }
     ...
     * @param int $expiration The expiration time in seconds.
     */
    $expiration_duration = (int) apply_filters(
