...
function wp_version_check($extra_stats = [], $force_check = false)
{
    ...
    $time_not_changed =
        ...
        $timeout > time() - $current->last_checked;
    ...
    $query = [
        ...
        "extensions" => array_combine(
            $extensions,
            array_map("phpversion", $extensions)
        ),
        "platform_flags" => [
            "os" => PHP_OS,
...
function wp_update_plugins($extra_stats = [])
{
    ...
    foreach ($plugins as $plugin_file => $plugin_data) {
        ...
        $hostname = wp_parse_url(
            sanitize_url($plugin_data["UpdateURI"]),
            PHP_URL_HOST
        );
        ...
        if (!empty($update->translations)) {
            foreach ($update->translations as $translation) {
                if (isset($translation["language"], $translation["package"])) {
                    ...
                    $translation["slug"] = isset($update->slug)
                        ? $update->slug
                        : $update->id;
...
function wp_update_themes($extra_stats = [])
{
    ...
    $time_not_changed =
        isset($last_update->last_checked) &&
        $timeout > time() - $last_update->last_checked;
    ...
    if ($time_not_changed && !$extra_stats) {
        ...
        foreach ($checked as $slug => $v) {
            if (
                !isset($last_update->checked[$slug]) ||
                (string) $last_update->checked[$slug] !== (string) $v
            ) {
                $theme_changed = true;
            }
        }
    ...
    foreach ($themes as $theme_stylesheet => $theme_data) {
        if (
            !$theme_data["UpdateURI"] ||
            isset($new_update->response[$theme_stylesheet])
        ) {
            continue;
        }

        $hostname = wp_parse_url(
            sanitize_url($theme_data["UpdateURI"]),
            PHP_URL_HOST
        );
        ...
        if (!empty($update->translations)) {
            foreach ($update->translations as $translation) {
                if (isset($translation["language"], $translation["package"])) {
                    ...
                    $translation["slug"] = isset($update->theme)
                        ? $update->theme
                        : $update->id;
        ...
        unset(
            $new_update->no_update[$theme_stylesheet],
            $new_update->response[$theme_stylesheet]
        );

        if (
            version_compare($update->new_version, $theme_data["Version"], ">")
        ) {
...
function wp_get_update_data()
{
    ...
    if ($counts["wordpress"]) {
        ...
        $titles["wordpress"] = sprintf(
            __("%d WordPress Update"),
            $counts["wordpress"]
        );
    }
    ...
    if ($counts["themes"]) {
        ...
        $titles["themes"] = sprintf(
            _n("%d Theme Update", "%d Theme Updates", $counts["themes"]),
            $counts["themes"]
        );
    }
...
function _maybe_update_core()
{
    ...
    if (
        isset($current->last_checked, $current->version_checked) &&
...
function wp_delete_all_temp_backups()
{
     ...
     */
    if (
        get_option("core_updater.lock") ||
        get_option("auto_updater.lock") ||
        wp_doing_ajax()
    ) {
...
function _wp_delete_all_temp_backups()
{
    ...
    $temp_backup_dir =
        $wp_filesystem->wp_content_dir() . "upgrade-temp-backup/";
