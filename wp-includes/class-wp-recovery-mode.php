...
class WP_Recovery_Mode
{
    ...
    public function __construct()
    {
        ...
        $this->link_service = new WP_Recovery_Mode_Link_Service(
            $this->cookie_service,
            $this->key_service
        );
        $this->email_service = new WP_Recovery_Mode_Email_Service(
            $this->link_service
        );
    }
    ...
    public function initialize()
    {
        ...
        add_action("login_form_" . self::EXIT_ACTION, [
            $this,
        ...
        if (
            !wp_next_scheduled("recovery_mode_clean_expired_keys") &&
            !wp_installing()
        ) {
            wp_schedule_event(
                time(),
    ...
    public function handle_error(array $error)
    {
        ...
        if (!$this->is_active()) {
            ...
            return $this->email_service->maybe_send_recovery_mode_email(
    ...
    public function handle_exit_recovery_mode()
    {
        ...
        if (
            !isset($_GET["_wpnonce"]) ||
            !wp_verify_nonce($_GET["_wpnonce"], self::EXIT_ACTION)
        ) {
