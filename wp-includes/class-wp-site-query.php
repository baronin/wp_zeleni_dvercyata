...
class WP_Site_Query
{
    ...
    public function get_sites()
    {
         ...
         * @param WP_Site_Query            $query     The WP_Site_Query instance, passed by reference.
         */
        $site_data = apply_filters_ref_array("sites_pre_query", [
            $site_data,
            &$this,
        ]);
        ...
        if ($this->found_sites && $this->query_vars["number"]) {
            $this->max_num_pages = (int) ceil(
                $this->found_sites / $this->query_vars["number"]
            );
        }
    ...
    protected function get_site_ids()
    {
        ...
        if (!empty($site_id)) {
            $this->sql_clauses["where"]["ID"] = $wpdb->prepare(
                "{$wpdb->blogs}.blog_id = %d",
                $site_id
            );
        }
        ...
        if (!empty($this->query_vars["site__in"])) {
            $this->sql_clauses["where"]["site__in"] =
                "{$wpdb->blogs}.blog_id IN ( " .
                implode(",", wp_parse_id_list($this->query_vars["site__in"])) .
                " )";
        }

        // Parse site IDs for a NOT IN clause.
        if (!empty($this->query_vars["site__not_in"])) {
            $this->sql_clauses["where"]["site__not_in"] =
                "{$wpdb->blogs}.blog_id NOT IN ( " .
                implode(
                    ",",
        ...
        if (!empty($network_id)) {
            $this->sql_clauses["where"]["network_id"] = $wpdb->prepare(
                "site_id = %d",
                $network_id
            );
        }
        ...
        if (!empty($this->query_vars["network__in"])) {
            $this->sql_clauses["where"]["network__in"] =
                "site_id IN ( " .
                implode(
                    ",",
        ...
        if (!empty($this->query_vars["network__not_in"])) {
            $this->sql_clauses["where"]["network__not_in"] =
                "site_id NOT IN ( " .
                implode(
                    ",",
        ...
        if (!empty($this->query_vars["domain"])) {
            $this->sql_clauses["where"]["domain"] = $wpdb->prepare(
                "domain = %s",
                $this->query_vars["domain"]
            );
        }

        // Parse site domain for an IN clause.
        if (is_array($this->query_vars["domain__in"])) {
            $this->sql_clauses["where"]["domain__in"] =
                "domain IN ( '" .
                implode(
                    "', '",
        ...
        if (is_array($this->query_vars["domain__not_in"])) {
            $this->sql_clauses["where"]["domain__not_in"] =
                "domain NOT IN ( '" .
                implode(
                    "', '",
        ...
        if (!empty($this->query_vars["path"])) {
            $this->sql_clauses["where"]["path"] = $wpdb->prepare(
                "path = %s",
                $this->query_vars["path"]
            );
        }

        // Parse site path for an IN clause.
        if (is_array($this->query_vars["path__in"])) {
            $this->sql_clauses["where"]["path__in"] =
                "path IN ( '" .
                implode("', '", $wpdb->_escape($this->query_vars["path__in"])) .
                "' )";
        }

        // Parse site path for a NOT IN clause.
        if (is_array($this->query_vars["path__not_in"])) {
            $this->sql_clauses["where"]["path__not_in"] =
                "path NOT IN ( '" .
                implode(
                    "', '",
        ...
        if (is_numeric($this->query_vars["mature"])) {
            ...
            $this->sql_clauses["where"]["mature"] = $wpdb->prepare(
                "mature = %d ",
                $mature
            );
        }
        ...
        if (is_numeric($this->query_vars["spam"])) {
            ...
            $this->sql_clauses["where"]["spam"] = $wpdb->prepare(
                "spam = %d ",
                $spam
            );
        }
        ...
        if (is_numeric($this->query_vars["deleted"])) {
            ...
            $this->sql_clauses["where"]["deleted"] = $wpdb->prepare(
                "deleted = %d ",
                $deleted
            );
        }
        ...
        if (is_numeric($this->query_vars["public"])) {
            ...
            $this->sql_clauses["where"]["public"] = $wpdb->prepare(
                "public = %d ",
                $public
            );
        }
        ...
        if (is_numeric($this->query_vars["lang_id"])) {
            ...
            $this->sql_clauses["where"]["lang_id"] = $wpdb->prepare(
                "lang_id = %d ",
                $lang_id
            );
        }
        ...
        if (!empty($this->query_vars["lang__in"])) {
            $this->sql_clauses["where"]["lang__in"] =
                "lang_id IN ( " .
                implode(",", wp_parse_id_list($this->query_vars["lang__in"])) .
                " )";
        }

        // Parse site language IDs for a NOT IN clause.
        if (!empty($this->query_vars["lang__not_in"])) {
            $this->sql_clauses["where"]["lang__not_in"] =
                "lang_id NOT IN ( " .
                implode(
                    ",",
         ...
         * @param WP_Site_Query $query   Current instance of WP_Site_Query (passed by reference).
         */
        $clauses = apply_filters_ref_array("sites_clauses", [
            compact($pieces),
            &$this,
        ]);
    ...
    private function set_found_sites()
    {
        ...
        if (
             ...
             */
            $found_sites_query = apply_filters(
                "found_sites_query",
                "SELECT FOUND_ROWS()",
                $this
            );
    ...
    protected function get_search_sql($search, $columns)
    {
        ...
        if (str_contains($search, "*")) {
            $like =
                ...
                implode(
                    "%",
                    array_map([$wpdb, "esc_like"], explode("*", $search))
                ) .
                "%";
        } else {
    ...
    protected function parse_orderby($orderby)
    {
        ...
        switch ($orderby) {
            case "site__in":
                $site__in = implode(
                    ",",
                    array_map("absint", $this->query_vars["site__in"])
                );
        ...
        if (
            !empty($primary_meta_query["key"]) &&
            $primary_meta_query["key"] === $orderby
        ) {
            $orderby = "meta_value";
        }
