...
class WP_Meta_Query
{
    ...
    public function __construct($meta_query = false)
    {
        ...
        if (
            isset($meta_query["relation"]) &&
            "OR" === strtoupper($meta_query["relation"])
        ) {
            $this->relation = "OR";
        } else {
            $this->relation = "AND";
    ...
    public function parse_query_vars($qv)
    {
        ...
        $primary_meta_query = [];
        foreach (
            ["key", "compare", "type", "compare_key", "type_key"]
            as $key
        ) {
        ...
        $existing_meta_query =
            isset($qv["meta_query"]) && is_array($qv["meta_query"])
                ? $qv["meta_query"]
                : [];
        ...
        foreach ($query as $key => &$clause) {
            if ("relation" === $key) {
                ...
                if ($this->is_first_order_clause($clause)) {
                    $clause_sql = $this->get_sql_for_clause(
                        $clause,
                        $query,
                        $key
                    );
    ...
    public function get_sql_for_clause(
        ...
        $numeric_operators = [">", ">=", "<", "<=", "BETWEEN", "NOT BETWEEN"];
        ...
        if (isset($clause["compare_key"])) {
            ...
            $clause["compare_key"] =
                isset($clause["key"]) && is_array($clause["key"]) ? "IN" : "=";
        }
        ...
        if (false === $alias) {
            ...
            if ("NOT EXISTS" === $meta_compare) {
                ...
                if ("LIKE" === $meta_compare_key) {
                    $join .= $wpdb->prepare(
                        " ON ( $this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.meta_key LIKE %s )",
        ...
        if (array_key_exists("key", $clause)) {
            if ("NOT EXISTS" === $meta_compare) {
                $sql_chunks["where"][] =
                    $alias . "." . $this->meta_id_column . " IS NULL";
            } else {
                /**
                ...
                switch ($meta_compare_key) {
                    ...
                    case "LIKE":
                        $meta_compare_value =
                            "%" . $wpdb->esc_like(trim($clause["key"])) . "%";
                        $where = $wpdb->prepare(
                            "$alias.meta_key LIKE %s",
                            $meta_compare_value
                        ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                        break;
                    case "IN":
                    ...
                    case "REGEXP":
                        ...
                        if (
                            isset($clause["type_key"]) &&
                            "BINARY" === strtoupper($clause["type_key"])
                        ) {
                            $cast = "BINARY";
        ...
        if (array_key_exists("value", $clause)) {
            ...
            switch ($meta_compare) {
                ...
                case "NOT IN":
                    $meta_compare_string =
                        "(" .
                        substr(str_repeat(",%s", count($meta_value)), 1) .
                        ")";
            ...
            if ($where) {
                if ("CHAR" === $meta_type) {
                    ...
                    $sql_chunks[
                        "where"
                    ][] = "CAST($alias.meta_value AS {$meta_type}) {$meta_compare} {$where}";
                }
            }
        }
    ...
    protected function find_compatible_table_alias($clause, $parent_query)
    {
        ...
        foreach ($parent_query as $sibling) {
            ...
            if (
                !is_array($sibling) ||
                !$this->is_first_order_clause($sibling)
            ) {
                continue;
            }
            ...
            if (
                in_array($clause_compare, $compatible_compares, true) &&
