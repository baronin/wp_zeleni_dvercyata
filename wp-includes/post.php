...
function create_initial_post_types()
{
    ...
    register_post_type("post", [
        ...
        "_edit_link" =>
            "post.php?post=%d" /* internal use only. don't use this when registering your own post type. */,
    ...
    register_post_type("revision", [
        ...
        "_builtin" => true /* internal use only. don't use this when registering your own post type. */,
        "_edit_link" =>
            "revision.php?revision=%d" /* internal use only. don't use this when registering your own post type. */,
        "capability_type" => "post",
        "map_meta_cap" => true,
        "hierarchical" => false,
        "rewrite" => false,
    ...
    register_post_type("wp_global_styles", [
        ...
        "_builtin" => true /* internal use only. don't use this when registering your own post type. */,
        "_edit_link" =>
            "/site-editor.php?canvas=edit" /* internal use only. don't use this when registering your own post type. */,
        "show_ui" => false,
        "show_in_rest" => true,
        "rewrite" => false,
    ...
    register_post_type("wp_font_face", [
        "labels" => [
            "name" => __("Font Faces"),
...
function get_post_status($post = null)
{
    ...
    if ("attachment" === $post->post_type && "inherit" === $post_status) {
        ...
        !in_array($post_status, ["private", "trash", "auto-draft"], true)
    ) {
        /*
...
function unregister_post_type($post_type)
{
    ...
    if ($post_type_object->_builtin) {
        return new WP_Error(
            "invalid_post_type",
            __("Unregistering a built-in post type is not allowed")
        );
    }
...
function get_post_type_labels($post_type_object)
{
    ...
    $labels = _get_custom_object_labels(
        $post_type_object,
        $nohier_vs_hier_defaults
    );

    if (
        !isset($post_type_object->labels->template_name) &&
        isset($post_type_object->labels->singular_name)
    ) {
        /* translators: %s: Post type name. */
        $labels->template_name = sprintf(
            __("Single item: %s"),
...
function _get_custom_object_labels($data_object, $nohier_vs_hier_defaults)
{
    ...
    if (
        !isset($data_object->labels["menu_name"]) &&
        isset($data_object->labels["name"])
    ) {
    ...
    if (
        !isset($data_object->labels["all_items"]) &&
        isset($data_object->labels["menu_name"])
    ) {
    ...
    if (
        !isset($data_object->labels["archives"]) &&
        isset($data_object->labels["all_items"])
    ) {
...
function get_post_types_by_support($feature, $operator = "and")
{
    ...
    return array_keys(
        wp_filter_object_list($_wp_post_type_features, $features, $operator)
    );
}
...
function is_post_status_viewable($post_status)
{
    ...
    $is_viewable =
        ...
        ($post_status->_builtin && $post_status->public);
...
function get_posts($args = null)
{
    ...
    if (empty($parsed_args["post_status"])) {
        $parsed_args["post_status"] =
            "attachment" === $parsed_args["post_type"] ? "inherit" : "publish";
    }
    if (
...
function wp_count_attachments($mime_type = "")
{
    ...
    $cache_key = sprintf(
        "attachments%s",
        !empty($mime_type)
            ? ":" . str_replace("/", "_", implode("-", (array) $mime_type))
            : ""
    );
    ...
    if (false === $counts) {
        ...
        $count = $wpdb->get_results(
            "SELECT post_mime_type, COUNT( * ) AS num_posts FROM $wpdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' $and GROUP BY post_mime_type",
            ARRAY_A
        );
        ...
        $counts["trash"] = $wpdb->get_var(
            "SELECT COUNT( * ) FROM $wpdb->posts WHERE post_type = 'attachment' AND post_status = 'trash' $and"
        );
...
function wp_match_mime_types($wildcard_mime_types, $real_mime_types)
{
    ...
    if (is_string($wildcard_mime_types)) {
        $wildcard_mime_types = array_map(
            "trim",
            explode(",", $wildcard_mime_types)
        );
    }
    ...
    foreach ((array) $wildcard_mime_types as $type) {
        ...
        foreach ($mimes as $mime) {
            $regex = str_replace(
                "__wildcard__",
                $wild,
                preg_quote(str_replace("*", "__wildcard__", $mime))
            );
    ...
    foreach ($patternses as $patterns) {
        foreach ($patterns as $type => $pattern) {
            foreach ((array) $real_mime_types as $real) {
                if (
                    preg_match("#$pattern#", $real) &&
                    (empty($matches[$type]) ||
                        false === array_search($real, $matches[$type], true))
                ) {
                    $matches[$type][] = $real;
                }
            }
    ...
    foreach ((array) $post_mime_types as $mime_type) {
        ...
        if (false !== $slashpos) {
            $mime_group = preg_replace(
                "/[^-*.a-zA-Z0-9]/",
                "",
                substr($mime_type, 0, $slashpos)
            );
            $mime_subgroup = preg_replace(
                "/[^-*.+a-zA-Z0-9]/",
                "",
                substr($mime_type, $slashpos + 1)
            );
        ...
        if (str_contains($mime_pattern, "%")) {
            $where_clauses[] = empty($table_alias)
...
function wp_delete_post($post_id = 0, $force_delete = false)
{
    ...
    $parent_where = ["post_parent" => $post_id];
    ...
    if (is_post_type_hierarchical($post->post_type)) {
        ...
        if ($children) {
            $wpdb->update(
                $wpdb->posts,
                $parent_data,
                $parent_where + ["post_type" => $post->post_type]
            );
        }
    }
    ...
    $wpdb->update(
        $wpdb->posts,
        $parent_data,
        $parent_where + ["post_type" => "attachment"]
    );
    ...
    $comment_ids = $wpdb->get_col(
        $wpdb->prepare(
            "SELECT comment_ID FROM $wpdb->comments WHERE comment_post_ID = %d ORDER BY comment_ID DESC",
            $post_id
        )
    );
...
function wp_untrash_post_comments($post = null)
{
    ...
    foreach ($group_by_status as $status => $comments) {
        ...
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE $wpdb->comments SET comment_approved = %s WHERE comment_ID IN ($comments_in)",
                $status
            )
        );
    }
...
function wp_insert_post($postarr, $wp_error = false, $fire_after_hooks = true)
{
    ...
    if (apply_filters("wp_insert_post_empty_content", $maybe_empty, $postarr)) {
        if ($wp_error) {
            return new WP_Error(
                "empty_content",
                __("Content, title, and excerpt are empty.")
            );
        } else {
            return 0;
        }
    ...
    if (
        ...
        !in_array(
            $post_status,
            ["inherit", "private", "trash", "auto-draft"],
            true
        )
    ) {
        $post_status = "inherit";
    ...
    if (
        empty($postarr["post_date_gmt"]) ||
        "0000-00-00 00:00:00" === $postarr["post_date_gmt"]
    ) {
        if (
            !in_array(
                $post_status,
                get_post_stati(["date_floating" => true]),
                true
            )
        ) {
    ...
    if ("attachment" !== $post_type) {
        ...
        if ("publish" === $post_status) {
            if (
                strtotime($post_date_gmt) - strtotime($now) >=
                MINUTE_IN_SECONDS
            ) {
                $post_status = "future";
            }
    ...
    if ("trash" === $previous_status && "trash" !== $post_status) {
        $desired_post_slug = get_post_meta(
            $post_id,
    ...
    if ("auto-draft" !== $post_status) {
        foreach (
            ...
            as $taxonomy => $tax_object
        ) {
            if (!empty($tax_object->default_term)) {
                // Filter out empty terms.
                if (
                    isset($postarr["tax_input"][$taxonomy]) &&
    ...
    if (!empty($postarr["tax_input"])) {
        foreach ($postarr["tax_input"] as $taxonomy => $tags) {
            ...
            if (!$taxonomy_obj) {
                ...
                _doing_it_wrong(
                    __FUNCTION__,
                    sprintf(__("Invalid taxonomy: %s."), $taxonomy),
                    "4.4.0"
                );
                continue;
            }
    ...
    if (!empty($postarr["page_template"])) {
        ...
        if (
            ...
            !isset($page_templates[$postarr["page_template"]])
        ) {
            if ($wp_error) {
                return new WP_Error(
...
function wp_update_post(
    ...
    if (
        isset($postarr["tags_input"]) &&
        is_object_in_taxonomy($postarr["post_type"], "post_tag")
    ) {
...
function wp_unique_post_slug(
    ...
    if (
        in_array($post_status, ["draft", "pending", "auto-draft"], true) ||
    ...
    if ("attachment" === $post_type) {
        ...
        $post_name_check = $wpdb->get_var(
            $wpdb->prepare($check_sql, $slug, $post_id)
        );
         ...
         * @param int    $post_parent Post parent ID.
         */
        $is_bad_hierarchical_slug = apply_filters(
        ...
        if (
            ...
            if ($slug_num) {
                $permastructs = array_values(
                    array_filter(
                        explode("/", get_option("permalink_structure"))
                    )
                );
                $postname_index = array_search(
                    "%postname%",
                    $permastructs,
                    true
                );
         ...
         * @param string $post_type Post type.
         */
        $is_bad_flat_slug = apply_filters(
...
function wp_set_post_categories(
    ...
    if (empty($post_categories)) {
         ...
         */
        $default_category_post_types = apply_filters(
            "default_category_post_types",
            []
        );

        // Regular posts always require a default category.
        $default_category_post_types = array_merge(
            $default_category_post_types,
            ["post"]
        );

        if (
            in_array($post_type, $default_category_post_types, true) &&
...
function trackback_url_list($tb_list, $post_id)
{
    if (!empty($tb_list)) {
        ...
        foreach ((array) $trackback_urls as $tb_url) {
            ...
            trackback(
                $tb_url,
                wp_unslash($postdata["post_title"]),
                $excerpt,
                $post_id
            );
        }
...
function get_pages($args = [])
{
    ...
    $orderby = array_map(static function ($orderby_field) {
        ...
        if (
            "post_modified_gmt" === $orderby_field ||
            "modified_gmt" === $orderby_field
        ) {
    ...
    if ($orderby) {
        $query_args["orderby"] = array_fill_keys(
            $orderby,
            $parsed_args["sort_order"]
        );
    }
...
function wp_delete_attachment_files($post_id, $meta, $backup_sizes, $file)
{
    ...
    if (!empty($meta["original_image"])) {
        if (empty($intermediate_dir)) {
            $intermediate_dir = path_join(
                $uploadpath["basedir"],
                dirname($file)
            );
        }

        $original_image = str_replace(
            wp_basename($file),
            $meta["original_image"],
            $file
        );
...
function wp_mime_type_icon($mime = 0, $preferred_ext = ".png")
{
    ...
    if (empty($icon)) {
        ...
        foreach ($icon_files as $file => $uri) {
            $types[preg_replace('/^([^.]*).*$/', '$1', wp_basename($file))] =
                &$icon_files[$file];
        }
...
function wp_check_for_changed_slugs($post_id, $post, $post_before)
{
    ...
    if (
        ...
        !in_array($post_before->post_name, $old_slugs, true)
    ) {
...
function get_posts_by_author_sql(
    ...
    foreach ($post_types as $post_type) {
        ...
        $post_type_clauses[] =
            "( post_type = '" . $post_type . "' AND ( $post_status_sql ) )";
    }
...
function get_lastpostmodified($timezone = "server", $post_type = "any")
{
     ...
     * @param string       $post_type        The post type to check.
     */
    $lastpostmodified = apply_filters(
...
function _get_last_post_time($timezone, $field, $post_type = "any")
{
    ...
    switch ($timezone) {
        case "gmt":
            $date = $wpdb->get_var(
                "SELECT post_{$field}_gmt FROM $wpdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1"
            );
            break;
        case "blog":
            $date = $wpdb->get_var(
                "SELECT post_{$field} FROM $wpdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1"
            );
            break;
        case "server":
            $add_seconds_server = gmdate("Z");
            $date = $wpdb->get_var(
                "SELECT DATE_ADD(post_{$field}_gmt, INTERVAL '$add_seconds_server' SECOND) FROM $wpdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1"
            );
            break;
    }
...
function _transition_post_status($new_status, $old_status, $post)
{
    ...
    if ("publish" === $new_status || "publish" === $old_status) {
        foreach (["server", "gmt", "blog"] as $timezone) {
            ...
            wp_cache_delete(
                "lastpostdate:$timezone:{$post->post_type}",
                "timeinfo"
            );
        }
    }
    ...
    if ($new_status !== $old_status) {
        ...
        wp_cache_delete(
            _count_posts_cache_key($post->post_type, "readable"),
            "counts"
        );
    }
...
function _future_post_hook($deprecated, $post)
{
    ...
    wp_schedule_single_event(
        strtotime(get_gmt_from_date($post->post_date) . " GMT"),
        "publish_future_post",
        [$post->ID]
    );
}
...
function wp_queue_posts_for_term_meta_lazyload($posts)
{
    ...
    foreach ($posts as $post) {
        ...
        if (!isset($post_type_taxonomies[$post->post_type])) {
            $post_type_taxonomies[$post->post_type] = get_object_taxonomies(
                $post->post_type
            );
        }
    ...
    if ($prime_post_terms) {
        foreach ($prime_post_terms as $taxonomy => $post_ids) {
            $cached_term_ids = wp_cache_get_multiple(
                $post_ids,
...
function _prime_post_caches(
    ...
    if (!empty($non_cached_ids)) {
        $fresh_posts = $wpdb->get_results(
            sprintf(
                "SELECT $wpdb->posts.* FROM $wpdb->posts WHERE ID IN (%s)",
                implode(",", $non_cached_ids)
            )
        );
...
function _prime_post_parent_id_caches(array $ids)
{
    ...
    if (!empty($non_cached_ids)) {
        $fresh_posts = $wpdb->get_results(
            sprintf(
                "SELECT $wpdb->posts.ID, $wpdb->posts.post_parent FROM $wpdb->posts WHERE ID IN (%s)",
                implode(",", $non_cached_ids)
            )
        );
        ...
        if ($fresh_posts) {
            ...
            foreach ($fresh_posts as $fresh_post) {
                $post_parent_data["post_parent:" . (string) $fresh_post->ID] =
                    (int) $fresh_post->post_parent;
            }
