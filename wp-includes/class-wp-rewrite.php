...
class WP_Rewrite
{
    ...
    public function page_rewrite_rules()
    {
        ...
        return $this->generate_rewrite_rules(
            $this->get_page_permastruct(),
            EP_PAGES,
            true,
            true,
            false,
    ...
    public function get_author_permastruct()
    {
        ...
        $this->author_structure =
            $this->front . $this->author_base . "/%author%";
    ...
    public function get_search_permastruct()
    {
        ...
        $this->search_structure =
            $this->root . $this->search_base . "/%search%";
    ...
    public function generate_rewrite_rules(
        ...
        for ($j = 0; $j < $num_dirs; ++$j) {
            ...
            $query =
                !empty($num_toks) && isset($queries[$num_toks - 1])
                    ? $queries[$num_toks - 1]
                    : "";
            ...
            if ($num_toks) {
                ...
                if ($post) {
                    ...
                    $subcommentquery =
                        $subquery . "&cpage=" . $this->preg_index(2);
                    ...
                    if (!empty($endpoints)) {
                        foreach ((array) $ep_query_append as $regex => $ep) {
                            if ($ep[0] & EP_ATTACHMENT) {
                                $rewrite[$sub1 . $regex] =
                                    $subquery . $ep[1] . $this->preg_index(3);
                                $rewrite[$sub2 . $regex] =
                                    $subquery . $ep[1] . $this->preg_index(3);
                            }
                        }
    ...
    public function rewrite_rules()
    {
        ...
        $robots_rewrite =
            empty($home_path["path"]) || "/" === $home_path["path"]
                ? ['robots\.txt$' => $this->index . "?robots=1"]
                : [];

        // favicon.ico -- only if installed at the root.
        $favicon_rewrite =
            empty($home_path["path"]) || "/" === $home_path["path"]
                ? ['favicon\.ico$' => $this->index . "?favicon=1"]
                : [];

        // sitemap.xml -- only if installed at the root.
        $sitemap_rewrite =
            empty($home_path["path"]) || "/" === $home_path["path"]
                ? ["sitemap\.xml" => $this->index . "??sitemap=index"]
                : [];
        ...
        if (is_multisite() && is_main_site()) {
            $registration_pages['.*wp-signup.php$'] =
                $this->index . "?signup=true";
            $registration_pages['.*wp-activate.php$'] =
                $this->index . "?activate=true";
        }

        // Deprecated.
        $registration_pages['.*wp-register.php$'] =
            $this->index . "?register=true";

        // Post rewrite rules.
        $post_rewrite = $this->generate_rewrite_rules(
            $this->permalink_structure,
            EP_PERMALINK
        );
        ...
        // Date rewrite rules.
        $date_rewrite = $this->generate_rewrite_rules(
            $this->get_date_permastruct(),
            EP_DATE
        );
        ...
        $root_rewrite = $this->generate_rewrite_rules(
            $this->root . "/",
            EP_ROOT
        );
        ...
        $comments_rewrite = $this->generate_rewrite_rules(
            $this->root . $this->comments_base,
            EP_COMMENTS,
            false,
            true,
            true,
         ...
         */
        $comments_rewrite = apply_filters(
        ...
        $search_rewrite = $this->generate_rewrite_rules(
            $search_structure,
            EP_SEARCH
        );
         ...
         */
        $search_rewrite = apply_filters(
            "search_rewrite_rules",
            $search_rewrite
        );

        // Author rewrite rules.
        $author_rewrite = $this->generate_rewrite_rules(
            $this->get_author_permastruct(),
            EP_AUTHORS
        );
        ...
        foreach ($this->extra_permastructs as $permastructname => $struct) {
            ...
            $this->extra_rules_top = array_merge(
    ...
    public function add_rule($regex, $query, $after = "bottom")
    {
        ...
        if ($external) {
            ...
            if ("bottom" === $after) {
                $this->extra_rules = array_merge($this->extra_rules, [
                    $regex => $query,
                ]);
            } else {
                $this->extra_rules_top = array_merge($this->extra_rules_top, [
                    $regex => $query,
                ]);
            }
        }
    ...
    public function flush_rules($hard = true)
    {
        ...
        if (!did_action("wp_loaded")) {
            add_action("wp_loaded", [$this, "flush_rules"]);
            $do_hard_later = isset($do_hard_later)
                ? $do_hard_later || $hard
                : $hard;
            return;
        }
    ...
    public function init()
    {
        ...
        $this->front = substr(
            $this->permalink_structure,
            0,
            strpos($this->permalink_structure, "%")
        );
        $this->root = "";
        ...
        $this->use_trailing_slashes = str_ends_with(
            $this->permalink_structure,
            "/"
        );
