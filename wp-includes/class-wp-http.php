...
if (!class_exists("WpOrg\Requests\Autoload")) {
    ...
    WpOrg\Requests\Requests::set_certificate_path(
...
class WP_Http
{
    ...
    public function request($url, $args = [])
    {
        $defaults = [
             ...
             * @param string $url      The request URL.
             */
            "reject_unsafe_urls" => apply_filters(
        ...
        if (
            function_exists("wp_kses_bad_protocol") &&
        ...
        if (!empty($parsed_args["cookies"])) {
            $options["cookies"] = WP_Http::normalize_cookies(
                $parsed_args["cookies"]
            );
        }
    ...
    public static function normalize_cookies($cookies)
    {
        ...
        foreach ($cookies as $name => $value) {
            if ($value instanceof WP_Http_Cookie) {
                ...
                $cookie_jar[$value->name] = new WpOrg\Requests\Cookie(
                    (string) $value->name,
                    $value->value,
                    $attributes,
                    ["host-only" => $value->host_only]
                );
            } elseif (is_scalar($value)) {
                $cookie_jar[$name] = new WpOrg\Requests\Cookie(
                    (string) $name,
                    (string) $value
                );
            }
    ...
    public static function validate_redirects($location)
    {
        if (!wp_http_validate_url($location)) {
            throw new WpOrg\Requests\Exception(
                __("A valid URL was not provided."),
                "wp_http.redirect_failed_validation"
            );
        }
    }
    ...
    public function _get_first_available_transport($args, $url = null)
    {
         ...
         */
        $request_order = apply_filters_deprecated(
            "http_api_transports",
            [$transports, $args, $url],
            "6.4.0"
        );
    ...
    public static function handle_redirects($url, $args, $response)
    {
        ...
        $redirect_location = WP_Http::make_absolute_url(
            $redirect_location,
            $url
        );
