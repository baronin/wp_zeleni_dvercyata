        ...
        if (is_array($blogs) && count($blogs) > 0) {
            ...
            foreach ((array) $blogs as $blog_id => $blog) {
                ...
                if (
                    ...
                    $ret = $details;
                    if (
                        (int) get_user_meta($user_id, "primary_blog", true) !==
                        $blog_id
                    ) {
...
function add_user_to_blog($blog_id, $user_id, $role)
{
    ...
    if (true !== $can_add_user) {
        ...
        return new WP_Error(
            ...
            __("User cannot be added to this site.")
        );
    }
...
function is_email_address_unsafe($user_email)
{
    ...
    if (
        $banned_names &&
        is_array($banned_names) &&
        false !== strpos($user_email, "@", 1)
    ) {
        $banned_names = array_map("strtolower", $banned_names);
        $normalized_email = strtolower($user_email);

        list($email_local_part, $email_domain) = explode(
            "@",
            $normalized_email
        );
...
function wpmu_validate_user_signup($user_name, $user_email)
{
    ...
    if (preg_match('/^[0-9]*$/', $user_name)) {
        $errors->add(
            "user_name",
            __("Sorry, usernames must have letters too!")
        );
    }
    ...
    if (is_array($limited_email_domains) && !empty($limited_email_domains)) {
        $limited_email_domains = array_map(
            "strtolower",
        ...
        $wpdb->get_var(
            $wpdb->prepare(
                "SELECT post_name FROM " .
                    $wpdb->get_blog_prefix($current_network->site_id) .
...
function wpmu_activate_signup(#[\SensitiveParameter] $key)
{
    ...
    if ($signup->active) {
        if (empty($signup->domain)) {
            return new WP_Error(
                "already_active",
                __("The user is already active."),
                $signup
            );
        } else {
            return new WP_Error(
                "already_active",
                __("The site is already active."),
                $signup
            );
        }
    }
    ...
    if (empty($signup->domain)) {
        ...
        if (isset($user_already_exists)) {
            return new WP_Error(
                "user_already_exists",
                __("That username is already activated."),
                $signup
            );
        }
...
function wpmu_create_blog(
    ...
    if (domain_exists($domain, $path, $network_id)) {
        return new WP_Error(
            "blog_taken",
            __("Sorry, that site already exists!")
        );
    }
    ...
    $blog_id = wp_insert_site(
        array_merge($site_data, $site_initialization_data)
    );
...
Disable these notifications: %3$s'
    ...
    wp_mail(
        $email,
        sprintf(__("New User Registration: %s"), $user->user_login),
        $msg
    );
...
function signup_nonce_check($result)
{
    ...
    if (
        !wp_verify_nonce(
            $_POST["_signup_form"],
            "signup_form_" . $_POST["signup_form_id"]
        )
    ) {
        $result["errors"]->add(
            "invalid_nonce",
            __("Unable to submit this form, please try again.")
        );
    }
...
###SITEURL###'
    ...
    // Get network name.
    $network_name = wp_specialchars_decode(
        get_site_option("site_name"),
        ENT_QUOTES
    );
