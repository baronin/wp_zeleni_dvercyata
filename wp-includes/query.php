    ...
    if (is_404() && "" !== get_query_var("name")) {
        ...
        if (get_query_var("paged") > 1) {
            $link = user_trailingslashit(
                trailingslashit($link) . "page/" . get_query_var("paged")
            );
        } elseif (is_embed()) {
...
function _find_post_by_old_date($post_type)
{
    ...
    if ($date_query) {
        $query = $wpdb->prepare(
            "SELECT post_id FROM $wpdb->postmeta AS pm_date, $wpdb->posts WHERE ID = post_id AND post_type = %s AND meta_key = '_wp_old_date' AND post_name = %s" .
                $date_query,
            $post_type,
            get_query_var("name")
        );
        ...
        if (false !== $cache) {
            ...
            if (!$id) {
                ...
                $id = (int) $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT ID FROM $wpdb->posts, $wpdb->postmeta AS pm_slug, $wpdb->postmeta AS pm_date WHERE ID = pm_slug.post_id AND ID = pm_date.post_id AND post_type = %s AND pm_slug.meta_key = '_wp_old_slug' AND pm_slug.meta_value = %s AND pm_date.meta_key = '_wp_old_date'" .
                            $date_query,
                        $post_type,
                        get_query_var("name")
                    )
