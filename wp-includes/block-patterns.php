...
function _load_remote_block_patterns($deprecated = null)
{
     ...
     */
    $should_load_remote = apply_filters(
    ...
    if ($supports_core_patterns && $should_load_remote) {
        ...
        foreach ($patterns as $pattern) {
            ...
            $pattern_name =
                "core/" . sanitize_title($normalized_pattern["title"]);
...
function _load_remote_featured_patterns()
{
    ...
    foreach ($patterns as $pattern) {
        ...
        $is_registered =
            ...
            $registry->is_registered("core/$pattern_name");
        if (!$is_registered) {
            register_block_pattern($pattern_name, $normalized_pattern);
        }
    }
...
function _register_theme_block_patterns()
{
    ...
    foreach ($themes as $theme) {
        ...
        foreach ($patterns as $file => $pattern_data) {
            ...
            $pattern_data["title"] = translate_with_gettext_context(
                $pattern_data["title"],
                "Pattern title",
                $text_domain
            );
            if (!empty($pattern_data["description"])) {
                // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText,WordPress.WP.I18n.NonSingularStringLiteralDomain,WordPress.WP.I18n.LowLevelTranslationFunction
                $pattern_data["description"] = translate_with_gettext_context(
