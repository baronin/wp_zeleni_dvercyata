...
function wp_get_server_protocol()
{
    ...
    if (
        !in_array($protocol, ["HTTP/1.1", "HTTP/2", "HTTP/2.0", "HTTP/3"], true)
    ) {
        $protocol = "HTTP/1.0";
    }
...
function wp_fix_server_vars()
{
    ...
    if (
        ...
        str_ends_with($_SERVER["SCRIPT_FILENAME"], "php.cgi")
    ) {
    ...
    if (
        ...
        str_contains($_SERVER["SCRIPT_NAME"], "php.cgi")
    ) {
...
function wp_get_environment_type()
{
    ...
    $wp_environments = ["local", "development", "staging", "production"];

    // Add a note about the deprecated WP_ENVIRONMENT_TYPES constant.
    if (
        defined("WP_ENVIRONMENT_TYPES") &&
        function_exists("_deprecated_argument")
    ) {
        if (function_exists("__")) {
            /* translators: %s: WP_ENVIRONMENT_TYPES */
            $message = sprintf(
                __("The %s constant is no longer supported."),
...
function wp_get_development_mode()
{
    ...
    if (
        ...
        isset($GLOBALS["_wp_tests_development_mode"])
    ) {
...
function wp_is_maintenance_mode()
{
    ...
    if (
        is_int($upgrading) &&
        isset($_REQUEST["wp_scrape_key"], $_REQUEST["wp_scrape_nonce"])
    ) {
...
function wp_set_lang_dir()
{
    if (!defined("WP_LANG_DIR")) {
        if (
            (file_exists(WP_CONTENT_DIR . "/languages") &&
                @is_dir(WP_CONTENT_DIR . "/languages")) ||
...
function is_protected_ajax_action()
{
     ...
     */
    $actions_to_protect = (array) apply_filters(
            ...
            foreach ($locations as $location) {
                if (file_exists($location . "/" . $locale . ".mo")) {
                    ...
                    if (
                        ...
                        file_exists($location . "/admin-" . $locale . ".mo")
                    ) {
                        load_textdomain(
                            "default",
...
function wp_is_ini_value_changeable($setting)
{
    ...
    if (
        isset($ini_all[$setting]["access"]) &&
        (INI_ALL === $ini_all[$setting]["access"] ||
            INI_USER === $ini_all[$setting]["access"])
    ) {
        return true;
    }
...
function wp_is_json_request()
{
    if (
        ...
        wp_is_json_media_type($_SERVER["HTTP_ACCEPT"])
    ) {
        return true;
    }
    ...
    if (
        ...
        wp_is_json_media_type($_SERVER["CONTENT_TYPE"])
    ) {
        return true;
    }
...
function wp_is_xml_request()
{
    ...
    if (
        isset($_SERVER["CONTENT_TYPE"]) &&
        in_array($_SERVER["CONTENT_TYPE"], $accepted, true)
    ) {
        return true;
    }
