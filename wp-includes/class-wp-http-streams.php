...
class WP_Http_Streams
{
    ...
    public function request($url, $args = [])
    {
        ...
        if (
            isset($parsed_args["headers"]["Host"]) ||
            isset($parsed_args["headers"]["host"])
        ) {
        ...
        $ssl_verify =
            isset($parsed_args["sslverify"]) && $parsed_args["sslverify"];
        ...
        $headers =
            strtoupper($parsed_args["method"]) .
            " " .
            $request_path .
            " HTTP/" .
            $parsed_args["httpversion"] .
            "\r\n";
        ...
        if ($parsed_args["stream"]) {
            ...
            while (!feof($handle) && $keep_reading) {
                ...
                if (!$body_started) {
                    $response .= $block;
                    if (strpos($response, "\r\n\r\n")) {
                        $processed_response = WP_Http::processResponse(
                            $response
                        );
                        $body_started = true;
                ...
                if (
                    ...
                    $this_block_size =
                        $parsed_args["limit_response_size"] - $bytes_written;
                ...
                if ($bytes_written_to_file !== $this_block_size) {
                    ...
                    return new WP_Error(
                        "http_request_failed",
                        __("Failed to write request to temporary file.")
                    );
                }
        ...
        $processed_headers = WP_Http::processHeaders(
        ...
        if (
            ...
            true ===
                WP_Http_Encoding::should_decode($processed_headers["headers"])
        ) {
            $processed_response["body"] = WP_Http_Encoding::decompress(
                $processed_response["body"]
            );
        }
        ...
        if (
            isset($parsed_args["limit_response_size"]) &&
            strlen($processed_response["body"]) >
                $parsed_args["limit_response_size"]
        ) {
            $processed_response["body"] = substr(
                $processed_response["body"],
                0,
                $parsed_args["limit_response_size"]
            );
