...
function determine_locale()
{
    ...
    if (
        ...
        (!empty($_REQUEST["language"]) ||
            isset($GLOBALS["wp_local_package"])) &&
        wp_installing()
    ) {
...
{
    $translations = get_translations_for_domain($domain);
    $translation = $translations->translate_plural(
        $single,
        $plural,
        $number,
        $context
...
function translate_nooped_plural($nooped_plural, $count, $domain = "default")
{
    ...
    if ($nooped_plural["context"]) {
        return _nx(
            $nooped_plural["singular"],
            $nooped_plural["plural"],
            $count,
...
function load_textdomain($domain, $mofile, $locale = null)
{
    ...
    if (!in_array($preferred_format, ["php", "mo"], true)) {
    ...
    if ("mo" !== $preferred_format) {
        $translation_files[] = substr_replace(
            $mofile,
            ".l10n.$preferred_format",
            -strlen(".mo")
        );
    }
    ...
    foreach ($translation_files as $file) {
        ...
        if ($success) {
            if (isset($l10n[$domain]) && $l10n[$domain] instanceof MO) {
                $i18n_controller->load_file(
                    $l10n[$domain]->get_filename(),
                    $domain,
                    $locale
                );
            }
...
function load_default_textdomain($locale = null)
{
    ...
    if (
        ...
        !file_exists(WP_LANG_DIR . "/admin-$locale.mo")
    ) {
...
function load_script_textdomain($handle, $domain = "default", $path = "")
{
    ...
    if (
        (!isset($content_url["path"]) ||
            str_starts_with($src_url["path"], $content_url["path"])) &&
        (!isset($src_url["host"]) ||
            !isset($content_url["host"]) ||
            $src_url["host"] === $content_url["host"])
    ) {
        ...
        (!isset($plugins_url["path"]) ||
            str_starts_with($src_url["path"], $plugins_url["path"])) &&
        (!isset($src_url["host"]) ||
            !isset($plugins_url["host"]) ||
            $src_url["host"] === $plugins_url["host"])
    ) {
...
function _load_textdomain_just_in_time($domain)
{
    ...
    if (
        !doing_action("after_setup_theme") &&
    ...
    if (
        str_starts_with($path, $template_directory) ||
...
function is_textdomain_loaded($domain)
{
    ...
    return isset($l10n[$domain]) &&
        !$l10n[$domain] instanceof NOOP_Translations;
}
...
function get_available_languages($dir = null)
{
    ...
    if ($lang_files) {
        foreach ($lang_files as $lang_file) {
            ...
            if (
                !str_starts_with($lang_file, "continents-cities") &&
                !str_starts_with($lang_file, "ms-") &&
...
function wp_get_installed_translations($type)
{
    ...
    foreach ($files as $file) {
        if (
            !preg_match(
                "/(?:(.+)-)?([a-z]{2,3}(?:_[A-Z]{2})?(?:_[a-z0-9]+)?)\.(?:mo|l10n\.php)/",
                basename($file),
                $match
            )
        ) {
        ...
        if (str_ends_with($file, ".mo")) {
            ...
            $language_data[$textdomain][$language] = wp_get_l10n_php_file_data(
                $file
            );
        }
    }
...
function wp_dropdown_languages($args = [])
{
    ...
    if (
        ...
        !$parsed_args["explicit_option_en_us"]
    ) {
    ...
    $translations_available =
        !empty($translations) && $parsed_args["show_available_translations"];
...
function translate_settings_using_i18n_schema(
    ...
    if (is_array($i18n_schema) && is_array($settings)) {
        ...
        foreach ($settings as $value) {
            $translated_settings[] = translate_settings_using_i18n_schema(
                $i18n_schema[0],
                $value,
                $textdomain
            );
...
function has_translation(
    ...
    return WP_Translation_Controller::get_instance()->has_translation(
        $singular,
        $textdomain,
        $locale
    );
