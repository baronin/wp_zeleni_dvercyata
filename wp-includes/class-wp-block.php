...
class WP_Block
{
    ...
    public function refresh_context_dependents()
    {
         ...
         */
        $this->available_context = array_merge(
            ...
            foreach ($this->block_type->uses_context as $context_name) {
                if (array_key_exists($context_name, $this->available_context)) {
                    $this->context[$context_name] =
                        $this->available_context[$context_name];
                }
            }
        }
        ...
        if (!empty($this->parsed_block["innerBlocks"])) {
            ...
            if (!empty($this->block_type->provides_context)) {
                foreach (
                    ...
                    as $context_name => $attribute_name
                ) {
    ...
    private function process_block_bindings()
    {
        ...
        if (
             ...
             */
            foreach (
                $supported_block_attributes[$parsed_block["blockName"]]
                as $attribute_name
            ) {
                // Retain any non-pattern override bindings that might be present.
                $updated_bindings[$attribute_name] = isset(
        ...
        foreach ($bindings as $attribute_name => $block_binding) {
            ...
            if (
                !in_array(
                    $attribute_name,
                    $supported_block_attributes[$this->name],
                    true
                )
            ) {
                continue;
            ...
            if (
                !isset($block_binding["source"]) ||
                !is_string($block_binding["source"])
            ) {
                continue;
            }

            $block_binding_source = get_block_bindings_source(
                $block_binding["source"]
            );
            ...
            $source_args =
                !empty($block_binding["args"]) &&
                is_array($block_binding["args"])
                    ? $block_binding["args"]
                    : [];
            $source_value = $block_binding_source->get_value(
                $source_args,
                $this,
                $attribute_name
            );
    ...
    private function replace_html(
        ...
        switch ($block_type->attributes[$attribute_name]["source"]) {
            ...
            case "rich-text":
                ...
                $selectors = explode(
                    ",",
                    $block_type->attributes[$attribute_name]["selector"]
                );
                ...
                if ("core/button" === $this->name) {
                    ...
                    foreach ($button_wrapper_attribute_names as $name) {
                        $button_wrapper_attrs[
                            $name
                        ] = $block_reader->get_attribute($name);
                    }
                }
                ...
                foreach ($selectors as $selector) {
                    ...
                    if (
                        ...
                        $selector_markup =
                            "<$selector>" .
                            wp_kses_post($source_value) .
                            "</$selector>";
                        $amended_content = new WP_HTML_Tag_Processor(
                            $selector_markup
                        );
                        ...
                        foreach (
                            ...
                            $amended_content->set_attribute(
                                $attribute_key,
                                $attribute_value
                            );
                        }
                        if (
                            "core/paragraph" === $this->name ||
                            "core/heading" === $this->name
                        ) {
                            return $amended_content->get_updated_html();
                        }
                        if ("core/button" === $this->name) {
                            $button_markup = "<$button_wrapper>{$amended_content->get_updated_html()}</$button_wrapper>";
                            $amended_button = new WP_HTML_Tag_Processor(
                                $button_markup
                            );
    ...
    public function render($options = [])
    {
         ...
         */
        $interactivity_process_directives_enabled = apply_filters(
        ...
        if (
            ...
            ((isset($this->block_type->supports["interactivity"]) &&
                true === $this->block_type->supports["interactivity"]) ||
                !empty(
        ...
        if (
            !$options["dynamic"] ||
            empty($this->block_type->skip_inner_blocks)
        ) {
            $index = 0;
        ...
        if (!empty($computed_attributes) && !empty($block_content)) {
            foreach ($computed_attributes as $attribute_name => $source_value) {
                $block_content = $this->replace_html(
                    $block_content,
                    $attribute_name,
                    $source_value
                );
        ...
        if (!empty($this->block_type->view_script_handles)) {
            foreach (
                $this->block_type->view_script_handles
                as $view_script_handle
            ) {
        ...
        if (!empty($this->block_type->view_style_handles)) {
            foreach (
                $this->block_type->view_style_handles
                as $view_style_handle
            ) {
        ...
        if ($root_interactive_block === $this) {
            ...
            $block_content = wp_interactivity_process_directives(
                $block_content
            );
