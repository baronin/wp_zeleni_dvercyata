...
class WP_Network_Query
{
    ...
    public function get_networks()
    {
         ...
         * @param WP_Network_Query $query        The WP_Network_Query instance, passed by reference.
         */
        $network_data = apply_filters_ref_array("networks_pre_query", [
            $network_data,
            &$this,
        ]);
        ...
        if ($this->found_networks && $this->query_vars["number"]) {
            $this->max_num_pages = (int) ceil(
                $this->found_networks / $this->query_vars["number"]
            );
        }
         ...
         * @param WP_Network_Query $query     Current instance of WP_Network_Query (passed by reference).
         */
        $_networks = apply_filters_ref_array("the_networks", [
            $_networks,
            &$this,
        ]);
    ...
    protected function get_network_ids()
    {
        ...
        if (strlen($this->query_vars["search"])) {
            $this->sql_clauses["where"]["search"] = $this->get_search_sql(
                ...
                ["$wpdb->site.domain", "$wpdb->site.path"]
            );
        }
         ...
         * @param WP_Network_Query $query   Current instance of WP_Network_Query (passed by reference).
         */
        $clauses = apply_filters_ref_array("networks_clauses", [
            compact($pieces),
            &$this,
        ]);
