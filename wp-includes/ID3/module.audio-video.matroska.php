...
class getid3_matroska extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        // calculate playtime
        if (
            isset($info["matroska"]["info"]) &&
            is_array($info["matroska"]["info"])
        ) {
            foreach ($info["matroska"]["info"] as $key => $infoarray) {
                if (isset($infoarray["Duration"])) {
                    ...
                    $info["playtime_seconds"] =
                        $infoarray["Duration"] *
                        ((isset($infoarray["TimecodeScale"])
                            ? $infoarray["TimecodeScale"]
                            : 1000000) /
                            1000000000);
                    break;
                }
        ...
        // extract tags
        if (
            isset($info["matroska"]["tags"]) &&
            is_array($info["matroska"]["tags"])
        ) {
        ...
        // process tracks
        if (
            isset($info["matroska"]["tracks"]["tracks"]) &&
            is_array($info["matroska"]["tracks"]["tracks"])
        ) {
            foreach (
                ...
                $track_info = [];
                $track_info["dataformat"] = self::CodecIDtoCommonName(
                    $trackarray["CodecID"]
                );
                $track_info["default"] = isset($trackarray["FlagDefault"])
                    ? $trackarray["FlagDefault"]
                    : true;
                ...
                switch ($trackarray["TrackType"]) {
                    case 1: // Video
                        ...
                        $track_info["resolution_y"] =
                            $trackarray["PixelHeight"];
                        $track_info["display_unit"] = self::displayUnit(
                            isset($trackarray["DisplayUnit"])
                                ? $trackarray["DisplayUnit"]
                                : 0
                        );
                        $track_info["display_x"] = isset(
                            $trackarray["DisplayWidth"]
                        )
                            ? $trackarray["DisplayWidth"]
                            : $trackarray["PixelWidth"];
                        $track_info["display_y"] = isset(
                        ...
                        if (!empty($trackarray["DefaultDuration"])) {
                            $track_info["frame_rate"] = round(
                                1000000000 / $trackarray["DefaultDuration"],
                                3
                            );
                        }
                        ...
                        switch ($trackarray["CodecID"]) {
                            case "V_MS/VFW/FOURCC":
                                ...
                                $parsed = getid3_riff::ParseBITMAPINFOHEADER(
                    ...
                    case 2: // Audio
                        $track_info["sample_rate"] = isset(
                            $trackarray["SamplingFrequency"]
                        )
                            ? $trackarray["SamplingFrequency"]
                            : 8000.0;
                        $track_info["channels"] = isset($trackarray["Channels"])
                            ? $trackarray["Channels"]
                            : 1;
                        $track_info["language"] = isset($trackarray["Language"])
                            ? $trackarray["Language"]
                            : "eng";
                        ...
                        switch ($trackarray["CodecID"]) {
                            ...
                            case "A_FLAC":
                                ...
                                if (
                                    ...
                                    $this->warning(
                                        "Unable to parse audio data [" .
                                            ...
                                            ":" .
                                            __LINE__ .
                                            '] because $info[matroska][track_data_offsets][' .
                                            $trackarray["TrackNumber"] .
                                            "] not set"
                                    );
                                    break;
                                ...
                                $header_data_key =
                                    $track_info["dataformat"][0] == "m"
                                        ? "mpeg"
                                        : $track_info["dataformat"];
                                $getid3_audio = new $class(
                                    $getid3_temp,
                                    __CLASS__
                                );
                                if ($track_info["dataformat"] == "flac") {
                                    $getid3_audio->AnalyzeString(
                                        $trackarray["CodecPrivate"]
                                    );
                                } else {
                                ...
                                if (
                                    !empty($getid3_temp->info[$header_data_key])
                                ) {
                                    $info["matroska"]["track_codec_parsed"][
                                        $trackarray["TrackNumber"]
                                    ] = $getid3_temp->info[$header_data_key];
                                    if (
                                        isset($getid3_temp->info["audio"]) &&
                                        is_array($getid3_temp->info["audio"])
                                    ) {
                                        foreach (
                            ...
                            case "A_VORBIS":
                                ...
                                getid3_lib::IncludeDependency(
                                    GETID3_INCLUDEPATH . "module.audio.ogg.php",
                                    __FILE__,
                                    true
                                );
                            ...
                            case "A_MS/ACM":
                                ...
                                $parsed = getid3_riff::parseWAVEFORMATex(
                                    $trackarray["CodecPrivate"]
                                );
                            ...
                            default:
                                $this->warning(
                                    'Unhandled audio type "' .
                                        (isset($trackarray["CodecID"])
                                            ? $trackarray["CodecID"]
                                            : "") .
                                        '"'
                                );
            ...
            if (!empty($info["video"]["streams"])) {
                $info["video"] = self::getDefaultStreamInfo(
                    $info["video"]["streams"]
                );
            }
            if (!empty($info["audio"]["streams"])) {
                $info["audio"] = self::getDefaultStreamInfo(
                    $info["audio"]["streams"]
                );
            }
        }
        ...
        // process attachments
        if (
            isset($info["matroska"]["attachments"]) &&
            $this->getid3->option_save_attachments !== getID3::ATTACHMENTS_NONE
        ) {
            foreach ($info["matroska"]["attachments"] as $i => $entry) {
                if (
                    strpos($entry["FileMimeType"], "image/") === 0 &&
                    !empty($entry["FileData"])
                ) {
        ...
        if (!empty($info["matroska"]["tags"])) {
            ...
            foreach ($info["matroska"]["tags"] as $key1 => $value1) {
                if (
                    !empty($value1["Targets"]["TagTrackUID"][0]) &&
                    !empty($value1["SimpleTag"])
                ) {
                    foreach ($value1["SimpleTag"] as $key2 => $value2) {
                        if (
                            !empty($value2["TagName"]) &&
                            isset($value2["TagString"])
                        ) {
                            $_STATISTICS_byTrackUID[
                                $value1["Targets"]["TagTrackUID"][0]
                            ][$value2["TagName"]] = $value2["TagString"];
                        }
                    }
                }
            }
            foreach (["audio", "video"] as $avtype) {
                if (!empty($info[$avtype]["streams"])) {
                    foreach (
                        $info[$avtype]["streams"]
                        as $trackUID => $trackdata
                    ) {
                        if (
                            !isset($trackdata["bitrate"]) &&
                            !empty($_STATISTICS_byTrackUID[$trackUID]["BPS"])
                        ) {
                            $info[$avtype]["streams"][$trackUID]["bitrate"] =
                                (int) $_STATISTICS_byTrackUID[$trackUID]["BPS"];
                            @$info[$avtype]["bitrate"] +=
                                $info[$avtype]["streams"][$trackUID]["bitrate"];
                        }
                    }
    ...
    private function parseEBML(&$info)
    {
        ...
        while ($this->getEBMLelement($top_element, $info["avdataend"])) {
            switch ($top_element["id"]) {
                case EBML_ID_EBML:
                    $info["matroska"]["header"]["offset"] =
                        $top_element["offset"];
                    $info["matroska"]["header"]["length"] =
                        $top_element["length"];
                    ...
                    while (
                        ...
                        switch ($element_data["id"]) {
                            ...
                            case EBML_ID_DOCTYPE:
                                ...
                                $info["matroska"]["doctype"] =
                                    $element_data["data"];
                                $info["fileformat"] = $element_data["data"];
                                break;

                            default:
                                $this->unhandledElement(
                                    "header",
                                    __LINE__,
                                    $element_data
                                );
                ...
                case EBML_ID_SEGMENT:
                    $info["matroska"]["segment"][0]["offset"] =
                        $top_element["offset"];
                    $info["matroska"]["segment"][0]["length"] =
                        $top_element["length"];
                    ...
                    while (
                        ...
                        if (
                            $element_data["id"] != EBML_ID_CLUSTER ||
                            !$this->hide_clusters
                        ) {
                                ...
                                while (
                                    ...
                                    switch ($seek_entry["id"]) {
                                        case EBML_ID_SEEK: // Contains a single seek entry to an EBML element
                                            ...
                                            if (
                                                !isset($seek_entry["target_id"])
                                            ) {
                                                $this->warning(
                                                    "seek_entry[target_id] unexpectedly not set at " .
                                                        $seek_entry["offset"]
                                                );
                                                break;
                                            }
                                                                            ...
                                                                            case EBML_ID_CONTENTCOMPRESSION:
                                                                                while (
                                                                                    ...
                                                                                    switch (
                                                                                        ...
                                                                                        default:
                                                                                            $this->unhandledElement(
                                                                                                "track.contentencodings.contentencoding.contentcompression",
                                                                                                __LINE__,
                                                                                                $sub_sub_sub_subelement
                                                                                            );
                                                                                            break;
                                                    ...
                                                    case EBML_ID_CUETRACKPOSITIONS:
                                                        ...
                                                        while (
                                                            ...
                                                            switch (
                                                                ...
                                                                default:
                                                                    $this->unhandledElement(
                                                                        "cues.cuepoint.cuetrackpositions",
                                                                        __LINE__,
                                                                        $sub_sub_subelement
                                                                    );
                                                                    break;
                                                                ...
                                                                case EBML_ID_CHAPTERTRACK:
                                                                    ...
                                                                    while (
                                                                        ...
                                                                        switch (
                                                                            ...
                                                                            default:
                                                                                $this->unhandledElement(
                                                                                    "chapters.editionentry.chapteratom.chaptertrack",
                                                                                    __LINE__,
                                                                                    $sub_sub_sub_subelement
                                                                                );
                                                                                break;
                                                                ...
                                                                case EBML_ID_CHAPTERDISPLAY:
                                                                    ...
                                                                    while (
                                                                        ...
                                                                        switch (
                                                                            ...
                                                                            default:
                                                                                $this->unhandledElement(
                                                                                    "chapters.editionentry.chapteratom.chapterdisplay",
                                                                                    __LINE__,
                                                                                    $sub_sub_sub_subelement
                                                                                );
                                                                                break;
                                            ...
                                            while (
                                                ...
                                                switch ($sub_subelement["id"]) {
                                                    case EBML_ID_CLUSTERSILENTTRACKNUMBER:
                                                        $cluster_silent_tracks[] = getid3_lib::BigEndian2Int(
                                                            $sub_subelement[
                                                                "data"
                                                            ]
                                                        );
    ...
    {
        if (
            $this->current_offset - $this->EBMLbuffer_offset >=
            $this->EBMLbuffer_length - $min_data
        ) {
            ...
            if ($this->EBMLbuffer_length == 0 && $this->feof()) {
                return $this->error(
                    "EBML parser: ran out of file at offset " .
                        $this->current_offset
                );
            }
        }
    ...
    private function readEBMLint()
    {
        ...
        if (0x80 & $first_byte_int) {
            ...
            throw new Exception(
                "invalid EBML integer (leading 0x00) at " .
                    $this->current_offset
            );
        }

        // read
        $int_value = self::EBML2Int(
            substr($this->EBMLbuffer, $actual_offset, $length)
        );
    ...
    private function getEBMLelement(&$element, $parent_end, $get_data = false)
    {
        ...
        if (
            ($get_data === true ||
                (is_array($get_data) &&
                    !in_array($element["id"], $get_data))) &&
            !$dont_parse
        ) {
            $element["data"] = $this->readEBMLelementData(
                $element["length"],
                $element
            );
        }
    ...
    private function ExtractCommentsSimpleTag($SimpleTagArray)
    {
        if (!empty($SimpleTagArray["SimpleTag"])) {
            foreach (
                ...
                as $SimpleTagKey => $SimpleTagData
            ) {
                if (
                    !empty($SimpleTagData["TagName"]) &&
                    !empty($SimpleTagData["TagString"])
                ) {
                    $this->getid3->info["matroska"]["comments"][
                        strtolower($SimpleTagData["TagName"])
                    ][] = $SimpleTagData["TagString"];
                }
    ...
    private function HandleEMBLSimpleTag($parent_end)
    {
        ...
        while (
            $this->getEBMLelement($element, $parent_end, [EBML_ID_SIMPLETAG])
        ) {
            switch ($element["id"]) {
                ...
                case EBML_ID_SIMPLETAG:
                    $simpletag_entry[
                        $element["id_name"]
                    ][] = $this->HandleEMBLSimpleTag($element["end"]);
                    break;

                case EBML_ID_TAGDEFAULT:
                    $simpletag_entry[
                        $element["id_name"]
                    ] = (bool) getid3_lib::BigEndian2Int($element["data"]);
                    break;
    ...
    private function HandleEMBLClusterBlock($element, $block_type, &$info)
    {
        ...
        $block_data["timecode"] = getid3_lib::BigEndian2Int(
            $this->readEBMLelementData(2),
            false,
            true
        );
        $block_data["flags_raw"] = getid3_lib::BigEndian2Int(
            $this->readEBMLelementData(1)
        );

        if ($block_type == EBML_ID_CLUSTERSIMPLEBLOCK) {
            $block_data["flags"]["keyframe"] =
                ($block_data["flags_raw"] & 0x80) >> 7;
            //$block_data['flags']['reserved1'] = (($block_data['flags_raw'] & 0x70) >> 4);
        } else {
            //$block_data['flags']['reserved1'] = (($block_data['flags_raw'] & 0xF0) >> 4);
        }
        $block_data["flags"]["invisible"] =
            (bool) (($block_data["flags_raw"] & 0x08) >> 3);
        $block_data["flags"]["lacing"] = ($block_data["flags_raw"] & 0x06) >> 1; // 00=no lacing; 01=Xiph lacing; 11=EBML lacing; 10=fixed-size lacing
        if ($block_type == EBML_ID_CLUSTERSIMPLEBLOCK) {
            $block_data["flags"]["discardable"] =
                $block_data["flags_raw"] & 0x01;
        } else {
            //$block_data['flags']['reserved2'] = (($block_data['flags_raw'] & 0x01) >> 0);
        }
        $block_data["flags"]["lacing_type"] = self::BlockLacingType(
            $block_data["flags"]["lacing"]
        );

        // Lace (when lacing bit is set)
        if ($block_data["flags"]["lacing"] > 0) {
            $block_data["lace_frames"] =
                getid3_lib::BigEndian2Int($this->readEBMLelementData(1)) + 1; // Number of frames in the lace-1 (uint8)
            if ($block_data["flags"]["lacing"] != 0x02) {
                for ($i = 1; $i < $block_data["lace_frames"]; $i++) {
                    ...
                    if ($block_data["flags"]["lacing"] == 0x03) {
                        // EBML lacing
                        $block_data["lace_frames_size"][
                            $i
                        ] = $this->readEBMLint(); // TODO: read size correctly, calc size for the last frame. For now offsets are deteminded OK with readEBMLint() and that's the most important thing.
                    } else {
                        // Xiph lacing
                        $block_data["lace_frames_size"][$i] = 0;
                        do {
                            $size = getid3_lib::BigEndian2Int(
                                $this->readEBMLelementData(1)
                            );
        ...
        if (
            ...
            $info["matroska"]["track_data_offsets"][$block_data["tracknumber"]][
                "length"
            ] = $element["end"] - $this->current_offset;
    ...
    public static function TargetTypeValue($target_type)
    {
        ...
        if (empty($TargetTypeValue)) {
            ...
            $TargetTypeValue[50] =
                "A:album/opera/concert ~ V:movie/episode/concert"; // the most common grouping level of music and video (equals to an episode for TV series)
            $TargetTypeValue[60] =
                "A:edition/issue/volume/opus ~ V:season/sequel/volume"; // a list of lower levels grouped together
            $TargetTypeValue[70] = "A:collection ~ V:collection"; // the high hierarchy consisting of many different lower items
        }
        return isset($TargetTypeValue[$target_type])
    ...
    public static function BlockLacingType($lacingtype)
    {
        ...
        return isset($BlockLacingType[$lacingtype])
    ...
    public static function CodecIDtoCommonName($codecid)
    {
        ...
        return isset($CodecIDlist[$codecid])
            ? $CodecIDlist[$codecid]
            : $codecid;
    }
    ...
    private static function EBMLidName($value)
    {
        ...
        if (empty($EBMLidList)) {
            ...
            $EBMLidList[EBML_ID_CHAPTERSEGMENTEDITIONUID] =
            ...
            $EBMLidList[EBML_ID_CHAPTERTRANSLATECODEC] =
                "ChapterTranslateCodec";
            $EBMLidList[EBML_ID_CHAPTERTRANSLATEEDITIONUID] =
            ...
            $EBMLidList[EBML_ID_CLUSTERBLOCKADDITIONAL] =
                "ClusterBlockAdditional";
            $EBMLidList[EBML_ID_CLUSTERBLOCKADDITIONID] =
                "ClusterBlockAdditionID";
            $EBMLidList[EBML_ID_CLUSTERBLOCKADDITIONS] =
            ...
            $EBMLidList[EBML_ID_CLUSTERENCRYPTEDBLOCK] =
            ...
            $EBMLidList[EBML_ID_CLUSTERREFERENCEPRIORITY] =
                "ClusterReferencePriority";
            $EBMLidList[EBML_ID_CLUSTERREFERENCEVIRTUAL] =
                "ClusterReferenceVirtual";
            $EBMLidList[EBML_ID_CLUSTERSILENTTRACKNUMBER] =
            ...
            $EBMLidList[EBML_ID_OUTPUTSAMPLINGFREQUENCY] =
            ...
            $EBMLidList[EBML_ID_TRACKTRANSLATETRACKID] =
