...
class getid3_ac3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        $this->AC3header["syncinfo"] = getid3_lib::BigEndian2Int(
            substr($tempAC3header, 0, 2)
        );
        $this->AC3header["bsi"] = getid3_lib::BigEndian2Bin(
            substr($tempAC3header, 2)
        );
        $thisfile_ac3_raw_bsi["bsid"] =
            (getid3_lib::LittleEndian2Int(substr($tempAC3header, 5, 1)) &
                0xf8) >>
            ...
            if (!$this->isDependencyFor("matroska")) {
                ...
                return $this->error(
                    'Expecting "' .
                        ...
                        $info["avdataoffset"] .
                        ', found "' .
                        dechex($this->AC3header["syncinfo"]) .
                        '"'
                );
            }
        }
        ...
        if ($thisfile_ac3_raw_bsi["bsid"] <= 8) {
            $thisfile_ac3_raw_bsi["crc1"] = getid3_lib::Bin2Dec(
                $this->readHeaderBSI(16)
            );
            ...
            if ($thisfile_ac3_raw_bsi["frmsizecod"] > 37) {
                ...
                $this->warning(
                    "Unexpected ac3.bsi.frmsizecod value: " .
            ...
            if ($thisfile_ac3_raw_bsi["acmod"] & 0x01) {
                ...
                $thisfile_ac3["center_mix_level"] = self::centerMixLevelLookup(
                    $thisfile_ac3_raw_bsi["cmixlev"]
                );
            }
            ...
            if ($thisfile_ac3_raw_bsi["acmod"] & 0x04) {
                ...
                $thisfile_ac3[
                    "surround_mix_level"
                ] = self::surroundMixLevelLookup(
                    $thisfile_ac3_raw_bsi["surmixlev"]
                );
            }
            ...
            if ($thisfile_ac3_raw_bsi["acmod"] == 0x02) {
                ...
                $thisfile_ac3[
                    "dolby_surround_mode"
                ] = self::dolbySurroundModeLookup(
                    $thisfile_ac3_raw_bsi["dsurmod"]
                );
            }

            $thisfile_ac3_raw_bsi["flags"][
                "lfeon"
            ] = (bool) $this->readHeaderBSI(1);
            ...
            $thisfile_ac3_raw_bsi["flags"][
                "compr"
            ] = (bool) $this->readHeaderBSI(1); // 5.4.2.9 compre: Compression Gain Word Exists, 1 Bit
            if ($thisfile_ac3_raw_bsi["flags"]["compr"]) {
                ...
                $thisfile_ac3["heavy_compression"] = self::heavyCompression(
                    $thisfile_ac3_raw_bsi["compr"]
                );
            }

            $thisfile_ac3_raw_bsi["flags"][
                "langcod"
            ] = (bool) $this->readHeaderBSI(1); // 5.4.2.11 langcode: Language Code Exists, 1 Bit
            ...
            $thisfile_ac3_raw_bsi["flags"][
                "audprodinfo"
            ] = (bool) $this->readHeaderBSI(1); // 5.4.2.13 audprodie: Audio Production Information Exists, 1 Bit
            if ($thisfile_ac3_raw_bsi["flags"]["audprodinfo"]) {
                ...
                $thisfile_ac3["mixing_level"] =
                    80 + $thisfile_ac3_raw_bsi["mixlevel"] . "dB";
                $thisfile_ac3["room_type"] = self::roomTypeLookup(
                    $thisfile_ac3_raw_bsi["roomtyp"]
                );
            }

            $thisfile_ac3_raw_bsi["dialnorm2"] = $this->readHeaderBSI(5); // 5.4.2.16 dialnorm2: Dialogue Normalization, ch2, 5 Bits
            $thisfile_ac3["dialogue_normalization2"] =
                "-" . $thisfile_ac3_raw_bsi["dialnorm2"] . "dB"; // This indicates how far the average dialogue level is below digital 100 percent. Valid values are 1-31. The value of 0 is reserved. The values of 1 to 31 are interpreted as -1 dB to -31 dB with respect to digital 100 percent.

            $thisfile_ac3_raw_bsi["flags"][
                "compr2"
            ] = (bool) $this->readHeaderBSI(1); // 5.4.2.17 compr2e: Compression Gain Word Exists, ch2, 1 Bit
            if ($thisfile_ac3_raw_bsi["flags"]["compr2"]) {
                ...
                $thisfile_ac3["heavy_compression2"] = self::heavyCompression(
                    $thisfile_ac3_raw_bsi["compr2"]
                );
            }

            $thisfile_ac3_raw_bsi["flags"][
                "langcod2"
            ] = (bool) $this->readHeaderBSI(1); // 5.4.2.19 langcod2e: Language Code Exists, ch2, 1 Bit
            ...
            $thisfile_ac3_raw_bsi["flags"][
                "audprodinfo2"
            ] = (bool) $this->readHeaderBSI(1); // 5.4.2.21 audprodi2e: Audio Production Information Exists, ch2, 1 Bit
            if ($thisfile_ac3_raw_bsi["flags"]["audprodinfo2"]) {
                ...
                $thisfile_ac3["mixing_level2"] =
                    80 + $thisfile_ac3_raw_bsi["mixlevel2"] . "dB";
                $thisfile_ac3["room_type2"] = self::roomTypeLookup(
                    $thisfile_ac3_raw_bsi["roomtyp2"]
                );
            }
            ...
            $thisfile_ac3_raw_bsi["flags"]["timecod1"] = $this->readHeaderBSI(
                2
            ...
            if ($thisfile_ac3_raw_bsi["flags"]["timecod1"] & 0x02) {
                ...
                $thisfile_ac3["timecode2"] +=
                    (($thisfile_ac3_raw_bsi["timecod2"] & 0x07c0) >> 6) *
                    (1 / 30); // The next 5 bits represents the time in frames, with valid values from 0�29 (one frame = 1/30th of a second)
                $thisfile_ac3["timecode2"] +=
                    (($thisfile_ac3_raw_bsi["timecod2"] & 0x003f) >> 0) *
                    (1 / 30 / 60); // The final 6 bits represents fractions of 1/64 of a frame, with valid values from 0�63
            }
            ...
            if ($thisfile_ac3_raw_bsi["flags"]["addbsi"]) {
                $thisfile_ac3_raw_bsi["addbsi_length"] =
                    $this->readHeaderBSI(6) + 1; // This 6-bit code, which exists only if addbside is a 1, indicates the length in bytes of additional bit stream information. The valid range of addbsil is 0�63, indicating 1�64 additional bytes, respectively.

                $this->AC3header["bsi"] .= getid3_lib::BigEndian2Bin(
                    $this->fread($thisfile_ac3_raw_bsi["addbsi_length"])
                );

                $thisfile_ac3_raw_bsi["addbsi_data"] = substr(
                    $this->AC3header["bsi"],
                    $this->BSIoffset,
            ...
            $this->error(
                "E-AC3 parsing is incomplete and experimental in this version of getID3 (" .
                    $this->getid3->version() .
            ...
            $thisfile_ac3["bsi"][
                "blocks_per_sync_frame"
            ] = self::blocksPerSyncFrame($thisfile_ac3_raw_bsi["numblkscod"]);
            $thisfile_ac3_raw_bsi["acmod"] = $this->readHeaderBSI(3);
            $thisfile_ac3_raw_bsi["flags"][
                "lfeon"
            ] = (bool) $this->readHeaderBSI(1);
            ...
            if ($thisfile_ac3_raw_bsi["flags"]["mixmdat"]) {
                ...
                if (
                    ...
                    $thisfile_ac3_raw_bsi["ltrtcmixlev"] = $this->readHeaderBSI(
                        3
                    );
                    $thisfile_ac3_raw_bsi["lorocmixlev"] = $this->readHeaderBSI(
                        3
                    );
                }
                if ($thisfile_ac3_raw_bsi["acmod"] & 0x04) {
                ...
                if ($thisfile_ac3_raw_bsi["strmtyp"] == 0) {
                    ...
                    if ($thisfile_ac3_raw_bsi["flags"]["pgmscl"]) {
                        $thisfile_ac3_raw_bsi["pgmscl"] = $this->readHeaderBSI(
                            6
                        );
                    }
                    if ($thisfile_ac3_raw_bsi["acmod"] == 0) {
                    ...
                    if ($thisfile_ac3_raw_bsi["mixdef"] == 1) {
                        ...
                        // mixing option 3
                        $thisfile_ac3_raw_bsi["mixdata"] = $this->readHeaderBSI(
                            12
                        );
                        ...
                        $mixdata_bits =
                            8 * ($thisfile_ac3_raw_bsi["mixdeflen"] + 2) -
                            $mixdefbitsread;
                        $mixdata_fill =
                            $mixdata_bits % 8 ? 8 - ($mixdata_bits % 8) : 0;
                        $thisfile_ac3_raw_bsi["mixdata"] = $this->readHeaderBSI(
                            $mixdata_bits
                        );
                        $thisfile_ac3_raw_bsi[
                            "mixdatafill"
                        ] = $this->readHeaderBSI($mixdata_fill);
                    ...
                    if ($thisfile_ac3_raw_bsi["flags"]["frmmixcfginfo"]) {
                        ...
                        if ($thisfile_ac3_raw_bsi["numblkscod"] == 0) {
                            $thisfile_ac3_raw_bsi[
                                "blkmixcfginfo"
                            ][0] = $this->readHeaderBSI(5);
                        } else {
                            for (
                                $blk = 0;
                                $blk < $thisfile_ac3_raw_bsi["numblkscod"];
                                $blk++
                            ) {
                                $thisfile_ac3_raw_bsi["flags"][
                                    "blkmixcfginfo" . $blk
                                ...
                                if (
                                    ...
                                    // mixing configuration information
                                    $thisfile_ac3_raw_bsi["blkmixcfginfo"][
                                        $blk
                                    ] = $this->readHeaderBSI(5);
                                }
                            }
            ...
            if ($thisfile_ac3_raw_bsi["flags"]["infomdat"]) {
                ...
                if ($thisfile_ac3_raw_bsi["acmod"] >= 6) {
                    ...
                    $thisfile_ac3_raw_bsi["dsurexmod"] = $this->readHeaderBSI(
                        2
                    );
                }
                $thisfile_ac3_raw_bsi["flags"][
            ...
            if (
                $thisfile_ac3_raw_bsi["strmtyp"] == 0 &&
            ...
            if ($thisfile_ac3_raw_bsi["strmtyp"] == 2) {
                ...
                if ($thisfile_ac3_raw_bsi["flags"]["blkid"]) {
                    $thisfile_ac3_raw_bsi["frmsizecod"] = $this->readHeaderBSI(
                        6
                    );
                }
            }
            ...
            if ($thisfile_ac3_raw_bsi["flags"]["addbsi"]) {
                ...
                $thisfile_ac3_raw_bsi["addbsi"] = $this->readHeaderBSI(
                    ($thisfile_ac3_raw_bsi["addbsil"] + 1) * 8
                );
            }
        } else {
            $this->error(
                "Bit stream identification is version " .
                    $thisfile_ac3_raw_bsi["bsid"] .
        ...
        if (isset($thisfile_ac3_raw_bsi["fscod2"])) {
            $thisfile_ac3["sample_rate"] = self::sampleRateCodeLookup2(
                $thisfile_ac3_raw_bsi["fscod2"]
            );
        } else {
            $thisfile_ac3["sample_rate"] = self::sampleRateCodeLookup(
        ...
        if ($thisfile_ac3_raw_bsi["fscod"] <= 3) {
            ...
            $this->warning(
                "Unexpected ac3.bsi.fscod value: " .
                    $thisfile_ac3_raw_bsi["fscod"]
            );
        }
        if (isset($thisfile_ac3_raw_bsi["frmsizecod"])) {
            ...
            $thisfile_ac3["bitrate"] = self::bitrateLookup(
                $thisfile_ac3_raw_bsi["frmsizecod"]
            );
        } elseif (!empty($thisfile_ac3_raw_bsi["frmsiz"])) {
            // this isn't right, but it's (usually) close, roughly 5% less than it should be.
            // but WHERE is the actual bitrate value stored in EAC3?? email info@getid3.org if you know!
            $thisfile_ac3["bitrate"] =
                ($thisfile_ac3_raw_bsi["frmsiz"] + 1) * 16 * 30; // The frmsiz field shall contain a value one less than the overall size of the coded syncframe in 16-bit words. That is, this field may assume a value ranging from 0 to 2047, and these values correspond to syncframe sizes ranging from 1 to 2048.
            // kludge-fix to make it approximately the expected value, still not "right":
            $thisfile_ac3["bitrate"] =
                round(($thisfile_ac3["bitrate"] * 1.05) / 16000) * 16000;
        }
        $info["audio"]["bitrate"] = $thisfile_ac3["bitrate"];

        if (
            isset($thisfile_ac3_raw_bsi["bsmod"]) &&
            isset($thisfile_ac3_raw_bsi["acmod"])
        ) {
            $thisfile_ac3["service_type"] = self::serviceTypeLookup(
                $thisfile_ac3_raw_bsi["bsmod"],
                $thisfile_ac3_raw_bsi["acmod"]
            );
        }
        $ac3_coding_mode = self::audioCodingModeLookup(
            $thisfile_ac3_raw_bsi["acmod"]
        );
        ...
        $thisfile_ac3["channels_enabled"] = self::channelsEnabledLookup(
            $thisfile_ac3_raw_bsi["acmod"],
            $thisfile_ac3_raw_bsi["flags"]["lfeon"]
        );
        $thisfile_ac3["dialogue_normalization"] =
            "-" . $thisfile_ac3_raw_bsi["dialnorm"] . "dB";
    ...
    public static function sampleRateCodeLookup($fscod)
    {
        ...
        return isset($sampleRateCodeLookup[$fscod])
    ...
    public static function sampleRateCodeLookup2($fscod2)
    {
        ...
        return isset($sampleRateCodeLookup2[$fscod2])
            ? $sampleRateCodeLookup2[$fscod2]
            : false;
    }
    ...
    public static function serviceTypeLookup($bsmod, $acmod)
    {
        ...
        return isset($serviceTypeLookup[$bsmod][$acmod])
    ...
    public static function audioCodingModeLookup($acmod)
    {
        ...
        static $audioCodingModeLookup = [
            ...
            5 => [
                "channel_config" => "3/1",
                "num_channels" => 4,
                "channel_order" => "L,C,R,S",
            ],
            6 => [
                "channel_config" => "2/2",
                "num_channels" => 4,
                "channel_order" => "L,R,SL,SR",
            ],
            7 => [
                "channel_config" => "3/2",
                "num_channels" => 5,
                "channel_order" => "L,C,R,SL,SR",
            ],
        ];
        return isset($audioCodingModeLookup[$acmod])
    ...
    public static function centerMixLevelLookup($cmixlev)
    {
        ...
        return isset($centerMixLevelLookup[$cmixlev])
    ...
    public static function surroundMixLevelLookup($surmixlev)
    {
        ...
        return isset($surroundMixLevelLookup[$surmixlev])
    ...
    public static function dolbySurroundModeLookup($dsurmod)
    {
        ...
        return isset($dolbySurroundModeLookup[$dsurmod])
    ...
    public static function roomTypeLookup($roomtyp)
    {
        ...
        return isset($roomTypeLookup[$roomtyp])
    ...
    public static function frameSizeLookup($frmsizecod, $fscod)
    {
        ...
        return isset($frameSizeLookup[$framesizeid][$fscod])
            ? $frameSizeLookup[$framesizeid][$fscod] + $paddingBytes
            : false;
    }
    ...
    public static function bitrateLookup($frmsizecod)
    {
        ...
        return isset($bitrateLookup[$framesizeid])
            ? $bitrateLookup[$framesizeid]
            : false;
    }
    ...
    public static function blocksPerSyncFrame($numblkscod)
    {
        ...
        return isset($blocksPerSyncFrameLookup[$numblkscod])
            ? $blocksPerSyncFrameLookup[$numblkscod]
            : false;
    }
}
