...
class getid3_flv extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        $info["flv"]["header"]["version"] = getid3_lib::BigEndian2Int(
            substr($FLVheader, 3, 1)
        );
        ...
        if ($info["flv"]["header"]["signature"] != self::magic) {
            $this->error(
                'Expecting "' .
                    getid3_lib::PrintHexBytes(self::magic) .
                    '" at offset ' .
                    $info["avdataoffset"] .
                    ', found "' .
        ...
        while (
            $this->ftell() + 16 < $info["avdataend"] &&
            ($tagParseCount++ <= $this->max_frames ||
                !$found_valid_meta_playtime)
        ) {
            $ThisTagHeader = $this->fread(16);

            $PreviousTagLength = getid3_lib::BigEndian2Int(
                substr($ThisTagHeader, 0, 4)
            );
            $TagType = getid3_lib::BigEndian2Int(substr($ThisTagHeader, 4, 1));
            $DataLength = getid3_lib::BigEndian2Int(
                substr($ThisTagHeader, 5, 3)
            );
            $Timestamp = getid3_lib::BigEndian2Int(
                substr($ThisTagHeader, 8, 3)
            );
            $LastHeaderByte = getid3_lib::BigEndian2Int(
                substr($ThisTagHeader, 15, 1)
            );
            ...
            switch ($TagType) {
                case GETID3_FLV_TAG_AUDIO:
                    ...
                    if (!$found_audio) {
                        $found_audio = true;
                        $info["flv"]["audio"]["audioFormat"] =
                            ($LastHeaderByte >> 4) & 0x0f;
                        $info["flv"]["audio"]["audioRate"] =
                            ($LastHeaderByte >> 2) & 0x03;
                        $info["flv"]["audio"]["audioSampleSize"] =
                            ($LastHeaderByte >> 1) & 0x01;
                        $info["flv"]["audio"]["audioType"] =
                            $LastHeaderByte & 0x01;
                    }
                    break;
                ...
                case GETID3_FLV_TAG_VIDEO:
                    ...
                    if (!$found_video) {
                        ...
                        if (
                            ...
                            $AVCPacketType = getid3_lib::BigEndian2Int(
                                substr($FLVvideoHeader, 0, 1)
                            );
                            if ($AVCPacketType == H264_AVC_SEQUENCE_HEADER) {
                                //	read AVCDecoderConfigurationRecord
                                $configurationVersion = getid3_lib::BigEndian2Int(
                                    substr($FLVvideoHeader, 4, 1)
                                );
                                $AVCProfileIndication = getid3_lib::BigEndian2Int(
                                    substr($FLVvideoHeader, 5, 1)
                                );
                                $profile_compatibility = getid3_lib::BigEndian2Int(
                                    substr($FLVvideoHeader, 6, 1)
                                );
                                $lengthSizeMinusOne = getid3_lib::BigEndian2Int(
                                    substr($FLVvideoHeader, 7, 1)
                                );
                                $numOfSequenceParameterSets = getid3_lib::BigEndian2Int(
                                    substr($FLVvideoHeader, 8, 1)
                                );
                                ...
                                if (($numOfSequenceParameterSets & 0x1f) != 0) {
                                    ...
                                    $spsSize = getid3_lib::LittleEndian2Int(
                                        substr($FLVvideoHeader, 9, 2)
                                    );
                                    //	read the first SequenceParameterSet
                                    $sps = $this->fread($spsSize);
                                    if (strlen($sps) == $spsSize) {
                                        //	make sure that whole SequenceParameterSet was red
                                        $spsReader = new AVCSequenceParameterSetReader(
                                            $sps
                                        );
                                        $spsReader->readData();
                                        $info["video"][
                            ...
                            switch ($PictureSizeType) {
                                case 0:
                                    ...
                                    $PictureSizeEnc["x"] =
                                        getid3_lib::BigEndian2Int(
                                            substr($FLVvideoHeader, 4, 2)
                                        ) >> 7;
                                    $PictureSizeEnc["y"] =
                                        getid3_lib::BigEndian2Int(
                                            substr($FLVvideoHeader, 5, 2)
                                        ) >> 7;
                                    $info["video"]["resolution_x"] =
                                        $PictureSizeEnc["x"] & 0xff;
                                    $info["video"]["resolution_y"] =
                                        $PictureSizeEnc["y"] & 0xff;
                                    break;

                                case 1:
                                    $PictureSizeEnc["x"] =
                                        getid3_lib::BigEndian2Int(
                                            substr($FLVvideoHeader, 4, 3)
                                        ) >> 7;
                                    $PictureSizeEnc["y"] =
                                        getid3_lib::BigEndian2Int(
                                            substr($FLVvideoHeader, 6, 3)
                                        ) >> 7;
                                    $info["video"]["resolution_x"] =
                                        $PictureSizeEnc["x"] & 0xffff;
                                    $info["video"]["resolution_y"] =
                                        $PictureSizeEnc["y"] & 0xffff;
                                    break;
                            ...
                            if (!isset($info["video"]["resolution_x"])) {
                                ...
                                $PictureSizeEnc[
                                    ...
                                    substr($FLVvideoHeader, 6, 2)
                                );
                                $PictureSizeEnc[
                                    "y"
                                ] = getid3_lib::BigEndian2Int(
                                    substr($FLVvideoHeader, 7, 2)
                                );
                                $info["video"]["resolution_x"] =
                                    ($PictureSizeEnc["x"] & 0xff) << 3;
                                $info["video"]["resolution_y"] =
                                    ($PictureSizeEnc["y"] & 0xff) << 3;
                            }
        ...
        if ($info["flv"]["header"]["hasAudio"]) {
            $info["audio"]["codec"] = self::audioFormatLookup(
                $info["flv"]["audio"]["audioFormat"]
            );
            $info["audio"]["sample_rate"] = self::audioRateLookup(
                $info["flv"]["audio"]["audioRate"]
            );
            $info["audio"]["bits_per_sample"] = self::audioBitDepthLookup(
                $info["flv"]["audio"]["audioSampleSize"]
            );

            $info["audio"]["channels"] = $info["flv"]["audio"]["audioType"] + 1; // 0=mono,1=stereo
            $info["audio"]["lossless"] = $info["flv"]["audio"]["audioFormat"]
                ? false
                : true; // 0=uncompressed
            $info["audio"]["dataformat"] = "flv";
        }
        if (!empty($info["flv"]["header"]["hasVideo"])) {
            $info["video"]["codec"] = self::videoCodecLookup(
                $info["flv"]["video"]["videoCodec"]
            );
