...
getid3_lib::IncludeDependency(
    GETID3_INCLUDEPATH . "module.tag.id3v2.php",
    __FILE__,
    true
...
class getid3_quicktime extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        $offset = 0;
        $atomcounter = 0;
        $atom_data_read_buffer_size = $info["php_memory_limit"]
            ? round($info["php_memory_limit"] / 4)
        ...
        while ($offset < $info["avdataend"]) {
            if (!getid3_lib::intValueSupported($offset)) {
                $this->error(
                    "Unable to parse atom at offset " .
                        $offset .
                        " because beyond " .
            ...
            if ($atomsize == 1) {
                ...
                $atomsize = getid3_lib::BigEndian2Int(
                    $this->fread($atom_size_extended_bytes)
                );
            }
            ...
            if ($offset + $atomsize > $info["avdataend"]) {
                ...
                $this->error(
                    "Atom at offset " .
                        $offset .
                        " claims to go beyond end-of-file (length: " .
                        $atomsize .
                        " bytes)"
                );
                return false;
            ...
            $parsedAtomData = $this->QuicktimeParseAtom(
                ...
                $this->fread(
                    min(
                        $atomsize - $atom_size_extended_bytes,
        ...
        if (
            isset($info["quicktime"]["comments"]["chapters"]) &&
            is_array($info["quicktime"]["comments"]["chapters"]) &&
            count($info["quicktime"]["comments"]["chapters"]) > 0
        ) {
            $durations = $this->quicktime_time_to_sample_table($info);
            for (
                $i = 0;
                $i < count($info["quicktime"]["comments"]["chapters"]);
                $i++
            ) {
                $bookmark = [];
                $bookmark["title"] =
                    $info["quicktime"]["comments"]["chapters"][$i];
                if (isset($durations[$i])) {
                    $bookmark["duration_sample"] =
                        $durations[$i]["sample_duration"];
                    if ($i > 0) {
                        $bookmark["start_sample"] =
                            $info["quicktime"]["bookmarks"][$i - 1][
                                "start_sample"
                            ] +
                            $info["quicktime"]["bookmarks"][$i - 1][
                                "duration_sample"
                    ...
                    if (
                        $time_scale = $this->quicktime_bookmark_time_scale(
                            $info
                        )
                    ) {
                        $bookmark["duration_seconds"] =
                            $bookmark["duration_sample"] / $time_scale;
                        $bookmark["start_seconds"] =
                            $bookmark["start_sample"] / $time_scale;
                    }
                }
        ...
        if (!empty($info["quicktime"]["comments"]["location.ISO6709"])) {
            ...
            foreach (
                $info["quicktime"]["comments"]["location.ISO6709"]
                as $ISO6709string
            ) {
                $ISO6709parsed = [
                    "latitude" => false,
                ...
                if (
                    ...
                    if (strlen($lat_deg) == 2) {
                        // [+-]DD.D
                        $ISO6709parsed["latitude"] =
                            ($lat_sign == "-" ? -1 : 1) *
                            floatval(ltrim($lat_deg, "0") . $lat_deg_dec);
                    } elseif (strlen($lat_deg) == 4) {
                        // [+-]DDMM.M
                        $ISO6709parsed["latitude"] =
                            ($lat_sign == "-" ? -1 : 1) *
                                floatval(ltrim(substr($lat_deg, 0, 2), "0")) +
                            floatval(
                                ltrim(substr($lat_deg, 2, 2), "0") .
                                    $lat_deg_dec / 60
                            );
                        ...
                        $ISO6709parsed["latitude"] =
                            ($lat_sign == "-" ? -1 : 1) *
                                floatval(ltrim(substr($lat_deg, 0, 2), "0")) +
                            floatval(
                                (int) ltrim(substr($lat_deg, 2, 2), "0") / 60
                            ) +
                            floatval(
                                ltrim(substr($lat_deg, 4, 2), "0") .
                                    $lat_deg_dec / 3600
                            );
                    }
                    ...
                    if (strlen($lon_deg) == 3) {
                        // [+-]DDD.D
                        $ISO6709parsed["longitude"] =
                            ($lon_sign == "-" ? -1 : 1) *
                            floatval(ltrim($lon_deg, "0") . $lon_deg_dec);
                    } elseif (strlen($lon_deg) == 5) {
                        // [+-]DDDMM.M
                        $ISO6709parsed["longitude"] =
                            ($lon_sign == "-" ? -1 : 1) *
                                floatval(ltrim(substr($lon_deg, 0, 2), "0")) +
                            floatval(
                                ltrim(substr($lon_deg, 2, 2), "0") .
                                    $lon_deg_dec / 60
                            );
                        ...
                        $ISO6709parsed["longitude"] =
                            ($lon_sign == "-" ? -1 : 1) *
                                floatval(ltrim(substr($lon_deg, 0, 2), "0")) +
                            floatval(
                                (int) ltrim(substr($lon_deg, 2, 2), "0") / 60
                            ) +
                            floatval(
                                ltrim(substr($lon_deg, 4, 2), "0") .
                                    $lon_deg_dec / 3600
                            );
                    }
                    ...
                    if (strlen($alt_deg) == 3) {
                        ...
                        $ISO6709parsed["altitude"] =
                            ($alt_sign == "-" ? -1 : 1) *
                            floatval(ltrim($alt_deg, "0") . $alt_deg_dec);
                    } elseif (strlen($alt_deg) == 5) {
                        // [+-]DDDMM.M
                        $ISO6709parsed["altitude"] =
                            ($alt_sign == "-" ? -1 : 1) *
                                floatval(ltrim(substr($alt_deg, 0, 2), "0")) +
                            floatval(
                                ltrim(substr($alt_deg, 2, 2), "0") .
                                    $alt_deg_dec / 60
                            );
                        ...
                        $ISO6709parsed["altitude"] =
                            ($alt_sign == "-" ? -1 : 1) *
                                floatval(ltrim(substr($alt_deg, 0, 2), "0")) +
                            floatval(
                                (int) ltrim(substr($alt_deg, 2, 2), "0") / 60
                            ) +
                            floatval(
                                ltrim(substr($alt_deg, 4, 2), "0") .
                                    $alt_deg_dec / 3600
                            );
                    }
                    ...
                    foreach (["latitude", "longitude", "altitude"] as $key) {
                        if ($ISO6709parsed[$key] !== false) {
                            $value =
                                ($lat_sign == "-" ? -1 : 1) *
                                floatval($ISO6709parsed[$key]);
                            if (
                                !isset(
                                    $info["quicktime"]["comments"][
                                        "gps_" . $key
                                    ]
                                ) ||
                                !in_array(
                ...
                if ($ISO6709parsed["latitude"] === false) {
                    $this->warning(
                        'location.ISO6709 string not parsed correctly: "' .
                            $ISO6709string .
        ...
        if (
            ...
            empty($info["video"]["bitrate"]) &&
            !empty($info["video"]["frame_rate"]) &&
            !empty($info["video"]["resolution_x"]) &&
        ...
        if (
            !empty($info["playtime_seconds"]) &&
            !isset($info["video"]["frame_rate"]) &&
            !empty($info["quicktime"]["stts_framecount"])
        ) {
            foreach (
                $info["quicktime"]["stts_framecount"]
                as $key => $samples_count
            ) {
                $samples_per_second =
                    $samples_count / $info["playtime_seconds"];
        ...
        if (
            empty($info["audio"]["dataformat"]) &&
            !empty($info["quicktime"]["audio"])
        ) {
            $info["audio"]["dataformat"] = "quicktime";
        }
        if (
            empty($info["video"]["dataformat"]) &&
            !empty($info["quicktime"]["video"])
        ) {
    ...
    public function QuicktimeParseAtom(
        ...
        if (substr($atomname, 0, 3) == "\x00\x00\x00") {
            ...
            $atom_structure["name"] = $atomname;
            $atom_structure["subatoms"] = $this->QuicktimeParseContainerAtom(
                $atom_data,
                $baseoffset + 8,
                $atomHierarchy,
                $ParseAllPossibleAtoms
            ...
            switch ($atomname) {
                ...
                case "gmhd": // Generic Media info HeaDer atom (seen on QTVR)
                    $atom_structure[
                        "subatoms"
                    ] = $this->QuicktimeParseContainerAtom(
                        $atom_data,
                        $baseoffset + 8,
                        $atomHierarchy,
                        $ParseAllPossibleAtoms
                ...
                case "ilst": // Item LiST container atom
                    if (
                        ...
                        foreach ($atom_structure["subatoms"] as $subatomarray) {
                            if (
                                !is_integer($subatomarray["name"]) ||
                                count($subatomarray["subatoms"]) != 1
                            ) {
                        ...
                        if ($allnumericnames) {
                            ...
                            foreach (
                                ...
                                foreach (
                                    $subatomarray["subatoms"]
                                    as $newData_subatomarray
                                ) {
                                    unset(
                                        $newData_subatomarray["hierarchy"],
                                        $newData_subatomarray["name"]
                                    );
                                    $newData[
                                        $subatomarray["name"]
                ...
                case "stbl": // Sample TaBLe container atom
                    ...
                    foreach (
                        ...
                        if (isset($value_array["sample_description_table"])) {
                            foreach (
                                $value_array["sample_description_table"]
                                as $key2 => $value_array2
                            ) {
                            ...
                            foreach (
                                $value_array["time_to_sample_table"]
                                as $key2 => $value_array2
                            ) {
                                if (
                                    isset($value_array2["sample_count"]) &&
                                    isset($value_array2["sample_duration"]) &&
                                    $value_array2["sample_duration"] > 0 &&
                                    !empty($info["quicktime"]["time_scale"])
                                ) {
                                    $framerate = round(
                                        $info["quicktime"]["time_scale"] /
                                            $value_array2["sample_duration"],
                                        3
                                    );
                    ...
                    if ($isVideo && $framerate) {
                        ...
                        $info["video"]["frame_rate"] =
                            $info["quicktime"]["video"]["frame_rate"];
                    }
                    if ($isVideo && $framecount) {
                        $info["quicktime"]["video"][
                            "frame_count"
                        ] = $framecount;
                    }
                    break;
                ...
                case "tvsn": // TV SeasoN
                    if ($atom_parent == "udta") {
                        ...
                        $atom_structure[
                            "data_length"
                        ] = getid3_lib::BigEndian2Int(substr($atom_data, 0, 2));
                        $atom_structure[
                            "language_id"
                        ] = getid3_lib::BigEndian2Int(substr($atom_data, 2, 2));
                        $atom_structure["data"] = substr($atom_data, 4);

                        $atom_structure[
                            "language"
                        ] = $this->QuicktimeLanguageLookup(
                            $atom_structure["language_id"]
                        );
                        if (
                            empty($info["comments"]["language"]) ||
                            !in_array(
                        ...
                        if (substr($atom_data, 2, 2) == "\x10\xB5") {
                            ...
                            while ($atomoffset < strlen($atom_data)) {
                                $boxsmallsize = getid3_lib::BigEndian2Int(
                                    substr($atom_data, $atomoffset, 2)
                                );
                                $boxsmalltype = substr(
                                    $atom_data,
                                    $atomoffset + 2,
                                ...
                                if ($boxsmallsize <= 1) {
                                    $this->warning(
                                        'Invalid QuickTime atom smallbox size "' .
                                            $boxsmallsize .
                                            '" in atom "' .
                                            preg_replace(
                                                "#[^a-zA-Z0-9 _\\-]#",
                                ...
                                switch ($boxsmalltype) {
                                    ...
                                    default:
                                        $this->warning(
                                            'Unknown QuickTime smallbox type: "' .
                                                preg_replace(
                                                    "#[^a-zA-Z0-9 _\\-]#",
                                                    "?",
                                                    $boxsmalltype
                            ...
                            while ($atomoffset < strlen($atom_data)) {
                                $boxsize = getid3_lib::BigEndian2Int(
                                    substr($atom_data, $atomoffset, 4)
                                );
                                $boxtype = substr(
                                    $atom_data,
                                    $atomoffset + 4,
                                ...
                                if ($boxsize <= 1) {
                                    $this->warning(
                                        'Invalid QuickTime atom box size "' .
                                            $boxsize .
                                            '" in atom "' .
                                            preg_replace(
                                                "#[^a-zA-Z0-9 _\\-]#",
                                            ...
                                            case 21: // tmpo/cpil flag
                                                switch ($atomname) {
                                                    ...
                                                    case "trkn":
                                                        ...
                                                        $num_total = getid3_lib::BigEndian2Int(
                                                            substr(
                                                                $boxdata,
                                                                12,
                                                                2
                                                    ...
                                                    case "gnre":
                                                        // enum
                                                        $GenreID = getid3_lib::BigEndian2Int(
                                                            substr(
                                                                $boxdata,
                                                                8,
                                                                4
                                                    ...
                                                    case "rtng":
                                                        ...
                                                        $atom_structure[
                                                            "data"
                                                        ] = $this->QuicktimeContentRatingLookup(
                                                            $atom_structure[
                                                                $atomname
                                                            ]
                                                        );
                                                    ...
                                                    case "sfID":
                                                        ...
                                                        $atom_structure[
                                                            "data"
                                                        ] = $this->QuicktimeStoreFrontCodeLookup(
                                                            $atom_structure[
                                                                $atomname
                                                            ]
                                                        );
                                            ...
                                            default:
                                                ...
                                                if ($atomname == "covr") {
                                                    if (
                                                        ...
                                                        $atom_structure[
                                                            "image_mime"
                                                        ] = "image/unknown"; // provide default MIME type to ensure array keys exist
                                                        if (
                                                            function_exists(
                                                        ...
                                                        $this->warning(
                                                            'Unknown empty "covr" image at offset ' .
                                                                $baseoffset
                                                        );
                                                    }
                                                }
                    ...
                    $this->CopyToAppropriateCommentsSection(
                        $atomname,
                        $atom_structure["data"],
                        $atom_structure["name"]
                    );
                ...
                case "play": // auto-PLAY atom
                    $atom_structure[
                        "autoplay"
                    ] = (bool) getid3_lib::BigEndian2Int(
                        substr($atom_data, 0, 1)
                    );

                    $info["quicktime"]["autoplay"] =
                        $atom_structure["autoplay"];
                    break;

                case "WLOC": // Window LOCation atom
                    $atom_structure["location_x"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 0, 2)
                    );
                    $atom_structure["location_y"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 2, 2)
                    );
                    break;
                ...
                case "AllF": // play ALL Frames atom
                    $atom_structure["data"] = getid3_lib::BigEndian2Int(
                        $atom_data
                    );
                    break;
                ...
                case "cmvd": // Compressed MooV Data atom
                    ...
                    if (
                        ...
                        $this->warning(
                            "Error decompressing compressed MOV atom at offset " .
                                $atom_structure["offset"]
                        );
                    }
                    break;
                ...
                case "dcom": // Data COMpression atom
                    ...
                    $atom_structure[
                        "compression_text"
                    ] = $this->QuicktimeDCOMLookup($atom_data);
                    break;

                case "rdrf": // Reference movie Data ReFerence atom
                    $atom_structure["version"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 0, 1)
                    );
                    $atom_structure["flags_raw"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 1, 3)
                    );
                    $atom_structure["flags"]["internal_data"] =
                        (bool) ($atom_structure["flags_raw"] & 0x000001);

                    $atom_structure["reference_type_name"] = substr(
                        $atom_data,
                        4,
                        4
                    );
                    ...
                    switch ($atom_structure["reference_type_name"]) {
                        case "url ":
                            $atom_structure["url"] = $this->NoNullString(
                                substr($atom_data, 12)
                            );
                            break;

                        case "alis":
                            $atom_structure["file_alias"] = substr(
                                $atom_data,
                                12
                            );
                            break;
                        ...
                        case "rsrc":
                            $atom_structure["resource_alias"] = substr(
                                $atom_data,
                                12
                            );
                            break;
                ...
                case "rmqu": // Reference Movie QUality atom
                    $atom_structure[
                        "movie_quality"
                    ] = getid3_lib::BigEndian2Int($atom_data);
                    break;
                ...
                case "rmvc": // Reference Movie Version Check atom
                    ...
                    $atom_structure["gestalt_selector"] = substr(
                        $atom_data,
                        4,
                        4
                    );
                    ...
                    $atom_structure[
                        "gestalt_value"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 12, 4));
                    $atom_structure[
                        "gestalt_check_type"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 14, 2));
                    break;
                ...
                case "rmcd": // Reference Movie Component check atom
                    ...
                    $atom_structure["component_type"] = substr(
                        $atom_data,
                        4,
                        4
                    );
                    $atom_structure["component_subtype"] = substr(
                        $atom_data,
                        8,
                        4
                    );
                    $atom_structure["component_manufacturer"] = substr(
                        $atom_data,
                        12,
                        4
                    );
                    $atom_structure[
                        "component_flags_raw"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 16, 4));
                    $atom_structure[
                        "component_flags_mask"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 20, 4));
                    $atom_structure[
                        "component_min_version"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 24, 4));
                    break;
                ...
                case "rmdr": // Reference Movie Data Rate atom
                    ...
                    $atom_structure["data_rate"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 4, 4)
                    );

                    $atom_structure["data_rate_bps"] =
                        $atom_structure["data_rate"] * 10;
                    break;
                ...
                case "rmla": // Reference Movie Language Atom
                    ...
                    $atom_structure["language_id"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 4, 2)
                    );
                ...
                case "ptv ": // Print To Video - defines a movie's full screen mode
                    ...
                    $atom_structure["reserved_1"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 2, 2)
                    ); // hardcoded: 0x0000
                    $atom_structure["reserved_2"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 4, 2)
                    ); // hardcoded: 0x0000
                    $atom_structure[
                        "slide_show_flag"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 6, 1));
                    $atom_structure[
                        "play_on_open_flag"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 7, 1));

                    $atom_structure["flags"]["play_on_open"] =
                        (bool) $atom_structure["play_on_open_flag"];
                    $atom_structure["flags"]["slide_show"] =
                        (bool) $atom_structure["slide_show_flag"];
                    ...
                    if (
                        isset($ptv_lookup[$atom_structure["display_size_raw"]])
                    ) {
                        $atom_structure["display_size"] =
                            $ptv_lookup[$atom_structure["display_size_raw"]];
                    } else {
                        $this->warning(
                            'unknown "ptv " display constant (' .
                                $atom_structure["display_size_raw"] .
                                ")"
                        );
                    }
                    break;
                ...
                case "stsd": // Sample Table Sample Description atom
                    ...
                    $atom_structure[
                        "number_entries"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 4, 4));
                    ...
                    if ($atom_structure["number_entries"] > 0x000fffff) {
                        if ($atom_structure["number_entries"] > 0x00ffffff) {
                            $this->warning(
                                '"stsd" atom contains improbably large number_entries (0x' .
                                    getid3_lib::PrintHexBytes(
                                        substr($atom_data, 4, 4),
                                        true,
                                        false
                                    ...
                                    " = " .
                                    $atom_structure["number_entries"] .
                                    "), probably in error. Ignoring upper byte and interpreting this as 0x" .
                                    getid3_lib::PrintHexBytes(
                                        substr($atom_data, 5, 3),
                                        true,
                                        false
                                    ) .
                                    " = " .
                            ...
                            $atom_structure["number_entries"] =
                                $atom_structure["number_entries"] & 0x00ffffff;
                        } else {
                            $this->warning(
                                '"stsd" atom contains improbably large number_entries (0x' .
                                    getid3_lib::PrintHexBytes(
                                    ...
                                    " = " .
                                    $atom_structure["number_entries"] .
                                    "), probably in error. Please report this to info@getid3.org referencing bug report #111"
                            );
                        }
                    }
                    ...
                    for ($i = 0; $i < $atom_structure["number_entries"]; $i++) {
                        $atom_structure["sample_description_table"][$i][
                            ...
                            substr($atom_data, $stsdEntriesDataOffset, 4)
                        );
                        ...
                        $atom_structure["sample_description_table"][$i][
                            ...
                            substr($atom_data, $stsdEntriesDataOffset, 6)
                        );
                        $stsdEntriesDataOffset += 6;
                        $atom_structure["sample_description_table"][$i][
                            "reference_index"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, $stsdEntriesDataOffset, 2)
                        );
                        ...
                        if (
                            ...
                            $this->warning(
                                'incomplete/incorrect handling of "stsd" with Parrot metadata in this version of getID3() [' .
                                    $this->getid3->version() .
                                    "]"
                            );
                            continue;
                            ...
                            case "\x00\x00\x00\x00":
                                ...
                                switch (
                                    ...
                                    case "yuv2":
                                        ...
                                        if (
                                            $this->QuicktimeVideoCodecLookup(
                                                $info["video"]["fourcc"]
                                            )
                                        ) {
                                            $info["video"][
                                    ...
                                    case "mp4a":
                                        ...
                                        $info["quicktime"]["audio"][
                                            "codec"
                                        ] = $this->QuicktimeAudioCodecLookup(
                                            $atom_structure[
                                                "sample_description_table"
                                            ][$i]["data_format"]
                                        );
                            ...
                            default:
                                switch (
                                    ...
                                    default:
                                        ...
                                        $atom_structure[
                                            "sample_description_table"
                                        ][$i]["video_pixel_color_type"] =
                                            (int) $atom_structure[
                                                "sample_description_table"
                                            ][$i]["video_pixel_color_depth"] >
                                            32
                                                ? "grayscale"
                                                : "color";
                                        $atom_structure[
                ...
                case "stts": // Sample Table Time-to-Sample atom
                    ...
                    $max_stts_entries_to_scan = $info["php_memory_limit"]
                        ? min(
                            floor($this->getid3->memory_limit / 10000),
                            $atom_structure["number_entries"]
                        )
                        : $atom_structure["number_entries"];
                    if (
                        ...
                        $this->warning(
                            'QuickTime atom "stts" has ' .
                                $atom_structure["number_entries"] .
                                " but only scanning the first " .
                                $max_stts_entries_to_scan .
                                " entries due to limited PHP memory available (" .
                                floor($this->getid3->memory_limit / 1048576) .
                                "MB)."
                        );
                    }
                    for ($i = 0; $i < $max_stts_entries_to_scan; $i++) {
                        $atom_structure["time_to_sample_table"][$i][
                            "sample_count"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, $sttsEntriesDataOffset, 4)
                        );
                ...
                case "stss": // Sample Table Sync Sample (key frames) atom
                    if ($ParseAllPossibleAtoms) {
                        ...
                        for (
                            $i = 0;
                            $i < $atom_structure["number_entries"];
                            $i++
                        ) {
                            $atom_structure["time_to_sample_table"][
                                $i
                            ] = getid3_lib::BigEndian2Int(
                                substr($atom_data, $stssEntriesDataOffset, 4)
                            );
                ...
                case "stsc": // Sample Table Sample-to-Chunk atom
                    if ($ParseAllPossibleAtoms) {
                        ...
                        for (
                            ...
                            $atom_structure["sample_to_chunk_table"][$i][
                                "first_chunk"
                            ] = getid3_lib::BigEndian2Int(
                                substr($atom_data, $stscEntriesDataOffset, 4)
                            );
                    ...
                    if ($ParseAllPossibleAtoms) {
                        ...
                        if ($atom_structure["sample_size"] == 0) {
                            for (
                                ...
                                $atom_structure["sample_size_table"][
                                    $i
                                ] = getid3_lib::BigEndian2Int(
                                    substr(
                                        $atom_data,
                ...
                case "stco": // Sample Table Chunk Offset atom
                    ...
                    if ($ParseAllPossibleAtoms) {
                        ...
                        for (
                            ...
                            $atom_structure["chunk_offset_table"][
                                $i
                            ] = getid3_lib::BigEndian2Int(
                                substr($atom_data, $stcoEntriesDataOffset, 4)
                            );
                    ...
                    if ($ParseAllPossibleAtoms) {
                        ...
                        for (
                            ...
                            $atom_structure["chunk_offset_table"][
                                ...
                                substr($atom_data, $stcoEntriesDataOffset, 8)
                            );
                            $stcoEntriesDataOffset += 8;
                        }
                    }
                ...
                case "dref": // Data REFerence atom
                    ...
                    for ($i = 0; $i < $atom_structure["number_entries"]; $i++) {
                        $atom_structure["data_references"][$i][
                            ...
                            substr($atom_data, $drefDataOffset, 4)
                        );
                        $drefDataOffset += 4;
                        $atom_structure["data_references"][$i]["type"] = substr(
                            $atom_data,
                            $drefDataOffset,
                            4
                        );
                        ...
                        $atom_structure["data_references"][$i][
                            ...
                            substr($atom_data, $drefDataOffset, 1)
                        );
                        $drefDataOffset += 1;
                        $atom_structure["data_references"][$i][
                            "flags_raw"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, $drefDataOffset, 3)
                        ); // hardcoded: 0x0000
                        $drefDataOffset += 3;
                        $atom_structure["data_references"][$i]["data"] = substr(
                            $atom_data,
                            $drefDataOffset,
                            $atom_structure["data_references"][$i]["size"] -
                                4 -
                                4 -
                                1 -
                                3
                        ...
                        $atom_structure["data_references"][$i]["flags"][
                            "self_reference"
                        ] =
                            (bool) ($atom_structure["data_references"][$i][
                                "flags_raw"
                            ] & 0x001);
                    }
                    break;
                ...
                case "gmin": // base Media INformation atom
                    ...
                    $atom_structure["opcolor_red"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 6, 2)
                    );
                    $atom_structure[
                        "opcolor_green"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 8, 2));
                    $atom_structure["opcolor_blue"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 10, 2)
                    );
                    $atom_structure["balance"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 12, 2)
                    );
                    $atom_structure["reserved"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 14, 2)
                    );
                    break;
                ...
                case "vmhd": // Video Media information HeaDer atom
                    ...
                    $atom_structure["flags"]["no_lean_ahead"] =
                        (bool) ($atom_structure["flags_raw"] & 0x001);
                    break;
                ...
                case "hdlr": // HanDLeR reference atom
                    ...
                    $atom_structure[
                        "component_name"
                    ] = $this->MaybePascal2String(substr($atom_data, 24));

                    if (
                        $atom_structure["component_subtype"] == "STpn" &&
                        $atom_structure["component_manufacturer"] == "zzzz"
                    ) {
                ...
                case "mdhd": // MeDia HeaDer atom
                    ...
                    $atom_structure["modify_time"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 8, 4)
                    );
                    $atom_structure["time_scale"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 12, 4)
                    );
                    $atom_structure["duration"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 16, 4)
                    );
                    $atom_structure["language_id"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 20, 2)
                    );
                    $atom_structure["quality"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 22, 2)
                    );
                    ...
                    $atom_structure[
                        ...
                        $atom_structure["creation_time"]
                    );
                    $atom_structure[
                        "modify_time_unix"
                    ] = getid3_lib::DateMac2Unix(
                        $atom_structure["modify_time"]
                    );
                    $atom_structure["playtime_seconds"] =
                        $atom_structure["duration"] /
                        $atom_structure["time_scale"];
                    $atom_structure[
                        "language"
                    ] = $this->QuicktimeLanguageLookup(
                    ...
                    $info["quicktime"]["timestamps_unix"]["create"][
                        $atom_structure["hierarchy"]
                    ] = $atom_structure["creation_time_unix"];
                    $info["quicktime"]["timestamps_unix"]["modify"][
                        $atom_structure["hierarchy"]
                    ] = $atom_structure["modify_time_unix"];
                    break;

                case "pnot": // Preview atom
                    $atom_structure[
                        "modification_date"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 0, 4)); // "standard Macintosh format"
                    $atom_structure[
                        "version_number"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 4, 2)); // hardcoded: 0x00
                    $atom_structure["atom_type"] = substr($atom_data, 6, 4); // usually: 'PICT'
                    $atom_structure["atom_index"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 10, 2)
                    ); // usually: 0x01
                    ...
                    $atom_structure[
                        ...
                        $atom_structure["modification_date"]
                    );
                    $info["quicktime"]["timestamps_unix"]["modify"][
                        $atom_structure["hierarchy"]
                    ] = $atom_structure["modification_date_unix"];
                    break;

                case "crgn": // Clipping ReGioN atom
                    $atom_structure["region_size"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 0, 2)
                    ); // The Region size, Region boundary box,
                    $atom_structure["boundary_box"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 2, 8)
                ...
                case "load": // track LOAD settings atom
                    ...
                    $atom_structure["default_hints"]["double_buffer"] =
                        (bool) ($atom_structure["default_hints_raw"] & 0x0020);
                    $atom_structure["default_hints"]["high_quality"] =
                        (bool) ($atom_structure["default_hints_raw"] & 0x0100);
                    break;
                ...
                case "elst": // Edit LiST atom
                    ...
                    for ($i = 0; $i < $atom_structure["number_entries"]; $i++) {
                        $atom_structure["edit_list"][$i][
                            "track_duration"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, 8 + $i * 12 + 0, 4)
                        );
                        $atom_structure["edit_list"][$i][
                            "media_time"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, 8 + $i * 12 + 4, 4)
                        );
                        $atom_structure["edit_list"][$i][
                            "media_rate"
                        ] = getid3_lib::FixedPoint16_16(
                            substr($atom_data, 8 + $i * 12 + 8, 4)
                        );
                    }
                    break;
                ...
                case "ctab": // Color TABle atom
                    $atom_structure[
                        "color_table_seed"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 0, 4)); // hardcoded: 0x00000000
                    $atom_structure[
                        "color_table_flags"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 4, 2)); // hardcoded: 0x8000
                    $atom_structure["color_table_size"] =
                        getid3_lib::BigEndian2Int(substr($atom_data, 6, 2)) + 1;
                    for (
                        $colortableentry = 0;
                        $colortableentry < $atom_structure["color_table_size"];
                        $colortableentry++
                    ) {
                        $atom_structure["color_table"][$colortableentry][
                            "alpha"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, 8 + $colortableentry * 8 + 0, 2)
                        );
                        $atom_structure["color_table"][$colortableentry][
                            "red"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, 8 + $colortableentry * 8 + 2, 2)
                        );
                        $atom_structure["color_table"][$colortableentry][
                            "green"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, 8 + $colortableentry * 8 + 4, 2)
                        );
                        $atom_structure["color_table"][$colortableentry][
                            "blue"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, 8 + $colortableentry * 8 + 6, 2)
                        );
                    }
                    break;
                ...
                case "mvhd": // MoVie HeaDer atom
                    ...
                    $atom_structure[
                        "preferred_rate"
                    ] = getid3_lib::FixedPoint16_16(substr($atom_data, 20, 4));
                    $atom_structure[
                        "preferred_volume"
                    ] = getid3_lib::FixedPoint8_8(substr($atom_data, 24, 2));
                    $atom_structure["reserved"] = substr($atom_data, 26, 10);
                    $atom_structure["matrix_a"] = getid3_lib::FixedPoint16_16(
                        substr($atom_data, 36, 4)
                    );
                    $atom_structure["matrix_b"] = getid3_lib::FixedPoint16_16(
                        substr($atom_data, 40, 4)
                    );
                    $atom_structure["matrix_u"] = getid3_lib::FixedPoint2_30(
                        substr($atom_data, 44, 4)
                    );
                    $atom_structure["matrix_c"] = getid3_lib::FixedPoint16_16(
                        substr($atom_data, 48, 4)
                    );
                    $atom_structure["matrix_d"] = getid3_lib::FixedPoint16_16(
                        substr($atom_data, 52, 4)
                    );
                    $atom_structure["matrix_v"] = getid3_lib::FixedPoint2_30(
                        substr($atom_data, 56, 4)
                    );
                    $atom_structure["matrix_x"] = getid3_lib::FixedPoint16_16(
                        substr($atom_data, 60, 4)
                    );
                    $atom_structure["matrix_y"] = getid3_lib::FixedPoint16_16(
                        substr($atom_data, 64, 4)
                    );
                    $atom_structure["matrix_w"] = getid3_lib::FixedPoint2_30(
                        substr($atom_data, 68, 4)
                    );
                    $atom_structure["preview_time"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 72, 4)
                    );
                    $atom_structure[
                        "preview_duration"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 76, 4));
                    $atom_structure["poster_time"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 80, 4)
                    );
                    $atom_structure[
                        "selection_time"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 84, 4));
                    $atom_structure[
                        "selection_duration"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 88, 4));
                    $atom_structure["current_time"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 92, 4)
                    );
                    $atom_structure[
                        "next_track_id"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 96, 4));
                    ...
                    $info["quicktime"]["time_scale"] =
                        ...
                        $info["quicktime"]["time_scale"] < 1000
                            ...
                            : $atom_structure["time_scale"];
                    $info["quicktime"]["display_scale"] =
                        $atom_structure["matrix_a"];
                    $info["playtime_seconds"] =
                        $atom_structure["duration"] /
                        $atom_structure["time_scale"];
                    break;
                ...
                case "tkhd": // TracK HeaDer atom
                    ...
                    $atom_structure["trackid"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 12, 4)
                    );
                    $atom_structure["reserved1"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 16, 4)
                    );
                    $atom_structure["duration"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 20, 4)
                    );
                    $atom_structure["reserved2"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 24, 8)
                    );
                    $atom_structure["layer"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 32, 2)
                    );
                    $atom_structure[
                        "alternate_group"
                    ] = getid3_lib::BigEndian2Int(substr($atom_data, 34, 2));
                    $atom_structure["volume"] = getid3_lib::FixedPoint8_8(
                        substr($atom_data, 36, 2)
                    );
                    $atom_structure["reserved3"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 38, 2)
                    );
                    ...
                    $atom_structure["width"] = getid3_lib::FixedPoint16_16(
                        substr($atom_data, 76, 4)
                    );
                    $atom_structure["height"] = getid3_lib::FixedPoint16_16(
                        substr($atom_data, 80, 4)
                    );
                    $atom_structure["flags"]["enabled"] =
                        (bool) ($atom_structure["flags_raw"] & 0x0001);
                    $atom_structure["flags"]["in_movie"] =
                        (bool) ($atom_structure["flags_raw"] & 0x0002);
                    $atom_structure["flags"]["in_preview"] =
                        (bool) ($atom_structure["flags_raw"] & 0x0004);
                    $atom_structure["flags"]["in_poster"] =
                        (bool) ($atom_structure["flags_raw"] & 0x0008);
                    $atom_structure[
                        "creation_time_unix"
                    ] = getid3_lib::DateMac2Unix(
                        $atom_structure["creation_time"]
                    ...
                    switch (
                        $atom_structure["matrix_a"] .
                        ":" .
                        $atom_structure["matrix_b"] .
                        ":" .
                        $atom_structure["matrix_c"] .
                        ":" .
                        $atom_structure["matrix_d"]
                    ) {
                        case "1:0:0:1":
                            $matrixRotation = 0;
                            break;
                    ...
                    if (
                        !isset($info["video"]["rotate"]) ||
                        ($info["video"]["rotate"] == 0 && $matrixRotation > 0)
                    ) {
                        $info["quicktime"]["video"]["rotate"] = $info["video"][
                            "rotate"
                        ] = $matrixRotation;
                    }
                    ...
                    if ($atom_structure["flags"]["enabled"] == 1) {
                        if (
                            !isset($info["video"]["resolution_x"]) ||
                            !isset($info["video"]["resolution_y"])
                        ) {
                            $info["video"]["resolution_x"] =
                                $atom_structure["width"];
                            $info["video"]["resolution_y"] =
                                $atom_structure["height"];
                        }
                        $info["video"]["resolution_x"] = max(
                            $info["video"]["resolution_x"],
                            $atom_structure["width"]
                        );
                ...
                case "iods": // Initial Object DeScriptor atom
                    ...
                    $offset = 0;
                    $atom_structure["version"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, $offset, 1)
                    );
                    $offset += 1;
                    $atom_structure["flags_raw"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, $offset, 3)
                    );
                    $offset += 3;
                    $atom_structure["mp4_iod_tag"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, $offset, 1)
                    );
                    $offset += 1;
                    $atom_structure[
                        "length"
                    ] = $this->quicktime_read_mp4_descr_length(
                        $atom_data,
                        $offset
                    );
                    ...
                    $atom_structure["num_iods_tracks"] =
                        ($atom_structure["length"] - 7) / 6; // 6 bytes would only be right if all tracks use 1-byte length fields
                    for (
                        $i = 0;
                        $i < $atom_structure["num_iods_tracks"];
                        $i++
                    ) {
                        $atom_structure["track"][$i][
                            "ES_ID_IncTag"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, $offset, 1)
                        );
                    ...
                    $atom_structure[
                        "audio_profile_name"
                    ] = $this->QuicktimeIODSaudioProfileName(
                        $atom_structure["audio_profile_id"]
                    );
                    $atom_structure[
                        "video_profile_name"
                    ] = $this->QuicktimeIODSvideoProfileName(
                        $atom_structure["video_profile_id"]
                    );
                    break;

                case "ftyp": // FileTYPe (?) atom (for MP4 it seems)
                    $atom_structure["signature"] = substr($atom_data, 0, 4);
                    $atom_structure["unknown_1"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 4, 4)
                    );
                ...
                case "mdat": // Media DATa atom
                    ...
                    while (true) {
                        if (
                            substr($atom_data, $mdat_offset, 8) ==
                            "\x00\x00\x00\x08" . "wide"
                        ) {
                            $mdat_offset += 8;
                        } elseif (
                            substr($atom_data, $mdat_offset, 8) ==
                            "\x00\x00\x00\x00" . "mdat"
                        ) {
                            $mdat_offset += 8;
                        } else {
                            break;
                    ...
                    if (substr($atom_data, $mdat_offset, 4) == "GPRO") {
                        $GOPRO_chunk_length = getid3_lib::LittleEndian2Int(
                            substr($atom_data, $mdat_offset + 4, 4)
                        );
                        $GOPRO_offset = 8;
                        $atom_structure["GPRO"]["raw"] = substr(
                            $atom_data,
                            $mdat_offset + 8,
                            $GOPRO_chunk_length - 8
                        );
                        $atom_structure["GPRO"]["firmware"] = substr(
                            $atom_structure["GPRO"]["raw"],
                            0,
                            15
                        );
                        $atom_structure["GPRO"]["unknown1"] = substr(
                            $atom_structure["GPRO"]["raw"],
                            15,
                            16
                        );
                        $atom_structure["GPRO"]["unknown2"] = substr(
                            $atom_structure["GPRO"]["raw"],
                            31,
                            32
                        );
                        $atom_structure["GPRO"]["unknown3"] = substr(
                            $atom_structure["GPRO"]["raw"],
                            63,
                            16
                        );
                        $atom_structure["GPRO"]["camera"] = substr(
                            $atom_structure["GPRO"]["raw"],
                            79,
                            32
                        );
                        $info["quicktime"]["camera"]["model"] = rtrim(
                            $atom_structure["GPRO"]["camera"],
                            "\x00"
                        );
                    }
                    ...
                    while (
                        $mdat_offset < strlen($atom_data) - 8 &&
                        ($chapter_string_length = getid3_lib::BigEndian2Int(
                            substr($atom_data, $mdat_offset, 2)
                        )) &&
                        $chapter_string_length < 1000 &&
                        $chapter_string_length <=
                            strlen($atom_data) - $mdat_offset - 2 &&
                        preg_match(
                            '#^([\x00-\xFF]{2})([\x20-\xFF]+)$#',
                            substr(
                                $atom_data,
                                $mdat_offset,
                                $chapter_string_length + 2
                            ),
                            $chapter_matches
                        )
                    ) {
                        ...
                        if (
                            substr($atom_data, $mdat_offset, 12) ==
                    ...
                    if (
                        ...
                        (!isset($info["avdataend_tmp"]) ||
                            $info["quicktime"][$atomname]["size"] >
                                $info["avdataend_tmp"] - $info["avdataoffset"])
                    ) {
                        $info["avdataoffset"] = $atom_structure["offset"] + 8; // $info['quicktime'][$atomname]['offset'] + 8;
                        $OldAVDataEnd = $info["avdataend"];
                        $info["avdataend"] =
                            $atom_structure["offset"] + $atom_structure["size"]; // $info['quicktime'][$atomname]['offset'] + $info['quicktime'][$atomname]['size'];

                        $getid3_temp = new getID3();
                        $getid3_temp->openfile(
                            $this->getid3->filename,
                            $this->getid3->info["filesize"],
                            $this->getid3->fp
                        );
                        $getid3_temp->info["avdataoffset"] =
                            $info["avdataoffset"];
                        ...
                        if (
                            ...
                            if (!empty($getid3_temp->info["mpeg"])) {
                                ...
                                if (isset($info["mpeg"]["audio"])) {
                                    ...
                                    $info["audio"]["codec"] = !empty(
                                        $info["mpeg"]["audio"]["encoder"]
                                    )
                                        ? $info["mpeg"]["audio"]["encoder"]
                                        : (!empty(
                                            ...
                                            ? $info["mpeg"]["audio"]["codec"]
                                            : (!empty(
                                                $info["mpeg"]["audio"]["LAME"]
                                            )
                                                ? "LAME"
                                                : "mp3"));
                                    $info["audio"]["sample_rate"] =
                                        $info["mpeg"]["audio"]["sample_rate"];
                                    $info["audio"]["channels"] =
                                        $info["mpeg"]["audio"]["channels"];
                                    $info["audio"]["bitrate"] =
                                        $info["mpeg"]["audio"]["bitrate"];
                                    $info["audio"]["bitrate_mode"] = strtolower(
                                        $info["mpeg"]["audio"]["bitrate_mode"]
                                    );
                                    $info["bitrate"] =
                                        $info["audio"]["bitrate"];
                                }
                ...
                case "ID32": // ID3v2
                    ...
                    $getid3_id3v2->StartingOffset =
                        $atom_structure["offset"] + 14; // framelength(4)+framename(4)+flags(4)+??(2)
                    if ($atom_structure["valid"] = $getid3_id3v2->Analyze()) {
                        $atom_structure["id3v2"] = $getid3_temp->info["id3v2"];
                    } else {
                        $this->warning(
                            "ID32 frame at offset " .
                                $atom_structure["offset"] .
                                " did not parse"
                        );
                    }
                ...
                case "pano": // PANOrama track (seen on QTVR)
                    $atom_structure["pano"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 0, 4)
                    );
                    break;
                ...
                case "imgt": // IMaGe Track reference (kQTVRImageTrackRefType) (seen on QTVR)
                    for ($i = 0; $i < $atom_structure["size"] - 8; $i += 4) {
                        $atom_structure["imgt"][] = getid3_lib::BigEndian2Int(
                            substr($atom_data, $i, 4)
                        );
                    }
                    break;
                ...
                case "\xA9" . "xyz": // GPS latitude+longitude+altitude
                    ...
                    if (
                        ...
                        $this->warning(
                            'QuickTime atom "©xyz" data does not match expected data pattern at offset ' .
                                $baseoffset .
                ...
                case "NCTG": // Nikon - https://exiftool.org/TagNames/Nikon.html#NCTG
                    getid3_lib::IncludeDependency(
                        GETID3_INCLUDEPATH . "module.tag.nikon-nctg.php",
                        __FILE__,
                        true
                    );
                ...
                case "data": // metaDATA atom
                    ...
                    $atom_structure["unknown"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 4 + 2, 2)
                    );
                    $atom_structure["data"] = substr($atom_data, 4 + 4);
                    $atom_structure["key_name"] = isset(
                        $info["quicktime"]["temp_meta_key_names"][$metaDATAkey]
                    )
                        ? $info["quicktime"]["temp_meta_key_names"][
                            $metaDATAkey
                        ]
                        : "";
                    $metaDATAkey++;
                ...
                case "keys": // KEYS that may be present in the metadata atom.
                    ...
                    $atom_structure["entry_count"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, 4, 4)
                    );
                    $keys_atom_offset = 8;
                    for ($i = 1; $i <= $atom_structure["entry_count"]; $i++) {
                        $atom_structure["keys"][$i][
                            "key_size"
                        ] = getid3_lib::BigEndian2Int(
                            substr($atom_data, $keys_atom_offset + 0, 4)
                        );
                        $atom_structure["keys"][$i]["key_namespace"] = substr(
                            $atom_data,
                            $keys_atom_offset + 4,
                            4
                        );
                        $atom_structure["keys"][$i]["key_value"] = substr(
                            $atom_data,
                            $keys_atom_offset + 8,
                            $atom_structure["keys"][$i]["key_size"] - 8
                        );
                        $keys_atom_offset +=
                            $atom_structure["keys"][$i]["key_size"]; // key_size includes the 4+4 bytes for key_size and key_namespace

                        $info["quicktime"]["temp_meta_key_names"][$i] =
                            $atom_structure["keys"][$i]["key_value"];
                    }
                    break;
                ...
                case "uuid": // user-defined atom often seen containing XML data, also used for potentially many other purposes, only a few specifically handled by getID3 (e.g. 360fly spatial data)
                    ...
                    $uuid_bytes_read = unpack(
                        "H8time_low/H4time_mid/H4time_hi/H4clock_seq_hi/H12clock_seq_low",
                        substr($atom_data, 0, 16)
                    );
                    $atom_structure["uuid_field_id"] = implode(
                        "-",
                        $uuid_bytes_read
                    );
                    ...
                    switch ($atom_structure["uuid_field_id"]) {
                        ...
                        case "d08a4f18-10f3-4a82-b6c8-32d8aba183d3": // PIFF Protection System Specific Header Box - http://fileformats.archiveteam.org/wiki/Protected_Interoperable_File_Format
                            $this->warning(
                                'Unhandled (but recognized) "uuid" atom identified by "' .
                                    $atom_structure["uuid_field_id"] .
                                    '" at offset ' .
                                    $atom_structure["offset"] .
                                    " (" .
                                    strlen($atom_data) .
                        ...
                        case "be7acfcb-97a9-42e8-9c71-999491e3afac": // XMP data (in XML format)
                            $atom_structure["xml"] = substr(
                                $atom_data,
                                16,
                                strlen($atom_data) - 16 - 8
                        ...
                        case "efe1589a-bb77-49ef-8095-27759eb1dc6f": // 360fly data
                            ...
                            $uuid_bytes_read = unpack(
                                "vheader_size/vheader_version/vtimescale/vhardware_version/x/x/x/x/x/x/x/x/x/x/x/x/x/x/x/x/",
                                substr($atom_data, 16, 32)
                            );
                            ...
                            foreach (
                                str_split($atom_SENSOR_data, 32)
                                as $sensor_key => $sensor_data
                            ) {
                                ...
                                switch ($uuid_bytes_read[1]) {
                                    case 250:
                                        $atom_structure["sensor_data"][
                                            "data_type"
                                        ]["fusion_count"]++;
                                        $uuid_bytes_read = unpack(
                                            "cmode/Jtimestamp/Gyaw/Gpitch/Groll/x*",
                                            $sensor_data_content
                                        );
                                        $sensor_data_array["mode"] =
                                            $uuid_bytes_read["mode"];
                                        $sensor_data_array["timestamp"] =
                                            $uuid_bytes_read["timestamp"];
                                        $sensor_data_array["yaw"] =
                                            $uuid_bytes_read["yaw"];
                                        $sensor_data_array["pitch"] =
                                            $uuid_bytes_read["pitch"];
                                        $sensor_data_array["roll"] =
                                            $uuid_bytes_read["roll"];
                                        array_push(
                                            $atom_structure["sensor_data"][
                                                "data_type"
                                            ]["fusion_data"],
                                    ...
                                    case 3:
                                        ...
                                        $uuid_bytes_read = unpack(
                                            "cmode/Jtimestamp/Gmagx/Gmagy/Gmagz/x*",
                                            $sensor_data_content
                                        );
                                        $sensor_data_array["mode"] =
                                            $uuid_bytes_read["mode"];
                                        $sensor_data_array["timestamp"] =
                                            $uuid_bytes_read["timestamp"];
                                        $sensor_data_array["magx"] =
                                            $uuid_bytes_read["magx"];
                                        $sensor_data_array["magy"] =
                                            $uuid_bytes_read["magy"];
                                        $sensor_data_array["magz"] =
                                            $uuid_bytes_read["magz"];
                                        array_push(
                                            $atom_structure["sensor_data"][
                                                "data_type"
                                            ]["magno_data"],
                                    ...
                                    case 5:
                                        ...
                                        $uuid_bytes_read = unpack(
                                            "cmode/Jtimestamp/Glat/Glon/Galt/Gspeed/nbearing/nacc/x*",
                                            $sensor_data_content
                                        );
                                        $sensor_data_array["mode"] =
                                            $uuid_bytes_read["mode"];
                                        $sensor_data_array["timestamp"] =
                                            $uuid_bytes_read["timestamp"];
                                        $sensor_data_array["lat"] =
                                            $uuid_bytes_read["lat"];
                                        $sensor_data_array["lon"] =
                                            $uuid_bytes_read["lon"];
                                        $sensor_data_array["alt"] =
                                            $uuid_bytes_read["alt"];
                                        $sensor_data_array["speed"] =
                                            $uuid_bytes_read["speed"];
                                        $sensor_data_array["bearing"] =
                                            $uuid_bytes_read["bearing"];
                                        $sensor_data_array["acc"] =
                                            $uuid_bytes_read["acc"];
                                        array_push(
                                            $atom_structure["sensor_data"][
                                                "data_type"
                                    ...
                                    case 6:
                                        ...
                                        $uuid_bytes_read = unpack(
                                            "cmode/Jtimestamp/Grotx/Groty/Grotz/x*",
                                            $sensor_data_content
                                        );
                                        $sensor_data_array["mode"] =
                                            $uuid_bytes_read["mode"];
                                        $sensor_data_array["timestamp"] =
                                            $uuid_bytes_read["timestamp"];
                                        $sensor_data_array["rotx"] =
                                            $uuid_bytes_read["rotx"];
                                        $sensor_data_array["roty"] =
                                            $uuid_bytes_read["roty"];
                                        $sensor_data_array["rotz"] =
                                            $uuid_bytes_read["rotz"];
                                        array_push(
                                            $atom_structure["sensor_data"][
                                                "data_type"
                                            ]["rotation_data"],
                            ...
                            $atom_structure["sensor_data"]["data_type"][
                                "debug_list"
                            ] = "No debug items in list!";
                            //}
                            break;

                        default:
                            $this->warning(
                                'Unhandled "uuid" atom identified by "' .
                                    $atom_structure["uuid_field_id"] .
                                    '" at offset ' .
                                    $atom_structure["offset"] .
                                    " (" .
                ...
                case "gps ":
                    ...
                    if (strlen($atom_data) > 0) {
                        if (strlen($atom_data) % $GPS_rowsize == 0) {
                            ...
                            foreach (
                                str_split($atom_data, $GPS_rowsize)
                                as $counter => $datapair
                            ) {
                                $atom_structure["gps_toc"][] = unpack(
                                    "Noffset/Nsize",
                                    substr(
                                        $atom_data,
                                        $counter * $GPS_rowsize,
                            ...
                            foreach (
								...
								*/
                                ...
                                if (
                                    ...
                                    $GPS_this_GPRMC["active"] =
                                        $GPS_this_GPRMC["raw"]["status"] == "A"; // A=Active,V=Void
                                    ...
                                    foreach (
                                        ...
                                        preg_match(
                                            '#^([0-9]{1,3})([0-9]{2}\\.[0-9]+)$#',
                                            $GPS_this_GPRMC["raw"][$latlon],
                                            $matches
                                        );
                                        list($dummy, $deg, $min) = $matches;
                                        $GPS_this_GPRMC[$latlon] =
                                            $deg + $min / 60;
                                    }
                                    $GPS_this_GPRMC["latitude"] *=
                                        $GPS_this_GPRMC["raw"][
                                            "latitude_direction"
                                        ] == "S"
                                            ? -1
                                            : 1;
                                    $GPS_this_GPRMC["longitude"] *=
                                        $GPS_this_GPRMC["raw"][
                                            "longitude_direction"
                                        ] == "W"
                                            ? -1
                                    ...
                                    $GPS_this_GPRMC["heading"] =
                                        $GPS_this_GPRMC["raw"]["angle"];
                                    $GPS_this_GPRMC["speed_knot"] =
                                        $GPS_this_GPRMC["raw"]["knots"];
                                    $GPS_this_GPRMC["speed_kmh"] =
                                        $GPS_this_GPRMC["raw"]["knots"] * 1.852;
                                    if ($GPS_this_GPRMC["raw"]["variation"]) {
                                        $GPS_this_GPRMC["variation"] =
                                    ...
                                    @$info["quicktime"]["gps_track"][
                                        $GPS_this_GPRMC["timestamp"]
                                    ] = [
                                        "latitude" =>
                                            (float) $GPS_this_GPRMC["latitude"],
                                        "longitude" =>
                                            (float) $GPS_this_GPRMC[
                                                "longitude"
                                            ],
                                        "speed_kmh" =>
                                            (float) $GPS_this_GPRMC[
                                        ...
                                        "heading" =>
                                            (float) $GPS_this_GPRMC["heading"],
                                    ];
                                } else {
                                    $this->warning(
                                        'Unhandled GPS format in "free" atom at offset ' .
                                            $gps_pointer["offset"]
                                    );
                                }
                            }
                ...
                case "chpl": // CHaPter List
                    ...
                    $chpl_version = getid3_lib::BigEndian2Int(
                        substr($atom_data, 4, 1)
                    ); // Expected to be 0
                    $chpl_flags = getid3_lib::BigEndian2Int(
                        substr($atom_data, 5, 3)
                    ); // Reserved, set to 0
                    $chpl_count = getid3_lib::BigEndian2Int(
                        substr($atom_data, 8, 1)
                    );
                    $chpl_offset = 9;
                    for ($i = 0; $i < $chpl_count; $i++) {
                        if ($chpl_offset + 9 >= strlen($atom_data)) {
                            $this->warning(
                                "QuickTime chapter " .
                                    $i .
                                    ' extends beyond end of "chpl" atom'
                            );
                            break;
                        }
                        $info["quicktime"]["chapters"][$i]["timestamp"] =
                            getid3_lib::BigEndian2Int(
                                substr($atom_data, $chpl_offset, 8)
                            ) / 10000000; // timestamps are stored as 100-nanosecond units
                        $chpl_offset += 8;
                        $chpl_title_size = getid3_lib::BigEndian2Int(
                            substr($atom_data, $chpl_offset, 1)
                        );
                        $chpl_offset += 1;
                        $info["quicktime"]["chapters"][$i]["title"] = substr(
                            $atom_data,
                            $chpl_offset,
                            $chpl_title_size
                        );
                ...
                case "CAME": // FIRMware version(?), seen on GoPro Hero4
                    $info["quicktime"]["camera"]["serial_hash"] = unpack(
                        "H*",
                        $atom_data
                    );
                    break;
                ...
                case "rcif":
                    // https://www.getid3.org/phpBB3/viewtopic.php?t=1908
                    if (
                        substr($atom_data, 0, 7) ==
                        "\x00\x00\x00\x00\x55\xC4" . "{"
                    ) {
                        if (
                            $json_decoded = @json_decode(
                                rtrim(substr($atom_data, 6), "\x00"),
                                true
                            )
                        ) {
                            $info["quicktime"]["camera"][
                                $atomname
                            ] = $json_decoded;
                            if (
                                $atomname == "rcif" &&
                                isset(
                                    $info["quicktime"]["camera"]["rcif"][
                                        "wxcamera"
                                    ]["rotate"]
                                )
                            ) {
                                $info["video"]["rotate"] = $info["quicktime"][
                                    "video"
                                ]["rotate"] =
                                    $info["quicktime"]["camera"]["rcif"][
                                        "wxcamera"
                            ...
                            $this->warning(
                                'Failed to JSON decode atom "' . $atomname . '"'
                            );
                            $atom_structure["data"] = $atom_data;
                        }
                        unset($json_decoded);
                        ...
                        $this->warning(
                            'Expecting 55 C4 7B at start of atom "' .
                                $atomname .
                                '", found ' .
                                getid3_lib::PrintHexBytes(
                                    substr($atom_data, 4, 3)
                                ) .
                                " instead"
                        );
                        $atom_structure["data"] = $atom_data;
                ...
                case "cdsc": // timed metadata reference
                    ...
                    $atom_structure["track_number"] = getid3_lib::BigEndian2Int(
                        $atom_data
                    );
                    break;
                ...
                case "esds": // Elementary Stream DeScriptor
                    ...
                    $atom_structure["ES_DescrTag"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, $esds_offset, 1)
                    );
                    $esds_offset += 1;
                    if ($atom_structure["ES_DescrTag"] != 0x03) {
                        $this->warning(
                            "expecting esds.ES_DescrTag = 0x03, found 0x" .
                                getid3_lib::PrintHexBytes(
                                    $atom_structure["ES_DescrTag"]
                                ) .
                                "), at offset " .
                                $atom_structure["offset"]
                        );
                    ...
                    $atom_structure["ES_ID"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, $esds_offset, 2)
                    );
                    $esds_offset += 2;
                    $atom_structure["ES_flagsraw"] = getid3_lib::BigEndian2Int(
                        substr($atom_data, $esds_offset, 1)
                    );
                    $esds_offset += 1;
                    $atom_structure["ES_flags"]["stream_dependency"] =
                        (bool) ($atom_structure["ES_flagsraw"] & 0x80);
                    $atom_structure["ES_flags"]["url_flag"] =
                        (bool) ($atom_structure["ES_flagsraw"] & 0x40);
                    $atom_structure["ES_flags"]["ocr_stream"] =
                        (bool) ($atom_structure["ES_flagsraw"] & 0x20);
                    $atom_structure["ES_stream_priority"] =
                        $atom_structure["ES_flagsraw"] & 0x1f;
                    if ($atom_structure["ES_flags"]["url_flag"]) {
                        $this->warning(
                            "Unsupported esds.url_flag enabled at offset " .
                                $atom_structure["offset"]
                        );
                        break;
                    }
                    ...
                    if ($atom_structure["ES_DecoderConfigDescrTag"] != 0x04) {
                        $this->warning(
                            "expecting esds.ES_DecoderConfigDescrTag = 0x04, found 0x" .
                                getid3_lib::PrintHexBytes(
                                    $atom_structure["ES_DecoderConfigDescrTag"]
                                ) .
                                "), at offset " .
                                $atom_structure["offset"]
                        );
                    ...
                    $streamTypePlusFlags = getid3_lib::BigEndian2Int(
                        substr($atom_data, $esds_offset, 1)
                    );
                    $esds_offset += 1;
                    $atom_structure["ES_streamType"] =
                        ($streamTypePlusFlags & 0xfc) >> 2;
                    $atom_structure["ES_upStream"] =
                        (bool) ($streamTypePlusFlags & 0x02) >> 1;
                    $atom_structure[
                        "ES_bufferSizeDB"
                    ] = getid3_lib::BigEndian2Int(
                        substr($atom_data, $esds_offset, 3)
                    );
                    $esds_offset += 3;
                    $atom_structure[
                        "ES_maxBitrate"
                    ] = getid3_lib::BigEndian2Int(
                        substr($atom_data, $esds_offset, 4)
                    );
                    $esds_offset += 4;
                    $atom_structure[
                        "ES_avgBitrate"
                    ...
                    if ($atom_structure["ES_avgBitrate"]) {
                        $info["quicktime"]["audio"]["bitrate"] =
                            $atom_structure["ES_avgBitrate"];
                        $info["audio"]["bitrate"] =
                            $atom_structure["ES_avgBitrate"];
                    }

                    $atom_structure[
                        "ES_DecSpecificInfoTag"
                    ] = getid3_lib::BigEndian2Int(
                        substr($atom_data, $esds_offset, 1)
                    );
                    $esds_offset += 1;
                    if ($atom_structure["ES_DecSpecificInfoTag"] != 0x05) {
                        $this->warning(
                            "expecting esds.ES_DecSpecificInfoTag = 0x05, found 0x" .
                                getid3_lib::PrintHexBytes(
                                    $atom_structure["ES_DecSpecificInfoTag"]
                                ) .
                                "), at offset " .
                                $atom_structure["offset"]
                        );
                    ...
                    $atom_structure[
                        ...
                        substr(
                            $atom_data,
                            $esds_offset,
                            $atom_structure["ES_DecSpecificInfoTagSize"]
                        )
                    );
                    $esds_offset +=
                        $atom_structure["ES_DecSpecificInfoTagSize"];
                    ...
                    if ($atom_structure["ES_SLConfigDescrTag"] != 0x06) {
                        $this->warning(
                            "expecting esds.ES_SLConfigDescrTag = 0x05, found 0x" .
                                getid3_lib::PrintHexBytes(
                                    $atom_structure["ES_SLConfigDescrTag"]
                                ) .
                                "), at offset " .
                                $atom_structure["offset"]
                        );
                    ...
                    $atom_structure[
                        ...
                        substr(
                            $atom_data,
                            $esds_offset,
                            $atom_structure["ES_SLConfigDescrTagSize"]
                        )
                    );
                ...
                case "trun": // Track fragment RUN box
                    $this->error(
                        "fragmented mp4 files not currently supported"
                    );
                    $atom_structure["data"] = $atom_data;
                    break;
                ...
                default:
                    $this->warning(
                        'Unknown QuickTime atom type: "' .
                            ...
                            '" (' .
                            trim(getid3_lib::PrintHexBytes($atomname)) .
                            "), " .
                            $atomsize .
                            " bytes at offset " .
                            $baseoffset
     ...
     * @return array|false
     */
    public function QuicktimeParseContainerAtom(
        $atom_data,
        $baseoffset,
        &$atomHierarchy,
        $ParseAllPossibleAtoms
        ...
        if (
            strlen($atom_data) == 4 &&
            getid3_lib::BigEndian2Int($atom_data) == 0x00000000
        ) {
            return false;
        }
        while ($subatomoffset < strlen($atom_data)) {
            $subatomsize = getid3_lib::BigEndian2Int(
                substr($atom_data, $subatomoffset + 0, 4)
            );
            ...
            $atom_structure[$subatomcounter++] = $this->QuicktimeParseAtom(
                $subatomname,
                $subatomsize,
                $subatomdata,
        ...
        if ($languageid > 138 && $languageid < 32767) {
			...
			*/
            ...
            $QuicktimeLanguageLookup[
                $languageid
            ] = getid3_id3v2::LanguageLookup($iso_language_id);
        }
        return isset($QuicktimeLanguageLookup[$languageid])
    ...
    public function QuicktimeVideoCodecLookup($codecid)
    {
        ...
        return isset($QuicktimeVideoCodecLookup[$codecid])
    ...
    public function QuicktimeAudioCodecLookup($codecid)
    {
        ...
        if (empty($QuicktimeAudioCodecLookup)) {
            ...
            $QuicktimeAudioCodecLookup["ima4"] =
            ...
            $QuicktimeAudioCodecLookup["MAC3"] =
                "Macintosh Audio Compression/Expansion (MACE) 3:1";
            $QuicktimeAudioCodecLookup["MAC6"] =
            ...
            $QuicktimeAudioCodecLookup["MS" . "\x00\x55"] =
            ...
            $QuicktimeAudioCodecLookup["sowt"] =
            ...
            $QuicktimeAudioCodecLookup["twos"] =
                'signed/two\'s complement (Big Endian)';
            $QuicktimeAudioCodecLookup["ulaw"] = "mu-law 2:1";
        }
        return isset($QuicktimeAudioCodecLookup[$codecid])
    ...
    public function QuicktimeDCOMLookup($compressionid)
    {
        ...
        return isset($QuicktimeDCOMLookup[$compressionid])
    ...
    public function QuicktimeColorNameLookup($colordepthid)
    {
        ...
        return isset($QuicktimeColorNameLookup[$colordepthid])
    ...
    public function QuicktimeSTIKLookup($stik)
    {
        ...
        return isset($QuicktimeSTIKLookup[$stik])
    ...
    public function QuicktimeIODSaudioProfileName($audio_profile_id)
    {
        ...
        return isset($QuicktimeIODSaudioProfileNameLookup[$audio_profile_id])
            ? $QuicktimeIODSaudioProfileNameLookup[$audio_profile_id]
    ...
    public function QuicktimeIODSvideoProfileName($video_profile_id)
    {
        ...
        return isset($QuicktimeIODSvideoProfileNameLookup[$video_profile_id])
            ? $QuicktimeIODSvideoProfileNameLookup[$video_profile_id]
    ...
    public function QuicktimeContentRatingLookup($rtng)
    {
        ...
        return isset($QuicktimeContentRatingLookup[$rtng])
    ...
    public function QuicktimeStoreAccountTypeLookup($akid)
    {
        ...
        return isset($QuicktimeStoreAccountTypeLookup[$akid])
    ...
    public function QuicktimeStoreFrontCodeLookup($sfid)
    {
        ...
        return isset($QuicktimeStoreFrontCodeLookup[$sfid])
     ...
     * @return bool
     */
    public function CopyToAppropriateCommentsSection(
        $keyname,
        $data,
        $boxname = ""
    ) {
        ...
        if ($boxname && $boxname != $keyname) {
            $comment_key = isset($handyatomtranslatorarray[$boxname])
                ? $handyatomtranslatorarray[$boxname]
                : $boxname;
        ...
        if ($comment_key) {
            ...
            foreach ($gooddata as $data) {
                if (
                    !empty($info["quicktime"]["comments"][$comment_key]) &&
                    in_array(
                        $data,
                        $info["quicktime"]["comments"][$comment_key],
                        true
                    )
                ) {
    ...
    public function LociString($lstring, &$count)
    {
        ...
        if (
            $len > 2 &&
            (($lstring[0] == "\xFE" && $lstring[1] == "\xFF") ||
                ($lstring[0] == "\xFF" && $lstring[1] == "\xFE"))
        ) {
            // UTF-16
        ...
        foreach ($res as $value) {
            ...
            if (count($stbl_res) > 0) {
                ...
                foreach ($ts_res as $sub_value) {
                    ...
                    if (
                        substr($stbl_res[0][0], 0, strlen($prefix)) ===
                            $prefix &&
