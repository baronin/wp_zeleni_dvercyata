...
class getid3_dts extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        for (
            $word_offset = 0;
            $word_offset <= strlen($DTSheader);
            $word_offset += 2
        ) {
            switch ($encoding) {
                case 0: // raw big-endian
                    $fhBS .= getid3_lib::BigEndian2Bin(
                        substr($DTSheader, $word_offset, 2)
                    );
                    break;
                case 1: // raw little-endian
                    $fhBS .= getid3_lib::BigEndian2Bin(
                        strrev(substr($DTSheader, $word_offset, 2))
                    );
                    break;
        ...
        $info["dts"]["flags"]["crc_present"] = (bool) $this->readBinData(
            $fhBS,
            1
        );
        $info["dts"]["raw"]["pcm_sample_blocks"] = $this->readBinData($fhBS, 7);
        $info["dts"]["raw"]["frame_byte_size"] = $this->readBinData($fhBS, 14);
        $info["dts"]["raw"]["channel_arrangement"] = $this->readBinData(
            $fhBS,
            6
        );
        $info["dts"]["raw"]["sample_frequency"] = $this->readBinData($fhBS, 4);
        $info["dts"]["raw"]["bitrate"] = $this->readBinData($fhBS, 5);
        $info["dts"]["flags"]["embedded_downmix"] = (bool) $this->readBinData(
            $fhBS,
            1
        );
        $info["dts"]["flags"]["dynamicrange"] = (bool) $this->readBinData(
            $fhBS,
            1
        );
        $info["dts"]["flags"]["timestamp"] = (bool) $this->readBinData(
            $fhBS,
            1
        );
        ...
        $info["dts"]["flags"]["extended_coding"] = (bool) $this->readBinData(
            $fhBS,
            1
        );
        $info["dts"]["flags"][
            "audio_sync_insertion"
        ] = (bool) $this->readBinData($fhBS, 1);
        $info["dts"]["raw"]["lfe_effects"] = $this->readBinData($fhBS, 2);
        $info["dts"]["flags"]["predictor_history"] = (bool) $this->readBinData(
            $fhBS,
            1
        );
        ...
        $info["dts"]["raw"]["encoder_soft_version"] = $this->readBinData(
            $fhBS,
            4
        );
        $info["dts"]["raw"]["copy_history"] = $this->readBinData($fhBS, 2);
        $info["dts"]["raw"]["bits_per_sample"] = $this->readBinData($fhBS, 2);
        $info["dts"]["flags"]["surround_es"] = (bool) $this->readBinData(
            $fhBS,
            1
        );
        $info["dts"]["flags"]["front_sum_diff"] = (bool) $this->readBinData(
            $fhBS,
            1
        );
        $info["dts"]["flags"]["surround_sum_diff"] = (bool) $this->readBinData(
            $fhBS,
            1
        );
        $info["dts"]["raw"]["dialog_normalization"] = $this->readBinData(
            $fhBS,
            4
        );

        $info["dts"]["bitrate"] = self::bitrateLookup(
            $info["dts"]["raw"]["bitrate"]
        );
        $info["dts"]["bits_per_sample"] = self::bitPerSampleLookup(
            $info["dts"]["raw"]["bits_per_sample"]
        );
        $info["dts"]["sample_rate"] = self::sampleRateLookup(
            $info["dts"]["raw"]["sample_frequency"]
        );
        $info["dts"]["dialog_normalization"] = self::dialogNormalization(
            $info["dts"]["raw"]["dialog_normalization"],
            $info["dts"]["raw"]["encoder_soft_version"]
        );
        $info["dts"]["flags"]["lossless"] =
            $info["dts"]["raw"]["bitrate"] == 31 ? true : false;
        $info["dts"]["bitrate_mode"] =
            $info["dts"]["raw"]["bitrate"] == 30 ? "vbr" : "cbr";
        $info["dts"]["channels"] = self::numChannelsLookup(
        ...
        if (
            isset($info["avdataend"]) &&
            !empty($info["dts"]["bitrate"]) &&
            is_numeric($info["dts"]["bitrate"])
        ) {
