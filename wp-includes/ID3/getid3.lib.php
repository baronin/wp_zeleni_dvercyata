...
if (!defined("GETID3_LIBXML_OPTIONS") && defined("LIBXML_VERSION")) {
    if (LIBXML_VERSION >= 20621) {
        define(
            ...
            LIBXML_NOENT | LIBXML_NONET | LIBXML_NOWARNING | LIBXML_COMPACT
        );
    } else {
        define(
            "GETID3_LIBXML_OPTIONS",
...
class getid3_lib
{
     ...
     * @return string
     */
    public static function PrintHexBytes(
        $string,
        $hex = true,
        $spaces = true,
        $htmlencoding = "UTF-8"
    ...
    public static function DecimalBinary2Float($binarynumerator)
    {
        ...
        $denominator = self::Bin2Dec(
            "1" . str_repeat("0", strlen($binarynumerator))
        );
     ...
     * @return array
     */
    public static function NormalizeBinaryPoint(
        $binarypointnumber,
        $maxbits = 52
    ) {
        ...
        while (
            ...
            if (substr($binarypointnumber, 1, 1) == ".") {
                ...
                $binarypointnumber =
                    $binarypointnumber[0] . "." . substr($binarypointnumber, 1);
            }
        }
        $binarypointnumber = str_pad(
            substr($binarypointnumber, 0, $maxbits + 2),
            $maxbits + 2,
            "0",
            STR_PAD_RIGHT
        );
    ...
    public static function Float2String($floatvalue, $bits)
    {
        ...
        $normalizedbinary = self::NormalizeBinaryPoint(
            self::Float2BinaryDecimal($floatvalue),
            $fractionbits
        );
        $biasedexponent =
            pow(2, $exponentbits - 1) - 1 + $normalizedbinary["exponent"]; // (127 or 1023) +/- exponent
        $exponentbitstring = str_pad(
            decbin($biasedexponent),
            $exponentbits,
            "0",
            STR_PAD_LEFT
        );
        $fractionbitstring = str_pad(
            substr($normalizedbinary["normalized"], 2),
            $fractionbits,
            "0",
            STR_PAD_RIGHT
        );

        return self::BigEndian2String(
            self::Bin2Dec($signbit . $exponentbitstring . $fractionbitstring),
            $bits % 8,
            false
        );
    }
    ...
    public static function BigEndian2Float($byteword)
    {
        ...
        switch (strlen($byteword) * 8) {
            ...
            case 80:
                ...
                $fraction =
                    $isnormalized + self::DecimalBinary2Float($fractionstring);
        ...
        if ($exponent == pow(2, $exponentbits) - 1 && $fraction != 0) {
            ...
            $floatvalue =
                pow(2, -1 * (pow(2, $exponentbits - 1) - 2)) *
                self::DecimalBinary2Float($fractionstring);
            if ($signbit == "1") {
                $floatvalue *= -1;
            }
        } elseif ($exponent != 0) {
            $floatvalue =
                pow(2, $exponent - (pow(2, $exponentbits - 1) - 1)) *
                (1 + self::DecimalBinary2Float($fractionstring));
            if ($signbit == "1") {
                $floatvalue *= -1;
            }
        }
     ...
     * @throws Exception
     */
    public static function BigEndian2Int(
        $byteword,
        $synchsafe = false,
        $signed = false
    ) {
        ...
        if ($signed && !$synchsafe) {
            ...
            if ($bytewordlen <= PHP_INT_SIZE) {
                ...
                throw new Exception(
                    "ERROR: Cannot have signed integers larger than " .
                        8 * PHP_INT_SIZE .
                        "-bits (" .
                        strlen($byteword) .
     ...
     * @throws Exception
     */
    public static function BigEndian2String(
        $number,
        $minbytes = 1,
        $synchsafe = false,
        $signed = false
        ...
        while ($number != 0) {
            ...
            $intstring =
                chr(ceil(($quotient - floor($quotient)) * $maskbyte)) .
                $intstring;
    ...
    public static function Dec2Bin($number)
    {
        if (!is_numeric($number)) {
            ...
            trigger_error(
                'TypeError: Dec2Bin(): Argument #1 ($number) must be numeric, ' .
                    gettype($number) .
                    " given",
                E_USER_WARNING
            );
        ...
        foreach ($bytes as $i => $byte) {
            $binstring =
                ($i == count($bytes) - 1
                    ? decbin($byte)
                    : str_pad(decbin($byte), 8, "0", STR_PAD_LEFT)) .
                $binstring;
        }
        return $binstring;
    }
    ...
    public static function Bin2Dec($binstring, $signed = false)
    {
        ...
        for ($i = 0; $i < strlen($binstring); $i++) {
            $decvalue +=
                ((int) substr($binstring, strlen($binstring) - $i - 1, 1)) *
                pow(2, $i);
        }
    ...
    public static function Bin2String($binstring)
    {
        ...
        for ($i = 0; $i < strlen($binstringreversed); $i += 8) {
            $string =
                chr(self::Bin2Dec(strrev(substr($binstringreversed, $i, 8)))) .
                $string;
        }
        return $string;
    }
     ...
     * @return string
     */
    public static function LittleEndian2String(
        $number,
        $minbytes = 1,
        $synchsafe = false
    ) {
    ...
    public static function array_merge_clobber($array1, $array2)
    {
        ...
        foreach ($array2 as $key => $val) {
            if (
                ...
                $newarray[$key] = self::array_merge_clobber(
                    $newarray[$key],
                    $val
                );
            } else {
    ...
    public static function array_merge_noclobber($array1, $array2)
    {
        ...
        foreach ($array2 as $key => $val) {
            if (
                ...
                $newarray[$key] = self::array_merge_noclobber(
                    $newarray[$key],
                    $val
                );
    ...
    public static function PlaytimeString($seconds)
    {
        ...
        return $sign .
            ($H ? $H . ":" : "") .
            ($H ? str_pad($M, 2, "0", STR_PAD_LEFT) : intval($M)) .
            ":" .
    ...
    {
        return self::BigEndian2Int(substr($rawdata, 0, 1)) +
            (float) (self::BigEndian2Int(substr($rawdata, 1, 1)) / pow(2, 8));
    }
    ...
    {
        return self::BigEndian2Int(substr($rawdata, 0, 2)) +
            (float) (self::BigEndian2Int(substr($rawdata, 2, 2)) / pow(2, 16));
    }
    ...
    public static function FixedPoint2_30($rawdata)
    {
        ...
        return self::Bin2Dec(substr($binarystring, 0, 2)) +
            (float) (self::Bin2Dec(substr($binarystring, 2, 30)) / pow(2, 30));
    }
    ...
    public static function CreateDeepArray($ArrayPath, $Separator, $Value)
    {
        ...
        if (($pos = strpos($ArrayPath, $Separator)) !== false) {
            $ReturnedArray[substr($ArrayPath, 0, $pos)] = self::CreateDeepArray(
                substr($ArrayPath, $pos + 1),
                $Separator,
                $Value
            );
        } else {
    ...
    public static function SimpleXMLelement2array($XMLobject)
    {
        ...
        $XMLarray =
            $XMLobject instanceof SimpleXMLElement
                ? get_object_vars($XMLobject)
                : $XMLobject;
    ...
    public static function hash_data($file, $offset, $end, $algorithm)
    {
        ...
        if (!in_array($algorithm, ["md5", "sha1"])) {
            throw new getid3_exception(
                "Invalid algorithm (" . $algorithm . ") in self::hash_data()"
            );
        }
     ...
     */
    public static function CopyFileParts(
        $filename_source,
        $filename_dest,
        $offset,
        $length
        ...
        if (!self::intValueSupported($offset + $length)) {
            throw new Exception(
                "cannot copy file portion, it extends beyond the " .
                    round(PHP_INT_MAX / 1073741824) .
                    "GB limit"
            );
        }
        if (
            is_readable($filename_source) &&
            is_file($filename_source) &&
            ...
            if ($fp_dest = fopen($filename_dest, "wb")) {
                if (fseek($fp_src, $offset) == 0) {
                    ...
                    while (
                        ...
                        ($buffer = fread(
                            $fp_src,
                            min($byteslefttowrite, getID3::FREAD_BUFFER_SIZE)
                        ))
                    ) {
                        $byteswritten = fwrite(
                            $fp_dest,
     ...
     * @return string
     */
    public static function iconv_fallback_iso88591_utf16be(
        $string,
        $bom = false
    ) {
        $newcharstring = "";
     ...
     * @return string
     */
    public static function iconv_fallback_iso88591_utf16le(
        $string,
        $bom = false
    ) {
        $newcharstring = "";
    ...
    public static function iconv_fallback_utf8_utf16be($string, $bom = false)
    {
        ...
        while ($offset < $stringlength) {
            ...
            if ($charval !== false) {
                $newcharstring .=
                    $charval < 65536
                        ? self::BigEndian2String($charval, 2)
                        : "\x00" . "?";
            }
        }
        return $newcharstring;
    ...
    public static function iconv_fallback_utf8_utf16le($string, $bom = false)
    {
        ...
        while ($offset < $stringlength) {
            ...
            if ($charval !== false) {
                $newcharstring .=
                    $charval < 65536
                        ? self::LittleEndian2String($charval, 2)
                        : "?" . "\x00";
            }
        }
        return $newcharstring;
    ...
    public static function iconv_fallback($in_charset, $out_charset, $string)
    {
        ...
        if (function_exists("mb_convert_encoding")) {
            if (
                strtoupper($in_charset) == "UTF-16" &&
                substr($string, 0, 2) != "\xFE\xFF" &&
                substr($string, 0, 2) != "\xFF\xFE"
            ) {
            ...
            if (
                $converted_string = @mb_convert_encoding(
                    $string,
                    $out_charset,
                    $in_charset
                )
        ...
        if (empty($ConversionFunctionList)) {
            $ConversionFunctionList["ISO-8859-1"]["UTF-8"] =
                "iconv_fallback_iso88591_utf8";
            $ConversionFunctionList["ISO-8859-1"]["UTF-16"] =
                "iconv_fallback_iso88591_utf16";
            $ConversionFunctionList["ISO-8859-1"]["UTF-16BE"] =
                "iconv_fallback_iso88591_utf16be";
            $ConversionFunctionList["ISO-8859-1"]["UTF-16LE"] =
                "iconv_fallback_iso88591_utf16le";
            $ConversionFunctionList["UTF-8"]["ISO-8859-1"] =
                "iconv_fallback_utf8_iso88591";
            $ConversionFunctionList["UTF-8"]["UTF-16"] =
                "iconv_fallback_utf8_utf16";
            $ConversionFunctionList["UTF-8"]["UTF-16BE"] =
                "iconv_fallback_utf8_utf16be";
            $ConversionFunctionList["UTF-8"]["UTF-16LE"] =
                "iconv_fallback_utf8_utf16le";
            $ConversionFunctionList["UTF-16"]["ISO-8859-1"] =
                "iconv_fallback_utf16_iso88591";
            $ConversionFunctionList["UTF-16"]["UTF-8"] =
                "iconv_fallback_utf16_utf8";
            $ConversionFunctionList["UTF-16LE"]["ISO-8859-1"] =
                "iconv_fallback_utf16le_iso88591";
            $ConversionFunctionList["UTF-16LE"]["UTF-8"] =
                "iconv_fallback_utf16le_utf8";
            $ConversionFunctionList["UTF-16BE"]["ISO-8859-1"] =
                "iconv_fallback_utf16be_iso88591";
            $ConversionFunctionList["UTF-16BE"]["UTF-8"] =
                "iconv_fallback_utf16be_utf8";
        }
        if (
            isset(
                $ConversionFunctionList[strtoupper($in_charset)][
                    strtoupper($out_charset)
                ]
            )
        ) {
            $ConversionFunction =
        ...
        throw new Exception(
            "PHP does not has mb_convert_encoding() or iconv() support - cannot convert from " .
                $in_charset .
                " to " .
                $out_charset
        );
     ...
     * @return mixed
     */
    public static function recursiveMultiByteCharString2HTML(
        $data,
        $charset = "ISO-8859-1"
    ) {
        if (is_string($data)) {
            ...
            foreach ($data as $key => $value) {
                $return_data[$key] = self::recursiveMultiByteCharString2HTML(
                    $value,
                    $charset
                );
            }
     ...
     * @return string
     */
    public static function MultiByteCharString2HTML(
        $string,
        $charset = "ISO-8859-1"
    ) {
    ...
    public static function RGADoriginatorLookup($originatorcode)
    {
        ...
        return isset($RGADoriginator[$originatorcode])
            ? $RGADoriginator[$originatorcode]
            : "";
    }
     ...
     * @return string
     */
    public static function RGADgainString(
        $namecode,
        $originatorcode,
        $replaygain
    ) {
    ...
    public static function GetDataImageSize($imgData, &$imageinfo = [])
    {
        if (PHP_VERSION_ID >= 50400) {
            ...
            if (
                ...
                !isset($GetDataImageSize[0], $GetDataImageSize[1])
            ) {
                return false;
            }
        ...
        if ($tempfilename = tempnam($tempdir, "gI3")) {
            if (
                is_writable($tempfilename) &&
                is_file($tempfilename) &&
                ...
                if (
                    $GetDataImageSize === false ||
                    !isset($GetDataImageSize[0]) ||
                    !isset($GetDataImageSize[1])
                ) {
                    return false;
                }
                $GetDataImageSize["height"] = $GetDataImageSize[0];
     ...
     * @return bool
     */
    public static function CopyTagsToComments(
        &$ThisFileInfo,
        ...
        if (!empty($ThisFileInfo["tags"])) {
                        ...
                        if (!empty($value)) {
                            if (empty($ThisFileInfo["comments"][$tagname])) {
                                ...
                                foreach (
                                    ...
                                    as $existingkey => $existingvalue
                                ) {
                                    $oldvaluelength = strlen(
                                        trim($existingvalue)
                                    );
                                    if (
                            ...
                            if (
                                is_array($value) ||
                                empty($ThisFileInfo["comments"][$tagname]) ||
                                !in_array(
                                    trim($value),
                                    $ThisFileInfo["comments"][$tagname]
                                )
            ...
            if (!empty($ThisFileInfo["comments"])) {
                ...
                foreach ($StandardizeFieldNames as $badkey => $goodkey) {
                    if (
                        array_key_exists($badkey, $ThisFileInfo["comments"]) &&
                        !array_key_exists($goodkey, $ThisFileInfo["comments"])
                    ) {
                        $ThisFileInfo["comments"][$goodkey] =
                            $ThisFileInfo["comments"][$badkey];
                ...
                if (!empty($ThisFileInfo["comments"])) {
                    foreach ($ThisFileInfo["comments"] as $field => $values) {
                        ...
                        foreach ($values as $index => $value) {
                            if (is_array($value)) {
                                $ThisFileInfo["comments_html"][$field][
                                    $index
                                ] = $value;
                            } else {
                                $ThisFileInfo["comments_html"][$field][
    ...
    public static function EmbeddedLookup($key, $begin, $end, $file, $name)
    {
        ...
        if (isset($cache[$file][$name])) {
            return isset($cache[$file][$name][$key])
                ? $cache[$file][$name][$key]
                : "";
        }
    ...
    public static function getFileSizeSyscall($path)
    {
        ...
        if (GETID3_OS_ISWINDOWS) {
            if (class_exists("COM")) {
                ...
                $commandline =
                    "for %I in (" . escapeshellarg($path) . ") do @echo %~zI";
            }
        } else {
            $commandline =
                "ls -l " . escapeshellarg($path) . ' | awk \'{print $5}\'';
        }
    ...
    public static function mb_basename($path, $suffix = "")
    {
        ...
        return substr(
            basename("X" . $splited[count($splited) - 1], $suffix),
            1
        );
    }
}
