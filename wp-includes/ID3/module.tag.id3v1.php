...
class getid3_id3v1 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (!getid3_lib::intValueSupported($info["filesize"])) {
            $this->warning(
                "Unable to check for ID3v1 because file is larger than " .
        ...
        if (substr($id3v1tag, 0, 3) == "TAG") {
            ...
            if ($id3v1tag[125] === "\x00" && $id3v1tag[126] !== "\x00") {
                $ParsedID3v1["track_number"] = ord(
                    substr($ParsedID3v1["comment"], 29, 1)
                );
                $ParsedID3v1["comment"] = substr(
                    $ParsedID3v1["comment"],
                    0,
                    28
                );
            ...
            $ParsedID3v1["genre"] = $this->LookupGenreName(
                $ParsedID3v1["genreid"]
            );
            ...
            $GoodFormatID3v1tag = $this->GenerateID3v1Tag(
                ...
                $ParsedID3v1["album"],
                $ParsedID3v1["year"],
                isset($ParsedID3v1["genre"])
                    ? $this->LookupGenreID($ParsedID3v1["genre"])
                    : false,
                $ParsedID3v1["comment"],
                !empty($ParsedID3v1["track_number"])
            ...
            $ParsedID3v1["tag_offset_start"] =
                $ParsedID3v1["tag_offset_end"] - 128;
        ...
        if (substr($preid3v1, 0, 3) == "TAG") {
            ...
            if (substr($preid3v1, 96, 8) == "APETAGEX") {
                ...
                $this->warning(
                    "Duplicate ID3v1 tag detected - this has been known to happen with iTunes"
                );
                $info["avdataend"] -= 128;
            }
        }
    ...
    public static function GenerateID3v1Tag(
        ...
        $ID3v1Tag = "TAG";
        $ID3v1Tag .= str_pad(
            trim(substr($title, 0, 30)),
            30,
            "\x00",
            STR_PAD_RIGHT
        );
        $ID3v1Tag .= str_pad(
            trim(substr($artist, 0, 30)),
            30,
            "\x00",
            STR_PAD_RIGHT
        );
        $ID3v1Tag .= str_pad(
            trim(substr($album, 0, 30)),
            30,
            "\x00",
            STR_PAD_RIGHT
        );
        $ID3v1Tag .= str_pad(
            trim(substr($year, 0, 4)),
            4,
            "\x00",
            STR_PAD_LEFT
        );
