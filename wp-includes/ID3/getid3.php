...
class getID3
{
    ...
    public function __construct()
    {
        ...
        if (version_compare(PHP_VERSION, $required_php_version, "<")) {
            $this->startup_error .=
                "getID3() requires PHP v" .
                $required_php_version .
                " or higher - you are running v" .
                PHP_VERSION .
                "\n";
            return;
        }
        ...
        if (
            ($mbstring_func_overload = (int) ini_get(
                "mbstring.func_overload"
            )) &&
            $mbstring_func_overload & 0x02
        ) {
            ...
            // phpcs:ignore PHPCompatibility.IniDirectives.RemovedIniDirectives.mbstring_func_overloadDeprecated
            $this->startup_error .=
                'WARNING: php.ini contains "mbstring.func_overload = ' .
        ...
        if (GETID3_OS_ISWINDOWS && !defined("GETID3_HELPERAPPSDIR")) {
            $helperappsdir =
                GETID3_INCLUDEPATH . ".." . DIRECTORY_SEPARATOR . "helperapps"; // must not have any space in this path
                    ...
                    if (strpos($value, " ") !== false) {
                        if (!empty($path_so_far)) {
                            $commandline =
                                "dir /x " .
                                escapeshellarg(
                                    implode(DIRECTORY_SEPARATOR, $path_so_far)
                                );
    ...
    public function openfile($filename, $filesize = null, $fp = null)
    {
        try {
            ...
            $this->info["php_memory_limit"] =
                $this->memory_limit > 0 ? $this->memory_limit : false;

            // remote files not supported
            if (preg_match("#^(ht|f)tps?://#", $filename)) {
                throw new getid3_exception(
            ...
            if (
                ...
                throw new getid3_exception(
                    'Could not open "' .
                        $filename .
                        '" (' .
                        implode("; ", $errormessagelist) .
                        ")"
                );
            }

            $this->info["filesize"] = !is_null($filesize)
                ? $filesize
                : filesize($filename);
            ...
            $this->info["filenamepath"] =
                $this->info["filepath"] . "/" . $this->info["filename"];
            ...
            if ($this->option_max_2gb_check) {
                ...
                if (
                    $fseek < 0 ||
                    ($this->info["filesize"] != 0 && ftell($this->fp) == 0) ||
                    $this->info["filesize"] < 0 ||
                    ftell($this->fp) < 0
                ) {
                    $real_filesize = getid3_lib::getFileSizeSyscall(
                        $this->info["filenamepath"]
                    );
                    ...
                    if ($real_filesize === false) {
                        ...
                        fclose($this->fp);
                        throw new getid3_exception(
                            "Unable to determine actual filesize. File is most likely larger than " .
                        ...
                        fclose($this->fp);
                        throw new getid3_exception(
                            "PHP seems to think the file is larger than " .
                                round(PHP_INT_MAX / 1073741824) .
    ...
    public function analyze(
        ...
        try {
            ...
            if (isset($this->info["id3v2"]["tag_offset_start"])) {
                $this->info["avdataoffset"] = max(
                    $this->info["avdataoffset"],
                    $this->info["id3v2"]["tag_offset_end"]
                );
            }
            foreach (
                ["id3v1" => "id3v1", "apetag" => "ape", "lyrics3" => "lyrics3"]
                as $tag_name => $tag_key
            ) {
                if (isset($this->info[$tag_key]["tag_offset_start"])) {
                    $this->info["avdataend"] = min(
                        $this->info["avdataend"],
            ...
            if (!$this->option_tag_id3v2) {
                ...
                if (substr($header, 0, 3) == "ID3" && strlen($header) == 10) {
                    ...
                    $this->info["avdataoffset"] +=
                        getid3_lib::BigEndian2Int(substr($header, 6, 4), 1) +
            ...
            // determine format
            $determined_format = $this->GetFileFormat(
                $formattest,
            ...
            if (
                isset($determined_format["fail_id3"]) &&
                (in_array("id3v1", $this->info["tags"]) ||
                    in_array("id3v2", $this->info["tags"]))
            ) {
            ...
            if (
                isset($determined_format["fail_ape"]) &&
                in_array("ape", $this->info["tags"])
            ) {
            ...
            if (
                !file_exists(GETID3_INCLUDEPATH . $determined_format["include"])
            ) {
                fclose($this->fp);
                return $this->error(
                    'Format not supported, module "' .
            ...
            if (
                ...
                $errormessage =
                    "mb_convert_encoding() or iconv() support is required for this module (" .
                    $determined_format["include"] .
            ...
            foreach (
                ...
                as $getid3_object_vars_key => $getid3_object_vars_value
            ) {
                if (
                    preg_match(
                        '#^options_([^_]+)_([^_]+)_(.+)$#i',
                    ...
                    $GOVgroup =
                        $GOVgroup == "audiovideo" ? "audio-video" : $GOVgroup; // variable names can only contain 0-9a-z_ so standardize here
                    if (
                        $GOVgroup == $determined_format["group"] &&
                        $GOVmodule == $determined_format["module"]
                    ) {
            ...
            if ($this->option_md5_data) {
                ...
                if (
                    !$this->option_md5_data_source ||
                    empty($this->info["md5_data_source"])
                ) {
    ...
    private function CleanUp()
    {
        ...
        foreach ($AVpossibleEmptyKeys as $dummy => $key) {
            if (
                empty($this->info["audio"][$key]) &&
                isset($this->info["audio"][$key])
            ) {
                unset($this->info["audio"][$key]);
            }
            if (
                empty($this->info["video"][$key]) &&
                isset($this->info["video"][$key])
            ) {
        ...
        if (!empty($this->info["error"])) {
            $this->info["error"] = array_values(
                array_unique($this->info["error"])
            );
        }
        if (!empty($this->info["warning"])) {
            $this->info["warning"] = array_values(
                array_unique($this->info["warning"])
            );
        }
...
*/
    ...
    public function GetFileFormat(&$filedata, $filename = "")
    {
        ...
        foreach ($this->GetFileFormatArray() as $format_name => $info) {
            ...
            if (
                ...
                $info["include"] =
                    "module." . $info["group"] . "." . $info["module"] . ".php";
                return $info;
            }
        }
        ...
        if (preg_match('#\\.mp[123a]$#i', $filename)) {
            ...
            preg_match('#\\.mp[cp\\+]$#i', $filename) &&
            preg_match(
                '#[\x00\x01\x10\x11\x40\x41\x50\x51\x80\x81\x90\x91\xC0\xC1\xD0\xD1][\x20-37][\x00\x20\x40\x60\x80\xA0\xC0\xE0]#s',
                $filedata
            )
        ) {
    ...
    public function HandleAllTags()
    {
        ...
        foreach ($tags as $comment_name => $tagname_encoding_array) {
            ...
            if (
                isset($this->info[$comment_name]) &&
                !isset($this->info[$comment_name]["encoding"])
            ) {
            ...
            if (!empty($this->info[$comment_name]["comments"])) {
                foreach (
                    $this->info[$comment_name]["comments"]
                ...
                foreach ($tagarray as $tagname => $tagdata) {
                    if ($tagname == "picture") {
                        foreach ($tagdata as $key => $tagarray) {
                            ...
                            if (
                                isset($tagarray["data"]) &&
                                isset($tagarray["image_mime"])
                            ) {
                                if (
                                    isset(
                                        $this->info["tags"][$tagtype][$tagname][
                                            $key
                                        ]
                                    )
                                ) {
    ...
    {
        return getid3_lib::CopyTagsToComments(
            $ThisFileInfo,
    ...
    public function getHashdata($algorithm)
    {
        switch ($algorithm) {
            ...
            default:
                return $this->error(
                    'bad algorithm "' . $algorithm . '" in getHashdata()'
                );
        }

        if (
            !empty($this->info["fileformat"]) &&
            !empty($this->info["dataformat"]) &&
            ...
            if (preg_match("#(1|ON)#i", ini_get("safe_mode"))) {
                $this->warning(
                    "Failed making system call to vorbiscomment.exe - " .
                        $algorithm .
                ...
                if (GETID3_OS_ISWINDOWS) {
                    if (
                        file_exists(GETID3_HELPERAPPSDIR . "vorbiscomment.exe")
                    ) {
                        $commandline =
                            '"' .
                            GETID3_HELPERAPPSDIR .
                        ...
                        $VorbisCommentError =
                            "vorbiscomment.exe not found in " .
                            GETID3_HELPERAPPSDIR;
                    }
                } else {
                    $commandline =
                ...
                if (!empty($VorbisCommentError)) {
                    ...
                    switch ($algorithm) {
                        ...
                        case "sha1":
                            $this->info[$algorithm . "_data"] = sha1_file(
                                $temp
                            );
                            break;
                    }
            ...
            if (
                !empty($this->info["avdataoffset"]) ||
                (isset($this->info["avdataend"]) &&
                    $this->info["avdataend"] < $this->info["filesize"])
            ) {
                // get hash from part of file
                $this->info[$algorithm . "_data"] = getid3_lib::hash_data(
                    $this->info["filenamepath"],
                    $this->info["avdataoffset"],
                    $this->info["avdataend"],
                    $algorithm
                ...
                switch ($algorithm) {
                    case "md5":
                        $this->info[$algorithm . "_data"] = md5_file(
                            $this->info["filenamepath"]
                        );
                        break;
    ...
    public function ChannelsBitratePlaytimeCalculations()
    {
        ...
        if (
            !empty($this->info["audio"]["channelmode"]) ||
            !isset($this->info["audio"]["channels"])
        ) {
            // ignore
        ...
        $CombinedBitrate = 0;
        $CombinedBitrate += isset($this->info["audio"]["bitrate"])
            ? $this->info["audio"]["bitrate"]
            : 0;
        $CombinedBitrate += isset($this->info["video"]["bitrate"])
            ? $this->info["video"]["bitrate"]
            : 0;
        ...
        if (
            isset($this->info["video"]["dataformat"]) &&
            $this->info["video"]["dataformat"] &&
            (!isset($this->info["video"]["bitrate"]) ||
                $this->info["video"]["bitrate"] == 0)
        ) {
            // if video bitrate not set
            if (
                isset($this->info["audio"]["bitrate"]) &&
                $this->info["audio"]["bitrate"] > 0 &&
                $this->info["audio"]["bitrate"] == $this->info["bitrate"]
            ) {
                // AND if audio bitrate is set to same as overall bitrate
                if (
                    isset($this->info["playtime_seconds"]) &&
                    $this->info["playtime_seconds"] > 0
                ) {
                    // AND if playtime is set
                    if (
                        isset($this->info["avdataend"]) &&
                        isset($this->info["avdataoffset"])
                    ) {
                        // AND if AV data offset start/end is known
                        // THEN we can calculate the video bitrate
                        $this->info["bitrate"] = round(
                            (($this->info["avdataend"] -
                                $this->info["avdataoffset"]) *
                                8) /
        ...
        if (
            (!isset($this->info["playtime_seconds"]) ||
                $this->info["playtime_seconds"] <= 0) &&
            !empty($this->info["bitrate"])
        ) {
            $this->info["playtime_seconds"] =
                (($this->info["avdataend"] - $this->info["avdataoffset"]) * 8) /
                $this->info["bitrate"];
        }

        if (
            !isset($this->info["bitrate"]) &&
            !empty($this->info["playtime_seconds"])
        ) {
            $this->info["bitrate"] =
                (($this->info["avdataend"] - $this->info["avdataoffset"]) * 8) /
                $this->info["playtime_seconds"];
        ...
        if (
            isset($this->info["bitrate"]) &&
            empty($this->info["audio"]["bitrate"]) &&
            empty($this->info["video"]["bitrate"])
        ) {
            if (
                isset($this->info["audio"]["dataformat"]) &&
                empty($this->info["video"]["resolution_x"])
            ) {
                // audio only
                $this->info["audio"]["bitrate"] = $this->info["bitrate"];
            } elseif (
                isset($this->info["video"]["resolution_x"]) &&
                empty($this->info["audio"]["dataformat"])
            ) {
                // video only
        ...
        if (
            !empty($this->info["playtime_seconds"]) &&
            empty($this->info["playtime_string"])
        ) {
            $this->info["playtime_string"] = getid3_lib::PlaytimeString(
                $this->info["playtime_seconds"]
            );
        }
    }
    ...
    public function CalculateCompressionRatioVideo()
    {
        ...
        if (
            empty($this->info["video"]["resolution_x"]) ||
            empty($this->info["video"]["resolution_y"])
        ) {
            return false;
        }
        ...
        $BitrateUncompressed =
            ...
            $this->info["video"]["resolution_y"] *
            $this->info["video"]["bits_per_sample"] *
            $FrameRate;

        $this->info["video"]["compression_ratio"] = getid3_lib::SafeDiv(
            $BitrateCompressed,
            $BitrateUncompressed,
            1
        );
    ...
    public function CalculateCompressionRatioAudio()
    {
        if (
            empty($this->info["audio"]["bitrate"]) ||
            empty($this->info["audio"]["channels"]) ||
            empty($this->info["audio"]["sample_rate"]) ||
            !is_numeric($this->info["audio"]["sample_rate"])
        ) {
            return false;
        }
        $this->info["audio"]["compression_ratio"] =
            $this->info["audio"]["bitrate"] /
            ($this->info["audio"]["channels"] *
                $this->info["audio"]["sample_rate"] *
                (!empty($this->info["audio"]["bits_per_sample"])
                    ? $this->info["audio"]["bits_per_sample"]
                    : 16));

        if (!empty($this->info["audio"]["streams"])) {
            foreach (
                $this->info["audio"]["streams"]
                as $streamnumber => $streamdata
            ) {
                if (
                    !empty($streamdata["bitrate"]) &&
                    !empty($streamdata["channels"]) &&
                    !empty($streamdata["sample_rate"])
                ) {
                    $this->info["audio"]["streams"][$streamnumber][
                        "compression_ratio"
                    ] =
                        $streamdata["bitrate"] /
                        ($streamdata["channels"] *
                            $streamdata["sample_rate"] *
                            (!empty($streamdata["bits_per_sample"])
                                ? $streamdata["bits_per_sample"]
                                : 16));
                }
            }
        }
    ...
    public function CalculateReplayGain()
    {
        if (isset($this->info["replay_gain"])) {
            ...
            if (isset($this->info["replay_gain"]["track"]["adjustment"])) {
                $this->info["replay_gain"]["track"]["volume"] =
                    $this->info["replay_gain"]["reference_volume"] -
                    $this->info["replay_gain"]["track"]["adjustment"];
            }
            if (isset($this->info["replay_gain"]["album"]["adjustment"])) {
                $this->info["replay_gain"]["album"]["volume"] =
                    $this->info["replay_gain"]["reference_volume"] -
                    $this->info["replay_gain"]["album"]["adjustment"];
            }

            if (isset($this->info["replay_gain"]["track"]["peak"])) {
                $this->info["replay_gain"]["track"]["max_noclip_gain"] =
                    0 -
                    getid3_lib::RGADamplitude2dB(
                        $this->info["replay_gain"]["track"]["peak"]
                    );
            }
            if (isset($this->info["replay_gain"]["album"]["peak"])) {
                $this->info["replay_gain"]["album"]["max_noclip_gain"] =
                    0 -
                    getid3_lib::RGADamplitude2dB(
                        $this->info["replay_gain"]["album"]["peak"]
                    );
            }
        }
        return true;
    ...
    public function include_module($name)
    {
        ...
        if (!file_exists(GETID3_INCLUDEPATH . "module." . $name . ".php")) {
            throw new getid3_exception(
                "Required module." . $name . ".php is missing."
            );
        }
...
abstract class getid3_handler
{
    ...
    public function AnalyzeString($string)
    {
        ...
        $saved_filesize = isset($this->getid3->info["filesize"])
            ? $this->getid3->info["filesize"]
            : null; // may be not set if called as dependency without openfile() call
    ...
    protected function fread($bytes)
    {
        ...
        if ($bytes == 0) {
            ...
            throw new getid3_exception(
                "cannot fread(" . $bytes . " from " . $this->ftell() . ")",
                10
            );
        }
        ...
        if (!getid3_lib::intValueSupported($pos)) {
            throw new getid3_exception(
                "cannot fread(" .
                    ...
                    " from " .
                    $this->ftell() .
                    ") because beyond PHP filesystem limit",
                10
            );
        }
        ...
        do {
            ...
            if (
                $this->getid3->memory_limit > 0 &&
                $bytes / $this->getid3->memory_limit > 0.99
            ) {
                ...
                throw new getid3_exception(
                    "cannot fread(" .
                        ...
                        " from " .
                        $this->ftell() .
                        ") that is more than available PHP memory (" .
                        $this->getid3->memory_limit .
                        ")",
                    10
                );
            }
    ...
    protected function fseek($bytes, $whence = SEEK_SET)
    {
        ...
        if ($result !== 0) {
            ...
            throw new getid3_exception(
                "cannot fseek(" .
                    $pos .
                    "). resource/stream does not appear to support seeking",
                10
            );
        }
        return $result;
    ...
    public function saveAttachment($name, $offset, $length, $image_mime = null)
    {
        ...
        try {
            ...
            if (
                ...
                // copy data
                $this->fseek($offset);
                $buffersize = $this->data_string_flag
                    ? $length
