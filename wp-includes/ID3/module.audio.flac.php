...
class getid3_flac extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if ($StreamMarker != self::syncword) {
            return $this->error(
                'Expecting "' .
                    getid3_lib::PrintHexBytes(self::syncword) .
                    '" at offset ' .
                    $info["avdataoffset"] .
                    ', found "' .
                    getid3_lib::PrintHexBytes($StreamMarker) .
                    '"'
            );
        }
    ...
    public function parseMETAdata()
    {
        ...
        do {
            ...
            $BlockLength = getid3_lib::BigEndian2Int(
                substr($BlockHeader, 1, 3)
            );
            $BlockTypeText = self::metaBlockTypeLookup($BlockType);

            if ($BlockOffset + 4 + $BlockLength > $info["avdataend"]) {
                $this->warning(
                    "METADATA_BLOCK_HEADER.BLOCK_TYPE (" .
                        $BlockTypeText .
                        ") at offset " .
                        $BlockOffset .
            ...
            if ($BlockLength < 1) {
                if ($BlockTypeText != "reserved") {
                    ...
                    $this->warning(
                        "METADATA_BLOCK_HEADER.BLOCK_LENGTH (" .
                            $BlockTypeText .
                            ") at offset " .
                            $BlockOffset .
                            " is zero bytes"
            ...
            switch ($BlockTypeText) {
                ...
                case "SEEKTABLE": // 0x03
                    if (
                        !$this->parseSEEKTABLE($BlockTypeText_raw["block_data"])
                    ) {
                        return false;
                    }
                    break;
                ...
                case "CUESHEET": // 0x05
                    if (
                        !$this->parseCUESHEET($BlockTypeText_raw["block_data"])
                    ) {
                        return false;
                    }
                    break;
                ...
                default:
                    $this->warning(
                        "Unhandled METADATA_BLOCK_HEADER.BLOCK_TYPE (" .
                            $BlockType .
                            ") at offset " .
                            $BlockOffset
                    );
        ...
        if (!empty($info["flac"]["VORBIS_COMMENT"]["comments"])) {
            $info["flac"]["comments"] =
                $info["flac"]["VORBIS_COMMENT"]["comments"];
        }
        if (!empty($info["flac"]["VORBIS_COMMENT"]["vendor"])) {
            $info["audio"]["encoder"] = str_replace(
                "reference ",
                "",
        ...
        if (isset($info["flac"]["STREAMINFO"]["audio_signature"])) {
            if (
                $info["flac"]["STREAMINFO"]["audio_signature"] ===
                str_repeat("\x00", 16)
            ) {
                $this->warning(
    ...
    {
        $streaminfo = [];
        $streaminfo["min_block_size"] = getid3_lib::BigEndian2Int(
            substr($BlockData, 0, 2)
        );
        $streaminfo["max_block_size"] = getid3_lib::BigEndian2Int(
            substr($BlockData, 2, 2)
        );
        $streaminfo["min_frame_size"] = getid3_lib::BigEndian2Int(
            substr($BlockData, 4, 3)
        );
        $streaminfo["max_frame_size"] = getid3_lib::BigEndian2Int(
            substr($BlockData, 7, 3)
        );

        $SRCSBSS = getid3_lib::BigEndian2Bin(substr($BlockData, 10, 8));
        $streaminfo["sample_rate"] = getid3_lib::Bin2Dec(
            substr($SRCSBSS, 0, 20)
        );
        $streaminfo["channels"] =
            getid3_lib::Bin2Dec(substr($SRCSBSS, 20, 3)) + 1;
        $streaminfo["bits_per_sample"] =
            getid3_lib::Bin2Dec(substr($SRCSBSS, 23, 5)) + 1;
        $streaminfo["samples_stream"] = getid3_lib::Bin2Dec(
            substr($SRCSBSS, 28, 36)
        );
    ...
    private function parseAPPLICATION($BlockData)
    {
        ...
        $info["flac"]["APPLICATION"][$ApplicationID][
            "name"
        ] = self::applicationIDLookup($ApplicationID);
        $info["flac"]["APPLICATION"][$ApplicationID]["data"] = substr(
            $BlockData,
            4
        );
    ...
    private function parseSEEKTABLE($BlockData)
    {
        ...
        while ($offset < $BlockLength) {
            ...
            if ($SampleNumberString == $placeholderpattern) {
                // placeholder point
                getid3_lib::safe_inc(
                    $info["flac"]["SEEKTABLE"]["placeholders"],
                    1
                );
                $offset += 10;
            } else {
                $SampleNumber = getid3_lib::BigEndian2Int($SampleNumberString);
                $info["flac"]["SEEKTABLE"][$SampleNumber][
                    "offset"
    ...
    private function parseCUESHEET($BlockData)
    {
        $info = &$this->getid3->info;
        $offset = 0;
        $info["flac"]["CUESHEET"]["media_catalog_number"] = trim(
            substr($BlockData, $offset, 128),
            "\0"
        );
        $offset += 128;
        $info["flac"]["CUESHEET"][
        ...
        $info["flac"]["CUESHEET"]["flags"]["is_cd"] =
            (bool) (getid3_lib::BigEndian2Int(substr($BlockData, $offset, 1)) &
                0x80);
        $offset += 1;

        $offset += 258; // reserved

        $info["flac"]["CUESHEET"]["number_tracks"] = getid3_lib::BigEndian2Int(
            substr($BlockData, $offset, 1)
        );
        $offset += 1;

        for (
            $track = 0;
            $track < $info["flac"]["CUESHEET"]["number_tracks"];
            $track++
        ) {
            $TrackSampleOffset = getid3_lib::BigEndian2Int(
                substr($BlockData, $offset, 8)
            );
            $offset += 8;
            $TrackNumber = getid3_lib::BigEndian2Int(
                substr($BlockData, $offset, 1)
            );
            $offset += 1;
            ...
            $info["flac"]["CUESHEET"]["tracks"][$TrackNumber]["isrc"] = substr(
                $BlockData,
                $offset,
                12
            );
            ...
            $TrackFlagsRaw = getid3_lib::BigEndian2Int(
                substr($BlockData, $offset, 1)
            );
            $offset += 1;
            $info["flac"]["CUESHEET"]["tracks"][$TrackNumber]["flags"][
                "is_audio"
            ] = (bool) ($TrackFlagsRaw & 0x80);
            $info["flac"]["CUESHEET"]["tracks"][$TrackNumber]["flags"][
                "pre_emphasis"
            ] = (bool) ($TrackFlagsRaw & 0x40);
            ...
            for (
                ...
                $IndexSampleOffset = getid3_lib::BigEndian2Int(
                    substr($BlockData, $offset, 8)
                );
                $offset += 8;
                $IndexNumber = getid3_lib::BigEndian2Int(
                    substr($BlockData, $offset, 1)
                );
                $offset += 1;
                ...
                $info["flac"]["CUESHEET"]["tracks"][$TrackNumber]["indexes"][
                    $IndexNumber
                ] = $IndexSampleOffset;
            }
        }
    ...
    public function parsePICTURE()
    {
        ...
        $picture["image_mime"] = $this->fread(
            getid3_lib::BigEndian2Int($this->fread(4))
        );
        ...
        if ($picture["image_mime"] == "-->") {
            ...
            $picture["data"] = $this->saveAttachment(
                str_replace("/", "_", $picture["picturetype"]) .
                    "_" .
                    $this->ftell(),
                $this->ftell(),
                $picture["datalength"],
    ...
    public static function applicationIDLookup($applicationid)
    {
        ...
        return isset($lookup[$applicationid])
