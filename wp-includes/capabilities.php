    ...
    switch ($cap) {
        ...
        case "delete_page":
            ...
            if ((int) get_option("wp_page_for_privacy_policy") === $post->ID) {
                $caps = array_merge(
                    $caps,
                    map_meta_cap("manage_privacy_options", $user_id)
                );
            }
        ...
        case "assign_term":
            ...
            if (
                "delete_term" === $cap &&
                ((int) get_option("default_" . $term->taxonomy) ===
                    $term->term_id ||
                    (int) get_option("default_term_" . $term->taxonomy) ===
                        $term->term_id)
            ) {
                $caps[] = "do_not_allow";
                break;
        ...
        case "edit_block_binding":
            ...
            if (!isset($object_id)) {
                if (
                    !isset($block_editor_context->name) ||
            ...
            if (!isset($post_type_object->capabilities)) {
                $post_type_object->capabilities = [];
            }
            $post_type_capabilities = get_post_type_capabilities(
                $post_type_object
            );
            $caps = map_meta_cap(
                $post_type_capabilities->edit_post,
                $user_id,
                $object_id
            );
            break;
...
{
    if (
        !empty($allcaps["update_core"]) ||
        !empty($allcaps["install_plugins"]) ||
        !empty($allcaps["install_themes"])
    ) {
