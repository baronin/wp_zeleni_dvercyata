...
final class WP_Customize_Widgets
{
    ...
    public function __construct($manager)
    {
        ...
        add_filter(
            ...
            [$this, "filter_customize_dynamic_setting_args"],
            10,
            2
        );
        ...
        add_filter(
            "is_active_sidebar",
            [$this, "tally_sidebars_via_is_active_sidebar_calls"],
            10,
            2
        );
        add_filter(
            "dynamic_sidebar_has_widgets",
            [$this, "tally_sidebars_via_dynamic_sidebar_calls"],
            10,
            2
        );
        ...
        if (!isset($this->selective_refreshable_widgets)) {
            ...
            foreach ($wp_widget_factory->widgets as $wp_widget) {
                $this->selective_refreshable_widgets[
                    ...
                    $wp_widget->widget_options["customize_selective_refresh"]
                );
            }
        }
    ...
    {
        $widget_setting_ids = [];
        $incoming_setting_ids = array_keys(
        ...
        if (
            ...
            $widget_setting_ids[] = $this->get_setting_id(
                wp_unslash($_REQUEST["widget-id"])
            );
        }
     ...
     * @return array
     */
    public function filter_customize_value_old_sidebars_widgets_data(
     ...
     * @return array
     */
    public function filter_option_sidebars_widgets_for_theme_switch(
        $sidebars_widgets
    ) {
    ...
    public function customize_register()
    {
        ...
        foreach ($sidebars_widgets as $sidebar_id => $sidebar_widget_ids) {
            ...
            $is_active_sidebar =
                $is_registered_sidebar && !$is_inactive_widgets;
            ...
            if ($is_registered_sidebar || $is_inactive_widgets) {
                ...
                if ($is_active_sidebar) {
                    ...
                    $section = new WP_Customize_Sidebar_Section(
                        $this->manager,
                        $section_id,
                        $section_args
                    );
            ...
            if (!$use_widgets_block_editor) {
                ...
                foreach ($sidebar_widget_ids as $i => $widget_id) {
                    ...
                    $setting_id = $this->get_setting_id($widget_id);
                    $id_base =
                        $wp_registered_widget_controls[$widget_id]["id_base"];
                    ...
                    $control = new WP_Widget_Form_Customize_Control(
                        ...
                        [
                            ...
                            "priority" => $i,
                            "width" =>
                                $wp_registered_widget_controls[$widget_id][
                                    "width"
                                ],
                            "height" =>
                                $wp_registered_widget_controls[$widget_id][
    ...
    public function enqueue_scripts()
    {
        ...
        for (
            ...
            $non_rendered_count < $registered_sidebar_count;
            $non_rendered_count++
        ) {
            $some_non_rendered_areas_messages[
                $non_rendered_count
        ...
        $wp_scripts->add_data(
            ...
            "data",
            sprintf(
                "var _wpCustomizeWidgetsSettings = %s;",
        ...
        if (wp_use_widgets_block_editor()) {
            ...
            if (!empty($registered_sources)) {
                ...
                $script = sprintf(
                    ...
                    wp_json_encode($filtered_sources)
                );
            ...
            wp_add_inline_script(
                ...
                sprintf(
                    ...
                    wp_json_encode(get_block_categories($block_editor_context))
                ),
                "after"
            );
    ...
    public function output_widget_control_templates()
    {
     ...
     printf(
         __("Customizing &#9656; %s"),
         esc_html($this->manager->get_panel("widgets")->title)
     ); ?>
					</span>
					<?php _e("Add a Widget"); ?>
				</h3>
     ...
     _e("The search results will be updated as you type."); ?>
			...
			<?php foreach ($this->get_available_widgets() as $available_widget): ?>
				<div id="widget-tpl-<?php echo esc_attr(
        $available_widget["id"]
    ); ?>" data-widget-id="<?php echo esc_attr(
    $available_widget["id"]
); ?>" class="widget-tpl <?php echo esc_attr(
    $available_widget["id"]
); ?>" tabindex="0">
    ...
    public function get_setting_args($id, $overrides = [])
    {
        ...
        if (
            preg_match(
                $this->setting_id_patterns["sidebar_widgets"],
                $id,
                $matches
            )
        ) {
            ...
            $args["transport"] = current_theme_supports(
            ...
            preg_match(
                $this->setting_id_patterns["widget_instance"],
                $id,
                $matches
            )
        ) {
            ...
            $args["transport"] = $this->is_widget_selective_refreshable(
                $matches["id_base"]
            )
                ? "postMessage"
                : "refresh";
    ...
    public function sanitize_sidebar_widgets($widget_ids)
    {
        ...
        foreach ($widget_ids as $widget_id) {
            $sanitized_widget_ids[] = preg_replace(
                "/[^a-z0-9_\-]/",
                "",
                $widget_id
            );
    ...
    public function get_available_widgets()
    {
        ...
        foreach ($sort as $widget) {
            ...
            $control_tpl = $this->get_widget_control(
                $list_widget_controls_args
            );

            // The properties here are mapped to the Backbone Widget model.
            $available_widget = array_merge($available_widget, [
                "temp_id" => isset($args["_temp_id"])
                    ? $args["_temp_id"]
                    : null,
                "is_multi" => $is_multi_widget,
                "control_tpl" => $control_tpl,
                "multi_number" =>
                    "multi" === $args["_add"] ? $args["_multi_num"] : false,
                "is_disabled" => $is_disabled,
                "id_base" => $id_base,
                "transport" => $this->is_widget_selective_refreshable($id_base)
                    ? "postMessage"
                    : "refresh",
                "width" =>
                    $wp_registered_widget_controls[$widget["id"]]["width"],
                "height" =>
                    $wp_registered_widget_controls[$widget["id"]]["height"],
    ...
    public function get_widget_control_parts($args)
    {
        ...
        $content_start_pos = strpos(
            $control_markup,
            $args[0]["before_widget_content"]
        );
        $content_end_pos = strrpos(
            $control_markup,
        ...
        $content = trim(
            substr(
                ...
                $content_end_pos -
                    $content_start_pos -
                    strlen($args[0]["before_widget_content"])
            )
        );
     ...
     */
    public function should_load_block_editor_scripts_and_styles(
    ...
    public function export_preview_data()
    {
        ...
        $settings = [
            "renderedSidebars" => array_fill_keys(
                array_keys($rendered_sidebars),
                true
            ),
            "renderedWidgets" => array_fill_keys(
                array_keys($rendered_widgets),
                true
            ),
            "registeredSidebars" => array_values($wp_registered_sidebars),
            "registeredWidgets" => $wp_registered_widgets,
        ...
        wp_print_inline_script_tag(
            sprintf(
                "var _wpWidgetCustomizerPreviewSettings = %s;",
                wp_json_encode($settings)
            )
        );
    }
     ...
     */
    public function tally_sidebars_via_is_active_sidebar_calls(
        $is_active,
        $sidebar_id
    ) {
     ...
     */
    public function tally_sidebars_via_dynamic_sidebar_calls(
        $has_widgets,
        $sidebar_id
    ) {
        if (is_registered_sidebar($sidebar_id)) {
    ...
    public function sanitize_widget_instance($value, $id_base = null)
    {
        ...
        if (
            isset($value["raw_instance"]) &&
            $id_base &&
            wp_use_widgets_block_editor()
        ) {
            $widget_object = $wp_widget_factory->get_widget_object($id_base);
            if (
                !empty($widget_object->widget_options["show_instance_in_rest"])
            ) {
                if (
                    "block" === $id_base &&
                     ...
                     */
                    $value["raw_instance"]["content"] = wp_kses_post(
                        $value["raw_instance"]["content"]
                    );
                }
        ...
        if (
            !hash_equals(
                $this->get_instance_hash_key($decoded),
    ...
    public function sanitize_widget_js_instance($value, $id_base = null)
    {
        ...
        if (empty($value["is_widget_customizer_js_value"])) {
            ...
            $js_value = [
                ...
                "instance_hash_key" => $this->get_instance_hash_key(
                    $serialized
                ),
            ];

            if ($id_base && wp_use_widgets_block_editor()) {
                $widget_object = $wp_widget_factory->get_widget_object(
                    $id_base
                );
                if (
                    !empty(
                        $widget_object->widget_options["show_instance_in_rest"]
                    )
                ) {
    ...
    public function sanitize_sidebar_widgets_js_instance($widget_ids)
    {
        ...
        $widget_ids = array_values(
            array_intersect($widget_ids, array_keys($wp_registered_widgets))
        );
        return $widget_ids;
    }
    ...
    public function call_widget_update($widget_id)
    {
        ...
        if (!empty($_POST["sanitized_widget_setting"])) {
            $sanitized_widget_setting = json_decode(
                $this->get_post_value("sanitized_widget_setting"),
                true
            );
            ...
            $instance = $this->sanitize_widget_instance(
        ...
        $this->manager->set_post_value(
            $setting_id,
            $this->sanitize_widget_js_instance($instance, $parsed_id["id_base"])
        );
    ...
    public function wp_ajax_update_widget()
    {
        ...
        $instance = $this->sanitize_widget_js_instance(
            $updated_widget["instance"],
            $id_base
        );
    ...
    public function filter_dynamic_sidebar_params($params)
    {
        ...
        $is_valid =
            ...
            (isset($this->current_dynamic_sidebar_id_stack[0]) &&
                $this->current_dynamic_sidebar_id_stack[0] ===
                    $sidebar_args["id"]) &&
            preg_match(
                "#^<(?P<tag_name>\w+)#",
        ...
        if (isset($this->context_sidebar_instance_number)) {
            ...
            isset($sidebar_args["id"]) &&
            isset($this->sidebar_instance_count[$sidebar_args["id"]])
        ) {
            $context["sidebar_instance_number"] =
                $this->sidebar_instance_count[$sidebar_args["id"]];
        }

        $attributes = sprintf(
            ' data-customize-partial-id="%s"',
            esc_attr("widget[" . $sidebar_args["widget_id"] . "]")
        );
        ...
        $attributes .= sprintf(
            ' data-customize-widget-id="%s"',
            esc_attr($sidebar_args["widget_id"])
        );
        $sidebar_args["before_widget"] = preg_replace(
            "#^(<\w+)#",
            '$1 ' . $attributes,
            $sidebar_args["before_widget"]
        );
    ...
    public function start_dynamic_sidebar($index)
    {
        ...
        if (!$this->manager->selective_refresh->is_render_partials_request()) {
            printf(
                ...
                esc_html($index),
                (int) $this->sidebar_instance_count[$index]
            );
        }
    }
     ...
     */
    public function filter_sidebars_widgets_for_rendering_widget(
        $sidebars_widgets
    ) {
        $sidebars_widgets[$this->rendering_sidebar_id] = [
            $this->rendering_widget_id,
        ];
        return $sidebars_widgets;
    }
    ...
    public function render_widget_partial($partial, $context)
    {
        ...
        if (isset($context["sidebar_instance_number"])) {
            $this->context_sidebar_instance_number =
                (int) $context["sidebar_instance_number"];
        }
    ...
    protected function start_capturing_option_updates()
    {
        ...
        add_filter(
            "pre_update_option",
            [$this, "capture_filter_pre_update_option"],
            10,
            3
        );
    }
     ...
     */
    public function capture_filter_pre_update_option(
        $new_value,
        $option_name,
        $old_value
    ) {
