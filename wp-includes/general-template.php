...
function get_search_form($args = [])
{
    ...
    $format = current_theme_supports("html5", "search-form")
        ? "html5"
        : "xhtml";
...
function wp_lostpassword_url($redirect = "")
{
    ...
    $lostpassword_url = add_query_arg(
        $args,
        network_site_url($wp_login_path, "login")
    );
...
function get_site_icon_url($size = 512, $url = "", $blog_id = 0)
{
    ...
    if (
        is_multisite() &&
        !empty($blog_id) &&
        get_current_blog_id() !== (int) $blog_id
    ) {
...
function has_custom_logo($blog_id = 0)
{
    ...
    $is_image = $custom_logo_id
        ? wp_attachment_is_image($custom_logo_id)
        : false;
...
function get_custom_logo($blog_id = 0)
{
    ...
    if (has_custom_logo()) {
         ...
         */
        $custom_logo_attr = apply_filters(
    ...
    if ("monthly" === $parsed_args["type"]) {
        ...
        if ($results) {
            ...
            foreach ((array) $results as $result) {
                ...
                $text = sprintf(
                    __('%1$s %2$d'),
                    $wp_locale->get_month($result->month),
                    $result->year
                );
                ...
                $selected =
                    is_archive() &&
                    (string) $parsed_args["year"] === $result->year &&
                    (string) $parsed_args["monthnum"] === $result->month;
        ...
        if ($results) {
            ...
            foreach ((array) $results as $result) {
                ...
                $selected =
                    ...
                    (string) $parsed_args["year"] === $result->year &&
                    (string) $parsed_args["monthnum"] === $result->month &&
                    (string) $parsed_args["day"] === $result->dayofmonth;
                $output .= get_archives_link(
                    $url,
                    $text,
                    $parsed_args["format"],
        ...
        if ($results) {
            ...
            foreach ((array) $results as $result) {
                if ($result->week !== $arc_w_last) {
                    ...
                    $arc_week = get_weekstartend(
                        $result->yyyymmdd,
                        get_option("start_of_week")
                    );
                    $arc_week_start = date_i18n(
                    ...
                    $selected =
                        is_archive() &&
                        (string) $parsed_args["year"] === $result->yr &&
                        (string) $parsed_args["w"] === $result->week;
                    $output .= get_archives_link(
                        $url,
                        $text,
                        $parsed_args["format"],
        ...
        "postbypost" === $parsed_args["type"] ||
        "alpha" === $parsed_args["type"]
    ) {
        $orderby =
            "alpha" === $parsed_args["type"]
                ? "post_title ASC "
...
function get_calendar($args = [])
{
    ...
    for ($day = 1; $day <= $daysinmonth; ++$day) {
        ...
        if (in_array($day, $daywithpost, true)) {
            ...
            $date_format = gmdate(
                ...
                strtotime("{$thisyear}-{$thismonth}-{$day}")
            );
        ...
        if (
            6 ===
            (int) calendar_week_mod(
                (int) gmdate(
                    "w",
                    mktime(0, 0, 0, $thismonth, $day, $thisyear)
                ) - $week_begins
            )
        ) {
            $newrow = true;
    ...
    $pad =
        7 -
        calendar_week_mod(
            (int) gmdate("w", mktime(0, 0, 0, $thismonth, $day, $thisyear)) -
                $week_begins
        );
    if (0 < $pad && $pad < 7) {
        $calendar_output .=
...
function the_modified_date(
     ...
     * @param string $after             HTML output after the date.
     */
    $the_modified_date = apply_filters(
        "the_modified_date",
        $the_modified_date,
        $format,
        $before,
...
function the_weekday_date($before = "", $after = "")
{
    ...
    if ($currentday !== $previousweekday) {
        ...
        $the_weekday_date .= $wp_locale->get_weekday(
            get_post_time("w", false, $post)
        );
...
function feed_links_extra($args = [])
{
    ...
    if (is_singular()) {
        ...
        /** This filter is documented in wp-includes/general-template.php */
        $show_comments_feed = apply_filters(
         ...
         */
        $show_post_comments_feed = apply_filters(
            "feed_links_extra_show_post_comments_feed",
            $show_comments_feed
        );

        if (
            $show_post_comments_feed &&
         ...
         */
        $show_post_type_archive_feed = apply_filters(
         ...
         */
        $show_category_feed = apply_filters(
         ...
         */
        $show_author_feed = apply_filters(
         ...
         */
        $show_search_feed = apply_filters(
...
function wp_resource_hints()
{
    ...
    foreach ($hints as $relation_type => $urls) {
        ...
        foreach ($urls as $key => $url) {
            ...
            if (
                in_array($relation_type, ["preconnect", "dns-prefetch"], true)
            ) {
...
function wp_preload_resources()
{
    ...
    foreach ($unique_resources as $unique_resource) {
        ...
        foreach ($unique_resource as $resource_key => $resource_value) {
            ...
            if (
                !in_array($resource_key, $non_supported_attributes, true) &&
            ...
            if (
                "imagesrcset" === $resource_key &&
                (!isset($unique_resource["as"]) ||
            ...
            if (
                "imagesizes" === $resource_key &&
                (!isset($unique_resource["as"]) ||
                    "image" !== $unique_resource["as"] ||
                    !isset($unique_resource["imagesrcset"]))
            ) {
                continue;
            }
            ...
            $resource_value =
                "href" === $resource_key
                    ? esc_url($resource_value, ["http", "https"])
...
function wp_dependencies_unique_hosts()
{
    ...
    foreach ([$wp_scripts, $wp_styles] as $dependencies) {
        if (
            $dependencies instanceof WP_Dependencies &&
            !empty($dependencies->queue)
        ) {
            foreach ($dependencies->queue as $handle) {
                ...
                if (
                    !empty($parsed["host"]) &&
                    !in_array($parsed["host"], $unique_hosts, true) &&
...
function wp_enqueue_code_editor($args)
{
    ...
    wp_add_inline_script(
        "code-editor",
        sprintf(
            "jQuery.extend( wp.codeEditor.defaultSettings, %s );",
...
function wp_get_code_editor_settings($args)
{
    ...
    if (isset($args["type"])) {
        ...
        isset($args["file"]) &&
        str_contains(basename($args["file"]), ".")
    ) {
    ...
    if (
        in_array(
            $type,
            ["text/css", "text/x-scss", "text/x-less", "text/x-sass"],
            true
        )
    ) {
...
function paginate_links($args = "")
{
    ...
    if (isset($url_parts[1])) {
        ...
        $format = explode(
            "?",
            str_replace("%_%", $args["format"], $args["base"])
        );
...
function get_the_generator($type = "")
{
    ...
    switch ($type) {
        ...
        case "atom":
            $gen =
                '<generator uri="https://wordpress.org/" version="' .
                esc_attr(get_bloginfo_rss("version")) .
                '">WordPress</generator>';
            break;
        case "rss2":
            $gen =
                "<generator>" .
                sanitize_url(
                    "https://wordpress.org/?v=" . get_bloginfo_rss("version")
                ) .
                "</generator>";
            break;
        case "rdf":
...
{
    return __checked_selected_helper(
        $readonly_value,
        $current,
        $display,
        "readonly"
