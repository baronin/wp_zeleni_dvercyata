    ...
    if ($block_template) {
        ...
        if (empty($block_template->content)) {
            if (is_user_logged_in()) {
                $_wp_current_template_content = wp_render_empty_block_template_warning(
                    $block_template
                );
            } else {
                if ($block_template->has_theme_file) {
                    // Show contents from theme template if user is not logged in.
                    $theme_template = _get_block_template_file(
                        "wp_template",
                        $block_template->slug
                    );
                    $_wp_current_template_content = file_get_contents(
                        $theme_template["path"]
                    );
                } else {
        ...
        if ("index" === $type) {
            if (isset($_GET["_wp-find-template"])) {
                wp_send_json_error([
                    "message" => __("No matching template found."),
                ]);
            }
        } else {
...
function resolve_block_template(
    ...
    usort($templates, static function ($template_a, $template_b) use (
        $slug_priorities
    ) {
        return $slug_priorities[$template_a->slug] -
    ...
    if (
        ...
        $fallback_template_slug = substr(
            ...
            strpos($fallback_template, $theme_base_path) +
                strlen($theme_base_path),
            // Remove '.php' suffix.
            -4
        );
...
function _resolve_template_for_new_post($wp_query)
{
    ...
    // Pages.
    $page_id = isset($wp_query->query["page_id"])
        ? $wp_query->query["page_id"]
        : null;
...
{
    return WP_Block_Templates_Registry::get_instance()->register(
        $template_name,
        $args
    );
}
...
{
    return WP_Block_Templates_Registry::get_instance()->unregister(
        $template_name
    );
}
