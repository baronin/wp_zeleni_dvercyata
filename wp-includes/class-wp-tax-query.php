...
class WP_Tax_Query
{
        ...
        foreach ($queries as $key => $query) {
            if ("relation" === $key) {
                ...
                if (
                    ...
                    "NOT IN" !== $cleaned_clause["operator"]
                ) {
                     ...
                     */
                    if (
                        !empty($cleaned_clause["terms"]) &&
                        !isset($this->queried_terms[$taxonomy]["terms"])
                    ) {
                        $this->queried_terms[$taxonomy]["terms"] =
                            $cleaned_clause["terms"];
                    }

                    if (
                        !empty($cleaned_clause["field"]) &&
                        !isset($this->queried_terms[$taxonomy]["field"])
                    ) {
                        $this->queried_terms[$taxonomy]["field"] =
                            $cleaned_clause["field"];
                    }
                }
        ...
        if (
            ...
            foreach ($query["terms"] as $term) {
                $children = array_merge(
                    $children,
                    get_term_children($term, $query["taxonomy"])
                );
                $children[] = $term;
            }
    ...
    public function transform_query(&$query, $resulting_field)
    {
        ...
        if (
            "AND" === $query["operator"] &&
            count($term_list) < count($query["terms"])
        ) {
