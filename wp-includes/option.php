...
function wp_prime_option_caches($options)
{
    ...
    foreach ($options as $option) {
        if (
            (!isset($cached_options[$option]) ||
                false === $cached_options[$option]) &&
    ...
    $options_not_found = array_diff(
        $options_to_prime,
        array_keys($options_found)
    );
...
function wp_set_option_autoload_values(array $options)
{
    ...
    $options_to_update = $wpdb->get_col(
        $wpdb->prepare(
    ...
    foreach ($grouped_options as $autoload => $options) {
        ...
        $success = $wpdb->query(
            $wpdb->prepare(
                "UPDATE $wpdb->options SET autoload = %s WHERE option_name IN (" .
                    implode(
                        ",",
                        array_fill(0, count($grouped_options[$autoload]), "%s")
                    ) .
                    ")",
                array_merge([$autoload], $grouped_options[$autoload])
            )
        );
        if (!$success) {
    ...
    if (!$alloptions) {
        ...
        $alloptions_db = $wpdb->get_results(
            "SELECT option_name, option_value FROM $wpdb->options WHERE autoload IN ( '" .
                implode("', '", esc_sql(wp_autoload_values_to_autoload())) .
                "' )"
        );
...
function wp_prime_network_option_caches($network_id, array $options)
{
    ...
    $cached_options = wp_cache_get_multiple(
    ...
    foreach ($cache_keys as $option => $cache_key) {
        if (
            (!isset($cached_options[$cache_key]) ||
                false === $cached_options[$cache_key]) &&
            !isset($notoptions[$option])
        ) {
            $options_to_prime[] = $option;
        }
...
function update_option($option, $value, $autoload = null)
{
    ...
    if (
        $value === $old_value ||
        maybe_serialize($value) === maybe_serialize($old_value)
    ) {
        return false;
    }
    ...
    if (null !== $autoload) {
        $update_args["autoload"] = wp_determine_option_autoload_value(
            $option,
            $value,
            $serialized_value,
            $autoload
...
function wp_filter_default_autoload_value_via_option_size(
     ...
     * @param string $option          The name of the option.
     */
    $max_option_size = (int) apply_filters(
...
function get_user_setting($name, $default_value = false)
{
    ...
    return isset($all_user_settings[$name])
...
function unregister_setting($option_group, $option_name, $deprecated = "")
{
    ...
    if (isset($new_allowed_options[$option_group])) {
        $pos = array_search(
            $option_name,
            (array) $new_allowed_options[$option_group],
            true
        );
    }
    ...
    if (isset($wp_registered_settings[$option_name])) {
        ...
        if (
            !empty($wp_registered_settings[$option_name]["sanitize_callback"])
        ) {
            remove_filter(
                "sanitize_option_{$option_name}",
                $wp_registered_settings[$option_name]["sanitize_callback"]
            );
        }

        // Remove the default filter if a default was provided during registration.
        if (
            array_key_exists("default", $wp_registered_settings[$option_name])
        ) {
            remove_filter(
                "default_option_{$option_name}",
                "filter_default_option",
