...
class WP_Textdomain_Registry
{
    ...
    public function init()
    {
        add_action(
            ...
            [$this, "invalidate_mo_files_cache"],
            10,
            2
        );
    }
    ...
    {
        $path =
            $this->all[$domain][$locale] ??
            $this->get_path_from_lang_dir($domain, $locale);
    ...
    {
        return isset($this->current[$domain]) ||
    ...
    public function invalidate_mo_files_cache($upgrader, $hook_extra)
    {
        ...
        $translation_types = array_unique(
            wp_list_pluck($hook_extra["translations"], "type")
        );
        ...
        foreach ($translation_types as $type) {
            switch ($type) {
                case "plugin":
                    wp_cache_delete(
                        md5(WP_LANG_DIR . "/plugins/"),
                        "translation_files"
                    );
                    break;
                case "theme":
                    wp_cache_delete(
                        md5(WP_LANG_DIR . "/themes/"),
                        "translation_files"
                    );
                    break;
                default:
        ...
        foreach ($locations as $location) {
            ...
            foreach ($files as $file_path) {
                if (
                    !in_array(
                        $domain,
                        $this->domains_with_translations,
                        true
                    ) &&
                    str_starts_with(
                        str_replace("$location/", "", $file_path),
                        "$domain-"
                    )
                ) {
