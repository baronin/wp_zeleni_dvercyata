...
abstract class ParagonIE_Sodium_Crypto
{
    ...
    {
        if (
            ParagonIE_Sodium_Core_Util::strlen($sKey) !==
    ...
    public static function generichash_init_salt_personal(
        ...
        if (!empty($salt)) {
            $p = ParagonIE_Sodium_Core_BLAKE2b::stringToSplFixedArray(
                $personal
            );
        } else {
            $p = null;
    ...
    {
        $q = ParagonIE_Sodium_Core_X25519::crypto_scalarmult_curve25519_ref10(
            $sKey,
            $pKey
        );
    ...
    {
        $q = ParagonIE_Sodium_Core_X25519::crypto_scalarmult_curve25519_ref10_base(
            $secret
        );
        self::scalarmult_throw_if_zero($q);
        return $q;
    ...
    public static function secretstream_xchacha20poly1305_rekey(&$state)
    {
        ...
        # }
        $new_key_and_inonce .= ParagonIE_Sodium_Core_Util::substR(
            $st->getNonce(),
            0,
            8
        );
    ...
    {
        return ParagonIE_Sodium_Core_Ed25519::verify_detached(
            $signature,
            $message,
            $pk
        );
