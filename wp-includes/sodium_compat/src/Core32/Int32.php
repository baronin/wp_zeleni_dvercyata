...
class ParagonIE_Sodium_Core32_Int32
{
    ...
    {
        $this->limbs = [(int) $array[0], (int) $array[1]];
        $this->overflow = 0;
    ...
    public function mask($m = 0)
    {
        ...
        return new ParagonIE_Sodium_Core32_Int32(
            [(int) ($this->limbs[0] & $hi), (int) ($this->limbs[1] & $lo)],
            $this->unsignedInt
        );
    }
    ...
    public function mulIntFast($int)
    {
        ...
        // Return our values
        $return = new ParagonIE_Sodium_Core32_Int32();
        $return->limbs = [$res[1] & 0xffff, $res[0] & 0xffff];
        if (count($res) > 2) {
    ...
    public function shiftRight($c = 0)
    {
        ...
        if ($c === 0) {
            ...
            $return->limbs[0] =
                (int) ((($this->limbs[0] >> $c) | ($carryLeft << 16 - $c)) &
                    0xffff);
            $carryRight = (int) ($this->limbs[0] & ((1 << $c + 1) - 1));
            $return->limbs[1] =
                (int) ((($this->limbs[1] >> $c) | ($carryRight << 16 - $c)) &
                    0xffff);
            $return->overflow >>= $c;
        }
        return $return;
    ...
    {
        return (int) ((($this->limbs[0] & 0xffff) << 16) |
            ($this->limbs[1] & 0xffff));
    }
