    ...
    ParagonIE_Sodium_Core32_Util
{
    ...
    public function update($message = "")
    {
        ...
        if ($this->leftover) {
            /** @var int $want */
            $want =
                ParagonIE_Sodium_Core32_Poly1305::BLOCK_SIZE - $this->leftover;
            if ($want > $bytes) {
                $want = $bytes;
            }
            for ($i = 0; $i < $want; ++$i) {
            ...
            $this->leftover += $want;
            if (
                $this->leftover < ParagonIE_Sodium_Core32_Poly1305::BLOCK_SIZE
            ) {
        ...
        if ($bytes >= ParagonIE_Sodium_Core32_Poly1305::BLOCK_SIZE) {
            /** @var int $want */
            $want =
                $bytes & ~(ParagonIE_Sodium_Core32_Poly1305::BLOCK_SIZE - 1);
    ...
    public function blocks($message, $bytes)
    {
        ...
        $hibit = ParagonIE_Sodium_Core32_Int32::fromInt(
            (int) ($this->final ? 0 : 1 << 24)
        ); /* 1 << 128 */
