...
class ParagonIE_Sodium_Core32_Int64
{
    ...
    public static function ctSelect(
        ...
        return [$a->xorInt64($x)->mulInt($m), $b->xorInt64($x)->mulInt($m)];
    }
    ...
    public function multiplyLong(array $a, array $b, $baseLog2 = 16)
    {
        ...
        for ($i = 0; $i < $a_l; ++$i) {
            ...
            for ($j = 0; $j < $a_l; ++$j) {
                ...
                $r[$i + $j] =
                    ((int) $product - (int) ($carry * $base)) & 0xffff;
                $r[$i + $j + 1] += $carry;
            }
        }
    ...
    public function rotateRight($c = 0)
    {
        ...
        if ($c === 0) {
            ...
            for ($i = 3; $i >= 0; --$i) {
                ...
                $k = ($i - $idx_shift - 1) & 3;
                $limbs[$i] =
                    (int) ((((int) $myLimbs[$j] >> (int) $sub_shift) |
    ...
    public function shiftLeft($c = 0)
    {
        ...
        if ($c >= 16) {
            if ($c >= 48) {
                $return->limbs = [$this->limbs[3], 0, 0, 0];
            } elseif ($c >= 32) {
                $return->limbs = [$this->limbs[2], $this->limbs[3], 0, 0];
            } else {
                $return->limbs = [
                    $this->limbs[1],
                    $this->limbs[2],
    ...
    {
        return (int) ((($this->limbs[2] & 0xffff) << 16) |
            ($this->limbs[3] & 0xffff));
    }
    ...
    public static function fromString($string)
    {
        ...
        if (ParagonIE_Sodium_Core32_Util::strlen($string) !== 8) {
            throw new RangeException(
                "String must be 8 bytes; " .
                    ParagonIE_Sodium_Core32_Util::strlen($string) .
                    " given."
            );
        }
        $return = new ParagonIE_Sodium_Core32_Int64();
        ...
        $return->limbs[0] =
            (int) ((ParagonIE_Sodium_Core32_Util::chrToInt($string[0]) &
                0xff) <<
                8);
        $return->limbs[0] |=
            ParagonIE_Sodium_Core32_Util::chrToInt($string[1]) & 0xff;
        $return->limbs[1] =
            (int) ((ParagonIE_Sodium_Core32_Util::chrToInt($string[2]) &
                0xff) <<
                8);
        $return->limbs[1] |=
            ParagonIE_Sodium_Core32_Util::chrToInt($string[3]) & 0xff;
        $return->limbs[2] =
            (int) ((ParagonIE_Sodium_Core32_Util::chrToInt($string[4]) &
                0xff) <<
                8);
        $return->limbs[2] |=
            ParagonIE_Sodium_Core32_Util::chrToInt($string[5]) & 0xff;
        $return->limbs[3] =
            (int) ((ParagonIE_Sodium_Core32_Util::chrToInt($string[6]) &
                0xff) <<
                8);
        $return->limbs[3] |=
    ...
    public static function fromReverseString($string)
    {
        ...
        $return->limbs[0] =
            (int) ((ParagonIE_Sodium_Core32_Util::chrToInt($string[7]) &
                0xff) <<
                8);
        $return->limbs[0] |=
            ParagonIE_Sodium_Core32_Util::chrToInt($string[6]) & 0xff;
        $return->limbs[1] =
            (int) ((ParagonIE_Sodium_Core32_Util::chrToInt($string[5]) &
                0xff) <<
                8);
        $return->limbs[1] |=
            ParagonIE_Sodium_Core32_Util::chrToInt($string[4]) & 0xff;
        $return->limbs[2] =
            (int) ((ParagonIE_Sodium_Core32_Util::chrToInt($string[3]) &
                0xff) <<
                8);
        $return->limbs[2] |=
            ParagonIE_Sodium_Core32_Util::chrToInt($string[2]) & 0xff;
        $return->limbs[3] =
            (int) ((ParagonIE_Sodium_Core32_Util::chrToInt($string[1]) &
                0xff) <<
                8);
        $return->limbs[3] |=
    ...
    public function toInt32()
    {
        ...
        $return->unsignedInt = $this->unsignedInt;
        $return->overflow =
            (int) (ParagonIE_Sodium_Core32_Util::abs($this->limbs[1], 16) &
                0xffff);
        return $return;
    }
    ...
    {
        return ParagonIE_Sodium_Core32_Util::intToChr(
            ($this->limbs[0] >> 8) & 0xff
        ) .
