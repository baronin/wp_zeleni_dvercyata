...
abstract class ParagonIE_Sodium_Core_Util
{
    ...
    public static function andStrings($a, $b)
    {
        ...
        if (self::strlen($b) !== $len) {
            throw new SodiumException(
                "Both strings must be of equal length to combine with bitwise AND"
            );
        }
        return $a & $b;
    }
    ...
    public static function chrToInt($chr)
    {
        ...
        if (!is_string($chr)) {
            throw new TypeError(
                "Argument 1 must be a string, " . gettype($chr) . " given."
            );
        }
        if (self::strlen($chr) !== 1) {
            throw new SodiumException(
                "chrToInt() expects a string that is exactly 1 character long"
            );
        }
        /** @var array<int, int> $chunk */
        $chunk = unpack("C", $chr);
    ...
    public static function compare($left, $right, $len = null)
    {
        ...
        while ($i !== 0) {
            --$i;
            $gt |=
                (self::chrToInt($right[$i]) - self::chrToInt($left[$i]) >> 8) &
                $eq;
            $eq &=
                (self::chrToInt($right[$i]) ^ self::chrToInt($left[$i])) - 1 >>
                8;
        }
        return $gt + $gt + $eq - 1;
    }
    ...
    public static function hashEquals($left, $right)
    {
        ...
        if (!is_string($left)) {
            throw new TypeError(
                "Argument 1 must be a string, " . gettype($left) . " given."
            );
        }
        if (!is_string($right)) {
            throw new TypeError(
                "Argument 2 must be a string, " . gettype($right) . " given."
            );
        }
        ...
        while ($hex_pos < $hex_len) {
            ...
            if (($c_num0 | $c_alpha0) === 0) {
                if (
                    $ignore &&
                    $state === 0 &&
                    strpos($ignore, self::intToChr($c)) !== false
                ) {
                    continue;
                }
                throw new RangeException(
    ...
    public static function load_3($string)
    {
        ...
        if (!is_string($string)) {
            throw new TypeError(
                "Argument 1 must be a string, " . gettype($string) . " given."
            );
        }
    ...
    public static function strlen($str)
    {
        ...
        return (int) (self::isMbStringOverride()
            ? mb_strlen($str, "8bit")
            : strlen($str));
    }
