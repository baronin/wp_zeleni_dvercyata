...
class ParagonIE_Sodium_Core_Poly1305_State extends ParagonIE_Sodium_Core_Util
{
    ...
    public function update($message = "")
    {
        ...
        if ($this->leftover) {
            $want =
                ParagonIE_Sodium_Core_Poly1305::BLOCK_SIZE - $this->leftover;
            if ($want > $bytes) {
                $want = $bytes;
            }
            for ($i = 0; $i < $want; ++$i) {
    ...
    public function blocks($message, $bytes)
    {
        ...
        while ($bytes >= ParagonIE_Sodium_Core_Poly1305::BLOCK_SIZE) {
            ...
            $h1 +=
                (self::load_4(self::substr($message, 3, 4)) >> 2) & 0x3ffffff;
            $h2 +=
                (self::load_4(self::substr($message, 6, 4)) >> 4) & 0x3ffffff;
            $h3 +=
                (self::load_4(self::substr($message, 9, 4)) >> 6) & 0x3ffffff;
