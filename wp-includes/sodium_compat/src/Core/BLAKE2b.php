...
abstract class ParagonIE_Sodium_Core_BLAKE2b extends ParagonIE_Sodium_Core_Util
{
    ...
    public static function increment_counter($ctx, $inc)
    {
        if ($inc < 0) {
            throw new SodiumException(
                "Increasing by a negative number makes no sense."
            );
        }
        $t = self::to64($inc);
    ...
    public static function finish(SplFixedArray $ctx, SplFixedArray $out)
    {
        ...
        if ($ctx[4] > 128) {
            ...
            if ($ctx[4] > 128) {
                throw new SodiumException(
                    "Failed to assert that buflen <= 128 bytes"
                );
            }
            for ($i = $ctx[4]; $i--; ) {
                $ctx[3][$i] = $ctx[3][$i + 128];
    ...
    public static function init(
        ...
        if (
            $salt instanceof SplFixedArray ||
            $personal instanceof SplFixedArray
        ) {
