    ...
    ParagonIE_Sodium_Core_Curve25519
{
    ...
    public static function seed_keypair(&$pk, &$sk, $seed)
    {
        if (self::strlen($seed) !== self::SEED_BYTES) {
            throw new RangeException(
                "crypto_sign keypair seed must be 32 bytes long"
            );
        }
    ...
    public static function secretkey($keypair)
    {
        if (self::strlen($keypair) !== self::KEYPAIR_BYTES) {
            throw new RangeException(
                "crypto_sign keypair must be 96 bytes long"
            );
        }
    ...
    public static function verify_detached($sig, $message, $pk)
    {
        ...
        $hDigest = hash(
            "sha512",
            self::substr($sig, 0, 32) . self::substr($pk, 0, 32) . $message,
            true
        );
    ...
    public static function scalar_random()
    {
        do {
            ...
            !self::check_S_lt_L($r) ||
            ParagonIE_Sodium_Compat::is_zero($r)
        );
        return $r;
    }
