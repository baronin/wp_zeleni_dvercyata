...
class ParagonIE_Sodium_Core_AES extends ParagonIE_Sodium_Core_Util
{
     ...
     * @return void
     */
    protected static function processInversion(
    ...
    public static function keySchedule($key)
    {
        ...
        for ($i = 0, $j = 0; $i < $nkf; ++$i, $j += 2) {
            $comp_skey[$i] =
                ($skey[$j] & 0x55555555) | ($skey[$j + 1] & 0xaaaaaaaa);
        }
        return new ParagonIE_Sodium_Core_AES_KeySchedule(
            $comp_skey,
            $num_rounds
        );
    }
    ...
    public static function addRoundKey(
        ...
        for ($j = 0; $j < 8; ++$j) {
            $q[$j] =
                ($q[$j] ^ $block[$j]) & ParagonIE_Sodium_Core_Util::U32_MAX;
        }
    }
    ...
    public static function decryptBlockECB($message, $key)
    {
        if (self::strlen($message) !== 16) {
            throw new SodiumException(
                "decryptBlockECB() expects a 16 byte message"
            );
        }
    ...
    public static function encryptBlockECB($message, $key)
    {
        if (self::strlen($message) !== 16) {
            throw new SodiumException(
                "encryptBlockECB() expects a 16 byte message"
            );
        }
