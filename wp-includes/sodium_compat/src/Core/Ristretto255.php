...
class ParagonIE_Sodium_Core_Ristretto255 extends ParagonIE_Sodium_Core_Ed25519
{
    ...
    public static function ristretto255_sqrt_ratio_m1(
        ...
        $x = self::fe_mul(
            self::fe_mul(self::fe_sq($v3), $u),
            $v
        ); /* x = uv^7 */

        $x = self::fe_mul(
            self::fe_mul(self::fe_pow22523($x) /* x = (uv^7)^((q-5)/8) */, $v3),
            $u
        ); /* x = uv^3(uv^7)^((q-5)/8) */
    ...
    public static function ristretto255_p3_tobytes(
        ...
        $sqrtm1 = ParagonIE_Sodium_Core_Curve25519_Fe::fromArray(self::$sqrtm1);
        $invsqrtamd = ParagonIE_Sodium_Core_Curve25519_Fe::fromArray(
            self::$invsqrtamd
        );
        ...
        return self::fe_tobytes(
            self::fe_abs(self::fe_mul($den_inv, self::fe_sub($h->Z, $y_)))
        );
    }
    ...
    public static function ristretto255_elligator(
        ...
        $sqrtm1 = ParagonIE_Sodium_Core_Curve25519_Fe::fromArray(self::$sqrtm1);
        $onemsqd = ParagonIE_Sodium_Core_Curve25519_Fe::fromArray(
            self::$onemsqd
        );
        $d = ParagonIE_Sodium_Core_Curve25519_Fe::fromArray(self::$d);
        $sqdmone = ParagonIE_Sodium_Core_Curve25519_Fe::fromArray(
            self::$sqdmone
        );
        $sqrtadm1 = ParagonIE_Sodium_Core_Curve25519_Fe::fromArray(
            self::$sqrtadm1
        );
        ...
        $n = self::fe_sub(
            self::fe_mul(self::fe_mul(self::fe_sub($r, $one), $c), $sqdmone),
            $v
        ); /* n =  c*(r-1)*(d-1)^2-v */
    ...
    protected static function _string_to_element($ctx, $msg, $hash_alg)
    {
        return self::ristretto255_from_hash(
            self::h2c_string_to_hash(
                self::crypto_core_ristretto255_HASHBYTES,
                $ctx,
                $msg,
                $hash_alg
            )
    ...
    public static function ristretto255_scalar_from_string(
        ...
        $h_be = self::stringToIntArray(
            self::h2c_string_to_hash(self::HASH_SC_L, $ctx, $msg, $hash_alg)
        );
    ...
    public static function scalarmult_ristretto255($n, $p)
    {
        if (self::strlen($n) !== 32) {
            throw new SodiumException(
                "Scalar must be 32 bytes, " . self::strlen($p) . " given."
            );
        }
        if (self::strlen($p) !== 32) {
            throw new SodiumException(
                "Point must be 32 bytes, " . self::strlen($p) . " given."
            );
        }
        $result = self::ristretto255_frombytes($p);
        if ($result["res"] !== 0) {
