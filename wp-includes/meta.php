...
function delete_metadata(
    ...
    $query = $wpdb->prepare(
        "SELECT $id_column FROM $table WHERE meta_key = %s",
        $meta_key
    );
    ...
    if ($delete_all) {
        if (
            ...
            $object_ids = $wpdb->get_col(
                $wpdb->prepare(
                    "SELECT $type_column FROM $table WHERE meta_key = %s AND meta_value = %s",
                    $meta_key,
                    $meta_value
                )
            );
...
function get_metadata_by_mid($meta_type, $meta_id)
{
    ...
    $meta = $wpdb->get_row(
        $wpdb->prepare("SELECT * FROM $table WHERE $id_column = %d", $meta_id)
    );
...
function update_meta_cache($meta_type, $object_ids)
{
    ...
    if (!empty($meta_list)) {
        foreach ($meta_list as $metarow) {
            ...
            if (
                !isset($cache[$mpid][$mkey]) ||
                !is_array($cache[$mpid][$mkey])
            ) {
                $cache[$mpid][$mkey] = [];
            }
...
function register_meta($object_type, $meta_key, $args, $deprecated = null)
{
    ...
    $object_subtype = !empty($args["object_subtype"])
        ? $args["object_subtype"]
        : "";
    if ($args["revisions_enabled"]) {
        if ("post" !== $object_type) {
            ...
            !empty($object_subtype) &&
            !post_type_supports($object_subtype, "revisions")
        ) {
            _doing_it_wrong(
                __FUNCTION__,
                __(
    ...
    if (array_key_exists("default", $args)) {
        ...
        if (
            is_array($args["show_in_rest"]) &&
            isset($args["show_in_rest"]["schema"])
        ) {
        ...
        if (
            !has_filter(
                "default_{$object_type}_metadata",
...
function get_registered_meta_keys($object_type, $object_subtype = "")
{
    ...
    if (
        !is_array($wp_meta_keys) ||
        !isset($wp_meta_keys[$object_type]) ||
        !isset($wp_meta_keys[$object_type][$object_subtype])
    ) {
        return [];
    }
