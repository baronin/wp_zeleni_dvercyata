...
final class WP_Block_Patterns_Registry
{
    ...
    public function register($pattern_name, $pattern_properties)
    {
        ...
        if (
            !isset($pattern_properties["title"]) ||
            !is_string($pattern_properties["title"])
        ) {
            _doing_it_wrong(
                __METHOD__,
        ...
        if (!isset($pattern_properties["filePath"])) {
            if (
                !isset($pattern_properties["content"]) ||
                !is_string($pattern_properties["content"])
            ) {
                _doing_it_wrong(
                    __METHOD__,
    ...
    private function get_content($pattern_name, $outside_init_only = false)
    {
        ...
        if (
            ...
            isset($patterns[$pattern_name]["filePath"])
        ) {
            ob_start();
...
{
    return WP_Block_Patterns_Registry::get_instance()->register(
        $pattern_name,
        $pattern_properties
    );
}
...
{
    return WP_Block_Patterns_Registry::get_instance()->unregister(
        $pattern_name
    );
}
