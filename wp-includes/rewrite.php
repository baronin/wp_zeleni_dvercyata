...
{
    if (
        !isset($query_vars["year"]) &&
        !isset($query_vars["monthnum"]) &&
    ...
    $permastructs = array_values(
        array_filter(explode("/", get_option("permalink_structure")))
    );
    ...
    if (
        0 === $postname_index &&
        (isset($query_vars["year"]) || isset($query_vars["monthnum"]))
    ) {
        $compare = "year";
    } elseif (
        $postname_index &&
    ...
    if (
        preg_match("/^([0-9]{4})\-([0-9]{2})/", $post->post_date, $matches) &&
        isset($query_vars["year"]) &&
        ("monthnum" === $compare || "day" === $compare)
    ) {
        ...
        if (
            "day" === $compare &&
            isset($query_vars["monthnum"]) &&
            (int) $query_vars["monthnum"] !== (int) $matches[2]
        ) {
            return $query_vars;
        }
    }
...
function url_to_postid($url)
{
    ...
    if (str_contains(trailingslashit($url), home_url("/"))) {
        ...
        $url = preg_replace(
            sprintf("#^%s#", preg_quote($home_path)),
            "",
            trailingslashit($url)
        );
    }
