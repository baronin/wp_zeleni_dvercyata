...
function get_lastcommentmodified($timezone = "server")
{
    ...
    switch ($timezone) {
        case "gmt":
            $comment_modified_date = $wpdb->get_var(
        ...
        case "server":
            ...
            $comment_modified_date = $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT DATE_ADD(comment_date_gmt, INTERVAL %s SECOND) FROM $wpdb->comments WHERE comment_approved = '1' ORDER BY comment_date_gmt DESC LIMIT 1",
                    $add_seconds_server
                )
            );
            break;
...
function get_comment_count($post_id = 0)
{
    ...
    foreach ($mapping as $key => $value) {
        $comment_count[$key] = get_comments(
            array_merge($args, ["status" => $value])
        );
    }

    $comment_count["all"] =
        $comment_count["approved"] + $comment_count["awaiting_moderation"];
    $comment_count["total_comments"] =
        $comment_count["all"] + $comment_count["spam"];
...
function wp_set_comment_cookies($comment, $user, $cookies_consent = true)
{
    ...
    $comment_cookie_lifetime =
        time() + apply_filters("comment_cookie_lifetime", YEAR_IN_SECONDS);
...
function sanitize_comment_cookies()
{
    ...
    if (isset($_COOKIE["comment_author_email_" . COOKIEHASH])) {
         ...
         */
        $comment_author_email = apply_filters(
...
function wp_allow_comment($commentdata, $wp_error = false)
{
    ...
    if ($dupe_id) {
         ...
         */
        $comment_duplicate_message = apply_filters(
    ...
    if ($is_flood) {
        /** This filter is documented in wp-includes/comment-template.php */
        $comment_flood_message = apply_filters(
...
function get_comment_pages_count(
    ...
    if (
        ...
        null === $per_page &&
        null === $threaded &&
        !empty($wp_query->max_num_comment_pages)
    ) {
    ...
    if ($threaded) {
        ...
        $count = ceil(
            $walker->get_number_of_root_elements($comments) / $per_page
        );
    } else {
    ...
    if ($wpdb->is_mysql) {
        foreach ($lengths as $column => $length) {
            ...
            if (!is_array($col_length) && (int) $col_length > 0) {
                ...
                is_array($col_length) &&
                isset($col_length["length"]) &&
                (int) $col_length["length"] > 0
            ) {
                $max_length = (int) $col_length["length"];

                if (
                    !empty($col_length["type"]) &&
                    "byte" === $col_length["type"]
                ) {
...
function wp_check_comment_data_max_lengths($comment_data)
{
    ...
    if (
        isset($comment_data["comment_author"]) &&
        mb_strlen($comment_data["comment_author"], "8bit") >
            $max_lengths["comment_author"]
    ) {
        return new WP_Error(
            "comment_author_column_length",
            __("<strong>Error:</strong> Your name is too long."),
            200
        );
    }

    if (
        isset($comment_data["comment_author_email"]) &&
        strlen($comment_data["comment_author_email"]) >
            $max_lengths["comment_author_email"]
    ) {
        return new WP_Error(
            "comment_author_email_column_length",
            __("<strong>Error:</strong> Your email address is too long."),
            200
        );
    }

    if (
        isset($comment_data["comment_author_url"]) &&
        strlen($comment_data["comment_author_url"]) >
            $max_lengths["comment_author_url"]
    ) {
        return new WP_Error(
            "comment_author_url_column_length",
            __("<strong>Error:</strong> Your URL is too long."),
            200
        );
    }

    if (
        isset($comment_data["comment_content"]) &&
        mb_strlen($comment_data["comment_content"], "8bit") >
            $max_lengths["comment_content"]
    ) {
        return new WP_Error(
            "comment_content_column_length",
            __("<strong>Error:</strong> Your comment is too long."),
            200
        );
    }
...
function wp_check_comment_data($comment_data)
{
    ...
    if (
        isset($user) &&
        ($comment_data["user_id"] === $post_author ||
...
function wp_delete_comment($comment_id, $force_delete = false)
{
    ...
    if (
        !$force_delete &&
        EMPTY_TRASH_DAYS &&
        !in_array(wp_get_comment_status($comment), ["trash", "spam"], true)
    ) {
    ...
    if (
        !$wpdb->delete($wpdb->comments, ["comment_ID" => $comment->comment_ID])
    ) {
        return false;
    }
...
function wp_untrash_comment($comment_id)
{
    ...
    $status = (string) get_comment_meta(
        $comment->comment_ID,
        "_wp_trash_meta_status",
        true
    );
...
function wp_get_unapproved_comment_author_email()
{
    ...
    if (!empty($_GET["unapproved"]) && !empty($_GET["moderation-hash"])) {
        ...
        if (
            $comment &&
            hash_equals(
                $_GET["moderation-hash"],
                wp_hash($comment->comment_date_gmt)
            )
        ) {
            // The comment will only be viewable by the comment author for 10 minutes.
            $comment_preview_expires = strtotime(
                $comment->comment_date_gmt . "+10 minutes"
            );
...
function wp_insert_comment($commentdata)
{
    ...
    $comment_author = !isset($data["comment_author"])
        ? ""
        : $data["comment_author"];
    $comment_author_email = !isset($data["comment_author_email"])
        ? ""
        : $data["comment_author_email"];
    $comment_author_url = !isset($data["comment_author_url"])
        ? ""
    ...
    $comment_date = !isset($data["comment_date"])
        ? current_time("mysql")
        : $data["comment_date"];
    $comment_date_gmt = !isset($data["comment_date_gmt"])
        ? get_gmt_from_date($comment_date)
        : $data["comment_date_gmt"];

    $comment_post_id = !isset($data["comment_post_ID"])
        ? 0
        : $data["comment_post_ID"];
    $comment_content = !isset($data["comment_content"])
        ? ""
        : $data["comment_content"];
    $comment_karma = !isset($data["comment_karma"])
        ? 0
        : $data["comment_karma"];
    $comment_approved = !isset($data["comment_approved"])
        ? 1
        : $data["comment_approved"];
    $comment_agent = !isset($data["comment_agent"])
        ? ""
        : $data["comment_agent"];
    $comment_type = empty($data["comment_type"])
        ? "comment"
        : $data["comment_type"];
    $comment_parent = !isset($data["comment_parent"])
        ? 0
        : $data["comment_parent"];
    ...
    if (
        isset($commentdata["comment_meta"]) &&
...
function wp_filter_comment($commentdata)
{
    if (isset($commentdata["user_ID"])) {
         ...
         */
        $commentdata["user_id"] = apply_filters(
            "pre_user_id",
            $commentdata["user_ID"]
        );
     ...
     */
    $commentdata["comment_agent"] = apply_filters(
    ...
    /** This filter is documented in wp-includes/comment.php */
    $commentdata["comment_author"] = apply_filters(
        "pre_comment_author_name",
     ...
     */
    $commentdata["comment_author_IP"] = apply_filters(
        "pre_comment_user_ip",
        $commentdata["comment_author_IP"]
    );
    /** This filter is documented in wp-includes/comment.php */
    $commentdata["comment_author_url"] = apply_filters(
        "pre_comment_author_url",
        $commentdata["comment_author_url"]
    );
    /** This filter is documented in wp-includes/comment.php */
    $commentdata["comment_author_email"] = apply_filters(
        "pre_comment_author_email",
        $commentdata["comment_author_email"]
    );
...
function wp_new_comment($commentdata, $wp_error = false)
{
    ...
    $prefiltered_user_id = isset($commentdata["user_id"])
        ? (int) $commentdata["user_id"]
        : 0;
    ...
    if (!isset($commentdata["comment_agent"])) {
        $commentdata["comment_agent"] = isset($_SERVER["HTTP_USER_AGENT"])
    ...
    if (
        ...
        $prefiltered_user_id !== (int) $commentdata["user_ID"]
    ) {
        $commentdata["user_ID"] = (int) $commentdata["user_ID"];
        $commentdata["user_id"] = $commentdata["user_ID"];
    } elseif (isset($commentdata["user_id"])) {
    ...
    $commentdata["comment_parent"] = isset($commentdata["comment_parent"])
        ? absint($commentdata["comment_parent"])
        : 0;

    $parent_status =
        $commentdata["comment_parent"] > 0
            ? wp_get_comment_status($commentdata["comment_parent"])
            : "";

    $commentdata["comment_parent"] =
        "approved" === $parent_status || "unapproved" === $parent_status
            ? $commentdata["comment_parent"]
            : 0;

    $commentdata["comment_author_IP"] = preg_replace(
        "/[^0-9a-fA-F:., ]/",
        "",
        $commentdata["comment_author_IP"]
    );

    $commentdata["comment_agent"] = substr(
        $commentdata["comment_agent"],
        0,
        254
    );
    ...
    $commentdata["comment_approved"] = wp_allow_comment(
        $commentdata,
        $wp_error
    );
    ...
    if (!in_array($commentdata["comment_approved"], ["trash", "spam"], true)) {
    ...
    if (!$comment_id) {
        ...
        foreach ($fields as $field) {
            if (isset($commentdata[$field])) {
                $commentdata[$field] = $wpdb->strip_invalid_text_for_column(
                    $wpdb->comments,
                    $field,
                    $commentdata[$field]
                );
...
function wp_set_comment_status($comment_id, $comment_status, $wp_error = false)
{
    ...
    if (
        ...
        if ($wp_error) {
            return new WP_Error(
                "db_update_error",
                __("Could not update comment status."),
                $wpdb->last_error
            );
        } else {
            return false;
...
function wp_update_comment($commentarr, $wp_error = false)
{
    ...
    if (
        !empty($commentarr["comment_post_ID"]) &&
        !get_post($commentarr["comment_post_ID"])
    ) {
        if ($wp_error) {
     ...
     */
    $data["comment_content"] = apply_filters(
        "comment_save_pre",
        $data["comment_content"]
    );
    ...
    $result = $wpdb->update($wpdb->comments, $data, [
    ...
    if (false === $result) {
        if ($wp_error) {
            return new WP_Error(
                "db_update_error",
                __("Could not update comment in the database."),
                $wpdb->last_error
            );
        } else {
            return false;
...
function pingback($content, $post)
{
    ...
    foreach ((array) $post_links as $pagelinkedto) {
        ...
        if ($pingback_server_url) {
             ...
             */
            $client->useragent = apply_filters(
                "pingback_useragent",
...
function _prime_comment_caches($comment_ids, $update_meta_cache = true)
{
    ...
    if (!empty($non_cached_ids)) {
        $fresh_comments = $wpdb->get_results(
            sprintf(
                "SELECT $wpdb->comments.* FROM $wpdb->comments WHERE comment_ID IN (%s)",
...
function _close_comments_for_old_posts($posts, $query)
{
    ...
    if (
        time() - strtotime($posts[0]->post_date_gmt) >
...
function wp_handle_comment_submission($comment_data)
{
    ...
    if (
        isset($comment_data["comment"]) &&
        is_string($comment_data["comment"])
    ) {
    ...
    if (isset($comment_data["comment_parent"])) {
        ...
        if (
            ...
            return new WP_Error(
                "comment_reply_to_unapproved_comment",
                __("Sorry, replies to unapproved comments are not allowed."),
                403
            );
        }
    }
    ...
    if (
        "private" === $status &&
        !current_user_can("read_post", $comment_post_id)
    ) {
        return new WP_Error("comment_id_not_found");
    }
    ...
    if (!comments_open($comment_post_id)) {
        ...
        if (current_user_can("read_post", $comment_post_id)) {
            return new WP_Error(
                "comment_on_draft",
                __("Sorry, comments are not allowed for this item."),
                403
            );
        } else {
        ...
        if (current_user_can("unfiltered_html")) {
            if (
                ...
                !wp_verify_nonce(
                    ...
                    "unfiltered-html-comment_" . $comment_post_id
                )
            ) {
     ...
     */
    $allow_empty_comment = apply_filters(
        "allow_empty_comment",
        false,
        $commentdata
    );
...
function wp_comments_personal_data_eraser($email_address, $page = 1)
{
    ...
    foreach ((array) $comments as $comment) {
        ...
        $anonymized_comment["comment_author_IP"] = wp_privacy_anonymize_data(
            "ip",
...
function _wp_check_for_scheduled_update_comment_type()
{
    if (
        ...
        !wp_next_scheduled("wp_update_comment_type_batch")
    ) {
        wp_schedule_single_event(
