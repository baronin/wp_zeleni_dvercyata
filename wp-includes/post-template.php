...
function get_the_content(
    ...
    if (
        str_contains($_post->post_content, "<!--noteaser-->") &&
    ...
    if (count($content) > 1) {
        if ($elements["more"]) {
            $output .=
                '<span id="more-' . $_post->ID . '"></span>' . $content[1];
        } else {
            if (!empty($more_link_text)) {
                ...
                $output .= apply_filters(
                    ...
                    ' <a href="' .
                        get_permalink($_post) .
                        "#more-{$_post->ID}\" class=\"more-link\">$more_link_text</a>",
                    $more_link_text
                );
            }
...
function post_class($css_class = "", $post = null)
{
    ...
    echo 'class="' .
        esc_attr(implode(" ", get_post_class($css_class, $post))) .
        '"';
}
    ...
    if (is_singular()) {
        ...
        if (is_page_template()) {
            ...
            foreach ($template_parts as $part) {
                $classes[] =
                    "{$post_type}-template-" .
                    sanitize_html_class(
                        str_replace([".", "/"], "-", basename($part, ".php"))
                    );
            }
            $classes[] =
                "{$post_type}-template-" .
                sanitize_html_class(str_replace(".", "-", $template_slug));
        } else {
        ...
        if (is_post_type_archive()) {
            ...
            if (isset($author->user_nicename)) {
                $classes[] =
                    "author-" .
                    sanitize_html_class($author->user_nicename, $author->ID);
    ...
    if ($multipage) {
        if ("number" === $parsed_args["next_or_number"]) {
            ...
            for ($i = 1; $i <= $numpages; $i++) {
                $link =
                    $parsed_args["link_before"] .
                    str_replace("%", $i, $parsed_args["pagelink"]) .
                    $parsed_args["link_after"];
                ...
                if ($i !== $page || (!$more && 1 === $page)) {
                    ...
                    $link =
                        ...
                        esc_attr($parsed_args["aria_current"]) .
                        '">' .
                        $link .
                        "</span>";
                }
...
function _wp_link_page($i)
{
    ...
    if (1 === $i) {
        ...
        if (
            ...
            $url =
                trailingslashit(get_permalink()) .
                user_trailingslashit($i, "single_paged");
        }
    }
...
function wp_dropdown_pages($args = "")
{
    ...
    if (!empty($pages)) {
        ...
        if ($parsed_args["show_option_no_change"]) {
            $output .=
                "\t<option value=\"-1\">" .
                $parsed_args["show_option_no_change"] .
                "</option>\n";
        }
        if ($parsed_args["show_option_none"]) {
            $output .=
                "\t<option value=\"" .
                esc_attr($parsed_args["option_none_value"]) .
                '">' .
                $parsed_args["show_option_none"] .
                "</option>\n";
        }
        $output .= walk_page_dropdown_tree(
            $pages,
            $parsed_args["depth"],
            $parsed_args
        );
...
function wp_list_pages($args = "")
{
    ...
    if (
        !in_array($parsed_args["item_spacing"], ["preserve", "discard"], true)
    ) {
    ...
    $parsed_args["exclude"] = preg_replace(
        "/[^0-9,]/",
        "",
        $parsed_args["exclude"]
    );
    ...
    $exclude_array = $parsed_args["exclude"]
        ? explode(",", $parsed_args["exclude"])
        : [];
    ...
    if (!empty($pages)) {
        if ($parsed_args["title_li"]) {
            $output .=
                '<li class="pagenav">' . $parsed_args["title_li"] . "<ul>";
        }
        global $wp_query;
...
function wp_get_attachment_link(
    ...
    if ("" === trim($link_text)) {
        $link_text = esc_html(
            pathinfo(get_attached_file($_post->ID), PATHINFO_FILENAME)
        );
    }
...
function get_the_password_form($post = 0)
{
    ...
    if (
        !empty($post->ID) &&
        wp_get_raw_referer() === get_permalink($post->ID) &&
         ...
         * @param WP_Post $post Post object.
         */
        $invalid_password = apply_filters(
	...
	<p><label for="' .
        ...
        '">' .
        __("Password:") .
        ' <input name="post_password" id="' .
        $field_id .
        '" type="password" spellcheck="false" required size="20"' .
        $aria .
...
function is_page_template($template = "")
{
    ...
    if (is_array($template)) {
        if (
            (in_array("default", $template, true) && !$page_template) ||
...
function wp_list_post_revisions($post = 0, $type = "all")
{
    ...
    foreach ($revisions as $revision) {
        ...
        if (
            ("revision" === $type && $is_autosave) ||
            ("autosave" === $type && !$is_autosave)
        ) {
            continue;
        }

        $rows .=
            "\t<li>" . wp_post_revision_title_expanded($revision) . "</li>\n";
    }

    echo "<div class='hide-if-js'><p>" .
...
function get_post_parent($post = null)
{
    ...
    return !empty($wp_post->post_parent)
