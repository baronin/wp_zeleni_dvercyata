...
function wp_maybe_load_embeds()
{
    ...
    wp_embed_register_handler(
        "audio",
        "#^https?://.+?\.(" . implode("|", wp_get_audio_extensions()) . ')$#i',
    ...
    wp_embed_register_handler(
        "video",
        "#^https?://.+?\.(" . implode("|", wp_get_video_extensions()) . ')$#i',
...
{
    global $wp_embed;
    $embed = $wp_embed->autoembed(
        sprintf("https://youtube.com/watch?v=%s", urlencode($matches[2]))
    );
...
function wp_oembed_add_discovery_links()
{
    ...
    if (is_singular() && is_post_embeddable()) {
        $output .=
            ...
            _x("oEmbed (JSON)", "oEmbed resource link name") .
        ...
        if (class_exists("SimpleXMLElement")) {
            $output .=
                '<link rel="alternate" title="' .
                _x("oEmbed (XML)", "oEmbed resource link name") .
...
function get_post_embed_html($width, $height, $post = null)
{
    ...
    $output .= wp_get_inline_script_tag(
        file_get_contents(
            ABSPATH . WPINC . "/js/wp-embed" . wp_scripts_get_suffix() . ".js"
        )
    );
...
function get_oembed_response_data_rich($data, $post, $width, $height)
{
    ...
    if ($thumbnail_id) {
        list(
            ...
            $thumbnail_width,
            $thumbnail_height,
        ) = wp_get_attachment_image_src($thumbnail_id, [$width, 0]);
...
function _oembed_rest_pre_serve_request($served, $result, $request, $server)
{
    ...
    if (
        "/oembed/1.0/embed" !== $request->get_route() ||
    ...
    if (!headers_sent()) {
        $server->send_header(
            "Content-Type",
            "text/xml; charset=" . get_option("blog_charset")
        );
    }
...
function wp_enqueue_embed_styles()
{
    ...
    wp_add_inline_style(
        $handle,
        file_get_contents(ABSPATH . WPINC . "/css/wp-embed-template$suffix.css")
    );
...
function print_embed_sharing_button()
{
	...
	<div class="wp-embed-share">
		<button type="button" class="wp-embed-share-dialog-open" aria-label="<?php esc_attr_e(
      "Open sharing dialog"
  ); ?>">
...
function print_embed_sharing_dialog()
{
    ...
    $description_wordpress_id =
        "wp-embed-share-description-wordpress-" . $unique_suffix;
    $description_html_id = "wp-embed-share-description-html-" . $unique_suffix;
    ?>
	<div class="wp-embed-share-dialog hidden" role="dialog" aria-label="<?php esc_attr_e(
     "Sharing options"
 ); ?>">
			...
			<div class="wp-embed-share-dialog-text">
				<ul class="wp-embed-share-tabs" role="tablist">
					<li class="wp-embed-share-tab-button wp-embed-share-tab-button-wordpress" role="presentation">
						<button type="button" role="tab" aria-controls="<?php echo $share_tab_wordpress_id; ?>" aria-selected="true" tabindex="0"><?php esc_html_e(
    "WordPress Embed"
); ?></button>
					</li>
					<li class="wp-embed-share-tab-button wp-embed-share-tab-button-html" role="presentation">
						<button type="button" role="tab" aria-controls="<?php echo $share_tab_html_id; ?>" aria-selected="false" tabindex="-1"><?php esc_html_e(
    "HTML Embed"
); ?></button>
					</li>
				</ul>
				<div id="<?php echo $share_tab_wordpress_id; ?>" class="wp-embed-share-tab" role="tabpanel" aria-hidden="false">
					<input type="text" value="<?php the_permalink(); ?>" class="wp-embed-share-input" aria-label="<?php esc_attr_e(
    "URL"
); ?>" aria-describedby="<?php echo $description_wordpress_id; ?>" tabindex="0" readonly/>
				...
				<div id="<?php echo $share_tab_html_id; ?>" class="wp-embed-share-tab" role="tabpanel" aria-hidden="true">
					<textarea class="wp-embed-share-input" aria-label="<?php esc_attr_e(
         "HTML"
     ); ?>" aria-describedby="<?php echo $description_html_id; ?>" tabindex="0" readonly><?php echo esc_textarea(
    get_post_embed_html(600, 400)
); ?></textarea>
			...
			<button type="button" class="wp-embed-share-dialog-close" aria-label="<?php esc_attr_e(
