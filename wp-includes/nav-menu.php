...
function register_nav_menus($locations = [])
{
    ...
    foreach ($locations as $key => $value) {
        if (is_int($key)) {
            _doing_it_wrong(
                __FUNCTION__,
                __("Nav menu locations must be strings."),
                "5.3.0"
            );
            break;
        }
    ...
    $_wp_registered_nav_menus = array_merge(
        (array) $_wp_registered_nav_menus,
        $locations
    );
}
...
function unregister_nav_menu($location)
{
    ...
    if (
        is_array($_wp_registered_nav_menus) &&
...
function is_nav_menu_item($menu_item_id = 0)
{
    return !is_wp_error($menu_item_id) &&
        "nav_menu_item" === get_post_type($menu_item_id);
}
...
function wp_update_nav_menu_object($menu_id = 0, $menu_data = [])
{
    ...
    $args = [
        "description" => isset($menu_data["description"])
            ? $menu_data["description"]
            : "",
        "name" => isset($menu_data["menu-name"]) ? $menu_data["menu-name"] : "",
        "parent" => isset($menu_data["parent"])
            ? (int) $menu_data["parent"]
            : 0,
        "slug" => null,
    ];

    // Double-check that we're not going to have one menu take the name of another.
    $_possible_existing = get_term_by(
        "name",
        $menu_data["menu-name"],
        "nav_menu"
    );
...
function wp_update_nav_menu_item(
    ...
    $original_parent =
        0 < $menu_item_db_id
            ? get_post_field("post_parent", $menu_item_db_id)
            : 0;
    ...
    if ("custom" === $args["menu-item-type"]) {
        ...
        if ("taxonomy" === $args["menu-item-type"]) {
            $original_object = get_term(
                $args["menu-item-object-id"],
                $args["menu-item-object"]
            );

            if ($original_object instanceof WP_Term) {
                $original_parent = get_term_field(
                    "parent",
                    $args["menu-item-object-id"],
                    $args["menu-item-object"],
                    "raw"
                );
                $original_title = get_term_field(
                    "name",
                    $args["menu-item-object-id"],
                    $args["menu-item-object"],
                    "raw"
        ...
        if (
            wp_unslash($args["menu-item-title"]) ===
            wp_specialchars_decode($original_title)
        ) {
    ...
    if (
        ...
        $update_terms = wp_set_object_terms(
            $menu_item_db_id,
            [$menu->term_id],
            "nav_menu"
        );
    ...
    update_post_meta(
        $menu_item_db_id,
        "_menu_item_type",
        sanitize_key($args["menu-item-type"])
    );
    update_post_meta(
        $menu_item_db_id,
        "_menu_item_menu_item_parent",
        (string) ((int) $args["menu-item-parent-id"])
    );
    update_post_meta(
        $menu_item_db_id,
        "_menu_item_object_id",
        (string) ((int) $args["menu-item-object-id"])
    );
    update_post_meta(
        $menu_item_db_id,
        "_menu_item_object",
        sanitize_key($args["menu-item-object"])
    );
    update_post_meta(
        $menu_item_db_id,
        "_menu_item_target",
        sanitize_key($args["menu-item-target"])
    );

    $args["menu-item-classes"] = array_map(
        "sanitize_html_class",
        explode(" ", $args["menu-item-classes"])
    );
    $args["menu-item-xfn"] = implode(
        " ",
        array_map("sanitize_html_class", explode(" ", $args["menu-item-xfn"]))
    );
    update_post_meta(
        $menu_item_db_id,
        "_menu_item_classes",
    ...
    update_post_meta(
        $menu_item_db_id,
        "_menu_item_url",
        sanitize_url($args["menu-item-url"])
    );
    ...
    if ($update) {
        ...
        $post["post_status"] =
            "draft" === $args["menu-item-status"] ? "draft" : "publish";
...
function wp_setup_nav_menu_item($menu_item)
{
    ...
    if (isset($menu_item->post_type)) {
            ...
            if ("post_type" === $menu_item->type) {
                ...
                if ($object) {
                    ...
                    if (function_exists("get_post_states")) {
                        ...
                        if ($post_states) {
                            $menu_item->type_label = wp_strip_all_tags(
                                implode(", ", $post_states)
                            );
                        }
                    }
                ...
                $post_type_description =
                    "" === $post_content
                        ? $post_type_description
                        : $post_content;
                $menu_item->url = get_post_type_archive_link(
                    $menu_item->object
                );
        ...
        $menu_item->target = "";
        $menu_item->attr_title = "";
        $menu_item->description = get_term_field(
            "description",
            $menu_item->term_id,
            $menu_item->taxonomy
        );
...
function wp_map_nav_menu_locations(
    ...
    $registered_nav_menus = get_registered_nav_menus();
    $new_nav_menu_locations = array_intersect_key(
        $new_nav_menu_locations,
    ...
    if (
        1 === count($old_nav_menu_locations) &&
        1 === count($registered_nav_menus)
    ) {
        $new_nav_menu_locations[key($registered_nav_menus)] = array_pop(
            $old_nav_menu_locations
        );
        return $new_nav_menu_locations;
    }
    ...
    foreach ($registered_nav_menus as $location => $name) {
        if (in_array($location, $old_locations, true)) {
            $new_nav_menu_locations[$location] =
                $old_nav_menu_locations[$location];
    ...
    foreach ($common_slug_groups as $slug_group) {
        ...
        foreach ($slug_group as $slug) {
            ...
            foreach ($registered_nav_menus as $new_location => $name) {
                // ...actually match!
                if (
                    is_string($new_location) &&
                    false === stripos($new_location, $slug) &&
                    false === stripos($slug, $new_location)
                ) {
                    continue;
                } elseif (
                    is_numeric($new_location) &&
                    $new_location !== $slug
                ) {
                    continue;
                }
                ...
                foreach ($old_nav_menu_locations as $location => $menu_id) {
                    ...
                    foreach ($slug_group as $slug) {
                        ...
                        if (
                            is_string($location) &&
                            false === stripos($location, $slug) &&
                            false === stripos($slug, $location)
                        ) {
                            continue;
                        } elseif (
                            is_numeric($location) &&
                        ...
                        if (!empty($old_nav_menu_locations[$location])) {
                            ...
                            $new_nav_menu_locations[$new_location] =
                                $old_nav_menu_locations[$location];
