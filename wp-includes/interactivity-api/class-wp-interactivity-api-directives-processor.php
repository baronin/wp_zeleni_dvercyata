 ...
 * @access private
 */
final class WP_Interactivity_API_Directives_Processor extends
    WP_HTML_Tag_Processor
{
     ...
     */
    public function append_content_after_template_tag_closer(
        string $new_content
    ): bool {
        if (
            empty($new_content) ||
            "TEMPLATE" !== $this->get_tag() ||
            !$this->is_tag_closer()
        ) {
            return false;
        }
        ...
        $after_closing_tag =
            ...
            $this->bookmarks[$bookmark]->length;
     ...
     */
    private function get_after_opener_tag_and_before_closer_tag_positions(
        bool $rewind = false
    ) {
        // Flushes any changes.
        $this->get_updated_html();
        ...
        $after_opener_tag =
            $this->bookmarks[$opener_tag]->start +
            $this->bookmarks[$opener_tag]->length;
    ...
    public function skip_to_tag_closer(): bool
    {
        ...
        while ($depth > 0 && $this->next_tag(["tag_closers" => "visit"])) {
            if (
                !$this->is_tag_closer() &&
                $this->get_attribute_names_with_prefix("data-wp-")
            ) {
    ...
    public function has_and_visits_its_closer_tag(): bool
    {
        ...
        return null !== $tag_name &&
            (!WP_HTML_Processor::is_void($tag_name) &&
                !in_array(
                    $tag_name,
                    self::TAGS_THAT_DONT_VISIT_CLOSER_TAG,
                    true
                ));
    }
}
