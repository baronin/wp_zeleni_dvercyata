...
final class WP_Interactivity_API
{
     ...
     * @var array
     */
    private static $directive_processors = [
    ...
    {
        add_filter("script_module_data_@wordpress/interactivity", [
            $this,
    ...
    private function _process_directives(string $html)
    {
        ...
        $directive_processor_prefixes_reversed = array_reverse(
        ...
        while ($p->next_tag(["tag_closers" => "visit"])) {
            ...
            if ($p->is_tag_closer()) {
                ...
                if (
                    ...
                    foreach (
                        ...
                        list(
                            $directive_prefix,
                        ) = $this->extract_prefix_and_suffix($attribute_name);
                        if (
                            array_key_exists(
                                $directive_prefix,
                    ...
                    if ($p->has_and_visits_its_closer_tag()) {
                        $tag_stack[] = [$tag_name, $directives_prefixes];
                    }
                }
            }
            /*
            ...
            $modes = [
                ...
                "exit" =>
                    $p->is_tag_closer() || !$p->has_and_visits_its_closer_tag(),
            ];
    ...
    private function evaluate($directive_value)
    {
        ...
        list($ns, $path) = $this->extract_directive_value(
            $directive_value,
            $default_namespace
        );
        if (!$ns || !$path) {
     ...
     *               second item.
     */
    private function extract_directive_value(
        $directive_value,
        ...
        if (1 === preg_match("/^([\w\-_\/]+)::./", $directive_value)) {
            list($default_namespace, $directive_value) = explode(
                "::",
                $directive_value,
                2
            );
        ...
        return [$default_namespace, $directive_value];
    }
     ...
     */
    private function data_wp_interactive_processor(
        WP_Interactivity_API_Directives_Processor $p,
        string $mode
    ) {
        ...
        $this->namespace_stack[] =
            $new_namespace &&
            1 === preg_match("/^([\w\-_\/]+)/", $new_namespace)
                ? $new_namespace
                : end($this->namespace_stack);
    }
     ...
     * @param string                                    $mode            Whether the processing is entering or exiting the tag.
     */
    private function data_wp_context_processor(
        WP_Interactivity_API_Directives_Processor $p,
        string $mode
    ) {
        ...
        list($namespace_value, $decoded_json) =
        ...
        if (is_string($namespace_value)) {
            $this->context_stack[] = array_replace_recursive(
                end($this->context_stack) !== false
                    ? end($this->context_stack)
                    : [],
                [
                    $namespace_value => is_array($decoded_json)
                        ? $decoded_json
                        : [],
                ]
            );
    ...
    private function data_wp_bind_processor(
        ...
        if ("enter" === $mode) {
            $all_bind_directives = $p->get_attribute_names_with_prefix(
                "data-wp-bind--"
            );

            foreach ($all_bind_directives as $attribute_name) {
                list(, $bound_attribute) = $this->extract_prefix_and_suffix(
                    $attribute_name
                );
     ...
     * @param string                                    $mode            Whether the processing is entering or exiting the tag.
     */
    private function data_wp_class_processor(
        WP_Interactivity_API_Directives_Processor $p,
        string $mode
    ) {
        if ("enter" === $mode) {
            $all_class_directives = $p->get_attribute_names_with_prefix(
                "data-wp-class--"
            );

            foreach ($all_class_directives as $attribute_name) {
                list(, $class_name) = $this->extract_prefix_and_suffix(
                    $attribute_name
                );
                if (empty($class_name)) {
                    return;
    ...
    private function data_wp_style_processor(
        ...
        if ("enter" === $mode) {
            $all_style_attributes = $p->get_attribute_names_with_prefix(
                "data-wp-style--"
            );

            foreach ($all_style_attributes as $attribute_name) {
                list(, $style_property) = $this->extract_prefix_and_suffix(
                    $attribute_name
                );
                ...
                $style_property_value = $this->evaluate(
                ...
                if ($style_property_value || $style_attribute_value) {
                    $style_attribute_value = $this->merge_style_property(
                        $style_attribute_value,
                        $style_property,
    ...
    private function merge_style_property(
        ...
        string $style_property_name,
        $style_property_value
    ): string {
        $style_assignments = explode(";", $style_attribute_value);
        $result = [];
        $style_property_value = !empty($style_property_value)
            ? rtrim(trim($style_property_value), ";")
            : null;
        $new_style_property = $style_property_value
            ? $style_property_name . ":" . $style_property_value . ";"
            : "";
     ...
     * @param string                                    $mode            Whether the processing is entering or exiting the tag.
     */
    private function data_wp_text_processor(
        WP_Interactivity_API_Directives_Processor $p,
        string $mode
    ) {
        if ("enter" === $mode) {
...
HTML;
     ...
     * @param string                                    $mode            Whether the processing is entering or exiting the tag.
     */
    private function data_wp_router_region_processor(
        WP_Interactivity_API_Directives_Processor $p,
        string $mode
    ) {
        if ("enter" === $mode && !$this->has_processed_router_region) {
            ...
            wp_add_inline_style(
                "wp-interactivity-router-animations",
                $this->get_router_animation_styles()
            );
            wp_enqueue_style("wp-interactivity-router-animations");

            // Adds the necessary markup to the footer.
            add_action("wp_footer", [$this, "print_router_markup"]);
        }
    }
     ...
     */
    private function data_wp_each_processor(
        WP_Interactivity_API_Directives_Processor $p,
        string $mode,
        array &$tag_stack
    ) {
        if ("enter" === $mode && "TEMPLATE" === $p->get_tag()) {
            $attribute_name = $p->get_attribute_names_with_prefix(
                "data-wp-each"
            )[0];
            $extracted_suffix = $this->extract_prefix_and_suffix(
                $attribute_name
            );
            $item_name = isset($extracted_suffix[1])
                ? $this->kebab_to_camel_case($extracted_suffix[1])
                : "item";
            $attribute_value = $p->get_attribute($attribute_name);
            $result = $this->evaluate($attribute_value);
            ...
            foreach ($result as $item) {
                ...
                $this->context_stack[] = array_replace_recursive(
                        ...
                        ? end($this->context_stack)
                        : [],
                    [$namespace_value => [$item_name => $item]]
                );
