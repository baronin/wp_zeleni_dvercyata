...
function wp_insert_site(array $data)
{
    ...
    if (false === $wpdb->insert($wpdb->blogs, $prepared_data)) {
        return new WP_Error(
            "db_insert_error",
            __("Could not insert site into the database."),
            $wpdb->last_error
        );
    }
    ...
    if (!$new_site) {
        return new WP_Error(
            "get_site_error",
            __("Could not retrieve site data.")
        );
    }
    ...
    if (has_action("wpmu_new_blog")) {
        ...
        if (!array_key_exists("WPLANG", $meta)) {
            $meta["WPLANG"] = get_network_option(
                $new_site->network_id,
                "WPLANG"
            );
        }
        ...
        $meta = array_merge(
            array_intersect_key($data, array_flip($allowed_data_fields)),
            $meta
        );
...
function wp_update_site($site_id, array $data)
{
    ...
    if (
        false ===
        $wpdb->update($wpdb->blogs, $data, ["blog_id" => $old_site->id])
    ) {
        return new WP_Error(
            "db_update_error",
            __("Could not update site in the database."),
            $wpdb->last_error
        );
    }
...
function wp_delete_site($site_id)
{
    ...
    if (is_site_meta_supported()) {
        $blog_meta_ids = $wpdb->get_col(
            $wpdb->prepare(
                "SELECT meta_id FROM $wpdb->blogmeta WHERE blog_id = %d ",
                $old_site->id
            )
        );
    ...
    if (false === $wpdb->delete($wpdb->blogs, ["blog_id" => $old_site->id])) {
        return new WP_Error(
            "db_delete_error",
            __("Could not delete site from the database."),
            $wpdb->last_error
        );
    }
...
function wp_validate_site_data($errors, $data, $old_site = null)
{
    ...
    if (empty($data["network_id"])) {
        $errors->add(
            "site_empty_network_id",
            __("Site network ID must be provided.")
        );
    }
    ...
    foreach ($date_fields as $date_field) {
        if (empty($data[$date_field])) {
            $errors->add(
                "site_empty_" . $date_field,
                __("Both registration and last updated dates must be provided.")
            );
            break;
        }
...
function wp_initialize_site($site_id, array $args = [])
{
    ...
    if (!$site) {
        return new WP_Error(
            "site_invalid_id",
            __("Site with the ID does not exist.")
        );
    }

    if (wp_is_site_initialized($site)) {
        return new WP_Error(
            "site_already_initialized",
            __("The site appears to be already initialized.")
        );
    }
    ...
    if (!is_subdomain_install()) {
        if (
            "https" ===
            parse_url(get_home_url($network->site_id), PHP_URL_SCHEME)
        ) {
            $home_scheme = "https";
        }
        if (
            "https" ===
            parse_url(
                get_network_option($network->id, "siteurl"),
                PHP_URL_SCHEME
            )
        ) {
    ...
    populate_options(
        array_merge(
            [
                "home" => untrailingslashit(
                    $home_scheme . "://" . $site->domain . $site->path
                ),
                "siteurl" => untrailingslashit(
                    $siteurl_scheme . "://" . $site->domain . $site->path
                ),
                "blogname" => wp_unslash($args["title"]),
                "admin_email" => "",
                "upload_path" => get_network_option(
                    $network->id,
                    "ms_files_rewriting"
                )
                    ? UPLOADBLOGSDIR . "/{$site->id}/files"
    ...
    if (
        !user_can($args["user_id"], "manage_network") &&
        !get_user_meta($args["user_id"], "primary_blog", true)
    ) {
...
function wp_uninitialize_site($site_id)
{
    ...
    if (!wp_is_site_initialized($site)) {
        return new WP_Error(
            "site_already_uninitialized",
            __("The site appears to be already uninitialized.")
        );
    }
...
{
    if (
        $old_site->domain !== $new_site->domain ||
        $old_site->path !== $new_site->path
    ) {
