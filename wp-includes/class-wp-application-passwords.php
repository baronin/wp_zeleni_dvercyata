...
class WP_Application_Passwords
{
    ...
    public static function get_user_application_passwords($user_id)
    {
        $passwords = get_user_meta(
            $user_id,
            static::USERMETA_KEY_APPLICATION_PASSWORDS,
            true
        );
    ...
    public static function update_application_password(
        ...
        foreach ($passwords as &$item) {
            ...
            if ($save) {
                $saved = static::set_user_application_passwords(
                    $user_id,
                    $passwords
                );
        ...
        foreach ($passwords as $key => $item) {
            if ($item["uuid"] === $uuid) {
                ...
                if (!$saved) {
                    return new WP_Error(
                        "db_error",
                        __("Could not delete application password.")
                    );
                }
    ...
    public static function delete_all_application_passwords($user_id)
    {
        ...
        if ($passwords) {
            ...
            if (!$saved) {
                return new WP_Error(
                    "db_error",
                    __("Could not delete application passwords.")
                );
            }
