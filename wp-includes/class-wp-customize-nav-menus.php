...
final class WP_Customize_Nav_Menus
{
    ...
    public function __construct($manager)
    {
        ...
        add_action("customize_save_nav_menus_created_posts", [
            $this,
        ...
        add_action("wp_ajax_load-available-menu-items-customizer", [
            $this,
            "ajax_load_available_items",
        ]);
        add_action("wp_ajax_search-available-menu-items-customizer", [
            $this,
            "ajax_search_available_items",
        ]);
        add_action("wp_ajax_customize-nav-menus-insert-auto-draft", [
            $this,
        ...
        add_filter(
            ...
            [$this, "customize_dynamic_partial_args"],
            10,
            2
        );
    }
    ...
    public function ajax_load_available_items()
    {
        ...
        foreach ($item_types as $item_type) {
            if (empty($item_type["type"]) || empty($item_type["object"])) {
                wp_send_json_error(
                    "nav_menus_missing_type_or_object_parameter"
                );
            }
            ...
            $all_items[
                $item_type["type"] . ":" . $item_type["object"]
            ] = $items;
        }
    ...
    public function load_available_items_query(
        ...
        if ("post_type" === $object_type) {
            ...
            if (0 === $page && "page" === $object_name) {
                ...
                $privacy_policy_page_id = (int) get_option(
            ...
            if (
                ...
                foreach (
                    $this->manager
                        ->get_setting("nav_menus_created_posts")
                        ->value()
                    as $post_id
                ) {
            ...
            foreach ($posts as $post) {
                ...
                $post_type_label = get_post_type_object($post->post_type)
            ...
            foreach ($terms as $term) {
                $items[] = [
                    ...
                    "type" => "taxonomy",
                    "type_label" => get_taxonomy($term->taxonomy)->labels
                        ->singular_name,
    ...
    public function search_available_items_query($args = [])
    {
        ...
        $args["pagenum"] = isset($args["pagenum"])
            ? absint($args["pagenum"])
            : 1;
        $query["offset"] =
            $args["pagenum"] > 1
                ? $query["posts_per_page"] * ($args["pagenum"] - 1)
                : 0;
        ...
        $nav_menus_created_posts_setting = $this->manager->get_setting(
            "nav_menus_created_posts"
        );
        if (
            1 === $args["pagenum"] &&
            $nav_menus_created_posts_setting &&
            count($nav_menus_created_posts_setting->value()) > 0
        ) {
        ...
        if (isset($args["s"])) {
            ...
            if (empty($front_page)) {
                ...
                $matches = function_exists("mb_stripos")
                    ? false !== mb_stripos($title, $args["s"])
                    : false !== stripos($title, $args["s"]);
                if ($matches) {
                    $items[] = [
                        "id" => "home",
                        "title" => $title,
    ...
    public function enqueue_scripts()
    {
        ...
        $temp_nav_menu_setting = new WP_Customize_Nav_Menu_Setting(
            $this->manager,
            "nav_menu[-1]"
        );
        $temp_nav_menu_item_setting = new WP_Customize_Nav_Menu_Item_Setting(
            $this->manager,
            "nav_menu_item[-1]"
        );
        ...
        if (1 === $num_locations) {
            $locations_description = __(
                "Your theme can display menus in one location."
            );
        } else {
            /* translators: %s: Number of menu locations. */
            $locations_description = sprintf(
                _n(
        ...
        $settings = [
            ...
            "l10n" => [
                ...
                "page_label" => get_post_type_object("page")->labels
                    ->singular_name,
        ...
        $data = sprintf(
            "var _wpCustomizeNavMenusSettings = %s;",
    ...
    {
        if (
            preg_match(WP_Customize_Nav_Menu_Setting::ID_PATTERN, $setting_id)
        ) {
            $setting_args = [
            ...
            preg_match(
                WP_Customize_Nav_Menu_Item_Setting::ID_PATTERN,
                $setting_id
            )
        ) {
            $setting_args = [
     ...
     * @return string
     */
    public function filter_dynamic_setting_class(
        $setting_class,
        $setting_id,
        $setting_args
    ) {
        ...
        if (
            !empty($setting_args["type"]) &&
            WP_Customize_Nav_Menu_Setting::TYPE === $setting_args["type"]
        ) {
            $setting_class = "WP_Customize_Nav_Menu_Setting";
        } elseif (
            !empty($setting_args["type"]) &&
            WP_Customize_Nav_Menu_Item_Setting::TYPE === $setting_args["type"]
        ) {
    ...
    public function customize_register()
    {
        ...
        $this->manager->register_control_type(
        ...
        foreach ($menus as $menu) {
            $choices[$menu->term_id] = wp_html_excerpt(
                $menu->name,
                40,
                "&hellip;"
            );
        ...
        if (!$this->manager->is_theme_active()) {
            $theme_mods = get_option(
                "theme_mods_" . $this->manager->get_stylesheet(),
                []
            );
            ...
            $mapped_nav_menu_locations = wp_map_nav_menu_locations(
                $theme_mods["nav_menu_locations"],
        ...
        foreach ($locations as $location => $description) {
            ...
            if ($setting) {
                ...
                add_filter("customize_sanitize_{$setting_id}", [
                    $this,
                    "intval_base10",
                ]);
            } else {
            ...
            if (
                empty($changeset[$setting_id]) &&
                isset($mapped_nav_menu_locations[$location])
            ) {
    ...
    public function insert_auto_draft_post($postarr)
    {
        ...
        $postarr["meta_input"]["_customize_draft_post_name"] =
            $postarr["post_name"];
        $postarr["meta_input"][
            "_customize_changeset_uuid"
        ] = $this->manager->changeset_uuid();
    ...
    public function ajax_insert_auto_draft_post()
    {
        ...
        $illegal_params = array_diff(array_keys($params), [
            "post_type",
            "post_title",
        ]);
        ...
        if (
            ...
            !post_type_exists($params["post_type"])
        ) {
            status_header(400);
        ...
        if (is_wp_error($r)) {
            ...
            $data = [
                ...
                "message" => sprintf(
                    __('%1$s could not be created: %2$s'),
                    $singular_name,
				...
				<div class="menu-item-bar">
					<div class="menu-item-handle">
						...
						<button type="button" class="button-link item-add">
							...
							<?php /* translators: Hidden accessibility text. 1: Title of a menu item, 2: Type of a menu item. */
       printf(
           __('Add to menu: %1$s (%2$s)'),
		...
		<script type="text/html" id="tmpl-nav-menu-submit-new-button">
			<p id="customize-new-menu-submit-description"><?php _e(
       "Click &#8220;Next&#8221; to start adding links to your new menu."
   ); ?></p>
			<button id="customize-new-menu-submit" type="button" class="button" aria-describedby="customize-new-menu-submit-description"><?php _e(
       "Next"
   ); ?></button>
		</script>
      ...
      _e("Back"); ?>
				...
				<h3>
					<span class="customize-action">
						<?php /* translators: &#9656; is the unicode right-pointing triangle. %s: Section title in the Customizer. */
      printf(
          __("Customizing &#9656; %s"),
          esc_html($this->manager->get_panel("nav_menus")->title)
      ); ?>
   ...
   foreach ($item_types as $item_type) {
    ...
    {
        $id = sprintf(
            "available-menu-items-%s-%s",
            $available_item_type["type"],
            $available_item_type["object"]
        ); ?>
		<div id="<?php echo esc_attr($id); ?>" class="accordion-section">
			<h4 class="accordion-section-title">
				<button type="button" class="accordion-trigger" aria-expanded="false" aria-controls="<?php echo esc_attr(
        $id
    ); ?>-content">
			...
			<div class="accordion-section-content" id="<?php echo esc_attr(
       $id
   ); ?>-content">
         ...
         current_user_can($post_type_obj->cap->publish_posts)
           ...
           "create-item-input-" . $available_item_type["object"]
       ); ?>"><?php echo esc_html(
    $post_type_obj->labels->add_new_item
            ...
            "create-item-input-" . $available_item_type["object"]
					...
					<?php endif; ?>
				<?php endif; ?>
				<ul class="available-menu-items-list" data-type="<?php echo esc_attr(
        $available_item_type["type"]
    ); ?>" data-object="<?php echo esc_attr($available_item_type["object"]); ?>" data-type_label="<?php echo esc_attr(isset($available_item_type["type_label"]) ? $available_item_type["type_label"] : $available_item_type["type"]); ?>"></ul>
			</div>
		</div>
		<?php
    }
		...
		<div id="new-custom-menu-item" class="accordion-section">
			...
			<div class="accordion-section-content customlinkdiv" id="new-custom-menu-item-content">
				...
				<p id="menu-item-url-wrap" class="wp-clearfix">
					...
					<span id="custom-url-error" class="error-message" style="display: none;"><?php _e(
         "Please provide a valid link."
     ); ?></span>
				</p>
				<p id="menu-item-name-wrap" class="wp-clearfix">
					<label class="howto" for="custom-menu-item-name"><?php _e(
         "Link Text"
     ); ?></label>
					<input id="custom-menu-item-name" name="menu-item[-1][menu-item-title]" type="text" class="regular-text menu-item-textbox">
					<span id="custom-name-error" class="error-message" style="display: none;"><?php _e(
         "The link text cannot be empty."
     ); ?></span>
				</p>
				<p class="button-controls">
					<span class="add-to-menu">
						<input type="submit" class="button submit-add-to-menu right" value="<?php esc_attr_e(
          "Add to Menu"
    ...
    public function filter_wp_nav_menu_args($args)
    {
        ...
        $can_partial_refresh =
            ...
            (!empty($args["theme_location"]) ||
                (!empty($args["menu"]) &&
                    (is_numeric($args["menu"]) || is_object($args["menu"])))) &&
            // ...and if the nav menu would be rendered with a wrapper container element (upon which to attach data-* attributes).
            (!empty($args["container"]) ||
                (isset($args["items_wrap"]) &&
                    str_starts_with($args["items_wrap"], "<")));
    ...
    public function filter_wp_nav_menu($nav_menu_content, $args)
    {
        if (
            ...
            $attributes .= sprintf(
                ...
                esc_attr(
                    wp_json_encode($args->customize_preview_nav_menus_args)
                )
            );
            $nav_menu_content = preg_replace(
                "#^(<\w+)#",
                '$1 ' . str_replace("\\", "\\\\", $attributes),
                $nav_menu_content,
                1
            );
        }
    ...
    public function export_preview_data()
    {
        ...
        wp_print_inline_script_tag(
            sprintf(
                "var _wpCustomizePreviewNavMenusExports = %s;",
    ...
    {
        $response["nav_menu_instance_args"] =
            $this->preview_nav_menu_instance_args;
        return $response;
    }
    ...
    public function render_nav_menu_partial($partial, $nav_menu_args)
    {
        ...
        if (
            !hash_equals(
                $this->hash_nav_menu_args($nav_menu_args),
                $nav_menu_args_hmac
            )
        ) {
