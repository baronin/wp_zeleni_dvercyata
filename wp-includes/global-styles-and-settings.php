...
function wp_get_global_settings($path = [], $context = [])
{
    ...
    if (false === $settings) {
        $settings = WP_Theme_JSON_Resolver::get_merged_data(
            $origin
        )->get_settings();
        if ($can_use_cached) {
...
function wp_get_global_stylesheet($types = [])
{
    ...
    $tree = WP_Theme_JSON_Resolver::resolve_theme_file_uris(
    ...
    if (!empty($types)) {
        ...
        if (
            !$supports_theme_json &&
            (current_theme_supports("appearance-tools") ||
                current_theme_supports("border")) &&
...
function wp_add_global_styles_for_blocks()
{
    ...
    foreach ($block_nodes as $metadata) {
        if ($can_use_cached) {
            ...
            $cache_node_key = isset($metadata["name"])
                ? $metadata["name"]
                : md5(wp_json_encode($metadata));
        ...
        if (!isset($metadata["name"]) && !empty($metadata["path"])) {
            $block_name = wp_get_block_name_from_theme_json_path(
                $metadata["path"]
            );
            if ($block_name) {
...
function wp_theme_has_theme_json()
{
    ...
    if (
        ...
        file_exists($stylesheet_directory . "/theme.json")
    ) {
...
function wp_get_theme_data_template_parts()
{
    ...
    if (false === $metadata) {
        $metadata = WP_Theme_JSON_Resolver::get_theme_data(
            [],
            ["with_supports" => false]
        )->get_template_parts();
        if ($can_use_cached) {
...
function wp_get_block_css_selector(
    ...
    if ($has_selectors && isset($block_type->selectors["root"])) {
        ...
        $block_name = str_replace(
            "/",
            "-",
            str_replace("core/", "", $block_type->name)
        );
    ...
    if (1 === count($target)) {
        ...
        if ($has_selectors) {
            ...
            $feature_selector = _wp_array_get(
            ...
            return is_string($feature_selector)
                ? $feature_selector
                : $fallback_selector;
        }
    ...
    if ($has_selectors) {
        $subfeature_selector = _wp_array_get(
            $block_type->selectors,
            $target,
            null
        );
