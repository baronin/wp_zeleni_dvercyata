...
function wptexturize($text, $reset = false)
{
    ...
    if ($reset || !isset($static_characters)) {
        ...
        if ("'" !== $apos || "'" !== $closing_single_quote) {
            $dynamic['/\'(\d\d)\'(?=\Z|[.,:;!?)}\-\]]|&gt;|' . $spaces . ")/"] =
                $apos_flag . '$1' . $closing_single_quote;
        }
        if ("'" !== $apos || '"' !== $closing_quote) {
            $dynamic['/\'(\d\d)"(?=\Z|[.,:;!?)}\-\]]|&gt;|' . $spaces . ")/"] =
                $apos_flag . '$1' . $closing_quote;
        }
        ...
        if ("'" !== $opening_single_quote && "'" !== $closing_single_quote) {
            $dynamic["/(?<=\A|" . $spaces . ')\'(\d[.,\d]*)\'/'] =
                $open_sq_flag . '$1' . $closing_single_quote;
        }
     ...
     */
    $no_texturize_tags = apply_filters(
        "no_texturize_tags",
        $default_no_texturize_tags
    );
    /**
     ...
     */
    $no_texturize_shortcodes = apply_filters(
        ...
        if ("<" === $first) {
            ...
            if (str_contains($curl, "'")) {
                $curl = preg_replace(
                    ...
                    $dynamic_replacements["apos"],
                    $curl
                );
                $curl = wptexturize_primes(
                    $curl,
                    "'",
                    $prime,
                    $open_sq_flag,
            ...
            if (str_contains($curl, '"')) {
                $curl = preg_replace(
                    $dynamic_characters["quote"],
                    $dynamic_replacements["quote"],
                    $curl
                );
                $curl = wptexturize_primes(
                    $curl,
            ...
            if (str_contains($curl, "-")) {
                $curl = preg_replace(
                    $dynamic_characters["dash"],
                    $dynamic_replacements["dash"],
                    $curl
                );
            }
...
function shortcode_unautop($text)
{
    ...
    $tagregexp = implode(
        "|",
        array_map("preg_quote", array_keys($shortcode_tags))
    );
    $spaces = wp_spaces_regexp();
...
function _wp_specialchars(
    ...
    if (empty($quote_style)) {
        ...
        !in_array(
            $quote_style,
            [ENT_NOQUOTES, ENT_COMPAT, ENT_QUOTES, "single", "double"],
            true
        )
    ) {
    ...
    $charset = _canonical_charset(
        $charset ? $charset : get_option("blog_charset")
    );
    ...
    if (!$double_encode) {
         ...
         */
        $text = wp_kses_normalize_entities(
            $text,
            $quote_style & ENT_XML1 ? "xml" : "html"
        );
    }
...
function wp_specialchars_decode($text, $quote_style = ENT_NOQUOTES)
{
    ...
    $text = preg_replace(
        array_keys($translation_preg),
        array_values($translation_preg),
        $text
    );
...
function utf8_uri_encode(
    ...
    for ($i = 0; $i < $string_length; $i++) {
        ...
        if ($value < 128) {
            $char = chr($value);
            $encoded_char = $encode_ascii_characters
                ? rawurlencode($char)
                : $char;
    ...
    while (preg_match($tag_pattern, $text, $regex)) {
        ...
        if ($has_leading_slash) {
            ...
            if ($has_self_closer) {
                ...
                if (
                    $stacksize > 0 &&
                    !in_array($tag, $nestable_tags, true) &&
...
function antispambot($email_address, $hex_encoding = 0)
{
    ...
    for ($i = 0, $len = strlen($email_address); $i < $len; $i++) {
        ...
        if (0 === $j) {
            ...
            $email_no_spam_address .=
                "%" . zeroise(dechex(ord($email_address[$i])), 2);
        }
    }
...
function _make_clickable_rel_attr($url)
{
    ...
    $nofollow_schemes = array_intersect(wp_allowed_protocols(), [
        "https",
        "http",
    ]);
    ...
    if (
        ...
        in_array($scheme, $nofollow_schemes, true)
    ) {
        $rel_parts[] = "nofollow";
    }
    ...
    foreach ($textarr as $piece) {
        ...
        if (10000 < strlen($piece)) {
            ...
            foreach (_split_str_by_whitespace($piece, 2100) as $chunk) {
                // 2100: Extra room for scheme and leading and trailing parentheses.
...
function wp_trim_excerpt($text = "", $post = null)
{
    ...
    if ("" === trim($text)) {
         ...
         */
        $filter_image_removed = remove_filter(
            "the_content",
...
function sanitize_option($option, $value)
{
    ...
    switch ($option) {
        ...
        case "new_admin_email":
            $value = $wpdb->strip_invalid_text_for_column(
                $wpdb->options,
                "option_value",
                $value
            );
        ...
        case "blogname":
            ...
            if ($value !== $original_value) {
                $value = $wpdb->strip_invalid_text_for_column(
                    $wpdb->options,
                    "option_value",
                    wp_encode_emoji($original_value)
                );
            }
    ...
    if (null !== $error) {
        if ("" === $error && is_wp_error($value)) {
            ...
            $error = sprintf(
                __('Could not sanitize the %1$s option. Error code: %2$s'),
                $option,
 ...
 */
function wp_pre_kses_block_attributes(
    $content,
    $allowed_html,
    $allowed_protocols
) {
...
function _links_add_base($m)
{
    ...
    return $m[1] .
        "=" .
        $m[2] .
        (preg_match("#^(\w{1,20}):#", $m[3], $protocol) &&
        in_array($protocol[1], wp_allowed_protocols(), true)
            ? $m[3]
...
{
    return urldecode(
        basename(str_replace(["%2F", "%5C"], "/", urlencode($path)), $suffix)
    );
}
...
function _print_emoji_detection_script()
{
    $settings = [
        ...
        "baseUrl" => apply_filters(
            "emoji_url",
            "https://s.w.org/images/core/emoji/15.1.0/72x72/"
        ),
         ...
         */
        "svgUrl" => apply_filters(
            "emoji_svg_url",
            "https://s.w.org/images/core/emoji/15.1.0/svg/"
        ),
    ...
    if (SCRIPT_DEBUG) {
        $settings["source"] = [
            ...
            "wpemoji" => apply_filters(
                "script_loader_src",
                includes_url("js/wp-emoji.js?$version"),
                "wpemoji"
            ),
            /** This filter is documented in wp-includes/class-wp-scripts.php */
            "twemoji" => apply_filters(
                "script_loader_src",
...
function wp_staticize_emoji($text)
{
    ...
    for ($i = 0; $i < $stop; $i++) {
        ...
        if (
            "" === $ignore_block_element &&
            preg_match("/^<(" . $tags_to_ignore . ")>/", $content, $matches)
        ) {
            $ignore_block_element = $matches[1];
        }
