...
function _wp_post_revision_data($post = [], $autosave = false)
{
    ...
    $revision_data["post_name"] = $autosave
        ? "$post[ID]-autosave-v1"
        : "$post[ID]-revision-v1"; // "1" is the revisioning system version.
    $revision_data["post_date"] = isset($post["post_modified"])
        ? $post["post_modified"]
        : "";
    $revision_data["post_date_gmt"] = isset($post["post_modified_gmt"])
        ...
        if (
            ...
            foreach (array_keys(_wp_post_revision_fields($post)) as $field) {
                if (
                    ...
                    normalize_whitespace($latest_revision->$field)
                ) {
...
function wp_check_revisioned_meta_fields_have_changed(
    ...
    foreach (wp_post_revision_meta_keys($post->post_type) as $meta_key) {
        if (
            ...
            get_post_meta($last_revision->ID, $meta_key)
        ) {
            $post_has_changed = true;
            break;
        }
...
function wp_get_latest_revision_id_and_total_count($post = 0)
{
    ...
    if (!wp_revisions_enabled($post)) {
        return new WP_Error(
            "revisions_not_enabled",
            __("Revisions not enabled.")
        );
    }
...
function _show_post_preview()
{
    if (isset($_GET["preview_id"]) && isset($_GET["preview_nonce"])) {
        ...
        if (
            false ===
            wp_verify_nonce($_GET["preview_nonce"], "post_preview_" . $id)
        ) {
...
function _wp_preview_terms_filter($terms, $post_id, $taxonomy)
{
    ...
    if ("standard" === $_REQUEST["post_format"]) {
        ...
        $term = get_term_by(
            "slug",
            "post-format-" . sanitize_key($_REQUEST["post_format"]),
            "post_format"
        );
...
function _wp_upgrade_revisions_of_post($post, $revisions)
{
    ...
    do {
         ...
         */
        if ($prev_revision) {
            $prev_revision_version = _wp_get_post_revision_version(
                $prev_revision
            );
        ...
        $result = $wpdb->update($wpdb->posts, $update, [
            "ID" => $this_revision->ID,
        ]);
...
function _wp_preview_meta_filter($value, $object_id, $meta_key, $single)
{
    ...
    if (
        ...
        !in_array(
            $meta_key,
            wp_post_revision_meta_keys($post->post_type),
            true
        ) ||
        "revision" === $post->post_type
    ) {
