...
class WP
{
    ...
    {
        $this->public_query_vars = array_diff($this->public_query_vars, [
            $name,
        ]);
    }
    ...
    public function parse_request($extra_query_vars = "")
    {
        ...
        if (!empty($rewrite)) {
            ...
            if (
                !empty($pathinfo) &&
                !preg_match("|^.*" . $wp_rewrite->index . '$|', $pathinfo)
            ) {
         ...
         */
        $this->public_query_vars = apply_filters(
            "query_vars",
        ...
        $this->query_vars = wp_resolve_numeric_slug_conflicts(
            $this->query_vars
        );
    ...
    public function send_headers()
    {
        ...
        if (!empty($this->query_vars["error"])) {
            ...
            if (
                ...
                $wp_last_modified_post = mysql2date(
                    $date_format,
                    get_lastpostmodified("GMT"),
                    false
                );
                $wp_last_modified_comment = mysql2date(
                    $date_format,
                ...
                if (
                    strtotime($wp_last_modified_post) >
                    strtotime($wp_last_modified_comment)
                ) {
            ...
            if (
                $client_last_modified && $client_etag
                    ? $client_modified_timestamp >= $wp_modified_timestamp &&
                        $client_etag === $wp_etag
                    : $client_modified_timestamp >= $wp_modified_timestamp ||
                        $client_etag === $wp_etag
            ) {
        ...
        if (is_singular()) {
            ...
            if ($post && pings_open($post)) {
                $headers["X-Pingback"] = get_bloginfo(
                    "pingback_url",
                    "display"
                );
            }
    ...
    public function build_query_string()
    {
        ...
        foreach ((array) array_keys($this->query_vars) as $wpvar) {
            if ("" !== $this->query_vars[$wpvar]) {
                ...
                $this->query_string .=
                    $wpvar . "=" . rawurlencode($this->query_vars[$wpvar]);
            }
        }
            ...
            if (is_singular()) {
                ...
                if ($post && !empty($this->query_vars["page"])) {
                    ...
                    if (str_contains($post->post_content, $next)) {
                        ...
                        $content_found =
                            (int) $page <=
                            substr_count($post->post_content, $next) + 1;
                    } else {
                        $content_found = false;
                    }
                }
