...
class WP_Image_Editor_Imagick extends WP_Image_Editor
{
    ...
    protected function update_size($width = null, $height = null)
    {
        ...
        if (
            ...
            ("image/avif" === $this->mime_type ||
                wp_is_heic_image_mime_type($this->mime_type))
        ) {
    ...
    public static function set_imagick_time_limit()
    {
        ...
        $imagick_timeout = Imagick::getResourceLimit(
            Imagick::RESOURCETYPE_TIME
        );

        // Convert to an integer, keeping in mind that: 0 === (int) PHP_FLOAT_MAX.
        $imagick_timeout =
            $imagick_timeout > PHP_INT_MAX
                ? PHP_INT_MAX
                : (int) $imagick_timeout;
    ...
    protected function thumbnail_image(
         ...
         */
        if (
            in_array($filter_name, $allowed_filters, true) &&
            defined("Imagick::" . $filter_name)
        ) {
        ...
        try {
            ...
            if (is_callable([$this->image, "sampleImage"])) {
                $resize_ratio =
                    ($dst_w / $this->size["width"]) *
                    ($dst_h / $this->size["height"]);
                $sample_factor = 5;

                if (
                    $resize_ratio < 0.111 &&
                    ($dst_w * $sample_factor > 128 &&
            ...
            if ("image/png" === $this->mime_type) {
                ...
                if (is_callable([$this->image, "getImageDepth"])) {
    ...
    public function rotate($angle)
    {
        ...
        try {
            ...
            if (
                is_callable([$this->image, "setImageOrientation"]) &&
        ...
        if (!is_wp_error($saved)) {
            ...
            try {
                $this->image->setImageFormat(
                    strtoupper($this->get_extension($this->mime_type))
                );
            } catch (Exception $e) {
                return new WP_Error(
                    "image_save_error",
    ...
    protected function remove_pdf_alpha_channel()
    {
        ...
        if ($version["versionNumber"] >= 0x675) {
            try {
                ...
                $this->image->setImageAlphaChannel(
                    defined("Imagick::ALPHACHANNEL_REMOVE")
                        ? Imagick::ALPHACHANNEL_REMOVE
                        : 12
                );
            } catch (Exception $e) {
                return new WP_Error(
                    "pdf_alpha_process_failed",
                    $e->getMessage()
                );
            }
        }
    ...
    protected function _save($image, $filename = null, $mime_type = null)
    {
        ...
        if (
            method_exists($this->image, "setInterlaceScheme") &&
    ...
    private function write_image($image, $filename)
    {
        if (wp_is_stream($filename)) {
             ...
             */
            if (
                file_put_contents($filename, $image->getImageBlob()) === false
            ) {
                return new WP_Error(
                    "image_save_error",
                    sprintf(
    ...
    public function stream($mime_type = null)
    {
        ...
        try {
            ...
            $this->image->setImageFormat(
                $this->get_extension($this->mime_type)
            );
        } catch (Exception $e) {
    ...
    protected function strip_meta()
    {
        ...
        try {
            // Strip profiles.
            foreach (
                $this->image->getImageProfiles("*", true)
                as $key => $value
            ) {
