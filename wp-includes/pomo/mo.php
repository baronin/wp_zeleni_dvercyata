...
if (!class_exists("MO", false)):
    class MO extends Gettext_Translations
    {
        ...
        public function export_to_file_handle($fh)
        {
            ...
            fwrite(
                $fh,
                pack("VV", $reader->strlen($exported_headers), $current_addr)
            );
            ...
            foreach ($entries as $entry) {
                $translations_table .=
                    $this->export_translations($entry) . "\0";
        ...
        {
            // TODO: Warnings for control characters.
            return $entry->is_plural
                ? implode("\0", $entry->translations)
        ...
        public function import_from_reader($reader)
        {
            ...
            // Parse header.
            $header = unpack(
                "{$endian}revision/{$endian}total/{$endian}originals_lengths_addr/{$endian}translations_lengths_addr/{$endian}hash_length/{$endian}hash_addr",
                $header
            );
            if (!is_array($header)) {
                return false;
            ...
            $originals_lengths_length =
                $header["translations_lengths_addr"] -
                $header["originals_lengths_addr"];
            ...
            $translations_lengths_length =
                $header["hash_addr"] - $header["translations_lengths_addr"];
            ...
            if (
                $reader->strlen($translations) !== $translations_lengths_length
            ) {
                return false;
            }
