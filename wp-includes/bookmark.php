...
function get_bookmark($bookmark, $output = OBJECT, $filter = "raw")
{
    ...
    if (empty($bookmark)) {
        ...
        if (
            isset($GLOBALS["link"]) &&
            $GLOBALS["link"]->link_id === $bookmark
        ) {
            $_bookmark = &$GLOBALS["link"];
        } else {
            $_bookmark = wp_cache_get($bookmark, "bookmark");
            if (!$_bookmark) {
                $_bookmark = $wpdb->get_row(
                    $wpdb->prepare(
...
function get_bookmarks($args = "")
{
    ...
    if (!empty($parsed_args["category_name"])) {
        $parsed_args["category"] = get_term_by(
            "name",
            $parsed_args["category_name"],
            "link_category"
        );
    ...
    if ($parsed_args["show_updated"]) {
        $recently_updated_test =
            ", IF (DATE_ADD(link_updated, INTERVAL 120 MINUTE) >= NOW(), 1,0) as recently_updated ";
    } else {
        $recently_updated_test = "";
    }

    $get_updated = $parsed_args["show_updated"]
...
function sanitize_bookmark($bookmark, $context = "display")
{
    ...
    foreach ($fields as $field) {
        if ($do_object) {
            if (isset($bookmark->$field)) {
                $bookmark->$field = sanitize_bookmark_field(
                    $field,
                    $bookmark->$field,
                    $link_id,
                    $context
