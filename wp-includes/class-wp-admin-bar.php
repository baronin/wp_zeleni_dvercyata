...
class WP_Admin_Bar
{
    ...
    public function initialize()
    {
        ...
        if (is_user_logged_in()) {
            ...
            if (is_multisite()) {
                $this->user->active_blog = get_active_blog_for_user(
                    get_current_user_id()
                );
                $this->user->domain = empty($this->user->active_blog)
                    ? user_admin_url()
                    : trailingslashit(
                ...
                $this->user->active_blog =
                    $this->user->blogs[get_current_blog_id()];
    ...
    public function add_node($args)
    {
        ...
        if (empty($args["id"])) {
            ...
            _doing_it_wrong(
                __METHOD__,
                __("The menu ID should not be empty."),
                "3.3.0"
            );
    ...
    final protected function _bind()
    {
        ...
        foreach ($this->_get_nodes() as $node) {
            ...
            $group_class =
                "root" === $node->parent ? "ab-top-menu" : "ab-submenu";
    ...
    final protected function _render($root)
    {
		...
		<div id="wpadminbar" class="<?php echo $class; ?>">
			<?php if (!is_admin() && !did_action("wp_body_open")) { ?>
				<a class="screen-reader-shortcut" href="#wp-toolbar" tabindex="1"><?php _e(
        "Skip to toolbar"
    ); ?></a>
			<?php } ?>
			<div class="quicklinks" id="wp-toolbar" role="navigation" aria-label="<?php esc_attr_e(
       "Toolbar"
   ); ?>">
    ...
    final protected function _render_item($node)
    {
        ...
        $tabindex =
            isset($node->meta["tabindex"]) &&
            is_numeric($node->meta["tabindex"])
        ...
        echo "<li role='group' id='" .
            esc_attr("wp-admin-bar-" . $node->id) .
            "'$menuclass>";

        if ($has_link) {
            $attributes = ["onclick", "target", "title", "rel", "lang", "dir"];
            echo "<a class='ab-item'$aria_attributes href='" .
                esc_url($node->href) .
                "'";
        } else {
            $attributes = ["onclick", "target", "title", "rel", "lang", "dir"];
