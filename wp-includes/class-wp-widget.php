...
class WP_Widget
{
    ...
    public function update_callback($deprecated = 1)
    {
        ...
        if (isset($_POST["delete_widget"]) && $_POST["delete_widget"]) {
            ...
            if (
                isset($_POST["widget-" . $this->id_base]) &&
                is_array($_POST["widget-" . $this->id_base])
            ) {
                $settings = $_POST["widget-" . $this->id_base];
            } elseif (
                isset($_POST["id_base"]) &&
                $_POST["id_base"] === $this->id_base
            ) {
                $num = $_POST["multi_number"]
                    ? (int) $_POST["multi_number"]
                    : (int) $_POST["widget_number"];
                $settings = [$num => []];
            } else {
                return;
            }
            ...
            foreach ($settings as $number => $new_instance) {
                ...
                $old_instance = isset($all_instances[$number])
    ...
    public function get_settings()
    {
        ...
        if (!empty($settings) && !isset($settings["_multiwidget"])) {
            ...
            $settings = wp_convert_widget_settings(
                $this->id_base,
                $this->option_name,
                $settings
            );
