...
final class WP_Style_Engine
{
     ...
     */
    protected static function get_slug_from_preset_value(
        $style_value,
        $property_key
    ) {
        if (
            is_string($style_value) &&
            is_string($property_key) &&
    ...
    protected static function get_css_var_value($style_value, $css_vars)
    {
        foreach ($css_vars as $property_key => $css_var_pattern) {
            $slug = static::get_slug_from_preset_value(
                $style_value,
                $property_key
            );
    ...
    public static function store_css_rule(
        ...
        if (
            empty($store_name) ||
            empty($css_selector) ||
            empty($css_declarations)
        ) {
            return;
        }
        static::get_store($store_name)
            ->add_rule($css_selector, $rules_group)
    ...
    public static function parse_block_styles($block_styles, $options)
    {
        ...
        foreach (
            static::BLOCK_STYLE_DEFINITIONS_METADATA
            as $definition_group_key => $definition_group_style
        ) {
            ...
            foreach ($definition_group_style as $style_definition) {
                ...
                $classnames = static::get_classnames(
                    $style_value,
                    $style_definition
                );
                if (!empty($classnames)) {
                    $parsed_styles["classnames"] = array_merge(
                        $parsed_styles["classnames"],
                        $classnames
                    );
                }

                $css_declarations = static::get_css_declarations(
                    $style_value,
                    $style_definition,
                    $options
                );
                if (!empty($css_declarations)) {
                    $parsed_styles["declarations"] = array_merge(
    ...
    protected static function get_classnames($style_value, $style_definition)
    {
        ...
        if (!empty($style_definition["classnames"])) {
            foreach (
                $style_definition["classnames"]
                as $classname => $property_key
            ) {
    ...
    protected static function get_css_declarations(
        ...
        $should_skip_css_vars =
            isset($options["convert_vars_to_classnames"]) &&
            true === $options["convert_vars_to_classnames"];
        ...
        if (is_string($style_value) && str_contains($style_value, "var:")) {
            if (
                !$should_skip_css_vars &&
                !empty($style_definition["css_vars"])
            ) {
                $css_var = static::get_css_var_value(
                    $style_value,
                    $style_definition["css_vars"]
                );
                if (static::is_valid_style_value($css_var)) {
                    $css_declarations[
                        $style_property_keys["default"]
                    ] = $css_var;
                }
            }
        ...
        if (is_array($style_value)) {
            ...
            foreach ($style_value as $key => $value) {
                if (
                    ...
                    $value = static::get_css_var_value(
                        $value,
                        $style_definition["css_vars"]
                    );
                }

                $individual_property = sprintf(
                    $style_property_keys["individual"],
                    _wp_to_kebab_case($key)
                );

                if (
                    $individual_property &&
                    static::is_valid_style_value($value)
                ) {
    ...
    protected static function get_individual_property_css_declarations(
        ...
        if (
            !is_array($style_value) ||
            empty($style_value) ||
            empty($individual_property_definition["path"])
        ) {
            return [];
        }
        ...
        foreach ($style_value as $css_property => $value) {
            ...
            if (
                $style_definition &&
                isset($style_definition["property_keys"]["individual"])
            ) {
                ...
                if (
                    ...
                    str_contains($value, "var:") &&
                    !$should_skip_css_vars &&
                    !empty($individual_property_definition["css_vars"])
                ) {
                    $value = static::get_css_var_value(
                        $value,
                        $individual_property_definition["css_vars"]
                    );
                }

                $individual_css_property = sprintf(
                    $style_definition["property_keys"]["individual"],
    ...
    protected static function get_url_or_value_css_declaration(
        ...
        if (isset($style_definition["property_keys"]["default"])) {
            ...
            if (null !== $value) {
                $css_declarations[
                    $style_definition["property_keys"]["default"]
                ] = $value;
            }
        }
    ...
    public static function compile_css($css_declarations, $css_selector)
    {
        ...
        if ($css_selector) {
            $css_rule = new WP_Style_Engine_CSS_Rule(
                $css_selector,
                $css_declarations
            );
