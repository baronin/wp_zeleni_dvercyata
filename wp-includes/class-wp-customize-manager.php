...
final class WP_Customize_Manager
{
    ...
    public function __construct($args = [])
    {
        ...
        if (
            !isset($args["messenger_channel"]) &&
            isset($_REQUEST["customize_messenger_channel"])
        ) {
            $args["messenger_channel"] = sanitize_key(
                wp_unslash($_REQUEST["customize_messenger_channel"])
            );
        }
        ...
        $this->theme = wp_get_theme(
            0 === validate_file($args["theme"]) ? $args["theme"] : null
        );
        ...
        require_once ABSPATH .
            WPINC .
            "/customize/class-wp-customize-code-editor-control.php";
        require_once ABSPATH .
            WPINC .
        ...
        require_once ABSPATH .
            WPINC .
            "/customize/class-wp-customize-nav-menu-locations-control.php";
        require_once ABSPATH .
            WPINC .
        ...
        require_once ABSPATH .
            WPINC .
            "/customize/class-wp-customize-themes-panel.php";
        require_once ABSPATH .
            WPINC .
        ...
        add_filter(
            "heartbeat_received",
            [$this, "check_changeset_lock_with_heartbeat"],
            10,
            3
        );
        ...
        add_filter(
            ...
            [$this, "export_header_video_settings"],
            10,
            3
        );
    ...
    public function doing_ajax($action = null)
    {
        ...
        if (!$action) {
            ...
            return isset($_REQUEST["action"]) &&
                wp_unslash($_REQUEST["action"]) === $action;
        }
    }
    ...
    protected function wp_die($ajax_message, $message = null)
    {
        ...
        if ($this->messenger_channel) {
			...
			<?php $message .= wp_get_inline_script_tag(
       wp_remove_surrounding_empty_script_tags(ob_get_clean())
   );
        }
    ...
    public function setup_theme()
    {
        ...
        if (
            isset($this->_changeset_uuid) &&
            false !== $this->_changeset_uuid &&
        ...
        $has_post_data_nonce =
            check_ajax_referer(
                "preview-customize_" . $this->get_stylesheet(),
                "nonce",
                false
            ) ||
            check_ajax_referer(
                "save-customize_" . $this->get_stylesheet(),
                "nonce",
                false
            ) ||
            check_ajax_referer(
    ...
    public function establish_loaded_changeset()
    {
        ...
        if (empty($this->_changeset_uuid)) {
            ...
            if (!$this->branching() && $this->is_theme_active()) {
                ...
                $unpublished_changeset_post = array_shift(
                    $unpublished_changeset_posts
                );
                if (
                    !empty($unpublished_changeset_post) &&
    ...
    public function stop_previewing_theme()
    {
        ...
        if (!$this->is_theme_active()) {
            ...
            // Handle custom theme roots.
            remove_filter("pre_option_stylesheet_root", [
                $this,
                "get_stylesheet_root",
            ]);
            remove_filter("pre_option_template_root", [
                $this,
                "get_template_root",
            ]);
        }
    ...
    protected function get_changeset_post_data($post_id)
    {
        ...
        if ("revision" === $changeset_post->post_type) {
            if (
                "customize_changeset" !==
                get_post_type($changeset_post->post_parent)
            ) {
    ...
    public function changeset_data()
    {
        ...
        if (!$changeset_post_id) {
            ...
            if ($this->autosaved() && is_user_logged_in()) {
                $autosave_post = wp_get_post_autosave(
                    $changeset_post_id,
                    get_current_user_id()
                );
                if ($autosave_post) {
    ...
    public function import_theme_starter_content($starter_content = [])
    {
        ...
        $sidebars_widgets =
            isset($starter_content["widgets"]) && !empty($this->widgets)
                ? $starter_content["widgets"]
                : [];
        $attachments =
            isset($starter_content["attachments"]) && !empty($this->nav_menus)
                ? $starter_content["attachments"]
                : [];
        $posts =
            isset($starter_content["posts"]) && !empty($this->nav_menus)
                ? $starter_content["posts"]
                : [];
        $options = isset($starter_content["options"])
            ? $starter_content["options"]
            : [];
        $nav_menus =
            isset($starter_content["nav_menus"]) && !empty($this->nav_menus)
                ? $starter_content["nav_menus"]
                : [];
        $theme_mods = isset($starter_content["theme_mods"])
            ? $starter_content["theme_mods"]
            : [];
        ...
        foreach ($sidebars_widgets as $sidebar_id => $widgets) {
            ...
            foreach ($widgets as $widget) {
                ...
                if (!isset($max_widget_numbers[$id_base])) {
                    ...
                    if (
                        $settings instanceof ArrayObject ||
                        $settings instanceof ArrayIterator
                    ) {
                ...
                $setting_value = $this->widgets->sanitize_widget_js_instance(
                    $instance
                );
                if (
                    empty($changeset_data[$setting_id]) ||
        ...
        if (!empty($changeset_data["nav_menus_created_posts"]["value"])) {
            $starter_content_auto_draft_post_ids = array_merge(
                $starter_content_auto_draft_post_ids,
        ...
        foreach (array_keys($posts) as $post_symbol) {
            if (
                empty($posts[$post_symbol]["post_name"]) &&
                empty($posts[$post_symbol]["post_title"])
            ) {
                unset($posts[$post_symbol]);
                continue;
            }
            if (empty($posts[$post_symbol]["post_name"])) {
                $posts[$post_symbol]["post_name"] = sanitize_title(
                    $posts[$post_symbol]["post_title"]
                );
            }
            ...
            $needed_posts[
                $posts[$post_symbol]["post_type"] .
                    ":" .
                    $posts[$post_symbol]["post_name"]
            ] = true;
        }
        ...
        $post_types = array_filter(
            array_merge(["attachment"], wp_list_pluck($posts, "post_type"))
        );
        ...
        if (!empty($starter_content_auto_draft_post_ids)) {
            ...
            foreach ($existing_posts_query->posts as $existing_post) {
                ...
                $existing_starter_content_posts[
                    $existing_post->post_type . ":" . $post_name
                ] = $existing_post;
            }
        }
        ...
        if (!empty($all_post_slugs)) {
            ...
            foreach ($existing_posts_query->posts as $existing_post) {
                $key =
                    $existing_post->post_type . ":" . $existing_post->post_name;
                if (
                    isset($needed_posts[$key]) &&
                    !isset($existing_starter_content_posts[$key])
                ) {
        ...
        if (!empty($posts)) {
            foreach (array_keys($posts) as $post_symbol) {
                if (
                    empty($posts[$post_symbol]["post_type"]) ||
                    empty($posts[$post_symbol]["post_name"])
                ) {
                    continue;
                }
                ...
                if (
                    !empty($posts[$post_symbol]["thumbnail"]) &&
                    preg_match(
                        '/^{{(?P<symbol>.+)}}$/',
                        $posts[$post_symbol]["thumbnail"],
                        $matches
                    ) &&
                    isset($attachment_ids[$matches["symbol"]])
                ) {
                    $posts[$post_symbol]["meta_input"]["_thumbnail_id"] =
                        $attachment_ids[$matches["symbol"]];
                }

                if (!empty($posts[$post_symbol]["template"])) {
                    $posts[$post_symbol]["meta_input"]["_wp_page_template"] =
                        $posts[$post_symbol]["template"];
                }

                $r = $this->nav_menus->insert_auto_draft_post(
                    $posts[$post_symbol]
                );
        ...
        if (
            !empty($this->nav_menus) &&
            !empty($starter_content_auto_draft_post_ids)
        ) {
            $setting_id = "nav_menus_created_posts";
            $this->set_post_value(
                $setting_id,
                array_unique(array_values($starter_content_auto_draft_post_ids))
            );
            $this->pending_starter_content_settings_ids[] = $setting_id;
        }
        ...
        foreach ($nav_menus as $nav_menu_location => $nav_menu) {
            ...
            if (!$nav_menu_term_id) {
                while (
                    isset(
                        $changeset_data[
                            sprintf("nav_menu[%d]", $placeholder_id)
                        ]
                    )
                ) {
                    --$placeholder_id;
            ...
            foreach ($nav_menu["items"] as $nav_menu_item) {
                $nav_menu_item_setting_id = sprintf(
                    "nav_menu_item[%d]",
                    $placeholder_id--
                );
                ...
                if (isset($nav_menu_item["object_id"])) {
                    if (
                        ...
                        isset($posts[$matches["symbol"]])
                    ) {
                        $nav_menu_item["object_id"] =
                            $posts[$matches["symbol"]]["ID"];
                        if (empty($nav_menu_item["title"])) {
                            $original_object = get_post(
                                $nav_menu_item["object_id"]
                            );
                            $nav_menu_item["title"] =
                ...
                if (
                    empty($changeset_data[$nav_menu_item_setting_id]) ||
                    !empty(
                        $changeset_data[$nav_menu_item_setting_id][
                            "starter_content"
                        ]
                    )
                ) {
        ...
        foreach ($options as $name => $value) {
            ...
            if (is_serialized($value)) {
                if (
                    preg_match('/s:\d+:"{{(?P<symbol>.+)}}"/', $value, $matches)
                ) {
            ...
            if (
                empty($changeset_data[$name]) ||
                !empty($changeset_data[$name]["starter_content"])
            ) {
    ...
    protected function prepare_starter_content_attachments($attachments)
    {
        ...
        foreach ($attachments as $symbol => $attachment) {
            ...
            if (
                ...
                preg_match('#^https?://$#', $attachment["file"])
            ) {
                continue;
            }
            ...
            if (file_exists($attachment["file"])) {
                ...
                is_child_theme() &&
                file_exists(
                    get_stylesheet_directory() . "/" . $attachment["file"]
                )
            ) {
                $file_path =
                    get_stylesheet_directory() . "/" . $attachment["file"];
                ...
                file_exists(
                    get_template_directory() . "/" . $attachment["file"]
                )
            ) {
                $file_path =
                    get_template_directory() . "/" . $attachment["file"];
            ...
            if (empty($attachment["post_name"])) {
                if (!empty($attachment["post_title"])) {
                    $attachment["post_name"] = sanitize_title(
                        $attachment["post_title"]
                    );
                } else {
                    $attachment["post_name"] = sanitize_title(
                        preg_replace('/\.\w+$/', "", $file_name)
                    );
                }
            }
    ...
    public function unsanitized_post_values($args = [])
    {
        ...
        if (!$this->is_theme_active()) {
            ...
            if (isset($stashed_theme_mods[$stylesheet])) {
                $values = array_merge(
                    $values,
                    wp_list_pluck($stashed_theme_mods[$stylesheet], "value")
                );
            }
        }
        ...
        if (!$args["exclude_changeset"]) {
            foreach (
                ...
                if (
                    ...
                    if (
                        preg_match($namespace_pattern, $setting_id, $matches) &&
                        $this->get_stylesheet() === $matches["stylesheet"]
                    ) {
                        $values[$matches["setting_id"]] =
        ...
        if (!$args["exclude_post_data"]) {
            if (!isset($this->_post_values)) {
                if (isset($_POST["customized"])) {
                    $post_values = json_decode(
                        wp_unslash($_POST["customized"]),
                        true
                    );
                } else {
                    $post_values = [];
    ...
    public function add_state_query_params($url)
    {
        ...
        if ($is_allowed) {
            ...
            if ($this->messenger_channel) {
                $query_params["customize_messenger_channel"] =
    ...
    public function remove_frameless_preview_messenger_channel()
    {
		...
		<?php wp_print_inline_script_tag(
      wp_remove_surrounding_empty_script_tags(ob_get_clean())
  );
    }
    ...
    public function validate_setting_values($setting_values, $options = [])
    {
        ...
        foreach ($setting_values as $setting_id => $unsanitized_value) {
            ...
            if (
                $options["validate_capability"] &&
                !current_user_can($setting->capability)
            ) {
                $validity = new WP_Error(
                    "unauthorized",
                    __("Unauthorized to modify setting due to capability.")
                );
            } else {
            ...
            if (!is_wp_error($validity)) {
                ...
                if (
                    is_wp_error($late_validity) &&
                    $late_validity->has_errors()
                ) {
    ...
    public function save()
    {
        ...
        if ($is_new_changeset) {
            if (
                !current_user_can(
                    get_post_type_object("customize_changeset")->cap
                        ->create_posts
                )
            ) {
        ...
        if (!empty($_POST["customize_changeset_data"])) {
            $input_changeset_data = json_decode(
                wp_unslash($_POST["customize_changeset_data"]),
                true
            );
        ...
        if (isset($_POST["customize_changeset_title"])) {
            $changeset_title = sanitize_text_field(
                wp_unslash($_POST["customize_changeset_title"])
            );
        }
        ...
        if (isset($_POST["customize_changeset_status"])) {
            $changeset_status = wp_unslash(
                $_POST["customize_changeset_status"]
            );
            if (
    ...
    public function save_changeset_post($args = [])
    {
        ...
        if ($args["date_gmt"]) {
            $is_future_dated =
                mysql2date("U", $args["date_gmt"], false) >
                mysql2date("U", $now, false);
            if (!$is_future_dated) {
                return new WP_Error(
                    "not_future_date",
                    __("You must supply a future date to schedule.")
                ); // Only future dates are allowed.
            }

            if (
                !$this->is_theme_active() &&
                ("future" === $args["status"] || $is_future_dated)
            ) {
            ...
            if (
                mysql2date("U", $changeset_post->post_date_gmt, false) <=
                mysql2date("U", $now, false)
            ) {
                return new WP_Error(
                    "not_future_date",
        ...
        foreach ($args["data"] as $setting_id => $setting_params) {
            if (
                is_array($setting_params) &&
                array_key_exists("value", $setting_params)
            ) {
        ...
        foreach ($post_values as $setting_id => $setting_value) {
            ...
            if ($setting && "theme_mod" === $setting->type) {
                $prefixed_setting_id =
                    $this->get_stylesheet() . "::" . $setting->id;
            } else {
            ...
            $is_value_changed =
                ...
                !array_key_exists(
                    "value",
                    $existing_changeset_data[$prefixed_setting_id]
                ) ||
                $existing_changeset_data[$prefixed_setting_id]["value"] !==
                    $setting_value;
            if ($is_value_changed) {
        ...
        $invalid_setting_count = count(
            array_filter($setting_validities, "is_wp_error")
        );
        ...
        foreach ($args["data"] as $setting_id => $setting_params) {
            ...
            if (
                isset($setting_validities[$setting_id]) &&
                is_wp_error($setting_validities[$setting_id])
            ) {
                continue;
            }
            ...
            if (null === $setting_params) {
                ...
                $merged_setting_params = array_merge(
                    $data[$changeset_setting_id],
                    $setting_params
                );
        ...
        $filter_context = [
            ...
            "previous_data" => is_wp_error($original_changeset_data)
                ? []
        ...
        add_filter(
            ...
            [$this, "_filter_revision_post_has_changed"],
            5,
            3
        );
        ...
        if ($changeset_post_id) {
            if (
                ...
                if (!empty($args["user_id"])) {
                    $autosave_draft = wp_get_post_autosave(
                        $changeset_post_id,
                        $args["user_id"]
                    );
                    if ($autosave_draft) {
        ...
        remove_filter("wp_save_post_revision_post_has_changed", [
            $this,
     ...
     */
    public function preserve_insert_changeset_post_content(
        $data,
        $postarr,
        $unsanitized_postarr
    ) {
     ...
     */
    public function grant_edit_post_capability_for_changeset(
        $caps,
        $cap,
        $user_id,
        $args
        ...
        if (
            "edit_post" === $cap &&
            !empty($args[0]) &&
            "customize_changeset" === get_post_type($args[0])
        ) {
    ...
    {
        if ($changeset_post_id) {
            $can_override = !(bool) get_post_meta(
                $changeset_post_id,
                "_edit_lock",
                true
            );
    ...
    protected function get_lock_user_data($user_id)
    {
        ...
        if (get_option("show_avatars")) {
            $user_details["avatar"] = get_avatar_url($lock_user->ID, [
                "size" => 128,
            ]);
        }
     ...
     */
    public function check_changeset_lock_with_heartbeat(
        $response,
        $data,
        $screen_id
    ) {
    ...
    public function _publish_changeset_values($changeset_post_id)
    {
        ...
        foreach ($this->_changeset_data as $raw_setting_id => $setting_params) {
            ...
            if ($actual_setting_id && isset($setting_params["user_id"])) {
                $setting_user_ids[$actual_setting_id] =
                    $setting_params["user_id"];
            }
        }
        ...
        foreach ($changeset_setting_ids as $setting_id) {
            ...
            if ($setting && !isset($setting_user_ids[$setting_id])) {
                $original_setting_capabilities[$setting->id] =
                    $setting->capability;
         ...
         */
        $revisions = wp_get_post_revisions($changeset_post_id, [
            "check_enabled" => false,
        ]);
     ...
     */
    protected function update_stashed_theme_mod_settings(
        $inactive_theme_mod_settings
    ) {
        $stashed_theme_mod_settings = get_option(
        ...
        foreach (
            $inactive_theme_mod_settings
            as $stylesheet => $theme_mod_settings
        ) {
    ...
    public function handle_dismiss_autosave_or_lock_request()
    {
        ...
        if (
            !check_ajax_referer(
                "customize_dismiss_autosave_or_lock",
                "nonce",
                false
            )
        ) {
            ...
            if (
                ...
                if ($revision) {
                    if (
                        ...
                        wp_send_json_error(
                            "cannot_delete_autosave_revision",
                            403
                        );
                    }
                    ...
                    if (!wp_delete_post($revision->ID, true)) {
                        wp_send_json_error(
                            "autosave_revision_deletion_failure",
                            500
                        );
                    } else {
    ...
    public function render_control_templates()
    {
      ...
      _e("Dismiss"); ?>
				...
				<div class="notification-message customize-changeset-locked-message {{ data.lockUser.avatar ? 'has-avatar' : '' }}">
					...
					<p class="currently-editing">
						...
						<# } else if ( data.allowOverride ) { #>
							<?php echo esc_html(
           sprintf($l10n["locked_allow_override"], "{{ data.lockUser.name }}")
       ); ?>
						<# } else { #>
							<?php echo esc_html(sprintf($l10n["locked"], "{{ data.lockUser.name }}")); ?>
						<# } #>
					</p>
					<p class="notice notice-error notice-alt" hidden></p>
					<p class="action-buttons">
						<# if ( data.returnUrl !== data.previewUrl ) { #>
							<a class="button customize-notice-go-back-button" href="{{ data.returnUrl }}"><?php _e(
           "Go back"
       ); ?></a>
						<# } #>
						<a class="button customize-notice-preview-button" href="{{ data.frontendPreviewUrl }}"><?php _e(
          "Preview"
      ); ?></a>
						<# if ( data.allowOverride ) { #>
							<button class="button button-primary wp-tab-last customize-notice-take-over-button"><?php _e(
           "Take over"
       ); ?></button>
						<# } #>
					</p>
		...
		<?php /* The following template is obsolete in core but retained for plugins. */
        ?>
		...
		<script type="text/html" id="tmpl-customize-preview-link-control" >
			...
			<p class="description customize-control-description"><?php esc_html_e(
     ...
     esc_html_e("Preview Link"); ?>
      ...
      _e("(opens in a new tab)"); ?>
				...
				<button class="customize-copy-preview-link preview-control-element button button-secondary" data-component="button" data-copy-text="<?php esc_attr_e(
        "Copy"
    ); ?>" data-copied-text="<?php esc_attr_e("Copied"); ?>" ><?php esc_html_e(
    "Copy"
); ?></button>
			</div>
		</script>
    ...
    public function prepare_controls()
    {
        ...
        foreach ($this->controls as $id => $control) {
            if (
                !isset($this->sections[$control->section]) ||
                !$control->check_capabilities()
            ) {
                continue;
            }
        ...
        foreach ($this->sections as $section) {
            ...
            if (!$section->panel) {
                ...
                if (isset($this->panels[$section->panel])) {
                    $this->panels[$section->panel]->sections[
                        $section->id
                    ] = $section;
                }
            }
    ...
    {
        return wp_is_mobile() &&
            preg_match("/iPad|iPod|iPhone/", $_SERVER["HTTP_USER_AGENT"]);
    }
    ...
    public function is_cross_domain()
    {
        ...
        $cross_domain =
            strtolower($admin_origin["host"]) !==
            strtolower($home_origin["host"]);
        return $cross_domain;
    }
    ...
    public function get_allowed_urls()
    {
         ...
         */
        $allowed_urls = array_unique(
            apply_filters("customize_allowed_urls", $allowed_urls)
        );
    ...
    public function get_return_url()
    {
        ...
        if ($this->return_url) {
            ...
            $return_url_basename = wp_basename(
                parse_url($this->return_url, PHP_URL_PATH)
            );
            ...
            if ("themes.php" === $return_url_basename && $return_url_query) {
                ...
                if (
                    ...
                    !isset(
                        $_registered_pages[
                            "appearance_page_{$query_vars["page"]}"
                        ]
                    )
                ) {
            ...
            $referer &&
            !in_array(
                wp_basename(parse_url($referer, PHP_URL_PATH)),
    ...
    public function set_autofocus($autofocus)
    {
        $this->autofocus = array_filter(
            wp_array_slice_assoc($autofocus, ["panel", "section", "control"]),
            "is_string"
        );
    }
    ...
    public function get_nonces()
    {
        $nonces = [
            ...
            "preview" => wp_create_nonce(
                "preview-customize_" . $this->get_stylesheet()
            ),
            "switch_themes" => wp_create_nonce("switch_themes"),
            "dismiss_autosave_or_lock" => wp_create_nonce(
                "customize_dismiss_autosave_or_lock"
            ),
            "override_lock" => wp_create_nonce(
                "customize_override_changeset_lock"
            ),
    ...
    public function customize_pane_settings()
    {
        ...
        if (!$this->saved_starter_content_changeset && !$this->autosaved()) {
            if ($changeset_post_id) {
                if (is_user_logged_in()) {
                    $autosave_revision_post = wp_get_post_autosave(
                        $changeset_post_id,
                        get_current_user_id()
                    );
                }
        ...
        $settings = [
            "changeset" => [
                ...
                "latestAutoDraftUuid" => $autosave_autodraft_post
                    ? $autosave_autodraft_post->post_name
                    : null,
   ...
   ?>
    ...
    public function register_controls()
    {
        ...
        $this->add_control(
            new WP_Customize_Cropped_Image_Control($this, "custom_logo", [
                ...
                "section" => "title_tagline",
                "priority" => 8,
                "height" => isset($custom_logo_args[0]["height"])
                    ? $custom_logo_args[0]["height"]
                    : null,
                "width" => isset($custom_logo_args[0]["width"])
                    ? $custom_logo_args[0]["width"]
                    : null,
                "flex_height" => isset($custom_logo_args[0]["flex-height"])
                    ? $custom_logo_args[0]["flex-height"]
                    : null,
                "flex_width" => isset($custom_logo_args[0]["flex-width"])
                    ? $custom_logo_args[0]["flex-width"]
                    : null,
                "button_labels" => [
         ...
         */
        if (
            get_theme_support("custom-background", "wp-head-callback") ===
            ...
            foreach (
                ...
                $this->get_setting("background_" . $prop)->transport =
                    "postMessage";
            }
        }
        ...
        $section_description .=
            '<p id="editor-keyboard-trap-help-1">' .
        ...
        $section_description .=
            '<button type="button" class="button-link section-description-close">' .
            __("Close") .
            "</button>";
        $section_description .= "</p>";
    ...
    public function handle_load_themes_request()
    {
        ...
        if ("installed" === $theme_action) {
            ...
            foreach (wp_prepare_themes_for_js() as $theme) {
                ...
                $theme["active"] =
                    isset($_POST["customized_theme"]) &&
                    $_POST["customized_theme"] === $theme["id"];
                $themes["themes"][] = $theme;
            }
            ...
            $themes_allowedtags["a"] = array_fill_keys(
                ["href", "title", "target"],
                true
            );
            $themes_allowedtags["acronym"]["title"] = true;
            $themes_allowedtags["abbr"]["title"] = true;
            $themes_allowedtags["img"] = array_fill_keys(
                ["src", "class", "alt"],
                true
            );
            ...
            foreach ($themes->themes as &$theme) {
                ...
                $theme->active =
                    isset($_POST["customized_theme"]) &&
                    $_POST["customized_theme"] === $theme->slug;

                // Map available theme properties to installed theme properties.
                $theme->id = $theme->slug;
                $theme->screenshot = [$theme->screenshot_url];
                $theme->authorAndUri = wp_kses(
                    $theme->author["display_name"],
                    $themes_allowedtags
                );
                $theme->compatibleWP = is_wp_version_compatible(
                    $theme->requires
                ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName
                $theme->compatiblePHP = is_php_version_compatible(
                    $theme->requires_php
                ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName
    ...
    public function _sanitize_background_setting($value, $setting)
    {
        if ("background_repeat" === $setting->id) {
            if (
                ...
                return new WP_Error(
                    "invalid_value",
                    __("Invalid value for background repeat.")
                );
            }
            ...
            if (!in_array($value, ["fixed", "scroll"], true)) {
                return new WP_Error(
                    "invalid_value",
                    __("Invalid value for background attachment.")
                );
            }
            ...
            if (!in_array($value, ["left", "center", "right"], true)) {
                return new WP_Error(
                    "invalid_value",
                    __("Invalid value for background position X.")
                );
            }
            ...
            if (!in_array($value, ["top", "center", "bottom"], true)) {
                return new WP_Error(
                    "invalid_value",
                    __("Invalid value for background position Y.")
                );
            }
            ...
            if (!in_array($value, ["auto", "contain", "cover"], true)) {
                return new WP_Error(
                    "invalid_value",
                    __("Invalid value for background size.")
                );
            }
            ...
            if (
                !in_array(
                    $value,
                    ["default", "fill", "fit", "repeat", "custom"],
                    true
                )
            ) {
                return new WP_Error(
            ...
            "background_image" === $setting->id ||
            "background_image_thumb" === $setting->id
        ) {
            $value = empty($value) ? "" : sanitize_url($value);
        } else {
            return new WP_Error(
                "unrecognized_setting",
                __("Unrecognized background setting.")
            );
        }
        return $value;
    }
     ...
     * @return array
     */
    public function export_header_video_settings(
        $response,
        $selective_refresh,
        $partials
    ) {
    ...
    public function _validate_header_video($validity, $value)
    {
        ...
        if ($video) {
            ...
            if (
                !str_ends_with($video, ".mp4") &&
                !str_ends_with($video, ".mov")
            ) {
    ...
    public function _validate_external_header_video($validity, $value)
    {
        ...
        if ($video) {
            if (
                ...
                $validity->add(
                    "invalid_url",
                    __("Please enter a valid YouTube URL.")
                );
            }
        }
        return $validity;
