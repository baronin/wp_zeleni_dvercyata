...
function generate_block_asset_handle($block_name, $field_name, $index = 0)
{
    ...
    $asset_handle =
        str_replace("/", "-", $block_name) . "-" . $field_mappings[$field_name];
    if ($index > 0) {
        $asset_handle .= "-" . ($index + 1);
    }
    return $asset_handle;
...
function get_block_asset_url($path)
{
    ...
    if (!isset($template_paths_norm[$template])) {
        $template_paths_norm[$template] = wp_normalize_path(
            realpath(get_template_directory())
        );
    }

    if (
        str_starts_with($path, trailingslashit($template_paths_norm[$template]))
    ) {
        return get_theme_file_uri(
            str_replace($template_paths_norm[$template], "", $path)
        );
    }
    ...
    if (is_child_theme()) {
        ...
        if (!isset($template_paths_norm[$stylesheet])) {
            $template_paths_norm[$stylesheet] = wp_normalize_path(
        ...
        if (
            str_starts_with(
                $path,
                trailingslashit($template_paths_norm[$stylesheet])
            )
        ) {
            return get_theme_file_uri(
                str_replace($template_paths_norm[$stylesheet], "", $path)
            );
        }
    }
...
function register_block_script_module_id($metadata, $field_name, $index = 0)
{
    ...
    $module_asset_raw_path =
        $path .
        "/" .
        substr_replace($module_path, ".asset.php", -strlen(".js"));
    $module_id = generate_block_asset_handle(
        $metadata["name"],
        $field_name,
        $index
    );
    ...
    $module_asset = !empty($module_asset_path)
        ? require $module_asset_path
        : [];
    $module_dependencies = isset($module_asset["dependencies"])
        ? $module_asset["dependencies"]
        : [];
    $block_version = isset($metadata["version"]) ? $metadata["version"] : false;
    $module_version = isset($module_asset["version"])
...
function register_block_script_handle($metadata, $field_name, $index = 0)
{
    ...
    $script_asset = !empty($script_asset_path)
    ...
    $script_dependencies = isset($script_asset["dependencies"])
    ...
    if (
        ...
        in_array("wp-i18n", $script_dependencies, true)
    ) {
...
function register_block_style_handle($metadata, $field_name, $index = 0)
{
    ...
    $style_handle_name = generate_block_asset_handle(
        $metadata["name"],
        $field_name,
        $index
    );
    ...
    $is_core_block =
        isset($metadata["file"]) &&
        str_starts_with($metadata["file"], $wpinc_path_norm);
    ...
    if ($is_core_block) {
        $style_path =
            "editorStyle" === $field_name
                ? "editor{$suffix}.css"
                : "style{$suffix}.css";
    }

    $style_path_norm = wp_normalize_path(
        realpath(dirname($metadata["file"]) . "/" . $style_path)
    );
    $style_uri = get_block_asset_url($style_path_norm);

    $version =
        !$is_core_block && isset($metadata["version"])
            ? $metadata["version"]
            : false;
...
function wp_register_block_types_from_metadata_collection($path, $manifest = "")
{
    ...
    $block_metadata_files = WP_Block_Metadata_Registry::get_collection_block_metadata_files(
        $path
    );
...
function register_block_type_from_metadata($file_or_folder, $args = [])
{
    ...
    if ($registry_metadata) {
        ...
        $metadata = wp_json_file_decode($metadata_file, [
            "associative" => true,
        ]);
    } else {
        $metadata = [];
    ...
    if (
        !is_array($metadata) ||
        (empty($metadata["name"]) && empty($args["name"]))
    ) {
        return false;
    }

    $metadata["file"] = $metadata_file_exists
        ? wp_normalize_path(realpath($metadata_file))
        : null;
    ...
    $textdomain = !empty($metadata["textdomain"])
        ? $metadata["textdomain"]
        : null;
    ...
    if (!empty($metadata["render"])) {
        ...
        if ($template_path) {
             ...
             */
            $settings["render_callback"] = static function (
                $attributes,
                $content,
                $block
            ) use ($template_path) {
    ...
    if (!empty($metadata["variations"]) && is_string($metadata["variations"])) {
        ...
        if ($variations_path) {
             ...
             */
            $settings["variation_callback"] = static function () use (
                $variations_path
            ) {
    ...
    if (!empty($metadata["blockHooks"])) {
        ...
        foreach ($metadata["blockHooks"] as $anchor_block_name => $position) {
            ...
            $settings["block_hooks"][$anchor_block_name] =
    ...
    $metadata["name"] = !empty($settings["name"])
        ? $settings["name"]
        : $metadata["name"];
...
function has_block($block_name, $post = null)
{
    ...
    if (!$has_block) {
        ...
        if ($serialized_block_name !== $block_name) {
            $has_block = str_contains(
                $post,
                "<!-- wp:" . $serialized_block_name . " "
            );
        }
    }
...
function get_hooked_blocks()
{
    ...
    foreach ($block_types as $block_type) {
        if (
            ...
            !is_array($block_type->block_hooks)
        ) {
            continue;
        }
        foreach (
            $block_type->block_hooks
            as $anchor_block_type => $relative_position
        ) {
            if (!isset($hooked_blocks[$anchor_block_type])) {
                $hooked_blocks[$anchor_block_type] = [];
            }
            if (
                !isset($hooked_blocks[$anchor_block_type][$relative_position])
            ) {
...
function insert_hooked_blocks(
    ...
    foreach ($hooked_block_types as $hooked_block_type) {
         ...
         *                                                             or pattern that the anchor block belongs to.
         */
        $parsed_hooked_block = apply_filters(
            "hooked_block",
            $parsed_hooked_block,
            $hooked_block_type,
            $relative_position,
        ...
        if (
            ...
            !in_array(
                $hooked_block_type,
                $parsed_anchor_block["attrs"]["metadata"][
                    "ignoredHookedBlocks"
                ],
                true
            )
 ...
 */
function set_ignored_hooked_blocks_metadata(
    &$parsed_anchor_block,
    $relative_position,
    $hooked_blocks,
    $context
    ...
    $previously_ignored_hooked_blocks = isset(
        $parsed_anchor_block["attrs"]["metadata"]["ignoredHookedBlocks"]
    )
        ? $parsed_anchor_block["attrs"]["metadata"]["ignoredHookedBlocks"]
        : [];
    ...
    $parsed_anchor_block["attrs"]["metadata"][
        ...
        array_merge($previously_ignored_hooked_blocks, $hooked_block_types)
    );
...
function apply_block_hooks_to_content(
    ...
    if (!empty($hooked_blocks) || has_filter("hooked_block_types")) {
        $before_block_visitor = make_before_block_visitor(
            $hooked_blocks,
            $context,
            $callback
        );
        $after_block_visitor = make_after_block_visitor(
            $hooked_blocks,
            $context,
            $callback
        );
    ...
    foreach ($hooked_blocks as $anchor_block_type => $relative_positions) {
        foreach (
            $relative_positions
            as $relative_position => $hooked_block_types
        ) {
            foreach ($hooked_block_types as $index => $hooked_block_type) {
                $hooked_block_type_definition = WP_Block_Type_Registry::get_instance()->get_registered(
                    $hooked_block_type
                );
     ...
     */
    $suppress_single_instance_blocks = static function (
        $hooked_block_types
    ) use (&$block_allows_multiple_instances, $content) {
        ...
        foreach ($hooked_block_types as $index => $hooked_block_type) {
            ...
            if (
                in_array(
                    $hooked_block_type,
                    $single_instance_blocks_present_in_content,
                    true
                ) ||
                has_block($hooked_block_type, $content)
            ) {
...
function apply_block_hooks_to_content_from_post_object(
    ...
    $ignored_hooked_blocks = get_post_meta(
        $post->ID,
     ...
     */
    $suppress_blocks_from_insertion_before_and_after_wrapper_block = static function (
        $hooked_block_types,
        $relative_position,
        $anchor_block_type
    ) use ($wrapper_block_type) {
        if (
            $wrapper_block_type === $anchor_block_type &&
            in_array($relative_position, ["before", "after"], true)
        ) {
            return [];
        }
        return $hooked_block_types;
    ...
    if (!empty($content_wrapped_in_classic_block)) {
         ...
         */
        $content = str_replace(
            $content_wrapped_in_classic_block,
            $original_content,
            $content
        );
    }
...
function extract_serialized_parent_block($serialized_block)
{
    $start = strpos($serialized_block, "-->") + strlen("-->");
    $end = strrpos($serialized_block, "<!--");
    return substr($serialized_block, 0, $start) .
        substr($serialized_block, $end);
}
...
function update_ignored_hooked_blocks_postmeta($post)
{
    ...
    $serialized_block = apply_block_hooks_to_content(
        $markup,
        $context,
    ...
    if (!empty($ignored_hooked_blocks)) {
        $existing_ignored_hooked_blocks = get_post_meta(
            $post->ID,
            "_wp_ignored_hooked_blocks",
            true
        );
        if (!empty($existing_ignored_hooked_blocks)) {
            $existing_ignored_hooked_blocks = json_decode(
                $existing_ignored_hooked_blocks,
                true
            );
            $ignored_hooked_blocks = array_unique(
                array_merge(
        ...
        $post->meta_input["_wp_ignored_hooked_blocks"] = json_encode(
            $ignored_hooked_blocks
        );
    }
 ...
 * @return string
 */
function insert_hooked_blocks_and_set_ignored_hooked_blocks_metadata(
    &$parsed_anchor_block,
    $relative_position,
    $hooked_blocks,
    $context
) {
    $markup = insert_hooked_blocks(
        $parsed_anchor_block,
        $relative_position,
        $hooked_blocks,
        $context
    );
    $markup .= set_ignored_hooked_blocks_metadata(
        $parsed_anchor_block,
        $relative_position,
        $hooked_blocks,
        $context
 ...
 */
function make_before_block_visitor(
    $hooked_blocks,
    $context,
    $callback = "insert_hooked_blocks"
) {
     ...
     */
    return function (&$block, &$parent_block = null, $prev = null) use (
        $hooked_blocks,
        $context,
        $callback
    ) {
        ...
        if ($parent_block && !$prev) {
            ...
            $markup .= call_user_func_array($callback, [
                &$parent_block,
                "first_child",
                $hooked_blocks,
                $context,
 ...
 */
function make_after_block_visitor(
    $hooked_blocks,
    $context,
    $callback = "insert_hooked_blocks"
) {
     ...
     */
    return function (&$block, &$parent_block = null, $next = null) use (
        $hooked_blocks,
        $context,
        $callback
    ) {
...
function serialize_block_attributes($block_attributes)
{
    ...
    $encoded_attributes = preg_replace(
        "/--/",
        "\\u002d\\u002d",
        $encoded_attributes
    );
...
function serialize_block($block)
{
    ...
    foreach ($block["innerContent"] as $chunk) {
        $block_content .= is_string($chunk)
            ? $chunk
            : serialize_block($block["innerBlocks"][$index++]);
    }
    ...
    foreach ($block["innerContent"] as $chunk) {
        if (is_string($chunk)) {
            ...
            if (is_callable($pre_callback)) {
                ...
                $block_content .= call_user_func_array($pre_callback, [
                    &$inner_block,
                    &$block,
                    $prev,
                ]);
            ...
            if (is_callable($post_callback)) {
                $next =
                    count($block["innerBlocks"]) - 1 === $block_index
                        ? null
                        : $block["innerBlocks"][$block_index + 1];

                $post_markup = call_user_func_array($post_callback, [
                    &$inner_block,
                    &$block,
                    $next,
                ]);
            ...
            $block_content .= traverse_and_serialize_block(
                $inner_block,
                $pre_callback,
                $post_callback
            );
 ...
 * @return string Serialized block markup.
 */
function traverse_and_serialize_blocks(
    $blocks,
    $pre_callback = null,
    $post_callback = null
) {
    ...
    foreach ($blocks as $index => $block) {
        if ($pre_callback_is_callable) {
            $prev = 0 === $index ? null : $blocks[$index - 1];

            $result .= call_user_func_array($pre_callback, [
                &$block,
                &$parent_block,
                $prev,
            ]);
        ...
        if ($post_callback_is_callable) {
            $next = count($blocks) - 1 === $index ? null : $blocks[$index + 1];
        ...
        $result .= traverse_and_serialize_block(
            $block,
            $pre_callback,
            $post_callback
        );
...
{
    $block["attrs"] = filter_block_kses_value(
        $block["attrs"],
        $allowed_html,
        $allowed_protocols,
        $block
 ...
 */
function filter_block_kses_value(
    $value,
    $allowed_html,
    $allowed_protocols = [],
    $block_context = null
    ...
    if (is_array($value)) {
        foreach ($value as $key => $inner_value) {
            $filtered_key = filter_block_kses_value(
                $key,
                $allowed_html,
                $allowed_protocols,
                $block_context
            ...
            if (
                isset($block_context["blockName"]) &&
                "core/template-part" === $block_context["blockName"]
            ) {
                $filtered_value = filter_block_core_template_part_attributes(
                    $filtered_value,
                    $filtered_key,
                    $allowed_html
                );
...
function excerpt_remove_blocks($content)
{
     ...
     */
    $allowed_wrapper_blocks = apply_filters(
    ...
    foreach ($blocks as $block) {
        if (in_array($block["blockName"], $allowed_blocks, true)) {
            if (!empty($block["innerBlocks"])) {
                if (
                    in_array($block["blockName"], $allowed_wrapper_blocks, true)
                ) {
...
{
    return WP_Block_Styles_Registry::get_instance()->register(
        $block_name,
        $style_properties
    );
}
...
{
    return WP_Block_Styles_Registry::get_instance()->unregister(
        $block_name,
        $block_style_name
    );
}
...
function wp_migrate_old_typography_shape($metadata)
{
    ...
    foreach ($typography_keys as $typography_key) {
        $support_for_key = isset($metadata["supports"][$typography_key])
            ? $metadata["supports"][$typography_key]
            : null;
...
function build_query_vars_from_query_block($block, $page)
{
    ...
    if (isset($block->context["query"])) {
        ...
        if (
            ...
            !empty($block->context["query"]["sticky"])
        ) {
        ...
        if (
            !empty($block->context["query"]["categoryIds"]) ||
            !empty($block->context["query"]["tagIds"])
        ) {
            $tax_query_back_compat = [];
            if (!empty($block->context["query"]["categoryIds"])) {
                $tax_query_back_compat[] = [
                    ...
                    "terms" => array_filter(
                        array_map(
                            "intval",
                            $block->context["query"]["categoryIds"]
                        )
                    ),
            ...
            if (!empty($block->context["query"]["tagIds"])) {
                $tax_query_back_compat[] = [
                    "taxonomy" => "post_tag",
                    "terms" => array_filter(
                        array_map("intval", $block->context["query"]["tagIds"])
                    ),
                    "include_children" => false,
                ];
            }
        ...
        if (
            ...
            if (!empty($formats)) {
                ...
                $terms = array_map(static function ($format) {
        ...
        if (
            ...
            in_array(
                strtoupper($block->context["query"]["order"]),
                ["ASC", "DESC"],
                true
            )
        ) {
        ...
        if (isset($block->context["query"]["author"])) {
            if (is_array($block->context["query"]["author"])) {
                $query["author__in"] = array_filter(
                    array_map("intval", $block->context["query"]["author"])
                );
            } elseif (is_string($block->context["query"]["author"])) {
                $query["author__in"] = array_filter(
                    array_map(
                        "intval",
                        explode(",", $block->context["query"]["author"])
                    )
                );
            } elseif (
                is_int($block->context["query"]["author"]) &&
                $block->context["query"]["author"] > 0
            ) {
        ...
        if (
            !empty($block->context["query"]["parents"]) &&
            is_post_type_hierarchical($query["post_type"])
        ) {
            $query["post_parent__in"] = array_unique(
                array_map("intval", $block->context["query"]["parents"])
            );
        }
    }
...
function get_query_pagination_arrow($block, $is_next)
{
    ...
    if (
        !empty($block->context["paginationArrow"]) &&
        array_key_exists($block->context["paginationArrow"], $arrow_map) &&
        !empty($arrow_map[$block->context["paginationArrow"]])
    ) {
        $pagination_type = $is_next ? "next" : "previous";
        $arrow_attribute = $block->context["paginationArrow"];
        $arrow =
            $arrow_map[$block->context["paginationArrow"]][$pagination_type];
    ...
    if (
        ...
        if ($per_page > 0) {
            ...
            if ($page) {
                ...
                $max_num_pages = (int) (new WP_Comment_Query($comment_args))
                    ->max_num_pages;
                if (0 !== $max_num_pages) {
...
function get_comments_pagination_arrow($block, $pagination_type = "next")
{
    ...
    if (
        !empty($block->context["comments/paginationArrow"]) &&
        !empty(
            $arrow_map[$block->context["comments/paginationArrow"]][
                $pagination_type
            ]
        )
    ) {
    ...
    foreach ($footnotes_decoded as $footnote) {
        if (!empty($footnote["content"]) && !empty($footnote["id"])) {
            $footnotes_sanitized[] = [
                ...
                "content" => wp_unslash(
                    wp_filter_post_kses(wp_slash($footnote["content"]))
                ),
            ];
        }
    }
