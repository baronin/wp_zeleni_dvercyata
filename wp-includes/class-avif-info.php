...
class Features
{
    ...
    private function get_item_features($target_item_id, $tile_depth)
    {
        foreach ($this->props as $prop) {
            ...
            if (
                $target_item_id == $this->primary_item_id &&
                ...
                foreach ($this->dim_props as $dim_prop) {
                    ...
                    if (
                        $this->primary_item_features["bit_depth"] !=
                            UNDEFINED &&
            ...
            // already done.
            if (
                $this->primary_item_features["bit_depth"] == UNDEFINED ||
...
class Parser
{
    ...
    private function parse_ipco($num_remaining_bytes)
    {
        ...
        do {
            ...
            if ($box->type == "ispe") {
                ...
                if (
                    count($this->features->dim_props) <= MAX_FEATURES &&
                    $box_index <= MAX_VALUE
                ) {
                    $dim_prop_count = count($this->features->dim_props);
                    $this->features->dim_props[
                        $dim_prop_count
                    ] = new Dim_Prop();
                    $this->features->dim_props[
                        $dim_prop_count
                    ]->property_index = $box_index;
                ...
                if (
                    ...
                    $this->features->chan_props[
                        $chan_prop_count
                    ]->bit_depth = $high_bitdepth ? ($twelve_bit ? 12 : 10) : 8;
                    $this->features->chan_props[
                        $chan_prop_count
                    ]->num_channels = $monochrome ? 1 : 3;
                } else {
                    $this->data_was_skipped = true;
                }
    ...
    private function parse_iprp($num_remaining_bytes)
    {
        do {
            ...
            if ($box->type == "ipco") {
                ...
                if (
                    !skip($this->handle, $box->content_size - $num_read_bytes)
                ) {
                    return TRUNCATED;
                }
            } else {
    ...
    private function parse_iref($num_remaining_bytes)
    {
        do {
            ...
            if ($box->type == "dimg") {
                ...
                $reference_count = read_big_endian(
                    substr($data, $num_bytes_per_id, 2),
                    2
                );
                ...
                for ($i = 0; $i < $reference_count; ++$i) {
                    ...
                    if (
                        $from_item_id <= MAX_VALUE &&
                        $to_item_id <= MAX_VALUE &&
                        $tile_count < MAX_TILES
                    ) {
                        $this->features->tiles[$tile_count] = new Tile();
                        $this->features->tiles[
                            $tile_count
                        ]->tile_item_id = $to_item_id;
                        $this->features->tiles[
                            $tile_count
                        ]->parent_item_id = $from_item_id;
                    } else {
                        $this->data_was_skipped = true;
                    }
                }
    ...
    private function parse_meta($num_remaining_bytes)
    {
        do {
            ...
            if ($box->type == "pitm") {
                ...
                if (
                    !skip($this->handle, $box->content_size - $num_bytes_per_id)
                ) {
                    return TRUNCATED;
                }
    ...
    public function parse_ftyp()
    {
        ...
        for ($i = 0; $i + 4 <= $box->content_size; $i += 4) {
            ...
            if (
                substr($data, 0, 4) == "avif" ||
                substr($data, 0, 4) == "avis"
            ) {
