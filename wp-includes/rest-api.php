...
function get_rest_url($blog_id = null, $path = "/", $scheme = "rest")
{
    ...
    if (
        (is_multisite() && get_blog_option($blog_id, "permalink_structure")) ||
    ...
    if (is_ssl() && isset($_SERVER["SERVER_NAME"])) {
        ...
        if (
            parse_url(get_home_url($blog_id), PHP_URL_HOST) ===
            $_SERVER["SERVER_NAME"]
        ) {
...
function rest_get_server()
{
    ...
    if (empty($wp_rest_server)) {
         ...
         */
        $wp_rest_server_class = apply_filters(
...
function rest_handle_deprecated_function($function_name, $replacement, $version)
{
    ...
    if (!empty($replacement)) {
        ...
        $string = sprintf(
            __('%1$s (since %2$s; use %3$s instead)'),
            $function_name,
            $version,
            $replacement
        );
        ...
        $string = sprintf(
            __('%1$s (since %2$s; no alternative available)'),
            $function_name,
            $version
        );
    }
...
function rest_send_allow_header($response, $server, $request)
{
    ...
    if ($allowed_methods) {
        $response->header(
            "Allow",
            implode(", ", array_map("strtoupper", array_keys($allowed_methods)))
        );
    }
...
function _rest_array_intersect_key_recursive($array1, $array2)
{
    ...
    foreach ($array1 as $key => $value) {
        if (is_array($value) && is_array($array2[$key])) {
            $array1[$key] = _rest_array_intersect_key_recursive(
                $value,
                $array2[$key]
            );
        }
...
function rest_filter_response_fields($response, $server, $request)
{
    ...
    if (wp_is_numeric_array($data)) {
        ...
        foreach ($data as $item) {
            $new_data[] = _rest_array_intersect_key_recursive(
                $item,
                $fields_as_keyed
            );
        }
...
function rest_output_rsd()
{
    ...
    ?>
	<api name="WP-API" blogID="1" preferred="false" apiLink="<?php echo esc_url(
     $api_root
 ); ?>" />
	<?php
}
...
function rest_output_link_header()
{
    ...
    header(
        sprintf(
            'Link: <%s>; rel="https://api.w.org/"',
            sanitize_url($api_root)
        ),
        false
    );
...
{
    return is_numeric($maybe_integer) &&
...
function rest_format_combining_operation_error($param, $error)
{
    ...
    if (isset($error["schema"]["title"])) {
        ...
        return new WP_Error(
            ...
            sprintf(
                __('%1$s is not a valid %2$s. Reason: %3$s'),
                $param,
                $title,
                $reason
            ),
    ...
    return new WP_Error(
        ...
        sprintf(
            __('%1$s does not match the expected format. Reason: %2$s'),
            $param,
            $reason
        ),
        ["position" => $position]
...
function rest_get_combining_operation_error($value, $param, $errors)
{
    ...
    foreach ($errors as $error) {
        ...
        if (
            "rest_invalid_type" !== $error_code ||
            (isset($error_data["param"]) && $param !== $error_data["param"])
        ) {
    ...
    if (
        ...
        foreach ($filtered_errors as $error) {
            if (isset($error["schema"]["properties"])) {
                $n = count(
                    array_intersect_key($error["schema"]["properties"], $value)
                );
                if ($n > $number) {
                    $result = $error;
                    $number = $n;
    ...
    if (count($matching_schemas) > 1) {
        ...
        if (count($schema_titles) === count($matching_schemas)) {
            return new WP_Error(
                ...
                /* translators: 1: Parameter, 2: Schema titles. */
                wp_sprintf(
                    __('%1$s matches %2$l, but should match only one.'),
                    $param,
                    $schema_titles
                ),
        ...
        return new WP_Error(
            ...
            /* translators: %s: Parameter. */
            sprintf(
                __("%s matches more than one of the expected formats."),
                $param
            ),
            ["positions" => $schema_positions]
        );
...
function rest_are_values_equal($value1, $value2)
{
    if (is_array($value1) && is_array($value2)) {
        ...
        foreach ($value1 as $index => $value) {
            if (
                !array_key_exists($index, $value2) ||
                !rest_are_values_equal($value, $value2[$index])
            ) {
                return false;
            }
        }
...
function rest_validate_enum($value, $args, $param)
{
    ...
    $encoded_enum_values = [];
    foreach ($args["enum"] as $enum_value) {
        $encoded_enum_values[] = is_scalar($enum_value)
            ? $enum_value
            : wp_json_encode($enum_value);
    }
    ...
    if (count($encoded_enum_values) === 1) {
        ...
        return new WP_Error(
            "rest_not_in_enum",
            wp_sprintf(__('%1$s is not %2$s.'), $param, $encoded_enum_values[0])
        );
    }
...
function rest_validate_value_from_schema($value, $args, $param = "")
{
    ...
    if (!isset($args["type"])) {
        ...
        _doing_it_wrong(
            __FUNCTION__,
            sprintf(
                __('The "type" schema keyword for %s is required.'),
                $param
            ),
            "5.5.0"
        );
    ...
    switch ($args["type"]) {
        ...
        case "boolean":
            ...
            break;
        case "object":
            $is_valid = rest_validate_object_value_from_schema(
                $value,
                $args,
                $param
            );
        ...
        case "array":
            ...
            break;
        case "number":
            $is_valid = rest_validate_number_value_from_schema(
                $value,
                $args,
                $param
            );
            break;
        case "string":
            $is_valid = rest_validate_string_value_from_schema(
                $value,
                $args,
                $param
            );
            break;
        case "integer":
            $is_valid = rest_validate_integer_value_from_schema(
                $value,
                $args,
                $param
            );
    ...
    if (
        ...
        (!isset($args["type"]) ||
            "string" === $args["type"] ||
            !in_array($args["type"], $allowed_types, true))
    ) {
        switch ($args["format"]) {
            case "hex-color":
            ...
            case "ip":
                if (!rest_is_ip_address($value)) {
                    ...
                    return new WP_Error(
                        "rest_invalid_ip",
                        sprintf(__("%s is not a valid IP address."), $param)
                    );
                }
                break;
            case "uuid":
                if (!wp_is_uuid($value)) {
                    ...
                    return new WP_Error(
                        "rest_invalid_uuid",
                        sprintf(__("%s is not a valid UUID."), $param)
                    );
                }
                break;
        }
...
function rest_validate_object_value_from_schema($value, $args, $param)
{
        ...
        foreach ($args["required"] as $name) {
            if (!array_key_exists($name, $value)) {
                return new WP_Error(
                    ...
                    sprintf(
                        __('%1$s is a required property of %2$s.'),
                        $name,
                        $param
                    )
                );
    ...
    foreach ($value as $property => $v) {
        ...
        $pattern_property_schema = rest_find_matching_pattern_property_schema(
            $property,
            $args
        );
        ...
        if (isset($args["additionalProperties"])) {
            if (false === $args["additionalProperties"]) {
                return new WP_Error(
                    ...
                    sprintf(
                        __('%1$s is not a valid property of Object.'),
                        $property
                    )
                );
            }
    ...
    if (
        isset($args["minProperties"]) &&
        count($value) < $args["minProperties"]
    ) {
        return new WP_Error(
...
function rest_validate_array_value_from_schema($value, $args, $param)
{
    ...
    if (
        !empty($args["uniqueItems"]) &&
        !rest_validate_array_contains_unique_items($value)
    ) {
        /* translators: %s: Parameter. */
        return new WP_Error(
            "rest_duplicate_items",
            sprintf(__("%s has duplicate items."), $param)
        );
    }
...
function rest_validate_number_value_from_schema($value, $args, $param)
{
    ...
    if (
        isset($args["multipleOf"]) &&
        fmod($value, $args["multipleOf"]) !== 0.0
    ) {
        return new WP_Error(
            "rest_invalid_multiple",
            /* translators: 1: Parameter, 2: Multiplier. */
            sprintf(
                __('%1$s must be a multiple of %2$s.'),
                $param,
                $args["multipleOf"]
            )
        );
    ...
    if (isset($args["minimum"]) && !isset($args["maximum"])) {
        if (!empty($args["exclusiveMinimum"]) && $value <= $args["minimum"]) {
            return new WP_Error(
                ...
                sprintf(
                    __('%1$s must be greater than %2$d'),
                    $param,
                    $args["minimum"]
                )
            );
        ...
        if (empty($args["exclusiveMinimum"]) && $value < $args["minimum"]) {
            return new WP_Error(
                ...
                /* translators: 1: Parameter, 2: Minimum number. */
                sprintf(
                    __('%1$s must be greater than or equal to %2$d'),
                    $param,
                    $args["minimum"]
                )
            );
    ...
    if (isset($args["maximum"]) && !isset($args["minimum"])) {
        ...
        if (empty($args["exclusiveMaximum"]) && $value > $args["maximum"]) {
            return new WP_Error(
                ...
                /* translators: 1: Parameter, 2: Maximum number. */
                sprintf(
                    __('%1$s must be less than or equal to %2$d'),
                    $param,
                    $args["maximum"]
                )
            );
...
function rest_validate_string_value_from_schema($value, $args, $param)
{
    ...
    if (
        isset($args["pattern"]) &&
        !rest_validate_json_schema_pattern($args["pattern"], $value)
    ) {
        return new WP_Error(
            "rest_invalid_pattern",
            /* translators: 1: Parameter, 2: Pattern. */
            sprintf(
                __('%1$s does not match pattern %2$s.'),
                $param,
                $args["pattern"]
            )
        );
...
{
    $is_valid_number = rest_validate_number_value_from_schema(
        $value,
        $args,
        $param
    );
...
function rest_sanitize_value_from_schema($value, $args, $param = "")
{
    ...
    if ("object" === $args["type"]) {
        ...
        foreach ($value as $property => $v) {
            if (isset($args["properties"][$property])) {
                $value[$property] = rest_sanitize_value_from_schema(
                    $v,
                    $args["properties"][$property],
                    $param . "[" . $property . "]"
                );
...
function rest_preload_api_request($memo, $path)
{
    ...
    if (200 === $response->status) {
        ...
        $embed = $request->has_param("_embed")
            ? rest_parse_embed_param($request["_embed"])
            : false;
...
function rest_filter_response_by_context($response_data, $schema, $context)
{
    if (isset($schema["anyOf"])) {
        $matching_schema = rest_find_any_matching_schema(
            $response_data,
            $schema,
            ""
        );
        if (!is_wp_error($matching_schema)) {
            ...
            $response_data = rest_filter_response_by_context(
                $response_data,
                $matching_schema,
                $context
            );
    ...
    if (isset($schema["oneOf"])) {
        $matching_schema = rest_find_one_matching_schema(
            $response_data,
            $schema,
            "",
            true
    ...
    $is_array_type =
        "array" === $type ||
        (is_array($type) && in_array("array", $type, true));
    $is_object_type =
        "object" === $type ||
    ...
    foreach ($response_data as $key => $value) {
        ...
        if (!in_array($context, $check["context"], true)) {
            ...
            $new_value = rest_filter_response_by_context(
                $value,
                $check,
                $context
            );
...
function rest_default_additional_properties_to_false($schema)
{
    ...
    if (in_array("object", $type, true)) {
        if (isset($schema["properties"])) {
            foreach ($schema["properties"] as $key => $child_schema) {
                $schema["properties"][
                    $key
                ] = rest_default_additional_properties_to_false($child_schema);
            }
        }
    ...
    if (in_array("array", $type, true)) {
        if (isset($schema["items"])) {
            $schema["items"] = rest_default_additional_properties_to_false(
                $schema["items"]
            );
        }
    }
...
function rest_get_route_for_post_type_items($post_type)
{
    ...
    $namespace = !empty($post_type->rest_namespace)
        ? $post_type->rest_namespace
        : "wp/v2";
...
function rest_get_route_for_taxonomy_items($taxonomy)
{
    ...
    $namespace = !empty($taxonomy->rest_namespace)
...
function rest_get_endpoint_args_for_schema(
    ...
    $schema_properties = !empty($schema["properties"])
        ? $schema["properties"]
        : [];
    $endpoint_args = [];
    $valid_schema_properties = rest_get_allowed_schema_keywords();
    $valid_schema_properties = array_diff($valid_schema_properties, [
        "default",
        "required",
    ]);
    ...
    foreach ($schema_properties as $field_id => $params) {
        ...
        if (isset($params["arg_options"])) {
            ...
            $endpoint_args[$field_id] = array_merge(
                $endpoint_args[$field_id],
                $params["arg_options"]
            );
        }
...
function wp_is_rest_endpoint()
{
    ...
    if (!$is_rest_endpoint) {
        ...
        $is_rest_endpoint =
            isset($wp_rest_server) && $wp_rest_server->is_dispatching();
    }
