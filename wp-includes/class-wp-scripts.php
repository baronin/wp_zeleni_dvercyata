...
class WP_Scripts extends WP_Dependencies
{
    ...
    public function do_item($handle, $group = false)
    {
        ...
        if ($translations) {
            $translations = wp_get_inline_script_tag($translations, [
        ...
        $has_conditional_data =
            $conditional && $this->get_data($handle, "data");
        ...
        if (
            ...
            !($this->content_url && str_starts_with($src, $this->content_url))
        ) {
    ...
    public function print_inline_script(
        ...
        _deprecated_function(
            __METHOD__,
            "6.3.0",
            "WP_Scripts::get_inline_script_data() or WP_Scripts::get_inline_script_tag()"
        );
    ...
    {
        if (
            isset($this->registered[$handle]->args) &&
            1 === $this->registered[$handle]->args
        ) {
            $calculated_group = 1;
        } else {
    ...
    public function print_translations($handle, $display = true)
    {
        if (
            ...
            empty($this->registered[$handle]->textdomain)
        ) {
            return false;
        }
...
JS;
    ...
    public function add_data($handle, $key, $value)
    {
        ...
        if ("strategy" === $key) {
            if (!empty($value) && !$this->is_delayed_strategy($value)) {
                ...
                $this->is_delayed_strategy($value)
            ) {
                _doing_it_wrong(
                    __METHOD__,
                    sprintf(
    ...
    private function get_eligible_loading_strategy($handle)
    {
        ...
        $eligible_strategies = $this->filter_eligible_strategies(
            $handle,
            $initial_strategy
        );
        ...
        return in_array("async", $eligible_strategies, true)
            ? "async"
            : "defer";
    }
    ...
    private function has_inline_script($handle, $position = null)
    {
        ...
        return (bool) ($this->get_data($handle, "before") ||
