...
class Requests
{
    ...
    public static function request(
        ...
        $options["hooks"]->dispatch("requests.before_request", [
            &$url,
            &$headers,
            &$data,
            &$type,
        ...
        $options["hooks"]->dispatch("requests.before_parse", [
            &$response,
            $url,
            $headers,
            $data,
    ...
    public static function request_multiple($requests, $options = [])
    {
        ...
        if (!empty($options["hooks"])) {
            $options["hooks"]->register("transport.internal.parse_response", [
                static::class,
                "parse_multiple",
            ]);
        ...
        foreach ($requests as $id => &$request) {
            ...
            if ($request["options"]["hooks"] !== $options["hooks"]) {
                $request["options"]["hooks"]->register(
                    "transport.internal.parse_response",
                    [static::class, "parse_multiple"]
                );
        ...
        foreach ($responses as $id => &$response) {
            ...
            if (is_string($response)) {
                ...
                $request["options"]["hooks"]->dispatch(
                    "multiple.request.complete",
                    [&$response, $id]
                );
            }
    ...
    {
        if (
            InputValidator::is_string_or_stringable($path) === false &&
            is_bool($path) === false
        ) {
            throw InvalidArgument::create(
                1,
                '$path',
                "string|Stringable|bool",
                gettype($path)
            );
        }
    ...
    protected static function parse_response(
        ...
        if ($return->is_redirect() && $options["follow_redirects"] === true) {
            if (
                ...
                $options["redirected"] < $options["redirects"]
            ) {
    ...
    public static function compatible_gzinflate($gz_data)
    {
        ...
        if (substr($gz_data, 0, 4) === "\x50\x4b\x03\x04") {
            ...
            $zip_compressed_on_the_fly =
                (0x08 & $general_purpose_flag) === 0x08;
            ...
            // offsets:
            $first_file_start = array_sum(
                unpack("v2", substr($gz_data, 26, 4))
            );
            $decompressed = @gzinflate(
                substr($gz_data, 30 + $first_file_start)
            );
            if ($decompressed !== false) {
                return $decompressed;
            }
