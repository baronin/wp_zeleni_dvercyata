...
class IdnaEncoder
{
    ...
    public static function to_ascii($text)
    {
        ...
        if (self::is_ascii($text)) {
            ...
            throw new Exception(
                "Prepared string is too long",
                "idna.prepared_too_long",
                $text
            );
        ...
        if (strpos($text, self::ACE_PREFIX) === 0) {
            throw new Exception(
                "Provided string begins with ACE prefix",
                "idna.provided_is_prefixed",
                $text
            );
        }
    ...
    protected static function adapt($delta, $numpoints, $firsttime)
    {
        ...
        $max = floor(
            ((self::BOOTSTRAP_BASE - self::BOOTSTRAP_TMIN) *
                self::BOOTSTRAP_TMAX) /
                2
        );
        while ($delta > $max) {
            ...
            $delta = floor(
                $delta / (self::BOOTSTRAP_BASE - self::BOOTSTRAP_TMIN)
            );
            // let k = k + base
            $k += self::BOOTSTRAP_BASE;
        } // end
        ...
        return $k +
            floor(
                ((self::BOOTSTRAP_BASE - self::BOOTSTRAP_TMIN + 1) * $delta) /
                    ($delta + self::BOOTSTRAP_SKEW)
            );
    }
}
