...
final class Fsockopen implements Transport
{
    ...
    public function request($url, $headers = [], $data = [], $options = [])
    {
        ...
        $options["hooks"]->dispatch("fsockopen.remote_socket", [
            &$remote_socket,
        ]);
        ...
        $options["hooks"]->dispatch("fsockopen.remote_host_path", [
            &$path,
            $url,
        ]);
        ...
        if (
            !isset($case_insensitive_headers["Accept-Encoding"]) &&
        ...
        if (!$options["blocking"]) {
            fclose($socket);
            $fake_headers = "";
            $options["hooks"]->dispatch("fsockopen.after_request", [
                &$fake_headers,
            ]);
            return "";
        }
        ...
        if ($timeout_sec === $options["timeout"]) {
            ...
            $timeout_msec =
                (self::SECOND_IN_MICROSECONDS * $options["timeout"]) %
                self::SECOND_IN_MICROSECONDS;
        }
        ...
        while (!feof($socket)) {
            ...
            if ($doingbody) {
                $options["hooks"]->dispatch("request.progress", [
                    $block,
                    $size,
                    $this->max_bytes,
                ]);
    ...
    public function verify_certificate_from_context($host, $context)
    {
        ...
        if (
            empty($meta) ||
            empty($meta["ssl"]) ||
            empty($meta["ssl"]["peer_certificate"])
        ) {
            throw new Exception(
                rtrim($this->connect_error),
                "ssl.connect_error"
