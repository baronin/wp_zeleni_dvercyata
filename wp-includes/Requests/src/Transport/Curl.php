...
final class Curl implements Transport
{
    ...
    public function request($url, $headers = [], $data = [], $options = [])
    {
        ...
        if (
            curl_errno($this->handle) === CURLE_WRITE_ERROR ||
            curl_errno($this->handle) === CURLE_BAD_CONTENT_ENCODING
        ) {
    ...
    public function request_multiple($requests, $options)
    {
        ...
        if (
            InputValidator::has_array_access($requests) === false ||
            InputValidator::is_iterable($requests) === false
        ) {
            throw InvalidArgument::create(
                1,
                '$requests',
        ...
        foreach ($requests as $id => $request) {
            ...
            $subhandles[$id] = $subrequests[$id]->get_subrequest_handle(
                $request["url"],
                $request["headers"],
                $request["data"],
                $request["options"]
            );
            $request["options"]["hooks"]->dispatch("curl.before_multi_add", [
                &$subhandles[$id],
            ]);
        ...
        $request["options"]["hooks"]->dispatch("curl.before_multi_exec", [
            &$multihandle,
        ]);
            ...
            foreach ($to_process as $key => $done) {
                ...
                if ($done["result"] !== CURLE_OK) {
                    ...
                    $options["hooks"]->dispatch(
                        ...
                        [&$responses[$key], $requests[$key]]
                    );
                } else {
                    $responses[$key] = $subrequests[$key]->process_response(
                        $subrequests[$key]->response_data,
                        $options
                    );
                ...
                if (!is_string($responses[$key])) {
                    $options["hooks"]->dispatch("multiple.request.complete", [
                        &$responses[$key],
                        $key,
                    ]);
                }
        ...
        $request["options"]["hooks"]->dispatch("curl.after_multi_exec", [
            &$multihandle,
        ]);
    ...
    private function setup_handle($url, $headers, $data, $options)
    {
        ...
        if (
            is_int($options["connect_timeout"]) ||
            $this->version < self::CURL_7_16_2
        ) {
            curl_setopt(
                $this->handle,
            ...
            curl_setopt(
                ...
                round($options["connect_timeout"] * 1000)
            );
        }
    ...
    {
        $this->hooks->dispatch("request.progress", [
            $data,
            $this->response_bytes,
        ...
        if ($this->response_byte_limit) {
            ...
            if (
                ...
                // Limit the length
                $limited_length =
                    $this->response_byte_limit - $this->response_bytes;
    ...
    public static function test($capabilities = [])
    {
        ...
        if (
            isset($capabilities[Capability::SSL]) &&
