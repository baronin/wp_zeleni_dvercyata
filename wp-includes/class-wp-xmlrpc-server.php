...
class wp_xmlrpc_server extends IXR_Server
{
    ...
    public function __construct()
    {
        $this->methods = [
            ...
            "pingback.extensions.getPingbacks" =>
                "this:pingback_extensions_getPingbacks",
    ...
    public function initialise_blog_option_info()
    {
         ...
         */
        $this->blog_options = apply_filters(
            "xmlrpc_blog_options",
            $this->blog_options
        );
    }
    ...
    protected function minimum_args($args, $count)
    {
        if (!is_array($args) || count($args) < $count) {
            $this->error = new IXR_Error(
                400,
                __("Insufficient arguments passed to this XML-RPC method.")
            );
            return false;
        }
    ...
    protected function _prepare_taxonomy($taxonomy, $fields)
    {
        ...
        if (in_array("object_type", $fields, true)) {
            $_taxonomy["object_type"] = array_unique(
    ...
    protected function _prepare_term($term)
    {
        ...
        // Get term meta.
        $_term["custom_fields"] = $this->get_term_custom_fields(
            $_term["term_id"]
        );
    ...
    protected function _prepare_post($post, $fields)
    {
        ...
        if ($thumbnail_id) {
            $thumbnail_size = current_theme_supports("post-thumbnail")
                ? "post-thumbnail"
                : "thumbnail";
            $post_fields["post_thumbnail"] = $this->_prepare_media_item(
        ...
        if (in_array("post", $fields, true)) {
            ...
            $requested_fields = array_intersect_key(
                $post_fields,
                array_flip($fields)
            );
        ...
        if ($all_taxonomy_fields || in_array("terms", $fields, true)) {
            $post_type_taxonomies = get_object_taxonomies(
                $post["post_type"],
                "names"
            );
        ...
        if (in_array("enclosure", $fields, true)) {
            ...
            if (!empty($enclosures)) {
                $encdata = explode("\n", $enclosures[0]);
                $_post["enclosure"]["url"] = trim(
                    htmlspecialchars($encdata[0])
                );
    ...
    protected function _prepare_post_type($post_type, $fields)
    {
        ...
        if (in_array("taxonomies", $fields, true)) {
            $_post_type["taxonomies"] = get_object_taxonomies(
                $post_type->name,
                "names"
            );
        }
    ...
    protected function _prepare_media_item(
        ...
        $_media_item = [
            ...
            "date_created_gmt" => $this->_convert_date_gmt(
                $media_item->post_date_gmt,
                $media_item->post_date
            ),
    ...
    protected function _prepare_page($page)
    {
        ...
        $page_date_gmt = $this->_convert_date_gmt(
            $page->post_date_gmt,
            $page->post_date
        );
    ...
    {
        // Format page date.
        $comment_date_gmt = $this->_convert_date_gmt(
    ...
    public function wp_newPost($args)
    {
        ...
        if (
            isset($content_struct["post_date"]) &&
            !($content_struct["post_date"] instanceof IXR_Date)
        ) {
            $content_struct["post_date"] = $this->_convert_date(
         ...
         */
        if (
            isset($content_struct["post_date_gmt"]) &&
            !($content_struct["post_date_gmt"] instanceof IXR_Date)
        ) {
            if (
                ...
                $content_struct["post_date_gmt"] = $this->_convert_date(
    ...
    private function _toggle_sticky($post_data, $update = false)
    {
        ...
        if (
            ...
            !empty($post_data["post_password"])
        ) {
            // Error if the client tried to stick the post, otherwise, silently unstick.
            if (!empty($post_data["sticky"])) {
                return new IXR_Error(
                    401,
                    __("Sorry, you cannot stick a private post.")
                );
            }
    ...
    protected function _insert_post($user, $content_struct)
    {
        ...
        $post_data = wp_parse_args(
            array_intersect_key($content_struct, $defaults),
            $defaults
        );
        ...
        if ($update) {
            ...
            if (get_post_type($post_data["ID"]) !== $post_data["post_type"]) {
                return new IXR_Error(
                    401,
                    __("The post type may not be changed.")
                );
            }
        } else {
            if (
        ...
        if (!empty($post_data["post_date_gmt"])) {
            ...
            $date_created =
                rtrim($post_data["post_date_gmt"]->getIso(), "Z") . "Z";
        ...
        if (!empty($date_created)) {
            ...
            $post_data["post_date_gmt"] = iso8601_to_datetime(
                $date_created,
                "gmt"
            );
        ...
        if (!isset($post_data["ID"])) {
            $post_data["ID"] = get_default_post_to_edit(
                $post_data["post_type"],
                true
            )->ID;
        }
        ...
        if (isset($post_data["terms"]) || isset($post_data["terms_names"])) {
            ...
            if (isset($post_data["terms"]) && is_array($post_data["terms"])) {
                ...
                foreach ($taxonomies as $taxonomy) {
                    ...
                    if (
                        !current_user_can(
                            $post_type_taxonomies[$taxonomy]->cap->assign_terms
                        )
                    ) {
                        return new IXR_Error(
                            401,
            ...
            if (
                isset($post_data["terms_names"]) &&
                is_array($post_data["terms_names"])
            ) {
                ...
                foreach ($taxonomies as $taxonomy) {
                    ...
                    if (is_taxonomy_hierarchical($taxonomy)) {
                        ...
                        $tax_term_names_count = array_count_values(
                            $tax_term_names
                        );
                    ...
                    foreach ($term_names as $term_name) {
                        ...
                        if (!$term) {
                            ...
                            if (
                                !current_user_can(
                                    $post_type_taxonomies[$taxonomy]->cap
                                        ->edit_terms
                                )
                            ) {
                                return new IXR_Error(
        ...
        // Handle enclosures.
        $enclosure = isset($post_data["enclosure"])
            ? $post_data["enclosure"]
            : null;
        ...
        if (!$post_id) {
            if ($update) {
                return new IXR_Error(
                    401,
                    __("Sorry, the post could not be updated.")
                );
            } else {
                return new IXR_Error(
                    401,
                    __("Sorry, the post could not be created.")
                );
            }
        }
    ...
    public function wp_editPost($args)
    {
        ...
        if (isset($content_struct["if_not_modified_since"])) {
            ...
            if (
                ...
                return new IXR_Error(
                    409,
                    __("There is a revision of this post that is more recent.")
                );
            }
        }
        ...
        if (
            ...
            $post["post_date_gmt"] = $this->_convert_date(
                $post["post_date_gmt"]
            );
        }
    ...
    public function wp_newTerm($args)
    {
        ...
        if (isset($content_struct["parent"])) {
            if (!$taxonomy["hierarchical"]) {
                return new IXR_Error(
                    403,
                    __("This taxonomy is not hierarchical.")
                );
            }
        ...
        if (!$term) {
            return new IXR_Error(
                500,
                __("Sorry, the term could not be created.")
            );
        }

        // Add term meta.
        if (isset($content_struct["custom_fields"])) {
            $this->set_term_custom_fields(
                $term["term_id"],
    ...
    public function wp_getTerm($args)
    {
        ...
        if (!current_user_can("assign_term", $term_id)) {
            return new IXR_Error(
                401,
                __("Sorry, you are not allowed to assign this term.")
            );
        }
    ...
    public function wp_getUsers($args)
    {
        ...
        $query["number"] = isset($filter["number"])
            ? absint($filter["number"])
            : 50;
        $query["offset"] = isset($filter["offset"])
            ? absint($filter["offset"])
            : 0;
    ...
    public function wp_getProfile($args)
    {
        ...
        if (!current_user_can("edit_user", $user->ID)) {
            return new IXR_Error(
                401,
                __("Sorry, you are not allowed to edit your profile.")
            );
        }
    ...
    public function wp_getPage($args)
    {
        ...
        if (!current_user_can("edit_page", $page_id)) {
            return new IXR_Error(
                401,
                __("Sorry, you are not allowed to edit this page.")
            );
        }
    ...
    public function wp_getPages($args)
    {
        ...
        if (!current_user_can("edit_pages")) {
            return new IXR_Error(
                401,
                __("Sorry, you are not allowed to edit pages.")
            );
        }
    ...
    public function wp_deletePage($args)
    {
        ...
        if (!current_user_can("delete_page", $page_id)) {
            return new IXR_Error(
                401,
                __("Sorry, you are not allowed to delete this page.")
            );
        }
    ...
    public function wp_getPageList($args)
    {
        ...
        for ($i = 0; $i < $num_pages; $i++) {
            $page_list[$i]->dateCreated = $this->_convert_date(
    ...
    public function wp_getAuthors($args)
    {
        ...
        $authors = [];
        foreach (
            get_users(["fields" => ["ID", "user_login", "display_name"]])
            as $user
        ) {
            $authors[] = [
                "user_id" => $user->ID,
    ...
    public function wp_newCategory($args)
    {
        ...
        if (!current_user_can("manage_categories")) {
            return new IXR_Error(
                401,
                __("Sorry, you are not allowed to add a category.")
            );
        }
    ...
    public function wp_deleteCategory($args)
    {
        ...
        if (!current_user_can("delete_term", $category_id)) {
            return new IXR_Error(
                401,
                __("Sorry, you are not allowed to delete this category.")
            );
        }
    ...
    public function wp_getComments($args)
    {
        ...
        if (isset($struct["post_type"])) {
            ...
            if (
                !$post_type_object ||
                !post_type_supports($post_type_object->name, "comments")
            ) {
    ...
    public function wp_editComment($args)
    {
        ...
        if (!empty($content_struct["date_created_gmt"])) {
            ...
            $comment["comment_date_gmt"] = iso8601_to_datetime(
                $date_created,
                "gmt"
            );
        }
        ...
        if (!$result) {
            return new IXR_Error(
                500,
                __("Sorry, the comment could not be updated.")
            );
        }
    ...
    public function wp_newComment($args)
    {
        ...
        if (
            ...
            return new IXR_Error(
                403,
                __("Sorry, you are not allowed to comment on this post.")
            );
        }
        ...
        if ($logged_in) {
            ...
            if (get_option("require_name_email")) {
                if (
                    strlen($comment["comment_author_email"]) < 6 ||
                    "" === $comment["comment_author"]
                ) {
                    return new IXR_Error(
                        403,
                        __("Comment author name and email are required.")
                    );
        ...
        $comment["comment_parent"] = isset($content_struct["comment_parent"])
            ? absint($content_struct["comment_parent"])
            : 0;
    ...
    public function wp_getCommentCount($args)
    {
        ...
        if (!current_user_can("edit_post", $post_id)) {
            return new IXR_Error(
                403,
                __("Sorry, you are not allowed to access details of this post.")
            );
        }
    ...
    public function wp_setOptions($args)
    {
        ...
        foreach ($options as $o_name => $o_value) {
            ...
            update_option(
                $this->blog_options[$o_name]["option"],
                wp_unslash($o_value)
            );
        }
    ...
    public function wp_getMediaLibrary($args)
    {
        ...
        $parent_id = isset($struct["parent_id"])
            ? absint($struct["parent_id"])
            : "";
    ...
    public function wp_getPostTypes($args)
    {
        ...
        foreach ($post_types as $post_type) {
            ...
            $struct[$post_type->name] = $this->_prepare_post_type(
                $post_type,
                $fields
            );
        }
    ...
    public function blogger_getRecentPosts($args)
    {
        ...
        foreach ($posts_list as $entry) {
            ...
            $content =
                "<title>" . wp_unslash($entry["post_title"]) . "</title>";
            $content .= "<category>" . $categories . "</category>";
    ...
    public function blogger_editPost($args)
    {
        ...
        if (
            ...
            return new IXR_Error(
                401,
                __("Sorry, you are not allowed to publish this post.")
            );
        }
    ...
    public function mw_newPost($args)
    {
        ...
        if (!empty($content_struct["post_type"])) {
            if ("page" === $content_struct["post_type"]) {
                if ($publish) {
                    ...
                    isset($content_struct["page_status"]) &&
                ...
                if ($publish) {
                    ...
                    isset($content_struct["post_status"]) &&
        ...
        if (
            !current_user_can(
                get_post_type_object($post_type)->cap->create_posts
            )
        ) {
            return new IXR_Error(
                401,
                __("Sorry, you are not allowed to publish posts on this site.")
            );
        }
        ...
        if (isset($content_struct["wp_post_format"])) {
            $content_struct["wp_post_format"] = sanitize_key(
                $content_struct["wp_post_format"]
            );
            if (
                !array_key_exists(
                    $content_struct["wp_post_format"],
        ...
        if (
            ...
            $user->ID !== (int) $content_struct["wp_author_id"]
        ) {
            switch ($post_type) {
                case "post":
        ...
        $post_title = isset($content_struct["title"])
            ? $content_struct["title"]
            : "";
        $post_content = isset($content_struct["description"])
        ...
        $post_excerpt = isset($content_struct["mt_excerpt"])
            ? $content_struct["mt_excerpt"]
            : "";
        $post_more = isset($content_struct["mt_text_more"])
            ? $content_struct["mt_text_more"]
            : "";
            ...
            if (!is_numeric($content_struct["mt_allow_pings"])) {
                switch ($content_struct["mt_allow_pings"]) {
                    ...
                    case "open":
                        ...
                        break;
                    default:
                        $ping_status = get_default_comment_status(
                            $post_type,
                            "pingback"
                        );
                        break;
        ...
        if (isset($content_struct["wp_post_thumbnail"])) {
            if (
                set_post_thumbnail(
                    $post_id,
                    $content_struct["wp_post_thumbnail"]
                ) === false
            ) {
                return new IXR_Error(404, __("Invalid attachment ID."));
            }
        ...
        // Handle enclosures.
        $enclosure = isset($content_struct["enclosure"])
            ? $content_struct["enclosure"]
            : null;
        $this->add_enclosure_if_new($post_id, $enclosure);
    ...
    public function add_enclosure_if_new($post_id, $enclosure)
    {
        if (
            is_array($enclosure) &&
            isset($enclosure["url"]) &&
            isset($enclosure["length"]) &&
            isset($enclosure["type"])
        ) {
            $encstring =
                $enclosure["url"] .
                "\n" .
    ...
    public function attach_uploads($post_id, $post_content)
    {
        ...
        if (is_array($attachments)) {
            foreach ($attachments as $file) {
                if (
                    !empty($file->guid) &&
                    str_contains($post_content, $file->guid)
                ) {
                    $wpdb->update(
                        $wpdb->posts,
                        ["post_parent" => $post_id],
    ...
    public function mw_editPost($args)
    {
        ...
        if (
            ...
            $content_struct["post_type"] !== $postdata["post_type"]
        ) {
        ...
        $page_template = "";
        if (
            !empty($content_struct["wp_page_template"]) &&
            "page" === $post_type
        ) {
            $page_template = $content_struct["wp_page_template"];
        }
        ...
        if ("publish" === $post_status || "private" === $post_status) {
            if ("page" === $post_type && !current_user_can("publish_pages")) {
                return new IXR_Error(
                    401,
                    __("Sorry, you are not allowed to publish this page.")
                );
    ...
    public function mw_getRecentPosts($args)
    {
        ...
        foreach ($posts_list as $entry) {
            ...
            $recent_posts[] = [
                ...
                "date_modified_gmt" => $post_modified_gmt,
                "sticky" =>
                    "post" === $entry["post_type"] && is_sticky($entry["ID"]),
    ...
    public function mw_newMediaObject($args)
    {
        ...
        if (!empty($upload["error"])) {
            ...
            $error_string = sprintf(
                __('Could not write file %1$s (%2$s).'),
                $name,
                $upload["error"]
            );
    ...
    public function pingback_ping($args)
    {
        ...
        if (
            ...
            return $this->pingback_error(
                48,
                __("The pingback has already been registered.")
            );
        }
        ...
        if (empty($title)) {
            return $this->pingback_error(
                32,
                __("A title on that page cannot be found.")
            );
        }
    ...
    public function pingback_extensions_getPingbacks($url)
    {
        ...
        if (!$actual_post) {
            ...
            return $this->pingback_error(
                32,
                __("The specified target URL does not exist.")
            );
        }
