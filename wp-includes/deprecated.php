...
function user_can_edit_post($user_id, $post_id, $blog_id = 1)
{
    ...
    if (
        ($user_id == $post_author_data->ID &&
            !(
                $post->post_status == "publish" && $author_data->user_level < 2
            )) ||
...
function user_can_set_post_date($user_id, $blog_id = 1, $category_id = "None")
{
    ...
    $author_data = get_userdata($user_id);
    return $author_data->user_level > 4 &&
        user_can_create_post($user_id, $blog_id, $category_id);
}
...
function user_can_edit_post_date($user_id, $post_id, $blog_id = 1)
{
    ...
    $author_data = get_userdata($user_id);
    return $author_data->user_level > 4 &&
        user_can_edit_post($user_id, $post_id, $blog_id);
}
...
function user_can_edit_user($user_id, $other_user)
{
    ...
    if (
        $user->user_level > $other->user_level ||
        $user->user_level > 8 ||
        $user->ID == $other->ID
    ) {
        return true;
...
function dropdown_cats(
    ...
    if ($optionnone) {
        $show_option_none = _x(
            "None",
            "Categories dropdown (show_option_none parameter)"
        );
    }
...
function get_category_children($id, $before = "/", $after = "", $visited = [])
{
    ...
    foreach ((array) $cat_ids as $cat_id) {
        ...
        if (
            $category->parent == $id &&
            !in_array($category->term_id, $visited)
        ) {
            $visited[] = $category->term_id;
            $chain .= $before . $category->term_id . $after;
            $chain .= get_category_children(
                $category->term_id,
                $before,
                $after
            );
...
function make_url_footnote($content)
{
    ...
    for ($i = 0, $c = count($matches[0]); $i < $c; $i++) {
        ...
        $content = str_replace(
            $link_match,
            $link_text . " " . $link_number,
            $content
        );
        $link_url =
            strtolower(substr($link_url, 0, 7)) !== "http://" &&
            strtolower(substr($link_url, 0, 8)) !== "https://"
                ? get_option("home") . $link_url
                : $link_url;
...
function get_users_of_blog($id = "")
{
    ...
    $users = $wpdb->get_results(
        "SELECT user_id, user_id AS ID, user_login, display_name, user_email, meta_value FROM $wpdb->users, $wpdb->usermeta WHERE {$wpdb->users}.ID = {$wpdb->usermeta}.user_id AND meta_key = '{$blog_prefix}capabilities' ORDER BY {$wpdb->usermeta}.user_id"
    );
    return $users;
}
        ...
        if (!($user instanceof WP_User)) {
            ...
            foreach (array_keys($vars) as $field) {
                if (is_string($user->$field) || is_numeric($user->$field)) {
                    $user->$field = sanitize_user_field(
                        $field,
                        $user->$field,
                        $user->ID,
                        $context
...
function start_post_rel_link(
    ...
    echo get_boundary_post_rel_link(
        $title,
        $in_same_cat,
        $excluded_categories,
        true
...
function get_index_rel_link()
{
    ...
    $link =
        ...
        "' href='" .
        esc_url(user_trailingslashit(get_bloginfo("url", "display"))) .
        "' />\n";
...
function get_theme_data($theme_file)
{
    _deprecated_function(__FUNCTION__, "3.4.0", "wp_get_theme()");
    $theme = new WP_Theme(
        wp_basename(dirname($theme_file)),
        dirname(dirname($theme_file))
    );
...
function rich_edit_exists()
{
    ...
    if (!isset($wp_rich_edit_exists)) {
        $wp_rich_edit_exists = file_exists(
            ABSPATH . WPINC . "/js/tinymce/tinymce.js"
        );
    }
...
function wp_get_http($url, $file_path = false, $red = 1)
{
    ...
    if (
        "HEAD" == $options["method"] &&
        in_array($headers["response"], [301, 302]) &&
...
function popuplinks($text)
{
    ...
    $text = preg_replace(
        "/<a (.+?)>/i",
        "<a $1 target='_blank' rel='external'>",
        $text
    );
    return $text;
}
...
function _sort_nav_menu_items($a, $b)
{
    ...
    if (
        !isset($a->$_menu_item_sort_prop) ||
        !isset($b->$_menu_item_sort_prop)
    ) {
        return 0;
    }
...
function wp_sensitive_page_meta()
{
	...
	<?php wp_strict_cross_origin_referrer();
}
...
function wp_typography_get_css_variable_inline_style(
    ...
    $style_value = _wp_array_get(
        $attributes,
        ["style", "typography", $feature],
        false
    );
    if (!$style_value) {
        return;
...
function wp_get_loading_attr_default($context)
{
    ...
    if (
        "the_post_thumbnail" === $context ||
        "wp_get_attachment_image" === $context
    ) {
        /*
...
function block_core_navigation_submenu_build_css_colors(
    ...
    if (!is_null($named_text_color)) {
        ...
        array_push(
            $colors["css_classes"],
            "has-text-color",
            sprintf("has-%s-color", $named_text_color)
        );
...
function _wp_theme_json_webfonts_handler()
{
    ...
    $fn_get_webfonts_from_theme_json = static function () {
        ...
        if (is_admin() || wp_is_rest_endpoint()) {
            ...
            foreach ($variations as $variation) {
                ...
                $settings["typography"]["fontFamilies"] = array_unique(
                    $settings["typography"]["fontFamilies"]
                );
            }
        }
    ...
    $fn_validate_webfont = static function ($webfont) {
        ...
        if (
            ...
            trigger_error(
                __("Webfont font family must be a non-empty string.")
            );
        ...
        foreach ((array) $webfont["src"] as $src) {
            if (!is_string($src) || "" === trim($src)) {
                trigger_error(
                    __("Each webfont src must be a non-empty string.")
                );
        ...
        if (
            !is_string($webfont["font-weight"]) &&
            !is_int($webfont["font-weight"])
        ) {
            trigger_error(
                __(
    ...
    $fn_register_webfonts = static function () use (
        ...
        foreach ($fn_get_webfonts_from_theme_json() as $webfont) {
            ...
            $webfont["src"] = $fn_transform_src_into_uri(
                (array) $webfont["src"]
            );
     ...
     */
    $fn_build_font_face_css = static function (array $webfont) use (
        $fn_compile_src,
     ...
     */
    $fn_get_css = static function () use (
        &$registered_webfonts,
        $fn_order_src,
        $fn_build_font_face_css
    ) {
        $css = "";
     ...
     * @uses $fn_get_css To run the function that gets the CSS.
     */
    $fn_generate_and_enqueue_editor_styles = static function () use (
        $fn_get_css
    ) {
        // Generate the styles.
        $styles = $fn_get_css();
...
function print_embed_styles()
{
	...
	<style<?php echo $type_attr; ?>>
      ...
      ABSPATH . WPINC . "/css/wp-embed-template$suffix.css"
  ); ?>
	</style>
	<?php
}
