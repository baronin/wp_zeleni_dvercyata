...
function wp_dropdown_categories($args = "")
{
    ...
    if (
        !isset($parsed_args["pad_counts"]) &&
        $parsed_args["show_count"] &&
    ...
    $aria_describedby_attribute = $parsed_args["aria_describedby"]
        ? ' aria-describedby="' .
            esc_attr($parsed_args["aria_describedby"]) .
            '"'
        : "";
    ...
    if (
        empty($categories) &&
        !$parsed_args["hide_if_empty"] &&
        !empty($parsed_args["show_option_none"])
    ) {
        /**
        ...
        $output .=
            ...
            "' selected='selected'>$show_option_none</option>\n";
    }
...
function wp_list_categories($args = "")
{
    ...
    if ($parsed_args["hierarchical"]) {
        ...
        if ($parsed_args["exclude_tree"]) {
            $exclude_tree = array_merge(
                $exclude_tree,
                wp_parse_id_list($parsed_args["exclude_tree"])
            );
        }
    ...
    if (empty($categories)) {
        if (!empty($show_option_none)) {
            if ("list" === $parsed_args["style"]) {
                $output .=
                    '<li class="cat-item-none">' . $show_option_none . "</li>";
            } else {
        ...
        if (!empty($show_option_all)) {
            ...
            if (
                !in_array("post", $taxonomy_object->object_type, true) &&
                !in_array("page", $taxonomy_object->object_type, true)
            ) {
        ...
        if (
            empty($parsed_args["current_category"]) &&
            (is_category() || is_tax() || is_tag())
        ) {
            $current_term_object = get_queried_object();
            if (
                $current_term_object &&
                $parsed_args["taxonomy"] === $current_term_object->taxonomy
            ) {
...
function wp_generate_tag_cloud($tags, $args = "")
{
    ...
    if (isset($args["topic_count_text"])) {
        ...
        $translate_nooped_plural = _n_noop(
            $args["single_text"],
            $args["multiple_text"]
        );
    } else {
    ...
    foreach ($tags as $key => $tag) {
        ...
        if ($translate_nooped_plural) {
            ...
            $formatted_count = call_user_func(
                $args["topic_count_text_callback"],
                $real_count,
                $tag,
                $args
            );
        ...
        $tags_data[] = [
            ...
            "font_size" =>
                $args["smallest"] + ($count - $min_count) * $font_step,
            ...
            "show_count" => $args["show_count"]
                ? '<span class="tag-link-count"> (' . $real_count . ")</span>"
                : "",
        ];
    }
    ...
    foreach ($tags_data as $key => $tag_data) {
        ...
        $a[] = sprintf(
            ...
            esc_attr($class),
            esc_attr(
                str_replace(",", ".", $tag_data["font_size"]) . $args["unit"]
            ),
            $tag_data["aria_label"],
