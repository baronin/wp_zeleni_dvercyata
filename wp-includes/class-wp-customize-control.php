...
class WP_Customize_Control
{
    ...
    {
        if (
            isset($this->settings[$setting_key]) &&
            $this->settings[$setting_key] instanceof WP_Customize_Setting
        ) {
            return 'data-customize-setting-link="' .
                esc_attr($this->settings[$setting_key]->id) .
                '"';
        } else {
    ...
    protected function render_content()
    {
        ...
        $describedby_attr = !empty($this->description)
            ...
            : "";
        switch ($this->type) { case "checkbox": ?>
				<span class="customize-inside-control-row">
					...
					/>
					<label for="<?php echo esc_attr($input_id); ?>"><?php echo esc_html(
    $this->label
); ?></label>
          ...
          $input_id . "-radio-" . $value
            ...
            case "select":
				...
				<?php if (!empty($this->label)): ?>
					<label for="<?php echo esc_attr(
         $input_id
     ); ?>" class="customize-control-title"><?php echo esc_html(
    $this->label
); ?></label>
				<?php endif; ?>
				<?php if (!empty($this->description)): ?>
    ...
    if ($nav_menus_created_posts_setting && current_user_can("publish_pages")) {
        ...
        if ($auto_draft_page_options) {
            $dropdown = str_replace(
                "</select>",
                $auto_draft_page_options . "</select>",
                $dropdown
            );
        }
    }
        ...
        current_user_can("edit_theme_options")
         ...
         ?>
					<button type="button" class="button-link add-new-toggle">
						<?php /* translators: %s: Add Page label. */
      printf(__("+ %s"), get_post_type_object("page")->labels->add_new_item); ?>
					</button>
					<div class="new-content-item-wrapper">
						<label for="create-input-<?php echo esc_attr($this->id); ?>"><?php _e(
    "New page title"
); ?></label>
						<div class="new-content-item">
							<input type="text" id="create-input-<?php echo esc_attr(
           $this->id
    ...
    {
        ?>
		<script type="text/html" id="tmpl-customize-control-<?php echo esc_attr(
      $this->type
  ); ?>-content">
...
require_once ABSPATH .
    WPINC .
    "/customize/class-wp-customize-date-time-control.php";
...
require_once ABSPATH .
    WPINC .
    "/customize/class-wp-sidebar-block-editor-control.php";
