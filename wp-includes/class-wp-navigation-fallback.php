...
class WP_Navigation_Fallback
{
    ...
    public static function update_wp_navigation_post_schema($schema)
    {
        ...
        $schema["properties"]["status"]["context"] = array_merge(
            $schema["properties"]["status"]["context"],
            ["embed"]
        );
        $schema["properties"]["content"]["context"] = array_merge(
            $schema["properties"]["content"]["context"],
            ["embed"]
        );
         ...
         */
        $schema["properties"]["content"]["properties"]["raw"][
            "context"
        ] = array_merge(
            $schema["properties"]["content"]["properties"]["raw"]["context"],
            ["embed"]
        );
        $schema["properties"]["content"]["properties"]["rendered"][
            "context"
        ] = array_merge(
            $schema["properties"]["content"]["properties"]["rendered"][
                "context"
        ...
        $schema["properties"]["content"]["properties"]["block_version"][
            "context"
        ] = array_merge(
            $schema["properties"]["content"]["properties"]["block_version"][
                "context"
         ...
         * @see WP_REST_Posts_Controller::get_item_schema()
         */
        $schema["properties"]["title"]["properties"]["raw"][
            "context"
        ] = array_merge(
            $schema["properties"]["title"]["properties"]["raw"]["context"],
            ["embed"]
        );
    ...
    public static function get_fallback()
    {
         ...
         */
        $should_create_fallback = apply_filters(
        ...
        if ($fallback && !is_wp_error($fallback)) {
            ...
            return $fallback instanceof WP_Post
                ? $fallback
    ...
    private static function create_classic_menu_fallback()
    {
        ...
        if (!$classic_nav_menu) {
            return new WP_Error(
                "no_classic_menus",
                __("No Classic Menus found.")
            );
        }

        // If there is a classic menu then convert it to blocks.
        $classic_nav_menu_blocks = WP_Classic_To_Block_Menu_Converter::convert(
            $classic_nav_menu
        );
     ...
     */
    private static function get_most_recently_created_nav_menu(
        $classic_nav_menus
    ) {
