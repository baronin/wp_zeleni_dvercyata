...
if (!CUSTOM_TAGS) {
     ...
     * @since 5.5.0
     */
    $allowedxmlentitynames = ["amp", "lt", "gt", "apos", "quot"];
    ...
    foreach ($required_kses_globals as $global_name) {
        if (
            !isset($GLOBALS[$global_name]) ||
            !is_array($GLOBALS[$global_name])
        ) {
    ...
    switch ($context) {
        case "post":
            ...
            if (
                !CUSTOM_TAGS &&
                !isset($tags["form"]) &&
                (isset($tags["input"]) || isset($tags["select"]))
            ) {
...
function wp_kses_attr($element, $attr, $allowed_html, $allowed_protocols)
{
    ...
    if (
        empty($allowed_html[$element_low]) ||
        true === $allowed_html[$element_low]
    ) {
    ...
    $required_attrs = array_filter(
        ...
        static function ($required_attr_limits) {
            return isset($required_attr_limits["required"]) &&
                true === $required_attr_limits["required"];
        }
    );
...
function wp_kses_attr_check(
    ...
    if (!isset($allowed_attr[$name_low]) || "" === $allowed_attr[$name_low]) {
         ...
         */
        if (
            str_starts_with($name_low, "data-") &&
            !empty($allowed_attr["data-*"]) &&
...
function wp_kses_hair($attr, $allowed_protocols)
{
    ...
    while (strlen($attr) !== 0) {
        ...
        switch ($mode) {
            case 0:
                if (
                    preg_match("/^([_a-zA-Z][-_a-zA-Z0-9:.]*)/", $attr, $match)
                ) {
                    $attrname = $match[1];
                    $working = 1;
                    $mode = 1;
...
function wp_kses_bad_protocol($content, $allowed_protocols)
{
    ...
    if (
        (str_starts_with($content, "https://") &&
            in_array("https", $allowed_protocols, true)) ||
        (str_starts_with($content, "http://") &&
            in_array("http", $allowed_protocols, true))
    ) {
        return $content;
    }
...
function wp_kses_bad_protocol_once($content, $allowed_protocols, $count = 1)
{
    ...
    if (isset($content2[1]) && !preg_match("%/\?%", $content2[0])) {
        ...
        $protocol = wp_kses_bad_protocol_once2(
            $content2[0],
            $allowed_protocols
        );
        if ("feed:" === $protocol) {
            ...
            $content = wp_kses_bad_protocol_once(
                $content,
                $allowed_protocols,
                ++$count
            );
...
function safecss_filter_attr($css, $deprecated = "")
{
    ...
    foreach ($css_array as $css_item) {
        ...
        if ($found && $gradient_attr) {
            ...
            if (
                preg_match(
                    '/^(repeating-)?(linear|radial|conic)-gradient\(([^()]|rgb[a]?\([^()]*\))*\)$/',
                    $css_value
                )
            ) {
...
function _wp_kses_allow_pdf_objects($url)
{
    ...
    if (
        str_starts_with($url, "http://$upload_host$upload_port/") ||
