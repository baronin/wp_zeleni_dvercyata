...
function wp_get_themes($args = [])
{
    ...
    if (is_array($wp_theme_directories) && count($wp_theme_directories) > 1) {
        ...
        if (isset($theme_directories[$current_theme])) {
            ...
            if (
                !in_array($root_of_current_theme, $wp_theme_directories, true)
            ) {
                $root_of_current_theme =
                    WP_CONTENT_DIR . $root_of_current_theme;
            }
            $theme_directories[$current_theme][
                "theme_root"
            ] = $root_of_current_theme;
        }
    }
    ...
    if (is_multisite() && null !== $args["allowed"]) {
        ...
        if ("network" === $allowed) {
            ...
            $theme_directories = array_intersect_key(
                $theme_directories,
                WP_Theme::get_allowed($args["blog_id"])
            );
        } else {
            $theme_directories = array_diff_key(
                $theme_directories,
                WP_Theme::get_allowed($args["blog_id"])
            );
        }
    ...
    foreach ($theme_directories as $theme => $theme_root) {
        if (isset($_themes[$theme_root["theme_root"] . "/" . $theme])) {
            $themes[$theme] =
                $_themes[$theme_root["theme_root"] . "/" . $theme];
        } else {
...
function get_locale_stylesheet_uri()
{
    ...
    if (file_exists("$dir/$locale.css")) {
        ...
        !empty($wp_locale->text_direction) &&
...
function register_theme_directory($directory)
{
    ...
    if (
        !empty($untrailed) &&
        !in_array($untrailed, $wp_theme_directories, true)
    ) {
...
function search_theme_directories($force = false)
{
    ...
    foreach ($wp_theme_directories as $theme_root) {
        if (str_starts_with($theme_root, WP_CONTENT_DIR)) {
            $relative_theme_roots[
                str_replace(WP_CONTENT_DIR, "", $theme_root)
            ] = $theme_root;
        } else {
        ...
        foreach ($dirs as $dir) {
            ...
            if (file_exists($theme_root . "/" . $dir . "/style.css")) {
                ...
                if (!$sub_dirs) {
                    wp_trigger_error(
                        __FUNCTION__,
                        "$theme_root/$dir is not readable"
                    );
                    continue;
                }
                foreach ($sub_dirs as $sub_dir) {
                    if (
                        !is_dir($theme_root . "/" . $dir . "/" . $sub_dir) ||
                        "." === $dir[0] ||
                        "CVS" === $dir
                    ) {
                        continue;
    ...
    foreach ($found_themes as $theme_dir => $theme_data) {
        $theme_roots[$theme_dir] =
            $relative_theme_roots[$theme_data["theme_root"]]; // Convert absolute to relative.
    }
    ...
    if ($stylesheet_or_template && $theme_root) {
        if (in_array($theme_root, (array) $wp_theme_directories, true)) {
            ...
            if (str_starts_with($theme_root, WP_CONTENT_DIR)) {
                ...
                $theme_root_uri = site_url(
                    str_replace(ABSPATH, "", $theme_root)
                );
            } elseif (
                str_starts_with($theme_root, WP_PLUGIN_DIR) ||
                str_starts_with($theme_root, WPMU_PLUGIN_DIR)
            ) {
                $theme_root_uri = plugins_url(
                    basename($theme_root),
                    $theme_root
                );
            } else {
...
function switch_theme($stylesheet)
{
    ...
    if ("wp_ajax_customize_save" === current_action()) {
        $old_sidebars_widgets_data_setting = $wp_customize->get_setting(
            "old_sidebars_widgets_data"
        );
        if ($old_sidebars_widgets_data_setting) {
            $_sidebars_widgets = $wp_customize->post_value(
...
function validate_current_theme()
{
    ...
    if (
        ...
        !file_exists(
            get_template_directory() . "/block-templates/index.html"
...
function validate_theme_requirements($stylesheet)
{
    ...
    $requirements = [
        "requires" => !empty($theme->get("RequiresWP"))
            ? $theme->get("RequiresWP")
            : "",
        "requires_php" => !empty($theme->get("RequiresPHP"))
...
{
    $header_image_mod = get_theme_mod(
        "header_image",
        get_theme_support("custom-header", "default-image")
    );
...
function get_uploaded_header_images()
{
    ...
    foreach ((array) $headers as $header) {
        ...
        $header_images[$header_index]["alt_text"] = get_post_meta(
            $header->ID,
            "_wp_attachment_image_alt",
            true
        );
...
function register_default_headers($headers)
{
    ...
    $_wp_default_headers = array_merge(
...
function has_custom_header()
{
    if (
        has_header_image() ||
        (has_header_video() && is_header_video_active())
    ) {
        return true;
    }
...
function is_header_video_active()
{
    ...
    $video_active_cb = get_theme_support(
        "custom-header",
...
function the_custom_header_markup()
{
    ...
    if (
        ...
        (has_header_video() || is_customize_preview())
    ) {
...
function _custom_background_cb()
{
    ...
    $style = $color
        ? "background-color: " . maybe_hash_hex_color($color) . ";"
        : "";
...
function get_theme_starter_content()
{
    ...
    if (
        ...
        !empty($theme_support[0]) &&
        is_array($theme_support[0])
    ) {
        ...
        switch ($type) {
                ...
                foreach ($config[$type] as $sidebar_id => $widgets) {
                    foreach ($widgets as $id => $widget) {
                        if (is_array($widget)) {
                            ...
                            $content[$type][$sidebar_id][] =
                                $core_content[$type][$widget];
                        }
                    }
                }
                break;
            ...
            case "nav_menus":
                foreach ($config[$type] as $nav_menu_location => $nav_menu) {
                    ...
                    $content[$type][$nav_menu_location]["name"] =
                        $nav_menu["name"];
                        ...
                        if (is_array($nav_menu_item)) {
                            ...
                            if (!empty($core_content[$type][$id])) {
                                $nav_menu_item = array_merge(
                                    $core_content[$type][$id],
                                    $nav_menu_item
                                );
                            }

                            $content[$type][$nav_menu_location][
                                "items"
                            ][] = $nav_menu_item;
                        } elseif (
                            is_string($nav_menu_item) &&
                            !empty($core_content[$type]) &&
                            !empty($core_content[$type][$nav_menu_item])
                        ) {
                            $content[$type][$nav_menu_location]["items"][] =
                                $core_content[$type][$nav_menu_item];
                        }
                    }
                }
                break;
    ...
    switch ($feature) {
        case "post-thumbnails":
            ...
            if (
                ...
                $args[0] = array_unique(
                    array_merge(
                        $_wp_theme_features["post-thumbnails"][0],
                        $args[0]
                    )
                );
            }
        ...
        case "custom-logo":
            ...
            $args[0] = wp_parse_args(
                array_intersect_key($args[0], $defaults),
                $defaults
            );
        ...
        case "custom-header":
            ...
            if (isset($_wp_theme_features["custom-header"])) {
                $args[0] = wp_parse_args(
                    $_wp_theme_features["custom-header"][0],
                    $args[0]
                );
            }
        ...
        case "custom-background":
            ...
            if (isset($_wp_theme_features["custom-background"])) {
                $args[0] = wp_parse_args(
                    $_wp_theme_features["custom-background"][0],
                    $args[0]
                );
            }
...
{
    if (
        !current_theme_supports("custom-header", "header-text") &&
...
function current_theme_supports($feature, ...$args)
{
    ...
    switch ($feature) {
        ...
        case "custom-background":
            ...
            return isset($_wp_theme_features[$feature][0][$args[0]]) &&
                $_wp_theme_features[$feature][0][$args[0]];
    }
...
function register_theme_feature($feature, $args = [])
{
    ...
    if (
        ...
        in_array($args["type"], ["array", "object"], true)
    ) {
        if (
            !is_array($args["show_in_rest"]) ||
            empty($args["show_in_rest"]["schema"])
        ) {
            return new WP_Error(
                "missing_schema",
                __(
        ...
        if (
            "array" === $args["type"] &&
            !isset($args["show_in_rest"]["schema"]["items"])
        ) {
            return new WP_Error(
        ...
        if (
            "object" === $args["type"] &&
            !isset($args["show_in_rest"]["schema"]["properties"])
        ) {
            return new WP_Error(
    ...
    if (is_array($args["show_in_rest"])) {
        if (
            isset($args["show_in_rest"]["prepare_callback"]) &&
        ...
        if (
            ...
            !in_array(
                "boolean",
                (array) $args["show_in_rest"]["schema"]["type"],
                true
            )
        ) {
            // Automatically include the "boolean" type when the default value is a boolean.
            $args["show_in_rest"]["schema"]["type"] =
                (array) $args["show_in_rest"]["schema"]["type"];
...
function get_registered_theme_feature($feature)
{
    ...
    return isset($_wp_registered_theme_features[$feature])
        ? $_wp_registered_theme_features[$feature]
        : null;
}
...
function _wp_customize_include()
{
    ...
    if (isset($input_vars["customize_messenger_channel"])) {
        $messenger_channel = sanitize_key(
            $input_vars["customize_messenger_channel"]
        );
    }
...
function _wp_customize_changeset_filter_insert_post_data(
    ...
    if (
        ...
        if (
            empty($post_data["post_name"]) &&
            !empty($supplied_post_data["post_name"])
        ) {
...
function _wp_customize_loader_settings()
{
    ...
    $cross_domain =
        strtolower($admin_origin["host"]) !== strtolower($home_origin["host"]);
    ...
    $script =
        "var _wpCustomizeLoaderSettings = " . wp_json_encode($settings) . ";";
