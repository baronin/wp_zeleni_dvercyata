...
class WP_User_Query
{
    ...
    public function prepare_query($query = [])
    {
        ...
        if (is_array($qv["fields"])) {
            ...
            "all_with_meta" === $qv["fields"] ||
            "all" === $qv["fields"] ||
            !in_array($qv["fields"], $allowed_fields, true)
        ) {
            $this->query_fields = "$wpdb->users.ID";
        } else {
            $field =
                "id" === strtolower($qv["fields"])
                    ? "ID"
                    : sanitize_key($qv["fields"]);
            $this->query_fields = "$wpdb->users.$field";
        }
        ...
        if ($qv["has_published_posts"] && $blog_id) {
            ...
            $this->query_where .=
                " AND $wpdb->users.ID IN ( SELECT DISTINCT $posts_table.post_author FROM $posts_table WHERE $posts_table.post_status = 'publish' AND $posts_table.post_type IN ( " .
                implode(", ", $post_types) .
                " ) )";
        }
        ...
        if (!empty($qv["nicename__not_in"])) {
            $sanitized_nicename__not_in = array_map(
                "esc_sql",
                $qv["nicename__not_in"]
            );
        ...
        if (!empty($qv["login__not_in"])) {
            $sanitized_login__not_in = array_map(
                "esc_sql",
                $qv["login__not_in"]
            );
        ...
        if (
            !empty($qv["capability"]) ||
            !empty($qv["capability__in"]) ||
            !empty($qv["capability__not_in"])
        ) {
        ...
        if (
            ...
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    $roles_clauses[] = [
                        "key" =>
                            $wpdb->get_blog_prefix($blog_id) . "capabilities",
                        "value" => '"' . $role . '"',
                        "compare" => "LIKE",
                    ];
                }
        ...
        if ($search) {
            ...
            $this->query_where .= $this->get_search_sql(
                $search,
                $search_columns,
                $wild
            );
    ...
    public function query()
    {
         ...
         */
        $this->results = apply_filters_ref_array("users_pre_query", [
            null,
            &$this,
        ]);
        ...
        if (null === $this->results) {
            ...
            $this->request = "SELECT {$this->query_fields}
				 {$this->query_from}
				 {$this->query_where}
				 {$this->query_orderby}
				 {$this->query_limit}";
            ...
            if (false !== $cache_value) {
                ...
                if (isset($qv["count_total"]) && $qv["count_total"]) {
                    ...
                    $this->total_users = (int) $wpdb->get_var(
                        $found_users_query
                    );
                }
    ...
    protected function parse_orderby($orderby)
    {
        ...
        if (
            ...
            $sanitized_nicename__in = array_map(
                "esc_sql",
                $this->query_vars["nicename__in"]
            );
            $nicename__in = implode("','", $sanitized_nicename__in);
            $_orderby = "FIELD( user_nicename, '$nicename__in' )";
        } elseif ("login__in" === $orderby) {
            $sanitized_login__in = array_map(
                "esc_sql",
                $this->query_vars["login__in"]
            );
            $login__in = implode("','", $sanitized_login__in);
    ...
    protected function generate_cache_key(array $args, $sql)
    {
        ...
        if (
            ...
            in_array("post_count", $ordersby, true)
        ) {
