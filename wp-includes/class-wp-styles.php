...
class WP_Styles extends WP_Dependencies
{
    ...
    public function do_item($handle, $group = false)
    {
        ...
        $rel =
            isset($obj->extra["alt"]) && $obj->extra["alt"]
                ? "alternate stylesheet"
                : "stylesheet";
        $title = isset($obj->extra["title"])
            ? sprintf(" title='%s'", esc_attr($obj->extra["title"]))
            : "";
        ...
        if (
            ...
            if (
                is_bool($obj->extra["rtl"]) ||
                "replace" === $obj->extra["rtl"]
            ) {
                $suffix = isset($obj->extra["suffix"])
                    ? $obj->extra["suffix"]
                    : "";
                $rtl_href = str_replace(
                    "{$suffix}.css",
                    "-rtl{$suffix}.css",
                    $this->_css_href($src, $ver, "$handle-rtl")
                );
            } else {
                $rtl_href = $this->_css_href(
                    $obj->extra["rtl"],
                    $ver,
                    "$handle-rtl"
                );
