...
function redirect_canonical($requested_url = null, $do_redirect = true)
{
    ...
    if (
        ...
        !in_array(strtoupper($_SERVER["REQUEST_METHOD"]), ["GET", "HEAD"], true)
    ) {
        return;
    }
    ...
    if (
        is_preview() &&
        get_query_var("p") &&
        "publish" === get_post_status(get_query_var("p"))
    ) {
        if (
            ...
            !wp_verify_nonce(
                $_GET["preview_nonce"],
                "post_preview_" . (int) $_GET["preview_id"]
            )
        ) {
    ...
    if (is_feed() && $post_id) {
        $redirect_url = get_post_comments_feed_link(
            $post_id,
            get_query_var("feed")
        );
    ...
    if (is_404()) {
        ...
        if (
            is_attachment() &&
            !array_diff(array_keys($wp->query_vars), [
                "attachment",
                "attachment_id",
            ]) &&
            !$redirect_url
            ...
            !$redirect_url &&
            "page" === get_option("show_on_front") &&
            get_query_var("page_id") === (int) get_option("page_for_posts")
        ) {
            ...
            switch (strlen($m)) {
                case 4: // Yearly.
                    ...
                    break;
                case 6: // Monthly.
                    $redirect_url = get_month_link(
                        substr($m, 0, 4),
                        substr($m, 4, 2)
                    );
                    break;
                case 8: // Daily.
                    $redirect_url = get_day_link(
                        substr($m, 0, 4),
                        substr($m, 4, 2),
                        substr($m, 6, 2)
                    );
            ...
            is_author() &&
            !empty($_GET["author"]) &&
            is_string($_GET["author"]) &&
            preg_match('|^[0-9]+$|', $_GET["author"])
        ) {
            ...
            if (
                ...
                $redirect_url = get_author_posts_url(
                    $author->ID,
            ...
            if ($term_count <= 1 && !empty($obj->term_id)) {
                ...
                if ($tax_url && !is_wp_error($tax_url)) {
                    if (!empty($redirect["query"])) {
                        ...
                        $rewrite_vars = array_diff(
                            array_keys($wp_query->query),
                            array_keys($_GET)
                        );
            ...
            is_single() &&
            str_contains($wp_rewrite->permalink_structure, "%category%")
        ) {
        ...
        if (is_singular() && get_query_var("page")) {
            ...
            if ($page > 1) {
                ...
                if (is_front_page()) {
                    $redirect_url .= user_trailingslashit(
        ...
        if (get_query_var("sitemap")) {
            $redirect_url = get_sitemap_url(
                get_query_var("sitemap"),
                get_query_var("sitemap-subtype"),
                get_query_var("paged")
            );
            $redirect["query"] = remove_query_arg(
                ["sitemap", "sitemap-subtype", "paged"],
                $redirect["query"]
            );
        } elseif (
            get_query_var("paged") ||
            ...
            while (
                ...
                preg_match(
                    "#/{$wp_rewrite->comments_pagination_base}-[0-9]+(/+)?$#",
                    $redirect["path"]
                )
            ) {
            ...
            if (is_feed() && in_array($feed, $wp_rewrite->feeds, true)) {
                ...
                if (
                    ("rss" === $default_feed && "feed" === $feed) ||
                    "rss" === $feed
                ) {
                    $format = "rss2" === $default_feed ? "" : "rss2";
                } else {
                    $format =
                        $default_feed === $feed || "feed" === $feed
                            ? ""
                            : $feed;
                }
                ...
                if (isset($old_feed_files[basename($redirect["path"])])) {
                    $redirect_url = get_feed_link(
                        $old_feed_files[basename($redirect["path"])]
                    );
            ...
            if (!empty($addl_path)) {
                $redirect["path"] =
                    trailingslashit($redirect["path"]) . $addl_path;
            }
    ...
    if (is_attachment() && !get_option("wp_attachment_pages_enabled")) {
        ...
        $attachment_parent_id = $attachment_post
    ...
    if (
        ...
        !is_404() &&
        (!is_front_page() || (is_front_page() && get_query_var("paged") > 1))
    ) {
        $user_ts_type = "";
        ...
        if (get_query_var("paged") > 0) {
            ...
            foreach (
                ["single", "category", "page", "day", "month", "year", "home"]
                as $type
            ) {
                $func = "is_" . $type;
        ...
        $redirect["path"] = user_trailingslashit(
            $redirect["path"],
            $user_ts_type
        );
    ...
    if (
        trailingslashit($redirect["path"]) ===
        trailingslashit($user_home["path"])
    ) {
        $redirect["path"] = trailingslashit($redirect["path"]);
    }
    ...
    if (
        !$redirect_url ||
        strip_fragment_from_url($redirect_url) ===
...
function redirect_guess_404_permalink()
{
    ...
    if (get_query_var("name")) {
        $publicly_viewable_statuses = array_filter(
            get_post_stati(),
            "is_post_status_viewable"
        );
        $publicly_viewable_post_types = array_filter(
        ...
        if (get_query_var("post_type")) {
            if (is_array(get_query_var("post_type"))) {
                ...
                $where .=
                    " AND post_type IN ('" .
                    join("', '", esc_sql(get_query_var("post_type"))) .
                    "')";
            } else {
                if (
                    !in_array(
            ...
            $where .=
                " AND post_type IN ('" .
                implode("', '", esc_sql($publicly_viewable_post_types)) .
                "')";
        }
        ...
        $post_id = $wpdb->get_var(
            "SELECT ID FROM $wpdb->posts WHERE $where AND post_status IN ('" .
                implode("', '", esc_sql($publicly_viewable_statuses)) .
                "')"
        );
