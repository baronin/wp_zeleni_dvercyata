...
class WP_Token_Map
{
    ...
    public static function from_array(
        ...
        foreach ($groups as $group_key => $group) {
            usort($groups[$group_key], static function (
                array $a,
                array $b
            ): int {
            ...
            while ($at < $group_length) {
                ...
                if (
                    ...
                    substr_compare(
                        $text,
                        $token,
                        $offset + $this->key_length,
                        $token_length,
                        $ignore_case
                    )
                ) {
                    $matched_token_byte_length =
                        $this->key_length + $token_length;
    ...
    private function read_small_token(
        ...
        while ($at < $small_length) {
            if (
                ...
                (!$ignore_case ||
                    strtoupper($this->small_words[$at]) !== $starting_char)
            ) {
    ...
    public function to_array(): array
    {
        ...
        while ($at < $small_length) {
            $key = rtrim(
                substr($this->small_words, $at, $this->key_length + 1),
                "\x00"
            );
    ...
    public function precomputed_php_source_table(string $indent = "\t"): string
    {
        ...
        foreach ($prefixes as $index => $prefix) {
            ...
            while ($at < $group_length) {
                ...
                $token_digits = str_pad(
                    dechex($token_length),
                    2,
                    "0",
                    STR_PAD_LEFT
                );
                $mapping_digits = str_pad(
                    dechex($mapping_length),
                    2,
                    "0",
                    STR_PAD_LEFT
                );
     ...
     */
    private static function longest_first_then_alphabetical(
        string $a,
        string $b
    ): int {
        if ($a === $b) {
