...
class WP_oEmbed
{
    ...
    public function __construct()
    {
        ...
        $providers = [
            ...
            "#https?://((play|www)\.)?anghami\.com/.*#i" => [
                "https://api.anghami.com/rest/v1/oembed.view",
                true,
            ],
            ...
            if (!$regex) {
                $matchmask =
                    ...
                    str_replace(
                        ...
                        "(.+)",
                        preg_quote(
                            str_replace("*", "___wildcard___", $matchmask),
                            "#"
                        )
                    ) .
                    "#i";
    ...
    public function discover($url)
    {
        ...
        if ($html) {
            ...
            if (
                $tagfound &&
                preg_match_all("#<link([^<>]+)/?>#iU", $html, $links)
            ) {
                foreach ($links[1] as $link) {
                    ...
                    if (
                        !empty($atts["type"]) &&
                        !empty($linktypes[$atts["type"]]) &&
                        !empty($atts["href"])
                    ) {
                        $providers[
                            $linktypes[$atts["type"]]
                        ] = htmlspecialchars_decode($atts["href"]);
    ...
    public function fetch($provider, $url, $args = "")
    {
        ...
        foreach (["json", "xml"] as $format) {
            ...
            if (
                is_wp_error($result) &&
                "not-implemented" === $result->get_error_code()
            ) {
                continue;
            }
    ...
    {
        $provider_url_with_args = add_query_arg(
            "format",
            $format,
