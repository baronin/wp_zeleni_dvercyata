...
class WP_HTML_Decoder
{
     ...
     */
    public static function attribute_starts_with(
        $haystack,
        $search_text,
    ...
    public static function decode($context, $text): string
    {
        ...
        while ($at < $end) {
            ...
            $character_reference = self::read_character_reference(
                $context,
                $text,
     ...
     */
    public static function read_character_reference(
        $context,
        $text,
        $at = 0,
        ...
        if ("#" === $text[$at + 1]) {
            ...
            $digit_count = strspn(
                $text,
                $numeric_digits,
                $digits_at + $zero_count
            );
            $after_digits = $digits_at + $zero_count + $digit_count;
            $has_semicolon =
                $after_digits < $length && ";" === $text[$after_digits];
        ...
        $name_length = 0;
        $replacement = $html5_named_character_references->read_token(
            $text,
            $name_at,
            $name_length
        );
        ...
        $ambiguous_follower =
            $after_name < $length &&
            $name_at < $length &&
            (ctype_alnum($text[$after_name]) || "=" === $text[$after_name]);
