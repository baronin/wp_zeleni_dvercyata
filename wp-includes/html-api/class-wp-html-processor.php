...
class WP_HTML_Processor extends WP_HTML_Tag_Processor
{
     ...
     */
    public static function create_fragment(
        $html,
        $context = "<body>",
        $encoding = "UTF-8"
    ) {
        ...
        $context_processor = static::create_full_parser(
            "<!DOCTYPE html>{$context}",
            $encoding
        );
        ...
        if (
            ...
            _doing_it_wrong(
                __METHOD__,
                __("No valid context element was detected."),
                "6.8.0"
            );
            return null;
        }
     ...
     * @return static|null The created processor if successful, otherwise null.
     */
    public static function create_full_parser(
        $html,
        $known_definite_encoding = "UTF-8"
    ) {
    ...
    public function __construct(
        $html,
        $use_the_static_create_methods_instead = null
    ) {
        parent::__construct($html);

        if (
            self::CONSTRUCTOR_UNLOCK_CODE !==
        ...
        $this->state->stack_of_open_elements->set_push_handler(function (
            WP_HTML_Token $token
        ): void {
            $is_virtual =
                !isset($this->state->current_token) || $this->is_tag_closer();
            $same_node =
            ...
            $this->change_parsing_namespace(
                $token->integration_node_type ? "html" : $token->namespace
            );
        });

        $this->state->stack_of_open_elements->set_pop_handler(function (
            WP_HTML_Token $token
        ): void {
            $is_virtual =
                !isset($this->state->current_token) || !$this->is_tag_closer();
         ...
         */
        $this->release_internal_bookmark_on_destruct = function (
            string $name
        ): void {
    ...
    private function create_fragment_at_current_node(string $html)
    {
        ...
        $root_node = new WP_HTML_Token("root-node", "HTML", false);
        ...
        $fragment_processor->breadcrumbs = [
            "HTML",
            $fragment_processor->context_node->node_name,
        ];

        if ("TEMPLATE" === $fragment_processor->context_node->node_name) {
            $fragment_processor->state->stack_of_template_insertion_modes[] =
        ...
        $fragment_processor->change_parsing_namespace(
            $this->current_element->token->integration_node_type
                ? "html"
                : $namespace
        );
    ...
    private function bail(string $message)
    {
        ...
        $active_formats = [];
        foreach (
            $this->state->active_formatting_elements->walk_down()
            as $item
        ) {
    ...
    {
        $visit_closers =
            isset($query["tag_closers"]) && "visit" === $query["tag_closers"];
        ...
        $needs_class =
            isset($query["class_name"]) && is_string($query["class_name"])
                ? $query["class_name"]
                : null;

        if (
            !(
                array_key_exists("breadcrumbs", $query) &&
                is_array($query["breadcrumbs"])
            )
        ) {
            while ($this->next_token()) {
                if ("#tag" !== $this->get_token_type()) {
                ...
                if (
                    ...
                    $query["tag_name"] !== $this->get_token_name()
                ) {
                    continue;
                }
        ...
        $match_offset = isset($query["match_offset"])
            ? (int) $query["match_offset"]
            : 1;
        ...
        while ($match_offset > 0 && $this->next_token()) {
            ...
            if (
                $this->matches_breadcrumbs($breadcrumbs) &&
                0 === --$match_offset
            ) {
                return true;
            }
    ...
    private function next_visitable_token(): bool
    {
        ...
        if (!isset($this->current_element)) {
            ...
            return empty($this->element_queue)
                ? false
                : $this->next_visitable_token();
        }
    ...
    {
        return isset($this->current_element->provenance) &&
    ...
    public function expects_closer(?WP_HTML_Token $node = null): ?bool
    {
        ...
        $token_has_self_closing =
            $node->has_self_closing_flag ?? $this->has_self_closing_flag();

        return !// Comments, text nodes, and other atomic tokens.
        (
            "#" === $token_name[0] ||
    ...
    public function step($node_to_process = self::PROCESS_NEXT_NODE): bool
    {
        ...
        if (
            WP_HTML_Tag_Processor::STATE_INCOMPLETE_INPUT ===
                $this->parser_state ||
        ...
        $parse_in_current_insertion_mode =
            ...
            ("math" === $adjusted_current_node->integration_node_type &&
                (($is_start_tag &&
                    !in_array($token_name, ["MGLYPH", "MALIGNMARK"], true)) ||
                    "#text" === $token_name)) ||
        ...
        try {
            ...
            switch ($this->state->insertion_mode) {
                ...
                default:
                    $this->bail(
                        "Unaware of the requested parsing mode: '{$this->state->insertion_mode}'."
                    );
            }
    ...
    private function step_initial(): bool
    {
        ...
        $token_type = $this->get_token_type();
        $op_sigil =
            "#tag" === $token_type ? (parent::is_tag_closer() ? "-" : "+") : "";
        ...
        switch ($op) {
            ...
            case "#text":
                if (
                    parent::TEXT_IS_WHITESPACE ===
                    $this->text_node_classification
                ) {
                    return $this->step();
                }
            ...
            case "html":
                ...
                $this->state->insertion_mode =
                    WP_HTML_Processor_State::INSERTION_MODE_BEFORE_HTML;
                $this->insert_html_element($this->state->current_token);
                return true;
        }
    ...
    private function step_before_head(): bool
    {
        ...
        switch ($op) {
            ...
            case "+HEAD":
                ...
                $this->state->insertion_mode =
                    WP_HTML_Processor_State::INSERTION_MODE_IN_HEAD;
                return true;
    ...
    private function step_in_head(): bool
    {
        ...
        switch ($op) {
            ...
            case "+NOSCRIPT":
                $this->insert_html_element($this->state->current_token);
                $this->state->insertion_mode =
                    WP_HTML_Processor_State::INSERTION_MODE_IN_HEAD_NOSCRIPT;
                return true;
            ...
            case "-TEMPLATE":
                ...
                if (
                    !$this->state->stack_of_open_elements->current_node_is(
                        "TEMPLATE"
                    )
                ) {
                    // @todo Indicate a parse error once it's possible.
    ...
    private function step_after_head(): bool
    {
        ...
        switch ($op) {
            ...
            case "+BODY":
                ...
                $this->state->frameset_ok = false;
                $this->state->insertion_mode =
                    WP_HTML_Processor_State::INSERTION_MODE_IN_BODY;
                return true;
            ...
            case "+FRAMESET":
                $this->insert_html_element($this->state->current_token);
                $this->state->insertion_mode =
                    WP_HTML_Processor_State::INSERTION_MODE_IN_FRAMESET;
                return true;
    ...
    private function step_in_body(): bool
    {
        ...
        switch ($op) {
            case "#text":
                 ...
                 */
                if (
                    parent::TEXT_IS_GENERIC === $this->text_node_classification
                ) {
                    $this->state->frameset_ok = false;
                }
            ...
            case "+BODY":
                if (
                    ...
                    "BODY" !==
                        ($this->state->stack_of_open_elements->at(2)
                            ->node_name ??
                            null) ||
                    $this->state->stack_of_open_elements->contains("TEMPLATE")
                ) {
            ...
            case "-BODY":
                ...
                $this->state->insertion_mode =
                    WP_HTML_Processor_State::INSERTION_MODE_AFTER_BODY;
                /*
             ...
             */
            case "+FORM":
                $stack_contains_template = $this->state->stack_of_open_elements->contains(
                    "TEMPLATE"
                );

                if (
                    isset($this->state->form_element) &&
                    !$stack_contains_template
                ) {
                    // Parse error: ignore the token.
                    return $this->step();
                }
            ...
            case "+LI":
                ...
                if (
                    $is_li
                        ? "LI" === $node->node_name
                        : "DD" === $node->node_name || "DT" === $node->node_name
                ) {
                ...
                if (
                     ...
                     */
                    foreach (
                        $this->state->stack_of_open_elements->walk_up($node)
                        as $item
                    ) {
                        $node = $item;
                        break;
            ...
            case "-H6":
                ...
                $this->state->stack_of_open_elements->pop_until(
                    "(internal: H1 through H6 - do not use)"
                );
                return true;
            ...
            case "+A":
                foreach (
                    $this->state->active_formatting_elements->walk_up()
                    as $item
                ) {
                    switch ($item->node_name) {
                        ...
                        case "A":
                            ...
                            $this->state->active_formatting_elements->remove_node(
                                $item
                            );
                            $this->state->stack_of_open_elements->remove_node(
                                $item
                            );
                            break 2;
                    }
                ...
                $this->reconstruct_active_formatting_elements();
                $this->insert_html_element($this->state->current_token);
                $this->state->active_formatting_elements->push(
            ...
            case "+INPUT":
                ...
                if (
                    !is_string($type_attribute) ||
                    "hidden" !== strtolower($type_attribute)
                ) {
                    $this->state->frameset_ok = false;
                }
            ...
            case "+SELECT":
                ...
                switch ($this->state->insertion_mode) {
                    ...
                    default:
                        $this->state->insertion_mode =
                            WP_HTML_Processor_State::INSERTION_MODE_IN_SELECT;
                        break;
                }
                return true;
            ...
            case "+RT":
                if (
                    ...
                    $current_node_name = $this->state->stack_of_open_elements->current_node()
                        ->node_name;
                    if (
                        "RTC" === $current_node_name ||
                        "RUBY" === $current_node_name
                    ) {
                        // @todo Indicate a parse error once it's possible.
                    }
                }
        ...
        $this->bail(
            "Should not have been able to reach end of IN BODY processing. Check HTML API code."
        );
    ...
    private function step_in_table(): bool
    {
        ...
        switch ($op) {
            ...
            case "+COLGROUP":
                ...
                $this->insert_html_element($this->state->current_token);
                $this->state->insertion_mode =
                    WP_HTML_Processor_State::INSERTION_MODE_IN_COLUMN_GROUP;
                return true;
            ...
            case "+TABLE":
                if (
                    !$this->state->stack_of_open_elements->has_element_in_table_scope(
                        "TABLE"
                    )
                ) {
                    return $this->step();
    ...
    {
        $this->bail(
            "No support for parsing in the " .
                WP_HTML_Processor_State::INSERTION_MODE_IN_TABLE_TEXT .
                " state."
        );
    }
    ...
    private function step_in_column_group(): bool
    {
         ...
         */
        if (
            !$this->state->stack_of_open_elements->current_node_is("COLGROUP")
        ) {
            // @todo Indicate a parse error once it's possible.
            return $this->step();
        }
        ...
        switch ($op) {
            ...
            case "-OPTGROUP":
                ...
                if ($current_node && "OPTION" === $current_node->node_name) {
                    foreach (
                        $this->state->stack_of_open_elements->walk_up(
                            $current_node
                        )
                        as $parent
                    ) {
            ...
            case "+SELECT":
                if (
                    !$this->state->stack_of_open_elements->has_element_in_select_scope(
                        "SELECT"
                    )
                ) {
                    // Parse error: ignore the token.
    ...
    private function step_after_body(): bool
    {
        ...
        $token_type = $this->get_token_type();
        $op_sigil =
            "#tag" === $token_type ? ($this->is_tag_closer() ? "-" : "+") : "";
        $op = "{$op_sigil}{$tag_name}";
        ...
        switch ($op) {
            ...
            case "-HTML":
                ...
                $this->state->insertion_mode =
                    WP_HTML_Processor_State::INSERTION_MODE_AFTER_AFTER_BODY;
                /*
    ...
    private function step_in_frameset(): bool
    {
        ...
        switch ($op) {
            ...
            case "-FRAMESET":
                 ...
                 */
                if (
                    !isset($this->context_node) &&
                    !$this->state->stack_of_open_elements->current_node_is(
                        "FRAMESET"
                    )
                ) {
                    $this->state->insertion_mode =
                        WP_HTML_Processor_State::INSERTION_MODE_AFTER_FRAMESET;
                }
    ...
    private function step_after_frameset(): bool
    {
        ...
        switch ($op) {
            ...
            case "-HTML":
                $this->state->insertion_mode =
                    WP_HTML_Processor_State::INSERTION_MODE_AFTER_AFTER_FRAMESET;
                /*
                 * The HTML element is not removed from the stack of open elements.
                 * Only internal state has changed, this does not qualify as a "step"
                 * in terms of advancing through the document to another token.
    ...
    private function step_in_foreign_content(): bool
    {
        ...
        switch ($op) {
            ...
            case "#cdata-section":
                ...
                $current_token =
                    $this->bookmarks[
                        $this->state->current_token->bookmark_name
                    ];
        ...
        if (
            $this->is_tag_closer() &&
            "SCRIPT" === $this->state->current_token->node_name &&
            "svg" === $this->state->current_token->namespace
        ) {
            $this->state->stack_of_open_elements->pop();
            return true;
        }
        ...
        $this->bail(
            "Should not have been able to reach end of IN FOREIGN CONTENT processing. Check HTML API code."
        );
        // This unnecessary return prevents tools from inaccurately reporting type errors.
        return false;
    }
    ...
    public function seek($bookmark_name): bool
    {
        ...
        $processor_started_at = $this->state->current_token
            ? $this->bookmarks[$this->state->current_token->bookmark_name]
                ->start
            : 0;
        $bookmark_starts_at = $this->bookmarks[$actual_bookmark_name]->start;
        $direction =
            $bookmark_starts_at > $processor_started_at
                ? "forward"
                : "backward";
        ...
        if ("backward" === $direction) {
            ...
            if (null === $this->context_node) {
                $this->change_parsing_namespace("html");
                $this->state->insertion_mode =
                    WP_HTML_Processor_State::INSERTION_MODE_INITIAL;
                $this->breadcrumbs = [];
     ...
     */
    private function generate_implied_end_tags(
        ?string $except_for_this_element = null
    ): void {
    ...
    private function reset_insertion_mode_appropriately(): void
    {
        ...
        $first_node = null;
        foreach (
            $this->state->stack_of_open_elements->walk_down()
            as $first_node
        ) {
            break;
        }
        ...
        foreach ($this->state->stack_of_open_elements->walk_up() as $node) {
            ...
            switch ($node->node_name) {
                 ...
                 */
                case "HTML":
                    $this->state->insertion_mode = isset(
                        $this->state->head_element
                    )
                        ? WP_HTML_Processor_State::INSERTION_MODE_AFTER_HEAD
                        : WP_HTML_Processor_State::INSERTION_MODE_BEFORE_HEAD;
                    return;
    ...
    private function run_adoption_agency_algorithm(): void
    {
        ...
        if (
            ...
            !$this->state->active_formatting_elements->contains_node(
                $current_node
            )
        ) {
            $this->state->stack_of_open_elements->pop();
        ...
        while ($budget-- > 0) {
            ...
            if (
                !$this->state->stack_of_open_elements->contains_node(
                    $formatting_element
                )
            ) {
                $this->state->active_formatting_elements->remove_node(
            ...
            if (null === $furthest_block) {
                foreach (
                    ...
                    if (
                        $formatting_element->bookmark_name ===
                        $item->bookmark_name
                    ) {
                        $this->state->active_formatting_elements->remove_node(
                            $formatting_element
     ...
     */
    private function insert_foreign_element(
        WP_HTML_Token $token,
     ...
     */
    private function insert_virtual_node(
        $token_name,
        $bookmark_name = null
    ): WP_HTML_Token {
        $here = $this->bookmarks[$this->state->current_token->bookmark_name];
    ...
    private function is_mathml_integration_point(): bool
    {
        ...
        if (
            "math" !== $current_token->namespace ||
            "M" !== $current_token->node_name[0]
        ) {
            return false;
        }
