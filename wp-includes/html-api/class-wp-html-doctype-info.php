...
class WP_HTML_Doctype_Info
{
    ...
    private function __construct(
        ...
        if (
            "html" === $name &&
            null === $public_identifier &&
            null === $system_identifier
        ) {
        ...
        $public_identifier =
            null === $public_identifier ? "" : strtolower($public_identifier);
        $system_identifier =
            null === $system_identifier ? "" : strtolower($system_identifier);
    ...
    public static function from_doctype_token(string $doctype_html): ?self
    {
        ...
        if (
            ">" !== $doctype_html[$end] ||
            strcspn($doctype_html, ">", $at) + $at < $end
        ) {
            return null;
        }
        ...
        $doctype_name = str_replace(
            "\0",
            "\u{FFFD}",
            strtolower(substr($doctype_html, $at, $name_length))
        );
        ...
        $identifier_length = strcspn(
            $doctype_html,
            $closer_quote,
            $at,
            $end - $at
        );
        $doctype_public_id = str_replace(
            "\0",
            "\u{FFFD}",
            substr($doctype_html, $at, $identifier_length)
        );
