...
class WP_HTML_Tag_Processor
{
    ...
    private function base_class_next_token(): bool
    {
        ...
        $this->token_length =
            $this->bytes_already_parsed - $this->token_starts_at;
    ...
    public function set_bookmark($name): bool
    {
        ...
        if (
            !array_key_exists($name, $this->bookmarks) &&
            count($this->bookmarks) >= static::MAX_BOOKMARKS
        ) {
            _doing_it_wrong(
                __METHOD__,
            ...
            if (!$this->is_closing_tag && "!" === $html[$at + 1]) {
                ...
                if (0 === substr_compare($html, "--", $at + 2, 2)) {
                    ...
                    if (">" === $html[$closer_at + $span_of_dashes]) {
                        ...
                        $this->comment_type =
                            self::COMMENT_AS_ABRUPTLY_CLOSED_COMMENT;
                        $this->token_length =
                            $closer_at +
                            $span_of_dashes +
                            1 -
                    ...
                    while (++$closer_at < $doc_length) {
                        ...
                        if (
                            $closer_at + 2 < $doc_length &&
                            ">" === $html[$closer_at + 2]
                        ) {
                            $this->parser_state = self::STATE_COMMENT;
                            $this->comment_type = self::COMMENT_AS_HTML_COMMENT;
                            $this->token_length =
                                $closer_at + 3 - $this->token_starts_at;
                            $this->text_starts_at = $this->token_starts_at + 4;
                            $this->text_length =
                                $closer_at - $this->text_starts_at;
                        ...
                        if (
                            ...
                            $this->comment_type = self::COMMENT_AS_HTML_COMMENT;
                            $this->token_length =
                                $closer_at + 4 - $this->token_starts_at;
                            $this->text_starts_at = $this->token_starts_at + 4;
                            $this->text_length =
                                $closer_at - $this->text_starts_at;
                ...
                if (
                    ...
                    $this->token_length =
                        $closer_at + 1 - $this->token_starts_at;
            ...
            if (!$this->is_closing_tag && "?" === $html[$at + 1]) {
                ...
                if ($this->token_length >= 5 && "?" === $html[$closer_at - 1]) {
                    ...
                    if (0 < $pi_target_length) {
                        ...
                        $this->comment_type =
                            self::COMMENT_AS_PI_NODE_LOOKALIKE;
    ...
    private function parse_next_attribute(): bool
    {
        ...
        $this->bytes_already_parsed += strspn(
            $this->html,
            " \t\f\r\n/",
            $this->bytes_already_parsed
        );
        ...
        if (
            0 === $name_length ||
            $this->bytes_already_parsed + $name_length >= $doc_length
        ) {
            return false;
        }
        ...
        if ($has_value) {
            ...
            switch ($this->html[$this->bytes_already_parsed]) {
                ...
                case '"':
                    ...
                    $end_quote_at =
                        false === $end_quote_at ? $doc_length : $end_quote_at;
         ...
         */
        $duplicate_span = new WP_HTML_Span(
            $attribute_start,
            $attribute_end - $attribute_start
        );
        if (null === $this->duplicate_attributes) {
            $this->duplicate_attributes = [
                $comparable_name => [$duplicate_span],
            ];
    ...
    private function class_name_updates_to_attributes_updates(): void
    {
        ...
        foreach ($this->classname_updates as $name => $operation) {
            ...
            if (
                self::ADD_CLASS === $operation &&
                !in_array($comparable_name, $seen, true)
            ) {
                $modified = true;
    ...
    private function apply_attributes_updates(int $shift_this_point): int
    {
        ...
        foreach ($this->lexical_updates as $diff) {
            ...
            $output_buffer .= substr(
                $this->html,
                $bytes_already_copied,
                $diff->start - $bytes_already_copied
            );
        ...
        $this->html =
            $output_buffer . substr($this->html, $bytes_already_copied);
        ...
        foreach ($this->bookmarks as $bookmark_name => $bookmark) {
            ...
            foreach ($this->lexical_updates as $diff) {
                ...
                if (
                    $bookmark->start < $diff->start &&
                    $bookmark_end < $diff->start
                ) {
                    break;
                }
     ...
     */
    private static function sort_start_ascending(
        WP_HTML_Text_Replacement $a,
        WP_HTML_Text_Replacement $b
    ): int {
    ...
    public function has_self_closing_flag(): bool
    {
         ...
         */
        return "/" ===
            $this->html[$this->token_starts_at + $this->token_length - 2];
    }
    ...
    {
        return self::STATE_MATCHED_TAG === $this->parser_state &&
            $this->is_closing_tag &&
            /*
    ...
    public function subdivide_text_appropriately(): bool
    {
        ...
        if ($leading_nulls > 0) {
            ...
            $this->bytes_already_parsed =
                $this->token_starts_at + $leading_nulls;
        ...
        while ($at < $end) {
            ...
            if ($at < $end && "&" === $this->html[$at]) {
                ...
                $replacement = WP_HTML_Decoder::read_character_reference(
                    "data",
                    $this->html,
                    $at,
                    $matched_byte_length
    ...
    public function get_modifiable_text(): string
    {
        ...
        if (
            !$has_enqueued_update &&
            (null === $this->text_starts_at || 0 === $this->text_length)
        ) {
            return "";
        }
    ...
    private function parse_query($query)
    {
        ...
        if (
            ...
            is_int($query["match_offset"]) &&
            0 < $query["match_offset"]
        ) {
    ...
    private function matches(): bool
    {
        ...
        if (
            null !== $this->sought_class_name &&
            !$this->has_class($this->sought_class_name)
        ) {
            return false;
        }
    ...
    public function get_doctype_info(): ?WP_HTML_Doctype_Info
    {
        ...
        return WP_HTML_Doctype_Info::from_doctype_token(
            substr($this->html, $this->token_starts_at, $this->token_length)
        );
    }
