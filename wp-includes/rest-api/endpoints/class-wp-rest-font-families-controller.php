...
class WP_REST_Font_Families_Controller extends WP_REST_Posts_Controller
{
    ...
    public function validate_font_family_settings($value, $request)
    {
        ...
        if (null === $settings) {
            return new WP_Error(
                ...
                sprintf(
                    __("%s parameter must be a valid JSON string."),
        ...
        $schema = $this->get_item_schema()["properties"][
            "font_family_settings"
        ];
    ...
    public function create_item($request)
    {
        ...
        if (!empty($query->posts)) {
            return new WP_Error(
                ...
                sprintf(
                    __('A font family with slug "%s" already exists.'),
                    $settings["slug"]
                ),
                ["status" => 400]
            );
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        if (rest_is_field_included("font_family_settings", $fields)) {
            $data["font_family_settings"] = $this->get_settings_from_post(
                $item
            );
        }
    ...
    public function get_public_item_schema()
    {
        ...
        foreach (
            $schema["properties"]["font_family_settings"]["properties"]
            as &$property
        ) {
        ...
        return [
            ...
            "slug" =>
                isset($post->post_name) && $post->post_name
                    ...
                    : "",
            "fontFamily" =>
                isset($settings_json["fontFamily"]) &&
                $settings_json["fontFamily"]
                    ? $settings_json["fontFamily"]
                    : "",
            "preview" =>
                isset($settings_json["preview"]) && $settings_json["preview"]
