...
class WP_REST_Posts_Controller extends WP_REST_Controller
{
    ...
    public function __construct($post_type)
    {
        ...
        $this->rest_base = !empty($obj->rest_base)
            ? $obj->rest_base
            : $obj->name;
        $this->namespace = !empty($obj->rest_namespace)
            ? $obj->rest_namespace
            : "wp/v2";
    ...
    public function get_items($request)
    {
        ...
        if (
            isset($registered["modified_before"], $request["modified_before"])
        ) {
            $args["date_query"][] = [
        ...
        if (isset($registered["sticky"], $request["sticky"])) {
            ...
            if ($request["sticky"]) {
                ...
                $args["post__in"] = $args["post__in"]
                    ? array_intersect($sticky_posts, $args["post__in"])
                    : $sticky_posts;
        ...
        if (
            isset(
                $registered["search_semantics"],
                $request["search_semantics"]
            ) &&
        ...
        // Reset filter.
        if ("edit" === $request["context"]) {
            remove_filter("post_password_required", [
                $this,
        ...
        $collection_url = rest_url(
            rest_get_route_for_post_type_items($this->post_type)
        );
    ...
    public function get_item_permissions_check($request)
    {
        ...
        if ($post && !empty($request->get_query_params()["password"])) {
            ...
            if (
                !hash_equals(
                    $post->post_password,
                    $request->get_query_params()["password"]
                )
            ) {
                return new WP_Error(
    ...
    public function get_item($request)
    {
        ...
        if (is_post_type_viewable(get_post_type_object($post->post_type))) {
            $response->link_header("alternate", get_permalink($post->ID), [
                "type" => "text/html",
            ]);
        }
    ...
    public function create_item($request)
    {
        ...
        if (
            ...
            in_array($prepared_post->post_status, ["draft", "pending"], true)
        ) {
            /*
        ...
        if (
            !empty($schema["properties"]["featured_media"]) &&
        ...
        if (
            ...
            !empty($request["format"])
        ) {
            set_post_format($post, $request["format"]);
        }

        if (
            !empty($schema["properties"]["template"]) &&
        ...
        if (!empty($schema["properties"]["meta"]) && isset($request["meta"])) {
            $meta_update = $this->meta->update_value(
                $request["meta"],
                $post_id
            );
    ...
    public function update_item($request)
    {
        ...
        if (
            ...
            in_array($post_status, ["draft", "pending"], true)
        ) {
        ...
        if (
            !empty($schema["properties"]["sticky"]) &&
    ...
    protected function prepare_item_for_database($request)
    {
        ...
        // Post excerpt.
        if (
            !empty($schema["properties"]["excerpt"]) &&
            isset($request["excerpt"])
        ) {
        ...
        if (!empty($schema["properties"]["date"]) && !empty($request["date"])) {
            ...
            if (!empty($date_data) && $current_date !== $date_data[0]) {
                list(
                    $prepared_post->post_date,
                    $prepared_post->post_date_gmt,
                ) = $date_data;
                $prepared_post->edit_date = true;
            }
        } elseif (
            !empty($schema["properties"]["date_gmt"]) &&
            !empty($request["date_gmt"])
        ) {
            $current_date = isset($prepared_post->ID)
                ? get_post($prepared_post->ID)->post_date_gmt
                : false;
        ...
        if (
            (!empty($schema["properties"]["date_gmt"]) &&
                $request->has_param("date_gmt") &&
                null === $request["date_gmt"]) ||
            (!empty($schema["properties"]["date"]) &&
                $request->has_param("date") &&
                null === $request["date"])
        ) {
        ...
        // Author.
        if (
            !empty($schema["properties"]["author"]) &&
            !empty($request["author"])
        ) {
        ...
        // Post password.
        if (
            !empty($schema["properties"]["password"]) &&
        ...
        // Parent.
        if (
            !empty($schema["properties"]["parent"]) &&
        ...
        // Menu order.
        if (
            !empty($schema["properties"]["menu_order"]) &&
            isset($request["menu_order"])
        ) {
        ...
        // Ping status.
        if (
            !empty($schema["properties"]["ping_status"]) &&
            !empty($request["ping_status"])
        ) {
    ...
    public function check_template($template, $request)
    {
        ...
        $allowed_templates = wp_get_theme()->get_page_templates(
            $post,
            $this->post_type
        );
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        if (rest_is_field_included("date", $fields)) {
            $data["date"] = $this->prepare_date_response(
                $post->post_date_gmt,
                $post->post_date
            );
        }
        ...
        if (rest_is_field_included("modified", $fields)) {
            $data["modified"] = $this->prepare_date_response(
            ...
            if ("0000-00-00 00:00:00" === $post->post_modified_gmt) {
                $post_modified_gmt = gmdate(
                    ...
                    strtotime($post->post_modified) -
                        (int) ((float) get_option("gmt_offset") *
                            HOUR_IN_SECONDS)
                );
            } else {
                $post_modified_gmt = $post->post_modified_gmt;
            }
            $data["modified_gmt"] = $this->prepare_date_response(
                $post_modified_gmt
            );
        }
        ...
        if (rest_is_field_included("title.rendered", $fields)) {
            ...
            remove_filter("protected_title_format", [
                $this,
                "protected_title_format",
            ]);
            remove_filter("private_title_format", [
                $this,
                "protected_title_format",
            ]);
        }
        ...
        if (rest_is_field_included("content.block_version", $fields)) {
            $data["content"]["block_version"] = block_version(
                $post->post_content
            );
        }
        ...
        if (rest_is_field_included("excerpt", $fields)) {
            if (isset($request["excerpt_length"])) {
                ...
                $override_excerpt_length = static function () use (
                    $excerpt_length
                ) {
                    return $excerpt_length;
                };

                add_filter("excerpt_length", $override_excerpt_length, 20);
            }
            ...
            if (isset($override_excerpt_length)) {
                remove_filter("excerpt_length", $override_excerpt_length, 20);
            }
        }
        ...
        foreach ($taxonomies as $taxonomy) {
            ...
            if (rest_is_field_included($base, $fields)) {
                ...
                $data[$base] = $terms
                    ? array_values(wp_list_pluck($terms, "term_id"))
                    : [];
            }
        }
        ...
        if (is_post_type_viewable($post_type_obj) && $post_type_obj->public) {
            $permalink_template_requested = rest_is_field_included(
                "permalink_template",
                $fields
            );
            $generated_slug_requested = rest_is_field_included(
                "generated_slug",
                $fields
            );
            ...
            if ($permalink_template_requested || $generated_slug_requested) {
                ...
                $sample_permalink = get_sample_permalink(
                    $post->ID,
                    $post->post_title,
                    ""
                );
            ...
            if (rest_is_field_included("class_list", $fields)) {
                $data["class_list"] = get_post_class([], $post->ID);
            }
        }
    ...
    protected function prepare_links($post)
    {
        ...
        if (
            ...
            $revisions_count = !is_wp_error($revisions)
                ? $revisions["count"]
                : 0;
            $revisions_base = sprintf(
                "/%s/%s/%d/revisions",
                $this->namespace,
                $this->rest_base,
                $post->ID
            ...
            if ($revisions_count > 0) {
                $links["predecessor-version"] = [
                    "href" => rest_url(
                        $revisions_base . "/" . $revisions["latest_id"]
                    ),
        ...
        if (
            !in_array(
                $post->post_type,
                ["attachment", "nav_menu_item", "revision"],
                true
            )
        ) {
            $attachments_url = rest_url(
    ...
    protected function get_available_actions($post, $request)
    {
        ...
        foreach ($taxonomies as $tax) {
            ...
            $create_cap = is_taxonomy_hierarchical($tax->name)
                ? $tax->cap->edit_terms
    ...
    public function get_item_schema()
    {
        ...
        foreach ($post_type_attributes as $attribute) {
            if (
                ...
                !isset($fixed_schemas[$this->post_type]) &&
                !post_type_supports($this->post_type, $attribute)
            ) {
                continue;
            }
        ...
        foreach ($taxonomies as $taxonomy) {
            ...
            if (array_key_exists($base, $schema["properties"])) {
                $taxonomy_field_name_with_conflict = !empty(
                    $taxonomy->rest_base
                )
                    ? "rest_base"
                    : "name";
            ...
            $schema["properties"][$base] = [
                ...
                "description" => sprintf(
                    __("The terms assigned to the post in the %s taxonomy."),
                    $taxonomy->name
                ),
                "type" => "array",
                "items" => [
    ...
    protected function get_schema_links()
    {
        ...
        foreach ($taxonomies as $tax) {
            ...
            /* translators: %s: Taxonomy name. */
            $assign_title = sprintf(
                __("The current user can assign terms in the %s taxonomy."),
                $tax->name
            );
            /* translators: %s: Taxonomy name. */
            $create_title = sprintf(
                __("The current user can create terms in the %s taxonomy."),
                $tax->name
            );
    ...
    public function get_collection_params()
    {
        ...
        if (
            "page" === $this->post_type ||
            post_type_supports($this->post_type, "page-attributes")
        ) {
    ...
    public function sanitize_post_statuses($statuses, $request, $parameter)
    {
        ...
        foreach ($statuses as $status) {
            ...
            if (
                ...
                $result = rest_validate_request_arg(
                    $status,
                    $request,
                    $parameter
                );
        ...
        foreach ($taxonomies as $taxonomy) {
            ...
            if ($tax_include) {
                ...
                if (rest_is_array($tax_include)) {
                    ...
                    $terms = empty($tax_include["terms"])
                        ? []
                        : $tax_include["terms"];
                    $include_children = !empty(
                        $tax_include["include_children"]
                    );

                    if (
                        isset($tax_include["operator"]) &&
            ...
            if ($tax_exclude) {
                ...
                if (rest_is_array($tax_exclude)) {
                    ...
                    $terms = empty($tax_exclude["terms"])
                        ? []
                        : $tax_exclude["terms"];
                    $include_children = !empty(
                        $tax_exclude["include_children"]
                    );
                }
    ...
    private function prepare_taxonomy_limit_schema(array $query_params)
    {
        ...
        foreach ($taxonomies as $taxonomy) {
            ...
            $query_params[$base]["description"] = sprintf(
                $query_params[$base]["description"],
                $base
            );

            $query_params[$base_exclude] = $exclude_schema;
            $query_params[$base_exclude]["description"] = sprintf(
                $query_params[$base_exclude]["description"],
                $base
            );

            if (!$taxonomy->hierarchical) {
                unset(
                    $query_params[$base]["oneOf"][1]["properties"][
                        "include_children"
                    ]
                );
                unset(
                    $query_params[$base_exclude]["oneOf"][1]["properties"][
                        "include_children"
                    ]
                );
            }
