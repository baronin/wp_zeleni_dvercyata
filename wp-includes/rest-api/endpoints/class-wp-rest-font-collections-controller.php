...
class WP_REST_Font_Collections_Controller extends WP_REST_Controller
{
    ...
    public function get_item($request)
    {
        ...
        $collection = WP_Font_Library::get_instance()->get_font_collection(
            $slug
        );

        if (!$collection) {
            return new WP_Error(
                "rest_font_collection_not_found",
                __("Font collection not found."),
                ["status" => 404]
            );
        }
    ...
    public function get_collection_params()
    {
        ...
        $query_params["context"] = $this->get_context_param([
            "default" => "view",
        ]);
