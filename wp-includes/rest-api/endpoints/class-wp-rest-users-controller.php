...
class WP_REST_Users_Controller extends WP_REST_Controller
{
    ...
    public function get_items_permissions_check($request)
    {
        ...
        if (
            in_array($request["orderby"], ["email", "registered_date"], true) &&
            !current_user_can("list_users")
        ) {
            return new WP_Error(
    ...
    public function get_items($request)
    {
        ...
        if (isset($registered["offset"]) && !empty($request["offset"])) {
            ...
            $prepared_args["offset"] =
                ($request["page"] - 1) * $prepared_args["number"];
        }
        ...
        if (!empty($request["has_published_posts"])) {
            $prepared_args["has_published_posts"] =
                true === $request["has_published_posts"]
        ...
        if (!empty($prepared_args["search"])) {
            ...
            $search_columns = array_map(static function ($column) use (
                $search_columns_mapping
            ) {
    ...
    {
        $error = new WP_Error("rest_user_invalid_id", __("Invalid user ID."), [
            "status" => 404,
        ]);
    ...
    public function get_item_permissions_check($request)
    {
        ...
        if ("edit" === $request["context"] && !current_user_can("list_users")) {
            ...
            !count_user_posts($user->ID, $types) &&
    ...
    public function create_item($request)
    {
        ...
        if (
            !empty($request["roles"]) &&
            !empty($schema["properties"]["roles"])
        ) {
            $check_permission = $this->check_role_update(
                $request["id"],
                $request["roles"]
            );
    ...
    public function update_item($request)
    {
        ...
        if (
            ...
            $request["username"] !== $user->user_login
        ) {
            return new WP_Error(
        ...
        if (
            !empty($request["slug"]) &&
            $request["slug"] !== $user->user_nicename &&
            get_user_by("slug", $request["slug"])
        ) {
            return new WP_Error("rest_user_invalid_slug", __("Invalid slug."), [
                "status" => 400,
            ]);
        }
    ...
    public function delete_item($request)
    {
        ...
        $id = $user->ID;
        $reassign =
            false === $request["reassign"]
                ? null
                : absint($request["reassign"]);
        ...
        if (!$force) {
            return new WP_Error(
                ...
                sprintf(
                    __("Users do not support trashing. Set '%s' to delete."),
                    "force=true"
                ),
                ["status" => 501]
            );
    ...
    protected function prepare_item_for_database($request)
    {
        ...
        if (
            ...
            !empty($schema["properties"]["username"])
        ) {
        ...
        if (
            ...
            !empty($schema["properties"]["first_name"])
        ) {
        ...
        if (
            ...
            !empty($schema["properties"]["last_name"])
        ) {
            $prepared_user->last_name = $request["last_name"];
        }

        if (
            isset($request["nickname"]) &&
            !empty($schema["properties"]["nickname"])
        ) {
        ...
        if (
            ...
            !empty($schema["properties"]["description"])
        ) {
        ...
        if (
            isset($request["locale"]) &&
            !empty($schema["properties"]["locale"])
        ) {
