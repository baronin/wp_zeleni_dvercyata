...
class WP_REST_Font_Faces_Controller extends WP_REST_Posts_Controller
{
    ...
    public function validate_create_font_face_settings($value, $request)
    {
        ...
        $has_valid_settings = rest_validate_value_from_schema(
            $settings,
            $schema,
            "font_face_settings"
        );
        ...
        foreach ($required as $key) {
            if (isset($settings[$key]) && !$settings[$key]) {
                return new WP_Error(
                    ...
                    sprintf(
                        __("%s cannot be empty."),
                        "font_face_setting[ $key ]"
                    ),
                    ["status" => 400]
                );
            }
        ...
        foreach ($srcs as $src) {
            ...
            if (empty($src)) {
                return new WP_Error(
                    ...
                    sprintf(
                        __("%s values must be non-empty strings."),
        ...
        foreach (array_keys($files) as $file) {
            if (!in_array($file, $srcs, true)) {
                return new WP_Error(
                    ...
                    sprintf(
                        __('File %1$s must be used in %2$s.'),
                        $file,
                        "font_face_settings[src]"
                    ),
                    ["status" => 400]
    ...
    public function sanitize_font_face_settings($value)
    {
        ...
        foreach ($settings as $key => $value) {
            $sanitize_callback =
                $schema[$key]["arg_options"]["sanitize_callback"];
    ...
    {
        $font_family = $this->get_parent_font_family_post(
            $request["font_family_id"]
        );
    ...
    public function create_item($request)
    {
        ...
        $settings["src"] =
            count($processed_srcs) === 1 ? $processed_srcs[0] : $processed_srcs;
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        if (rest_is_field_included("theme_json_version", $fields)) {
            $data["theme_json_version"] =
                static::LATEST_THEME_JSON_VERSION_SUPPORTED;
        }
    ...
    public function get_public_item_schema()
    {
        ...
        foreach (
            $schema["properties"]["font_face_settings"]["properties"]
            as &$property
        ) {
    ...
    protected function get_parent_font_family_post($font_family_id)
    {
        ...
        if (
            empty($font_family_post) ||
            empty($font_family_post->ID) ||
    ...
    protected function prepare_links($post)
    {
        ...
        return [
            ...
            "parent" => [
                "href" => rest_url(
                    $this->namespace . "/font-families/" . $post->post_parent
                ),
            ],
        ];
    }
