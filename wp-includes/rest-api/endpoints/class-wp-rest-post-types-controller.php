...
class WP_REST_Post_Types_Controller extends WP_REST_Controller
{
    ...
    public function get_items($request)
    {
        ...
        foreach ($types as $type) {
            ...
            $data[$type->name] = $this->prepare_response_for_collection(
                $post_type
            );
        }
    ...
    public function get_item($request)
    {
        ...
        if (empty($obj)) {
            return new WP_Error("rest_type_invalid", __("Invalid post type."), [
                "status" => 404,
            ]);
        }
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        $base = !empty($post_type->rest_base)
            ? $post_type->rest_base
            : $post_type->name;
        ...
        if (rest_is_field_included("template_lock", $fields)) {
            $data["template_lock"] = !empty($post_type->template_lock)
                ? $post_type->template_lock
                : false;
        }
    ...
    protected function prepare_links($post_type)
    {
        return [
            ...
            "https://api.w.org/items" => [
                "href" => rest_url(
                    rest_get_route_for_post_type_items($post_type->name)
                ),
            ],
        ];
    }
