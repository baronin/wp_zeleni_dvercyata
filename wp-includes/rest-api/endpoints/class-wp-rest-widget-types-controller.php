...
class WP_REST_Widget_Types_Controller extends WP_REST_Controller
{
    ...
    public function get_widget($id)
    {
        ...
        return new WP_Error(
            "rest_widget_type_invalid",
            __("Invalid widget type."),
            ["status" => 404]
        );
    }
    ...
    protected function get_widgets()
    {
        ...
        foreach ($wp_registered_widgets as $widget) {
            ...
            if (isset($widget["description"])) {
                $widget["description"] = html_entity_decode(
                    $widget["description"],
                    ENT_QUOTES,
                    get_bloginfo("charset")
                );
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        foreach ($extra_fields as $extra_field) {
            ...
            if (isset($widget_type[$extra_field])) {
                ...
                array_key_exists("default", $schema["properties"][$extra_field])
            ) {
            ...
            $data[$extra_field] = rest_sanitize_value_from_schema(
                $field,
                $schema["properties"][$extra_field]
            );
        }
    ...
    public function encode_form_data($request)
    {
        ...
        if (
            isset($request["instance"]["encoded"], $request["instance"]["hash"])
        ) {
            $serialized_instance = base64_decode(
                $request["instance"]["encoded"]
            );
            if (
                !hash_equals(
                    wp_hash($serialized_instance),
                    $request["instance"]["hash"]
                )
            ) {
                return new WP_Error(
                    "rest_invalid_widget",
        ...
        if (
            ...
            $new_instance = array_values(
                $request["form_data"]["widget-$id"]
            )[0];
        ...
        $response = [
            ...
            "preview" => trim(
                $this->get_widget_preview($widget_key, $instance)
            ),
            "instance" => [
        ...
        if (!empty($widget_object->widget_options["show_instance_in_rest"])) {
            ...
            $response["instance"]["raw"] = empty($instance)
                ? new stdClass()
                : $instance;
        }
