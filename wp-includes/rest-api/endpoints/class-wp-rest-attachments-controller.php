...
class WP_REST_Attachments_Controller extends WP_REST_Posts_Controller
{
    ...
    protected function prepare_items_query($prepared_args = [], $request = null)
    {
        ...
        if (!empty($request["mime_type"])) {
            ...
            if (
                isset($media_types[$parts[0]]) &&
                in_array($request["mime_type"], $media_types[$parts[0]], true)
            ) {
    ...
    public function create_item_permissions_check($request)
    {
        ...
        $prevent_unsupported_uploads = apply_filters(
            ...
            true,
            isset($files["file"]["type"]) ? $files["file"]["type"] : null
        );
    ...
    protected function insert_attachment($request)
    {
        ...
        if (empty($attachment->post_title)) {
            $attachment->post_title = preg_replace(
                '/\.[^.]+$/',
                "",
                wp_basename($file)
            );
    ...
    public function edit_media_item($request)
    {
        ...
        foreach ($modifiers as $modifier) {
            ...
            switch ($modifier["type"]) {
                ...
                case "crop":
                    ...
                    $crop_x = (int) round(
                        ($size["width"] * $args["left"]) / 100.0
                    );
                    $crop_y = (int) round(
                        ($size["height"] * $args["top"]) / 100.0
                    );
                    $width = (int) round(
                        ($size["width"] * $args["width"]) / 100.0
                    );
                    $height = (int) round(
                        ($size["height"] * $args["height"]) / 100.0
                    );
        ...
        if ($attachment_post) {
            $new_attachment_post["post_content"] =
                $attachment_post->post_content;
            $new_attachment_post["post_excerpt"] =
        ...
        $new_image_meta = wp_generate_attachment_metadata(
            $new_attachment_id,
            $saved["path"]
        );
        ...
        if (
            isset($image_meta["image_meta"]) &&
            isset($new_image_meta["image_meta"]) &&
            is_array($new_image_meta["image_meta"])
        ) {
            // Merge but skip empty values.
            foreach ((array) $image_meta["image_meta"] as $key => $value) {
                if (
                    empty($new_image_meta["image_meta"][$key]) &&
                    !empty($value)
                ) {
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        if (in_array("media_type", $fields, true)) {
            $data["media_type"] = wp_attachment_is_image($post->ID)
                ? "image"
                : "file";
        }
        ...
        if (in_array("post", $fields, true)) {
            $data["post"] = !empty($post->post_parent)
        ...
        if (in_array("missing_image_sizes", $fields, true)) {
            require_once ABSPATH . "wp-admin/includes/image.php";
            $data["missing_image_sizes"] = array_keys(
                wp_get_missing_image_subsizes($post->ID)
            );
        }
    ...
    protected function upload_from_data($data, $headers, $time = null)
    {
        ...
        $filename = self::get_filename_from_disposition(
    ...
    protected function upload_from_file($files, $headers, $time = null)
    {
        ...
        if (isset($file["error"])) {
            return new WP_Error("rest_upload_unknown_error", $file["error"], [
                "status" => 500,
            ]);
        }
