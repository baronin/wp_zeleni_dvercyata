...
class WP_REST_Templates_Controller extends WP_REST_Controller
{
    ...
    public function get_template_fallback($request)
    {
        ...
        do {
            $fallback_template = resolve_block_template(
                $request["slug"],
                $hierarchy,
                ""
            );
        ...
        $response = $fallback_template
            ? $this->prepare_item_for_response($fallback_template, $request)
            : new stdClass();
    ...
    public function _sanitize_template_id($id)
    {
        ...
        return substr($id, 0, $last_slash_pos) .
            "/" .
    ...
    public function get_item($request)
    {
        if (
            ...
            ("theme" === $request["source"] || "plugin" === $request["source"])
        ) {
    ...
    public function delete_item($request)
    {
        ...
        if ("custom" !== $template->source) {
            return new WP_Error(
                ...
                __('Templates based on theme files can\'t be removed.'),
                ["status" => 400]
            );
        }
    ...
    protected function prepare_item_for_database($request)
    {
        ...
        if (
            "wp_template" === $this->post_type &&
            isset($request["is_wp_suggestion"])
        ) {
        ...
        if ("wp_template_part" === $this->post_type) {
            if (isset($request["area"])) {
                ...
                ] = _filter_block_template_part_area($request["area"]);
            } elseif (
                null !== $template &&
                ...
                $changes->tax_input[
                    "wp_template_part_area"
                ] = _filter_block_template_part_area($template->area);
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        if (rest_is_field_included("title.rendered", $fields)) {
            if ($template->wp_id) {
                ...
                $data["title"]["rendered"] = apply_filters(
                    "the_title",
                    $template->title,
                    $template->wp_id
                );
        ...
        if (
            rest_is_field_included("is_custom", $fields) &&
            "wp_template" === $template->type
        ) {
        ...
        if (
            rest_is_field_included("area", $fields) &&
        ...
        if (rest_is_field_included("author_text", $fields)) {
            $data["author_text"] = self::get_wp_templates_author_text_field(
                $template
            );
        }
        ...
        if (rest_is_field_included("original_source", $fields)) {
            $data[
                "original_source"
            ] = self::get_wp_templates_original_source_field($template);
        }
    ...
    private static function get_wp_templates_original_source_field(
        ...
        if (
            "wp_template" === $template_object->type ||
            "wp_template_part" === $template_object->type
        ) {
            /*
             * Added by theme.
             ...
             */
            if (
                $template_object->has_theme_file &&
                ("theme" === $template_object->origin ||
                    (empty($template_object->origin) &&
                        in_array(
             ...
             */
            if (
                empty($template_object->has_theme_file) &&
                "custom" === $template_object->source &&
                empty($template_object->author)
            ) {
                return "site";
            }
        }
    ...
    {
        $original_source = self::get_wp_templates_original_source_field(
            $template_object
        );
        switch ($original_source) {
            case "theme":
                $theme_name = wp_get_theme($template_object->theme)->get(
                    "Name"
                );
                return empty($theme_name)
            ...
            case "plugin":
                ...
                $plugin_basename = plugin_basename(
                    sanitize_text_field($template_object->theme . ".php")
                );
                if (
                    isset($plugins[$plugin_basename]) &&
                    isset($plugins[$plugin_basename]["Name"])
                ) {
