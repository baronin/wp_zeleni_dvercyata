...
class WP_REST_Menu_Items_Controller extends WP_REST_Posts_Controller
{
    ...
    public function create_item($request)
    {
        ...
        $nav_menu_item_id = wp_update_nav_menu_item(
            $prepared_nav_item["menu-id"],
            $prepared_nav_item["menu-item-db-id"],
            wp_slash($prepared_nav_item),
            false
        );
    ...
    protected function prepare_item_for_database($request)
    {
        ...
        if (!is_wp_error($menu_item_obj)) {
            ...
            $position =
                0 === $menu_item_obj->menu_order
                    ? 1
                    : $menu_item_obj->menu_order;
        ...
        if (
            ...
            if (is_string($request["title"])) {
                ...
                $prepared_nav_item["menu-item-title"] =
                    $request["title"]["raw"];
            }
        }
        ...
        if (!$prepared_nav_item["menu-item-object"]) {
            ...
            if ("taxonomy" === $prepared_nav_item["menu-item-type"]) {
                $original = get_term(
                    absint($prepared_nav_item["menu-item-object-id"])
                );
                if (empty($original) || is_wp_error($original)) {
                    ...
                    $prepared_nav_item["menu-item-object"] = get_term_field(
                        "taxonomy",
                        $original
                    );
                }
                ...
                if (empty($original)) {
                    ...
                    $prepared_nav_item["menu-item-object"] = get_post_type(
                        $original
                    );
                }
            }
        ...
        if ("post_type_archive" === $prepared_nav_item["menu-item-type"]) {
            $post_type = $prepared_nav_item["menu-item-object"]
    ...
    public function get_item_schema()
    {
        ...
        foreach ($taxonomies as $taxonomy) {
            ...
            $schema["properties"][$base] = [
                ...
                "description" => sprintf(
                    __("The terms assigned to the object in the %s taxonomy."),
                    $taxonomy->name
                ),
                "type" => "array",
                "items" => [
    ...
    {
        $menu_ids = wp_get_post_terms($menu_item_id, "nav_menu", [
            "fields" => "ids",
        ]);
        $menu_id = 0;
