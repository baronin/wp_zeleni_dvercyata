...
class WP_REST_Comments_Controller extends WP_REST_Controller
{
        ...
        if (!current_user_can("edit_posts")) {
            ...
            if (!empty($forbidden_params)) {
                return new WP_Error(
                    ...
                    sprintf(
                        __("Query parameter not permitted: %s"),
                        implode(", ", $forbidden_params)
                    ),
                    ["status" => rest_authorization_required_code()]
                );
            }
    ...
    public function get_items($request)
    {
        ...
        if (isset($registered["orderby"])) {
            $prepared_args["orderby"] = $this->normalize_query_param(
                $request["orderby"]
            );
        }
        ...
        if (isset($registered["page"]) && empty($request["offset"])) {
            $prepared_args["offset"] =
                $prepared_args["number"] * (absint($request["page"]) - 1);
        }
    ...
    public function create_item_permissions_check($request)
    {
        ...
        if (
            ...
            return new WP_Error(
                ...
                sprintf(
                    __("Sorry, you are not allowed to edit '%s' for comments."),
                    "author"
                ),
                ["status" => rest_authorization_required_code()]
            );
        ...
        if (
            ...
            if (
                ...
                $request["author_ip"] !== $_SERVER["REMOTE_ADDR"]
            ) {
                return new WP_Error(
    ...
    public function create_item($request)
    {
        ...
        $missing_author =
            empty($prepared_comment["user_id"]) &&
        ...
        $prepared_comment["comment_approved"] = wp_allow_comment(
            $prepared_comment,
            true
        );

        if (is_wp_error($prepared_comment["comment_approved"])) {
            $error_code = $prepared_comment[
                "comment_approved"
            ]->get_error_code();
            $error_message = $prepared_comment[
                "comment_approved"
            ]->get_error_message();
         ...
         */
        $prepared_comment = apply_filters(
        ...
        $comment_id = wp_insert_comment(
            wp_filter_comment(wp_slash((array) $prepared_comment))
        );
    ...
    public function update_item($request)
    {
        ...
        if (
            isset($request["type"]) &&
            get_comment_type($id) !== $request["type"]
        ) {
            return new WP_Error(
        ...
        if (empty($prepared_args) && isset($request["status"])) {
            ...
            if (
                isset($prepared_args["comment_content"]) &&
                empty($prepared_args["comment_content"])
            ) {
                return new WP_Error(
                    "rest_comment_content_invalid",
                    __("Invalid comment content."),
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        if (in_array("status", $fields, true)) {
            $data["status"] = $this->prepare_status_response(
    ...
    protected function prepare_item_for_database($request)
    {
        ...
        if (isset($request["content"]) && is_string($request["content"])) {
            ...
            isset($request["content"]["raw"]) &&
            is_string($request["content"]["raw"])
        ) {
            $prepared_comment["comment_content"] = trim(
                $request["content"]["raw"]
            );
        }
        ...
        if (
            ...
            !empty($_SERVER["REMOTE_ADDR"]) &&
            rest_is_ip_address($_SERVER["REMOTE_ADDR"])
        ) {
        ...
        if (!empty($request["author_user_agent"])) {
            ...
            $prepared_comment["comment_agent"] = $request->get_header(
                "user_agent"
            );
        }
        ...
        if (!empty($request["date"])) {
            ...
            if (!empty($date_data)) {
                list(
                    $prepared_comment["comment_date"],
                    $prepared_comment["comment_date_gmt"],
                ) = $date_data;
            }
        ...
        if (get_option("show_avatars")) {
            ...
            foreach ($avatar_sizes as $size) {
                $avatar_properties[$size] = [
                    ...
                    "description" => sprintf(
                        __("Avatar URL with image size of %d pixels."),
                        $size
                    ),
                    "type" => "string",
                    "format" => "uri",
    ...
    protected function check_read_post_permission($post, $request)
    {
        ...
        $requested_post =
            !empty($request["post"]) &&
            (!is_array($request["post"]) || 1 === count($request["post"]));
        $requested_comment = !empty($request["id"]);
        if (
            ($requested_post || $requested_comment) &&
