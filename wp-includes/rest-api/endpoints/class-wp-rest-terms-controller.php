...
class WP_REST_Terms_Controller extends WP_REST_Controller
{
    ...
    public function __construct($taxonomy)
    {
        ...
        $this->rest_base = !empty($tax_obj->rest_base)
            ? $tax_obj->rest_base
            : $tax_obj->name;
        $this->namespace = !empty($tax_obj->rest_namespace)
            ? $tax_obj->rest_namespace
            : "wp/v2";
    ...
    public function get_items($request)
    {
        ...
        if (isset($prepared_args["orderby"]) && isset($request["orderby"])) {
            ...
            if (isset($orderby_mappings[$request["orderby"]])) {
                $prepared_args["orderby"] =
                    $orderby_mappings[$request["orderby"]];
            }
        }
        ...
        if (
            $taxonomy_obj->hierarchical &&
            isset($registered["parent"], $request["parent"])
        ) {
            if (0 === $request["parent"]) {
        ...
        if (!empty($prepared_args["post"])) {
            $query_result = wp_get_object_terms(
                $prepared_args["post"],
                $this->taxonomy,
                $prepared_args
            );
        ...
        $collection_url = rest_url(
            rest_get_route_for_taxonomy_items($this->taxonomy)
        );
    ...
    {
        $error = new WP_Error("rest_term_invalid", __("Term does not exist."), [
            "status" => 404,
        ]);
    ...
    public function create_item($request)
    {
        ...
        $term = wp_insert_term(
            wp_slash($prepared_term->name),
            $this->taxonomy,
            wp_slash((array) $prepared_term)
        );
        if (is_wp_error($term)) {
            /*
        ...
        $response->header(
            "Location",
            rest_url(
                $this->namespace . "/" . $this->rest_base . "/" . $term->term_id
            )
        );
    ...
    public function delete_item($request)
    {
        ...
        if (!$force) {
            return new WP_Error(
                ...
                sprintf(
                    __("Terms do not support trashing. Set '%s' to delete."),
                    "force=true"
                ),
                ["status" => 501]
            );
    ...
    public function prepare_item_for_database($request)
    {
        ...
        if (
            isset($request["taxonomy"]) &&
            !empty($schema["properties"]["taxonomy"])
        ) {
    ...
    protected function prepare_links($term)
    {
        $links = [
            ...
            "about" => [
                "href" => rest_url(
                    sprintf("wp/v2/taxonomies/%s", $this->taxonomy)
                ),
            ],
        ];
