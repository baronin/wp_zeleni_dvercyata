 ...
 */
class WP_REST_Global_Styles_Revisions_Controller extends
    WP_REST_Revisions_Controller
{
    ...
    protected function get_decoded_global_styles_json($raw_json)
    {
        ...
        if (
            ...
            isset($decoded_json["isGlobalStylesUserThemeJSON"]) &&
            true === $decoded_json["isGlobalStylesUserThemeJSON"]
        ) {
            return $decoded_json;
        }
    ...
    public function get_items($request)
    {
        ...
        $global_styles_config = $this->get_decoded_global_styles_json(
            $parent->post_content
        );
        ...
        if (wp_revisions_enabled($parent)) {
            ...
            $offset = isset($query_args["offset"])
                ? (int) $query_args["offset"]
                : 0;
            $page = isset($query_args["paged"])
            ...
            if ($revisions_query->query_vars["posts_per_page"] > 0) {
                $max_pages = (int) ceil(
                    $total_revisions /
                        (int) $revisions_query->query_vars["posts_per_page"]
                );
            } else {
    ...
    public function prepare_item_for_response($post, $request)
    {
        ...
        if (
            !empty($global_styles_config["styles"]) ||
            !empty($global_styles_config["settings"])
        ) {
            $theme_json = new WP_Theme_JSON($global_styles_config, "custom");
            $global_styles_config = $theme_json->get_raw_data();
            if (rest_is_field_included("settings", $fields)) {
                $data["settings"] = !empty($global_styles_config["settings"])
                    ? $global_styles_config["settings"]
                    : new stdClass();
            }
            if (rest_is_field_included("styles", $fields)) {
                $data["styles"] = !empty($global_styles_config["styles"])
                    ? $global_styles_config["styles"]
                    : new stdClass();
            }
        }
        ...
        if (rest_is_field_included("date_gmt", $fields)) {
            $data["date_gmt"] = $this->prepare_date_response(
                $post->post_date_gmt
            );
        }
        ...
        $resolved_theme_uris = WP_Theme_JSON_Resolver::get_resolved_theme_uris(
            $theme_json
        );
