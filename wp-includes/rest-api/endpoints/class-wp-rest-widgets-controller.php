...
class WP_REST_Widgets_Controller extends WP_REST_Controller
{
    ...
    public function get_items_permissions_check($request)
    {
        ...
        if (
            isset($request["sidebar"]) &&
            $this->check_read_sidebar_permission($request["sidebar"])
        ) {
            return true;
        }
    ...
    public function get_items($request)
    {
        ...
        foreach (wp_get_sidebars_widgets() as $sidebar_id => $widget_ids) {
            ...
            if (
                ...
                !$this->check_read_sidebar_permission($sidebar_id)
            ) {
                continue;
            }
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        if (rest_is_field_included("instance", $fields)) {
            ...
            if ($widget_object && isset($parsed_id["number"])) {
                ...
                $prepared["instance"]["encoded"] = base64_encode(
                    $serialized_instance
                );
                ...
                if (
                    ...
                    $prepared["instance"]["raw"] = empty($instance)
                        ? new stdClass()
                        : $instance;
                }
            }
    ...
    {
        $id_base = !empty($prepared["id_base"])
            ? $prepared["id_base"]
            : $prepared["id"];
        ...
        return [
            ...
            "https://api.w.org/sidebar" => [
                "href" => rest_url(
                    sprintf("wp/v2/sidebars/%s/", $prepared["sidebar"])
                ),
            ],
        ];
    }
