...
class WP_REST_Plugins_Controller extends WP_REST_Controller
{
            ...
            [
                ...
                "args" => [
                    ...
                    "status" => [
                        ...
                        "type" => "string",
                        "enum" => is_multisite()
                            ? ["inactive", "active", "network-active"]
                            : ["inactive", "active"],
                        "default" => "inactive",
                    ],
                ],
    ...
    public function create_item($request)
    {
        ...
        if ("inactive" !== $request["status"]) {
            $can_change_status = $this->plugin_status_permission_check(
                $file,
                $request["status"],
                "inactive"
            );
            ...
            $changed_status = $this->handle_plugin_status(
                $file,
                $request["status"],
                "inactive"
            );
        ...
        $installed_locales = apply_filters(
        ...
        $language_packs = array_filter($language_packs, static function (
            $pack
        ) use ($installed_locales) {
    ...
    public function update_item($request)
    {
        ...
        if ($request["status"] && $status !== $request["status"]) {
            $handled = $this->handle_plugin_status(
                $request["plugin"],
                $request["status"],
                $status
            );
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        $marked = _get_plugin_data_markup_translate(
            $item["_file"],
            $item,
            true
        );
    ...
    protected function plugin_status_permission_check(
        ...
        if (
            is_multisite() &&
            ("network-active" === $current_status ||
                "network-active" === $new_status) &&
            !current_user_can("manage_network_plugins")
        ) {
            return new WP_Error(
        ...
        if (
            ("active" === $new_status || "network-active" === $new_status) &&
     ...
     * @return true|WP_Error
     */
    protected function handle_plugin_status(
        $plugin,
        $new_status,
        $current_status
    ) {
    ...
    public function validate_plugin_param($file)
    {
        if (
            !is_string($file) ||
            !preg_match("/" . self::PATTERN . "/u", $file)
        ) {
            return false;
        }
    ...
    protected function does_plugin_match_request($request, $item)
    {
        ...
        if ($search) {
            ...
            foreach ($item as $field) {
                if (
                    is_string($field) &&
                    str_contains(strip_tags($field), $search)
                ) {
                    $matched_search = true;
                    break;
                }
        ...
        if (
            $status &&
            !in_array($this->get_plugin_status($item["_file"]), $status, true)
        ) {
            return false;
        }
    ...
    protected function is_filesystem_available()
    {
        ...
        return new WP_Error(
            "fs_unavailable",
            __("The filesystem is currently unavailable for managing plugins."),
            ["status" => 500]
        );
    }
