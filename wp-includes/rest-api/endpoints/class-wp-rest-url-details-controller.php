...
class WP_REST_URL_Details_Controller extends WP_REST_Controller
{
    ...
    public function parse_url_details($request)
    {
        ...
        if (!empty($cached_response)) {
            ...
            if (
                is_wp_error($remote_url_response) ||
                empty($remote_url_response)
            ) {
    ...
    private function get_icon($html, $url)
    {
        ...
        $pattern =
            '#<link\s[^>]*rel=(?:[\"\']??)\s*(?:icon|shortcut icon|icon shortcut)\s*(?:[\"\']??)[^>]*\/?>#isU';
        ...
        if (
            isset($parsed_icon["scheme"]) &&
            "data" === $parsed_icon["scheme"]
        ) {
            return $icon;
        }
     ...
     */
    private function get_metadata_from_meta_element(
        $meta_elements,
        $attr,
        $attr_value
    ) {
        ...
        foreach ($meta_elements[0] as $index => $element) {
             ...
             */
            if (
                isset($meta_elements[2][$index]) &&
                is_string($meta_elements[2][$index])
            ) {
