...
class WP_REST_Themes_Controller extends WP_REST_Controller
{
    ...
    public function get_items_permissions_check($request)
    {
        ...
        if (
            isset($registered["status"], $request["status"]) &&
    ...
    public function get_items($request)
    {
        ...
        foreach ($active_themes as $theme) {
            $theme_status = $this->is_same_theme($theme, $current_theme)
                ? "active"
                : "inactive";
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        if (rest_is_field_included("status", $fields)) {
            $data["status"] = $this->is_same_theme($theme, $current_theme)
                ? "active"
                : "inactive";
        }

        if (
            rest_is_field_included("theme_supports", $fields) &&
            $this->is_same_theme($theme, $current_theme)
        ) {
            foreach (get_registered_theme_features() as $feature => $config) {
                ...
                if (
                    !rest_is_field_included("theme_supports.{$name}", $fields)
                ) {
                    continue;
                }

                if (!current_theme_supports($feature)) {
                    $data["theme_supports"][$name] =
                        $config["show_in_rest"]["schema"]["default"];
                    continue;
                }
        ...
        if (
            rest_is_field_included("default_template_types", $fields) &&
            $this->is_same_theme($theme, $current_theme)
        ) {
            $default_template_types = [];
            foreach (
                get_default_block_template_types()
                as $slug => $template_type
            ) {
        ...
        if (
            rest_is_field_included("default_template_part_areas", $fields) &&
            $this->is_same_theme($theme, $current_theme)
        ) {
            $data[
     ...
     */
    protected function prepare_theme_support(
        $support,
        $args,
        $feature,
        $request
    ...
    public function get_item_schema()
    {
        ...
        foreach (get_registered_theme_features() as $feature => $config) {
            ...
            $schema["properties"]["theme_supports"]["properties"][$name] =
                $config["show_in_rest"]["schema"];
        }
