...
class WP_REST_Autosaves_Controller extends WP_REST_Revisions_Controller
{
    ...
    public function __construct($parent_post_type)
    {
        ...
        if (!$parent_controller) {
            $parent_controller = new WP_REST_Posts_Controller(
                $parent_post_type
            );
        }
        ...
        if (!$revisions_controller) {
            $revisions_controller = new WP_REST_Revisions_Controller(
                $parent_post_type
            );
        }
        $this->revisions_controller = $revisions_controller;
        $this->rest_base = "autosaves";
        $this->parent_base = !empty($post_type_object->rest_base)
            ? $post_type_object->rest_base
            : $post_type_object->name;
        $this->namespace = !empty($post_type_object->rest_namespace)
        ...
        register_rest_route(
            ...
            [
                ...
                [
                    ...
                    "args" => $this->parent_controller->get_endpoint_args_for_item_schema(
    ...
    public function create_item_permissions_check($request)
    {
        ...
        return $this->parent_controller->update_item_permissions_check(
            $request
        );
    }
    ...
    public function create_item($request)
    {
        ...
        $prepared_post = $this->parent_controller->prepare_item_for_database(
            $request
        );
    ...
    public function create_post_autosave($post_data, array $meta = [])
    {
        ...
        if (!empty($meta)) {
            $revisioned_meta_keys = wp_post_revision_meta_keys(
                $post->post_type
            );
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        $response = $this->revisions_controller->prepare_item_for_response(
            $post,
            $request
        );
        ...
        if (in_array("preview_link", $fields, true)) {
            ...
            if (false !== $parent_id) {
                ...
                $preview_query_args["preview_nonce"] = wp_create_nonce(
                    "post_preview_" . $parent_id
                );
            }

            $response->data["preview_link"] = get_preview_post_link(
                $preview_post_id,
                $preview_query_args
            );
        }
        ...
        $response->data = $this->add_additional_fields_to_object(
            $response->data,
            $request
        );
        $response->data = $this->filter_response_by_context(
            $response->data,
            $context
        );
