...
class WP_REST_Sidebars_Controller extends WP_REST_Controller
{
    ...
    public function get_item($request)
    {
        ...
        if (!$sidebar) {
            return new WP_Error(
                "rest_sidebar_not_found",
                __("No sidebar exists with that id."),
                ["status" => 404]
            );
        }
            ...
            foreach ($sidebars as $sidebar_id => $widgets) {
                foreach ($widgets as $i => $widget_id) {
                    ...
                    if (
                        ...
                        in_array($widget_id, $request["widgets"], true)
                    ) {
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        if (isset($wp_registered_sidebars[$id])) {
            ...
            $sidebar["name"] = isset($registered_sidebar["name"])
                ? $registered_sidebar["name"]
                : "";
            $sidebar["description"] = isset($registered_sidebar["description"])
                ? wp_sidebar_description($id)
                : "";
            $sidebar["class"] = isset($registered_sidebar["class"])
                ? $registered_sidebar["class"]
                : "";
            $sidebar["before_widget"] = isset(
                $registered_sidebar["before_widget"]
            )
                ? $registered_sidebar["before_widget"]
                : "";
            $sidebar["after_widget"] = isset(
                $registered_sidebar["after_widget"]
            )
                ? $registered_sidebar["after_widget"]
                : "";
            $sidebar["before_title"] = isset(
                $registered_sidebar["before_title"]
            )
                ? $registered_sidebar["before_title"]
                : "";
            $sidebar["after_title"] = isset($registered_sidebar["after_title"])
                ? $registered_sidebar["after_title"]
                : "";
        } else {
