...
class WP_REST_Server
{
    ...
    public function serve_request($path = null)
    {
        ...
        if ($jsonp_callback) {
            if (!$jsonp_enabled) {
                echo $this->json_error(
                    ...
                    __("JSONP support is disabled on this site."),
                    400
                );
                return false;
            }
            ...
            if (!wp_check_jsonp_callback($jsonp_callback)) {
                echo $this->json_error(
                    ...
                    __("Invalid JSONP callback function."),
                    400
                );
                return false;
            }
        ...
        if (
            ...
            (true === $method_overridden && str_starts_with($code, "4"))
        ) {
        ...
        if (!$served) {
            ...
            $result = wp_json_encode(
                $result,
                $this->get_json_encode_options($request)
            );
    ...
    protected static function get_target_hints_for_link($link)
    {
        ...
        $headers = rest_send_allow_header(
            $response,
            $server,
            $request
        )->get_headers();
    ...
    public static function get_compact_response_links($response)
    {
        ...
        foreach ($links as $rel => $items) {
            ...
            foreach ($curies as $curie) {
                ...
                $rel_regex = str_replace(
                    "\{rel\}",
                    "(.+)",
                    preg_quote($curie["href"], "!")
                );
            ...
            foreach ($links as $item) {
                ...
                if (!array_key_exists($item["href"], $this->embed_cache)) {
                    ...
                    if (empty($request["per_page"])) {
                        ...
                        if (
                            !is_wp_error($matched) &&
                            isset($matched[1]["args"]["per_page"]["maximum"])
                        ) {
                            $request["per_page"] =
                                (int) $matched[1]["args"]["per_page"][
                                    "maximum"
                                ];
                        }
                    }
                    ...
                    $this->embed_cache[$item["href"]] = $this->response_to_data(
                        $response,
                        false
                    );
                }
    ...
    public function register_route(
        ...
        if ($override || empty($this->endpoints[$route])) {
            ...
            $this->endpoints[$route] = array_merge(
    ...
    public function get_routes($route_namespace = "")
    {
        ...
        if ($route_namespace) {
            $endpoints = wp_list_filter($endpoints, [
    ...
    public function dispatch($request)
    {
        ...
        $response = $this->respond_to_request(
            $request,
            $route,
            $handler,
            $error
    ...
    protected function match_request_to_handler($request)
    {
        ...
        foreach ($this->get_namespaces() as $namespace) {
            if (
                str_starts_with(trailingslashit(ltrim($path, "/")), $namespace)
            ) {
    ...
    public function get_index($request)
    {
        ...
        $available = [
            ...
            "authentication" => [],
            "routes" => $this->get_data_for_routes(
                $this->get_routes(),
                $request["context"]
            ),
        ];
        ...
        if (
            ...
            if (
                rest_is_field_included("site_icon", $fields) ||
                rest_is_field_included("site_icon_url", $fields)
            ) {
                $this->add_site_icon_to_index($response);
            }
        }
     ...
     */
    protected function add_active_theme_link_to_index(
        ...
        if ($should_add) {
            $theme = wp_get_theme();
            $response->add_link(
                "https://api.w.org/active-theme",
                rest_url("wp/v2/themes/" . $theme->get_stylesheet())
            );
        }
    }
     ...
     */
    protected function add_image_to_index(
        WP_REST_Response $response,
        $image_id,
        $type
    ) {
    ...
    public function get_data_for_route($route, $callbacks, $context = "view")
    {
        ...
        if (isset($this->route_options[$route])) {
            ...
            $allow_batch = isset($options["allow_batch"])
                ? $options["allow_batch"]
                : false;
        ...
        $allowed_schema_keywords = array_flip(
        ...
        foreach ($callbacks as $callback) {
            ...
            $data["methods"] = array_merge(
                $data["methods"],
                array_keys($callback["methods"])
            );
            $endpoint_data = [
                "methods" => array_keys($callback["methods"]),
            ];

            $callback_batch = isset($callback["allow_batch"])
                ? $callback["allow_batch"]
                : $allow_batch;
    ...
    public function serve_batch_request_v1(WP_REST_Request $batch_request)
    {
        ...
        foreach ($requests as $single_request) {
            ...
            if (!$error) {
                ...
                if (isset($handler["allow_batch"])) {
                    ...
                    $allow_batch = isset($route_options["allow_batch"])
                        ? $route_options["allow_batch"]
                        : false;
                }
        ...
        if (
            $has_error &&
            "require-all-validate" === $batch_request["validation"]
        ) {
            foreach ($validation as $valid) {
                if (is_wp_error($valid)) {
                    $responses[] = $this->envelope_response(
                        $this->error_to_response($valid),
                        false
                    )->get_data();
                } else {
                    $responses[] = null;
    ...
    public function get_headers($server)
    {
        ...
        foreach ($server as $key => $value) {
            if (str_starts_with($key, "HTTP_")) {
                ...
                "REDIRECT_HTTP_AUTHORIZATION" === $key &&
