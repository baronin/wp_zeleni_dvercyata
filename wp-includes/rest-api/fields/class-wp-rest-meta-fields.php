...
abstract class WP_REST_Meta_Fields
{
    ...
    public function get_value($object_id, $request)
    {
        ...
        foreach ($fields as $meta_key => $args) {
            ...
            if ($args["single"]) {
                ...
                $value = $this->prepare_value_for_response(
                    $value,
                    $request,
                    $args
                );
    ...
    public function update_value($meta, $object_id)
    {
        ...
        foreach ($fields as $meta_key => $args) {
            ...
            if ($args["single"]) {
                ...
                $result = $this->update_multi_meta_value(
                    $object_id,
                    $meta_key,
                    $name,
                    $value
    ...
    protected function delete_meta_value($object_id, $meta_key, $name)
    {
        ...
        if (
            null ===
            get_metadata_raw($meta_type, $object_id, wp_slash($meta_key))
        ) {
            return true;
        }
     ...
     */
    protected function update_multi_meta_value(
        $object_id,
        $meta_key,
        $name,
        $values
        ...
        $current_values = get_metadata_raw(
            $meta_type,
            $object_id,
            $meta_key,
            false
        ...
        foreach ($to_add as $add_key => $value) {
            $remove_keys = array_keys(
                array_filter($current_values, function ($stored_value) use (
                    ...
                    return $this->is_meta_value_same_as_stored_value(
                        $meta_key,
                        $subtype,
                        $stored_value,
                        $value
        ...
        $to_remove = array_map(
            "maybe_unserialize",
            array_unique(array_map("maybe_serialize", $to_remove))
        );
    ...
    protected function update_meta_value($object_id, $meta_key, $name, $value)
    {
        ...
        if (
            ...
            return new WP_Error(
                ...
                /* translators: %s: Custom field key. */
                sprintf(
                    __("Could not update the meta value of %s in database."),
                    $meta_key
                ),
                [
                    "key" => $name,
     ...
     * @return bool
     */
    protected function is_meta_value_same_as_stored_value(
        $meta_key,
        $subtype,
        $stored_value,
        $user_value
    ...
    protected function get_registered_fields()
    {
        ...
        if (!empty($meta_subtype)) {
            $meta_keys = array_merge(
                $meta_keys,
                get_registered_meta_keys($meta_type, $meta_subtype)
            );
        }
        ...
        foreach ($meta_keys as $name => $args) {
            ...
            $rest_args["schema"] = array_merge(
                $default_schema,
                $rest_args["schema"]
            );

            $type = !empty($rest_args["type"]) ? $rest_args["type"] : null;
            $type = !empty($rest_args["schema"]["type"])
                ? $rest_args["schema"]["type"]
                : $type;

            if (null === $rest_args["schema"]["default"]) {
                $rest_args["schema"][
                    "default"
                ] = static::get_empty_value_for_type($type);
            }
    ...
    public static function prepare_value($value, $request, $args)
    {
        ...
        if (
            "" === $value &&
            in_array($schema["type"], ["boolean", "integer", "number"], true)
        ) {
