...
class WP_REST_Post_Format_Search_Handler extends WP_REST_Search_Handler
{
    ...
    public function search_items(WP_REST_Request $request)
    {
        ...
        foreach ($format_slugs as $format_slug) {
            if (!empty($query_args["search"])) {
                ...
                $format_slug_match =
                    stripos($format_slug, $query_args["search"]) !== false;
                $format_string_match =
                    stripos($format_string, $query_args["search"]) !== false;
    ...
    public function prepare_item($id, array $fields)
    {
        ...
        if (in_array(WP_REST_Search_Controller::PROP_URL, $fields, true)) {
            $data[WP_REST_Search_Controller::PROP_URL] = get_post_format_link(
                $id
            );
        }
