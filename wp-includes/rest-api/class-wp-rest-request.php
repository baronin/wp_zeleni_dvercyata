...
class WP_REST_Request implements ArrayAccess
{
    ...
    public function is_json_content_type()
    {
        ...
        return isset($content_type["value"]) &&
            wp_is_json_media_type($content_type["value"]);
    }
    ...
    public function has_param($key)
    {
        ...
        foreach ($order as $type) {
            if (
                is_array($this->params[$type]) &&
                array_key_exists($key, $this->params[$type])
            ) {
                return true;
            }
        }
    ...
    public function sanitize_params()
    {
        ...
        foreach ($order as $type) {
            ...
            foreach ($this->params[$type] as $key => $value) {
                ...
                if (
                    !array_key_exists("sanitize_callback", $param_args) &&
                ...
                if (is_wp_error($sanitized_value)) {
                    $invalid_params[$key] = implode(
                        " ",
                        $sanitized_value->get_error_messages()
                    );
                    $invalid_details[$key] = rest_convert_error_to_response(
                        $sanitized_value
                    )->get_data();
                } else {
        ...
        foreach ($args as $key => $arg) {
            ...
            if (null !== $param && !empty($arg["validate_callback"])) {
                ...
                if (is_wp_error($valid_check)) {
                    $invalid_params[$key] = implode(
                        " ",
                        $valid_check->get_error_messages()
                    );
                    $invalid_details[$key] = rest_convert_error_to_response(
                        $valid_check
                    )->get_data();
