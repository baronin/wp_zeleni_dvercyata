...
class WP_Theme_JSON
{
    ...
    const PRESETS_METADATA = [
        [
            ...
            "prevent_override" => ["dimensions", "defaultAspectRatios"],
        ...
        [
            ...
            "prevent_override" => ["typography", "defaultFontSizes"],
        ...
        [
            ...
            "prevent_override" => ["spacing", "defaultSpacingSizes"],
            "use_default_names" => true,
            "value_key" => "size",
    ...
    const INDIRECT_PROPERTIES_METADATA = [
        ...
        "column-gap" => [["spacing", "blockGap", "left"]],
        "row-gap" => [["spacing", "blockGap", "top"]],
        "max-width" => [["layout", "contentSize"], ["layout", "wideSize"]],
        "background-image" => [["background", "backgroundImage", "url"]],
    ];
    ...
    public function __construct(
        $theme_json = ["version" => self::LATEST_SCHEMA],
        $origin = "theme"
    ) {
        ...
        $this->theme_json = static::unwrap_shared_block_style_variations(
            $this->theme_json,
            $valid_variations
        );
        $this->theme_json = static::sanitize(
            $this->theme_json,
            $valid_block_names,
            $valid_element_names,
            $valid_variations
        );
        $this->theme_json = static::maybe_opt_in_into_settings(
            $this->theme_json
        );
            ...
            foreach (static::PRESETS_METADATA as $preset_metadata) {
                ...
                if (null !== $preset) {
                    ...
                    if (isset($preset[0]) || empty($preset)) {
                        _wp_array_set($this->theme_json, $path, [
                            $origin => $preset,
                        ]);
                    }
                }
        ...
        $scale_path = ["settings", "spacing", "spacingScale"];
        ...
        if (null !== $spacing_scale) {
            ...
            if (
                empty(
                    array_intersect(
                        array_keys($spacing_scale),
                        static::VALID_ORIGINS
                    )
                )
            ) {
                _wp_array_set($this->theme_json, $scale_path, [
                    $origin => $spacing_scale,
                ]);
            }
        }

        // Pre-generate the spacingSizes from spacingScale.
        $scale_path = ["settings", "spacing", "spacingScale", $origin];
        $spacing_scale = _wp_array_get($this->theme_json, $scale_path, null);
        if (isset($spacing_scale)) {
            $sizes_path = ["settings", "spacing", "spacingSizes", $origin];
            $spacing_sizes = _wp_array_get($this->theme_json, $sizes_path, []);
            $spacing_scale_sizes = static::compute_spacing_sizes(
                $spacing_scale
            );
            $merged_spacing_sizes = static::merge_spacing_sizes(
                $spacing_scale_sizes,
                $spacing_sizes
            );
            _wp_array_set(
     ...
     */
    private static function unwrap_shared_block_style_variations(
        $theme_json,
        $valid_variations
    ) {
        if (
            empty($theme_json["styles"]["variations"]) ||
            empty($valid_variations)
        ) {
            return $theme_json;
        }
        ...
        foreach ($valid_variations as $block_type => $registered_variations) {
            foreach ($registered_variations as $variation_name) {
                $block_level_data =
                    $new_theme_json["styles"]["blocks"][$block_type][
                        "variations"
                    ][$variation_name] ?? [];
                $top_level_data = $variations[$variation_name] ?? [];
    ...
    protected static function maybe_opt_in_into_settings($theme_json)
    {
        ...
        if (
            isset($new_theme_json["settings"]["blocks"]) &&
            is_array($new_theme_json["settings"]["blocks"])
        ) {
            foreach ($new_theme_json["settings"]["blocks"] as &$block) {
                if (
                    isset($block["appearanceTools"]) &&
    ...
    protected static function sanitize(
        ...
        $output = array_intersect_key(
            $input,
            array_flip(static::VALID_TOP_LEVEL_KEYS)
        );
        ...
        foreach (array_keys($styles_non_top_level) as $section) {
            ...
            if (
                array_key_exists($section, $styles_non_top_level) &&
        ...
        foreach ($valid_element_names as $element) {
            ...
            if (isset(static::VALID_ELEMENT_PSEUDO_SELECTORS[$element])) {
                foreach (
                    static::VALID_ELEMENT_PSEUDO_SELECTORS[$element]
                    as $pseudo_selector
                ) {
                    $schema_styles_elements[$element][
                        $pseudo_selector
        ...
        foreach ($valid_block_names as $block) {
            ...
            if (
                ...
                $style_variation_names = array_intersect(
                    array_keys(
                        $input["styles"]["blocks"][$block]["variations"]
                    ),
            ...
            if (!empty($style_variation_names)) {
                $schema_styles_variations = array_fill_keys(
                    $style_variation_names,
                    $block_style_variation_styles
                );
            }

            $schema_styles_blocks[$block][
                "variations"
            ] = $schema_styles_variations;
        }
        ...
        $schema["settings"]["typography"][
            "fontFamilies"
        ] = static::schema_in_root_and_per_origin(static::FONT_FAMILY_SCHEMA);
        ...
        foreach (["styles", "settings"] as $subtree) {
            ...
            $result = static::remove_keys_not_in_schema(
                $input[$subtree],
                $schema[$subtree]
            );
    ...
    protected static function get_blocks_metadata()
    {
        ...
        if (empty($blocks)) {
            ...
            foreach (
                ...
                as $block_name => $block_metadata
            ) {
                if (!empty($registered_styles[$block_name])) {
                    $style_selectors = $block_metadata["styleVariations"] ?? [];
                    ...
                    static::$blocks_metadata[$block_name][
                        "styleVariations"
                    ] = $style_selectors;
                }
            }
        ...
        foreach ($blocks as $block_name => $block_type) {
            ...
            static::$blocks_metadata[$block_name][
                "selectors"
            ] = static::get_block_selectors($block_type, $root_selector);
            ...
            $duotone_selector = wp_get_block_css_selector(
                $block_type,
                "filter.duotone"
            );
            ...
            if (null === $duotone_selector) {
                ...
                if ($duotone_support) {
                    $root_selector = wp_get_block_css_selector($block_type);
                    $duotone_selector = static::scope_selector(
                        $root_selector,
                        $duotone_support
                    );
                }
            ...
            $registered_styles = $style_registry->get_registered_styles_for_block(
                $block_name
            );
        ...
        foreach ($tree as $key => $value) {
            ...
            if (is_array($schema[$key])) {
                if (!is_array($value)) {
                    ...
                    $tree[$key] = self::remove_keys_not_in_schema(
                        $value,
                        $schema[$key]
                    );
    ...
    public function get_stylesheet(
        ...
        $setting_nodes = static::get_setting_nodes(
            $this->theme_json,
            $blocks_metadata
        );

        $root_style_key = array_search(
            static::ROOT_BLOCK_SELECTOR,
            array_column($style_nodes, "selector"),
            true
        );
        $root_settings_key = array_search(
            static::ROOT_BLOCK_SELECTOR,
            array_column($setting_nodes, "selector"),
            true
        );

        if (!empty($options["scope"])) {
            foreach ($setting_nodes as &$node) {
                $node["selector"] = static::scope_selector(
                    $options["scope"],
                    $node["selector"]
                );
            }
            foreach ($style_nodes as &$node) {
                $node = static::scope_style_node_selectors(
                    $options["scope"],
                    $node
                );
            }
        ...
        if (!empty($options["root_selector"])) {
            if (false !== $root_settings_key) {
                $setting_nodes[$root_settings_key]["selector"] =
                    $options["root_selector"];
            }
            if (false !== $root_style_key) {
                $style_nodes[$root_style_key]["selector"] =
                    $options["root_selector"];
            }
        }
        ...
        if (in_array("styles", $types, true)) {
            if (
                false !== $root_style_key &&
                empty($options["skip_root_layout_styles"])
            ) {
                $stylesheet .= $this->get_root_layout_rules(
                    $style_nodes[$root_style_key]["selector"],
                    $style_nodes[$root_style_key]
                );
            }
            ...
            if (!empty($options["scope"])) {
                $root_selector = static::scope_selector(
                    $options["scope"],
                    $root_selector
                );
                $columns_selector = static::scope_selector(
                    $options["scope"],
                    $columns_selector
                );
                $post_template_selector = static::scope_selector(
                    $options["scope"],
    ...
    public function get_custom_css()
    {
        ...
        $stylesheet = isset($this->theme_json["styles"]["css"])
            ? $this->theme_json["styles"]["css"]
            : "";

        // Add the global styles block CSS.
        if (isset($this->theme_json["styles"]["blocks"])) {
            foreach ($this->theme_json["styles"]["blocks"] as $name => $node) {
                $custom_block_css = isset(
                ...
                if ($custom_block_css) {
                    ...
                    $stylesheet .= $this->process_blocks_custom_css(
                        $custom_block_css,
                        $selector
                    );
                }
    ...
    public function get_custom_templates()
    {
        $custom_templates = [];
        if (
            !isset($this->theme_json["customTemplates"]) ||
            !is_array($this->theme_json["customTemplates"])
        ) {
    ...
    public function get_template_parts()
    {
        $template_parts = [];
        if (
            !isset($this->theme_json["templateParts"]) ||
            !is_array($this->theme_json["templateParts"])
        ) {
     ...
     */
    protected function get_layout_styles($block_metadata, $types = [])
    {
        $block_rules = "";
        $block_type = null;
        ...
        if (isset($block_metadata["name"])) {
            ...
            if (
                !block_has_support($block_type, "layout", false) &&
                !block_has_support($block_type, "__experimentalLayout", false)
            ) {
                return $block_rules;
            }
        }

        $selector = isset($block_metadata["selector"])
            ? $block_metadata["selector"]
            : "";
        $has_block_gap_support = isset(
            $this->theme_json["settings"]["spacing"]["blockGap"]
        );
        ...
        if ($has_block_gap_support || $has_fallback_gap_support) {
            ...
            if (!$has_block_gap_support) {
                $block_gap_value =
                    static::ROOT_BLOCK_SELECTOR === $selector ? "0.5em" : null;
                if (!empty($block_type)) {
                    $block_gap_value = isset(
                        $block_type->supports["spacing"]["blockGap"][
                ...
                $block_gap_value = static::get_property_value($node, [
                    "spacing",
                    "blockGap",
                ]);
            }
            ...
            if (is_array($block_gap_value)) {
                if (
                    isset($block_gap_value["top"]) &&
                    isset($block_gap_value["left"])
                ) {
                    $gap_row = static::get_property_value($node, [
                        "spacing",
                        "blockGap",
                        "top",
                    ]);
                    $gap_column = static::get_property_value($node, [
                        "spacing",
                        "blockGap",
                        "left",
                    ]);
            ...
            if (
                null !== $block_gap_value &&
                false !== $block_gap_value &&
                "" !== $block_gap_value
            ) {
                foreach (
                    $layout_definitions
                    as $layout_definition_key => $layout_definition
                ) {
                    ...
                    $class_name = isset($layout_definition["className"])
                        ? $layout_definition["className"]
                        : false;
                    $spacing_rules = isset($layout_definition["spacingStyles"])
                        ? $layout_definition["spacingStyles"]
                        : [];
                    ...
                    if (!empty($class_name) && !empty($spacing_rules)) {
                        foreach ($spacing_rules as $spacing_rule) {
                            ...
                            if (
                                ...
                                foreach (
                                    $spacing_rule["rules"]
                                    as $css_property => $css_value
                                ) {
                                    $current_css_value = is_string($css_value)
                                        ? $css_value
                                        : $block_gap_value;
                                    if (
        ...
        if (static::ROOT_BLOCK_SELECTOR === $selector) {
            ...
            foreach ($layout_definitions as $layout_definition) {
                $class_name = isset($layout_definition["className"])
                    ? $layout_definition["className"]
                    : false;
                $base_style_rules = isset($layout_definition["baseStyles"])
                    ? $layout_definition["baseStyles"]
                    : [];
                ...
                if (!empty($class_name) && is_array($base_style_rules)) {
                    ...
                    foreach ($base_style_rules as $base_style_rule) {
                        ...
                        if (
                            in_array("base-layout-styles", $types, true) &&
                        ...
                        if (
                            ...
                            foreach (
                                ...
                                if (
                                    ...
                                    !isset(
                                        $this->theme_json["settings"]["layout"][
                                            "contentSize"
                                        ]
                                    ) &&
                                    !isset(
    ...
    protected function get_preset_classes($setting_nodes, $origins)
    {
        ...
        foreach ($setting_nodes as $metadata) {
            ...
            $preset_rules .= static::compute_preset_classes(
                $node,
                $selector,
                $origins
            );
     ...
     */
    protected static function compute_preset_classes(
        $settings,
        $selector,
        $origins
    ) {
        if (
            ...
            static::ROOT_CSS_PROPERTIES_SELECTOR === $selector
        ) {
            /*
        ...
        foreach (static::PRESETS_METADATA as $preset_metadata) {
            ...
            $slugs = static::get_settings_slugs(
                $settings,
                $preset_metadata,
                $origins
            );
            foreach ($preset_metadata["classes"] as $class => $property) {
                foreach ($slugs as $slug) {
                    $css_var = static::replace_slug_in_string(
                        $preset_metadata["css_vars"],
                        $slug
                    );
    ...
    protected static function scope_style_node_selectors($scope, $node)
    {
        ...
        foreach ($node["selectors"] as $feature => $selector) {
            if (is_string($selector)) {
                $node["selectors"][$feature] = static::scope_selector(
                    $scope,
                    $selector
                );
            }
     ...
     */
    protected static function get_settings_values_by_slug(
        $settings,
        $preset_metadata,
        $origins
    ) {
        $preset_per_origin = _wp_array_get(
            $settings,
            ...
            foreach ($preset_per_origin[$origin] as $preset) {
                ...
                if (
                    isset(
                        ...
                        $preset[$preset_metadata["value_key"]]
                    )
                ) {
     ...
     */
    protected static function get_settings_slugs(
        $settings,
        $preset_metadata,
        $origins = null
    ) {
    ...
    protected static function compute_preset_vars($settings, $origins)
    {
        ...
        foreach (static::PRESETS_METADATA as $preset_metadata) {
            ...
            foreach ($values_by_slug as $slug => $value) {
                $declarations[] = [
                    "name" => static::replace_slug_in_string(
                        $preset_metadata["css_vars"],
                        $slug
                    ),
                    "value" => $value,
    ...
    {
        $declarations = [];
        $custom_values = isset($settings["custom"]) ? $settings["custom"] : [];
    ...
    protected static function flatten_tree($tree, $prefix = "", $token = "--")
    {
        ...
        foreach ($tree as $property => $value) {
            $new_key =
                $prefix .
                str_replace("/", "-", strtolower(_wp_to_kebab_case($property)));

            if (is_array($value)) {
                $new_prefix = $new_key . $token;
                $flattened_subtree = static::flatten_tree(
                    $value,
                    $new_prefix,
                    $token
                );
    ...
    protected static function compute_style_properties(
        ...
        foreach ($properties as $css_property => $value_path) {
            ...
            $value = static::get_property_value(
                $styles,
                $value_path,
                $theme_json
            );
             ...
             */
            if (
                "--wp--style--root--padding" === $css_property &&
                is_string($value)
            ) {
                continue;
            }
            ...
            if ($is_root_style && $use_root_padding) {
                $root_variable_duplicates[] = substr(
                    $css_property,
                    $root_style_length
                );
            }
            ...
            if (
                empty($value) &&
                static::ROOT_BLOCK_SELECTOR !== $selector &&
                !empty($styles["background"]["backgroundImage"]["id"])
            ) {
                ...
                if ("background-position" === $css_property) {
                    $background_size =
                        $styles["background"]["backgroundSize"] ?? null;
            ...
            if (
                ...
                _wp_array_get(
                    $settings,
                    static::PROTECTED_PROPERTIES[$path_string],
                    null
                ) === null
            ) {
                continue;
            ...
            if ("font-size" === $css_property) {
                 ...
                 */
                $value = wp_get_typography_font_size_value(
                    ["size" => $value],
                    $settings
                );
            }
    ...
    protected static function get_property_value(
        ...
        if (is_array($value) && isset($value["ref"])) {
            ...
            if (
                !empty($ref_value) &&
                (is_string($ref_value) || is_string($ref_value_url))
            ) {
                $value = $ref_value;
            }
     ...
     */
    protected static function get_style_nodes(
        $theme_json,
        $selectors = [],
        $options = []
    ) {
            ...
            foreach (self::ELEMENTS as $element => $selector) {
                ...
                if (isset(static::VALID_ELEMENT_PSEUDO_SELECTORS[$element])) {
                    foreach (
                        ...
                        if (
                            isset(
                                $theme_json["styles"]["elements"][$element][
                                    $pseudo_selector
                                ]
                            )
                        ) {
                            $nodes[] = [
                                "path" => ["styles", "elements", $element],
                                "selector" => static::append_to_selector(
    ...
    private static function update_separator_declarations($declarations)
    {
        ...
        foreach ($declarations as $declaration) {
            if (
                "background-color" === $declaration["name"] &&
                !$background_color &&
     ...
     * @return array The block nodes in theme.json.
     */
    private static function get_block_nodes(
        $theme_json,
        $selectors = [],
        $options = []
    ) {
        ...
        $include_variations =
            $options["include_block_style_variations"] ?? false;
        ...
        foreach ($theme_json["styles"]["blocks"] as $name => $node) {
                ...
                if ($include_variations && isset($node["variations"])) {
                    foreach ($node["variations"] as $variation => $node) {
                        $variation_selectors[] = [
                            ...
                            "selector" =>
                                $selectors[$name]["styleVariations"][
                                    $variation
                                ],
                        ];
                    }
            ...
            if (isset($theme_json["styles"]["blocks"][$name]["elements"])) {
                foreach (
                    $theme_json["styles"]["blocks"][$name]["elements"]
                    as $element => $node
                ) {
                    $node_path = [
                        "styles",
                    ...
                    // Handle any pseudo selectors for the element.
                    if (
                        isset(static::VALID_ELEMENT_PSEUDO_SELECTORS[$element])
                    ) {
                        foreach (
                            static::VALID_ELEMENT_PSEUDO_SELECTORS[$element]
                            as $pseudo_selector
                            ...
                            if (
                                ...
                                $nodes[] = [
                                    "path" => $node_path,
                                    "selector" => static::append_to_selector(
                                        $selectors[$name]["elements"][$element],
                                        $pseudo_selector
                                    ),
                                ];
                            }
    ...
    public function get_styles_for_block($block_metadata)
    {
        ...
        $use_root_padding =
            isset(
                $this->theme_json["settings"]["useRootPaddingAwareAlignments"]
            ) &&
            true ===
                $this->theme_json["settings"]["useRootPaddingAwareAlignments"];
        ...
        $settings = isset($this->theme_json["settings"])
            ...
            : [];
        $feature_declarations = static::get_feature_declarations_for_node(
            $block_metadata,
            $node
        );
        ...
        if (!empty($block_metadata["variations"])) {
            foreach ($block_metadata["variations"] as $style_variation) {
                $style_variation_node = _wp_array_get(
                    $this->theme_json,
                    $style_variation["path"],
                    []
                );
                $clean_style_variation_selector = trim(
                    $style_variation["selector"]
                );

                // Generate any feature/subfeature style declarations for the current style variation.
                $variation_declarations = static::get_feature_declarations_for_node(
                    $block_metadata,
                ...
                foreach (
                    ...
                    as $current_selector => $new_declarations
                ) {
                    /*
                     ...
                     */
                    $clean_current_selector = preg_replace(
                        "/,\s+/",
                        ",",
                        $current_selector
                    );
                    $shortened_selector = str_replace(
                        $block_metadata["selector"],
                        "",
                        $clean_current_selector
                    );

                    // Prepend the variation selector to the current selector.
                    $split_selectors = explode(",", $shortened_selector);
                    $updated_selectors = array_map(static function (
                        $split_selector
                    ) use ($clean_style_variation_selector) {
                ...
                if (isset($style_variation_node["css"])) {
                    $style_variation_custom_css[
                        $style_variation["selector"]
                    ] = $this->process_blocks_custom_css(
                        $style_variation_node["css"],
                        $style_variation["selector"]
                    );
                }
            }
         ...
         */
        $is_processing_element = in_array(
            "elements",
            $block_metadata["path"],
            true
        );

        $current_element = $is_processing_element
            ? $block_metadata["path"][count($block_metadata["path"]) - 1]
            : null;
        ...
        if (isset(static::VALID_ELEMENT_PSEUDO_SELECTORS[$current_element])) {
            $element_pseudo_allowed =
                static::VALID_ELEMENT_PSEUDO_SELECTORS[$current_element];
        }
        ...
        $pseudo_matches = array_values(
            array_filter($element_pseudo_allowed, static function (
                $pseudo_selector
            ) use ($selector) {
                /*
                 ...
                 */
                return preg_match(
                    "/" . preg_quote($pseudo_selector, "/") . "(?!-)/",
                    $selector
                ) === 1;
            })
        );

        $pseudo_selector = isset($pseudo_matches[0])
            ? $pseudo_matches[0]
            : null;
        ...
        if (
            ...
            $declarations = static::compute_style_properties(
        ...
        foreach ($declarations as $index => $declaration) {
            ...
            if (
                $is_root_selector &&
                ("background-image" === $declaration["name"] ||
        ...
        if ($should_set_root_min_height) {
            $block_rules .= static::to_ruleset("html", [
                [
                    "name" => "min-height",
                    "value" =>
                        "calc(100% - var(--wp-admin--admin-bar--height, 0px))",
                ],
            ]);
        }
        ...
        if (".wp-block-separator" === $selector) {
            $declarations = static::update_separator_declarations(
                $declarations
            );
        }
        ...
        if (
            isset($block_metadata["duotone"]) &&
            !empty($declarations_duotone)
        ) {
            $block_rules .= static::to_ruleset(
                $block_metadata["duotone"],
                $declarations_duotone
            );
        }

        // 4. Generate Layout block gap styles.
        if (!$is_root_selector && !empty($block_metadata["name"])) {
        ...
        foreach (
            ...
            as $feature_selector => $individual_feature_declarations
        ) {
            $block_rules .= static::to_ruleset(
                ":root :where($feature_selector)",
    ...
    public function get_root_layout_rules($selector, $block_metadata)
    {
         ...
         */
        if (
            isset($settings["layout"]["contentSize"]) ||
            isset($settings["layout"]["wideSize"])
        ) {
            $content_size = isset($settings["layout"]["contentSize"])
            ...
            $wide_size = isset($settings["layout"]["wideSize"])
                ? $settings["layout"]["wideSize"]
                : $settings["layout"]["contentSize"];
            $wide_size = static::is_safe_css_declaration(
                "max-width",
                $wide_size
            )
                ? $wide_size
                : "initial";
            $css .=
                static::ROOT_CSS_PROPERTIES_SELECTOR .
        ...
        if (isset($this->theme_json["settings"]["spacing"]["blockGap"])) {
            $block_gap_value = static::get_property_value($this->theme_json, [
                "styles",
                "spacing",
                "blockGap",
            ]);
            ...
            $css .=
                static::ROOT_CSS_PROPERTIES_SELECTOR .
                " { --wp--style--block-gap: $block_gap_value; }";
        }
     ...
     */
    protected static function get_metadata_boolean(
        $data,
        $path,
        $default_value = false
    ) {
    ...
    public function merge($incoming)
    {
        ...
        foreach (static::VALID_ORIGINS as $origin) {
            ...
            $flattened_spacing_scale = array_replace(
                $flattened_spacing_scale,
                $base_spacing_scale
            );
            ...
            $sizes_path = ["settings", "spacing", "spacingSizes", $origin];
            $spacing_sizes = _wp_array_get($incoming_data, $sizes_path, []);
            $spacing_scale_sizes = static::compute_spacing_sizes(
                $flattened_spacing_scale
            );
            $merged_spacing_sizes = static::merge_spacing_sizes(
        ...
        $slugs_global = static::get_default_slugs($this->theme_json, [
            "settings",
        ]);
        foreach ($nodes as $node) {
            ...
            foreach (static::PRESETS_METADATA as $preset_metadata) {
                ...
                if (is_array($prevent_override)) {
                    $prevent_override = _wp_array_get(
                        $this->theme_json["settings"],
                        $preset_metadata["prevent_override"]
                    );
                }
                ...
                foreach (static::VALID_ORIGINS as $origin) {
                    ...
                    if (
                        "theme" === $origin &&
                        $preset_metadata["use_default_names"]
                    ) {
                        foreach ($content as $key => $item) {
                            if (!isset($item["name"])) {
                                $name = static::get_name_from_defaults(
                                    $item["slug"],
                                    $base_path
                                );
                                if (null !== $name) {
                    ...
                    if ("theme" === $origin && $prevent_override) {
                        $slugs_node = static::get_default_slugs(
                            $this->theme_json,
                            $node["path"]
                        );
                        $preset_global = _wp_array_get(
                            $slugs_global,
                            $preset_metadata["path"],
                            []
                        );
                        $preset_node = _wp_array_get(
                            $slugs_node,
                            $preset_metadata["path"],
                            []
                        );
                        $preset_slugs = array_merge_recursive(
                            $preset_global,
                            $preset_node
                        );

                        $content = static::filter_slugs(
                            $content,
                            $preset_slugs
                        );
                    }
        ...
        foreach ($style_nodes as $style_node) {
             ...
             */
            $background_image_path = array_merge(
                $path,
                static::PROPERTIES_METADATA["background-image"]
            );
            $content = _wp_array_get(
                $incoming_data,
        ...
        foreach ($setting_nodes as $metadata) {
            ...
            foreach ($origins as $origin) {
                ...
                foreach ($duotone_presets[$origin] as $duotone_preset) {
                    $filters .= WP_Duotone::get_filter_svg_from_preset(
                        $duotone_preset
                    );
                }
            }
     ...
     * @return bool
     */
    protected static function should_override_preset(
        $theme_json,
        $path,
        $override
    ) {
    ...
    protected static function filter_slugs($node, $slugs)
    {
        ...
        foreach ($node as $value) {
            if (
                isset($value["slug"]) &&
                !in_array($value["slug"], $slugs, true)
            ) {
                $new_node[] = $value;
            }
        }
    ...
    public static function remove_insecure_properties(
        ...
        $blocks_metadata = static::get_blocks_metadata();
        $style_options = ["include_block_style_variations" => true]; // Allow variations data.
        $style_nodes = static::get_style_nodes(
            $theme_json,
            $blocks_metadata,
            $style_options
        ...
        foreach ($style_nodes as $metadata) {
            ...
            if (isset($metadata["variations"])) {
                foreach ($metadata["variations"] as $variation) {
                    ...
                    $variation_output = static::remove_insecure_styles(
                        $variation_input
                    );
    ...
    protected static function remove_insecure_element_styles($elements)
    {
        ...
        foreach ($valid_element_names as $element_name) {
            ...
            if ($element_input) {
                $element_output = static::remove_insecure_styles(
                    $element_input
                );

                if (
                    isset(static::VALID_ELEMENT_PSEUDO_SELECTORS[$element_name])
                ) {
                    foreach (
                        static::VALID_ELEMENT_PSEUDO_SELECTORS[$element_name]
                        as $pseudo_selector
                    ) {
     ...
     * @return bool
     */
    protected static function is_safe_css_declaration(
        $property_name,
        $property_value
    ) {
    ...
    public static function get_from_editor_settings($settings)
    {
        ...
        if (isset($settings["disableCustomColors"])) {
            $theme_settings["settings"]["color"]["custom"] = !$settings[
                "disableCustomColors"
            ];
        }

        if (isset($settings["disableCustomGradients"])) {
            $theme_settings["settings"]["color"]["customGradient"] = !$settings[
        ...
        if (isset($settings["enableCustomLineHeight"])) {
            $theme_settings["settings"]["typography"]["lineHeight"] =
                $settings["enableCustomLineHeight"];
        }

        if (isset($settings["enableCustomUnits"])) {
            $theme_settings["settings"]["spacing"]["units"] =
                true === $settings["enableCustomUnits"]
                    ? ["px", "em", "rem", "vh", "vw", "%"]
                    : $settings["enableCustomUnits"];
        }

        if (isset($settings["colors"])) {
            $theme_settings["settings"]["color"]["palette"] =
                $settings["colors"];
        }

        if (isset($settings["gradients"])) {
            $theme_settings["settings"]["color"]["gradients"] =
                $settings["gradients"];
        }
        ...
        if (isset($settings["enableCustomSpacing"])) {
            $theme_settings["settings"]["spacing"]["padding"] =
                $settings["enableCustomSpacing"];
        }

        if (isset($settings["spacingSizes"])) {
            $theme_settings["settings"]["spacing"]["spacingSizes"] =
    ...
    {
        if (
            isset($this->theme_json["patterns"]) &&
            is_array($this->theme_json["patterns"])
        ) {
            ...
            if ($all_opt_ins_are_set) {
                ...
                foreach (static::APPEARANCE_TOOLS_OPT_INS as $opt_in_path) {
                    ...
                    if (
                        ...
                        unset(
                            $output["settings"]["blocks"][$block_name][
                                $opt_in_path[0]
                            ][$opt_in_path[1]]
                        );
                        if (
    ...
    public function set_spacing_sizes()
    {
        ...
        if ($spacing_scale["steps"] <= 7) {
            for (
                ...
                $spacing_sizes_count < count($spacing_sizes);
                $spacing_sizes_count++
            ) {
                $spacing_sizes[$spacing_sizes_count]["name"] =
                    (string) ($spacing_sizes_count + 1);
            }
        }
    ...
    private static function compute_spacing_sizes($spacing_scale)
    {
        ...
        $unit =
            "%" === $spacing_scale["unit"]
                ? "%"
                : sanitize_title($spacing_scale["unit"]);
        ...
        for (
            ...
            $below_sizes[] = [
                ...
                "name" =>
                    $below_midpoint_count === $steps_mid_point - 1
                        ? __("Small")
                        : sprintf(__("%sX-Small"), (string) $x_small_count),
                "slug" => (string) $slug,
        ...
        for (
            ...
            $above_sizes[] = [
                ...
                "name" =>
                    0 === $above_midpoint_count
                        ? __("Large")
                        : sprintf(__("%sX-Large"), (string) $x_large_count),
                "slug" => (string) $slug,
                "size" => round($current_step, 2) . $unit,
            ];
    ...
    protected static function get_block_element_selectors($root_selector)
    {
        ...
        foreach (static::ELEMENTS as $el_name => $el_selector) {
            ...
            foreach ($block_selectors as $selector) {
                ...
                $element_selector[] = static::prepend_to_selector(
                    $el_selector,
                    $selector . " "
                );
            }
    ...
    protected function get_feature_declarations_for_node($metadata, &$node)
    {
        ...
        foreach ($metadata["selectors"] as $feature => $feature_selectors) {
            ...
            if (is_array($feature_selectors)) {
                foreach (
                    $feature_selectors
                    as $subfeature => $subfeature_selector
                ) {
                    if (
                        "root" === $subfeature ||
                        empty($node[$feature][$subfeature])
                    ) {
                        continue;
                    }
                    ...
                    $new_declarations = static::compute_style_properties(
                        $subfeature_node,
                        $settings,
                        null,
                        $this->theme_json
            ...
            if (
                ...
                (isset($feature_selectors["root"]) &&
                    $feature_selectors["root"])
            ) {
                $feature_selector = is_string($feature_selectors)
                    ? $feature_selectors
                    : $feature_selectors["root"];
    ...
    private static function convert_variables_to_value($styles, $values)
    {
        foreach ($styles as $key => $style) {
            ...
            if (is_array($style)) {
                $styles[$key] = self::convert_variables_to_value(
                    $style,
                    $values
                );
                continue;
    ...
    public static function resolve_variables($theme_json)
    {
        ...
        $preset_vars = static::compute_preset_vars(
            $settings,
            static::VALID_ORIGINS
        );
        ...
        $theme_json->theme_json["styles"] = self::convert_variables_to_value(
            $styles,
            $vars
        );
        return $theme_json;
     ...
     */
    protected static function get_block_style_variation_selector(
        $variation_name,
        $block_selector
    ) {
    ...
    protected static function get_valid_block_style_variations(
        ...
        $valid_variations = [];
        $blocks_metadata = empty($blocks_metadata)
            ? static::get_blocks_metadata()
            : $blocks_metadata;
