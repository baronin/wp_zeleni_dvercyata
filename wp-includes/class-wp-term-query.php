...
class WP_Term_Query
{
    ...
    public function get_terms()
    {
        ...
        if (
            "term_order" === $_orderby &&
            empty($this->query_vars["object_ids"])
        ) {
            $_orderby = "term_id";
        }
        ...
        if (!empty($inclusions)) {
            $this->sql_clauses["where"]["inclusions"] =
                "t.term_id IN ( " . $inclusions . " )";
        }
        ...
        if (!empty($exclusions)) {
            $exclusions =
                "t.term_id NOT IN (" .
                implode(",", array_map("intval", $exclusions)) .
                ")";
        } else {
            $exclusions = "";
        }
        ...
        if (!empty($exclusions)) {
            ...
            $this->sql_clauses["where"]["exclusions"] = preg_replace(
                "/^\s*AND\s*/",
                "",
                $exclusions
            );
        ...
        if (!empty($args["name"])) {
            ...
            $this->sql_clauses["where"]["name"] =
                "t.name IN ('" .
                implode("', '", array_map("esc_sql", $names)) .
                "')";
        }
        ...
        if ("" === $args["term_taxonomy_id"]) {
            ...
            $args["term_taxonomy_id"] = array_map(
                "intval",
         ...
         */
        $this->terms = apply_filters_ref_array("terms_pre_query", [
            $this->terms,
            &$this,
        ]);
        ...
        if ($child_of) {
            foreach ($taxonomies as $_tax) {
                ...
                if (!empty($children)) {
                    $term_objects = _get_term_children(
                        $child_of,
                        $term_objects,
                        $_tax
                    );
    ...
    protected function parse_orderby($orderby_raw)
    {
        ...
        if (
            in_array($_orderby, ["term_id", "name", "slug", "term_group"], true)
        ) {
            $orderby = "t.$_orderby";
        } elseif (
            in_array(
            ...
            $include = implode(
                ",",
                wp_parse_id_list($this->query_vars["include"])
            );
            $orderby = "FIELD( t.term_id, $include )";
        } elseif (
            "slug__in" === $_orderby &&
            !empty($this->query_vars["slug"]) &&
            is_array($this->query_vars["slug"])
        ) {
            $slugs = implode(
                "', '",
                array_map("sanitize_title_for_query", $this->query_vars["slug"])
            );
    ...
    protected function populate_terms($terms)
    {
        ...
        foreach ($terms as $key => $term_data) {
            if (
                is_object($term_data) &&
                property_exists($term_data, "term_id")
            ) {
    ...
    protected function generate_cache_key(array $args, $sql)
    {
        ...
        $cache_args = wp_array_slice_assoc(
            $args,
        ...
        unset(
            ...
            $cache_args["update_term_meta_cache"]
        );
