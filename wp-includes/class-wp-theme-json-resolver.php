...
class WP_Theme_JSON_Resolver
{
    ...
    protected static function read_json_file($file_path)
    {
        if ($file_path) {
            ...
            $decoded_file = wp_json_file_decode($file_path, [
                "associative" => true,
            ]);
    ...
    protected static function translate($theme_json, $domain = "default")
    {
        ...
        return translate_settings_using_i18n_schema(
            static::$i18n_schema,
            $theme_json,
            $domain
        );
    ...
    public static function get_core_data()
    {
        if (
            null !== static::$core &&
            static::has_same_registered_blocks("core")
        ) {
            return static::$core;
        }
    ...
    public static function get_theme_data($deprecated = [], $options = [])
    {
        ...
        if (
            ...
            if (is_readable($theme_json_file)) {
                ...
                $theme_json_data = static::translate(
                    $theme_json_data,
                    $wp_theme->get("TextDomain")
                );
            } else {
             ...
             *
             */
            $theme_json_data = static::inject_variations_from_block_style_variation_files(
                $theme_json_data,
                $variations
            );
            $theme_json_data = static::inject_variations_from_block_styles_registry(
                $theme_json_data
            );
            ...
            if ($wp_theme->parent()) {
                ...
                $parent_theme_json_file = $wp_theme
                    ->parent()
                    ->get_file_path("theme.json");
                if (
                    $theme_json_file !== $parent_theme_json_file &&
                    is_readable($parent_theme_json_file)
                ) {
                    $parent_theme_json_data = static::read_json_file(
                        $parent_theme_json_file
                    );
                    $parent_theme_json_data = static::translate(
        ...
        if (!wp_theme_has_theme_json()) {
            ...
            $theme_support_data["settings"]["typography"]["defaultFontSizes"] =
                !isset(
                    $theme_support_data["settings"]["typography"]["fontSizes"]
                ) || current_theme_supports("default-font-sizes");
            $theme_support_data["settings"]["spacing"]["defaultSpacingSizes"] =
                !isset(
                    $theme_support_data["settings"]["spacing"]["spacingSizes"]
        ...
        foreach ($blocks as $block_name => $block_type) {
            if (isset($block_type->supports["__experimentalStyle"])) {
                $config["styles"]["blocks"][
                    $block_name
                ] = static::remove_json_comments(
                    $block_type->supports["__experimentalStyle"]
                );
            }
            ...
            if (
                ...
                !isset(
                    $config["styles"]["blocks"][$block_name]["spacing"][
                        "blockGap"
                    ]
                )
            ) {
    ...
    public static function get_user_data_from_wp_global_styles(
         ...
         */
        if (
            $theme->get_stylesheet() === get_stylesheet() &&
        ...
        if (array_key_exists("post_content", $user_cpt)) {
            ...
            if (JSON_ERROR_NONE !== $json_decoding_error) {
                ...
                $theme_json = apply_filters(
                    ...
                    new WP_Theme_JSON_Data($config, "custom")
                );
    ...
    public static function get_user_global_styles_post_id()
    {
        ...
        $user_cpt = static::get_user_data_from_wp_global_styles(
            wp_get_theme(),
            true
        );
     ...
     */
    protected static function get_file_path_from_theme(
        $file_name,
        $template = false
    ) {
        $path = $template
    ...
    private static function recursively_iterate_json($dir)
    {
        ...
        $nested_json_files = iterator_to_array(
            new RegexIterator(
                $nested_files,
                '/^.+\.json$/i',
    ...
    public static function get_style_variations($scope = "theme")
    {
        ...
        if (is_dir($base_directory)) {
            $variation_files = static::recursively_iterate_json(
                $base_directory
            );
        }
        if (
            ...
            $variation_files_parent = static::recursively_iterate_json(
                $template_directory
            );
        ...
        foreach ($variation_files as $path => $file) {
            ...
            if (
                ...
                static::style_variation_has_scope($decoded_file, $scope)
            ) {
                $translated = static::translate(
                    $decoded_file,
    ...
    public static function get_resolved_theme_uris($theme_json)
    {
        ...
        $background_image_url =
            $theme_json_data["styles"]["background"]["backgroundImage"][
                "url"
            ] ?? null;
        if (
        ...
        if (!empty($theme_json_data["styles"]["blocks"])) {
            foreach (
                $theme_json_data["styles"]["blocks"]
                as $block_name => $block_styles
            ) {
                if (
                    !isset(
                        $block_styles["background"]["backgroundImage"]["url"]
                    )
                ) {
                    continue;
                }
     ...
     */
    private static function inject_variations_from_block_style_variation_files(
        $data,
        $variations
    ) {
        if (empty($variations)) {
        ...
        foreach ($variations as $variation) {
            ...
            $variation_name =
                $variation["slug"] ?? _wp_to_kebab_case($variation["title"]);

            foreach ($variation["blockTypes"] as $block_type) {
                // First, override partial styles with any top-level styles.
                $top_level_data =
                    $data["styles"]["variations"][$variation_name] ?? [];
                if (!empty($top_level_data)) {
                    $variation["styles"] = array_replace_recursive(
                        $variation["styles"],
                        $top_level_data
                    );
                }
                ...
                $block_level_data =
                    $data["styles"]["blocks"][$block_type]["variations"][
                        $variation_name
                    ] ?? [];
        ...
        foreach ($styles as $block_type => $variations) {
            foreach ($variations as $variation_name => $variation) {
                ...
                $top_level_data =
                    $data["styles"]["variations"][$variation_name] ?? [];
                if (!empty($top_level_data)) {
                    $variation["style_data"] = array_replace_recursive(
