...
function ms_site_check()
{
    ...
    if ("1" === $blog->archived || "1" === $blog->spam) {
        if (file_exists(WP_CONTENT_DIR . "/blog-suspended.php")) {
            ...
            wp_die(__("This site has been archived or suspended."), "", [
                "response" => 410,
            ]);
        }
    }
...
function ms_load_current_site_and_network($domain, $path, $subdomain = false)
{
    ...
    if (defined("DOMAIN_CURRENT_SITE") && defined("PATH_CURRENT_SITE")) {
        ...
        $current_site->id = defined("SITE_ID_CURRENT_SITE")
        ...
        if (
            0 === strcasecmp($current_site->domain, $domain) &&
            0 === strcasecmp($current_site->path, $path)
        ) {
             ...
             */
            $current_blog = get_site_by_path(
                $domain,
                $path,
                1 + count(explode("/", trim($current_site->path, "/")))
            );
        } else {
        ...
        if (empty($current_site)) {
            ...
            $current_blog = get_site_by_path(
                $domain,
                $path,
                substr_count($current_site->path, "/")
            );
        }
    } else {
        ...
        if ($current_blog) {
            $current_site = WP_Network::get_instance(
                $current_blog->site_id ? $current_blog->site_id : 1
            );
        } else {
