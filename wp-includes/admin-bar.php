...
function wp_admin_bar_wp_menu($wp_admin_bar)
{
    if (current_user_can("read")) {
        ...
        $contribute_url = get_dashboard_url(
            get_current_user_id(),
            "contribute.php"
        );
    } else {
        ...
        if (
            ...
            $wp_admin_bar->add_node([
                ...
                "href" => network_admin_url(
                    "site-info.php?id=" . get_current_blog_id()
                ),
            ]);
        }
...
function wp_admin_bar_my_sites_menu($wp_admin_bar)
{
    ...
    if (
        count($wp_admin_bar->user->blogs) < 1 &&
    ...
    if ($wp_admin_bar->user->active_blog) {
        $my_sites_url = get_admin_url(
            $wp_admin_bar->user->active_blog->blog_id,
            "my-sites.php"
        );
    } else {
    ...
    $wp_admin_bar->add_group([
        ...
        "id" => "my-sites-list",
        "meta" => [
            "class" => current_user_can("manage_network")
                ? "ab-sub-secondary"
                : "",
        ],
    ]);
    ...
    foreach ((array) $wp_admin_bar->user->blogs as $blog) {
        ...
        if (true === $show_site_icons && has_site_icon()) {
            $blavatar = sprintf(
                ...
                wp_lazy_loading_enabled("img", "site_icon_in_toolbar")
                    ? ' loading="lazy"'
                    : ""
            );
        } else {
...
function wp_admin_bar_edit_menu($wp_admin_bar)
{
    ...
    if (is_admin()) {
        ...
        if (!empty($current_object->post_type)) {
            ...
            $current_object instanceof WP_User &&
...
function wp_admin_bar_new_content_menu($wp_admin_bar)
{
    ...
    if (
        isset($cpts["post"]) &&
        current_user_can($cpts["post"]->cap->create_posts)
    ) {
        $actions["post-new.php"] = [
            $cpts["post"]->labels->name_admin_bar,
            "new-post",
        ];
    }
    ...
    if (isset($cpts["attachment"]) && current_user_can("upload_files")) {
        $actions["media-new.php"] = [
            $cpts["attachment"]->labels->name_admin_bar,
            "new-media",
        ];
    }
    ...
    if (current_user_can("manage_links")) {
        $actions["link-add.php"] = [
            _x("Link", "add new from admin bar"),
            "new-link",
        ];
    }
    ...
    if (
        isset($cpts["page"]) &&
        current_user_can($cpts["page"]->cap->create_posts)
    ) {
        $actions["post-new.php?post_type=page"] = [
            $cpts["page"]->labels->name_admin_bar,
            "new-page",
        ];
    }
    ...
    if (
        ...
        $actions["user-new.php"] = [
            _x("User", "add new from admin bar"),
            "new-user",
        ];
    }
...
function wp_admin_bar_updates_menu($wp_admin_bar)
{
    ...
    $icon = '<span class="ab-icon" aria-hidden="true"></span>';
    $title =
        '<span class="ab-label" aria-hidden="true">' .
        number_format_i18n($update_data["counts"]["total"]) .
        "</span>";
    $title .=
