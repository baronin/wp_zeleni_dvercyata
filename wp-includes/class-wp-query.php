...
class WP_Query
{
    ...
    public function parse_query($query = "")
    {
        ...
        $qv["monthnum"] = is_scalar($qv["monthnum"])
            ? absint($qv["monthnum"])
            : 0;
        ...
        $qv["author"] = is_scalar($qv["author"])
            ? preg_replace("|[^0-9,-]|", "", $qv["author"])
            : ""; // Comma-separated list of positive or negative integers.
        $qv["pagename"] = is_scalar($qv["pagename"])
            ? trim($qv["pagename"])
            : "";
        ...
        $qv["attachment_id"] = is_scalar($qv["attachment_id"])
            ? absint($qv["attachment_id"])
            : 0;
        ...
        $this->is_singular =
            $this->is_single || $this->is_page || $this->is_attachment;

        if (
            $this->is_feed &&
            (!empty($qv["withcomments"]) ||
                (empty($qv["withoutcomments"]) && $this->is_singular))
        ) {
        ...
        if ("" !== $qv["pagename"]) {
            ...
            if (
                "page" === get_option("show_on_front") &&
                isset($this->queried_object_id) &&
                get_option("page_for_posts") == $this->queried_object_id
            ) {
                $this->is_page = false;
                $this->is_home = true;
        ...
        if ($qv["page_id"]) {
            if (
                "page" === get_option("show_on_front") &&
                get_option("page_for_posts") == $qv["page_id"]
            ) {
                $this->is_page = false;
                $this->is_home = true;
                $this->is_posts_page = true;
        ...
        if (!empty($qv["post_type"])) {
            if (is_array($qv["post_type"])) {
                $qv["post_type"] = array_map(
                    "sanitize_key",
                    array_unique($qv["post_type"])
                );
                sort($qv["post_type"]);
            } else {
        ...
        if (!empty($qv["post_status"])) {
            if (is_array($qv["post_status"])) {
                $qv["post_status"] = array_map(
                    "sanitize_key",
                    array_unique($qv["post_status"])
                );
                sort($qv["post_status"]);
            } else {
                $qv["post_status"] = preg_replace(
                    "|[^a-z0-9_,-]|",
                    "",
                    $qv["post_status"]
                );
        ...
        $this->is_embed =
            $this->is_embed && ($this->is_singular || $this->is_404);
    ...
    public function parse_tax_query(&$q)
    {
        ...
        if (!empty($q["category__in"])) {
            $q["category__in"] = array_map(
                "absint",
                array_unique((array) $q["category__in"])
            );
            sort($q["category__in"]);
            $tax_query[] = [
                "taxonomy" => "category",
        ...
        if (!empty($q["category__not_in"])) {
            $q["category__not_in"] = array_map(
                "absint",
                array_unique((array) $q["category__not_in"])
            );
        ...
        if (!empty($q["category__and"])) {
            $q["category__and"] = array_map(
                "absint",
                array_unique((array) $q["category__and"])
            );
        ...
        if (!empty($q["tag__not_in"])) {
            $q["tag__not_in"] = array_map(
                "absint",
                array_unique((array) $q["tag__not_in"])
            );
            sort($q["tag__not_in"]);
            $tax_query[] = [
                "taxonomy" => "post_tag",
        ...
        if (!empty($q["tag__and"])) {
            $q["tag__and"] = array_map(
                "absint",
                array_unique((array) $q["tag__and"])
            );
            sort($q["tag__and"]);
            $tax_query[] = [
                "taxonomy" => "post_tag",
        ...
        if (!empty($q["tag_slug__in"])) {
            $q["tag_slug__in"] = array_map(
                "sanitize_title_for_query",
                array_unique((array) $q["tag_slug__in"])
            );
            sort($q["tag_slug__in"]);
            $tax_query[] = [
                "taxonomy" => "post_tag",
        ...
        if (!empty($q["tag_slug__and"])) {
            $q["tag_slug__and"] = array_map(
                "sanitize_title_for_query",
                array_unique((array) $q["tag_slug__and"])
            );
    ...
    protected function parse_search(&$q)
    {
        ...
        if (!empty($q["sentence"])) {
            ...
            if (
                ...
                if (
                    empty($q["search_terms"]) ||
                    count($q["search_terms"]) > 9
                ) {
                    $q["search_terms"] = [$q["s"]];
                }
            } else {
        ...
        foreach ($q["search_terms"] as $term) {
            ...
            $exclude =
                $exclusion_prefix && str_starts_with($term, $exclusion_prefix);
            if ($exclude) {
                $like_op = "NOT LIKE";
                $andor_op = "AND";
                $term = substr($term, 1);
            ...
            foreach ($search_columns as $search_column) {
                $search_columns_parts[$search_column] = $wpdb->prepare(
                    "({$wpdb->posts}.$search_column $like_op %s)",
                    $like
                );
            }

            if (!empty($this->allow_query_attachment_by_filename)) {
                $search_columns_parts["attachment"] = $wpdb->prepare(
                    "(sq1.meta_value $like_op %s)",
                    $like
                );
            }
            ...
            $search .=
                "$searchand(" .
                implode(" $andor_op ", $search_columns_parts) .
                ")";
    ...
    protected function parse_search_terms($terms)
    {
        ...
        foreach ($terms as $term) {
            ...
            if (
                in_array(call_user_func($strtolower, $term), $stopwords, true)
            ) {
                continue;
            }
    ...
    protected function parse_search_order(&$q)
    {
        ...
        if ($q["search_terms_count"] > 1) {
            ...
            if ($num_terms < 7) {
                ...
                $search_orderby .=
                    "WHEN " .
                    implode(" AND ", $q["search_orderby_title"]) .
                    " THEN 2 ";
                // Any word in title, not needed when $num_terms == 1.
                if ($num_terms > 1) {
                    $search_orderby .=
                        "WHEN " .
                        implode(" OR ", $q["search_orderby_title"]) .
                        " THEN 3 ";
                }
            }
            ...
            if ($like) {
                $search_orderby .= $wpdb->prepare(
                    "WHEN {$wpdb->posts}.post_excerpt LIKE %s THEN 4 ",
                    $like
                );
                $search_orderby .= $wpdb->prepare(
        ...
        switch ($orderby) {
            ...
            case "post__in":
                if (!empty($this->query_vars["post__in"])) {
                    $orderby_clause =
                        ...
                        implode(
                            ",",
                            array_map("absint", $this->query_vars["post__in"])
                        ) .
                        ")";
                }
                break;
            case "post_parent__in":
                if (!empty($this->query_vars["post_parent__in"])) {
                    $orderby_clause =
                        "FIELD( {$wpdb->posts}.post_parent," .
                        implode(
                            ", ",
                            array_map(
                                "absint",
                                $this->query_vars["post_parent__in"]
                            )
                        ) .
                        " )";
                }
            ...
            case "post_name__in":
                if (!empty($this->query_vars["post_name__in"])) {
                    $post_name__in = array_map(
                        "sanitize_title_for_query",
                        $this->query_vars["post_name__in"]
                    );
                    $post_name__in_string =
                        "'" . implode("','", $post_name__in) . "'";
                    $orderby_clause =
            ...
            default:
                if (array_key_exists($orderby, $meta_clauses)) {
                    ...
                    $orderby_clause =
                        "{$wpdb->posts}.post_" . sanitize_key($orderby);
                }
    ...
    public function get_posts()
    {
         ...
         */
        $this->allow_query_attachment_by_filename = apply_filters(
        ...
        if (
            isset($q["posts_per_archive_page"]) &&
            0 != $q["posts_per_archive_page"] &&
        ...
        if (
            $this->is_home &&
            (empty($this->query) || "true" === $q["preview"]) &&
            "page" === get_option("show_on_front") &&
            get_option("page_on_front")
        ) {
            $this->is_page = true;
        ...
        if ($q["m"]) {
            $where .=
                " AND YEAR({$wpdb->posts}.post_date)=" . substr($q["m"], 0, 4);
            if (strlen($q["m"]) > 5) {
                $where .=
                    " AND MONTH({$wpdb->posts}.post_date)=" .
                    substr($q["m"], 4, 2);
            }
            if (strlen($q["m"]) > 7) {
                $where .=
                    " AND DAYOFMONTH({$wpdb->posts}.post_date)=" .
                    substr($q["m"], 6, 2);
            }
            if (strlen($q["m"]) > 9) {
                $where .=
                    " AND HOUR({$wpdb->posts}.post_date)=" .
                    substr($q["m"], 8, 2);
            }
            if (strlen($q["m"]) > 11) {
                $where .=
                    " AND MINUTE({$wpdb->posts}.post_date)=" .
                    substr($q["m"], 10, 2);
            }
            if (strlen($q["m"]) > 13) {
                $where .=
                    " AND SECOND({$wpdb->posts}.post_date)=" .
                    substr($q["m"], 12, 2);
            }
        }
        ...
        if ("" !== $q["name"]) {
            ...
            if (
                ...
                $reqpage != $page_for_posts
            ) {
                $q["pagename"] = sanitize_title_for_query(
                    wp_basename($q["pagename"])
                );
                $q["name"] = $q["pagename"];
                $where .= " AND ({$wpdb->posts}.ID = '$reqpage')";
                $reqpage_obj = get_post($reqpage);
                if (
                    is_object($reqpage_obj) &&
            ...
            $q["attachment"] = sanitize_title_for_query(
                wp_basename($q["attachment"])
            );
            $q["name"] = $q["attachment"];
            $where .=
                " AND {$wpdb->posts}.post_name = '" . $q["attachment"] . "'";
        } elseif (
            is_array($q["post_name__in"]) &&
            !empty($q["post_name__in"])
        ) {
            $q["post_name__in"] = array_map(
                "sanitize_title_for_query",
                $q["post_name__in"]
            );
        ...
        if ($q["p"]) {
            ...
            $post__in_for_where = array_unique(
                array_map("absint", $post__in_for_where)
            );
            ...
            sort($q["post__not_in"]);
            $post__not_in = implode(
                ",",
                array_map("absint", $q["post__not_in"])
            );
        ...
        if (is_numeric($q["post_parent"])) {
            ...
            $post_parent__in_for_where = array_unique(
                array_map("absint", $post_parent__in_for_where)
            );
            sort($post_parent__in_for_where);
            $post_parent__in = implode(
                ",",
                array_map("absint", $post_parent__in_for_where)
            );
            ...
            $post_parent__not_in = implode(
                ",",
                array_map("absint", $q["post_parent__not_in"])
            );
        ...
        if ($q["page_id"]) {
            if (
                "page" !== get_option("show_on_front") ||
                get_option("page_for_posts") != $q["page_id"]
            ) {
                $q["p"] = $q["page_id"];
        ...
        if (!$q["suppress_filters"]) {
             ...
             */
            $search = apply_filters_ref_array("posts_search", [
                $search,
                &$this,
            ]);
        }
        ...
        if (!empty($this->tax_query->queried_terms)) {
            ...
            if (!isset($q["taxonomy"])) {
                foreach (
                    $this->tax_query->queried_terms
                    as $queried_taxonomy => $queried_items
                ) {
        ...
        if (
            !empty($this->tax_query->queries) ||
            !empty($this->meta_query->queries) ||
            !empty($this->allow_query_attachment_by_filename)
        ) {
        ...
        if (!empty($q["author"]) && "0" != $q["author"]) {
            ...
            $authors = array_unique(
                array_map("intval", preg_split("/[,\s]+/", $q["author"]))
            );
            sort($authors);
        ...
        if (!empty($q["author__not_in"])) {
            ...
            if (is_array($q["author__in"])) {
                $q["author__in"] = array_unique(
                    array_map("absint", $q["author__in"])
                );
                sort($q["author__in"]);
            }
            $author__in = implode(
                ",",
                array_map("absint", array_unique((array) $q["author__in"]))
            );
        ...
        if (isset($q["comment_count"])) {
            ...
            if (isset($q["comment_count"]["value"])) {
                ...
                $where .= $wpdb->prepare(
                    " AND {$wpdb->posts}.comment_count {$q["comment_count"]["compare"]} %d",
                    $q["comment_count"]["value"]
                );
            }
        }
        ...
        if (isset($q["post_mime_type"]) && "" !== $q["post_mime_type"]) {
            $whichmimetype = wp_post_mime_type_where(
                $q["post_mime_type"],
                $wpdb->posts
            );
        }
        ...
        if (!empty($this->meta_query->queries)) {
            $clauses = $this->meta_query->get_sql(
                "post",
                $wpdb->posts,
                "ID",
                $this
        ...
        if (empty($q["orderby"])) {
            ...
            if (
                isset($q["orderby"]) &&
                (is_array($q["orderby"]) || false === $q["orderby"])
            ) {
                $orderby = "";
            } else {
        ...
        if (isset($q["post_password"])) {
            $where .= $wpdb->prepare(
                " AND {$wpdb->posts}.post_password = %s",
                $q["post_password"]
            );
            if (empty($q["perm"])) {
                $q["perm"] = "readable";
        ...
        if (!empty($q["comment_status"])) {
            $where .= $wpdb->prepare(
                " AND {$wpdb->posts}.comment_status = %s ",
                $q["comment_status"]
            );
        }
        ...
        if ("any" === $post_type) {
            ...
            $post_type_where =
                " AND {$wpdb->posts}.post_type IN ('" .
                implode("', '", esc_sql($post_type)) .
                "')";
        } elseif (!empty($post_type)) {
            $post_type_where = $wpdb->prepare(
        ...
        if ($skip_post_status) {
            ...
            if (!empty($r_status)) {
                if (
                    !empty($q["perm"]) &&
                    "editable" === $q["perm"] &&
                    !current_user_can($edit_others_cap)
                ) {
                    $statuswheres[] =
            ...
            if (!empty($queried_post_types)) {
                ...
                foreach ($queried_post_types as $queried_post_type) {
                    $queried_post_type_object = get_post_type_object(
                        $queried_post_type
                    );
                    ...
                    if (
                        is_user_logged_in() &&
                        $queried_post_type_object instanceof WP_Post_Type
                    ) {
                        $read_private_cap =
                            $queried_post_type_object->cap->read_private_posts;
                ...
                if (!empty($status_type_clauses)) {
                    $where .=
                        " AND (" . implode(" OR ", $status_type_clauses) . ")";
                }
            } else {
                $where .= " AND 1=0 ";
            }
        ...
        if ($this->is_comment_feed && !$this->is_singular) {
            if ($this->is_archive || $this->is_search) {
                ...
                $cwhere =
                    "WHERE ( post_status = 'publish' OR ( post_status = 'inherit' AND post_type = 'attachment' ) ) AND comment_approved = '1'";
                $cgroupby = "";
            }
            ...
            if (!$q["suppress_filters"]) {
                 ...
                 * @param WP_Query $query The WP_Query instance (passed by reference).
                 */
                $cjoin = apply_filters_ref_array("comment_feed_join", [
                    $cjoin,
                    &$this,
                ]);
                 ...
                 * @param WP_Query $query    The WP_Query instance (passed by reference).
                 */
                $corderby = apply_filters_ref_array("comment_feed_orderby", [
        ...
        if (!$q["suppress_filters"]) {
             ...
             * @param WP_Query $query The WP_Query instance (passed by reference).
             */
            $where = apply_filters_ref_array("posts_where_paged", [
                $where,
                &$this,
            ]);
             ...
             * @param WP_Query $query   The WP_Query instance (passed by reference).
             */
            $groupby = apply_filters_ref_array("posts_groupby", [
                $groupby,
                &$this,
            ]);
             ...
             * @param WP_Query $query The WP_Query instance (passed by reference).
             */
            $join = apply_filters_ref_array("posts_join_paged", [
                $join,
                &$this,
            ]);
             ...
             * @param WP_Query $query   The WP_Query instance (passed by reference).
             */
            $orderby = apply_filters_ref_array("posts_orderby", [
                $orderby,
                &$this,
            ]);
             ...
             * @param WP_Query $query    The WP_Query instance (passed by reference).
             */
            $distinct = apply_filters_ref_array("posts_distinct", [
                $distinct,
                &$this,
            ]);
             ...
             * @param WP_Query $query  The WP_Query instance (passed by reference).
             */
            $fields = apply_filters_ref_array("posts_fields", [
                $fields,
                &$this,
            ]);
             ...
             * @param WP_Query $query   The WP_Query instance (passed by reference).
             */
            $clauses = (array) apply_filters_ref_array("posts_clauses", [
                compact($pieces),
                &$this,
            ]);
        ...
        if (!$q["suppress_filters"]) {
             ...
             * @param WP_Query $query  The WP_Query instance (passed by reference).
             */
            $clauses = (array) apply_filters_ref_array(
         ...
         */
        $old_request = "SELECT $found_rows $distinct $fields
            ...
            if (null === $this->posts) {
                ...
                if ($cached_results) {
                    ...
                    if ("ids" === $q["fields"]) {
                        ...
                        $post_parent_cache_keys = [];
                        foreach ($post_ids as $post_id) {
                            $post_parent_cache_keys[] =
                                "post_parent:" . (string) $post_id;
                        }

                        /** @var int[] */
                        $post_parents = wp_cache_get_multiple(
                            $post_parent_cache_keys,
                            "posts"
                        );
                        ...
                        _prime_post_caches(
                            $post_ids,
                            $q["update_post_term_cache"],
                            $q["update_post_meta_cache"]
                        );
                        /** @var WP_Post[] */
        ...
        if ("id=>parent" === $q["fields"]) {
            ...
            foreach ($this->posts as $key => $post) {
                ...
                $post_parents_cache["post_parent:" . (string) $post->ID] =
        ...
        $is_unfiltered_query =
            $old_request === $this->request && "{$wpdb->posts}.*" === $fields;
        ...
        if (!$q["suppress_filters"]) {
             ...
             * @param WP_Query  $query The WP_Query instance (passed by reference).
             */
            $this->posts = apply_filters_ref_array("posts_results", [
                $this->posts,
                &$this,
            ]);
        }
        ...
        if (!empty($this->posts) && ($this->is_single || $this->is_page)) {
            ...
            if (
                ...
                0 === (int) $this->posts[0]->post_parent
            ) {
                $this->is_page = false;
                $this->is_single = true;
                $this->is_attachment = true;
                    ...
                    if (!is_user_logged_in()) {
                        ...
                        if ($post_status_obj->protected) {
                            ...
                            if (
                                ...
                                $this->is_preview = true;
                                if ("future" !== $status) {
                                    $this->posts[0]->post_date = current_time(
                                        "mysql"
                                    );
                                }
                            }
            ...
            if (
                $this->is_preview &&
                $this->posts &&
                current_user_can($edit_cap, $this->posts[0]->ID)
            ) {
                /**
                 ...
                 * @param WP_Query $query         The WP_Query instance (passed by reference).
                 */
                $this->posts[0] = get_post(
                    apply_filters_ref_array("the_preview", [
                        $this->posts[0],
                        &$this,
                    ])
                );
        ...
        if (
            ...
            for ($i = 0; $i < $num_posts; $i++) {
                if (in_array($this->posts[$i]->ID, $sticky_posts, true)) {
                    ...
                    array_splice($this->posts, $sticky_offset, 0, [
                        $sticky_post,
                    ]);
        ...
        if (!$q["suppress_filters"]) {
             ...
             * @param WP_Query  $query The WP_Query instance (passed by reference).
             */
            $this->posts = apply_filters_ref_array("the_posts", [
                $this->posts,
                &$this,
            ]);
        }
    ...
    private function set_found_posts($q, $limits)
    {
         ...
         * @param WP_Query $query       The WP_Query instance (passed by reference).
         */
        $this->found_posts = (int) apply_filters_ref_array("found_posts", [
            $this->found_posts,
            &$this,
        ]);

        if (!empty($limits)) {
            $this->max_num_pages = (int) ceil(
                $this->found_posts / $q["posts_per_page"]
            );
        }
    }
    ...
    public function have_posts()
    {
        if ($this->current_post + 1 < $this->post_count) {
            ...
            $this->current_post + 1 === $this->post_count &&
            $this->post_count > 0
        ) {
            /**
    ...
    protected function generate_cache_key(array $args, $sql)
    {
         ...
         */
        $sortable_arrays_with_int_values = ["post__in", "post_parent__in"];
