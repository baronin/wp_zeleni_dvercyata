...
class WP_Comment_Query
{
    ...
    public function get_comments()
    {
        ...
        if (!empty($this->meta_query->queries)) {
            $this->meta_query_clauses = $this->meta_query->get_sql(
                "comment",
                $wpdb->comments,
                "comment_ID",
                $this
         ...
         * @param WP_Comment_Query $query        The WP_Comment_Query instance, passed by reference.
         */
        $comment_data = apply_filters_ref_array("comments_pre_query", [
            $comment_data,
            &$this,
        ]);
        ...
        unset(
            $_args["fields"],
            $_args["update_comment_meta_cache"],
            $_args["update_comment_post_cache"]
        );
        ...
        if ($this->found_comments && $this->query_vars["number"]) {
            $this->max_num_pages = (int) ceil(
                $this->found_comments / $this->query_vars["number"]
            );
        }
         ...
         * @param WP_Comment_Query $query     Current instance of WP_Comment_Query (passed by reference).
         */
        $_comments = apply_filters_ref_array("the_comments", [
            $_comments,
            &$this,
        ]);
    ...
    protected function get_comment_ids()
    {
        ...
        if (!in_array("any", $statuses, true)) {
            ...
            if (!empty($status_clauses)) {
                $approved_clauses[] =
                    "( " . implode(" OR ", $status_clauses) . " )";
            }
        }

        // User IDs or emails whose unapproved comments are included, regardless of $status.
        if (!empty($this->query_vars["include_unapproved"])) {
            $include_unapproved = wp_parse_list(
                $this->query_vars["include_unapproved"]
            );
            ...
            foreach ($include_unapproved as $unapproved_identifier) {
                ...
                if (is_numeric($unapproved_identifier)) {
                    ...
                    if (
                        !empty($_GET["unapproved"]) &&
                        !empty($_GET["moderation-hash"])
                    ) {
                        // Only include requested comment.
                        $approved_clauses[] = $wpdb->prepare(
                            "( comment_author_email = %s AND comment_approved = '0' AND {$wpdb->comments}.comment_ID = %d )",
                            $unapproved_identifier,
                            (int) $_GET["unapproved"]
                        );
                    } else {
        ...
        if (!empty($approved_clauses)) {
            if (1 === count($approved_clauses)) {
                ...
                $this->sql_clauses["where"]["approved"] =
                    "( " . implode(" OR ", $approved_clauses) . " )";
            }
        }

        $order =
            "ASC" === strtoupper($this->query_vars["order"]) ? "ASC" : "DESC";
        ...
        if (in_array($this->query_vars["orderby"], ["none", [], false], true)) {
            ...
            foreach ($ordersby as $_key => $_value) {
                ...
                if (
                    ...
                    in_array($_orderby, ["comment_ID", "comment__in"], true)
                ) {
        ...
        if (!empty($this->query_vars["comment__in"])) {
            $this->sql_clauses["where"]["comment__in"] =
        ...
        if (!empty($this->query_vars["comment__not_in"])) {
            $this->sql_clauses["where"]["comment__not_in"] =
        ...
        if (!empty($this->query_vars["parent__in"])) {
            $this->sql_clauses["where"]["parent__in"] =
                "comment_parent IN ( " .
                implode(
                    ",",
        ...
        if (!empty($this->query_vars["parent__not_in"])) {
            $this->sql_clauses["where"]["parent__not_in"] =
                "comment_parent NOT IN ( " .
                implode(
                    ",",
        ...
        if (!empty($this->query_vars["post__in"])) {
            $this->sql_clauses["where"]["post__in"] =
                "comment_post_ID IN ( " .
                implode(",", wp_parse_id_list($this->query_vars["post__in"])) .
                " )";
        }

        // Parse comment post IDs for a NOT IN clause.
        if (!empty($this->query_vars["post__not_in"])) {
            $this->sql_clauses["where"]["post__not_in"] =
                "comment_post_ID NOT IN ( " .
                implode(
                    ",",
        ...
        if ("" !== $this->query_vars["karma"]) {
            $this->sql_clauses["where"]["karma"] = $wpdb->prepare(
                "comment_karma = %d",
                $this->query_vars["karma"]
            );
        }
        ...
        foreach ($raw_types as $operator => $_raw_types) {
            ...
            foreach ($_raw_types as $type) {
                switch ($type) {
                    ...
                    default:
                        $comment_types[$operator][] = $wpdb->prepare(
                            "%s",
                            $type
                        );
                        break;
            ...
            if (!empty($comment_types[$operator])) {
                ...
                $this->sql_clauses["where"][
                    "comment_type__" .
                        strtolower(str_replace(" ", "_", $operator))
        ...
        if ("" !== $parent) {
            $this->sql_clauses["where"]["parent"] = $wpdb->prepare(
                "comment_parent = %d",
                $parent
            );
        }
        ...
        if (is_array($this->query_vars["user_id"])) {
            $this->sql_clauses["where"]["user_id"] =
                "user_id IN (" .
                implode(
                    ",",
            ...
            $this->sql_clauses["where"]["user_id"] = $wpdb->prepare(
                "user_id = %d",
                $this->query_vars["user_id"]
            );
        }

        // Falsey search strings are ignored.
        if (
            isset($this->query_vars["search"]) &&
            strlen($this->query_vars["search"])
        ) {
            ...
            // Strip leading 'AND'.
            $this->sql_clauses["where"]["search"] = preg_replace(
                "/^\s*AND\s*/",
                "",
                $search_sql
            );
        ...
        $plucked = wp_array_slice_assoc($this->query_vars, [
            "post_author",
            "post_name",
            "post_parent",
        ]);
        ...
        if (!empty($this->query_vars["author__in"])) {
            $this->sql_clauses["where"]["author__in"] =
                "user_id IN ( " .
                implode(
                    ",",
        ...
        if (!empty($this->query_vars["author__not_in"])) {
            $this->sql_clauses["where"]["author__not_in"] =
                "user_id NOT IN ( " .
                implode(
                    ",",
        ...
        if (!empty($this->query_vars["post_author__in"])) {
            $join_posts_table = true;
            $this->sql_clauses["where"]["post_author__in"] =
                "post_author IN ( " .
                implode(
                    ",",
        ...
        if (!empty($this->query_vars["post_author__not_in"])) {
            $join_posts_table = true;
            $this->sql_clauses["where"]["post_author__not_in"] =
                "post_author NOT IN ( " .
                implode(
                    ",",
        ...
        if (
            !empty($this->query_vars["date_query"]) &&
            is_array($this->query_vars["date_query"])
        ) {
            $this->date_query = new WP_Date_Query(
                $this->query_vars["date_query"],
                "comment_date"
            );

            // Strip leading 'AND'.
            $this->sql_clauses["where"]["date_query"] = preg_replace(
                "/^\s*AND\s*/",
                "",
                $this->date_query->get_sql()
            );
        }
         ...
         * @param WP_Comment_Query $query   Current instance of WP_Comment_Query (passed by reference).
         */
        $clauses = apply_filters_ref_array("comments_clauses", [
            compact($pieces),
            &$this,
        ]);
        ...
        $this->request = "{$this->sql_clauses["select"]}
    ...
    private function set_found_comments()
    {
        ...
        if (
             ...
             */
            $found_comments_query = apply_filters(
    ...
    protected function fill_descendants($comments)
    {
        ...
        do {
            ...
            if ($_parent_ids) {
                ...
                $cache_data = wp_cache_get_multiple(
                    array_values($cache_keys),
                    "comment-queries"
                );
                foreach ($_parent_ids as $parent_id) {
            ...
            if ($uncached_parent_ids) {
                ...
                foreach ($level_comments as $level_comment) {
                    $parent_map[$level_comment->comment_parent][] =
                        $level_comment->comment_ID;
        ...
        _prime_comment_caches(
            $descendant_ids,
            $this->query_vars["update_comment_meta_cache"]
        );
        ...
        if ("threaded" === $this->query_vars["hierarchical"]) {
            ...
            foreach ($all_comments as $k => $c) {
                ...
                if (!isset($ref[$c->comment_parent])) {
                    ...
                    $ref[$_c->comment_ID] = $ref[
                        $_c->comment_parent
                    ]->get_child($_c->comment_ID);
                }
            }
    ...
    protected function parse_orderby($orderby)
    {
        ...
        if ($meta_query_clauses) {
            $allowed_keys = array_merge(
                $allowed_keys,
                array_keys($meta_query_clauses)
            );
        }

        $parsed = false;
        if (
            $this->query_vars["meta_key"] === $orderby ||
            "meta_value" === $orderby
        ) {
            ...
            $comment__in = implode(
                ",",
                array_map("absint", $this->query_vars["comment__in"])
            );
