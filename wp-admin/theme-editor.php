...
$theme = !empty($_REQUEST["theme"])
    ? sanitize_text_field($_REQUEST["theme"])
    : "";
...
if (isset($allowed_files["functions.php"])) {
    $allowed_files =
        ["functions.php" => $allowed_files["functions.php"]] + $allowed_files;
}
if (isset($allowed_files["style.css"])) {
    $allowed_files =
        ["style.css" => $allowed_files["style.css"]] + $allowed_files;
}
...
if ("POST" === $_SERVER["REQUEST_METHOD"]) {
    ...
    if (is_wp_error($r)) {
        ...
        if (
            check_ajax_referer(
                "edit-theme_" . $stylesheet . "_" . $relative_file,
                "nonce",
                false
            ) &&
...
if (!empty($posted_content)) {
    ...
    if (str_ends_with($file, ".php")) {
        ...
        if (!empty($functions)) {
            ...
            $docs_select .=
                '<option value="">' .
                esc_html__("Function Name&hellip;") .
                "</option>";
...
if (isset($_GET["a"])) {
    ...
    $error_code = esc_html(
        $edit_error->get_error_message()
            ? $edit_error->get_error_message()
            : $edit_error->get_error_code()
    );
    $message =
        "<p>" .
        __(
...
<div class="alignleft">
<h2>
	<?php if (wp_get_theme()->get("Name") === $theme->display("Name")) {
...
<div class="alignright">
	<form action="theme-editor.php" method="get">
		<label for="theme" id="theme-plugin-editor-selector"><?php _e(
      "Select theme to edit:"
  ); ?> </label>
		<select name="theme" id="theme">
		<?php foreach (wp_get_themes(["errors" => null]) as $a_stylesheet => $a_theme) {
      if (
          $a_theme->errors() &&
          "theme_no_stylesheet" === $a_theme->errors()->get_error_code()
      ) {
          continue;
      }
      ...
      echo "\n\t" .
          '<option value="' .
          esc_attr($a_stylesheet) .
          '"' .
          $selected .
          ">" .
          $a_theme->display("Name") .
          "</option>";
  } ?>
		</select>
...
<div id="templateside">
	...
	<ul role="tree" aria-labelledby="theme-files-label">
        ...
        sprintf(
            '<a href="%s">%s</a>',
            self_admin_url(
                "theme-editor.php?theme=" . urlencode($theme->get_template())
            ),
...
<?php if ($error):
	...
	<form name="template" id="template" action="theme-editor.php" method="post">
      ...
      "nonce"
		...
		<div>
			<label for="newcontent" id="theme-plugin-editor-label"><?php _e(
       "Selected file content:"
			...
			<input type="hidden" name="file" value="<?php echo esc_attr(
       $relative_file
   ); ?>" />
			<input type="hidden" name="theme" value="<?php echo esc_attr(
       $theme->get_stylesheet()
		...
		<?php if (!empty($functions)): ?>
			<div id="documentation" class="hide-if-no-js">
				...
				<input disabled id="docs-lookup" type="button" class="button" value="<?php esc_attr_e(
        "Look Up"
    ); ?>" onclick="if ( '' !== jQuery('#docs-list').val() ) { window.open( 'https://api.wordpress.org/core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=<?php echo urlencode(
    get_user_locale()
); ?>&amp;version=<?php echo urlencode(
...
if (!in_array("theme_editor_notice", $dismissed_pointers, true)) {
    ...
    if ($referer) {
        ...
        if (
            is_string($referer_path) &&
            !in_array(
                basename($referer_path),
	...
	<div id="file-editor-warning" class="notification-dialog-wrap file-editor-warning hide-if-no-js hidden">
		...
		<div class="notification-dialog">
         ...
         "If you decide to go ahead with direct edits anyway, use a file manager to create a copy with a new name and hang on to the original. That way, you can re-enable a functional version if something goes wrong."
				...
				<p>
					<a class="button file-editor-warning-go-back" href="<?php echo esc_url(
         $return_url
     ); ?>"><?php _e("Go back"); ?></a>
					<button type="button" class="file-editor-warning-dismiss button button-primary"><?php _e(
         "I understand"
     ); ?></button>
				</p>
			</div>
