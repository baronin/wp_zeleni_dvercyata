...
if ($action) {
    switch ($action) {
        case "activate":
            ...
            $result = activate_plugin(
                $plugin,
                self_admin_url(
                    "plugins.php?error=true&plugin=" . urlencode($plugin)
                ),
                is_network_admin()
            );
            if (is_wp_error($result)) {
                if ("unexpected_output" === $result->get_error_code()) {
                    $redirect = self_admin_url(
        ...
        case "activate-selected":
            ...
            $plugins = isset($_POST["checked"])
                ? (array) wp_unslash($_POST["checked"])
                : [];
        ...
        case "deactivate":
            ...
            if (headers_sent()) {
                echo "<meta http-equiv='refresh' content='" .
                    esc_attr(
                        "0;url=plugins.php?deactivate=true&plugin_status=$status&paged=$page&s=$s"
                    ) .
                    "' />";
            } else {
                wp_redirect(
        ...
        case "deactivate-selected":
            ...
            if (is_network_admin()) {
                ...
                $plugins = array_diff(
                    $plugins,
                    array_filter($plugins, "is_plugin_active_for_network")
                );
            ...
            if (!is_network_admin()) {
                update_option(
                    "recently_activated",
                    $deactivated + (array) get_option("recently_activated"),
                    false
                );
            } else {
                update_site_option(
        ...
        case "delete-selected":
            ...
            $plugins = isset($_REQUEST["checked"])
                ? (array) wp_unslash($_REQUEST["checked"])
                : [];
            if (empty($plugins)) {
                wp_redirect(
                    self_admin_url(
        ...
        if ("." === $plugin_slug) {
            ...
            if ($folder_plugins) {
                foreach ($folder_plugins as $plugin_file => $data) {
                    $plugin_info[
                        $plugin_file
                    ] = _get_plugin_data_markup_translate($plugin_file, $data);
                    $plugin_info[$plugin_file][
                        "is_uninstallable"
                    ] = is_uninstallable_plugin($plugin);
    ...
    ?>
         ...
         $maybe_active_plugin =
             ...
             __("Caution:") .
             "</strong> " .
             __("This plugin may be active on other sites in the network.");
         wp_admin_notice($maybe_active_plugin, [
         ...
         $maybe_active_plugins =
             ...
             __("Caution:") .
             "</strong> " .
             __("These plugins may be active on other sites in the network.");
         wp_admin_notice($maybe_active_plugins, [
             "additional_classes" => ["error"],
         ]);
     endif; ?>
      ...
      foreach ($plugin_info as $plugin) {
          if ($plugin["is_uninstallable"]) {
              ...
              echo "<li>",
                  sprintf(
                      _x('%1$s by %2$s', "plugin"),
                      "<strong>" . $plugin["Name"] . "</strong>",
                      "<em>" . $plugin["AuthorName"]
                  ) . "</em>",
                  "</li>";
          }
      }
      ?>
        ...
        $_SERVER["REQUEST_URI"]
         ...
         echo '<input type="hidden" name="checked[]" value="' .
             ...
             '" />';
					...
					<?php submit_button(
         $data_to_delete
             ? __("Yes, delete these files and data")
             : __("Yes, delete these files"),
         "",
         "submit",
         false
     ); ?>
        ...
        : ""; ?>" style="display:inline;">
					<?php submit_button(
         __("No, return me to the plugin list"),
         "",
         "submit",
         false
     ); ?>
    ...
    exit();
            ...
            wp_redirect(
                self_admin_url(
                    "plugins.php?deleted=$plugins_to_delete&plugin_status=$status&paged=$page&s=$s"
                )
            );
            exit();
...
if (!empty($invalid)) {
    foreach ($invalid as $plugin_file => $error) {
        ...
        wp_admin_notice($deactivated_message, [
            "id" => "message",
            "additional_classes" => ["error"],
        ]);
    }
...
if (isset($_GET["error"])) {
    ...
    wp_admin_notice(
        __("All selected plugins are up to date."),
        $updated_notice_args
    );
    ...
    wp_admin_notice(
        __("Plugin will no longer be auto-updated."),
        $updated_notice_args
    );
} elseif (isset($_GET["enabled-auto-update-multi"])) {
    wp_admin_notice(
        __("Selected plugins will be auto-updated."),
        $updated_notice_args
    );
} elseif (isset($_GET["disabled-auto-update-multi"])) {
    wp_admin_notice(
        __("Selected plugins will no longer be auto-updated."),
        $updated_notice_args
    );
}
?>
...
if (
 ...
 ); ?>" class="page-title-action"><?php echo esc_html__("Add Plugin"); ?></a>
	<?php }
if (strlen($s)) {
