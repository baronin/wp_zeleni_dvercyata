...
if (
    !empty($_GET["action"]) &&
    in_array($_GET["action"], $core_actions_get, true)
) {
    add_action(
        "wp_ajax_" . $_GET["action"],
...
if (
    !empty($_POST["action"]) &&
    in_array($_POST["action"], $core_actions_post, true)
) {
    add_action(
        "wp_ajax_" . $_POST["action"],
        "wp_ajax_" . str_replace("-", "_", $_POST["action"]),
        1
    );
}

add_action(
    "wp_ajax_nopriv_generate-password",
...
add_action("wp_ajax_check_plugin_dependencies", [
