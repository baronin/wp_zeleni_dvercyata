...
wp_localize_script("theme", "_wpThemeSettings", [
    ...
    "settings" => [
        ...
        "installURI" =>
            !is_multisite() && current_user_can("install_themes")
...
<div class="wrap">
	<h1 class="wp-heading-inline"><?php esc_html_e("Themes"); ?>
		<span class="title-count theme-count"><?php echo !empty($_GET["search"])
      ? __("&hellip;")
      : count($themes); ?></span>
	</h1>
	<?php if (!is_multisite() && current_user_can("install_themes")): ?>
      ...
      admin_url("theme-install.php")
  ); ?>" class="hide-if-no-js page-title-action"><?php echo esc_html__(
    "Add Theme"
); ?></a>
	<?php endif; ?>
...
if (
    ...
    wp_admin_notice(
        "<strong>" .
            __("Error:") .
            "</strong> " .
            $current_theme->errors()->get_error_message(),
        [
            "additional_classes" => ["error"],
        ]
    );
...
if (is_array($submenu) && isset($submenu["themes.php"])) {
    ...
    foreach ((array) $submenu["themes.php"] as $item) {
        ...
        if (
            in_array($item[2], $forbidden_paths, true) ||
            str_starts_with($item[2], "customize.php")
        ) {
            continue;
        }
        ...
        if (
            (0 === strcmp($self, $item[2]) && empty($parent_file)) ||
        ...
        if (!empty($submenu[$item[2]])) {
            ...
            if (
                file_exists(WP_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}") ||
                !empty($menu_hook)
            ) {
...
<div class="theme<?php echo $active_class; ?>">
	<?php if (!empty($theme["screenshot"][0])) { ?>
       ...
       $theme["screenshot"][0] . "?ver=" . $theme["version"]
   ); ?>" alt="" />
		</div>
	<?php } else { ?>
 ...
 if ($theme["hasUpdate"]): ?>
      ...
      wp_admin_notice($new_version_available, [
          "type" => "warning",
      ...
      wp_admin_notice($theme_update_error, [
          "type" => "error",
          "additional_classes" => ["notice-alt", "inline", "update-message"],
      ]);
  endif;endif;
 ...
 /* translators: %s: Theme name. */
 $details_aria_label = sprintf(
     _x("View Theme Details for %s", "theme"),
     $theme["name"]
 );
 ?>
	<button type="button" class="more-details" id="<?php echo esc_attr(
     $aria_action
 ); ?>"
  ...
  printf(__("By %s"), $theme["author"]); ?>
	...
	<div class="theme-id-container">
		...
		<?php } else { ?>
			<h2 class="theme-name" id="<?php echo esc_attr(
       $aria_name
   ); ?>"><?php echo $theme["name"]; ?></h2>
		<?php } ?>
       ...
       /* translators: %s: Theme name. */
       $customize_aria_label = sprintf(
           _x("Customize %s", "theme"),
           $theme["name"]
       ); ?>
   ...
   $aria_label = sprintf(_x("Activate %s", "theme"), "{{ data.name }}"); ?>
			...
			<?php // Only classic themes require the "customize" capability.
   if (
       current_user_can("edit_theme_options") &&
       ($theme["blockTheme"] || current_user_can("customize"))
   ) {
       /* translators: %s: Theme name. */
       $live_preview_aria_label = sprintf(
           _x("Live Preview %s", "theme"),
           "{{ data.name }}"
       ); ?>
...
<div class="theme-overlay" tabindex="0" role="dialog" aria-label="<?php esc_attr_e(
    "Theme Details"
); ?>"></div>
...
<table>
	...
	<?php foreach ($broken_themes as $broken_theme): ?>
		<tr>
			<td><?php echo $broken_theme->get("Name")
       ? $broken_theme->display("Name")
       : esc_html($broken_theme->get_stylesheet()); ?></td>
   ...
   if (
       ...
       if (!is_wp_error($parent_theme)) {
           ...
           ?>
					<td><a href="<?php echo esc_url(
         $install_url
     ); ?>" class="button install-theme"><?php _e(
