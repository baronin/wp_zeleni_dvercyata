...
if (isset($_POST["savewidget"]) || isset($_POST["removewidget"])) {
    ...
    $position = isset($_POST[$sidebar_id . "_position"])
        ? (int) $_POST[$sidebar_id . "_position"] - 1
        : 0;

    $id_base = $_POST["id_base"];
    $sidebar = isset($sidebars_widgets[$sidebar_id])
...
if (isset($_GET["editwidget"]) && $_GET["editwidget"]) {
    ...
    if (isset($wp_registered_widget_controls[$widget_id]) && !isset($control)) {
        ...
        $name = esc_html(
            strip_tags($wp_registered_widgets[$widget_id]["name"])
        );
    }
    ...
    if (!isset($sidebar)) {
        $sidebar = isset($_GET["sidebar"])
            ? $_GET["sidebar"]
            : "wp_inactive_widgets";
    }

    if (!isset($multi_number)) {
        $multi_number = isset($control["params"][0]["number"])
            ? $control["params"][0]["number"]
            : "";
    }

    $id_base = isset($control["id_base"])
        ? $control["id_base"]
        : $control["id"];
 ...
 printf(__("Widget %s"), $name); ?>
	...
	<p class="describe"><?php _e(
     "Select both the sidebar for this widget and the position of the widget in that sidebar."
 ); ?></p>
	<div class="widget-position">
	<table class="widefat"><thead><tr><th><?php _e("Sidebar"); ?></th><th><?php _e(
    "Position"
); ?></th></tr></thead><tbody>
	<?php foreach ($wp_registered_sidebars as $sbname => $sbvalue) {
     echo "\t\t<tr><td><label><input type='radio' name='sidebar' value='" .
         esc_attr($sbname) .
         "'" .
         checked($sbname, $sidebar, false) .
         " /> $sbvalue[name]</label></td><td>";
     if (
         "wp_inactive_widgets" === $sbname ||
         str_starts_with($sbname, "orphaned_widgets")
     ) {
         echo "&nbsp;";
     } else {
         if (
             !isset($sidebars_widgets[$sbname]) ||
             !is_array($sidebars_widgets[$sbname])
         ) {
             $j = 1;
             ...
             if (
                 isset($_GET["addnew"]) ||
                 !in_array($widget_id, $sidebars_widgets[$sbname], true)
             ) {
                 ++$j;
             }
         }
	...
	<div class="widget-control-actions">
		<div class="alignleft">
			<?php if (!isset($_GET["addnew"])): ?>
				<input type="submit" name="removewidget" id="removewidget" class="button-link button-link-delete widget-control-remove" value="<?php esc_attr_e(
        "Delete"
    ); ?>" />
				<span class="widget-control-close-wrapper">
					| <a href="widgets.php" class="button-link widget-control-close"><?php _e(
         "Cancel"
     ); ?></a>
				</span>
			<?php else: ?>
       ...
       false
			...
			<input type="hidden" name="widget-id" class="widget-id" value="<?php echo esc_attr(
       $widget_id
   ); ?>" />
			<input type="hidden" name="id_base" class="id_base" value="<?php echo esc_attr(
       $id_base
   ); ?>" />
			<input type="hidden" name="multi_number" class="multi_number" value="<?php echo esc_attr(
       $multi_number
   ); ?>" />
...
<div class="widget-access-link">
	<a id="access-on" href="widgets.php?widgets-access=on&_wpnonce=<?php echo urlencode(
     $nonce
 ); ?>"><?php _e(
    "Enable accessibility mode"
); ?></a><a id="access-off" href="widgets.php?widgets-access=off&_wpnonce=<?php echo urlencode(
    $nonce
); ?>"><?php _e("Disable accessibility mode"); ?></a>
</div>
...
<div id="widgets-left">
	<div id="available-widgets" class="widgets-holder-wrap">
     ...
     _e("Available Widgets"); ?>
			...
			<h2><?php _e("Available Widgets"); ?> <span id="removing-widget"><?php _ex(
     "Deactivate",
     "removing-widget"
 ); ?> <span></span></span></h2>
		</div>
...
foreach ($wp_registered_sidebars as $sidebar => $registered_sidebar) {
    if (
        str_contains($registered_sidebar["class"], "inactive-sidebar") ||
        ...
        $is_inactive_widgets =
            "wp_inactive_widgets" === $registered_sidebar["id"];
        ?>
		<div class="<?php echo esc_attr($wrap_class); ?>">
			<div class="widget-holder inactive">
				<?php wp_list_widget_controls(
        $registered_sidebar["id"],
...
<?php foreach ($theme_sidebars as $sidebar => $registered_sidebar) {
    ...
    ?>
	<div class="<?php echo esc_attr($wrap_class); ?>">
		<?php // Show the control forms for each of the widgets in this sidebar.
...
<div class="widgets-chooser">
	...
	<div class="widgets-chooser-actions">
		...
		<button class="button button-primary widgets-chooser-add"><?php _e(
      "Add Widget"
  ); ?></button>
	</div>
</div>
