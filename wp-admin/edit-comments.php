...
if ($post_id) {
    ...
    $draft_or_post_title = wp_html_excerpt(
...
if (
    ...
    $untrashed = isset($_REQUEST["untrashed"])
        ? (int) $_REQUEST["untrashed"]
        : 0;
    $spammed = isset($_REQUEST["spammed"]) ? (int) $_REQUEST["spammed"] : 0;
    $unspammed = isset($_REQUEST["unspammed"])
                    ...
                    case "1":
                        $messages[] =
                            ...
                            sprintf(
                                ...
                                esc_url(
                                    admin_url(
                                        "comment.php?action=editcomment&c=$same"
                                    )
                                ),
                                __("Edit comment")
                            );
...
<?php endif; ?>
<input type="hidden" name="comment_status" value="<?php echo esc_attr(
    $comment_status
); ?>" />
<input type="hidden" name="pagegen_timestamp" value="<?php echo esc_attr(
    current_time("mysql", 1)
); ?>" />

<input type="hidden" name="_total" value="<?php echo esc_attr(
    $wp_list_table->get_pagination_arg("total_items")
); ?>" />
<input type="hidden" name="_per_page" value="<?php echo esc_attr(
    $wp_list_table->get_pagination_arg("per_page")
); ?>" />
<input type="hidden" name="_page" value="<?php echo esc_attr(
    $wp_list_table->get_pagination_arg("page")
); ?>" />
