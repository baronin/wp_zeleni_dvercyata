...
$wp_http_referer = !empty($_REQUEST["wp_http_referer"])
    ? sanitize_url($_REQUEST["wp_http_referer"])
    : "";
...
if (IS_PROFILE_PAGE && isset($_GET["newuseremail"]) && $current_user->ID) {
    ...
    if ($new_email && hash_equals($new_email["hash"], $_GET["newuseremail"])) {
        ...
        wp_redirect(
            add_query_arg(["updated" => "true"], self_admin_url("profile.php"))
        );
        die();
    } else {
        wp_redirect(
...
switch ($action) {
  ...
  if (
      !IS_PROFILE_PAGE &&
      is_super_admin($profile_user->ID) &&
      current_user_can("manage_network_options")
  ):
      $message =
          "<strong>" .
          __("Important:") .
          "</strong> " .
          __("This user has super admin privileges.");
  ...
  if (isset($_GET["updated"])):
      ...
      if (
          $wp_http_referer &&
          !str_contains($wp_http_referer, "user-new.php") &&
          !IS_PROFILE_PAGE
      ):
          $message .= sprintf(
  ...
  ?>
		...
		<div class="wrap" id="profile-page">
			...
			<?php if (!IS_PROFILE_PAGE): ?>
				<?php if (current_user_can("create_users")): ?>
					<a href="user-new.php" class="page-title-action"><?php echo esc_html__(
         "Add User"
     ); ?></a>
         ...
         "Add Existing User"
			...
			<form id="your-profile" action="<?php echo esc_url(
    ...
    do_action("user_edit_form_tag"); ?>
				...
				<?php if ($wp_http_referer): ?>
					<input type="hidden" name="wp_http_referer" value="<?php echo esc_url(
         $wp_http_referer
     ); ?>" />
				<?php endif; ?>
				<p>
				...
				<table class="form-table" role="presentation">
					<?php if (
         !(IS_PROFILE_PAGE && !$user_can_edit) &&
         "false" === $profile_user->rich_editing
     ): ?>
						<tr class="user-rich-editing-wrap">
							<th scope="row"><?php _e("Visual Editor"); ?></th>
							<td>
								<label for="rich_editing"><input name="rich_editing" type="checkbox" id="rich_editing" value="false" <?php checked(
            "false",
            $profile_user->rich_editing
					...
					<?php $show_syntax_highlighting_preference =
         ...
         user_can($profile_user, "edit_themes"); ?>
					...
					<tr class="user-syntax-highlighting-wrap">
						...
						<td>
							<label for="syntax_highlighting"><input name="syntax_highlighting" type="checkbox" id="syntax_highlighting" value="false" <?php checked(
           "false",
           $profile_user->syntax_highlighting
					...
					<?php if (
         count($_wp_admin_css_colors) > 1 &&
         has_action("admin_color_scheme_picker")
     ): ?>
					...
					<tr class="user-comment-shortcuts-wrap">
						...
						<td>
							<label for="comment_shortcuts">
								<input type="checkbox" name="comment_shortcuts" id="comment_shortcuts" value="true" <?php checked(
            "true",
					...
					<tr class="show-admin-bar user-admin-bar-front-wrap">
						...
						<td>
							<label for="admin_bar_front">
								<input name="admin_bar_front" type="checkbox" id="admin_bar_front" value="1"<?php checked(
     ...
     ?>
       ...
       if ("en_US" === $user_locale) {
           ...
           "" === $user_locale ||
           !in_array($user_locale, $languages, true)
       ) {
        ...
        do_action("profile_personal_options", $profile_user);
				...
				<table class="form-table" role="presentation">
					<tr class="user-user-login-wrap">
						...
						<td><input type="text" name="user_login" id="user_login" value="<?php echo esc_attr(
          $profile_user->user_login
      ); ?>" readonly="readonly" class="regular-text" /> <span class="description"><?php _e(
    "Usernames cannot be changed."
         ...
         $user_roles = array_intersect(
             array_values($profile_user->roles),
        ...
        ?>
         ...
         !isset($super_admins)
            ...
            0 !==
                strcasecmp(
                    $profile_user->user_email,
                    get_site_option("admin_email")
                ) ||
            !is_super_admin($profile_user->ID)
        ): ?>
									<p><label><input type="checkbox" id="super_admin" name="super_admin"<?php checked(
             is_super_admin($profile_user->ID)
         ); ?> /> <?php _e(
     "Grant this user super admin privileges for the Network."
             ...
             "Super admin privileges cannot be removed because this user has the network admin email."
					...
					<tr class="user-first-name-wrap">
						...
						<td><input type="text" name="first_name" id="first_name" value="<?php echo esc_attr(
          $profile_user->first_name
					...
					<tr class="user-last-name-wrap">
						...
						<td><input type="text" name="last_name" id="last_name" value="<?php echo esc_attr(
          $profile_user->last_name
      ); ?>" class="regular-text" /></td>
					</tr>

					<tr class="user-nickname-wrap">
						<th><label for="nickname"><?php _e(
          "Nickname"
      ); ?> <span class="description"><?php _e(
     "(required)"
 ); ?></span></label></th>
						<td><input type="text" name="nickname" id="nickname" value="<?php echo esc_attr(
        ...
        if (
            !empty($profile_user->first_name) &&
            !empty($profile_user->last_name)
        ) {
            $public_display["display_firstlast"] =
                $profile_user->first_name . " " . $profile_user->last_name;
            $public_display["display_lastfirst"] =
                $profile_user->last_name . " " . $profile_user->first_name;
        }

        if (!in_array($profile_user->display_name, $public_display, true)) {
            // Only add this if it isn't duplicated elsewhere.
            $public_display =
                ["display_displayname" => $profile_user->display_name] +
                $public_display;
        }
             ...
             $item
				...
				<table class="form-table" role="presentation">
					<tr class="user-email-wrap">
						<th><label for="email"><?php _e("Email"); ?> <span class="description"><?php _e(
     "(required)"
 ); ?></span></label></th>
						<td>
							<?php if ($profile_user->ID === $current_user->ID): ?>
								<input type="email" name="email" id="email" aria-describedby="email-description" value="<?php echo esc_attr(
            $profile_user->user_email
             ...
             "If you change this, an email will be sent at your new address to confirm it. <strong>The new address will not become active until confirmed.</strong>"
							...
							<?php else: ?>
								<input type="email" name="email" id="email" value="<?php echo esc_attr(
            $profile_user->user_email
        ); ?>" class="regular-text ltr" />
							<?php endif; ?>
       ...
       if (
           $new_email &&
           $new_email["newemail"] !== $current_user->user_email &&
       ...
       ?>
					...
					<tr class="user-url-wrap">
						...
						<td><input type="url" name="url" id="url" value="<?php echo esc_attr(
          $profile_user->user_url
       ...
       echo apply_filters("user_{$name}_label", $desc); ?>
						...
						<td>
							<input type="text" name="<?php echo $name; ?>" id="<?php echo $name; ?>" value="<?php echo esc_attr(
    $profile_user->$name
); ?>" class="regular-text" />
						</td>
					</tr>
				...
				<table class="form-table" role="presentation">
					<tr class="user-description-wrap">
						...
						<td><textarea name="description" id="description" rows="5" cols="30"><?php echo $profile_user->description;
        // textarea_escaped
        ?></textarea>
      ...
      */
     $show_password_fields = apply_filters(
						...
						<table class="form-table" role="presentation">
							<tr id="password" class="user-pass1-wrap">
								...
								<td>
									...
									<button type="button" class="button wp-generate-pw hide-if-no-js" aria-expanded="false"><?php _e(
             "Set New Password"
         ); ?></button>
									<div class="wp-pwd hide-if-js">
										<div class="password-input-wrapper">
											<input type="password" name="pass1" id="pass1" class="regular-text" value="" autocomplete="new-password" spellcheck="false" data-pw="<?php echo esc_attr(
               wp_generate_password(24)
           ); ?>" aria-describedby="pass-strength-result" />
											<div style="display:none" id="pass-strength-result" aria-live="polite"></div>
										</div>
										<button type="button" class="button wp-hide-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e(
              "Hide password"
          ); ?>">
											<span class="dashicons dashicons-hidden" aria-hidden="true"></span>
											<span class="text"><?php _e("Hide"); ?></span>
										</button>
										<button type="button" class="button wp-cancel-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e(
              "Cancel password change"
							...
							<tr class="user-pass2-wrap hide-if-js">
								<th scope="row"><label for="pass2"><?php _e(
            "Repeat New Password"
        ); ?></label></th>
								<td>
								<input type="password" name="pass2" id="pass2" class="regular-text" value="" autocomplete="new-password" spellcheck="false" aria-describedby="pass2-desc" />
									<?php if (IS_PROFILE_PAGE): ?>
										<p class="description" id="pass2-desc"><?php _e(
							...
							<tr class="pw-weak">
								...
								<td>
									<label>
										...
										<span id="pw-weak-text-label"><?php _e(
        ...
        ?>
							<?php if (
           !IS_PROFILE_PAGE &&
           true === wp_is_password_reset_allowed_for_user($profile_user)
       ): ?>
							...
							<?php if (IS_PROFILE_PAGE && count($sessions->get_all()) === 1): ?>
								<tr class="user-sessions-wrap hide-if-no-js">
									...
									<td aria-live="assertive">
										<div class="destroy-sessions"><button type="button" disabled class="button"><?php _e(
           ...
           elseif (IS_PROFILE_PAGE && count($sessions->get_all()) > 1): ?>
								<tr class="user-sessions-wrap hide-if-no-js">
									<th><?php _e("Sessions"); ?></th>
									<td aria-live="assertive">
										<div class="destroy-sessions"><button type="button" class="button" id="destroy-sessions"><?php _e(
              "Log Out Everywhere Else"
          ); ?></button></div>
										<p class="description">
											<?php _e(
               "Did you lose your phone or leave your account logged in at a public computer? You can log out everywhere else, and stay logged in here."
							...
							<?php elseif (!IS_PROFILE_PAGE && $sessions->get_all()): ?>
								<tr class="user-sessions-wrap hide-if-no-js">
									<th><?php _e("Sessions"); ?></th>
									<td>
										<p><button type="button" class="button" id="destroy-sessions"><?php _e(
              "Log Out Everywhere"
          ); ?></button></p>
										<p class="description">
											<?php printf(
         ...
         !wp_is_application_passwords_supported()
           ...
           ?>
								...
								<?php if (!wp_is_site_protected_by_basic_auth("front")): ?>
									<div class="create-application-password form-wrap">
										<div class="form-field">
											<label for="new_application_password_name"><?php _e(
               "New Application Password Name"
											...
											<p class="description" id="new_application_password_name_desc"><?php _e(
          ...
          do_action("wp_create_application_password_form", $profile_user); ?>
										...
										<button type="button" name="do_new_application_password" id="do_new_application_password" class="button button-secondary"><?php _e(
              "Add Application Password"
          ); ?></button>
									</div>
									<?php else:wp_admin_notice(
                __(
         ...
         ?>
           ...
           /* translators: %s: Documentation URL. */
           elseif (!wp_is_application_passwords_supported()): ?>
								<p><?php _e(
      ...
      * @param WP_User $profile_user The current WP_User object.
      */
     $display_additional_caps = apply_filters(
         ...
         $profile_user
				...
				<?php if (
        count($profile_user->caps) > count($profile_user->roles) &&
        true === $display_additional_caps
    ): ?>
        ...
        ?>
				...
				<input type="hidden" name="user_id" id="user_id" value="<?php echo esc_attr(
        $user_id
    ); ?>" />

				<?php submit_button(
        IS_PROFILE_PAGE ? __("Update Profile") : __("Update User")
    ); ?>
	...
	<script type="text/html" id="tmpl-new-application-password">
		<div class="notice notice-success is-dismissible new-application-password-notice" role="alert">
         ...
         "<strong>{{ data.name }}</strong>"
				...
				<button type="button" class="button copy-button" data-clipboard-text="{{ data.password }}"><?php _e(
        "Copy"
    ); ?></button>
