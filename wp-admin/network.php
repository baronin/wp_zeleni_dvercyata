...
if (is_multisite()) {
    ...
    if (!defined("MULTISITE")) {
        wp_die(
            __("The Network creation panel is not for WordPress MU networks.")
        );
    }
}
...
$network_help =
    ...
    "<p>" .
    __(
        '<a href="https://developer.wordpress.org/advanced-administration/multisite/create-network/">Documentation on Creating a Network</a>'
    ) .
    "</p>" .
    "<p>" .
    __(
        '<a href="https://wordpress.org/documentation/article/tools-network-screen/">Documentation on the Network Screen</a>'
    ) .
    "</p>";
...
<?php if ($_POST) {
    ...
    if (!network_domain_check()) {
        ...
        if (is_wp_error($result)) {
            if (
                1 === count($result->get_error_codes()) &&
                "no_wildcard_dns" === $result->get_error_code()
            ) {
