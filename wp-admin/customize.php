...
if ($wp_customize->changeset_post_id()) {
  ...
  wp_die(
      "<h1>" .
          ...
          "</h1>" .
          '<p><a href="' .
          esc_url(remove_query_arg("changeset_uuid")) .
          '">' .
    ...
    if (
        in_array(
            get_post_status($changeset_post->ID),
            ["publish", "trash"],
            true
        )
    ) {
...
$url = !empty($_REQUEST["url"])
    ? esc_url_raw(wp_unslash($_REQUEST["url"]))
    : "";
$return = !empty($_REQUEST["return"])
    ? esc_url_raw(wp_unslash($_REQUEST["return"]))
    : "";
$autofocus =
    !empty($_REQUEST["autofocus"]) && is_array($_REQUEST["autofocus"])
...
$admin_title = sprintf(
    $wp_customize->get_document_title_template(),
    __("Loading&hellip;")
);
?>
...
var ajaxurl = <?php echo wp_json_encode(
...
<div class="wp-full-overlay expanded">
	<form id="customize-controls" class="wrap wp-full-overlay-sidebar">
		<div id="customize-header-actions" class="wp-full-overlay-header">
			<?php
   $compatible_wp = is_wp_version_compatible(
       $wp_customize->theme()->get("RequiresWP")
   );
   $compatible_php = is_php_version_compatible(
       $wp_customize->theme()->get("RequiresPHP")
   );
   ?>
			<?php if ($compatible_wp && $compatible_php): ?>
				<?php $save_text = $wp_customize->is_theme_active()
        ? __("Publish")
        : __("Activate &amp; Publish"); ?>
				<div id="customize-save-button-wrapper" class="customize-save-button-wrapper" >
					...
					<button id="publish-settings" class="publish-settings button-primary button dashicons dashicons-admin-generic" aria-label="<?php esc_attr_e(
         "Publish Settings"
			...
			<?php else: ?>
				...
				<div id="customize-save-button-wrapper" class="customize-save-button-wrapper disabled" >
					<button class="button button-primary disabled" aria-label="<?php esc_attr_e(
         "Publish Settings"
     ); ?>" aria-expanded="false" disabled><?php echo $save_text; ?></button>
				</div>
			<?php endif; ?>
			...
			<a class="customize-controls-close" href="<?php echo esc_url(
       $wp_customize->get_return_url()
   ); ?>">
			...
			<div class="wp-full-overlay-sidebar-content" tabindex="-1">
				<div id="customize-info" class="accordion-section customize-info" data-block-theme="<?php echo (int) wp_is_block_theme(); ?>">
					<div class="accordion-section-title">
						...
						<?php /* translators: %s: The site/panel title in the Customizer. */
      printf(
          __("You are customizing %s"),
          '<strong class="panel-title site-title">' .
...
?></ul>
		...
		<div id="customize-footer-actions" class="wp-full-overlay-footer">
			<button type="button" class="collapse-sidebar button" aria-expanded="true" aria-label="<?php echo esc_attr_x(
       "Hide Controls",
      ...
      ?>
						<button type="button" class="<?php echo esc_attr(
          $class
      ); ?>" aria-pressed="<?php echo esc_attr(
    $active
); ?>" data-device="<?php echo esc_attr($device); ?>">
							<span class="screen-reader-text"><?php echo esc_html(
           $settings["label"]
       ); ?></span>
						</button>
					<?php endforeach; ?>
