...
$action = !empty($_REQUEST["action"])
    ...
    : "";
$option_page = !empty($_REQUEST["option_page"])
    ? sanitize_text_field($_REQUEST["option_page"])
    : "";
...
if ("update" === $action) {
    ...
    if ("options" === $option_page) {
        ...
        $options = isset($_POST["page_options"])
            ? explode(",", wp_unslash($_POST["page_options"]))
            : null;
    } else {
    ...
    if ("general" === $option_page) {
        ...
        if (
            !empty($_POST["date_format"]) &&
            isset($_POST["date_format_custom"]) &&
        ...
        if (
            !empty($_POST["time_format"]) &&
            isset($_POST["time_format_custom"]) &&
        ...
        if (
            !empty($_POST["timezone_string"]) &&
            preg_match("/^UTC[+-]/", $_POST["timezone_string"])
        ) {
            $_POST["gmt_offset"] = $_POST["timezone_string"];
            $_POST["gmt_offset"] = preg_replace(
                "/UTC\+?/",
                "",
                $_POST["gmt_offset"]
            );
...
foreach ((array) $options as $option):
    ...
    ?>
<tr>
	<th scope="row"><label for="<?php echo $name; ?>"><?php echo esc_html(
    $option->option_name
); ?></label></th>
<td>
	<?php if (str_contains($value, "\n")): ?>
		<textarea class="<?php echo $class; ?>" name="<?php echo $name; ?>" id="<?php echo $name; ?>" cols="30" rows="5"><?php echo esc_textarea(
    $value
); ?></textarea>
	<?php else: ?>
		<input class="regular-text <?php echo $class; ?>" type="text" name="<?php echo $name; ?>" id="<?php echo $name; ?>" value="<?php echo esc_attr(
    $value
...
<input type="hidden" name="page_options" value="<?php echo esc_attr(
    implode(",", $options_to_update)
); ?>" />
