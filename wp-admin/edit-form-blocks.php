...
$global_styles_endpoint_context = current_user_can("edit_theme_options")
    ? "edit"
    : "view";
$template_lookup_slug =
    "page" === $post->post_type ? "page" : "single-" . $post->post_type;
...
$preload_paths = [
    ...
    WP_Theme_JSON_Resolver::get_user_global_styles_post_id() .
    "?context=" .
    $global_styles_endpoint_context,
...
$available_templates = !empty($available_templates)
    ? array_replace(
        [
...
if ($user_id) {
    ...
    if ($locked) {
        ...
        if (get_option("show_avatars")) {
            $user_details["avatar"] = get_avatar_url($user_id, ["size" => 128]);
        }
    }
 ...
 */
$body_placeholder = apply_filters(
    "write_your_story",
...
$editor_settings = [
    ...
    "titlePlaceholder" => apply_filters(
        "enter_title_here",
        __("Add title"),
        $post
    ),
    ...
    "enableCustomFields" => (bool) get_user_meta(
        get_current_user_id(),
...
if (!empty($post_type_object->template)) {
    ...
    $editor_settings["templateLock"] = !empty($post_type_object->template_lock)
...
<div class="block-editor">
	<h1 class="screen-reader-text hide-if-no-js"><?php echo esc_html(
     $title
 ); ?></h1>
...
?>
  ...
  if (file_exists(WP_PLUGIN_DIR . "/classic-editor/classic-editor.php")) {
      ...
      $plugin_install_url = wp_nonce_url(
          self_admin_url(
              "update.php?action=install-plugin&plugin=classic-editor"
          ),
