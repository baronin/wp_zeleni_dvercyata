...
switch ($wp_list_table->current_action()) {
    ...
    case "dodelete":
        ...
        if (empty($_REQUEST["delete_option"])) {
            $url = self_admin_url(
                "users.php?action=delete&users[]=" .
                    implode("&users[]=", $user_ids) .
                    "&error=true"
            );
        ...
        foreach ($user_ids as $id) {
            if (!current_user_can("delete_user", $id)) {
                wp_die(
                    __("Sorry, you are not allowed to delete that user."),
                    403
                );
            }
    ...
    case "delete":
        ...
        if (!current_user_can("delete_users")) {
            $errors = new WP_Error(
                "edit_users",
                __("Sorry, you are not allowed to delete users.")
            );
        }
         ...
         */
        $users_have_content = (bool) apply_filters(
  ...
  ?>
    ...
    case "remove":
        ...
        if (!current_user_can("remove_users")) {
            $error = new WP_Error(
                "edit_users",
                __("Sorry, you are not allowed to remove users.")
            );
        }
    ...
    default:
        ...
        if (isset($_GET["update"])):
            switch ($_GET["update"]) {
                case "del":
                case "del_many":
                    $delete_count = isset($_GET["delete_count"])
                        ? (int) $_GET["delete_count"]
                        : 0;
                    if (1 === $delete_count) {
  ...
  ?>
		...
		<?php if (!empty($_REQUEST["role"])) { ?>
			<input type="hidden" name="role" value="<?php echo esc_attr(
       $_REQUEST["role"]
   ); ?>" />
		<?php } ?>
