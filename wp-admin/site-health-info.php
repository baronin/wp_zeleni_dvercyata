...
<div class="health-check-body health-check-debug-tab hide-if-no-js">
 ...
 ?>
	...
	<p>
		<?php /* translators: %s: URL to Site Health Status page. */
  printf(
      __(
	...
	<div class="site-health-copy-buttons">
		<div class="copy-button-wrapper">
			<button type="button" class="button copy-button" data-clipboard-text="<?php echo esc_attr(
       WP_Debug_Data::format($info, "debug")
   ); ?>">
      ...
      if (!isset($details["fields"]) || empty($details["fields"])) {
          continue;
			...
			<h3 class="health-check-accordion-heading">
				<button aria-expanded="false" class="health-check-accordion-trigger" aria-controls="health-check-accordion-block-<?php echo esc_attr(
        $section
    ); ?>" type="button">
					<span class="title">
          ...
          printf("(%s)", number_format_i18n(count($details["fields"])));
			...
			<div id="health-check-accordion-block-<?php echo esc_attr(
       $section
        ...
        printf("<p>%s</p>", $details["description"]);
         ...
         if (in_array($field_name, $sizes_fields, true)) {
             printf(
                 '<tr><th scope="row">%s</th><td class="%s">%s</td></tr>',
