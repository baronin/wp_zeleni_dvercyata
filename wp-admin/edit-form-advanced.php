...
if (isset($_GET["message"])) {
    ...
    if (isset($messages[$post_type][$_GET["message"]])) {
        ...
        !isset($messages[$post_type]) &&
        isset($messages["post"][$_GET["message"]])
    ) {
...
if (
    ...
    mysql2date("U", $autosave->post_modified_gmt, false) >
        mysql2date("U", $post->post_modified_gmt, false)
) {
    foreach (
        _wp_post_revision_fields($post)
        as $autosave_field => $_autosave_field
    ) {
        if (
            normalize_whitespace($autosave->$autosave_field) !==
            normalize_whitespace($post->$autosave_field)
        ) {
            $notice = sprintf(
...
if ("post" === $post_type) {
    ...
    if (
        ...
        post_type_supports("post", "post-formats")
    ) {
        $publish_box .=
            "<li>" .
            __(
    ...
    if (
        ...
        post_type_supports("post", "thumbnail")
    ) {
        $publish_box .=
            "<li>" .
            sprintf(
...
wp_admin_notice($connection_lost_message, [
...
get_current_screen()->get_columns()
    ? "1"
    : "2"; ?>">
...
<div id="titlewrap">
 ...
 $title_placeholder = apply_filters(
	...
	<input type="text" name="post_title" size="30" value="<?php echo esc_attr(
     $post->post_title
 ); ?>" id="title" spellcheck="true" autocomplete="off" />
	<?php if (post_type_supports($post_type, "editor")) { ?>
		<a href="#content" class="button-secondary screen-reader-text skiplink" onclick="if (tinymce) { tinymce.execCommand( 'mceFocus', false, 'content' ); }"><?php esc_html_e(
      "Skip to Editor"
  ); ?></a>
		<?php } ?>
</div>
...
<div class="inside">
	<?php if ($viewable):
     $sample_permalink_html = $post_type_object->public
         ? get_sample_permalink_html($post->ID)
         : "";
     ...
     if (has_filter("pre_get_shortlink") || has_filter("get_shortlink")) {
         ...
         if (
             !empty($shortlink) &&
             $shortlink !== $permalink &&
             home_url("?page_id=" . $post->ID) !== $permalink
         ) {
             $sample_permalink_html .=
                 '<input id="shortlink" type="hidden" value="' .
                 esc_attr($shortlink) .
                 '" />' .
     ...
     if (
         ...
         $has_sample_permalink =
             $sample_permalink_html && "auto-draft" !== $post->post_status; ?>
	<div id="edit-slug-box" class="hide-if-no-js">
			<?php if ($has_sample_permalink) {
...
<?php if (post_type_supports($post_type, "comments")) {
    wp_comment_reply();
} ?>
