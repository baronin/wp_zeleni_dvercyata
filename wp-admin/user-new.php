...
if (isset($_REQUEST["action"]) && "adduser" === $_REQUEST["action"]) {
    ...
    if (str_contains($user_email, "@")) {
        ...
        if (current_user_can("manage_network_users")) {
            ...
            wp_redirect(
                add_query_arg(["update" => "enter_email"], "user-new.php")
            );
            die();
        }
    }
    ...
    if (!$user_details) {
        wp_redirect(
            add_query_arg(["update" => "does_not_exist"], "user-new.php")
        );
        die();
    }
    ...
    if (is_multisite()) {
        ...
        switch ($_GET["update"]) {
            ...
            case "addexisting":
                ...
                break;
            case "could_not_add":
                $add_user_errors = new WP_Error(
                    "could_not_add",
                    __("That user could not be added to this site.")
                );
                break;
...
if (isset($errors) && is_wp_error($errors)):
    ...
    wp_admin_notice("<ul>" . $error_message . "</ul>", [
...
<table class="form-table" role="presentation">
	<tr class="form-field form-required">
		<th scope="row"><label for="adduser-email"><?php echo esc_html(
      $label
  ); ?></label></th>
		<td><input name="email" type="<?php echo esc_attr(
      $type
  ); ?>" id="adduser-email" class="wp-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
	...
	<tr>
		...
		<td>
			...
			<label for="adduser-noconfirmation"><?php _e(
...
<input name="action" type="hidden" value="createuser" />
 ...
 $new_user_login =
     $creating && isset($_POST["user_login"])
         ? wp_unslash($_POST["user_login"])
         : "";
 $new_user_firstname =
     $creating && isset($_POST["first_name"])
         ? wp_unslash($_POST["first_name"])
         : "";
 $new_user_lastname =
     $creating && isset($_POST["last_name"])
         ? wp_unslash($_POST["last_name"])
         : "";
 $new_user_email =
     $creating && isset($_POST["email"]) ? wp_unslash($_POST["email"]) : "";
 $new_user_uri =
     $creating && isset($_POST["url"]) ? wp_unslash($_POST["url"]) : "";
 $new_user_role =
     $creating && isset($_POST["role"]) ? wp_unslash($_POST["role"]) : "";
 $new_user_send_notification =
     $creating && !isset($_POST["send_user_notification"]) ? false : true;
 $new_user_ignore_pass =
     $creating && isset($_POST["noconfirmation"])
         ? wp_unslash($_POST["noconfirmation"])
         : "";
 ?>
<table class="form-table" role="presentation">
	<tr class="form-field form-required">
		<th scope="row"><label for="user_login"><?php _e(
      "Username"
  ); ?> <span class="description"><?php _e("(required)"); ?></span></label></th>
		<td><input name="user_login" type="text" id="user_login" value="<?php echo esc_attr(
      $new_user_login
  ); ?>" aria-required="true" autocapitalize="none" autocorrect="off" autocomplete="off" maxlength="60" /></td>
	</tr>
	<tr class="form-field form-required">
		<th scope="row"><label for="email"><?php _e(
      "Email"
  ); ?> <span class="description"><?php _e("(required)"); ?></span></label></th>
		<td><input name="email" type="email" id="email" value="<?php echo esc_attr(
      $new_user_email
  ); ?>" /></td>
	</tr>
	<?php if (!is_multisite()) { ?>
	<tr class="form-field">
		...
		<td><input name="first_name" type="text" id="first_name" value="<?php echo esc_attr(
      $new_user_firstname
  ); ?>" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="last_name"><?php _e("Last Name"); ?> </label></th>
		<td><input name="last_name" type="text" id="last_name" value="<?php echo esc_attr(
      $new_user_lastname
  ); ?>" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="url"><?php _e("Website"); ?></label></th>
		<td><input name="url" type="url" id="url" class="code" value="<?php echo esc_attr(
      $new_user_uri
  ); ?>" /></td>
	</tr>
		<?php
  ...
  ?>
	<tr class="form-field form-required user-pass1-wrap">
		...
		<td>
			...
			<button type="button" class="button wp-generate-pw hide-if-no-js"><?php _e(
       "Generate password"
   ); ?></button>
			<div class="wp-pwd">
				<?php $initial_password = wp_generate_password(24); ?>
				<div class="password-input-wrapper">
					<input type="password" name="pass1" id="pass1" class="regular-text" autocomplete="new-password" spellcheck="false" data-reveal="1" data-pw="<?php echo esc_attr(
         $initial_password
	...
	<tr>
		...
		<td>
			<input type="checkbox" name="send_user_notification" id="send_user_notification" value="1" <?php checked(
       $new_user_send_notification
   ); ?> />
			<label for="send_user_notification"><?php _e(
   ...
   ?>
	...
	<tr>
		<th scope="row"><?php _e("Skip Confirmation Email"); ?></th>
		<td>
			<input type="checkbox" name="noconfirmation" id="noconfirmation" value="1" <?php checked(
       $new_user_ignore_pass
   ); ?> />
			<label for="noconfirmation"><?php _e(
       "Add the user without sending an email that requires their confirmation"
   ); ?></label>
		</td>
	</tr>
 ...
 do_action("user_new_form", "add-new-user"); ?>
	...
	<?php submit_button(__("Add User"), "primary", "createuser", true, [
     "id" => "createusersub",
 ]); ?>
