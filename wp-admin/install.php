...
function display_setup_form($error = null)
{
    ...
    $user_table =
        $wpdb->get_var(
            $wpdb->prepare("SHOW TABLES LIKE %s", $wpdb->esc_like($wpdb->users))
        ) !== null;
...
<form id="setup" method="post" action="install.php?step=2" novalidate="novalidate">
	<table class="form-table" role="presentation">
		<tr>
			<th scope="row"><label for="weblog_title"><?php _e(
       "Site Title"
   ); ?></label></th>
       ...
       echo '<input name="user_name" type="hidden" value="admin" />';
        ...
        ?>
				<input name="user_name" type="text" id="user_login" size="25" aria-describedby="user-name-desc" value="<?php echo esc_attr(
        sanitize_user($user_name, true)
    ); ?>" />
				<p id="user-name-desc"><?php _e(
		...
		<tr class="form-field form-required user-pass1-wrap">
			...
			<td>
				<div class="wp-pwd">
					<?php $initial_password = isset($_POST["admin_password"])
         ? stripslashes($_POST["admin_password"])
         : wp_generate_password(18); ?>
					<div class="password-input-wrapper">
						<input type="password" name="admin_password" id="pass1" class="regular-text" autocomplete="new-password" spellcheck="false" data-reveal="1" data-pw="<?php echo esc_attr(
          $initial_password
      ); ?>" aria-describedby="pass-strength-result admin-password-desc" />
						<div id="pass-strength-result" aria-live="polite"></div>
					</div>
					<button type="button" class="button wp-hide-pw hide-if-no-js" data-start-masked="<?php echo (int) isset(
         $_POST["admin_password"]
     ); ?>" data-toggle="0" aria-label="<?php esc_attr_e("Hide password"); ?>">
      ...
      ?>
		...
		<tr>
			...
			<td><input name="admin_email" type="email" id="admin_email" size="25" aria-describedby="admin-email-desc" value="<?php echo esc_attr(
       $admin_email
   ); ?>" />
			<p id="admin-email-desc"><?php _e(
       ...
       ? _e("Site visibility")
       : _e("Search engine visibility"); ?></th>
			<td>
				<fieldset>
          ...
          $blog_public
						...
						<label for="blog-public"><?php _e(
          "Allow search engines to index this site"
      ); ?></label><br />
						<input id="blog-norobots" type="radio" name="blog_public"  aria-describedby="public-desc" value="0" <?php checked(
          0,
          $blog_public
      ); ?> />
          ...
          "Discourage search engines from indexing this site"
						...
						<p id="public-desc" class="description"><?php _e(
          "Note: Discouraging search engines does not block access to your site &mdash; it is up to search engines to honor your request."
						...
						<?php /** This action is documented in wp-admin/options-reading.php */
      do_action("blog_privacy_selector");} else { ?>
						<label for="blog_public"><input name="blog_public" type="checkbox" id="blog_public" aria-describedby="privacy-desc" value="0" <?php checked(
          0,
          $blog_public
      ); ?> />
						<?php _e("Discourage search engines from indexing this site"); ?></label>
						<p id="privacy-desc" class="description"><?php _e(
	...
	<input type="hidden" name="language" value="<?php echo isset(
     $_REQUEST["language"]
 )
...
$mysql_compat =
    version_compare($mysql_version, $required_mysql_version, ">=") ||
    ...
    if (count($missing_extensions) > 0) {
        ...
        die(
            "<h1>" .
                __("Requirements Not Met") .
                "</h1><p>" .
                implode("</p><p>", $missing_extensions) .
                "</p></body></html>"
        );
    }
}
  ...
  break;
    ...
    case 2:
        ...
        $admin_email = isset($_POST["admin_email"])
            ? trim(wp_unslash($_POST["admin_email"]))
            : "";
        $public = isset($_POST["blog_public"])
            ? (int) $_POST["blog_public"]
            : 1;
        ...
        if (empty($user_name)) {
            ...
            display_setup_form(
                __("The username you provided has invalid characters.")
            );
            $error = true;
        } elseif ($admin_password !== $admin_password_check) {
            // TODO: Poka-yoke.
            display_setup_form(
                __("Your passwords do not match. Please try again.")
            );
            $error = true;
...
<p class="step"><a href="<?php echo esc_url(wp_login_url()); ?>"><?php _e(
    "Log In"
); ?></a></p>
