...
function populate_options(array $options = [])
{
    ...
    // 3.3.0
    if (!is_multisite()) {
        $defaults["initial_db_version"] =
            !empty($wp_current_db_version) &&
    ...
    $wpdb->query(
        "DELETE FROM $wpdb->options WHERE option_name REGEXP '^rss_[0-9a-f]{32}(_ts)?$'"
    );
...
function populate_network(
    ...
    if ("" === $site_name) {
        $errors->add(
            "empty_sitename",
            __("You must provide a name for your network of sites.")
        );
    }
        ...
        if (!$vhost_ok) {
            ...
            if (!empty($errstr)) {
                ...
                $msg .=
                    " " .
                    sprintf(
                        __("This resulted in an error message: %s"),
                        "<code>" . $errstr . "</code>"
                    );
            }
            $msg .= "</p>";
...
function populate_network_meta($network_id, array $meta = [])
{
    ...
    $subdomain_install = isset($meta["subdomain_install"])
        ? (int) $meta["subdomain_install"]
        : 0;
...
--The Team @ SITE_NAME'
    ...
    $sitemeta = [
        ...
        "add_new_users" => "0",
        "upload_space_check_disabled" => is_multisite()
            ? get_site_option("upload_space_check_disabled")
            : "1",
        "subdomain_install" => $subdomain_install,
        "ms_files_rewriting" => is_multisite()
    ...
    $wpdb->query(
        "INSERT INTO $wpdb->sitemeta ( site_id, meta_key, meta_value ) VALUES " .
            $insert
    ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
}
...
function populate_site_meta($site_id, array $meta = [])
{
    ...
    $wpdb->query(
        "INSERT INTO $wpdb->blogmeta ( blog_id, meta_key, meta_value ) VALUES " .
            $insert
    ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
