...
if (!function_exists("wp_install")):
    ...
    function wp_install(
        ...
        $is_public,
        $deprecated = "",
        #[\SensitiveParameter] $user_password = "",
        $language = ""
    ) {
        if (!empty($deprecated)) {
        ...
        wp_new_blog_notification(
            ...
            $email_password
                ? $user_password
                : __("The password you chose during installation.")
        );
...
if (!function_exists("wp_install_defaults")):
    ...
    function wp_install_defaults($user_id)
    {
        ...
        $cat_slug = sanitize_title(
            _x("Uncategorized", "Default category slug")
        );
        ...
        if (is_multisite()) {
            ...
            $first_comment_url = get_site_option(
                "first_comment_url",
                network_home_url()
            );
        ...
        $first_comment_author = !empty($first_comment_author)
            ? $first_comment_author
            : __("A WordPress Commenter");
        $first_comment_email = !empty($first_comment_email)
            ? $first_comment_email
            : "wapuu@wordpress.example";
        $first_comment_url = !empty($first_comment_url)
            ? $first_comment_url
            : esc_url(__("https://wordpress.org/"));
        $first_comment = !empty($first_comment)
            ? $first_comment
            : sprintf(
...
Commenter avatars come from <a href="%s">Gravatar</a>.'
        ...
        if (is_multisite()) {
            ...
            $privacy_policy_content = get_site_option(
        ...
        if (!is_multisite()) {
            ...
            !metadata_exists("user", $user_id, "show_welcome_panel")
        ) {
...
function wp_install_maybe_enable_pretty_permalinks()
{
    ...
    foreach ((array) $permalink_structures as $permalink_structure) {
        ...
        $pretty_permalinks =
            $x_pingback_header &&
            get_bloginfo("pingback_url") === $x_pingback_header;
...
function upgrade_100()
{
    ...
    if ($posts) {
        foreach ($posts as $post) {
            if ("" === $post->post_name) {
                ...
                $wpdb->query(
                    $wpdb->prepare(
                        "UPDATE $wpdb->posts SET post_name = %s WHERE ID = %d",
                        $newtitle,
                        $post->ID
                    )
                );
    ...
    $done_ids = $wpdb->get_results(
        "SELECT DISTINCT post_id FROM $wpdb->post2cat"
    );
    if ($done_ids):
        $done_posts = [];
    ...
    if ($allposts):
        foreach ($allposts as $post) {
            ...
            if (!$cat && 0 !== (int) $post->post_category) {
...
function upgrade_110()
{
    ...
    $got_gmt_fields =
        "0000-00-00 00:00:00" !==
        $wpdb->get_var("SELECT MAX(post_date_gmt) FROM $wpdb->posts");
    ...
    if (!$got_gmt_fields) {
        ...
        $wpdb->query(
            "UPDATE $wpdb->posts SET post_date_gmt = DATE_ADD(post_date, INTERVAL '$add_hours:$add_minutes' HOUR_MINUTE)"
        );
        $wpdb->query("UPDATE $wpdb->posts SET post_modified = post_date");
        $wpdb->query(
            "UPDATE $wpdb->posts SET post_modified_gmt = DATE_ADD(post_modified, INTERVAL '$add_hours:$add_minutes' HOUR_MINUTE) WHERE post_modified != '0000-00-00 00:00:00'"
        );
        $wpdb->query(
            "UPDATE $wpdb->comments SET comment_date_gmt = DATE_ADD(comment_date, INTERVAL '$add_hours:$add_minutes' HOUR_MINUTE)"
        );
        $wpdb->query(
            "UPDATE $wpdb->users SET user_registered = DATE_ADD(user_registered, INTERVAL '$add_hours:$add_minutes' HOUR_MINUTE)"
        );
    }
}
...
function upgrade_130()
{
    ...
    if ($posts) {
        foreach ($posts as $post) {
            ...
            $wpdb->update(
                $wpdb->posts,
                compact("post_title", "post_content", "post_excerpt", "guid"),
                ["ID" => $post->ID]
            );
        }
    }
    ...
    if ($comments) {
        foreach ($comments as $comment) {
            ...
            $wpdb->update(
                $wpdb->comments,
                compact("comment_content", "comment_author"),
    ...
    if ($links) {
        foreach ($links as $link) {
            ...
            $wpdb->update(
                $wpdb->links,
                compact("link_name", "link_description"),
    ...
    $wpdb->query(
        "DROP TABLE IF EXISTS " . $wpdb->prefix . "optiongroup_options"
    );

    // Update comments table to use comment_type.
    $wpdb->query(
        "UPDATE $wpdb->comments SET comment_type='trackback', comment_content = REPLACE(comment_content, '<trackback />', '') WHERE comment_content LIKE '<trackback />%'"
    );
    $wpdb->query(
        "UPDATE $wpdb->comments SET comment_type='pingback', comment_content = REPLACE(comment_content, '<pingback />', '') WHERE comment_content LIKE '<pingback />%'"
    );

    // Some versions have multiple duplicate option_name rows with the same values.
    $options = $wpdb->get_results(
        "SELECT option_name, COUNT(option_name) AS dupes FROM `$wpdb->options` GROUP BY option_name"
    );
    foreach ($options as $option) {
        if ($option->dupes > 1) {
            // Could this be done in the query?
            $limit = $option->dupes - 1;
            $dupe_ids = $wpdb->get_col(
                $wpdb->prepare(
            ...
            if ($dupe_ids) {
                ...
                $wpdb->query(
                    "DELETE FROM $wpdb->options WHERE option_id IN ($dupe_ids)"
                );
            }
        }
    }
...
function upgrade_160()
{
    ...
    foreach ($users as $user):
        ...
        if (!empty($user->user_description)) {
            update_user_meta(
                $user->ID,
                "description",
                wp_slash($user->user_description)
            );
        }
    ...
    $comments = $wpdb->get_results(
        "SELECT comment_post_ID, COUNT(*) as c FROM $wpdb->comments WHERE comment_approved = '1' GROUP BY comment_post_ID"
    );
...
function upgrade_210()
{
    ...
    if ($wp_current_db_version < 3506) {
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) {
                ...
                $wpdb->query(
                    $wpdb->prepare(
                        "UPDATE $wpdb->posts SET post_status = %s, post_type = %s WHERE ID = %d",
                        $status,
                        $type,
                        $post->ID
                    )
    ...
    if ($wp_current_db_version < 3531) {
        ...
        $wpdb->query(
            "UPDATE $wpdb->posts SET post_status = 'future' WHERE post_status = 'publish' AND post_date_gmt > '$now'"
        );
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) {
                wp_schedule_single_event(
                    mysql2date("U", $post->post_date, false),
                    "publish_future_post",
                    [$post->ID]
                );
            }
...
function upgrade_230()
{
    ...
    foreach ($categories as $category) {
        ...
        if (!empty($category->category_count)) {
            ...
            $wpdb->query(
                $wpdb->prepare(
                    "INSERT INTO $wpdb->term_taxonomy (term_id, taxonomy, description, parent, count) VALUES ( %d, %s, %s, %d, %d)",
                    $term_id,
                    $taxonomy,
                    $description,
                    $parent,
        ...
        if (!empty($category->tag_count)) {
            ...
            $wpdb->insert(
                $wpdb->term_taxonomy,
                compact("term_id", "taxonomy", "description", "parent", "count")
            );
            $tt_ids[$term_id][$taxonomy] = (int) $wpdb->insert_id;
        }
    ...
    $posts = $wpdb->get_results(
        "SELECT $select FROM $wpdb->post2cat GROUP BY post_id, category_id"
    );
    foreach ($posts as $post) {
    ...
    if ($wp_current_db_version < 4772) {
        ...
        $wpdb->query(
            "DROP TABLE IF EXISTS " . $wpdb->prefix . "linkcategories"
        );
    }
...
function upgrade_old_slugs()
{
    ...
    global $wpdb;
    $wpdb->query(
        "UPDATE $wpdb->postmeta SET meta_key = '_wp_old_slug' WHERE meta_key = 'old_slug'"
    );
}
...
function upgrade_270()
{
    ...
    if ($wp_current_db_version < 8921) {
        $wpdb->query(
            "UPDATE $wpdb->posts SET post_date = post_modified WHERE post_date = '0000-00-00 00:00:00'"
        );
    }
}
    ...
    if (wp_should_upgrade_global_tables()) {
        ...
        $wpdb->query(
            $wpdb->prepare(
                ...
                $prefix .
                    "%" .
                    $wpdb->esc_like("manage-") .
                    "%" .
                    $wpdb->esc_like("-columns-hidden") .
                    "%",
...
function upgrade_330()
{
    ...
    if ($wp_current_db_version < 19061 && wp_should_upgrade_global_tables()) {
        $wpdb->query(
            "DELETE FROM $wpdb->usermeta WHERE meta_key IN ('show_admin_bar_admin', 'plugins_last_view')"
        );
    }
    ...
    if (
        isset($sidebars_widgets["wp_inactive_widgets"]) ||
...
function upgrade_340()
{
    ...
    if ($wp_current_db_version < 20022 && wp_should_upgrade_global_tables()) {
        $wpdb->query(
            "DELETE FROM $wpdb->usermeta WHERE meta_key = 'themes_last_view'"
        );
    }
...
function upgrade_350()
{
    ...
    if (
        $wp_current_db_version < 22006 &&
        $wpdb->get_var("SELECT link_id FROM $wpdb->links LIMIT 1")
    ) {
    ...
    if ($wp_current_db_version < 21811 && wp_should_upgrade_global_tables()) {
        ...
        foreach (array_merge(get_post_types(), get_taxonomies()) as $name) {
            if (str_contains($name, "-")) {
                $meta_keys[] =
                    "edit_" . str_replace("-", "_", $name) . "_per_page";
            }
        }
        if ($meta_keys) {
            $meta_keys = implode("', '", $meta_keys);
            $wpdb->query(
                "DELETE FROM $wpdb->usermeta WHERE meta_key IN ('$meta_keys')"
            );
        }
    }
...
function upgrade_440()
{
    ...
    if ($wp_current_db_version < 34030) {
        $wpdb->query(
            "ALTER TABLE {$wpdb->options} MODIFY option_name VARCHAR(191)"
        );
    }
...
function upgrade_450()
{
    ...
    if ($wp_current_db_version < 36679 && is_multisite()) {
        $wpdb->query(
            "DELETE FROM $wpdb->options WHERE option_name REGEXP '^[0-9]+_new_email$'"
        );
    }
...
function upgrade_560()
{
    ...
    if ($wp_current_db_version < 49572) {
         ...
         */
        $post_category_exists = $wpdb->get_var(
            "SHOW COLUMNS FROM $wpdb->posts LIKE 'post_category'"
        );
        if (!is_null($post_category_exists)) {
            $wpdb->query(
                "ALTER TABLE $wpdb->posts DROP COLUMN `post_category`"
            );
        }
    ...
    if ($wp_current_db_version < 49752) {
        ...
        if (!empty($results)) {
            ...
            update_network_option(
                $network_id,
                WP_Application_Passwords::OPTION_KEY_IN_USE,
                1
            );
        }
    }
...
function upgrade_network()
{
    ...
    if ($wp_current_db_version < 11549) {
        ...
        if ($wpmu_sitewide_plugins) {
            if (!$active_sitewide_plugins) {
                ...
                $sitewide_plugins = array_merge(
                    (array) $active_sitewide_plugins,
                    (array) $wpmu_sitewide_plugins
                );
            }
    ...
    if ($wp_current_db_version < 31351 && "utf8mb4" === $wpdb->charset) {
        if (wp_should_upgrade_global_tables()) {
            $wpdb->query(
                "ALTER TABLE $wpdb->usermeta DROP INDEX meta_key, ADD INDEX meta_key(meta_key(191))"
            );
            $wpdb->query(
                "ALTER TABLE $wpdb->site DROP INDEX domain, ADD INDEX domain(domain(140),path(51))"
            );
            $wpdb->query(
                "ALTER TABLE $wpdb->sitemeta DROP INDEX meta_key, ADD INDEX meta_key(meta_key(191))"
            );
            $wpdb->query(
                "ALTER TABLE $wpdb->signups DROP INDEX domain_path, ADD INDEX domain_path(domain(140),path(51))"
            );
            ...
            if (
                !$wpdb->get_var(
                    "SHOW TABLES LIKE '{$tables["sitecategories"]}'"
                )
            ) {
...
function get_alloptions_110()
{
    ...
    if ($options) {
        foreach ($options as $option) {
            if (
                "siteurl" === $option->option_name ||
                "home" === $option->option_name ||
                "category_base" === $option->option_name
            ) {
                $option->option_value = untrailingslashit(
                    $option->option_value
                );
            }
            $all_options->{$option->option_name} = stripslashes(
                $option->option_value
            );
        }
    }
...
function __get_option($setting)
{
    ...
    if (
        in_array(
            $setting,
            ["siteurl", "home", "category_base", "tag_base"],
            true
        )
    ) {
...
function dbDelta($queries = "", $execute = true)
{
    ...
    foreach ($cqueries as $table => $qry) {
        ...
        if (
            in_array($table, $global_tables, true) &&
                ...
                case "spatial":
                    ...
                    $index_name =
                        "PRIMARY KEY" === $index_type
                            ? ""
                            : "`" .
                                strtolower($index_matches["index_name"]) .
                                "`";

                    // Parse the columns. Multiple columns are separated by a comma.
                    $index_columns = array_map(
                        "trim",
                        explode(",", $index_matches["index_columns"])
                    );
                    ...
                    foreach ($index_columns as $id => &$index_column) {
                        ...
                        $index_column =
                            "`" . $index_column_matches["column_name"] . "`";
        ...
        foreach ($tablefields as $tablefield) {
            ...
            if (array_key_exists($tablefield_field_lowercased, $cfields)) {
                ...
                preg_match(
                    "|`?" . $tablefield->Field . "`? ([^ ]*( unsigned)?)|i",
                    $cfields[$tablefield_field_lowercased],
                    $matches
                );
                $fieldtype = $matches[1];
                ...
                if ($tablefield->Type !== $fieldtype) {
                    $do_change = true;
                    if (
                        in_array($fieldtype_lowercased, $text_fields, true) &&
                        in_array(
                            $tablefield_type_lowercased,
                            $text_fields,
                            true
                    ...
                    if (
                        in_array($fieldtype_lowercased, $blob_fields, true) &&
                        in_array(
                            $tablefield_type_lowercased,
                            $blob_fields,
                            true
                    ...
                    if (
                        in_array($fieldtype_base, $int_fields, true) &&
                        in_array($tablefield_type_base, $int_fields, true) &&
                        $fieldtype_without_parentheses ===
                            $tablefield_type_without_parentheses
                    ) {
                        /*
                         ...
                         */
                        if (
                            version_compare($db_version, "8.0.17", ">=") &&
        ...
        if ($tableindices) {
            ...
            foreach ($tableindices as $tableindex) {
                ...
                $index_ary[$keyname]["unique"] =
                    "0" === $tableindex->Non_unique ? true : false;
...
function make_site_theme_from_default($theme_name, $template)
{
    ...
    if ($images_dir) {
        while (($image = readdir($images_dir)) !== false) {
            ...
            if (
                !copy("$default_dir/images/$image", "$site_dir/images/$image")
            ) {
                return;
            }
...
function pre_schema_upgrade()
{
    ...
    if ($wp_current_db_version < 11557) {
        ...
        $wpdb->query(
            "DELETE o1 FROM $wpdb->options AS o1 JOIN $wpdb->options AS o2 USING (`option_name`) WHERE o2.option_id > o1.option_id"
        );

        // Drop the old primary key and add the new.
        $wpdb->query(
            "ALTER TABLE $wpdb->options DROP PRIMARY KEY, ADD PRIMARY KEY(option_id)"
        );
    ...
    if (
        ...
        if ($wp_current_db_version < 25179) {
            ...
            $wpdb->query(
                "ALTER TABLE $wpdb->signups ADD signup_id BIGINT(20) NOT NULL AUTO_INCREMENT PRIMARY KEY FIRST"
            );
    ...
    if ($wp_current_db_version < 31351) {
        ...
        $wpdb->query(
            "ALTER TABLE $wpdb->terms DROP INDEX slug, ADD INDEX slug(slug(191))"
        );
        $wpdb->query(
            "ALTER TABLE $wpdb->terms DROP INDEX name, ADD INDEX name(name(191))"
        );
        $wpdb->query(
            "ALTER TABLE $wpdb->commentmeta DROP INDEX meta_key, ADD INDEX meta_key(meta_key(191))"
        );
        $wpdb->query(
            "ALTER TABLE $wpdb->postmeta DROP INDEX meta_key, ADD INDEX meta_key(meta_key(191))"
        );
        $wpdb->query(
            "ALTER TABLE $wpdb->posts DROP INDEX post_name, ADD INDEX post_name(post_name(191))"
        );
    }
    ...
    if ($wp_current_db_version < 34978) {
        ...
        if (
            $wpdb->get_var("SHOW TABLES LIKE '{$wpdb->termmeta}'") &&
            $wpdb->get_results(
                "SHOW INDEX FROM {$wpdb->termmeta} WHERE Column_name = 'meta_key'"
            )
        ) {
            $wpdb->query(
                "ALTER TABLE $wpdb->termmeta DROP INDEX meta_key, ADD INDEX meta_key(meta_key(191))"
            );
