...
function _wp_ajax_menu_quick_search($request = [])
{
    ...
    $object_type = isset($request["object_type"])
        ? $request["object_type"]
        : "";
    $query = isset($request["q"]) ? $request["q"] : "";
    $response_format = isset($request["response-format"])
        ? $request["response-format"]
        : "";

    if (
        !$response_format ||
        !in_array($response_format, ["json", "markup"], true)
    ) {
    ...
    if ("get-post-item" === $type) {
        ...
        if ("posttype" === $matches[1] && get_post_type_object($matches[2])) {
            ...
            if (isset($post_type_obj->_default_query)) {
                $args = array_merge(
                    $args,
                    (array) $post_type_obj->_default_query
                );
            }
...
function wp_initial_nav_menu_meta_boxes()
{
    ...
    if (
        get_user_option("metaboxhidden_nav-menus") !== false ||
    ...
    foreach (array_keys($wp_meta_boxes["nav-menus"]) as $context) {
        foreach (
            array_keys($wp_meta_boxes["nav-menus"][$context])
            as $priority
        ) {
...
function wp_nav_menu_item_link_meta_box()
{
    ...
    $_nav_menu_placeholder =
        0 > $_nav_menu_placeholder ? $_nav_menu_placeholder - 1 : -1;
    ?>
		...
		<p class="button-controls wp-clearfix">
			<span class="add-to-menu">
				<input id="submit-customlinkdiv" name="add-custom-menu-item"
					...
					class="button submit-add-to-menu right" value="<?php esc_attr_e(
         "Add to Menu"
     ); ?>"
				/>
...
function wp_nav_menu_item_post_type_meta_box($data_object, $box)
{
    ...
    $per_page = 50;
    $pagenum =
        isset($_REQUEST[$tab_name]) && isset($_REQUEST["paged"])
    ...
    $page_links = paginate_links([
        ...
        "prev_text" =>
            '<span aria-label="' .
            esc_attr__("Previous page") .
            '">' .
            __("&laquo;") .
            "</span>",
        "next_text" =>
            '<span aria-label="' .
            esc_attr__("Next page") .
            '">' .
            __("&raquo;") .
            "</span>",
        /* translators: Hidden accessibility text. */
        "before_page_number" =>
            '<span class="screen-reader-text">' . __("Page") . "</span> ",
        "total" => $num_pages,
        "current" => $pagenum,
    ]);
    ...
    if (
        isset($_REQUEST[$tab_name]) &&
        in_array($_REQUEST[$tab_name], ["all", "search"], true)
    ) {
	...
	<div id="<?php echo esc_attr(
     "posttype-{$post_type_name}"
		...
		<ul id="<?php echo esc_attr(
      "posttype-{$post_type_name}-tabs"
  ); ?>" class="posttype-tabs add-menu-item-tabs">
			<li <?php echo "most-recent" === $current_tab ? ' class="tabs"' : ""; ?>>
				<a class="nav-tab-link"
					data-type="<?php echo esc_attr(
         "tabs-panel-posttype-{$post_type_name}-most-recent"
     ); ?>"
					href="<?php echo esc_url(
         $most_recent_url . "#tabs-panel-posttype-{$post_type_name}-most-recent"
     ); ?>"
				>
					<?php _e("Most Recent"); ?>
				</a>
			...
			<li <?php echo "search" === $current_tab ? ' class="tabs"' : ""; ?>>
				<a class="nav-tab-link"
					data-type="<?php echo esc_attr(
         "tabs-panel-posttype-{$post_type_name}-search"
     ); ?>"
					href="<?php echo esc_url(
         $search_url . "#tabs-panel-posttype-{$post_type_name}-search"
     ); ?>"
				>
					<?php _e("Search"); ?>
				</a>
      ...
      "tabs-panel-posttype-{$post_type_name}-most-recent"
			...
			class="tabs-panel <?php echo "most-recent" === $current_tab
       ? "tabs-panel-active"
       : "tabs-panel-inactive"; ?>"
    ...
    ?>
      ...
      "tabs-panel-posttype-{$post_type_name}-search"
			...
			class="tabs-panel <?php echo "search" === $current_tab
       ? "tabs-panel-active"
       : "tabs-panel-inactive"; ?>"
			role="region" aria-label="<?php echo esc_attr(
       $post_type->labels->search_items
   ); ?>" tabindex="0"
		>
			<?php if (isset($_REQUEST["quick-search-posttype-{$post_type_name}"])) {
       $searched = esc_attr(
    ...
    ?>
		...
		<div id="<?php echo esc_attr("{$post_type_name}-all"); ?>"
			class="tabs-panel tabs-panel-view-all <?php echo "all" === $current_tab
       ? "tabs-panel-active"
       : "tabs-panel-inactive"; ?>"
			role="region" aria-label="<?php echo esc_attr(
    ...
    if ($post_type->has_archive) {
        $_nav_menu_placeholder =
            0 > $_nav_menu_placeholder ? (int) $_nav_menu_placeholder - 1 : -1;
        array_unshift(
            $posts,
            (object) [
                "ID" => 0,
    ...
    ?>
		...
		<p class="button-controls wp-clearfix" data-items-type="<?php echo esc_attr(
      "posttype-{$post_type_name}"
  ); ?>">
			<span class="list-controls hide-if-no-js">
				<input type="checkbox"<?php wp_nav_menu_disabled_check(
        ...
        $tab_name
         ...
         "Add to Menu"
					...
					name="add-post-type-menu-item" id="<?php echo esc_attr(
         "submit-posttype-{$post_type_name}"
     ); ?>"
				/>
				<span class="spinner"></span>
			</span>
...
function wp_nav_menu_item_taxonomy_meta_box($data_object, $box)
{
    ...
    if (
        isset($_REQUEST[$tab_name]) &&
        in_array($_REQUEST[$tab_name], ["all", "most-used", "search"], true)
    ) {
        $current_tab = $_REQUEST[$tab_name];
    }
     ...
     "taxonomy-{$taxonomy_name}"
      ...
      "taxonomy-{$taxonomy_name}-tabs"
         ...
         $most_used_url . "#tabs-panel-{$taxonomy_name}-pop"
			...
			<li <?php echo "all" === $current_tab ? ' class="tabs"' : ""; ?>>
				<a class="nav-tab-link"
					...
					href="<?php echo esc_url(
         $view_all_url . "#tabs-panel-{$taxonomy_name}-all"
     ); ?>"
				>
					<?php _e("View All"); ?>
				</a>
			...
			<li <?php echo "search" === $current_tab ? ' class="tabs"' : ""; ?>>
				<a class="nav-tab-link"
					data-type="<?php echo esc_attr(
         "tabs-panel-search-taxonomy-{$taxonomy_name}"
     ); ?>"
					href="<?php echo esc_url(
         $search_url . "#tabs-panel-search-taxonomy-{$taxonomy_name}"
     ); ?>"
				>
					<?php _e("Search"); ?>
				</a>
		...
		<div id="<?php echo esc_attr("tabs-panel-{$taxonomy_name}-pop"); ?>"
			class="tabs-panel <?php echo "most-used" === $current_tab
       ? "tabs-panel-active"
       : "tabs-panel-inactive"; ?>"
			role="region" aria-label="<?php echo esc_attr(
    ...
    ?>
       ...
       : "tabs-panel-inactive"; ?>"
			role="region" aria-label="<?php echo esc_attr(
       $taxonomy->labels->search_items
   ); ?>" tabindex="0">
			<?php if (isset($_REQUEST["quick-search-taxonomy-{$taxonomy_name}"])) {
       $searched = esc_attr(
           $_REQUEST["quick-search-taxonomy-{$taxonomy_name}"]
       );
      ...
      "taxonomy-{$taxonomy_name}"
        ...
        $tab_name
         ...
         "Add to Menu"
					...
					name="add-taxonomy-menu-item" id="<?php echo esc_attr(
         "submit-taxonomy-{$taxonomy_name}"
     ); ?>"
				/>
				<span class="spinner"></span>
			</span>
...
function wp_save_nav_menu_items($menu_id = 0, $menu_data = [])
{
    ...
    if (0 === $menu_id || is_nav_menu($menu_id)) {
        ...
        foreach ((array) $menu_data as $_possible_db_id => $_item_object_data) {
            if (
                ...
                (!isset($_item_object_data["menu-item-type"]) ||
                    ...
                    !(
                        ...
                        !isset($_item_object_data["menu-item-db-id"])
                    ) ||
            ...
            $args = [
                "menu-item-db-id" => isset(
                    $_item_object_data["menu-item-db-id"]
                )
                    ? $_item_object_data["menu-item-db-id"]
                    : "",
                "menu-item-object-id" => isset(
                    $_item_object_data["menu-item-object-id"]
                )
                    ? $_item_object_data["menu-item-object-id"]
                    : "",
                "menu-item-object" => isset(
                    $_item_object_data["menu-item-object"]
                )
                    ? $_item_object_data["menu-item-object"]
                    : "",
                "menu-item-parent-id" => isset(
                    $_item_object_data["menu-item-parent-id"]
                )
                    ? $_item_object_data["menu-item-parent-id"]
                    : "",
                "menu-item-position" => isset(
                    $_item_object_data["menu-item-position"]
                )
                    ? $_item_object_data["menu-item-position"]
                    : "",
                "menu-item-type" => isset($_item_object_data["menu-item-type"])
                    ? $_item_object_data["menu-item-type"]
                    : "",
                "menu-item-title" => isset(
                    $_item_object_data["menu-item-title"]
                )
                    ? $_item_object_data["menu-item-title"]
                    : "",
                "menu-item-url" => isset($_item_object_data["menu-item-url"])
                    ? $_item_object_data["menu-item-url"]
                    : "",
                "menu-item-description" => isset(
                    $_item_object_data["menu-item-description"]
                )
                    ? $_item_object_data["menu-item-description"]
                    : "",
                "menu-item-attr-title" => isset(
                    $_item_object_data["menu-item-attr-title"]
                )
                    ? $_item_object_data["menu-item-attr-title"]
                    : "",
                "menu-item-target" => isset(
                    $_item_object_data["menu-item-target"]
                )
                    ? $_item_object_data["menu-item-target"]
                    : "",
                "menu-item-classes" => isset(
                    $_item_object_data["menu-item-classes"]
                )
                    ? $_item_object_data["menu-item-classes"]
                    : "",
                "menu-item-xfn" => isset($_item_object_data["menu-item-xfn"])
                    ? $_item_object_data["menu-item-xfn"]
                    : "",
            ];

            $items_saved[] = wp_update_nav_menu_item(
                $menu_id,
                $_actual_db_id,
                $args
            );
...
function wp_get_nav_menu_to_edit($menu_id = 0)
{
    ...
    if (is_nav_menu($menu)) {
        ...
        $result .=
            "<p>" . __("Add menu items from the column on the left.") . "</p>";
        $result .= "</div>";
...
function wp_nav_menu_update_menu_items(
    ...
    if (!empty($_POST["menu-item-db-id"])) {
        foreach ((array) $_POST["menu-item-db-id"] as $_key => $k) {
            ...
            if (
                !isset($_POST["menu-item-title"][$_key]) ||
                "" === $_POST["menu-item-title"][$_key]
            ) {
                continue;
            }
            ...
            foreach ($post_fields as $field) {
                $args[$field] = isset($_POST[$field][$_key])
                    ? $_POST[$field][$_key]
                    : "";
            }
    ...
    if ($auto_add) {
        if (
            !in_array($nav_menu_selected_id, $nav_menu_option["auto_add"], true)
        ) {
        ...
        foreach ($data as $post_input_data) {
            ...
            for ($i = count($array_bits) - 1; $i >= 0; $i--) {
                if (count($array_bits) - 1 === $i) {
                    $new_post_data[$array_bits[$i]] = wp_slash(
