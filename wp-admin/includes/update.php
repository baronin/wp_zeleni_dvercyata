...
function find_core_auto_update()
{
    ...
    foreach ($updates->updates as $update) {
        ...
        if (
            !$auto_update ||
            version_compare($update->current, $auto_update->current, ">")
        ) {
            $auto_update = $update;
        }
    }
...
function core_update_footer($msg = "")
{
    ...
    $is_development_version = preg_match(
        "/alpha|beta|RC/",
        wp_get_wp_version()
    );
...
function update_right_now_message()
{
    ...
    $msg .= sprintf(
        '<span id="wp-version">' . $content . "</span>",
        get_bloginfo("version", "display"),
        $theme_name
    );
...
function get_plugin_updates()
{
    ...
    foreach ((array) $all_plugins as $plugin_file => $plugin_data) {
        if (isset($current->response[$plugin_file])) {
            ...
            $upgrade_plugins[$plugin_file]->update =
...
function wp_plugin_update_row($file, $plugin_data)
{
    ...
    if (is_network_admin() || !is_multisite()) {
        if (is_network_admin()) {
            $active_class = is_plugin_active_for_network($file)
                ? " active"
                : "";
        } else {
        ...
        $requires_php = isset($response->requires_php)
            ...
            if ($compatible_php) {
                printf(
                    ...
                    sprintf(
                        ...
                        esc_attr(
                            sprintf(_x("Update %s now", "plugin"), $plugin_name)
                        )
                    )
                );
            } else {
    ...
    if ($compatible_wp && $compatible_php) {
        if (!current_user_can("update_themes")) {
            ...
            printf(
                ...
                $response["new_version"],
                wp_nonce_url(
                    self_admin_url("update.php?action=upgrade-theme&theme=") .
                        $theme_key,
