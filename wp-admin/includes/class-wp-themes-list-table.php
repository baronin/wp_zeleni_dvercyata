...
class WP_Themes_List_Table extends WP_List_Table
{
   ...
   $delete_action = isset($actions["delete"])
       ? '<div class="delete-theme">' . $actions["delete"] . "</div>"
       : "";
   ...
   ?>
    ...
    public function search_theme($theme)
    {
        ...
        foreach ($this->search_terms as $word) {
            ...
            foreach (
                ["Name", "Description", "Author", "AuthorURI"]
                as $header
            ) {
                ...
                if (
                    false !==
                    stripos(
                        strip_tags($theme->display($header, false, true)),
                        $word
                    )
                ) {
                    continue 2;
    ...
    {
        $search_string = isset($_REQUEST["s"])
            ? esc_attr(wp_unslash($_REQUEST["s"]))
            : "";
        ...
        $args = [
            ...
            "total_pages" => !empty($this->_pagination_args["total_pages"])
