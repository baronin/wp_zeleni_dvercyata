...
class Custom_Background
{
    ...
    public function __construct(
        ...
        add_action("wp_ajax_custom-background-add", [
            $this,
            "ajax_background_add",
        ]);

        // Unused since 3.5.0.
        add_action("wp_ajax_set-background-image", [
            $this,
...
<td>
      ...
      if ($bgcolor) {
          $background_styles .=
              "background-color: " . maybe_hash_hex_color($bgcolor) . ";";
      }

      $background_image_thumb = get_background_image();
      if ($background_image_thumb) {
          $background_image_thumb = esc_url(
              set_url_scheme(
                  get_theme_mod(
          ...
          $background_styles .=
              " background-image: url('$background_image_thumb');" .
      ...
      ?>
	<div id="custom-background-image" style="<?php echo $background_styles; ?>"><?php
...
<form method="post">
       ...
       "This will remove the background image. You will not be able to restore any customizations."
		...
		<?php $default_image = get_theme_support(
      "custom-background",
      "default-image"
  ); ?>
	...
	<p>
      ...
      "Or choose an image from your media library:"
		...
		<button id="choose-from-library-link" class="button"
			...
			data-update="<?php esc_attr_e("Set as background"); ?>"><?php _e(
    "Choose Image"
); ?></button>
	</p>
	</form>
...
<div class="background-position-control">
	...
	<div class="button-group">
		...
		<label>
			<input class="ui-helper-hidden-accessible" name="background-position" type="radio" value="<?php echo esc_attr(
       $value
   ); ?>"<?php checked($value, $background_position); ?>>
			<span class="button display-options position"><span class="<?php echo esc_attr(
       $input["icon"]
...
<tr>
<th scope="row"><label for="background-size"><?php _e(
    "Image Size"
); ?></label></th>
<td><fieldset><legend class="screen-reader-text"><span>
			<?php /* translators: Hidden accessibility text. */
...
); ?>><?php _ex("Original", "Original Size"); ?></option>
<option value="contain"<?php selected(
    "contain",
    get_theme_mod(
        "background_size",
        get_theme_support("custom-background", "default-size")
    )
); ?>><?php _e("Fit to Screen"); ?></option>
<option value="cover"<?php selected(
    "cover",
    get_theme_mod(
        "background_size",
        get_theme_support("custom-background", "default-size")
    )
); ?>><?php _e("Fill Screen"); ?></option>
</select>
</fieldset></td>
</tr>
...
<label><input type="checkbox" name="background-repeat" value="repeat"<?php checked(
    "repeat",
    get_theme_mod(
        "background_repeat",
        get_theme_support("custom-background", "default-repeat")
    )
); ?>> <?php _e("Repeat Background Image"); ?></label>
</fieldset></td>
</tr>
...
<label><input name="background-attachment" type="checkbox" value="scroll" <?php checked(
    "scroll",
    get_theme_mod(
        "background_attachment",
        get_theme_support("custom-background", "default-attachment")
    )
); ?>> <?php _e("Scroll with Page"); ?></label>
</fieldset></td>
</tr>
...
<td><fieldset><legend class="screen-reader-text"><span>
  ...
  ?>
<input type="text" name="background-color" id="background-color" value="#<?php echo esc_attr(
