...
class WP_Posts_List_Table extends WP_List_Table
{
    ...
    public function __construct($args = [])
    {
        ...
        if ("post" === $post_type && $sticky_posts) {
            $sticky_posts = implode(
                ", ",
                array_map("absint", (array) $sticky_posts)
            );
    ...
    public function ajax_user_can()
    {
        return current_user_can(
            get_post_type_object($this->screen->post_type)->cap->edit_posts
        );
    }
    ...
    public function prepare_items()
    {
        ...
        if ($this->hierarchical_display) {
            ...
            if (
                ...
                isset($_REQUEST["show_sticky"]) &&
                $_REQUEST["show_sticky"]
            ) {
                $total_items = $this->sticky_posts_count;
            } elseif (
                isset($_GET["author"]) &&
                get_current_user_id() === (int) $_GET["author"]
            ) {
                ...
                foreach (
                    get_post_stati(["show_in_admin_all_list" => false])
                    as $state
                ) {
    ...
    public function no_items()
    {
        if (
            ...
            echo get_post_type_object($this->screen->post_type)->labels
                ->not_found_in_trash;
        } else {
            echo get_post_type_object($this->screen->post_type)->labels
                ->not_found;
    ...
    protected function get_views()
    {
        ...
        if (
            $this->user_posts_count &&
            $this->user_posts_count !== $total_posts
        ) {
            if (
                isset($_GET["author"]) &&
                $current_user_id === (int) $_GET["author"]
            ) {
                $class = "current";
            }
        ...
        $status_links["all"] = [
            ...
            "current" =>
                empty($class) &&
                ($this->is_base_request() || isset($_REQUEST["all_posts"])),
        ];
        ...
        foreach (
            ...
            if (
                !in_array($status_name, $avail_post_stati, true) ||
                empty($num_posts->$status_name)
            ) {
                continue;
            }
            ...
            if (
                isset($_REQUEST["post_status"]) &&
                $status_name === $_REQUEST["post_status"]
            ) {
                $class = "current";
            }
        ...
        if (!empty($this->sticky_posts_count)) {
            ...
            $split =
                1 +
                array_search(
                    isset($status_links["publish"]) ? "publish" : "all",
    ...
    protected function categories_dropdown($post_type)
    {
        ...
        if (is_object_in_taxonomy($post_type, "category")) {
            $dropdown_options = [
                "show_option_all" => get_taxonomy("category")->labels
                    ->all_items,
                "hide_empty" => 0,
                "hierarchical" => 1,
                "show_count" => 0,
    ...
    protected function formats_dropdown($post_type)
    {
        ...
        if (
            !is_object_in_taxonomy($post_type, "post_format") ||
            $this->is_trash
        ) {
            return;
        }
        ...
        $displayed_post_format = isset($_GET["post_format"])
            ? $_GET["post_format"]
            : "";
        ?>
   ...
   _e("Filter by post format"); ?>
       ...
       ""
   ); ?> value=""><?php _e("All formats"); ?></option>
			<?php foreach ($used_post_formats as $used_post_format) {
       ...
       ?>
        ...
        $displayed_post_format,
        $slug
    ); ?> value="<?php echo esc_attr($slug); ?>"><?php echo esc_html(
    $pretty_name
); ?></option>
				<?php
   } ?>
  ...
  if (
      ...
      $this->has_items() &&
      current_user_can(
          get_post_type_object($this->screen->post_type)->cap->edit_others_posts
      )
  ) {
  ...
  do_action("manage_posts_extra_tablenav", $which);
        ...
        if ("page" === $post_type) {
            ...
            $sortables = [
                ...
                "comments" => [
                    ...
                    false,
                    __("Comments"),
                    __("Table ordered by Comments."),
                ],
                "date" => [
                    "date",
                    true,
                    __("Date"),
                    __("Table ordered by Date."),
                ],
            ];
        } else {
            $sortables = [
    ...
    public function display_rows($posts = [], $level = 0)
    {
        ...
        if ($this->hierarchical_display) {
            $this->_display_rows_hierarchical(
                $posts,
                $this->get_pagenum(),
                $per_page
            );
    ...
    public function column_title($post)
    {
        ...
        if (isset($parent_name)) {
            ...
            echo " | " .
                $post_type_object->labels->parent_item_colon .
                " " .
        ...
        if (
            ...
            if (post_password_required($post)) {
                echo '<span class="protected-post-excerpt">' .
        ...
        $quick_edit_enabled = apply_filters(
            ...
            if (is_array($terms)) {
                ...
                foreach ($terms as $t) {
                    ...
                    if ($taxonomy_object->query_var) {
                        $posts_in_term_qv[$taxonomy_object->query_var] =
                            $t->slug;
                    } else {
    ...
    protected function handle_row_actions($item, $column_name, $primary)
    {
        ...
        if ("wp_block" === $post->post_type) {
            $actions["export"] = sprintf(
                ...
                /* translators: %s: Post title. */
                esc_attr(
                    sprintf(__("Export &#8220;%s&#8221; as JSON"), $title)
                ),
                __("Export as JSON")
            );
        }
  ...
  while ($bulk < 2):
      ...
      ?>
			<tr id="<?php echo $bulk
       ? "bulk-edit"
       : "inline-edit"; ?>" class="<?php echo $classes; ?>" style="display: none">
			<td colspan="<?php echo $this->get_column_count(); ?>" class="colspanchange">
			<div class="inline-edit-wrapper" role="region" aria-labelledby="<?php echo $bulk
       ? "bulk"
       : "quick"; ?>-edit-legend">
			<fieldset class="inline-edit-col-left">
				<legend class="inline-edit-legend" id="<?php echo $bulk
        ? "bulk"
        : "quick"; ?>-edit-legend"><?php echo $bulk
    ? __("Bulk Edit")
    : __("Quick Edit"); ?></legend>
      ...
      ?>
				...
				<?php if (post_type_supports($screen->post_type, "author")) {
        $authors_dropdown = "";
				...
				<?php if (!$bulk && $can_publish): ?>
					...
					<div class="inline-edit-group wp-clearfix">
						...
						<span class="alignleft inline-edit-or">
							<?php /* translators: Between password field and private checkbox on post quick edit interface. */
       _e("&ndash;OR&ndash;"); ?>
						</span>
			...
			<?php if (count($hierarchical_taxonomies) && !$bulk): ?>
				...
				<fieldset class="inline-edit-col-center inline-edit-categories">
					...
					<?php foreach ($hierarchical_taxonomies as $taxonomy): ?>
						...
						<span class="title inline-edit-categories-label"><?php echo esc_html(
          $taxonomy->labels->name
      ); ?></span>
						<input type="hidden" name="<?php echo "category" === $taxonomy->name
          ? "post_category[]"
          : "tax_input[" . esc_attr($taxonomy->name) . "][]"; ?>" value="0" />
      ...
      ?>
			...
			<fieldset class="inline-edit-col-right">
				...
				<?php if (post_type_supports($screen->post_type, "author") && $bulk) {
        echo $authors_dropdown;
    } ?>
       ...
       if ($bulk) {
           $dropdown_args["show_option_no_change"] = __(
               "&mdash; No Change &mdash;"
           );
      ...
      ?>
        ...
        // $bulk
        ?>
							<?php /** This filter is documented in wp-admin/includes/meta-boxes.php */
					...
					<?php foreach ($flat_taxonomies as $taxonomy): ?>
						...
						<?php if (current_user_can($taxonomy->cap->assign_terms)): ?>
							...
							<label class="inline-edit-tags">
								...
								<textarea data-wp-taxonomy="<?php echo $taxonomy_name; ?>" cols="22" rows="1" name="tax_input[<?php echo esc_attr(
    $taxonomy->name
); ?>]" class="tax_input_<?php echo esc_attr(
    $taxonomy->name
); ?>" aria-describedby="inline-edit-<?php echo esc_attr(
    $taxonomy->name
); ?>-desc"></textarea>
							</label>
							<p class="howto" id="inline-edit-<?php echo esc_attr(
           $taxonomy->name
       ); ?>-desc"><?php echo esc_html(
						...
						<select name="post_format">
							...
							<?php if (is_array($post_formats[0])): ?>
								<?php foreach ($post_formats[0] as $format): ?>
									<option value="<?php echo esc_attr($format); ?>"><?php echo esc_html(
   ...
   ?>
			...
			<div class="submit inline-edit-save">
				<?php if (!$bulk): ?>
					...
					<button type="button" class="button button-primary save"><?php _e(
         "Update"
     ); ?></button>
				<?php else: ?>
				...
				<input type="hidden" name="screen" value="<?php echo esc_attr(
        $screen->id
    ); ?>" />
				<?php if (!$bulk && !post_type_supports($screen->post_type, "author")): ?>
					<input type="hidden" name="post_author" value="<?php echo esc_attr(
         $post->post_author
     ); ?>" />
				<?php endif; ?>
