...
function iis7_save_url_rewrite_rules()
{
    ...
    if (
        iis7_supports_permalinks() &&
        ((!file_exists($web_config_file) &&
            win_is_writable($home_path) &&
            $wp_rewrite->using_mod_rewrite_permalinks()) ||
...
function wp_print_theme_file_tree($tree, $level = 2, $size = 1, $index = 1)
{
    ...
    if (is_array($tree)) {
        ...
        foreach ($tree as $label => $theme_file):
			...
			<li role="treeitem" aria-expanded="true" tabindex="-1"
				...
				<span class="folder-label"><?php echo esc_html(
        $label
    ); ?> <span class="screen-reader-text">
					<?php /* translators: Hidden accessibility text. */
     _e("folder"); ?>
				</span><span aria-hidden="true" class="icon"></span></span>
				<ul role="group" class="tree-folder"><?php wp_print_theme_file_tree(
        $theme_file,
        $level + 1,
        $index,
        $size
        ...
        ?>
		<li role="none" class="<?php echo esc_attr(
      $relative_file === $filename ? "current-file" : ""
  ); ?>">
			<a role="treeitem" tabindex="<?php echo esc_attr(
       $relative_file === $filename ? "0" : "-1"
   ); ?>"
    ...
    if (
        ...
        wp_basename($filename) !== $file_description
    ) {
        $file_description .=
    ...
    if ($relative_file === $filename) {
        echo '<span class="notice notice-info">' .
            $file_description .
            "</span>";
    } else {
...
function wp_print_plugin_file_tree(
    ...
    if (is_array($tree)) {
     ...
     _e("folder"); ?>
				...
				<ul role="group" class="tree-folder"><?php wp_print_plugin_file_tree(
        $plugin_file,
        "",
        $level + 1,
        $index,
...
function show_message($message)
{
    if (is_wp_error($message)) {
        if (
            ...
            is_string($message->get_error_data())
        ) {
            $message =
    ...
    for ($t = 0; $t < $count - 2; $t++) {
        ...
        if (
            ...
            if (
                ...
                (isset($tokens[$t - 2][0]) &&
                    T_OBJECT_OPERATOR === $tokens[$t - 1][0])
            ) {
...
function set_screen_options()
{
    ...
    switch ($map_option) {
        ...
        default:
            ...
            if (
                str_ends_with($option, "_page") ||
                "layout_columns" === $option
            ) {
                /**
...
function iis7_rewrite_rule_exists($filename)
{
    ...
    $rules = $xpath->query(
        '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'wordpress\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'WordPress\')]'
    );
    ...
    if ($xml_nodes->length > 0) {
        ...
        if ($xml_nodes->length > 0) {
            ...
            if ($xml_nodes->length > 0) {
                ...
                $system_web_server_node = $doc->createElement(
                    "system.webServer"
                );
                $system_web_server_node->appendChild($rewrite_node);
...
function admin_color_scheme_picker($user_id)
{
  ...
  foreach ($_wp_admin_css_colors as $color => $color_info): ?>
			<div class="color-option <?php echo $color === $current_color
       ? "selected"
       : ""; ?>">
				<input name="admin_color" id="admin_color_<?php echo esc_attr(
        $color
    ); ?>" type="radio" value="<?php echo esc_attr(
    $color
); ?>" class="tog" <?php checked($color, $current_color); ?> />
				<input type="hidden" class="css_url" value="<?php echo esc_url(
        $color_info->url
    ); ?>" />
				<input type="hidden" class="icon_colors" value="<?php echo esc_attr(
        wp_json_encode(["icons" => $color_info->icon_colors])
    ); ?>" />
				<label for="admin_color_<?php echo esc_attr($color); ?>"><?php echo esc_html(
    $color_info->name
); ?></label>
				<div class="color-palette">
				<?php foreach ($color_info->colors as $html_color) { ?>
					<div class="color-palette-shade" style="background-color: <?php echo esc_attr(
         $html_color
     ); ?>">&nbsp;</div>
					<?php } ?>
				</div>
...
function wp_check_locked_posts($response, $data, $screen_id)
{
    ...
    if (
        array_key_exists("wp-check-locked-posts", $data) &&
        is_array($data["wp-check-locked-posts"])
    ) {
            ...
            if ($user_id) {
                ...
                if ($user && current_user_can("edit_post", $post_id)) {
                    ...
                    if (get_option("show_avatars")) {
                        $send["avatar_src"] = get_avatar_url($user->ID, [
                            "size" => 18,
                        ]);
                        $send["avatar_src_2x"] = get_avatar_url($user->ID, [
                            "size" => 36,
                        ]);
                    }
    ...
    if (array_key_exists("wp-refresh-post-lock", $data)) {
        ...
        if ($user) {
            $error = [
                ...
                /* translators: %s: User's display name. */
                "text" => sprintf(
                    __("%s has taken over and is currently editing."),
                    $user->display_name
                ),
            ];

            if (get_option("show_avatars")) {
                $error["avatar_src"] = get_avatar_url($user->ID, [
                    "size" => 64,
                ]);
                $error["avatar_src_2x"] = get_avatar_url($user->ID, [
                    "size" => 128,
                ]);
            }
    ...
    if (!empty($data["wp_autosave"])) {
        ...
        if (is_wp_error($saved)) {
            ...
            $response["wp_autosave"] = [
                ...
                "message" => sprintf(
                    __("Draft saved at %s."),
                    date_i18n($draft_saved_date_format)
                ),
            ];
        }
    }
...
function wp_admin_canonical_url()
{
    ...
    ?>
	<link id="wp-admin-canonical" rel="canonical" href="<?php echo esc_url(
     $filtered_url
 ); ?>" />
	<script>
...
###SITEURL###'
    ...
    $content = str_replace(
        "###ADMIN_URL###",
        esc_url(self_admin_url("options.php?adminhash=" . $hash)),
        $content
    );
...
function wp_check_php_version()
{
    ...
    if (isset($response["is_acceptable"]) && $response["is_acceptable"]) {
         ...
         */
        $response["is_acceptable"] = (bool) apply_filters(
