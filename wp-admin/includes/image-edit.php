...
function wp_image_editor($post_id, $msg = false)
{
    ...
    if (
        !empty($backup_sizes) &&
        isset($backup_sizes["full-orig"], $meta["file"])
    ) {
        $can_restore =
            wp_basename($meta["file"]) !== $backup_sizes["full-orig"]["file"];
    }
     ...
     */
    $edit_thumbnails_separately = (bool) apply_filters(
	...
	<div class="imgedit-panel-content imgedit-panel-tools wp-clearfix">
		<div class="imgedit-menu wp-clearfix">
			...
			<button type="button" class="imgedit-scale button" onclick="imageEdit.toggleControls(this);" aria-expanded="false" aria-controls="imgedit-scale"><?php esc_html_e(
       "Scale"
   ); ?></button>
			<div class="imgedit-rotate-menu-container">
				<button type="button" aria-controls="imgedit-rotate-menu" class="imgedit-rotate button" aria-expanded="false" onclick="imageEdit.togglePopup(this)" onblur="imageEdit.monitorPopup()"><?php esc_html_e(
        "Image Rotation"
    ); ?></button>
				<div id="imgedit-rotate-menu" class="imgedit-popup-menu">
			<?php // On some setups GD library does not provide imagerotate() - Ticket #11536.
   if (
       ...
       $note_no_rotate = ""; ?>
					<button type="button" class="imgedit-rleft button" onkeydown="imageEdit.browsePopup(event, this)" onclick="imageEdit.rotate( 90, <?php echo "$post_id, '$nonce'"; ?>, this)" onblur="imageEdit.monitorPopup()"><?php esc_html_e(
    "Rotate 90&deg; left"
); ?></button>
					<button type="button" class="imgedit-rright button" onkeydown="imageEdit.browsePopup(event, this)" onclick="imageEdit.rotate(-90, <?php echo "$post_id, '$nonce'"; ?>, this)" onblur="imageEdit.monitorPopup()"><?php esc_html_e(
    "Rotate 90&deg; right"
); ?></button>
					<button type="button" class="imgedit-rfull button" onkeydown="imageEdit.browsePopup(event, this)" onclick="imageEdit.rotate(180, <?php echo "$post_id, '$nonce'"; ?>, this)" onblur="imageEdit.monitorPopup()"><?php esc_html_e(
    "Rotate 180&deg;"
); ?></button>
				<?php
   } else {
       $note_no_rotate =
           '<p class="note-no-rotate"><em>' .
	...
	<div class="imgedit-panel-content wp-clearfix">
		<div class="imgedit-tools">
			...
			<img id="image-preview-<?php echo $post_id; ?>" onload="imageEdit.imgLoaded('<?php echo $post_id; ?>')"
				src="<?php echo esc_url(admin_url("admin-ajax.php", "relative")) .
      ...
      esc_html_e("Scale Image Help"); ?>
          ...
          "You can proportionally scale the original image. For best results, scaling should be done before you crop, flip, or rotate. Images can only be scaled down, not up."
           ...
           '<span class="imgedit-original-dimensions">' .
               $meta["width"] .
               " &times; " .
               $meta["height"] .
               "</span>"
       ...
       _e("scale height"); ?>
							...
							<input type="number" step="1" min="0" max="<?php echo isset($meta["width"])
           ? $meta["width"]
           : ""; ?>" aria-describedby="imgedit-scale-warn-<?php echo $post_id; ?>"  id="imgedit-scale-width-<?php echo $post_id; ?>" onkeyup="imageEdit.scaleChanged(<?php echo $post_id; ?>, 1, this)" onblur="imageEdit.scaleChanged(<?php echo $post_id; ?>, 1, this)" value="<?php echo isset($meta["width"]) ? $meta["width"] : 0; ?>" />
							<span class="imgedit-separator" aria-hidden="true">&times;</span>
							<label for="imgedit-scale-height-<?php echo $post_id; ?>" class="screen-reader-text"><?php _e("scale height"); ?></label>
							<input type="number" step="1" min="0" max="<?php echo isset($meta["height"])
           ? $meta["height"]
           : ""; ?>" aria-describedby="imgedit-scale-warn-<?php echo $post_id; ?>" id="imgedit-scale-height-<?php echo $post_id; ?>" onkeyup="imageEdit.scaleChanged(<?php echo $post_id; ?>, 0, this)" onblur="imageEdit.scaleChanged(<?php echo $post_id; ?>, 0, this)" value="<?php echo isset($meta["height"]) ? $meta["height"] : 0; ?>" />
		...
		<?php if ($can_restore) { ?>
				<div class="imgedit-group">
				<div class="imgedit-group-top">
					<h2><button type="button" onclick="imageEdit.toggleHelp(this);" class="button-link" aria-expanded="false"><?php _e(
     ...
     if (!defined("IMAGE_EDIT_OVERWRITE") || !IMAGE_EDIT_OVERWRITE) {
         echo " " .
             __("Previously edited copies of the image will not be deleted.");
     }
     ?>
					</p>
					<div class="imgedit-submit">
						<input type="button" onclick="imageEdit.action(<?php echo "$post_id, '$nonce'"; ?>, 'restore')" class="button button-primary" value="<?php esc_attr_e(
    "Restore image"
	...
	<?php if ($edit_thumbnails_separately && $thumb && $sub_sizes) {
     $thumb_img = wp_constrain_dimensions(
         $thumb["width"],
         $thumb["height"],
         160,
         120
       ...
       "You can edit the image while preserving the thumbnail. For example, you may wish to have a square thumbnail that displays just a section of the image."
		...
		<div class="imgedit-thumbnail-preview-group">
			<figure class="imgedit-thumbnail-preview">
				<img src="<?php echo esc_url($thumb["url"]); ?>" width="<?php echo esc_attr(
    $thumb_img[0]
); ?>" height="<?php echo esc_attr(
    $thumb_img[1]
); ?>" class="imgedit-size-preview" alt="" draggable="false" />
				<figcaption class="imgedit-thumbnail-preview-caption"><?php _e(
        "Current thumbnail"
    ); ?></figcaption>
			</figure>
			...
			<fieldset>
				...
				<span class="imgedit-label">
					...
					<label for="imgedit-target-nothumb"><?php _e(
...
function wp_stream_image($image, $mime_type, $attachment_id)
{
    if ($image instanceof WP_Image_Editor) {
         ...
         * @param int              $attachment_id The attachment post ID.
         */
        $image = apply_filters_deprecated(
            "image_save_pre",
            [$image, $attachment_id],
            "3.5.0",
        ...
        switch ($operation->type) {
            ...
            case "crop":
                ...
                if ($image instanceof WP_Image_Editor) {
                    ...
                    $scale = isset($sel->r)
                        ? $sel->r
                        : 1 / _image_get_preview_ratio($w, $h); // Discard preview scaling.
                    $image->crop(
                        (int) ($sel->x * $scale),
                        (int) ($sel->y * $scale),
                        (int) ($sel->w * $scale),
...
function stream_preview_image($post_id)
{
    ...
    $changes = !empty($_REQUEST["history"])
        ? json_decode(wp_unslash($_REQUEST["history"]))
        : null;
    if ($changes) {
...
function wp_restore_image($post_id)
{
    ...
    if (
        ...
        is_array($backup_sizes["full-orig"])
    ) {
    ...
    foreach ($default_sizes as $default_size) {
        if (isset($backup_sizes["$default_size-orig"])) {
            ...
            if (
                isset($meta["sizes"][$default_size]) &&
                $meta["sizes"][$default_size]["file"] !== $data["file"]
            ) {
                if (defined("IMAGE_EDIT_OVERWRITE") && IMAGE_EDIT_OVERWRITE) {
                    // Delete only if it's an edited image.
                    if (
                        preg_match(
                            "/-e[0-9]{13}-/",
                            $meta["sizes"][$default_size]["file"]
                        )
                    ) {
                        $delete_file = path_join(
                            $parts["dirname"],
                            $meta["sizes"][$default_size]["file"]
                        );
                    ...
                    $backup_sizes["$default_size-{$suffix}"] =
...
function wp_save_image($post_id)
{
    ...
    $full_height = !empty($_REQUEST["fheight"])
        ? (int) $_REQUEST["fheight"]
        : 0;
    $target = !empty($_REQUEST["target"])
    ...
    if ($scale) {
        ...
        if ($full_width > 0 && $full_height > 0) {
            ...
            $diff =
                round($original_width / $original_height, 2) -
                round($full_width / $full_height, 2);
        ...
        $return->error = esc_js(
            __("Nothing to save, the image has not changed.")
        );
        return $return;
    }
    ...
    if (!is_array($meta)) {
        $return->error = esc_js(
            __("Image data does not exist. Please re-upload the image.")
        );
        return $return;
    }
    ...
    if (
        ...
        $success =
            $path === $new_path || update_attached_file($post_id, $new_path);
    ...
    if (isset($sizes)) {
        ...
        $meta["sizes"] = array_merge(
            $meta["sizes"],
            $img->multi_resize($_sizes)
        );
    }
    ...
    if ($success) {
        ...
        if (
            ...
            if (
                ...
                "edit-attachment" === $_REQUEST["context"]
            ) {
