...
final class WP_Privacy_Policy_Content
{
    ...
    public static function get_suggested_policy_text()
    {
        ...
        foreach ($new as $new_key => $new_data) {
            foreach ($old as $old_key => $old_data) {
                ...
                if ($new_data["policy_text"] === $old_data["policy_text"]) {
                    ...
                    $new_data["plugin_name"] === $old_data["plugin_name"]
                ) {
        ...
        if (!empty($new)) {
            ...
            foreach ($new as $new_data) {
                if (
                    !empty($new_data["plugin_name"]) &&
                    !empty($new_data["policy_text"])
                ) {
                    $new_data["added"] = $time;
                    $checked[] = $new_data;
                }
        ...
        if (!empty($old)) {
            ...
            foreach ($old as $old_data) {
                if (
                    !empty($old_data["plugin_name"]) &&
                    !empty($old_data["policy_text"])
                ) {
                    $data = [
                        "plugin_name" => $old_data["plugin_name"],
                        "policy_text" => $old_data["policy_text"],
    ...
    public static function get_default_content(
        ...
        $strings[] =
            '<h2 class="wp-block-heading">' . __("Who we are") . "</h2>";
