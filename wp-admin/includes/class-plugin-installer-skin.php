...
class Plugin_Installer_Skin extends WP_Upgrader_Skin
{
    ...
    public function after()
    {
        ...
        if ("import" === $from) {
            $install_actions["activate_plugin"] = sprintf(
                ...
                wp_nonce_url(
                    "plugins.php?action=activate&amp;from=import&amp;plugin=" .
            ...
            $install_actions["activate_plugin"] = sprintf(
                '<a class="button button-primary" href="%s" target="_parent">%s</a>',
                wp_nonce_url(
                    "plugins.php?action=activate&amp;from=press-this&amp;plugin=" .
                        urlencode($plugin_file),
                    "activate-plugin_" . $plugin_file
                ),
        ...
        if (is_multisite() && current_user_can("manage_network_plugins")) {
            $install_actions["network_activate"] = sprintf(
                '<a class="button button-primary" href="%s" target="_parent">%s</a>',
                wp_nonce_url(
                    "plugins.php?action=activate&amp;networkwide=1&amp;plugin=" .
                        urlencode($plugin_file),
                    "activate-plugin_" . $plugin_file
                ),
        ...
        if (!$this->result || is_wp_error($this->result)) {
            unset(
                ...
                $install_actions["network_activate"]
            );
        } elseif (
    ...
    private function do_overwrite()
    {
        ...
        echo '<h2 class="update-from-upload-heading">' .
            esc_html__("This plugin is already installed.") .
            "</h2>";

        $this->is_downgrading = version_compare(
        ...
        $table .=
            "<tr><th></th><th>" . esc_html_x("Current", "plugin") . "</th>";
        $table .= "<th>" . esc_html_x("Uploaded", "plugin") . "</th></tr>";

        $is_same_plugin = true; // Let's consider only these rows.

        foreach ($rows as $field => $label) {
            $old_value = !empty($current_plugin_data[$field])
                ? (string) $current_plugin_data[$field]
                : "-";
            $new_value = !empty($new_plugin_data[$field])
                ? (string) $new_plugin_data[$field]
                : "-";
            ...
            $table .=
                '<tr><td class="name-label">' .
                $label .
                "</td><td>" .
                wp_strip_all_tags($old_value) .
                "</td>";
            $table .=
                $diff_field || $diff_version ? '<td class="warning">' : "<td>";
        ...
        $requires_php = isset($new_plugin_data["RequiresPHP"])
            ? $new_plugin_data["RequiresPHP"]
            : null;
        $requires_wp = isset($new_plugin_data["RequiresWP"])
            ? $new_plugin_data["RequiresWP"]
            : null;
        ...
        if ($can_update) {
            ...
            $install_actions["overwrite_plugin"] = sprintf(
                ...
                wp_nonce_url(
                    add_query_arg("overwrite", $overwrite, $this->url),
                    "plugin-upload"
                ),
        ...
        if (!empty($install_actions)) {
            ...
            echo '<p class="update-from-upload-actions">' .
