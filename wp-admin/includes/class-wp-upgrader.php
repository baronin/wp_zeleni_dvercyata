...
class WP_Upgrader
{
    ...
    public function generic_strings()
    {
        ...
        $this->strings["fs_no_root_dir"] = __(
            "Unable to locate WordPress root directory."
        );
        /* translators: %s: Directory name. */
        $this->strings["fs_no_content_dir"] = sprintf(
            __("Unable to locate WordPress content directory (%s)."),
            "wp-content"
        );
        $this->strings["fs_no_plugins_dir"] = __(
            "Unable to locate WordPress plugin directory."
        );
        $this->strings["fs_no_themes_dir"] = __(
            "Unable to locate WordPress theme directory."
        );
        /* translators: %s: Directory name. */
        $this->strings["fs_no_folder"] = __(
            "Unable to locate needed folder (%s)."
        );

        $this->strings["no_package"] = __("Package not available.");
        $this->strings["download_failed"] = __("Download failed.");
        $this->strings["installing_package"] = __(
            "Installing the latest version&#8230;"
        );
        $this->strings["no_files"] = __("The package contains no files.");
        $this->strings["folder_exists"] = __(
            "Destination folder already exists."
        );
        $this->strings["mkdir_failed"] = __("Could not create directory.");
        $this->strings["incompatible_archive"] = __(
            "The package could not be installed."
        );
        $this->strings["files_not_writable"] = __(
        ...
        $this->strings["dir_not_readable"] = __(
            "A directory could not be read."
        );

        $this->strings["maintenance_start"] = __(
            "Enabling Maintenance mode&#8230;"
        );
        $this->strings["maintenance_end"] = __(
            "Disabling Maintenance mode&#8230;"
        );

        /* translators: %s: upgrade-temp-backup */
        $this->strings["temp_backup_mkdir_failed"] = sprintf(
            __("Could not create the %s directory."),
            "upgrade-temp-backup"
        );
        /* translators: %s: upgrade-temp-backup */
        $this->strings["temp_backup_move_failed"] = sprintf(
            __("Could not move the old version to the %s directory."),
            "upgrade-temp-backup"
        );
        /* translators: %s: The plugin or theme slug. */
        $this->strings["temp_backup_restore_failed"] = __(
            "Could not restore the original version of %s."
        );
        /* translators: %s: The plugin or theme slug. */
        $this->strings["temp_backup_delete_failed"] = __(
     ...
     */
    public function fs_connect(
        $directories = [],
        $allow_relaxed_file_ownership = false
    ) {
        global $wp_filesystem;

        $credentials = $this->skin->request_filesystem_credentials(
            false,
            $directories[0],
        ...
        if (
            ...
            $this->skin->request_filesystem_credentials(
                $error,
                $directories[0],
                $allow_relaxed_file_ownership
            );
    ...
    public function download_package(
        ...
        if (
            ...
            !$download_file->get_error_data("softfail-filename")
        ) {
            return new WP_Error(
                "download_failed",
    ...
    public function unpack_package($package, $delete_package = true)
    {
        ...
        $working_dir =
            $upgrade_folder . basename(basename($package, ".tmp"), ".zip");
    ...
    public function install_package($args = [])
    {
        ...
        if (
            ...
            $wp_filesystem->is_dir(
                trailingslashit($args["source"]) . $source_files[0] . "/"
            )
        ) {
            // Only one folder? Then we want its contents.
            $source =
                trailingslashit($args["source"]) .
        ...
        if (is_array($wp_theme_directories)) {
            $protected_directories = array_merge(
                $protected_directories,
        ...
        if (in_array($destination, $protected_directories, true)) {
            $remote_destination =
                trailingslashit($remote_destination) .
                trailingslashit(basename($source));
            $destination =
                trailingslashit($destination) .
                trailingslashit(basename($source));
        }
        ...
        if ($clear_destination) {
            ...
            $args["abort_if_destination_exists"] &&
        ...
        $destination_name = basename(
            str_replace($local_destination, "", $destination)
        );
        if ("." === $destination_name) {
            $destination_name = "";
        }
    ...
    public function run($options)
    {
        ...
        if (
            ...
            $download->get_error_data("softfail-filename")
        ) {
        ...
        if (is_wp_error($result)) {
            ...
            if (
                !method_exists($this->skin, "hide_process_failed") ||
                !$this->skin->hide_process_failed($result)
            ) {
    ...
    public function move_to_temp_backup_dir($args)
    {
        ...
        if (!$wp_filesystem->is_dir($sub_dir)) {
            ...
            if (!$wp_filesystem->mkdir($sub_dir, FS_CHMOD_DIR)) {
                ...
                return new WP_Error(
                    ...
                    $this->strings["temp_backup_mkdir_failed"]
                );
            }
        }
        ...
        if (is_wp_error($result)) {
            return new WP_Error(
                "fs_temp_backup_move",
                $this->strings["temp_backup_move_failed"]
            );
        }
    ...
    public function restore_temp_backup(array $temp_backups = [])
    {
        ...
        foreach ($temp_backups as $args) {
            ...
            if ($wp_filesystem->is_dir($src)) {
                // Cleanup.
                if (
                    $wp_filesystem->is_dir($dest) &&
                    !$wp_filesystem->delete($dest, true)
                ) {
                    $errors->add(
                        "fs_temp_backup_delete",
                        sprintf(
                            $this->strings["temp_backup_restore_failed"],
                            $args["slug"]
                        )
                    );
                    continue;
    ...
    public function delete_temp_backup(array $temp_backups = [])
    {
        ...
        foreach ($temp_backups as $args) {
            ...
            $temp_backup_dir =
                $wp_filesystem->wp_content_dir() .
                "upgrade-temp-backup/{$args["dir"]}/{$args["slug"]}";

            if (!$wp_filesystem->delete($temp_backup_dir, true)) {
                $errors->add(
                    "temp_backup_delete_failed",
                    sprintf(
                        $this->strings["temp_backup_delete_failed"],
                        $args["slug"]
                    )
                );
                continue;
