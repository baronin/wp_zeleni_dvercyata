...
function list_files(
    ...
    if ($dir) {
        while (($file = readdir($dir)) !== false) {
            ...
            if (
                (!$include_hidden && "." === $file[0]) ||
                in_array($file, $exclusions, true)
            ) {
                continue;
            }
...
function wp_edit_theme_plugin_file($args)
{
    ...
    if (!empty($args["plugin"])) {
        ...
        if (!current_user_can("edit_plugins")) {
            return new WP_Error(
                "unauthorized",
                __("Sorry, you are not allowed to edit plugins for this site.")
            );
        }
        ...
        if (0 !== validate_file($file, get_plugin_files($plugin))) {
            return new WP_Error(
                "bad_plugin_file_path",
                __("Sorry, that file cannot be edited.")
            );
        }
        ...
        if (
            $theme->errors() &&
            "theme_no_stylesheet" === $theme->errors()->get_error_code()
        ) {
            return new WP_Error(
                "theme_no_stylesheet",
                __("The requested theme does not exist.") .
        ...
        $is_active =
            get_stylesheet() === $stylesheet || get_template() === $stylesheet;
    } else {
    ...
    if ($is_active && "php" === $extension) {
        ...
        if (false === $scrape_result_position) {
            ...
            $error_output = substr(
                $body,
                $scrape_result_position + strlen($needle_start)
            );
            $error_output = substr(
                $error_output,
                0,
                strpos($error_output, $needle_end)
            );
...
function _wp_handle_upload(&$file, $overrides, $time, $action)
{
    ...
    $test_form = isset($overrides["test_form"])
        ? $overrides["test_form"]
        : true;
    $test_size = isset($overrides["test_size"])
        ? $overrides["test_size"]
        : true;

    // If you override this, you must provide $ext and $type!!
    $test_type = isset($overrides["test_type"])
        ? $overrides["test_type"]
        : true;
    ...
    if ($test_type) {
        ...
        $proper_filename = empty($wp_filetype["proper_filename"])
            ? ""
...
function download_url($url, $timeout = 300, $signature_verification = false)
{
    ...
    if (!$tmpfname) {
        return new WP_Error(
            "http_no_file",
            __("Could not create temporary file.")
        );
    }
    ...
    $content_disposition = wp_remote_retrieve_header(
        $response,
        "Content-Disposition"
    );
    ...
    if ($content_disposition) {
        ...
        if (str_starts_with($content_disposition, "attachment; filename=")) {
            $tmpfname_disposition = sanitize_file_name(
        ...
        if (
            ...
            $tmpfname_disposition =
                dirname($tmpfname) . "/" . $tmpfname_disposition;
    ...
    if ($signature_verification) {
        ...
        $signature_verification = in_array(
    ...
    if ($signature_verification) {
        $signature = wp_remote_retrieve_header(
            $response,
            "X-Content-Signature"
        );
        ...
        if (!$signature) {
            ...
            if (
                is_string($url_path) &&
                (str_ends_with($url_path, ".zip") ||
                    str_ends_with($url_path, ".tar.gz"))
            ) {
                $signature_url = str_replace(
                    $url_path,
                    $url_path . ".sig",
                    $url
                );
            ...
            if ($signature_url) {
                ...
                if (
                    !is_wp_error($signature_request) &&
                    200 === wp_remote_retrieve_response_code($signature_request)
                ) {
                    $signature = explode(
                        "\n",
        ...
        // Perform the checks.
        $signature_verification = verify_file_signature(
            $tmpfname,
            $signature,
            $url_filename
        );
...
function verify_file_signature(
    ...
    if (
        ...
        !in_array("sha384", array_map("strtolower", hash_algos()), true)
    ) {
        return new WP_Error(
    ...
    if (
        ...
        if (method_exists("ParagonIE_Sodium_Compat", "runtime_speed_test")) {
            ...
            $sodium_compat_is_fast = ParagonIE_Sodium_Compat::runtime_speed_test(
                100,
                10
            );
        ...
        if (!$sodium_compat_is_fast) {
            return new WP_Error(
                ...
                [
                    "php" => PHP_VERSION,
                    "sodium" => defined("SODIUM_LIBRARY_VERSION")
...
function unzip_file($file, $to)
{
    ...
    if (!$wp_filesystem || !is_object($wp_filesystem)) {
        return new WP_Error(
            "fs_unavailable",
            __("Could not access filesystem.")
        );
    }
...
function _unzip_file_ziparchive($file, $to, $needed_dirs = [])
{
    ...
    foreach ($needed_dirs as $_dir) {
        ...
        if (
            !$wp_filesystem->mkdir($_dir, FS_CHMOD_DIR) &&
...
function _unzip_file_pclzip($file, $to, $needed_dirs = [])
{
    ...
    foreach ($archive_files as $file) {
        ...
        $needed_dirs[] =
            $to .
            untrailingslashit(
                $file["folder"] ? $file["filename"] : dirname($file["filename"])
            );
    }
...
function move_dir($from, $to, $overwrite = false)
{
    ...
    if (
        trailingslashit(strtolower($from)) === trailingslashit(strtolower($to))
    ) {
        return new WP_Error(
    ...
    if ($wp_filesystem->exists($to)) {
        if (!$overwrite) {
            return new WP_Error(
                ...
                __("The destination folder already exists."),
                $to
            );
...
function WP_Filesystem(
    ...
    if (!class_exists("WP_Filesystem_$method")) {
        ...
        $abstraction_file = apply_filters(
            ...
            ABSPATH .
                "wp-admin/includes/class-wp-filesystem-" .
                $method .
                ".php",
            $method
        );
...
function request_filesystem_credentials(
    ...
    if (
        !isset($_POST["_fs_nonce"]) ||
        !wp_verify_nonce($_POST["_fs_nonce"], "filesystem-credentials")
    ) {
        unset(
    ...
    $credentials["hostname"] = preg_replace(
        "|\w+://|",
        "",
    ...
    if (strpos($credentials["hostname"], ":")) {
        list($credentials["hostname"], $credentials["port"]) = explode(
            ":",
            $credentials["hostname"],
            2
        );
    ...
    if (
        (defined("FTP_SSH") && FTP_SSH) ||
        (defined("FS_METHOD") && "ssh2" === FS_METHOD)
    ) {
    ...
    $public_key = isset($credentials["public_key"])
        ? $credentials["public_key"]
        : "";
    $private_key = isset($credentials["private_key"])
        ? $credentials["private_key"]
        : "";
    $port = isset($credentials["port"]) ? $credentials["port"] : "";
    $connection_type = isset($credentials["connection_type"])
...
<div id="request-filesystem-credentials-form" class="request-filesystem-credentials-form">
 ...
 echo "<$heading_tag id='request-filesystem-credentials-title'>" .
...
<label for="hostname">
	...
	<input name="hostname" type="text" id="hostname" aria-describedby="request-filesystem-credentials-desc" class="code" placeholder="<?php esc_attr_e(
     "example: www.wordpress.org"
 ); ?>" value="<?php echo $hostname_value; ?>"<?php disabled(
    defined("FTP_HOST")
); ?> />
</label>
<div class="ftp-username">
	<label for="username">
		...
		<input name="username" type="text" id="username" value="<?php echo esc_attr(
      $username
  ); ?>"<?php disabled(defined("FTP_USER")); ?> />
	</label>
</div>
<div class="ftp-password">
	<label for="password">
		<span class="field-title"><?php echo $label_pass; ?></span>
		<input name="password" type="password" id="password" value="<?php echo $password_value; ?>"<?php disabled(
    defined("FTP_PASS")
); ?> spellcheck="false" />
...
<legend><?php _e("Connection Type"); ?></legend>
	<?php
 $disabled = disabled(
     (defined("FTP_SSL") && FTP_SSL) || (defined("FTP_SSH") && FTP_SSH),
     true,
     false
 );
 foreach ($types as $name => $text): ?>
	<label for="<?php echo esc_attr($name); ?>">
		<input type="radio" name="connection_type" id="<?php echo esc_attr(
      $name
  ); ?>" value="<?php echo esc_attr($name); ?>" <?php checked(
    $name,
    $connection_type
); ?> <?php echo $disabled; ?> />
		<?php echo $text; ?>
...
<label for="public_key">
	...
	<input name="public_key" type="text" id="public_key" aria-describedby="auth-keys-desc" value="<?php echo esc_attr(
     $public_key
 ); ?>"<?php disabled(defined("FTP_PUBKEY")); ?> />
</label>
<label for="private_key">
	<span class="field-title"><?php _e("Private Key:"); ?></span>
	<input name="private_key" type="text" id="private_key" value="<?php echo esc_attr(
     $private_key
 ); ?>"<?php disabled(defined("FTP_PRIKEY")); ?> />
</label>
<p id="auth-keys-desc"><?php _e(
 ...
 ?>
	<p class="request-filesystem-credentials-action-buttons">
		...
		<button class="button cancel-button" data-js-action="close" type="button"><?php _e(
      "Cancel"
  ); ?></button>
...
function wp_opcache_invalidate_directory($dir)
{
     ...
     */
    $invalidate_directory = static function ($dirlist, $path) use (
