...
function wp_dashboard_setup()
{
    ...
    if ($check_php && current_user_can("update_php")) {
        ...
        if (
            ...
            add_filter(
                "postbox_classes_dashboard_dashboard_php_nag",
    ...
    foreach ($dashboard_widgets as $widget_id) {
        $name = empty($wp_registered_widgets[$widget_id]["all_link"])
            ...
            : $wp_registered_widgets[$widget_id]["name"] .
                " <a href='{$wp_registered_widgets[$widget_id]["all_link"]}' class='edit-box open-box'>" .
                __("View all") .
                "</a>";
        wp_add_dashboard_widget(
            $widget_id,
...
function _wp_dashboard_control_callback($dashboard, $meta_box)
{
    ...
    wp_nonce_field(
        "edit-dashboard-widget_" . $meta_box["id"],
        "dashboard-widget-nonce"
    );
...
function wp_dashboard_right_now()
{
 ...
 if ($num_comm && ($num_comm->approved || $num_comm->moderated)) {
  ...
  ?>
		<li class="comment-mod-count<?php echo !$num_comm->moderated
      ? " hidden"
      : ""; ?>">
...
function wp_network_dashboard_right_now()
{
    ...
    $user_text = sprintf(
        _n("%s user", "%s users", $c_users),
        number_format_i18n($c_users)
    );
    /* translators: %s: Number of sites on the network. */
    $blog_text = sprintf(
        _n("%s site", "%s sites", $c_blogs),
 ...
 do_action("wpmuadminresult"); ?>
    ...
    _e("Search Users"); ?>
			...
			<?php submit_button(__("Search Users"), "", false, false, [
       "id" => "submit_users",
   ]); ?>
		</p>
	</form>
    ...
    _e("Search Sites"); ?>
			...
			<?php submit_button(__("Search Sites"), "", false, false, [
       "id" => "submit_sites",
   ]); ?>
		</p>
	</form>
...
function wp_dashboard_quick_press($error_msg = false)
{
    ...
    if ($last_post_id) {
        ...
        if (
            in_array(
                get_current_blog_id(),
                array_keys(get_blogs_of_user($user_id)),
                true
            )
        ) {
            update_user_option(
	...
	<form name="post" action="<?php echo esc_url(
     admin_url("post.php")
 ); ?>" method="post" id="quick-press" class="initial-form hide-if-no-js">

		<?php if ($error_msg) {
      wp_admin_notice($error_msg, [
		...
		<div class="input-text-wrap" id="title-wrap">
			<label for="title">
				<?php /** This filter is documented in wp-admin/edit-form-advanced.php */
    echo apply_filters("enter_title_here", __("Title"), $post); ?>
		...
		<div class="textarea-wrap" id="description-wrap">
			...
			<textarea name="content" id="content" placeholder="<?php esc_attr_e(
       "What&#8217;s on your mind?"
		...
		<p class="submit">
			...
			<?php submit_button(__("Save Draft"), "primary", "save", false, [
       "id" => "save-post",
   ]); ?>
			<br class="clear" />
		</p>
	...
	<?php wp_dashboard_recent_drafts();
}
...
function _wp_dashboard_recent_comments_row(&$comment, $show_date = true)
{
    ...
    if (current_user_can("edit_comment", $comment->comment_ID)) {
        ...
        $approve_nonce = esc_html(
            "_wpnonce=" .
                wp_create_nonce("approve-comment_" . $comment->comment_ID)
        );
        $del_nonce = esc_html(
            "_wpnonce=" .
                wp_create_nonce("delete-comment_" . $comment->comment_ID)
        );

        $action_string =
            "comment.php?action=%s&p=" .
        ...
        foreach ($actions as $action => $link) {
            ...
            if (
                (("approve" === $action || "unapprove" === $action) &&
                    2 === $i) ||
                1 === $i
            ) {
                $separator = "";
...
function wp_dashboard_recent_posts($args)
{
    ...
    if ($posts->have_posts()) {
        ...
        while ($posts->have_posts()) {
            ...
            $recent_post_link = current_user_can("edit_post", get_the_ID())
                ? get_edit_post_link()
                : get_permalink();
            ...
            printf(
                ...
                /* translators: %s: Post title. */
                esc_attr(
                    sprintf(__("Edit &#8220;%s&#8221;"), $draft_or_post_title)
                ),
                $draft_or_post_title
            );
        }
...
function wp_dashboard_cached_rss_widget(
    ...
    $loading =
        '<p class="widget-loading hide-if-no-js">' .
        __("Loading&hellip;") .
        "</p>";
    $loading .= wp_get_admin_notice(__("This widget requires JavaScript."), [
        "type" => "error",
...
function wp_dashboard_rss_control($widget_id, $form_inputs = [])
{
    ...
    if (
        ...
        isset($_POST["widget-rss"][$number])
    ) {
        $_POST["widget-rss"][$number] = wp_unslash(
            $_POST["widget-rss"][$number]
        );
        $widget_options[$widget_id] = wp_widget_rss_process(
            $_POST["widget-rss"][$number]
        );
        $widget_options[$widget_id]["number"] = $number;
        ...
        if (
            !$widget_options[$widget_id]["title"] &&
            isset($_POST["widget-rss"][$number]["title"])
        ) {
            $rss = fetch_feed($widget_options[$widget_id]["url"]);
            if (is_wp_error($rss)) {
                $widget_options[$widget_id]["title"] = htmlentities(
                    __("Unknown Feed")
                );
            } else {
                $widget_options[$widget_id]["title"] = htmlentities(
...
function wp_dashboard_events_news()
{
	...
	<p class="community-events-footer">
      ...
      esc_url(
          _x("https://wordpress.org/news/", "Events and News dashboard widget")
      ),
      __("News"),
      /* translators: Hidden accessibility text. */
      __("(opens in a new tab)")
...
function wp_print_community_events_markup()
{
    $community_events_notice =
        '<p class="hide-if-js">' . "This widget requires JavaScript." . "</p>";
    $community_events_notice .=
    ...
    wp_admin_notice($community_events_notice, [
        "type" => "error",
        "additional_classes" => [
		...
		<div class="activity-block">
			<p>
				...
				<button class="button-link community-events-toggle-location" aria-expanded="false">
					...
					<span class="community-events-location-edit"><?php _e(
         "Select location"
     ); ?></span>
				</button>
			</p>

			<form class="community-events-form" aria-hidden="true" action="<?php echo esc_url(
       admin_url("admin-ajax.php")
   ); ?>" method="post">
				<label for="community-events-location">
					<?php _e("City:"); ?>
				</label>
				<?php /* translators: Replace with a city related to your locale.
				 ...
				 */
    ?>
				<input id="community-events-location" class="regular-text" type="text" name="community-events-location" placeholder="<?php esc_attr_e(
        "Cincinnati"
    ); ?>" />
...
function wp_dashboard_browser_nag()
{
    ...
    if ($response) {
        ...
        if (!empty($response["img_src"])) {
            $img_src =
                is_ssl() && !empty($response["img_src_ssl"])
                    ? $response["img_src_ssl"]
                    : $response["img_src"];
        ...
        $notice .=
            ...
            esc_attr__("Dismiss the browser warning panel") .
            '">' .
            __("Dismiss") .
            "</a></p>";
...
function wp_check_browser_version()
{
    ...
    if (false === $response) {
        ...
        $options = [
            ...
            "user-agent" =>
                "WordPress/" . wp_get_wp_version() . "; " . home_url("/"),
        ];
...
function wp_dashboard_php_nag()
{
	...
	<p>
      ...
      "PHP is one of the programming languages used to build WordPress. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance."
		...
		<?php if (!empty($response["recommended_version"])) {
      printf(
...
function wp_welcome_panel()
{
	...
	<div class="welcome-panel-header">
       ...
       dirname(__DIR__) . "/images/dashboard-background.svg"
   ); ?>
		</div>
		<h2><?php _e("Welcome to WordPress!"); ?></h2>
		<p>
			<a href="<?php echo esc_url(admin_url("about.php")); ?>">
			<?php /* translators: %s: Current WordPress version. */
   printf(
	...
	<div class="welcome-panel-column-container">
        ...
        "Design everything on your site &#8212; from the header down to the footer, all using blocks and patterns."
				...
				<a href="<?php echo esc_url(admin_url("site-editor.php")); ?>"><?php _e(
    "Open site editor"
); ?></a>
			<?php else: ?>
        ...
        "Configure your site&#8217;s logo, header, menus, and more in the Customizer."
				...
				<?php if ($can_customize): ?>
					<a class="load-customize hide-if-no-customize" href="<?php echo wp_customize_url(); ?>"><?php _e(
    "Open the Customizer"
); ?></a>
				<?php endif; ?>
			<?php endif; ?>
        ...
        "There is a new kind of WordPress theme, called a block theme, that lets you build the site you&#8217;ve always wanted &#8212; with blocks and styles."
				...
				<a href="<?php echo esc_url(
        __("https://wordpress.org/documentation/article/block-themes/")
    ); ?>"><?php _e("Learn about block themes"); ?></a>
			<?php endif; ?>
			</div>
		</div>
	</div>
