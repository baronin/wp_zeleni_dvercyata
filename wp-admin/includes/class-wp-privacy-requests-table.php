...
abstract class WP_Privacy_Requests_Table extends WP_List_Table
{
    ...
    public function process_bulk_action()
    {
        ...
        $request_ids = isset($_REQUEST["request_id"])
            ? wp_parse_id_list(wp_unslash($_REQUEST["request_id"]))
            : [];
    ...
    public function prepare_items()
    {
        ...
        $posts_per_page = $this->get_items_per_page(
            $this->request_type . "_requests_per_page"
        );
        $args = [
            "post_type" => $this->post_type,
            "post_name__in" => [$this->request_type],
            "posts_per_page" => $posts_per_page,
            "offset" => isset($_REQUEST["paged"])
                ? max(0, absint($_REQUEST["paged"]) - 1) * $posts_per_page
                : 0,
            "post_status" => "any",
            "s" => isset($_REQUEST["s"])
        ...
        if (
            ...
            isset($orderby_mapping[$_REQUEST["orderby"]])
        ) {
        ...
        if (
            ...
            in_array(strtoupper($_REQUEST["order"]), ["ASC", "DESC"], true)
        ) {
