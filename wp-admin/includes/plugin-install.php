...
function install_search_form($deprecated = true)
{
	...
	<form class="search-form search-plugins" method="get">
		...
		<input type="search" name="s" id="search-plugins" value="<?php echo esc_attr(
      $term
   ...
   _e("Search plugins by:"); ?>
		...
		<select name="type" id="typeselector">
			<option value="term"<?php selected(
       "term",
       $type
   ); ?>><?php _e("Keyword"); ?></option>
			<option value="author"<?php selected(
       "author",
       $type
   ); ?>><?php _e("Author"); ?></option>
			<option value="tag"<?php selected(
       "tag",
       $type
   ); ?>><?php _ex("Tag", "Plugin Installer"); ?></option>
		</select>
		<?php submit_button(__("Search Plugins"), "hide-if-js", false, false, [
      "id" => "search-submit",
  ]); ?>
	</form>
	<?php
...
<div class="upload-plugin">
	...
	<form method="post" enctype="multipart/form-data" class="wp-upload-form" action="<?php echo esc_url(
     self_admin_url("update.php?action=upload-plugin")
 ); ?>">
...
function install_plugins_favorites_form()
{
	...
	<form method="get">
		...
		<p>
			...
			<input type="search" id="user" name="user" value="<?php echo esc_attr(
       $user
   ); ?>" />
    ...
    if ("install" === $status) {
        if (is_dir(WP_PLUGIN_DIR . "/" . $api->slug)) {
            ...
            if (current_user_can("install_plugins")) {
                $url = wp_nonce_url(
                    self_admin_url(
                        "update.php?action=install-plugin&plugin=" . $api->slug
                    ),
                    "install-plugin_" . $api->slug
                );
            }
...
function install_plugin_information()
{
    ...
    if (
        !empty($api->banners) &&
        (!empty($api->banners["low"]) || !empty($api->banners["high"]))
    ) {

        $_with_banner = "with-banner";
        $low = empty($api->banners["low"])
            ? $api->banners["high"]
            : $api->banners["low"];
        $high = empty($api->banners["high"])
            ? $api->banners["low"]
            : $api->banners["high"];
        ?>
		<style type="text/css">
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
   ...
   if (!empty($api->version)) { ?>
			...
			<?php }
   if (!empty($api->author)) { ?>
				<li><strong><?php _e("Author:"); ?></strong> <?php echo links_add_target(
    $api->author,
    "_blank"
); ?></li>
			<?php }
   ...
   if (isset($api->active_installs)) { ?>
				...
				<?php if ($api->active_installs >= 1000000) {
   ...
   if (!empty($api->slug) && empty($api->external)) { ?>
        ...
        __("https://wordpress.org/plugins/") . $api->slug
    ); ?>/"><?php _e("WordPress.org Plugin Page &#187;"); ?></a></li>
			<?php }
   if (!empty($api->homepage)) { ?>
				<li><a target="_blank" href="<?php echo esc_url($api->homepage); ?>"><?php _e(
    "Plugin Homepage &#187;"
); ?></a></li>
			<?php }
   if (!empty($api->donate_link) && empty($api->contributors)) { ?>
				<li><a target="_blank" href="<?php echo esc_url(
        $api->donate_link
    ); ?>"><?php _e("Donate to this plugin &#187;"); ?></a></li>
			<?php }
   ?>
		</ul>
		<?php
  ...
  if (!empty($api->ratings) && array_sum((array) $api->ratings) > 0) { ?>
			...
			<p class="fyi-description"><?php _e(
       "Read all reviews on WordPress.org or write your own!"
   ); ?></p>
			<?php foreach ($api->ratings as $key => $ratecount) {
       ...
       ?>
           ...
           sprintf(_n("%d star", "%d stars", $key), $key)
					...
					<span class="counter-count" aria-hidden="true"><?php echo number_format_i18n(
         $ratecount
     ); ?></span>
				</div>
				<?php
  ...
  if (!empty($api->contributors)) { ?>
			...
			<ul class="contributors">
				<?php foreach (
        (array) $api->contributors
        as $contrib_username => $contrib_details
    ) {
        ...
        $contrib_avatar = esc_url(
            add_query_arg("s", "36", $contrib_details["avatar"])
        );
        ...
        echo "<li><a href='{$contrib_profile}' target='_blank'><img src='{$contrib_avatar}' width='18' height='18' alt='' />{$contrib_name}</a></li>";
				...
				<a target="_blank" href="<?php echo esc_url($api->donate_link); ?>"><?php _e(
 ...
 $tested_wp =
     empty($api->tested) ||
     version_compare(get_bloginfo("version"), $api->tested, "<=");

 if (!$compatible_php) {
     $compatible_php_notice_message = "<p>";
     $compatible_php_notice_message .= __(
 ...
 if (!$tested_wp) {
     ...
     $compatible_wp_notice_message = __(
 ...
 foreach ((array) $api->sections as $section_name => $content) {
     $content = links_add_base_url(
         $content,
         "https://wordpress.org/plugins/" . $api->slug . "/"
     );
...
function wp_get_plugin_action_button(
    ...
    foreach ($requires_plugins as $dependency) {
        foreach (array_keys($installed_plugins) as $installed_plugin_file) {
            if (
                str_contains($installed_plugin_file, "/") &&
                explode("/", $installed_plugin_file)[0] === $dependency
            ) {
        ...
        foreach ($active_plugins as $active_plugin_file) {
            if (
                str_contains($active_plugin_file, "/") &&
                explode("/", $active_plugin_file)[0] === $dependency
            ) {
                ...
                if ($status["url"]) {
                    if (
                        ...
                        $button = sprintf(
                            ...
                            esc_attr(
                                sprintf(_x("Install %s now", "plugin"), $name)
                            ),
                            esc_attr($name),
                            _x("Install Now", "plugin")
                        );
                ...
                if ($status["url"]) {
                    if ($compatible_php && $compatible_wp) {
                        $button = sprintf(
                            ...
                            /* translators: %s: Plugin name and version. */
                            esc_attr(
                                sprintf(_x("Update %s now", "plugin"), $name)
                            ),
                            esc_attr($name),
