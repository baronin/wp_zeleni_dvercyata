...
function wp_get_revision_ui_diff($post, $compare_from, $compare_to)
{
    ...
    if (
        $compare_from &&
        $compare_from->post_parent !== $post->ID &&
        $compare_from->ID !== $post->ID
    ) {
        return false;
    }
    if (
        $compare_to->post_parent !== $post->ID &&
        $compare_to->ID !== $post->ID
    ) {
        return false;
    }
    ...
    if (
        $compare_from &&
        strtotime($compare_from->post_date_gmt) >
            strtotime($compare_to->post_date_gmt)
    ) {
        $temp = $compare_from;
    ...
    foreach (_wp_post_revision_fields($post) as $field => $name) {
        ...
        if (!$diff && "post_title" === $field) {
            ...
            if (true === $args["show_split_view"]) {
                $diff .=
                    "<td>" .
                    esc_html($compare_from->post_title) .
                    "</td><td></td><td>" .
...
function wp_prepare_revisions_for_js($post, $selected_revision_id, $from = null)
{
    ...
    foreach ($revisions as $revision) {
        ...
        if (!isset($authors[$revision->post_author])) {
            $authors[$revision->post_author] = [
                ...
                "avatar" => $show_avatars
                    ? get_avatar($revision->post_author, 32)
                    : "",
        ...
        $current =
            !$autosave &&
            $revision->post_modified_gmt === $post->post_modified_gmt;
    ...
    if (1 === count($revisions)) {
        $revisions[$post->ID] = [
            ...
            "timeAgo" => sprintf(
                __("%s ago"),
                human_time_diff(strtotime($post->post_modified_gmt), $now_gmt)
            ),
            "autosave" => false,
            "current" => true,
            "restoreUrl" => false,
...
function wp_print_revision_templates()
{
	...
	<script id="tmpl-revisions-buttons" type="text/html">
		<div class="revisions-previous">
			<input class="button" type="button" value="<?php echo esc_attr_x(
       "Previous",
       "Button label for a previous revision"
		...
		<div class="revisions-next">
			<input class="button" type="button" value="<?php echo esc_attr_x(
       "Next",
       "Button label for a next revision"
   ); ?>" />
		</div>
	</script>
	...
	<script id="tmpl-revisions-slider-hidden-help" type="text/html">
		...
		<p id="revisions-slider-hidden-help" hidden><?php esc_html_e(
	...
	<script id="tmpl-revisions-meta" type="text/html">
		<# if ( ! _.isUndefined( data.attributes ) ) { #>
			<div class="diff-title">
				<# if ( 'from' === data.type ) { #>
					<strong id="diff-title-from"><?php _ex(
         "From:",
         "Followed by post revision info"
     ); ?></strong>
				<# } else if ( 'to' === data.type ) { #>
					<strong id="diff-title-to"><?php _ex(
         "To:",
         "Followed by post revision info"
     ); ?></strong>
				<# } #>
          ...
          '<span class="author-name">{{ data.attributes.author.name }}</span>'
				...
				<# if ( 'to' === data.type && data.attributes.restoreUrl ) { #>
					...
					<?php } ?>
					<# if ( data.attributes.autosave ) { #>
						type="button" class="restore-revision button button-primary" value="<?php esc_attr_e(
