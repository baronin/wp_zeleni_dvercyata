...
class Theme_Upgrader extends WP_Upgrader
{
    ...
    public function upgrade_strings()
    {
        ...
        $this->strings["remove_old"] = __(
            "Removing the old version of the theme&#8230;"
        );
        $this->strings["remove_old_failed"] = __(
    ...
    {
        $this->strings["no_package"] = __(
            "Installation package not available."
        );
        /* translators: %s: Package URL. */
        $this->strings["downloading_package"] = sprintf(
            __("Downloading installation package from %s&#8230;"),
            '<span class="code pre">%s</span>'
        );
        ...
        $this->strings["process_success_specific"] = __(
            'Successfully installed the theme <strong>%1$s %2$s</strong>.'
        );
        $this->strings["parent_theme_search"] = __(
            "This theme requires a parent theme. Checking if it is installed&#8230;"
        );
        /* translators: 1: Theme name, 2: Theme version. */
        $this->strings["parent_theme_prepare_install"] = __(
            'Preparing to install <strong>%1$s %2$s</strong>&#8230;'
        );
        /* translators: 1: Theme name, 2: Theme version. */
        $this->strings["parent_theme_currently_installed"] = __(
            'The parent theme, <strong>%1$s %2$s</strong>, is currently installed.'
        );
        /* translators: 1: Theme name, 2: Theme version. */
        $this->strings["parent_theme_install_success"] = __(
            'Successfully installed the parent theme, <strong>%1$s %2$s</strong>.'
        );
        /* translators: %s: Theme name. */
        $this->strings["parent_theme_not_found"] = sprintf(
            __(
        ...
        $this->strings["current_theme_has_errors"] = __(
        ...
        if (!empty($this->skin->overwrite)) {
            if ("update-theme" === $this->skin->overwrite) {
                ...
                $this->strings["process_failed"] = __("Theme update failed.");
                $this->strings["process_success"] = __(
            ...
            if ("downgrade-theme" === $this->skin->overwrite) {
                ...
                $this->strings["process_failed"] = __(
     ...
     * @return bool
     */
    public function check_parent_theme_filter(
        $install_result,
        $hook_extra,
        $child_result
    ) {
        ...
        $this->strings["process_success_specific"] =
            $this->strings["parent_theme_install_success"];
    ...
    public function bulk_upgrade($themes, $args = [])
    {
        ...
        foreach ($themes as $theme) {
            ...
            if (
                isset($r["requires"]) &&
                !is_wp_version_compatible($r["requires"])
            ) {
                $result = new WP_Error(
                ...
                isset($r["requires_php"]) &&
                !is_php_version_compatible($r["requires_php"])
            ) {
                $result = new WP_Error(
        ...
        foreach ($results as $theme => $result) {
            ...
            if (
                !$result ||
                is_wp_error($result) ||
                !isset($past_failure_emails[$theme])
            ) {
                continue;
            }
    ...
    public function check_package($source)
    {
        ...
        $requires_php = isset($info["RequiresPHP"])
            ? $info["RequiresPHP"]
            : null;
