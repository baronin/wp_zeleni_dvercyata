...
function update_gallery_tab($tabs)
{
    ...
    $tabs["gallery"] = sprintf(
        __("Gallery (%s)"),
        "<span id='attachments-count'>$attachments</span>"
    );
...
function media_handle_sideload(
    ...
    if (
        isset($post_data["post_date"]) &&
        substr($post_data["post_date"], 0, 4) > 0
    ) {
...
function wp_iframe($content_func, ...$args)
{
	...
	<title><?php bloginfo(
     "name"
 ); ?> &rsaquo; <?php _e("Uploads"); ?> &#8212; <?php _e("WordPress"); ?></title>
	<?php
 wp_enqueue_style("colors");
 // Check callback name for 'media'.
 if (
     (is_array($content_func) &&
         !empty($content_func[1]) &&
         str_starts_with((string) $content_func[1], "media")) ||
...
function media_buttons($editor_id = "content")
{
     ...
     */
    $legacy_filter = apply_filters_deprecated(
...
function get_upload_iframe_src($type = null, $post_id = null, $tab = null)
{
    ...
    $upload_iframe_src = add_query_arg(
        "post_id",
        (int) $post_id,
...
function media_upload_form_handler()
{
        ...
        foreach ($_POST["attachments"] as $attachment_id => $attachment) {
            ...
            foreach (get_attachment_taxonomies($post) as $t) {
                if (isset($attachment[$t])) {
                    wp_set_object_terms(
                        $attachment_id,
                        array_map("trim", preg_split("/,+/", $attachment[$t])),
                        $t,
                        false
                    );
                }
    ...
    if (isset($send_id)) {
        ...
        if (!empty($attachment["url"])) {
            ...
            if (
                str_contains($attachment["url"], "attachment_id") ||
                get_attachment_link($send_id) === $attachment["url"]
            ) {
...
function wp_media_upload_handler()
{
    ...
    if (isset($_GET["tab"]) && "type_url" === $_GET["tab"]) {
        ...
        if (
            isset($_GET["type"]) &&
            in_array($_GET["type"], ["video", "audio", "file"], true)
        ) {
            $type = $_GET["type"];
        }
...
function media_sideload_image(
    ...
    if (!empty($file)) {
        ...
        preg_match(
            "/[^\?]+\.(" . implode("|", $allowed_extensions) . ")\b/i",
            $file,
            $matches
        );
...
function image_align_input_fields($post, $checked = "")
{
    ...
    foreach ($alignments as $name => $label) {
        $name = esc_attr($name);
        $output[] =
            "<input type='radio' name='attachments[{$post->ID}][align]' id='image-align-{$name}-{$post->ID}' value='$name'" .
...
function image_size_input_fields($post, $check = "")
{
    ...
    foreach ($size_names as $size => $label) {
        ...
        $html =
            "<div class='image-size-item'><input type='radio' " .
...
function image_link_input_fields($post, $url_type = "")
{
    ...
    return "
	<input type='text' class='text urlfield' name='attachments[$post->ID][url]' value='" .
        esc_attr($url) .
        "' /><br />
...
function image_media_send_to_editor($html, $attachment_id, $attachment)
{
    ...
    if (str_starts_with($post->post_mime_type, "image")) {
        ...
        $size = !empty($attachment["image-size"])
            ? $attachment["image-size"]
            : "medium";
        $alt = !empty($attachment["image_alt"]) ? $attachment["image_alt"] : "";
        $rel =
            str_contains($url, "attachment_id") ||
            get_attachment_link($attachment_id) === $url;

        return get_image_send_to_editor(
            $attachment_id,
...
function get_attachment_fields_to_edit($post, $errors = null)
{
    ...
    $form_fields = [
        ...
        "image_url" => [
            ...
            "input" => "html",
            "html" =>
                "<input type='text' class='text urlfield' readonly='readonly' name='attachments[$post->ID][url]' value='" .
                esc_attr($image_url) .
                "' /><br />",
    ...
    if (str_starts_with($post->post_mime_type, "image")) {
        ...
        $form_fields["align"] = [
            "label" => __("Alignment"),
            "input" => "html",
            "html" => image_align_input_fields(
                $post,
                get_option("image_default_align")
            ),
        ];

        $form_fields["image-size"] = image_size_input_fields(
            $post,
...
function get_media_items($post_id, $errors)
{
    ...
    foreach ((array) $attachments as $id => $attachment) {
        ...
        $item = get_media_item($id, [
            "errors" => isset($errors[$id]) ? $errors[$id] : null,
        ]);
...
function get_media_item($attachment_id, $args = null)
{
    ...
    if ($attachment_id) {
        $thumb_url = wp_get_attachment_image_src(
            $attachment_id,
            "thumbnail",
            true
        );
    ...
    $default_args = [
        "errors" => null,
        "send" => $current_post_id
            ? post_type_supports(get_post_type($current_post_id), "editor")
            : true,
        "delete" => true,
        "toggle" => true,
        "show_title" => true,
    ...
    $keys = array_keys(
        wp_match_mime_types(array_keys($post_mime_types), $post->post_mime_type)
    );
    $type = reset($keys);
    $type_html =
        "<input type='hidden' id='type-of-$attachment_id' value='" .
        esc_attr($type) .
        "' />";
    ...
    $display_title = $parsed_args["show_title"]
        ? "<div class='filename new'><span class='title'>" .
            wp_html_excerpt($display_title, 60, "&hellip;") .
            "</span></div>"
        : "";

    $gallery =
        (isset($_REQUEST["tab"]) && "gallery" === $_REQUEST["tab"]) ||
        (isset($redir_tab) && "gallery" === $redir_tab);
    $order = "";
    ...
    foreach ($form_fields as $key => $val) {
        if ("menu_order" === $key) {
            if ($gallery) {
                $order =
                    "<div class='menu_order'> <input class='menu_order_input' type='text' id='attachments[$attachment_id][menu_order]' name='attachments[$attachment_id][menu_order]' value='" .
                    esc_attr($val["value"]) .
                    "' /></div>";
            } else {
                $order =
                    "<input type='hidden' name='attachments[$attachment_id][menu_order]' value='" .
                    esc_attr($val["value"]) .
                    "' />";
            }
    ...
    if (
        ...
        wp_image_editor_supports(["mime_type" => $post->post_mime_type])
    ) {
        $nonce = wp_create_nonce("image_editor-$post->ID");
        $image_edit_button =
            "<input type='button' id='imgedit-open-btn-$post->ID' onclick='imageEdit.open( $post->ID, \"$nonce\" )' class='button' value='" .
    ...
    $item =
        ...
        "</strong> $filename</p>
			<p><strong>" .
        __("File type:") .
        "</strong> $post->post_mime_type</p>
			<p><strong>" .
        __("Upload date:") .
        "</strong> " .
        mysql2date(__("F j, Y"), $post->post_date) .
        "</p>";

    if (!empty($media_dims)) {
        $item .=
            "<p><strong>" . __("Dimensions:") . "</strong> $media_dims</p>\n";
    }
    ...
    if ($parsed_args["send"]) {
        $parsed_args["send"] = get_submit_button(
            __("Insert into Post"),
            "",
            "send[$attachment_id]",
            false
    ...
    if ($delete && current_user_can("delete_post", $attachment_id)) {
        if (!EMPTY_TRASH_DAYS) {
            $delete =
                "<a href='" .
                wp_nonce_url(
                    "post.php?action=delete&amp;post=$attachment_id",
                    "delete-post_" . $attachment_id
                ) .
            ...
            $delete =
                "<a href='" .
                wp_nonce_url(
                    "post.php?action=trash&amp;post=$attachment_id",
                    "trash-post_" . $attachment_id
                ) .
			...
			<a href='" .
                wp_nonce_url(
                    "post.php?action=untrash&amp;post=$attachment_id",
                    "untrash-post_" . $attachment_id
                ) .
    ...
    if (
        ...
        $calling_post_type_object = get_post_type_object(
            $calling_post->post_type
        );

        $ajax_nonce = wp_create_nonce("set_post_thumbnail-$calling_post_id");
        $thumbnail =
            "<a class='wp-post-thumbnail' id='wp-post-thumbnail-" .
            $attachment_id .
    ...
    foreach ($form_fields as $id => $field) {
        ...
        if (!empty($field[$field["input"]])) {
            ...
            $item .=
                "<textarea id='$name' name='$name'{$required_attr}>" .
                $field["value"] .
                "</textarea>";
        } else {
            $item .=
...
function get_compat_media_markup($attachment_id, $args = null)
{
    ...
    foreach ($form_fields as $id => $field) {
        ...
        if (
            (!$field["show_in_edit"] && !$args["in_modal"]) ||
            (!$field["show_in_modal"] && $args["in_modal"])
        ) {
            continue;
        }
        ...
        $readonly =
            !$user_can_edit && !empty($field["taxonomy"])
                ? " readonly='readonly' "
                : "";
        $required = $field["required"]
            ? " " . wp_required_field_indicator()
        ...
        if (!empty($field[$field["input"]])) {
            ...
            $item .=
                "<textarea id='$id_attr' name='$name'{$required_attr}>" .
                $field["value"] .
                "</textarea>";
        } else {
            $item .=
                "<input type='text' class='text' id='$id_attr' name='$name' value='" .
                esc_attr($field["value"]) .
...
function media_upload_form($errors = null)
{
	...
	<?php if (
     isset($errors["upload_error"]) &&
     is_wp_error($errors["upload_error"])
 ) {
 ...
 $prevent_unsupported_uploads = apply_filters(
 ...
 do_action(
	...
	<div id="drag-drop-area">
		...
		<p class="drag-drop-buttons"><input id="plupload-browse-button" type="button" value="<?php esc_attr_e(
      "Select Files"
  ); ?>" class="button" /></p>
		</div>
	</div>
 ...
 do_action(
   ...
   _ex("Upload", "verb"); ?>
		...
		<a href="#" onclick="try{top.tb_remove();}catch(e){}; return false;"><?php _e(
      "Cancel"
  ); ?></a>
	</p>
	<div class="clear"></div>
...
function media_upload_type_form($type = "file", $errors = null, $id = null)
{
    ...
    $form_action_url = admin_url(
        "media-upload.php?type=$type&tab=type&post_id=$post_id"
    );
    ...
    ?>
	<form enctype="multipart/form-data" method="post" action="<?php echo esc_url(
     $form_action_url
	...
	<?php if ($id) {
     if (!is_wp_error($id)) {
         ...
         echo '<div id="media-upload-error">' .
...
function media_upload_type_url_form($type = null, $errors = null, $id = null)
{
	...
	insert : function() {
		...
		<?php /** This filter is documented in wp-admin/includes/media.php */
  if (!apply_filters("disable_captions", "")) { ?>
	...
	resetImageData : function() {
		...
		else document.getElementById('status_img').innerHTML = '<img src="<?php echo esc_url(
	...
	updateImageData : function() {
		...
		document.getElementById('status_img').innerHTML = '<img src="<?php echo esc_url(
	...
	getImageData : function() {
		...
		document.getElementById('status_img').innerHTML = '<img src="<?php echo esc_url(
      admin_url("images/spinner-2x.gif")
...
function media_upload_gallery_form($errors)
{
    ...
    $post_id = (int) $_REQUEST["post_id"];
    $form_action_url = admin_url(
        "media-upload.php?type=$type&tab=gallery&post_id=$post_id"
    );
    /** This filter is documented in wp-admin/includes/media.php */
    $form_action_url = apply_filters(
        "media_upload_form_url",
	...
	<input type="hidden" name="tab" value="<?php echo esc_attr(
     $GLOBALS["tab"]
 ); ?>" />
	</p>
	...
	<table id="basic" class="describe"><tbody>
		...
		<td class="field">
			<select id="orderby" name="orderby">
				<option value="menu_order" selected="selected"><?php _e(
        "Menu order"
    ); ?></option>
	...
	<p class="ml-submit">
	<input type="button" class="button" style="display:none;" onMouseDown="wpgallery.update();" name="insert-gallery" id="insert-gallery" value="<?php esc_attr_e(
     "Insert gallery"
 ); ?>" />
	<input type="button" class="button" style="display:none;" onMouseDown="wpgallery.update();" name="update-gallery" id="update-gallery" value="<?php esc_attr_e(
...
function media_upload_library_form($errors)
{
    ...
    $form_action_url = admin_url(
        "media-upload.php?type=$type&tab=library&post_id=$post_id"
    );
    /** This filter is documented in wp-admin/includes/media.php */
    $form_action_url = apply_filters(
        "media_upload_form_url",
	...
	<input type="hidden" name="post_mime_type" value="<?php echo isset(
     $_GET["post_mime_type"]
 )
     ? esc_attr($_GET["post_mime_type"])
     : ""; ?>" />
	<input type="hidden" name="context" value="<?php echo isset($_GET["context"])
     ? esc_attr($_GET["context"])
     : ""; ?>" />
  ...
  if (
      ...
      (1 !== $month_count || 0 !== (int) $months[0]->month)
			...
			<select name='m'>
				<option<?php selected($selected_month, 0); ?> value='0'><?php _e(
     "All dates"
 ); ?></option>
...
function media_upload_max_image_resize()
{
	...
	<?php /* translators: 1: Link start tag, 2: Link end tag, 3: Width, 4: Height. */
 printf(
     __(
     ...
     $a,
     $end,
     (int) get_option("large_size_w", "1024"),
     (int) get_option("large_size_h", "1024")
 ); ?>
	</label></p>
	<?php
}
...
function edit_form_image_editor($post)
{
	...
	<?php if (wp_attachment_is_image($post->ID)):
     ...
     ?>
		...
		<div<?php echo $open_style; ?> class="wp_attachment_image wp-clearfix" id="media-head-<?php echo $attachment_id; ?>">
			<p id="thumbnail-head-<?php echo $attachment_id; ?>"><img class="thumbnail" src="<?php echo set_url_scheme(
    $thumb_url[0]
 ...
 elseif (isset($thumb_url[0])): ?>
		<div class="wp_attachment_image wp-clearfix" id="media-head-<?php echo $attachment_id; ?>">
			<p id="thumbnail-head-<?php echo $attachment_id; ?>">
				<img class="thumbnail" src="<?php echo set_url_scheme(
        $thumb_url[0]
    ); ?>" style="max-width:100%" alt="" />
			</p>
		</div>
		<?php else:do_action("wp_edit_form_attachment_display", $post);endif; ?>
	</div>
	<div class="wp_attachment_details edit-form-section">
	<?php if (str_starts_with($post->post_mime_type, "image")): ?>
		<p class="attachment-alt-text">
			<label for="attachment_alt"><strong><?php _e(
       "Alternative Text"
   ); ?></strong></label><br />
			<textarea class="widefat" name="_wp_attachment_image_alt" id="attachment_alt" aria-describedby="alt-text-description"><?php echo esc_attr(
       $alt_text
   ); ?></textarea>
		</p>
	...
	<?php endif; ?>
		...
		<p>
			<label for="attachment_caption"><strong><?php _e(
       "Caption"
   ); ?></strong></label><br />
 ...
 ?>
	...
	<label for="attachment_content" class="attachment-content-description"><strong><?php _e(
     "Description"
 ); ?></strong>
	<?php if (preg_match("#^(audio|video)/#", $post->post_mime_type)) {
     echo ": " . __("Displayed on attachment pages.");
 } ?>
	</label>
	<?php wp_editor(
     format_to_edit($post->post_content),
     "attachment_content",
     $editor_args
 ); ?>
...
function attachment_submitbox_metadata()
{
    ...
    if (isset($meta["width"], $meta["height"])) {
        /* translators: 1: A number of pixels wide, 2: A number of pixels tall. */
        $media_dims .=
            "<span id='media-dims-$attachment_id'>" .
            sprintf(
                __('%1$s by %2$s pixels'),
                $meta["width"],
                $meta["height"]
    ...
    if ($author->exists()) {
        $uploaded_by_name = $author->display_name
            ? $author->display_name
            : $author->nickname;
	...
	<?php if ($post->post_parent) {
     ...
     if ($post_parent) {
         ...
         $uploaded_to_title = $post_parent->post_title
	...
	<div class="misc-pub-section misc-pub-attachment">
		...
		<input type="text" class="widefat urlfield" readonly="readonly" name="attachment_url" id="attachment_url" value="<?php echo esc_attr(
      $att_url
  ); ?>" />
		<span class="copy-to-clipboard-container">
			<button type="button" class="button copy-attachment-url edit-media" data-clipboard-target="#attachment_url"><?php _e(
	...
	<div class="misc-pub-section misc-pub-download">
		<a href="<?php echo esc_attr($att_url); ?>" download><?php _e(
    "Download file"
); ?></a>
	</div>
	...
	<div class="misc-pub-section misc-pub-filetype">
		...
		<strong>
		<?php if (
      preg_match('/^.*?\.(\w+)$/', get_attached_file($post->ID), $matches)
  ) {
 ...
 if (!empty($file_size)) { ?>
		<div class="misc-pub-section misc-pub-filesize">
			<?php _e("File size:"); ?> <strong><?php echo size_format(
     $file_size
 ); ?></strong>
		</div>
		<?php }
 ...
 if (preg_match("#^(audio|video)/#", $post->post_mime_type)) {
     ...
     foreach ($fields as $key => $label) {
         if (empty($meta[$key])) {
			...
			<div class="misc-pub-section misc-pub-mime-meta misc-pub-<?php echo sanitize_html_class(
       $key
   ); ?>">
				<?php echo $label; ?>
				<strong>
     ...
     foreach ($audio_fields as $key => $label) {
         if (empty($meta["audio"][$key])) {
			...
			<div class="misc-pub-section misc-pub-audio misc-pub-<?php echo sanitize_html_class(
       $key
   ); ?>">
				<?php echo $label; ?> <strong><?php echo esc_html(
     $meta["audio"][$key]
 ); ?></strong>
			</div>
			<?php
 ...
 if (!empty($meta["original_image"])) { ?>
        ...
        wp_basename(wp_get_original_image_path($attachment_id))
    ); ?></strong>
			</a>
		</div>
		<?php }
        ...
        if (!empty($data[$version]["comments"])) {
            foreach ($data[$version]["comments"] as $key => $list) {
                if ("length" !== $key && !empty($list)) {
                    ...
                    if (
                        "terms_of_use" === $key &&
                        str_starts_with($metadata[$key], "yright notice.")
                    ) {
...
function wp_read_video_metadata($file)
{
    ...
    $file_format = isset($metadata["fileformat"])
        ? $metadata["fileformat"]
        : null;
...
function wp_get_media_creation_timestamp($metadata)
{
    ...
    switch ($metadata["fileformat"]) {
        case "asf":
            if (
                isset(
                    $metadata["asf"]["file_properties_object"][
                        "creation_date_unix"
                    ]
                )
            ) {
                $creation_date =
                    (int) $metadata["asf"]["file_properties_object"][
                        "creation_date_unix"
                    ];
            }
            break;
        ...
        case "webm":
            if (isset($metadata["matroska"]["comments"]["creation_time"][0])) {
                $creation_date = strtotime(
                    $metadata["matroska"]["comments"]["creation_time"][0]
                );
            } elseif (isset($metadata["matroska"]["info"][0]["DateUTC_unix"])) {
                $creation_date =
                    (int) $metadata["matroska"]["info"][0]["DateUTC_unix"];
            }
            break;
        ...
        case "mp4":
            if (
                isset(
                    $metadata["quicktime"]["moov"]["subatoms"][0][
                        "creation_time_unix"
                    ]
                )
            ) {
                $creation_date =
                    (int) $metadata["quicktime"]["moov"]["subatoms"][0][
                        "creation_time_unix"
                    ];
            }
            break;
...
function wp_media_attach_action($parent_id, $action = "attach")
{
    ...
    if (!empty($ids)) {
        ...
        if ("attach" === $action) {
            $result = $wpdb->query(
                $wpdb->prepare(
                    "UPDATE $wpdb->posts SET post_parent = %d WHERE post_type = 'attachment' AND ID IN ( $ids_string )",
                    $parent_id
                )
            );
        } else {
            $result = $wpdb->query(
                "UPDATE $wpdb->posts SET post_parent = 0 WHERE post_type = 'attachment' AND ID IN ( $ids_string )"
            );
        }
    }
