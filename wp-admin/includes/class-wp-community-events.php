...
class WP_Community_Events
{
    ...
    public function get_events($location_search = "", $timezone = "")
    {
        ...
        $request_args["user-agent"] =
            "WordPress/" . $wp_version . "; " . home_url("/");
        ...
        if (is_wp_error($response)) {
            ...
            !isset($response_body["location"], $response_body["events"])
        ) {
            $response_error = new WP_Error(
                "api-invalid-response",
                isset($response_body["error"])
                    ? $response_body["error"]
                    : __("Unknown API error.")
            );
        }
        ...
        if (is_wp_error($response_error)) {
            ...
            if (
                ...
                $response_body["location"]["description"] =
                    $this->user_location["description"];
            }
            ...
            $response_body["events"] = $this->trim_events(
    ...
    protected function get_request_args($search = "", $timezone = "")
    {
        ...
        if (
            empty($search) &&
            isset(
                $this->user_location["latitude"],
                $this->user_location["longitude"]
            )
        ) {
    ...
    protected function get_events_transient_key($location)
    {
        ...
        if (isset($location["ip"])) {
            ...
            $key =
                "community-events-" .
                md5($location["latitude"] . $location["longitude"]);
        }
    ...
    public function get_cached_events()
    {
        ...
        if (isset($cached_response["events"])) {
            $cached_response["events"] = $this->trim_events(
            ...
            foreach ($response_body["events"] as $key => $event) {
                ...
                if (isset($event["end_date"])) {
                    ...
                    if (
                        ...
                        $formatted_date = wp_maybe_decline_date(
                            $formatted_date,
                            "F j, Y"
                        );
                    }
    ...
    protected function trim_events(array $events)
    {
        ...
        foreach ($events as $event) {
            ...
            if (time() < $end_time) {
                ...
                $event["title"] = html_entity_decode(
                    $event["title"],
                    ENT_QUOTES,
                    "UTF-8"
                );
        ...
        $future_wordcamps = array_filter($future_events, static function (
            $wordcamp
        ) {
        ...
        if (
            $future_wordcamps &&
            !in_array("wordcamp", $trimmed_event_types, true)
        ) {
