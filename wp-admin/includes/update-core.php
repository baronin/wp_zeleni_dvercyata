...
function update_core($from, $to)
{
     ...
     */
    $versions_file =
        trailingslashit($wp_filesystem->wp_content_dir()) .
    ...
    if (function_exists("get_core_checksums")) {
        ...
        $working_dir_local =
            WP_CONTENT_DIR . "/upgrade/" . basename($from) . $distro;

        $checksums = get_core_checksums(
            $wp_version,
    ...
    if ($check_is_writable && "direct" === $wp_filesystem->method) {
        $files_writable = array_filter($check_is_writable, [
            $wp_filesystem,
            "is_writable",
        ]);

        if ($files_writable !== $check_is_writable) {
            $files_not_writable = array_diff_key(
                $check_is_writable,
                $files_writable
            );
            ...
            foreach (
                $files_not_writable
                as $relative_file_not_writable => $file_not_writable
            ) {
            ...
            $error_data = version_compare($old_wp_version, "3.7-beta2", ">")
                ? array_keys($files_not_writable)
                : "";
    ...
    if (
        !is_wp_error($result) &&
        $wp_filesystem->is_dir($from . $distro . "wp-content/languages")
    ) {
        if (
        ...
        if (@is_dir($lang_dir)) {
            ...
            if ($wp_lang_dir) {
                $result = copy_dir(
                    $from . $distro . "wp-content/languages/",
                    $wp_lang_dir
                );
    ...
    if ("3.5" === $old_wp_version) {
        if (
            is_dir(WP_CONTENT_DIR . "/themes/twentytwelve") &&
            !file_exists(WP_CONTENT_DIR . "/themes/twentytwelve/style.css")
        ) {
            $wp_filesystem->delete(
                $wp_filesystem->wp_themes_dir() . "twentytwelve/"
            );
        }
    }
    ...
    if (
        ...
        foreach ((array) $_new_bundled_files as $file => $introduced_version) {
            ...
            if (
                $development_build ||
                version_compare($introduced_version, $old_wp_version, ">")
            ) {
                ...
                if (!$directory) {
                    if (
                        !$development_build &&
                        $wp_filesystem->exists($dest . $filename)
                    ) {
                        continue;
                    }
                    ...
                    if (
                        !$development_build &&
                        $wp_filesystem->is_dir($dest . $filename)
                    ) {
                        continue;
                    }
    ...
    foreach ($_old_files as $old_file) {
        ...
        if (
            ...
            $wp_filesystem->is_file($old_file)
        ) {
...
function _upgrade_422_remove_genericons()
{
    ...
    foreach ($wp_theme_directories as $directory) {
        $affected_theme_files = _upgrade_422_find_genericons_files_in_folder(
            $directory
        );
    ...
    // Plugins.
    $affected_plugin_files = _upgrade_422_find_genericons_files_in_folder(
        WP_PLUGIN_DIR
    );
...
function _upgrade_422_find_genericons_files_in_folder($directory)
{
    ...
    if ($dirs) {
        foreach ($dirs as $dir) {
            $files = array_merge(
                $files,
                _upgrade_422_find_genericons_files_in_folder($dir)
            );
        }
    }
...
function _upgrade_440_force_deactivate_incompatible_plugins()
{
    if (
        ...
        version_compare(REST_API_VERSION, "2.0-beta4", "<=")
    ) {
...
function _upgrade_core_deactivate_incompatible_plugins()
{
    if (
        ...
        version_compare(GUTENBERG_VERSION, "17.6", "<")
    ) {
        ...
        if (is_plugin_active_for_network("gutenberg/gutenberg.php")) {
            $deactivated_plugins = get_site_option(
            ...
            $deactivated_plugins = get_option(
                "wp_force_deactivated_plugins",
                []
            );
            $deactivated_plugins = array_merge(
