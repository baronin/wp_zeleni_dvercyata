...
class WP_Plugins_List_Table extends WP_List_Table
{
    ...
    public function __construct($args = [])
    {
        ...
        if (
            ...
            in_array($_REQUEST["plugin_status"], $allowed_statuses, true)
        ) {
    ...
    public function prepare_items()
    {
            ...
            if (current_user_can("update_plugins")) {
                ...
                foreach (
                    ...
                    if (isset($current->response[$plugin_file])) {
                        ...
                        $plugins["upgrade"][$plugin_file] =
                            $plugins["all"][$plugin_file];
                    }
                }
            }
        }
        ...
        if (!$screen->in_admin("network")) {
             ...
             */
            $show_network_active = apply_filters(
        ...
        foreach ((array) $plugins["all"] as $plugin_file => $plugin_data) {
            ...
            if (isset($plugin_info->response[$plugin_file])) {
                $plugin_data = array_merge(
                    (array) $plugin_info->response[$plugin_file],
                    ["update-supported" => true],
                    $plugin_data
                );
            } elseif (isset($plugin_info->no_update[$plugin_file])) {
                $plugin_data = array_merge(
                    (array) $plugin_info->no_update[$plugin_file],
                    ["update-supported" => true],
                    $plugin_data
                );
            ...
            $filter_payload = (object) wp_parse_args(
                $plugin_data,
                $filter_payload
            );
            ...
            if ($this->show_autoupdates) {
                $enabled =
                    in_array($plugin_file, $auto_updates, true) &&
                    $plugin_data["update-supported"];
        ...
        if (strlen($s)) {
            $status = "search";
            $plugins["search"] = array_filter($plugins["all"], [
                $this,
                "_search_callback",
            ]);
        }
        ...
        if (
            empty($plugins[$status]) &&
            !in_array($status, ["all", "search"], true)
        ) {
            $status = "all";
        }
        ...
        if (!empty($_REQUEST["s"])) {
            ...
            if (!is_multisite() && current_user_can("install_plugins")) {
                echo ' <a href="' .
                    esc_url(
                        admin_url(
                            "plugin-install.php?tab=search&s=" . urlencode($s)
                        )
                    ) .
                    '">' .
                    __(
    ...
    public function search_box($text, $input_id)
    {
		...
		<p class="search-box">
			...
			<input type="search" id="<?php echo esc_attr(
       $input_id
   ); ?>" class="wp-filter-search" name="s" value="<?php _admin_search_query(); ?>" />
			<?php submit_button($text, "hide-if-js", "", false, [
       "id" => "search-submit",
   ]); ?>
		</p>
		<?php
    ...
    public function get_columns()
    {
        ...
        $columns = [
            "cb" => !in_array($status, ["mustuse", "dropins"], true)
                ? '<input type="checkbox" />'
                : "",
            "name" => __("Plugin"),
    ...
    protected function get_bulk_actions()
    {
        ...
        if ("active" !== $status) {
            $actions["activate-selected"] = $this->screen->in_admin("network")
                ? _x("Network Activate", "plugin")
                : _x("Activate", "plugin");
        }

        if ("inactive" !== $status && "recent" !== $status) {
            $actions["deactivate-selected"] = $this->screen->in_admin("network")
                ? _x("Network Deactivate", "plugin")
                : _x("Deactivate", "plugin");
        }
        ...
        if (!is_multisite() || $this->screen->in_admin("network")) {
            ...
            if ($this->show_autoupdates) {
                if ("auto-update-enabled" !== $status) {
                    $actions["enable-auto-update-selected"] = __(
                        "Enable Auto-updates"
                    );
                }
                if ("auto-update-disabled" !== $status) {
                    $actions["disable-auto-update-selected"] = __(
    ...
    protected function extra_tablenav($which)
    {
        ...
        if (
            !in_array(
                $status,
                ["recently_activated", "mustuse", "dropins"],
                true
            )
        ) {
            return;
    ...
    public function single_row($item)
    {
        ...
        $plugin_slug = isset($plugin_data["slug"])
            ? $plugin_data["slug"]
        ...
        $requires_php = isset($plugin_data["RequiresPHP"])
            ? $plugin_data["RequiresPHP"]
            : null;
        $requires_wp = isset($plugin_data["RequiresWP"])
        ...
        $has_active_dependents = WP_Plugin_Dependencies::has_active_dependents(
            $plugin_file
        );
        $has_unmet_dependencies = WP_Plugin_Dependencies::has_unmet_dependencies(
            $plugin_file
        );
        $has_circular_dependency = WP_Plugin_Dependencies::has_circular_dependency(
            $plugin_file
        );
        ...
        if ("mustuse" === $context) {
            ...
            if (true === $dropins[$plugin_file][1]) {
                ...
                $is_active = true;
                $description =
                    "<p><strong>" . $dropins[$plugin_file][0] . "</strong></p>";
            } elseif (
                defined($dropins[$plugin_file][1]) &&
                constant($dropins[$plugin_file][1])
            ) {
                // Constant is true.
                $is_active = true;
                $description =
            ...
            if ($screen->in_admin("network")) {
                if ($is_active) {
                    ...
                    if (
                        ...
                        if ($has_dependents && !$has_circular_dependency) {
                            ...
                            $delete_url =
                                "plugins.php?action=delete-selected" .
                                "&amp;checked[]=" .
                                urlencode($plugin_file) .
                                "&amp;plugin_status=" .
                                $context .
                ...
                if ($restrict_network_active) {
                    ...
                    if (
                        current_user_can("resume_plugin", $plugin_file) &&
                        is_plugin_paused($plugin_file)
                    ) {
                        $resume_url =
        ...
        if (
            (!empty($totals["upgrade"]) && !empty($plugin_data["update"])) ||
            !$compatible_php ||
            !$compatible_wp
        ) {
            $class .= " update";
        ...
        $paused =
            !$screen->in_admin("network") && is_plugin_paused($plugin_file);
        ...
        foreach ($columns as $column_name => $column_display_name) {
            ...
            switch ($column_name) {
                ...
                case "description":
                    ...
                    if ($has_dependents) {
                        $this->add_dependents_to_dependency_plugin_row(
                            $plugin_file
                        );
                    }

                    if (
                        WP_Plugin_Dependencies::has_dependencies($plugin_file)
                    ) {
                        $this->add_dependencies_to_dependent_plugin_row(
                            $plugin_file
                        );
                    }
        ...
        if (!$compatible_php || !$compatible_wp) {
            ...
            wp_admin_notice($incompatible_message, [
                "type" => "error",
                "additional_classes" => [
                    "notice-alt",
                    "inline",
    ...
    {
        $dependent_names = WP_Plugin_Dependencies::get_dependent_names(
            $dependency
        );
    ...
    protected function add_dependencies_to_dependent_plugin_row($dependent)
    {
        ...
        $is_active = is_multisite()
            ? is_plugin_active_for_network($dependent)
            : is_plugin_active($dependent);
        $comma = wp_get_list_item_separator();
        $requires = sprintf(
        ...
        if (WP_Plugin_Dependencies::has_unmet_dependencies($dependent)) {
            ...
            $notice = wp_get_admin_notice($error_message, [
                "type" => "error",
