...
function edit_user($user_id = 0)
{
    ...
    if (isset($_POST["url"])) {
        if (empty($_POST["url"]) || "http://" === $_POST["url"]) {
            ...
            $protocols = implode(
                "|",
                array_map("preg_quote", wp_allowed_protocols())
            );
    ...
    if ($update) {
        $user->rich_editing =
            isset($_POST["rich_editing"]) && "false" === $_POST["rich_editing"]
                ? "false"
                : "true";
        $user->syntax_highlighting =
            isset($_POST["syntax_highlighting"]) &&
    ...
    if ($update && empty($user->nickname)) {
        $errors->add(
            "nickname",
            __("<strong>Error:</strong> Please enter a nickname.")
        );
    }
    ...
    if (!$update && empty($pass1)) {
        $errors->add(
            "pass",
            __("<strong>Error:</strong> Please enter a password."),
            ["form-field" => "pass1"]
        );
    }
     ...
     */
    do_action_ref_array("user_profile_update_errors", [
        &$errors,
        $update,
        &$user,
    ]);
    ...
    if (null === $reassign) {
        ...
        foreach (get_post_types([], "objects") as $post_type) {
            if ($post_type->delete_with_user) {
                ...
                post_type_supports($post_type->name, "author")
            ) {
                $post_types_to_delete[] = $post_type->name;
            }
        }
         ...
         * @param int      $id                   User ID.
         */
        $post_types_to_delete = apply_filters(
...
function default_password_nag_handler($errors = false)
{
    ...
    if (
        "hide" === get_user_setting("default_password_nag") ||
        (isset($_GET["default_password_nag"]) &&
...
function default_password_nag()
{
    ...
    if (
        "profile.php" === $pagenow ||
        !get_user_option("default_password_nag")
    ) {
        return;
    }
    ...
    wp_admin_notice($default_password_nag_message, [
...
function wp_is_authorize_application_password_request_valid($request, $user)
{
    ...
    if (isset($request["success_url"])) {
        $validated_success_url = wp_is_authorize_application_redirect_url_valid(
            $request["success_url"]
        );
    ...
    if (isset($request["reject_url"])) {
        $validated_reject_url = wp_is_authorize_application_redirect_url_valid(
            $request["reject_url"]
        );
...
function wp_is_authorize_application_redirect_url_valid($url)
{
     ...
     * @param string   $url The redirect URL to be validated.
     */
    $invalid_protocols = apply_filters(
    ...
    if (
        empty($host) ||
        empty($scheme) ||
        in_array(strtolower($scheme), $invalid_protocols, true)
    ) {
        return new WP_Error(
            "invalid_redirect_url_format",
            __("Invalid URL format.")
