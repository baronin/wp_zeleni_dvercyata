...
function get_plugin_data($plugin_file, $markup = true, $translate = true)
{
    ...
    if (!$plugin_data["Network"] && $plugin_data["_sitewide"]) {
        ...
        _deprecated_argument(
            ...
            "3.0.0",
            sprintf(
                __('The %1$s plugin header is deprecated. Use %2$s instead.'),
...
function _get_plugin_data_markup_translate(
     ...
     */
    $plugin_data["Name"] = wp_kses(
        $plugin_data["Name"],
        $allowed_tags_in_links
    );
    $plugin_data["Author"] = wp_kses($plugin_data["Author"], $allowed_tags);

    $plugin_data["Description"] = wp_kses(
...
function get_mu_plugins()
{
    ...
    if (
        isset($wp_plugins["index.php"]) &&
        filesize(WPMU_PLUGIN_DIR . "/index.php") <= 30
    ) {
        // Silence is golden.
...
function _get_dropins()
{
    $dropins = [
        ...
        "fatal-error-handler.php" => [
            __("Custom PHP fatal error handler."),
            true,
        ], // Auto on error.
    ];
    ...
    if (is_multisite()) {
        $dropins["sunrise.php"] = [
            __("Executed before Multisite is loaded."),
            "SUNRISE",
        ]; // SUNRISE
        $dropins["blog-deleted.php"] = [
            __("Custom site deleted message."),
            true,
        ]; // Auto on deleted blog.
        $dropins["blog-inactive.php"] = [
            __("Custom site inactive message."),
            true,
...
function deactivate_plugins($plugins, $silent = false, $network_wide = null)
{
    ...
    foreach ((array) $plugins as $plugin) {
        ...
        $network_deactivating =
            false !== $network_wide && is_plugin_active_for_network($plugin);
...
function activate_plugins(
    ...
    if (!empty($errors)) {
        return new WP_Error(
            "plugins_invalid",
            __("One of the plugins is invalid."),
            $errors
        );
    }
...
function delete_plugins($plugins, $deprecated = "")
{
    ...
    $url = wp_nonce_url(
        "plugins.php?action=delete-selected&verify-delete=1&" .
            implode("&", $checked),
        "bulk-plugins"
    );
    ...
    if (empty($plugins_dir)) {
        return new WP_Error(
            "fs_no_plugins_dir",
            __("Unable to locate WordPress plugin directory.")
        );
    }
    ...
    foreach ($plugins as $plugin_file) {
        ...
        $this_plugin_dir = trailingslashit(
            dirname($plugins_dir . $plugin_file)
        );
        ...
        if (
            "." !== $plugin_slug &&
            !empty($plugin_translations[$plugin_slug])
        ) {
    ...
    if (!empty($errors)) {
        ...
        return new WP_Error(
            ...
            sprintf($message, implode(", ", $errors))
        );
    }
...
function validate_plugin_requirements($plugin)
{
    ...
    $requirements = [
        "requires" => !empty($plugin_headers["RequiresWP"])
            ? $plugin_headers["RequiresWP"]
            : "",
        "requires_php" => !empty($plugin_headers["RequiresPHP"])
            ? $plugin_headers["RequiresPHP"]
            : "",
        "requires_plugins" => !empty($plugin_headers["RequiresPlugins"])
            ? $plugin_headers["RequiresPlugins"]
            : "",
    ];
        ...
        return new WP_Error(
            ...
            "<p>" .
                sprintf(
                    ...
                    _x(
                        '<strong>Error:</strong> Current versions of WordPress (%1$s) and PHP (%2$s) do not meet minimum requirements for %3$s. The plugin requires WordPress %4$s and PHP %5$s.',
                        "plugin"
                    ),
        ...
        return new WP_Error(
            ...
            "<p>" .
                sprintf(
                    ...
                    _x(
                        '<strong>Error:</strong> Current PHP version (%1$s) does not meet minimum requirements for %2$s. The plugin requires PHP %3$s.',
                        "plugin"
                    ),
                    PHP_VERSION,
                    $plugin_headers["Name"],
        ...
        return new WP_Error(
            ...
            "<p>" .
                sprintf(
                    ...
                    _x(
                        '<strong>Error:</strong> Current WordPress version (%1$s) does not meet minimum requirements for %2$s. The plugin requires WordPress %3$s.',
                        "plugin"
                    ),
                    get_bloginfo("version"),
                    $plugin_headers["Name"],
    ...
    if (WP_Plugin_Dependencies::has_unmet_dependencies($plugin)) {
        $dependency_names = WP_Plugin_Dependencies::get_dependency_names(
            $plugin
        );
        $unmet_dependencies = [];
        ...
        foreach ($dependency_names as $dependency => $dependency_name) {
            $dependency_file = WP_Plugin_Dependencies::get_dependency_filepath(
                $dependency
            );

            if (false === $dependency_file) {
                $unmet_dependencies["not_installed"][
                    $dependency
                ] = $dependency_name;
        ...
        if (is_multisite()) {
            if (current_user_can("manage_network_plugins")) {
                ...
                $error_message .=
                    " " . __("Please contact your network administrator.");
            }
        } else {
            $error_message .=
                " " .
...
function is_uninstallable_plugin($plugin)
{
    ...
    if (
        isset($uninstallable_plugins[$file]) ||
        file_exists(WP_PLUGIN_DIR . "/" . dirname($file) . "/uninstall.php")
    ) {
        return true;
    }
...
function add_submenu_page(
    ...
    if (!isset($submenu[$parent_slug]) && $menu_slug !== $parent_slug) {
        foreach ((array) $menu as $parent_menu) {
            if (
                $parent_menu[2] === $parent_slug &&
                current_user_can($parent_menu[1])
            ) {
    ...
    if (
        ...
        (!isset($submenu[$parent_slug]) ||
            $position >= count($submenu[$parent_slug]))
    ) {
    ...
    if ("tools.php" === $parent_slug) {
        $_registered_pages[
            get_plugin_page_hookname($menu_slug, "edit.php")
        ] = true;
    }
...
function get_admin_page_parent($parent_page = "")
{
    ...
    if (
        isset($plugin_page) &&
        isset($_wp_submenu_nopriv[$pagenow][$plugin_page])
    ) {
        $parent_file = $pagenow;
    ...
    foreach (array_keys((array) $submenu) as $parent_page) {
        foreach ($submenu[$parent_page] as $submenu_array) {
            ...
            if (
                !empty($typenow) &&
                "$pagenow?post_type=$typenow" === $submenu_array[2]
            ) {
                $parent_file = $parent_page;
                return $parent_page;
            } elseif (
                empty($typenow) &&
                $pagenow === $submenu_array[2] &&
            ...
            foreach ($submenu[$parent] as $submenu_array) {
                if (
                    ...
                    ($pagenow === $parent ||
                        ...
                        $plugin_page === $hook ||
                        ("admin.php" === $pagenow &&
                            $parent1 !== $submenu_array[2]) ||
                        (!empty($typenow) &&
                            "$pagenow?post_type=$typenow" === $parent))
                ) {
...
function get_plugin_page_hookname($plugin_page, $parent_page)
{
    ...
    if (
        empty($parent_page) ||
        "admin.php" === $parent_page ||
        isset($admin_page_hooks[$plugin_page])
    ) {
...
function user_can_access_admin_page()
{
    ...
    if (empty($parent)) {
        ...
        foreach (array_keys($_wp_submenu_nopriv) as $key) {
            ...
            if (
                isset($plugin_page) &&
                isset($_wp_submenu_nopriv[$key][$plugin_page])
            ) {
                return false;
            }
        }
    ...
    if (
        isset($plugin_page) &&
        $plugin_page === $parent &&
        isset($_wp_menu_nopriv[$plugin_page])
    ) {
        return false;
    }
...
function add_allowed_options($new_options, $options = "")
{
    ...
    foreach ($new_options as $page => $keys) {
        foreach ($keys as $key) {
            if (
                !isset($allowed_options[$page]) ||
                !is_array($allowed_options[$page])
            ) {
...
function paused_plugins_notice()
{
    ...
    if (
        !isset($GLOBALS["_paused_plugins"]) ||
        empty($GLOBALS["_paused_plugins"])
    ) {
        return;
    }
...
function deactivated_plugins_notice()
{
    ...
    if (false === $blog_deactivated_plugins) {
        ...
        update_option("wp_force_deactivated_plugins", [], false);
    }

    if (is_multisite()) {
        $site_deactivated_plugins = get_site_option(
    ...
    $deactivated_plugins = array_merge(
    ...
    foreach ($deactivated_plugins as $plugin) {
        if (
            !empty($plugin["version_compatible"]) &&
