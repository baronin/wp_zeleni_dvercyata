...
function get_editable_user_ids(
    ...
    if (!$user->has_cap($post_type_obj->cap->edit_others_posts)) {
        if (
            $user->has_cap($post_type_obj->cap->edit_posts) ||
            !$exclude_zeros
        ) {
            return [$user->ID];
        } else {
...
if (!class_exists("WP_User_Search", false)):
    ...
    class WP_User_Search
    {
         ...
         * @return WP_User_Search
         */
        public function WP_User_Search(
            $search_term = "",
            $page = "",
            $role = ""
        ) {
        ...
        public function prepare_query()
        {
            ...
            if ($this->search_term) {
                ...
                foreach (
                    ...
                    $searches[] = $wpdb->prepare(
                        $col . " LIKE %s",
                        "%" . like_escape($this->search_term) . "%"
                    );
                }
        ...
        public function query()
        {
            ...
            if ($this->results) {
                $this->total_users_for_query = $wpdb->get_var(
            ...
            // No limit.
                ...
                $this->search_errors = new WP_Error(
        ...
        public function do_paging()
        {
            if ($this->total_users_for_query > $this->users_per_page) {
                ...
                $this->paging_text = paginate_links([
                    "total" => ceil(
                        $this->total_users_for_query / $this->users_per_page
                    ),
                    "current" => $this->page,
                    "base" => "users.php?%_%",
                    "format" => "userspage=%#%",
                ...
                if ($this->paging_text) {
                    $this->paging_text = sprintf(
                        ...
                        '<span class="displaying-num">' .
                            ...
                            "</span>%s",
                        number_format_i18n(
                            ($this->page - 1) * $this->users_per_page + 1
                        ),
                        number_format_i18n(
                            min(
                                $this->page * $this->users_per_page,
...
function get_broken_themes()
{
    _deprecated_function(
        __FUNCTION__,
        "3.4.0",
        "wp_get_themes( array( 'errors' => true )"
    );
...
function wp_dashboard_plugins_output($rss, $args = [])
{
    ...
    foreach ([$popular] as $feed) {
        ...
        while (
            null !== ($item_key = array_rand($items)) &&
            str_contains($items[$item_key]->get_description(), "Plugin Name:")
        ) {
            unset($items[$item_key]);
        }
        ...
        echo '<li class="dashboard-news-plugin"><span>' .
            ...
            '&nbsp;<a href="' .
            $ilink .
            '" class="thickbox open-plugin-details-modal" aria-label="' .
            /* translators: %s: Plugin name. */
            esc_attr(sprintf(_x("Install %s", "plugin"), $raw_title)) .
            '">(' .
            __("Install") .
            ")</a></li>";
    ...
    WP_Privacy_Data_Export_Requests_List_Table
{
    function __construct($args)
    {
        ...
        if (
            !isset($args["screen"]) ||
            $args["screen"] === "export_personal_data"
        ) {
    ...
    WP_Privacy_Data_Removal_Requests_List_Table
{
    function __construct($args)
    {
        ...
        if (
            !isset($args["screen"]) ||
            $args["screen"] === "remove_personal_data"
        ) {
