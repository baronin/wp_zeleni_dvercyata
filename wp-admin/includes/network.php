...
function network_step1($errors = false)
{
    ...
    if (defined("DO_NOT_UPGRADE_GLOBAL_TABLES")) {
        ...
        wp_admin_notice($cannot_define_constant_message, [
    ...
    if (is_wp_error($errors)) {
        ...
        wp_admin_notice($network_created_error_message, [
            "additional_classes" => ["error"],
    ...
    if (
        ...
        !in_array("empty_sitename", $error_codes, true)
    ) {
    ...
    if (
        !empty($_POST["email"]) &&
        !in_array("invalid_email", $error_codes, true)
    ) {
     ...
     if ($got_mod_rewrite) {
         ...
         $message .=
             ...
             sprintf(
                 ...
                 __(
                     "Please make sure the Apache %s module is installed as it will be used at the end of this installation."
                 ),
     ...
     if ($got_mod_rewrite || $is_apache) {
         ...
         $message .=
             ...
             sprintf(
                 ...
                 __(
                     'If %1$s is disabled, ask your administrator to enable that module, or look at the <a href="%2$s">Apache documentation</a> or <a href="%3$s">elsewhere</a> for help setting it up.'
                 ),
                 "<code>mod_rewrite</code>",
 ...
 if (allow_subdomain_install() && allow_subdirectory_install()): ?>
     ...
     ?>
		<table class="form-table" role="presentation">
			<tr>
				<th><label><input type="radio" name="subdomain_install" value="1"<?php checked(
        $subdomain_install
        ...
        $hostname
			...
			<tr>
				<th><label><input type="radio" name="subdomain_install" value="0"<?php checked(
        !$subdomain_install
    ); ?> /> <?php _e("Sub-directories"); ?></label></th>
				<td>
				<?php printf(
        /* translators: 1: Host name. */
        _x(
 ...
 if (
     ...
     $subdirectory_warning_message .= __(
         "Subdirectory networks may not be fully compatible with custom wp-content directories."
     );
     wp_admin_notice($subdirectory_warning_message, [
 ...
 ?>
    ...
    printf(
        ...
        __(
            'Because you are using %1$s, the sites in your WordPress network must use sub-directories. Consider using %2$s if you wish to use sub-domains.'
        ),
        "<code>localhost</code>",
    ...
    ?>
		...
		<?php endif; ?>
			<tr>
				<th scope='row'><label for="sitename"><?php esc_html_e(
        "Network Title"
    ); ?></label></th>
				<td>
					<input name='sitename' id='sitename' type='text' size='45' value='<?php echo esc_attr(
         $site_name
     ); ?>' />
					<p class="description">
			...
			<tr>
				<th scope='row'><label for="email"><?php esc_html_e(
        "Network Admin Email"
    ); ?></label></th>
				<td>
					<input name='email' id='email' type='text' size='45' value='<?php echo esc_attr(
         $admin_email
     ); ?>' />
					<p class="description">
...
function network_step2($errors = false)
{
    ...
    $home_path = str_starts_with($abspath_fix, $document_root_fix)
        ...
        : get_home_path();
    $wp_siteurl_subdir = preg_replace(
        "#^" . preg_quote($home_path, "#") . "#",
        "",
        $abspath_fix
    );
    $rewrite_base = !empty($wp_siteurl_subdir)
        ? ltrim(trailingslashit($wp_siteurl_subdir), "/")
        : "";
    ...
    if (is_wp_error($errors)) {
        wp_admin_notice($errors->get_error_message(), [
            "additional_classes" => ["error"],
        ]);
    }
    ...
    if ($_POST) {
        if (allow_subdomain_install()) {
            $subdomain_install = allow_subdirectory_install()
                ? !empty($_POST["subdomain_install"])
                : true;
        } else {
            $subdomain_install = false;
        }
     ...
     "The original configuration steps are shown here for reference."
            ...
            wp_admin_notice(
                "<strong>" .
                    __("Warning:") .
                    "</strong> " .
                    __("An existing WordPress network was detected."),
                [
                    "additional_classes" => ["error"],
                ]
            );
...
define( 'SUBDOMAIN_INSTALL', <?php echo $subdomain_install
    ? "true"
    : "false"; ?> );
  ...
  if (!empty($keys_salts)) {
      ...
      if (is_wp_error($from_api)) {
          ...
          foreach ($keys_salts as $c => $v) {
              $keys_salts_str .=
                  "\ndefine( '$c', '" .
                  substr(array_shift($from_api), 28, 64) .
                  "' );";
          }
      }
      ...
      ?>
       ...
       printf(
           /* translators: %s: wp-config.php */
           __(
               "This unique authentication key is also missing from your %s file."
           ),
           "<code>wp-config.php</code>"
       );
   } else {
       printf(
           /* translators: %s: wp-config.php */
           __(
               "These unique authentication keys are also missing from your %s file."
           ),
           "<code>wp-config.php</code>"
       );
   } ?>
		...
		<p class="configuration-rules-label"><label for="network-wpconfig-authentication"><?php _e(
      "Network configuration authentication keys"
  ); ?></label></p>
		<textarea id="network-wpconfig-authentication" class="code" readonly="readonly" cols="100" rows="<?php echo $num_keys_salts; ?>" aria-describedby="network-wpconfig-authentication-description"><?php echo esc_textarea(
    $keys_salts_str
); ?></textarea>
			<?php
  }
    ...
    <system.webServer>
     ...
     if (is_multisite() && get_site_option("ms_files_rewriting")) {
         $web_config_file .=
             ...
             $iis_rewrite_base .
             WPINC .
             '/ms-files.php?file={R:1}" appendQueryString="false" />
                </rule>';
     }
     $web_config_file .=
         '
...
';
     ...
     ?>
        ...
        "<code>web.config</code>"
			...
			<textarea id="network-webconfig-rules" class="code" readonly="readonly" cols="100" rows="20" aria-describedby="network-webconfig-rules-description"><?php echo esc_textarea(
       $web_config_file
   ); ?></textarea>
		</li>
	</ol>

		<?php // End iis7_supports_permalinks(). Link to Nginx documentation instead:
     /* translators: %s: Documentation URL. */ // End $is_nginx. Construct an .htaccess file instead:
     /* translators: 1: File name (.htaccess or web.config), 2: File path. */
     /* translators: %s: File name (wp-config.php, .htaccess or web.config). */
 ...
 elseif ($is_nginx):
     ...
     if (is_multisite() && get_site_option("ms_files_rewriting")) {
         ...
         $ms_files_rewriting .=
             ...
             "files/(.+) {$rewrite_base}" .
             WPINC .
             "/ms-files.php?file={$subdir_replacement_12} [L]" .
             "\n";
     }
...
EOF;
     ...
     ?>
        ...
        "<code>.htaccess</code>"
			...
			<textarea id="network-htaccess-rules" class="code" readonly="readonly" cols="100" rows="<?php echo substr_count(
       $htaccess_file,
       "\n"
   ) +
       1; ?>" aria-describedby="network-htaccess-rules-description"><?php echo esc_textarea(
    $htaccess_file
); ?></textarea>
		</li>
	</ol>
 ...
 if (!is_multisite()) { ?>
  ...
  ); ?> <a href="<?php echo esc_url(wp_login_url()); ?>"><?php _e(
    "Log In"
); ?></a></p>
		<?php }
}
