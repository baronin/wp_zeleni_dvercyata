...
class WP_Plugin_Install_List_Table extends WP_List_Table
{
    ...
    public function prepare_items()
    {
        ...
        if (
            empty($tab) ||
            (!isset($tabs[$tab]) &&
                !in_array($tab, (array) $nonmenu_tabs, true))
        ) {
            $tab = key($tabs);
        }
        ...
        if ($installed_plugins) {
            ...
            $js_plugins["all"] = array_values(
                wp_list_pluck($installed_plugins, "plugin")
            );
            $upgrade_plugins = wp_filter_object_list(
                $installed_plugins,
                ["upgrade" => true],
                "and",
                "plugin"
    ...
    public function no_items()
    {
		...
		<?php
             ...
             ?>
			<div class="no-plugin-results"><?php _e(
...
<div class="wp-filter">
    ...
    public function display()
    {
        ...
        ?>
<div class="wp-list-table <?php echo implode(
    " ",
    ...
    public function display_rows()
    {
        ...
        foreach ((array) $this->items as $plugin) {
            ...
            if (isset($plugin["group"]) && $plugin["group"] !== $group) {
                ...
                echo '<div class="plugin-group"><h3>' .
                    esc_html($group_name) .
                    "</h3>";
            ...
            $action_links[] = wp_get_plugin_action_button(
                $name,
                $plugin,
                $compatible_php,
                $compatible_wp
            ...
            ?>
		<div class="plugin-card plugin-card-<?php echo sanitize_html_class(
      $plugin["slug"]
  ); ?>">
			<?php if (!$compatible_php || !$compatible_wp) {
       $incompatible_notice_message = "";
       if (!$compatible_php && !$compatible_wp) {
           $incompatible_notice_message .= __(
   ...
   ?>
         ...
         "number" => $plugin["num_ratings"],
					...
					<span class="num-ratings" aria-hidden="true">(<?php echo number_format_i18n(
         $plugin["num_ratings"]
     ); ?>)</span>
				</div>
     ...
     if ($plugin["active_installs"] >= 1000000) {
         $active_installs_millions = floor(
             $plugin["active_installs"] / 1000000
         );
     ...
     ?>
				...
				<div class="column-compatibility">
					<?php if (!$tested_wp) {
         echo '<span class="compatibility-untested">' .
             __("Untested with your version of WordPress") .
             "</span>";
     } elseif (!$compatible_wp) {
         echo '<span class="compatibility-incompatible">' .
             __(
                 "<strong>Incompatible</strong> with your version of WordPress"
             ) .
             "</span>";
     } else {
         echo '<span class="compatibility-compatible">' .
    ...
    protected function get_dependencies_notice($plugin_data)
    {
        ...
        foreach ($plugin_data["requires_plugins"] as $dependency) {
            $dependency_data = WP_Plugin_Dependencies::get_dependency_data(
                $dependency
            );
            ...
            if (
                ...
                $more_details_link = $this->get_more_details_link(
                    $dependency_data["name"],
                    $dependency_data["slug"]
                );
                $dependencies_list .= sprintf(
                    $has_name_markup,
                    esc_html($dependency_data["name"]),
                    $more_details_link
                );
                continue;
            }
