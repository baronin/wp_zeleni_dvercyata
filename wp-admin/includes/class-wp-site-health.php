...
class WP_Site_Health
{
    ...
    public function __construct()
    {
        ...
        add_action("wp_site_health_scheduled_check", [
            $this,
    ...
    public function enqueue_scripts()
    {
        ...
        $health_check_js_variables = [
            ...
            "nonce" => [
                ...
                "site_status_result" => wp_create_nonce(
        ...
        if (
            ...
            foreach ($tests["direct"] as $test) {
                if (is_string($test["test"])) {
                    ...
                    if (
                        method_exists($this, $test_function) &&
                        is_callable([$this, $test_function])
                    ) {
                        $health_check_js_variables["site_status"][
                            "direct"
                        ][] = $this->perform_test([$this, $test_function]);
                        continue;
                    }
                }
            ...
            foreach ($tests["async"] as $test) {
                if (is_string($test["test"])) {
                    $health_check_js_variables["site_status"]["async"][] = [
                        "test" => $test["test"],
                        "has_rest" => isset($test["has_rest"])
                            ? $test["has_rest"]
                            : false,
                        "completed" => false,
                        "headers" => isset($test["headers"])
                            ? $test["headers"]
                            : [],
                    ];
                }
    ...
    private function prepare_sql_data()
    {
        ...
        if (stristr($mysql_server_type, "mariadb")) {
            ...
            $this->mysql_recommended_version =
                $this->mariadb_recommended_version;
        }

        $this->is_acceptable_mysql_version = version_compare(
    ...
    public function check_wp_version_check_exists()
    {
        if (
            ...
            !isset($_GET["health-check-test-wp_version_check"])
        ) {
            return;
        }
    ...
    public function get_test_php_version()
    {
        ...
        if (
            !$response ||
            version_compare(PHP_VERSION, $response["recommended_version"], ">=")
        ) {
            return $result;
        }
        ...
        if (
            $response["is_secure"] &&
            $response["is_lower_than_future_minimum"]
        ) {
    ...
    public function get_test_php_extensions()
    {
        ...
        foreach ($modules as $library => $module) {
            $extension_name = isset($module["extension"])
                ? $module["extension"]
                : null;
            $function_name = isset($module["function"])
                ? $module["function"]
                : null;
            $constant_name = isset($module["constant"])
                ? $module["constant"]
                : null;
            ...
            if (
                !$this->test_php_extension_availability(
                    $extension_name,
                    $function_name,
                    $constant_name,
                    $class_name
                ...
                $failures[
                    $library
                ] = "<span class='dashicons $class'><span class='screen-reader-text'>$screen_reader</span></span> $message";
            }
        }
        ...
        if (!wp_is_using_https()) {
                ...
                if (is_ssl()) {
                    $result["description"] = sprintf(
                        ...
                        sprintf(
                            ...
                            esc_url(
                                admin_url("options-general.php") . "#siteurl"
                            ),
                            esc_url(admin_url("options-general.php") . "#home")
                        )
                    );
            ...
            if (wp_is_https_supported()) {
                ...
                if (defined("WP_HOME") || defined("WP_SITEURL")) {
                    ...
                    $default_direct_update_url = add_query_arg(
                        "action",
                        "update_https",
                        wp_nonce_url(
    ...
    public function get_test_update_temp_backup_writable()
    {
        ...
        $backup_dir_exists = $wp_filesystem->is_dir(
            "$wp_content/upgrade-temp-backup"
        );
        $backup_dir_is_writable = $wp_filesystem->is_writable(
            "$wp_content/upgrade-temp-backup"
        );

        $plugins_dir_exists = $wp_filesystem->is_dir(
            "$wp_content/upgrade-temp-backup/plugins"
        );
        $plugins_dir_is_writable = $wp_filesystem->is_writable(
            "$wp_content/upgrade-temp-backup/plugins"
        );
        $themes_dir_exists = $wp_filesystem->is_dir(
            "$wp_content/upgrade-temp-backup/themes"
        );
        $themes_dir_is_writable = $wp_filesystem->is_writable(
            "$wp_content/upgrade-temp-backup/themes"
        );
        ...
        if (
            (!$plugins_dir_exists || !$themes_dir_exists) &&
            $backup_dir_exists &&
    ...
    public function get_test_page_cache()
    {
        ...
        switch ($page_cache_detail["status"]) {
            ...
            default:
                if (
                    empty($page_cache_detail["headers"]) &&
        ...
        if (empty($page_cache_detail["response_time"])) {
            ...
            if (empty($page_cache_detail["headers"])) {
                ...
                $headers_summary .=
                    " <code>" .
                    implode("</code>, <code>", $page_cache_detail["headers"]) .
                    "</code>.";
        ...
        $result["description"] .=
            "<ul><li>" .
            implode("</li><li>", $page_cache_test_summary) .
            "</li></ul>";
        return $result;
    }
    ...
    public function get_test_autoloaded_options()
    {
         ...
         */
        $result["description"] = apply_filters(
    ...
    private function get_cron_tasks()
    {
        ...
        if (empty($cron_tasks)) {
            $this->crons = new WP_Error(
                "no_tasks",
                __("No scheduled events exist on this site.")
            );
            return;
        }
    ...
    public function detect_plugin_theme_auto_update_issues()
    {
        $mock_plugin = (object) [
            ...
            "package" =>
                "https://downloads.wordpress.org/plugin/a-fake-plugin.9.9.zip",
            "icons" => [
                "2x" =>
                    "https://ps.w.org/a-fake-plugin/assets/icon-256x256.png",
                "1x" =>
                    "https://ps.w.org/a-fake-plugin/assets/icon-128x128.png",
            ],
            "banners" => [
                "2x" =>
                    "https://ps.w.org/a-fake-plugin/assets/banner-1544x500.png",
                "1x" =>
                    "https://ps.w.org/a-fake-plugin/assets/banner-772x250.png",
            ],
            "banners_rtl" => [],
            "tested" => "5.5.0",
            "requires_php" => "5.6.20",
        ...
        $mock_theme = (object) [
            ...
            "package" =>
                "https://downloads.wordpress.org/theme/a-fake-theme.9.9.zip",
            "requires" => "5.0.0",
            "requires_php" => "5.6.20",
        ];
        ...
        $test_themes_enabled = wp_is_auto_update_forced_for_item(
            "theme",
            true,
            $mock_theme
        );
        ...
        if (
            (!$test_plugins_enabled && $ui_enabled_for_plugins) ||
    ...
    public function wp_cron_scheduled_check()
    {
        ...
        foreach ($tests["async"] as $test) {
            ...
            if (is_string($test["test"])) {
                ...
                if (
                    !is_wp_error($result_fetch) &&
                    200 === wp_remote_retrieve_response_code($result_fetch)
                ) {
                    $result = json_decode(
                        wp_remote_retrieve_body($result_fetch),
                        true
                    );
                } else {
                    $result = false;
    ...
    private function check_for_page_caching()
    {
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            foreach ($caching_headers as $header => $callback) {
                $header_values = wp_remote_retrieve_header(
                    $http_response,
                    $header
                );
    ...
    private function get_page_cache_detail()
    {
        ...
        $headers = [];
        foreach (
            $page_cache_detail["page_caching_response_headers"]
            as $page_caching_response_headers
        ) {
            $headers = array_merge(
                $headers,
                array_keys($page_caching_response_headers)
            );
        }
        ...
        $has_page_caching =
            count($headers) > 0 || $page_cache_detail["advanced_cache_present"];

        if (
            $page_speed &&
            $page_speed < $this->get_good_response_time_threshold()
        ) {
