...
function wpmu_delete_blog($blog_id, $drop = false)
{
    ...
    if (
        ...
        (1 === $blog_id ||
            ...
            ($blog->path === $current_network->path &&
                $blog->domain === $current_network->domain))
    ) {
        $drop = false;
    }
...
function display_space_usage()
{
    ...
    ?>
	<strong>
	<?php /* translators: Storage space that's been used. 1: Percentage of used space, 2: Total space allowed in megabytes or gigabytes. */
...
function upload_space_setting($id)
{
    ...
    ?>
	<tr>
		<th><label for="blog-upload-space-number"><?php _e(
      "Site Upload Space Quota"
		...
		<td>
			<input type="number" step="1" min="0" style="width: 100px"
				...
				aria-describedby="blog-upload-space-desc" value="<?php echo esc_attr(
        $quota
    ); ?>" />
...
function _access_denied_splash()
{
    ...
    foreach ($blogs as $blog) {
        ...
        $output .=
            '<td><a href="' .
            esc_url(get_admin_url($blog->userblog_id)) .
            '">' .
            __("Visit Dashboard") .
            "</a> | " .
            '<a href="' .
            esc_url(get_home_url($blog->userblog_id)) .
            '">' .
            __("View Site") .
            "</a></td>";
        $output .= "</tr>";
    ...
    foreach ((array) $lang_files as $val) {
        ...
        if ("en_US" === $code_lang) {
            ...
            $output[$ae] =
                ...
                esc_attr($code_lang) .
                '"' .
                selected($current, $code_lang, false) .
                "> " .
                $ae .
                "</option>";
...
function site_admin_notice()
{
    ...
    if ((int) get_site_option("wpmu_upgrade_site") !== $wp_db_version) {
        ...
        wp_admin_notice($upgrade_network_message, [
            "type" => "warning",
...
function choose_primary_blog()
{
    ...
    ?>
		<th scope="row"><label for="primary_blog"><?php _e(
      "Primary Site"
  ); ?></label></th>
		<td>
		<?php
  $all_blogs = get_blogs_of_user(get_current_user_id());
  $primary_blog = (int) get_user_meta(
      get_current_user_id(),
      "primary_blog",
      true
  );
  if (count($all_blogs) > 1) {
    ...
    $primary_blog,
    $blog->userblog_id
); ?>><?php echo esc_url(get_home_url($blog->userblog_id)); ?></option>
					<?php
    } ?>
			</select>
			<?php if (!$found) {
...
var tb_pathToImage = "<?php echo esc_js(
    includes_url("js/thickbox/loadingAnimation.gif", "relative")
); ?>";
</script>
	<?php
}
         ...
         ?>
			<tr>
				<th scope="row"><?php echo $delete_user->user_login; ?>
					<?php echo '<input type="hidden" name="user[]" value="' .
         esc_attr($user_id) .
         '" />' .
         "\n"; ?>
				</th>
   ...
   if (!empty($blogs)) { ?>
    ...
    foreach ((array) $blogs as $key => $details) {
        ...
        if (is_array($blog_users) && !empty($blog_users)) {
            ...
            $user_site =
                "<a href='" .
                esc_url(get_home_url($details->userblog_id)) .
                "'>{$details->blogname}</a>";
            $user_dropdown =
            ...
            ?>
						<ul style="list-style:none;">
							<li>
								<?php /* translators: %s: Link to user's site. */
        printf(__("Site: %s"), $user_site); ?>
							</li>
							<li><label><input type="radio" id="delete_option0" name="delete[<?php echo $details->userblog_id .
           "][" .
           $delete_user->ID; ?>]" value="delete" checked="checked" />
							<?php _e("Delete all content."); ?></label></li>
							<li><label><input type="radio" id="delete_option1" name="delete[<?php echo $details->userblog_id .
           "][" .
           $delete_user->ID; ?>]" value="reassign" />
...
function network_edit_site_nav($args = [])
{
    ...
    foreach ($parsed_args["links"] as $link_id => $link) {
        ...
        if (
            $parsed_args["selected"] === $link_id ||
            $link["url"] === $GLOBALS["pagenow"]
        ) {
        ...
        $url = add_query_arg(
            ["id" => $parsed_args["blog_id"]],
            network_admin_url($link["url"])
        );

        // Add link to nav links.
        $screen_links[$link_id] =
            '<a href="' .
            esc_url($url) .
            '" id="' .
            esc_attr($link_id) .
