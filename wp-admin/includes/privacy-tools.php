...
function _wp_personal_data_handle_actions()
{
    if (isset($_POST["privacy_action_email_retry"])) {
        ...
        $request_id = absint(
            current(
                array_keys(
                    (array) wp_unslash($_POST["privacy_action_email_retry"])
                )
            )
        );
        ...
        switch ($action) {
            ...
            case "add_remove_personal_data_request":
                ...
                $action_type = sanitize_text_field(
                    wp_unslash($_POST["type_of_action"])
                );
                $username_or_email_address = sanitize_text_field(
                    wp_unslash($_POST["username_or_email_for_privacy_request"])
                );
                $email_address = "";
                $status = "pending";
...
function wp_privacy_generate_personal_data_export_group_html(
    ...
    $group_id_attr = sanitize_title_with_dashes(
        $group_data["group_label"] . "-" . $group_id
    );
    ...
    if (!empty($group_data["group_description"])) {
        $group_html .=
            "<p>" . esc_html($group_data["group_description"]) . "</p>";
    }

    $group_html .= "<div>";

    foreach (
        (array) $group_data["items"]
        as $group_item_id => $group_item_data
    ) {
        $group_html .= "<table>";
        $group_html .= "<tbody>";
        ...
        foreach ((array) $group_item_data as $group_item_datum) {
            ...
            $group_html .= "<tr>";
            $group_html .=
                "<th>" . esc_html($group_item_datum["name"]) . "</th>";
            $group_html .=
...
function wp_privacy_generate_personal_data_export_file($request_id)
{
    ...
    if (!$request || "export_personal_data" !== $request->action_name) {
        wp_send_json_error(
            __("Invalid request ID when generating personal data export file.")
        );
    }
    ...
    $html_report_filename = wp_unique_filename(
        $exports_dir,
        $file_basename . ".html"
    );
    $html_report_pathname = wp_normalize_path(
        $exports_dir . $html_report_filename
    );
    $json_report_filename = $file_basename . ".json";
    $json_report_pathname = wp_normalize_path(
        $exports_dir . $json_report_filename
    );
    ...
    $about_group = [
        ...
        "group_description" => _x(
            "Overview of export report.",
            "personal data group description"
        ),
        "items" => [
            "about-1" => [
                [
    ...
    if (is_array($groups)) {
        ...
        if (false !== $groups) {
            _doing_it_wrong(
                ...
                sprintf(
                    __("The %s post meta must be an array."),
                    "<code>_export_data_grouped</code>"
                ),
                "5.8.0"
            );
        }
    ...
    if (false === $file) {
        wp_send_json_error(
            __("Unable to open personal data export (HTML report) for writing.")
        );
    }
    ...
    fwrite(
        $file,
        "table { background: #f0f0f0; border: 1px solid #ddd; margin-bottom: 20px; width: 100%; }"
    );
    ...
    fwrite(
        $file,
        '<h1 id="top">' . esc_html__("Personal Data Export") . "</h1>"
    );
...
###SITEURL###'
    ...
    if (!$mail_success) {
        return new WP_Error(
            "privacy_email_error",
            __("Unable to send personal data export email.")
        );
    }
...
function wp_privacy_process_personal_data_export_page(
    ...
    if (!$request || "export_personal_data" !== $request->action_name) {
        wp_send_json_error(
            __("Invalid request ID when merging personal data to export.")
        );
    }
    ...
    if (1 === $exporter_index && 1 === $page) {
        ...
        $accumulated_data = get_post_meta(
            $request_id,
            "_export_data_raw",
            true
        );
    ...
    if ($send_as_email) {
        ...
        $exports_url = wp_privacy_exports_url();
        $export_file_name = get_post_meta(
            $request_id,
            "_export_file_name",
            true
        );
...
function wp_privacy_process_personal_data_erasure_page(
    ...
    if (!$request || "remove_personal_data" !== $request->action_name) {
        wp_send_json_error(
            __("Invalid request ID when processing personal data to erase.")
        );
    }
