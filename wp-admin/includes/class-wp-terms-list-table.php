...
class WP_Terms_List_Table extends WP_List_Table
{
    ...
    public function __construct($args = [])
    {
        ...
        if (
            empty($post_type) ||
            !in_array($post_type, get_post_types(["show_ui" => true]), true)
        ) {
            $post_type = "post";
        }
    }
    ...
    public function ajax_user_can()
    {
        return current_user_can(
            get_taxonomy($this->screen->taxonomy)->cap->manage_terms
        );
    }
    ...
    public function prepare_items()
    {
        ...
        if ("post_tag" === $taxonomy) {
             ...
             * @param int $tags_per_page Number of tags to be displayed. Default 20.
             */
            $tags_per_page = apply_filters_deprecated(
                "tagsperpage",
                [$tags_per_page],
                "2.8.0",
                "edit_tags_per_page"
    ...
    protected function get_bulk_actions()
    {
        ...
        if (
            current_user_can(
                get_taxonomy($this->screen->taxonomy)->cap->delete_terms
            )
        ) {
    ...
    protected function get_sortable_columns()
    {
        ...
        return [
            ...
            "description" => [
                ...
                false,
                __("Description"),
                __("Table ordered by Description."),
            ],
            "slug" => ["slug", false, __("Slug"), __("Table ordered by Slug.")],
            "posts" => [
                "count",
                false,
                _x("Count", "Number/count of items"),
                __("Table ordered by Posts Count."),
            ],
            "links" => [
                "count",
                false,
                __("Links"),
                __("Table ordered by Links."),
            ],
        ];
    }
    ...
    public function display_rows_or_placeholder()
    {
        ...
        if (
            ...
            !isset($this->callback_args["orderby"])
        ) {
    ...
    public function column_name($tag)
    {
        ...
        if ($quick_edit_enabled) {
            $output .=
                '<div class="hidden" id="inline_' . $qe_data->term_id . '">';
            ...
            $output .=
                ...
                "</div>";
            $output .=
                '<div class="parent">' . $qe_data->parent . "</div></div>";
        }
    ...
    protected function handle_row_actions($item, $column_name, $primary)
    {
        ...
        if (is_term_publicly_viewable($tag)) {
            $actions["view"] = sprintf(
                ...
                /* translators: %s: Taxonomy term name. */
                esc_attr(
                    sprintf(__("View &#8220;%s&#8221; archive"), $tag->name)
                ),
                __("View")
            );
        }
   ...
   ?>
			...
			<div class="inline-edit-save submit">
				<button type="button" class="save button button-primary"><?php echo $tax->labels
        ->update_item; ?></button>
