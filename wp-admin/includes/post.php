...
function _wp_translate_postdata($update = false, $post_data = null)
{
    ...
    if (
        isset($post_data["saveasprivate"]) &&
        "" !== $post_data["saveasprivate"]
    ) {
        $post_data["post_status"] = "private";
    }
    if (
        isset($post_data["publish"]) &&
        "" !== $post_data["publish"] &&
    ...
    if (!isset($post_data["post_status"])) {
        $post_data["post_status"] =
            "auto-draft" === $previous_status ? "draft" : $previous_status;
    }

    if (
        isset($post_data["post_password"]) &&
        !current_user_can($ptype->cap->publish_posts)
    ) {
    ...
    foreach (["aa", "mm", "jj", "hh", "mn"] as $timeunit) {
        if (
            !empty($post_data["hidden_" . $timeunit]) &&
            $post_data["hidden_" . $timeunit] !== $post_data[$timeunit]
        ) {
...
function _wp_get_allowed_postdata($post_data = null)
{
    ...
    return array_diff_key(
        $post_data,
        array_flip(["meta_input", "file", "guid"])
    );
}
...
function edit_post($post_data = null)
{
    ...
    foreach ($format_meta_urls as $format_meta_url) {
        ...
        if (isset($post_data[$keyed])) {
            update_post_meta(
                $post_id,
                $keyed,
                wp_slash(sanitize_url(wp_unslash($post_data[$keyed])))
            );
        }
    }
        ...
        if (isset($post_data[$keyed])) {
            if (current_user_can("unfiltered_html")) {
                ...
                update_post_meta(
                    $post_id,
                    $keyed,
                    wp_filter_post_kses($post_data[$keyed])
                );
            }
        }
    }
    ...
    if (
        ...
        preg_match("#^(audio|video)/#", $post_data["post_mime_type"])
    ) {
        ...
        foreach (wp_get_attachment_id3_keys($post, "edit") as $key => $label) {
            if (isset($post_data["id3_" . $key])) {
                $id3data[$key] = sanitize_text_field(
                    wp_unslash($post_data["id3_" . $key])
                );
            }
        }
    ...
    if (isset($post_data["meta"]) && $post_data["meta"]) {
        foreach ($post_data["meta"] as $key => $value) {
            ...
            if (
                is_protected_meta($meta->meta_key, "post") ||
            ...
            if (
                is_protected_meta($value["key"], "post") ||
    ...
    if ("attachment" === $post_data["post_type"]) {
        ...
        $attachment_data = isset($post_data["attachments"][$post_id])
            ? $post_data["attachments"][$post_id]
            : [];
    ...
    if (isset($post_data["tax_input"])) {
        foreach ((array) $post_data["tax_input"] as $taxonomy => $terms) {
            ...
            if ($tax_object && isset($tax_object->meta_box_sanitize_cb)) {
                $translated["tax_input"][$taxonomy] = call_user_func_array(
    ...
    if (!$success && is_callable([$wpdb, "strip_invalid_text_for_column"])) {
        ...
        foreach ($fields as $field) {
            if (isset($translated[$field])) {
                $translated[$field] = $wpdb->strip_invalid_text_for_column(
                    $wpdb->posts,
                    $field,
                    $translated[$field]
                );
...
function bulk_edit_posts($post_data = null)
{
    ...
    foreach ($reset as $field) {
        if (
            isset($post_data[$field]) &&
            ("" === $post_data[$field] || "-1" === $post_data[$field])
        ) {
    ...
    foreach ($post_ids as $post_id) {
        ...
        foreach ($tax_names as $tax_name) {
            ...
            if ($taxonomy_obj->hierarchical) {
                $current_terms = (array) wp_get_object_terms(
                    $post_id,
                    $tax_name,
                    ["fields" => "ids"]
                );
            ...
            $post_data["tax_input"][$tax_name] = array_merge(
                $current_terms,
                $new_terms
            );
        }
        ...
        if (isset($new_cats) && in_array("category", $tax_names, true)) {
            ...
            if (
                ...
                $indeterminate_post_category =
                    $post_data["indeterminate_post_category"];
            } else {
                $indeterminate_post_category = [];
            }

            $indeterminate_cats = array_intersect(
                $cats,
                $indeterminate_post_category
            );
            $determinate_cats = array_diff(
                $new_cats,
                $indeterminate_post_category
            );
            $post_data["post_category"] = array_unique(
                array_merge($indeterminate_cats, $determinate_cats)
            );
...
function get_default_post_to_edit($post_type = "post", $create_in_db = false)
{
     ...
     */
    $post->post_content = (string) apply_filters(
        "default_content",
        $post_content,
        $post
    );
     ...
     * @param WP_Post $post       Post object.
     */
    $post->post_title = (string) apply_filters(
        "default_title",
        $post_title,
        $post
    );
     ...
     * @param WP_Post $post         Post object.
     */
    $post->post_excerpt = (string) apply_filters(
        "default_excerpt",
        $post_excerpt,
        $post
    );
...
function post_exists(
    ...
    $post_title = wp_unslash(
        sanitize_post_field("post_title", $title, 0, "db")
    );
    $post_content = wp_unslash(
        sanitize_post_field("post_content", $content, 0, "db")
    );
    $post_date = wp_unslash(sanitize_post_field("post_date", $date, 0, "db"));
    $post_type = wp_unslash(sanitize_post_field("post_type", $type, 0, "db"));
    $post_status = wp_unslash(
        sanitize_post_field("post_status", $status, 0, "db")
    );
...
function wp_write_post()
{
    ...
    if (!current_user_can($ptype->cap->edit_posts)) {
        if ("page" === $ptype->name) {
            return new WP_Error(
                "edit_pages",
                __("Sorry, you are not allowed to create pages on this site.")
            );
        } else {
            return new WP_Error(
                "edit_posts",
...
function add_meta($post_id)
{
    ...
    $metakeyselect = isset($_POST["metakeyselect"])
        ? wp_unslash(trim($_POST["metakeyselect"]))
        : "";
    $metakeyinput = isset($_POST["metakeyinput"])
        ? wp_unslash(trim($_POST["metakeyinput"]))
        : "";
    ...
    if (
        ("#NONE#" !== $metakeyselect && !empty($metakeyselect)) ||
        !empty($metakeyinput)
    ) {
        /*
        ...
        if (
            ...
            !current_user_can("add_post_meta", $post_id, $metakey)
        ) {
            return false;
        }
...
function _fix_attachment_links($post)
{
    ...
    if (
        !strpos($content, "?attachment_id=") ||
        !preg_match_all("/<a ([^>]+)>[\s\S]+?<\/a>/", $content, $link_matches)
    ) {
        return;
    }
    ...
    foreach ($link_matches[1] as $key => $value) {
        ...
        if (
            ...
            !$rel_id ||
            $url_id !== $rel_id ||
            !str_contains($url_match[0], $site_url)
        ) {
            continue;
        }
        ...
        $replace = str_replace(
            $url_match[0],
            "href=" . $quote . get_attachment_link($url_id) . $quote,
            $link
        );
...
function wp_edit_posts_query($q = false)
{
    ...
    if (
        isset($q["post_type"]) &&
        in_array($q["post_type"], get_post_types(), true)
    ) {
    ...
    if (
        isset($q["post_status"]) &&
        in_array($q["post_status"], $post_statuses, true)
    ) {
    ...
    if (isset($q["orderby"])) {
        ...
        isset($q["post_status"]) &&
        in_array($q["post_status"], ["pending", "draft"], true)
    ) {
        $orderby = "modified";
    }
...
function wp_edit_attachments_query_vars($q = false)
{
    ...
    $q["post_status"] =
        isset($q["status"]) && "trash" === $q["status"] ? "trash" : $states;
    $q["post_status"] =
        isset($q["attachment-filter"]) && "trash" === $q["attachment-filter"]
            ? "trash"
            : $states;
    ...
    if (
        isset($q["post_mime_type"]) &&
        !array_intersect(
            (array) $q["post_mime_type"],
    ...
    foreach (array_keys($post_mime_types) as $type) {
        if (
            isset($q["attachment-filter"]) &&
            "post_mime_type:$type" === $q["attachment-filter"]
        ) {
    ...
    if (
        isset($q["mine"]) ||
        (isset($q["attachment-filter"]) && "mine" === $q["attachment-filter"])
    ) {
...
function get_sample_permalink($post, $title = null, $name = null)
{
    ...
    if (
        in_array(
            $post->post_status,
            ["auto-draft", "draft", "pending", "future"],
            true
        )
    ) {
...
function get_sample_permalink_html($post, $new_title = null, $new_slug = null)
{
    ...
    list($permalink, $post_name) = get_sample_permalink(
        $post->ID,
        $new_title,
        $new_slug
    );
    ...
    if (
        !str_contains($permalink, "%postname%") &&
        !str_contains($permalink, "%pagename%")
    ) {
        ...
        if (
            ...
            $return .=
                ...
                __("Change Permalink Structure") .
                "</a></span>\n";
        }
    } else {
        if (mb_strlen($post_name) > 34) {
            $post_name_abridged =
                mb_substr($post_name, 0, 16) .
                "&hellip;" .
                mb_substr($post_name, -16);
        } else {
        ...
        $post_name_html =
            ...
            esc_html($post_name_abridged) .
            "</span>";
        $display_link = str_replace(
            ["%pagename%", "%postname%"],
            $post_name_html,
            esc_html(urldecode($permalink))
        );
...
function _wp_post_thumbnail_html($thumbnail_id = null, $post = null)
{
    ...
    $set_thumbnail_link =
        '<p class="hide-if-no-js"><a href="%s" id="set-post-thumbnail"%s class="thickbox">%s</a></p>';
    ...
    if ($thumbnail_id && get_post($thumbnail_id)) {
        $size = isset($_wp_additional_image_sizes["post-thumbnail"])
            ? "post-thumbnail"
            : [266, 266];
...
function _admin_notice_post_locked()
{
	...
	<?php if ($locked) {
     ...
     ?>
      ...
      printf(
          __("%s is currently editing this post. Do you want to take over?"),
          esc_html($user->display_name)
      );
  } else {
      /* translators: %s: User's display name. */
      printf(
          __("%s is currently editing this post."),
          esc_html($user->display_name)
      );
  } ?>
		</p>
  ...
  do_action("post_locked_dialog", $post, $user); ?>
      ...
      $sendback
  ); ?>"><?php echo $sendback_text; ?></a>
		<?php
  if ($preview_link) { ?>
		<a class="button<?php echo $tab_last; ?>" href="<?php echo esc_url(
    $preview_link
); ?>"><?php _e("Preview"); ?></a>
			<?php }
  ...
  if ($override) { ?>
	<a class="button button-primary wp-tab-last" href="<?php echo esc_url(
     add_query_arg(
         "get-post-lock",
         "1",
         wp_nonce_url(
  ...
  ?>
      ...
      ?>
		<div class="post-taken-over">
			...
			<span class="locked-saving hidden"><img src="<?php echo esc_url(
       admin_url("images/spinner-2x.gif")
   ); ?>" width="16" height="16" alt="" /> <?php _e(
    "Saving revision&hellip;"
); ?></span>
			<span class="locked-saved hidden"><?php _e(
   ...
   do_action("post_lock_lost_dialog", $post); ?>
			<p><a class="button button-primary wp-tab-last" href="<?php echo esc_url(
       $sendback
   ); ?>"><?php echo $sendback_text; ?></a></p>
		</div>
		<?php
 } ?>
	</div>
...
function wp_create_post_autosave($post_data)
{
    ...
    if ($old_autosave) {
        ...
        foreach (
            array_intersect(
                array_keys($new_autosave),
                array_keys(_wp_post_revision_fields($post))
            )
            as $field
        ) {
            if (
                normalize_whitespace($new_autosave[$field]) !==
                normalize_whitespace($post->$field)
            ) {
...
function wp_autosave_post_revisioned_meta_fields($new_autosave)
{
     ...
     */
    $posted_data = isset($_POST["data"]["wp_autosave"])
        ? $_POST["data"]["wp_autosave"]
        : $_POST;
    ...
    foreach (wp_post_revision_meta_keys($post_type) as $meta_key) {
        if (
            ...
            get_post_meta($new_autosave["ID"], $meta_key, true) !==
                wp_unslash($posted_data[$meta_key])
        ) {
            /*
...
function post_preview()
{
    ...
    if ($is_autosave && $saved_post_id) {
        $query_args["preview_id"] = $post->ID;
        $query_args["preview_nonce"] = wp_create_nonce(
            "post_preview_" . $post->ID
        );

        if (isset($_POST["post_format"])) {
            $query_args["post_format"] = empty($_POST["post_format"])
                ? "standard"
                : sanitize_key($_POST["post_format"]);
        }

        if (isset($_POST["_thumbnail_id"])) {
            $query_args["_thumbnail_id"] =
                (int) $_POST["_thumbnail_id"] <= 0
                    ? "-1"
...
function the_block_editor_meta_boxes()
{
	...
	<form id="toggle-custom-fields-form" method="post" action="<?php echo esc_url(
     admin_url("post.php")
 ); ?>">
      ...
      $location
			...
			<div id="poststuff" class="sidebar-open">
				<div id="postbox-container-2" class="postbox-container">
					<?php do_meta_boxes($current_screen, $location, $post); ?>
				</div>
			</div>
		</form>
	<?php endforeach; ?>
 ...
 foreach ($locations as $location) {
     ...
     foreach ($priorities as $priority) {
         if (
             !isset($wp_meta_boxes[$current_screen->id][$location][$priority])
         ) {
             continue;
         }

         $meta_boxes =
             (array) $wp_meta_boxes[$current_screen->id][$location][$priority];
         foreach ($meta_boxes as $meta_box) {
             ...
             if (
                 isset($meta_box["args"]["__back_compat_meta_box"]) &&
  ...
  */
 $enable_custom_fields = (bool) get_user_meta(
     get_current_user_id(),
...
function the_block_editor_meta_box_post_form_hidden_fields($post)
{
	...
	<input type="hidden" id="referredby" name="referredby" value="<?php echo $referer
     ? esc_url($referer)
     : ""; ?>" />
