...
class WP_Debug_Data
{
    ...
    public static function debug_data()
    {
         ...
         */
        $info = array_filter($info, static function ($section) {
        ...
        if (is_array($core_updates)) {
            foreach ($core_updates as $core => $update) {
                if ("upgrade" === $update->response) {
                    ...
                    $core_update_needed =
                        " " .
                        sprintf(__("(Latest version: %s)"), $update->version);
                } else {
                    $core_update_needed = "";
                }
            }
    ...
    private static function get_wp_server(): array
    {
        ...
        $php_version = sprintf(
            ...
            $php_version_debug,
            $php64bit
                ? __("(Supports 64bit values)")
                : __("(Does not support 64bit values)")
        );
        ...
        $fields["httpd_software"] = [
            "label" => __("Web server"),
            "value" => isset($_SERVER["SERVER_SOFTWARE"])
                ? $_SERVER["SERVER_SOFTWARE"]
                : __("Unable to determine what web server software is used"),
            "debug" => isset($_SERVER["SERVER_SOFTWARE"])
                ? $_SERVER["SERVER_SOFTWARE"]
                : "unknown",
        ];
        $fields["php_version"] = [
        ...
        if (!function_exists("ini_get")) {
            ...
            if (
                WP_Site_Health::get_instance()->php_memory_limit !==
        ...
        $suhosin_loaded =
            ...
            (defined("SUHOSIN_PATCH") && constant("SUHOSIN_PATCH"));
        ...
        if (is_file(ABSPATH . ".htaccess")) {
            ...
            $filtered_htaccess_content = trim(
                preg_replace(
                    "/\# BEGIN WordPress[\s\S]+?# END WordPress/si",
                    "",
                    $htaccess_content
                )
            );
            ...
            if ($filtered_htaccess_content) {
                ...
                $htaccess_rules_string = sprintf(
                    __("Custom rules have been added to your %s file."),
                    ".htaccess"
                );
            } else {
                /* translators: %s: .htaccess */
                $htaccess_rules_string = sprintf(
                    __("Your %s file contains only core WordPress features."),
                    ".htaccess"
                );
            }
    ...
    private static function get_wp_media(): array
    {
        ...
        $fields["imagick_module_version"] = [
            "label" => __("ImageMagick version number"),
            "value" => is_array($imagemagick_version)
                ? $imagemagick_version["versionNumber"]
                : $imagemagick_version,
        ];

        $fields["imagemagick_version"] = [
            "label" => __("ImageMagick version string"),
            "value" => is_array($imagemagick_version)
                ? $imagemagick_version["versionString"]
                : $imagemagick_version,
        ];

        $imagick_version = phpversion("imagick");

        $fields["imagick_version"] = [
        ...
        if (
            "WP_Image_Editor_Imagick" === _wp_image_editor_choose() &&
            isset($imagick) &&
            $imagick instanceof Imagick
        ) {
            $limits = [
                "area" => defined("imagick::RESOURCETYPE_AREA")
                    ? size_format(
                        $imagick->getResourceLimit(imagick::RESOURCETYPE_AREA)
                    )
                    : $not_available,
            ...
            $limits_debug = [
                "imagick::RESOURCETYPE_AREA" => defined(
                    ...
                    : "not available",
                "imagick::RESOURCETYPE_DISK" => defined(
                    "imagick::RESOURCETYPE_DISK"
                )
                    ? $imagick->getResourceLimit(imagick::RESOURCETYPE_DISK)
                    : "not available",
                "imagick::RESOURCETYPE_FILE" => defined(
                    "imagick::RESOURCETYPE_FILE"
                )
                    ? $imagick->getResourceLimit(imagick::RESOURCETYPE_FILE)
                    : "not available",
                "imagick::RESOURCETYPE_MAP" => defined(
                    ...
                    : "not available",
                "imagick::RESOURCETYPE_MEMORY" => defined(
                    ...
                    : "not available",
                "imagick::RESOURCETYPE_THREAD" => defined(
                    "imagick::RESOURCETYPE_THREAD"
                )
                    ? $imagick->getResourceLimit(imagick::RESOURCETYPE_THREAD)
                    : "not available",
                "imagick::RESOURCETYPE_TIME" => defined(
            ...
            $fields["imagemagick_file_formats"] = [
    ...
    private static function get_wp_mu_plugins(): array
    {
        ...
        foreach ($mu_plugins as $plugin_path => $plugin) {
            ...
            $plugin_version_string = __(
                "No version or author information is available."
            );
            $plugin_version_string_debug =
                "author: (undefined), version: (undefined)";

            if (!empty($plugin_version) && !empty($plugin_author)) {
                /* translators: 1: Plugin version number. 2: Plugin author name. */
                $plugin_version_string = sprintf(
                    __('Version %1$s by %2$s'),
                    $plugin_version,
                    $plugin_author
                );
                $plugin_version_string_debug = sprintf(
                    "version: %s, author: %s",
                    $plugin_version,
                    $plugin_author
                );
    ...
    private static function get_wp_plugins_raw_data(): array
    {
        ...
        foreach ($plugins as $plugin_path => $plugin) {
            $plugin_part = is_plugin_active($plugin_path)
                ? "wp-plugins-active"
                : "wp-plugins-inactive";
            ...
            if ($auto_updates_enabled) {
                ...
                $auto_update_forced = wp_is_auto_update_forced_for_item(
                    "plugin",
                    null,
                    (object) $item
                );
    ...
    private static function get_wp_active_theme(): array
    {
        ...
        if (array_key_exists($active_theme->stylesheet, $theme_updates)) {
            $theme_update_new_version =
                $theme_updates[$active_theme->stylesheet]->update[
                    "new_version"
                ];
            ...
            $active_theme_version .=
                " " .
                sprintf(__("(Latest version: %s)"), $theme_update_new_version);
            $active_theme_version_debug .= sprintf(
                " (latest version: %s)",
                $theme_update_new_version
            );
        }
        ...
        $fields = [
            ...
            "author_website" => [
                ...
                "value" => $active_theme_author_uri
                    ? $active_theme_author_uri
                    : __("Undefined"),
                "debug" => $active_theme_author_uri
                    ? $active_theme_author_uri
                    : "(undefined)",
            ],
            "parent_theme" => [
        ...
        if ($parent_theme) {
            ...
            if (array_key_exists($parent_theme->stylesheet, $theme_updates)) {
                $parent_theme_update_new_version =
                    $theme_updates[$parent_theme->stylesheet]->update[
                        "new_version"
                    ];
                ...
                $parent_theme_version_debug .= sprintf(
                    " (latest version: %s)",
                    $parent_theme_update_new_version
                );
            }
            ...
            $fields = [
                ...
                "author_website" => [
                    "label" => __("Author website"),
                    "value" => $parent_theme_author_uri
                        ? $parent_theme_author_uri
                        : __("Undefined"),
                    "debug" => $parent_theme_author_uri
                        ? $parent_theme_author_uri
                        : "(undefined)",
                ],
                "theme_path" => [
            ...
            if ($auto_updates_enabled) {
                if (isset($transient->response[$parent_theme->stylesheet])) {
                    ...
                    isset($transient->no_update[$parent_theme->stylesheet])
                ) {
                ...
                /** This filter is documented in wp-admin/includes/class-wp-debug-data.php */
                $parent_theme_auto_update_string = apply_filters(
                    "theme_auto_update_debug_string",
    ...
    private static function get_wp_themes_inactive(): array
    {
        ...
        foreach ($all_themes as $theme_slug => $theme) {
            ...
            $fields[sanitize_text_field($theme->name)] = [
                "label" => sprintf(
                    /* translators: 1: Theme name. 2: Theme slug. */
                    __('%1$s (%2$s)'),
                    $theme->name,
    ...
    private static function get_wp_filesystem(): array
    {
        ...
        $is_writable_template_directory = wp_is_writable(
            get_theme_root(get_template())
        );
        $is_writable_fonts_dir = $fonts_dir_exists
            ? wp_is_writable(wp_get_font_dir()["basedir"])
            : false;

        $fields = [
            "wordpress" => [
                "label" => __("The main WordPress directory"),
                "value" => $is_writable_abspath
                    ? __("Writable")
                    : __("Not writable"),
            ...
            "wp-content" => [
                ...
                "value" => $is_writable_wp_content_dir
                    ? __("Writable")
                    : __("Not writable"),
                "debug" => $is_writable_wp_content_dir
                    ? "writable"
                    : "not writable",
            ],
            "uploads" => [
                ...
                "value" => $is_writable_upload_dir
                    ? __("Writable")
                    : __("Not writable"),
                "debug" => $is_writable_upload_dir
                    ? "writable"
                    : "not writable",
            ],
            "plugins" => [
                "label" => __("The plugins directory"),
                "value" => $is_writable_wp_plugin_dir
                    ? __("Writable")
                    : __("Not writable"),
                "debug" => $is_writable_wp_plugin_dir
                    ? "writable"
                    : "not writable",
            ],
            "themes" => [
                "label" => __("The themes directory"),
                "value" => $is_writable_template_directory
                    ? __("Writable")
                    : __("Not writable"),
                "debug" => $is_writable_template_directory
                    ? "writable"
                    : "not writable",
            ],
            "fonts" => [
        ...
        if (defined("WPMU_PLUGIN_DIR") && is_dir(WPMU_PLUGIN_DIR)) {
            ...
            $fields["mu-plugins"] = [
                ...
                "value" => $is_writable_wpmu_plugin_dir
                    ? __("Writable")
                    : __("Not writable"),
                "debug" => $is_writable_wpmu_plugin_dir
                    ? "writable"
                    : "not writable",
            ];
        }
    ...
    public static function format($info_array, $data_type)
    {
        ...
        foreach ($info_array as $section => $details) {
            ...
            if (
                ...
                (isset($details["private"]) && $details["private"])
            ) {
                continue;
            }

            $section_label =
                "debug" === $data_type ? $section : $details["label"];

            $return .= sprintf(
                "### %s%s ###\n\n",
                $section_label,
                isset($details["show_count"]) && $details["show_count"]
                    ? sprintf(" (%d)", count($details["fields"]))
                    : ""
            );
            ...
            foreach ($details["fields"] as $field_name => $field) {
                ...
                if (is_array($debug_data)) {
                    ...
                    foreach (
                        $debug_data
                        as $sub_field_name => $sub_field_value
                    ) {
                        $value .= sprintf(
                            "\n\t%s: %s",
                            $sub_field_name,
