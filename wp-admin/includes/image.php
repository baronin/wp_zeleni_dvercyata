...
function wp_crop_image(
    ...
    if (!$dst_file) {
        $dst_file = str_replace(
            wp_basename($src_file),
            "cropped-" . wp_basename($src_file),
            $src_file
        );
    }
    ...
    $dst_file =
        dirname($dst_file) .
        "/" .
        wp_unique_filename(dirname($dst_file), wp_basename($dst_file));
 ...
 */
function _wp_image_meta_replace_original(
    $saved_data,
    $original_file,
    $image_meta,
    $attachment_id
...
function wp_create_image_subsizes($file, $attachment_id)
{
    ...
    if (
        $threshold &&
        ($image_meta["width"] > $threshold ||
            $image_meta["height"] > $threshold)
    ) {
    ...
    if ($scale_down || $convert) {
        ...
        if (!is_wp_error($resized)) {
            ...
            if (!is_wp_error($saved)) {
                $image_meta = _wp_image_meta_replace_original(
                    $saved,
                    $file,
                    $image_meta,
                    $attachment_id
                ...
                if (
                    true === $rotated &&
                    !empty($image_meta["image_meta"]["orientation"])
                ) {
        ...
        !empty($exif_meta["orientation"]) &&
        1 !== (int) $exif_meta["orientation"]
    ) {
...
function wp_copy_parent_attachment_properties(
    ...
    $attachment = [
        "post_title" => $use_original_title
            ? $parent->post_title
            : wp_basename($cropped),
        "post_content" => $use_original_description
            ? $parent->post_content
            : $url,
    ...
    if ("" !== trim($parent->_wp_attachment_image_alt)) {
        $attachment["meta_input"] = [
            "_wp_attachment_image_alt" => wp_slash(
                $parent->_wp_attachment_image_alt
            ),
        ];
    }
...
function wp_generate_attachment_metadata($attachment_id, $file)
{
    ...
    if ($support && !empty($metadata["image"]["data"])) {
        ...
        if (!empty($exists)) {
            ...
            $basename =
                str_replace(".", "-", wp_basename($file)) . "-image" . $ext;
            ...
            if (false === $uploaded["error"]) {
                 ...
                 * }
                 */
                $image_attachment = apply_filters(
                ...
                $sub_attachment_id = wp_insert_attachment(
                    $image_attachment,
                    $uploaded["file"]
                );
        ...
        $registered_sizes = wp_get_registered_image_subsizes();
        $merged_sizes = array_intersect_key(
            $registered_sizes,
            array_flip($fallback_sizes)
        );

        // Force thumbnails to be soft crops.
        if (
            isset($merged_sizes["thumbnail"]) &&
            is_array($merged_sizes["thumbnail"])
        ) {
...
function wp_read_image_metadata($file)
{
    ...
    if (
        ...
        in_array($image_type, $exif_image_types, true)
    ) {
        // Don't silence errors when in debug mode, unless running unit tests.
            ...
            if (
                ...
                if (
                    ...
                    if ($exif_description === $exif_usercomment) {
                        ...
                        $caption = trim(
                            $exif_description . " " . $exif_usercomment
                        );
                    }
                }
                $meta["caption"] = $caption;
        ...
        if (!empty($exif["FocalLength"])) {
            ...
            if (is_scalar($exif["FocalLength"])) {
                $meta["focal_length"] = (string) wp_exif_frac2dec(
                    $exif["FocalLength"]
                );
            }
        }
        if (!empty($exif["ISOSpeedRatings"])) {
            $meta["iso"] = is_array($exif["ISOSpeedRatings"])
                ? reset($exif["ISOSpeedRatings"])
        ...
        if (!empty($exif["ExposureTime"])) {
            ...
            if (is_scalar($exif["ExposureTime"])) {
                $meta["shutter_speed"] = (string) wp_exif_frac2dec(
                    $exif["ExposureTime"]
                );
            }
        }
    ...
    foreach (
        ["title", "caption", "credit", "copyright", "camera", "iso"]
        as $key
    ) {
...
function _copy_image_file($attachment_id)
{
    ...
    if ($src_file) {
        $dst_file = str_replace(
            wp_basename($dst_file),
            "copy-" . wp_basename($dst_file),
            $dst_file
        );
        $dst_file =
            dirname($dst_file) .
