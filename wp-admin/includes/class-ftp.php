...
class ftp_base
{
    ...
    function __construct($port_mode = false, $verb = false, $le = false)
    {
        ...
        $this->SendMSG(
            "Staring FTP client class" .
                ($this->_port_available ? "" : " without PORT mode support")
        );
    ...
    function connect($server = null)
    {
        ...
        if (
            !($this->_ftp_control_sock = $this->_connect(
                $this->_host,
                $this->_port
            ))
        ) {
        ...
        if (!$syst) {
            ...
            $this->SendMSG(
                "Remote OS: " . $this->OS_FullName[$this->OS_remote]
            );
        }
        if (!$this->features()) {
            $this->SendMSG(
                "Cannot get features list. All supported - disabled"
            );
        } else {
            $this->SendMSG(
                "Supported features: " .
    ...
    function fget($fp, $remotefile, $rest = 0)
    {
        ...
        if (
            $this->_type == FTP_ASCII or
            $this->_type == FTP_AUTOASCII and
    ...
    function mput($local = ".", $remote = null, $continious = false)
    {
        ...
        if (!@file_exists($local)) {
            $this->PushError(
                "mput",
                "cannot open local folder",
                "Cannot stat folder \"" . $local . "\""
            );
            return false;
        }
        if (!is_dir($local)) {
    ...
    function mget($remote, $local = ".", $continious = false)
    {
        ...
        if ($list === false) {
            $this->PushError(
                "mget",
                "cannot read remote folder list",
