...
function delete_theme($stylesheet, $redirect = "")
{
    ...
    if (empty($themes_dir)) {
        return new WP_Error(
            "fs_no_themes_dir",
            __("Unable to locate WordPress theme directory.")
        );
    }
    ...
    foreach ((array) $feature_list as $feature_category => $feature_items) {
        ...
        foreach ($feature_items as $feature) {
            if (isset($features[$feature_category][$feature])) {
                $wporg_features[$feature_category][$feature] =
...
function wp_prepare_themes_for_js($themes = null)
{
    ...
    foreach ($themes as $theme) {
        ...
        $update_requires_wp = isset($updates[$slug]["requires"])
            ? $updates[$slug]["requires"]
            : null;
        $update_requires_php = isset($updates[$slug]["requires_php"])
            ? $updates[$slug]["requires_php"]
            : null;

        $auto_update = in_array($slug, $auto_updates, true);
        $auto_update_action = $auto_update
            ? "disable-auto-update"
            : "enable-auto-update";
        ...
        $prepared_themes[$slug] = [
            ...
            "compatibleWP" => is_wp_version_compatible(
                $theme->get("RequiresWP")
            ),
            "compatiblePHP" => is_php_version_compatible(
            ...
            "hasPackage" =>
                isset($updates[$slug]) && !empty($updates[$slug]["package"]),
     ...
     _e("Close details dialog"); ?>
      ...
      printf(__("Version: %s"), "{{ data.version }}"); ?>
					...
					<h3 class="theme-author">
						<?php /* translators: %s: Theme author link. */
      printf(__("By %s"), "{{{ data.authorAndUri }}}"); ?>
        ...
        ?>
					...
					<# } else if ( ! data.active && data.blockTheme ) { #>
						<div class="notice notice-error notice-alt notice-large"><p>
						<?php _e('This theme doesn\'t support Customizer.'); ?>
           ...
           "{{{ data.actions.activate }}}"
			...
			<div class="theme-actions">
				<# if ( data.active ) { #>
					<button type="button" class="button button-primary customize-theme"><?php _e(
         "Customize"
     ); ?></button>
      ...
      $aria_label = sprintf(_x("Activate %s", "theme"), "{{ data.name }}"); ?>
						<# if ( data.compatibleWP && data.compatiblePHP && data.actions.activate ) { #>
							<a href="{{{ data.actions.activate }}}" class="button button-primary activate" aria-label="<?php echo esc_attr(
           $aria_label
       ); ?>"><?php _e("Activate"); ?></a>
						<# } #>
					<# } else { #>
						<# if ( data.compatibleWP && data.compatiblePHP ) { #>
							<button type="button" class="button button-primary preview-theme" data-slug="{{ data.id }}"><?php _e(
           "Live Preview"
       ); ?></button>
						<# } else { #>
							<button class="button button-primary disabled"><?php _e(
           "Live Preview"
       ); ?></button>
						<# } #>
					<# } #>
					...
					<?php if (current_user_can("delete_themes")) { ?>
						<# if ( data.actions && data.actions['delete'] ) { #>
							<a href="{{{ data.actions['delete'] }}}" data-slug="{{ data.id }}" class="button button-secondary delete-theme"><?php _e(
           "Delete"
       ); ?></a>
						<# } #>
					<?php } ?>
				<# } else { #>
					<# if ( data.compatibleWP && data.compatiblePHP ) { #>
						<button type="button" class="button theme-install" data-slug="{{ data.id }}"><?php _e(
          "Install"
      ); ?></button>
						<button type="button" class="button button-primary theme-install preview" data-slug="{{ data.id }}"><?php _e(
          "Install &amp; Preview"
      ); ?></button>
					<# } else { #>
						<button type="button" class="button disabled"><?php _ex(
          "Cannot Install",
          "theme"
      ); ?></button>
						<button type="button" class="button button-primary disabled"><?php _e(
          "Install &amp; Preview"
      ); ?></button>
					<# } #>
				<# } #>
...
function paused_themes_notice()
{
    ...
    if (
        !isset($GLOBALS["_paused_themes"]) ||
        empty($GLOBALS["_paused_themes"])
    ) {
        return;
    }
