...
class Core_Upgrader extends WP_Upgrader
{
    ...
    {
        $this->strings["up_to_date"] = __(
            "WordPress is at the latest version."
        );
        $this->strings["locked"] = __(
            "Another update is currently in progress."
        );
        $this->strings["no_package"] = __("Update package not available.");
        /* translators: %s: Package URL. */
        $this->strings["downloading_package"] = sprintf(
            __("Downloading update from %s&#8230;"),
            '<span class="code pre">%s</span>'
        );
        $this->strings["unpack_package"] = __("Unpacking the update&#8230;");
        $this->strings["copy_failed"] = __("Could not copy files.");
        $this->strings["copy_failed_space"] = __(
            "Could not copy files. You may have run out of disk space."
        );
        $this->strings["start_rollback"] = __(
            "Attempting to restore the previous version."
        );
        $this->strings["rollback_was_required"] = __(
    ...
    public function upgrade($current, $args = [])
    {
        ...
        if ($parsed_args["do_rollback"] && $current->packages->rollback) {
            ...
            $current->packages->partial &&
            "reinstall" !== $current->response &&
            $wp_version === $current->partial_version &&
            $partial
        ) {
            $to_download = "partial";
        } elseif (
            $current->packages->new_bundled &&
            version_compare($wp_version, $current->new_bundled, "<") &&
        ...
        if (
            ...
            if ($try_rollback) {
                ...
                $rollback_result = $this->upgrade(
                    $current,
                    array_merge($parsed_args, ["do_rollback" => true])
                );
        ...
        if (!$parsed_args["do_rollback"]) {
            $stats = [
                ...
                "fs_method_forced" =>
                    defined("FS_METHOD") || has_filter("filesystem_method"),
                "fs_method_direct" => !empty(
    ...
    public static function should_update_to_version($offered_ver)
    {
        ...
        $current_branch = implode(
            ".",
            array_slice(preg_split("/[.-]/", $wp_version), 0, 2)
        ); // x.y
        $new_branch = implode(
            ".",
            array_slice(preg_split("/[.-]/", $offered_ver), 0, 2)
        ); // x.y
