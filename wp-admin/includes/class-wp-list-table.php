        ...
        if (!empty($_REQUEST["orderby"])) {
            if (is_array($_REQUEST["orderby"])) {
                foreach ($_REQUEST["orderby"] as $key => $value) {
                    echo '<input type="hidden" name="orderby[' .
                        esc_attr($key) .
                        ']" value="' .
                        esc_attr($value) .
                        '" />';
...
<p class="search-box">
    ...
    protected function get_views_links($link_data = [])
    {
        ...
        foreach ($link_data as $view => $link) {
            ...
            $views_links[$view] = sprintf(
                '<a href="%s"%s>%s</a>',
                esc_url($link["url"]),
                isset($link["current"]) && true === $link["current"]
    ...
    protected function bulk_actions($which = "")
    {
        ...
        submit_button(__("Apply"), "action", "bulk_action", false, [
            "id" => "doaction$two",
        ]);
        echo "\n";
    }
    ...
    protected function months_dropdown($post_type)
    {
        ...
        ?>
		<label for="filter-by-date" class="screen-reader-text"><?php echo get_post_type_object(
      $post_type
  )->labels->filter_by_date; ?></label>
    ...
    protected function view_switcher($current_mode)
    {
      ...
      $current_mode
		...
		<?php foreach ($this->modes as $mode => $title) {
      $classes = ["view-" . $mode];
      $aria_current = "";
    ...
    public function get_pagenum()
    {
        ...
        if (
            isset($this->_pagination_args["total_pages"]) &&
    ...
    public function print_column_headers($with_id = true)
    {
        ...
        foreach ($columns as $column_key => $column_display_name) {
            ...
            if ("cb" === $column_key) {
                ...
                in_array($column_key, ["posts", "comments", "links"], true)
            ) {
                $class[] = "num";
            }
            ...
            if (isset($sortable[$column_key])) {
                $orderby = isset($sortable[$column_key][0])
                    ? $sortable[$column_key][0]
                    : "";
                $desc_first = isset($sortable[$column_key][1])
                    ? $sortable[$column_key][1]
                    : false;
                $abbr = isset($sortable[$column_key][2])
                    ? $sortable[$column_key][2]
                    : "";
                $orderby_text = isset($sortable[$column_key][3])
                    ? $sortable[$column_key][3]
                    : "";
                $initial_order = isset($sortable[$column_key][4])
                    ? $sortable[$column_key][4]
                    : "";
                ...
                $column_display_name = sprintf(
                    '<a href="%1$s">' .
                        ...
                        "</a>",
                    esc_url(
                        add_query_arg(compact("orderby", "order"), $current_url)
                    ),
                    $column_display_name,
                    $order_text
                );
        ...
        foreach (array_keys($columns) as $column_key) {
            if (isset($sortable[$column_key])) {
                ...
                if ($current_orderby === $orderby) {
                    ...
                    $order_text =
                        "asc" === $current_order ? $asc_text : $desc_text;
...
<table class="wp-list-table <?php echo implode(
  ...
  $this->pagination($which);?>
    ...
    public function _js_vars()
    {
        ...
        printf(
            "<script type='text/javascript'>list_args = %s;</script>\n",
            wp_json_encode($args)
        );
    }
}
