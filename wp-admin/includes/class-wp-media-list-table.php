...
class WP_Media_List_Table extends WP_List_Table
{
    ...
    {
        $this->detached =
            isset($_REQUEST["attachment-filter"]) &&
            "detached" === $_REQUEST["attachment-filter"];
    ...
    public function prepare_items()
    {
        ...
        if (
            !empty($_REQUEST["post__not_in"]) &&
            is_array($_REQUEST["post__not_in"])
        ) {
            $not_in = array_merge(
                array_values($_REQUEST["post__not_in"]),
                $not_in
            );
        }
    ...
    protected function get_views()
    {
        ...
        $filter = empty($_GET["attachment-filter"])
            ? ""
            : $_GET["attachment-filter"];
        ...
        foreach ($post_mime_types as $mime_type => $label) {
            ...
            $selected = selected(
                $filter &&
                    str_starts_with($filter, "post_mime_type:") &&
                    wp_match_mime_types(
                        $mime_type,
                        str_replace("post_mime_type:", "", $filter)
                    ),
                true,
                false
            );
        ...
        $type_links["detached"] =
            ...
            ($this->detached ? ' selected="selected"' : "") .
            ">" .
   ...
   if (
    ...
    ?>
     ...
     esc_html_e("Search Media"); ?>
					...
					<input id="search-submit" type="submit" class="button" value="<?php esc_attr_e(
         "Search Media"
     ); ?>">
				</p>
			</div>
    ...
    protected function get_sortable_columns()
    {
        return [
            "title" => [
                ...
                false,
                _x("File", "column name"),
                __("Table ordered by File Name."),
            ],
            "author" => [
                "author",
                false,
                __("Author"),
                __("Table ordered by Author."),
            ],
            "parent" => [
                "parent",
                false,
                _x("Uploaded to", "column name"),
                __("Table ordered by Uploaded To."),
            ],
            "comments" => [
                "comment_count",
                __("Comments"),
    ...
    public function column_cb($item)
    {
        ...
        if (current_user_can("edit_post", $post->ID)) { ?>
			...
			<label for="cb-select-<?php echo $post->ID; ?>">
				...
				<?php /* translators: Hidden accessibility text. %s: Attachment title. */
   ...
   if ($thumb): ?>
				<span class="media-icon <?php echo sanitize_html_class(
        $mime . "-icon"
    ); ?>"><?php echo $thumb; ?></span>
				<?php endif;
   ...
   echo esc_html(wp_basename($file));?>
    ...
    public function column_parent($post)
    {
        ...
        if ($parent) {
            ...
            if ($user_can_edit):
                ...
                printf(
                    ...
                    esc_attr(
                        sprintf(__("Detach from &#8220;%s&#8221;"), $title)
                    ),
                    __("Detach")
                );
            endif;
       ...
       printf(
           ...
           esc_attr(
               sprintf(
                   __("Attach &#8220;%s&#8221; to existing content"),
                   $title
               )
           ),
           __("Attach")
    ...
    public function column_default($item, $column_name)
    {
        ...
        if ($taxonomy) {
            ...
            if (is_array($terms)) {
                ...
                echo '<span aria-hidden="true">&#8212;</span><span class="screen-reader-text">' .
                    get_taxonomy($taxonomy)->labels->no_terms .
                    "</span>";
            }
        ...
        if (current_user_can("delete_post", $post->ID)) {
            if ($this->is_trash) {
                ...
                $actions["trash"] = sprintf(
                    ...
                    esc_url(
                        wp_nonce_url(
                            "post.php?action=trash&amp;post=$post->ID",
                            "trash-post_" . $post->ID
                        )
                    ),
                    /* translators: %s: Attachment title. */
