...
class WP_MS_Users_List_Table extends WP_List_Table
{
    ...
    protected function get_sortable_columns()
    {
        return [
            ...
            "email" => [
                ...
                false,
                __("E-mail"),
                __("Table ordered by E-mail."),
            ],
            "registered" => [
                "id",
                false,
                _x("Registered", "user"),
                __("Table ordered by User Registered Date."),
            ],
        ];
    }
    ...
    public function column_cb($item)
    {
		...
		<label for="blog_<?php echo $user->ID; ?>">
			...
			<?php /* translators: Hidden accessibility text. %s: User login. */
    ...
    public function column_blogs($user)
    {
        ...
        foreach ($blogs as $site) {
            ...
            if (is_array($site_classes) && !empty($site_classes)) {
                $site_classes = array_map(
                    "sanitize_html_class",
                    array_unique($site_classes)
                );
                echo '<span class="' .
                    esc_attr(implode(" ", $site_classes)) .
                    '">';
            } else {
                echo "<span>";
            }
            ...
            echo '<a href="' .
                esc_url(
                    network_admin_url("site-info.php?id=" . $site->userblog_id)
                ) .
                '">' .
                str_replace(
                    "." . get_network()->domain,
            ...
            $actions["view"] =
                ...
                $class .
                '" href="' .
                esc_url(get_home_url($site->userblog_id)) .
                '">' .
                __("View") .
                "</a>";
                ...
                esc_url(
                    network_admin_url(
                        add_query_arg(
                            "_wp_http_referer",
                            urlencode(wp_unslash($_SERVER["REQUEST_URI"])),
                            wp_nonce_url("users.php", "deleteuser") .
