...
class WP_Theme_Install_List_Table extends WP_Themes_List_Table
{
    ...
    public function prepare_items()
    {
        ...
        if (!empty($_REQUEST["s"])) {
            ...
            $search_terms = array_unique(
                array_filter(array_map("trim", explode(",", $search_string)))
            );
        }
    ...
    public function single_row($theme)
    {
        ...
        switch ($status) {
            case "update_available":
                $actions[] = sprintf(
                    ...
                    esc_attr(
                        sprintf(__("Update to version %s"), $theme->version)
                    ),
                    __("Update")
                );
                break;
        ...
        ?>
		<a class="screenshot install-theme-preview" href="<?php echo esc_url(
      $preview_url
  ); ?>" aria-label="<?php echo esc_attr($preview_title); ?>">
			<img src="<?php echo esc_url(
       $theme->screenshot_url . "?ver=" . $theme->version
   ); ?>" width="150" alt="" />
		</a>
  ...
  printf(__("By %s"), $author); ?>
		...
		<div class="action-links">
			<ul>
				...
				<?php endforeach; ?>
				<li class="hide-if-no-js"><a href="#" class="theme-detail"><?php _e(
        "Details"
    ); ?></a></li>
			</ul>
		</div>
    ...
    public function install_theme_info($theme)
    {
      ...
      case "update_available":
          printf(
              ...
              esc_url(
                  wp_nonce_url($update_url, "upgrade-theme_" . $theme->slug)
              ),
              /* translators: %s: Theme version. */
      ...
      default:
          printf(
              ...
              esc_url(
                  wp_nonce_url($install_url, "install-theme_" . $theme->slug)
              ),
              __("Install")
          );
          break;
   ...
   printf(__("By %s"), $author); ?>
			...
			<?php if (isset($theme->screenshot_url)): ?>
				<img class="theme-screenshot" src="<?php echo esc_url(
        $theme->screenshot_url . "?ver=" . $theme->version
    ); ?>" alt="" />
			<?php endif; ?>
        ...
        "number" => $theme->num_ratings,
			...
			<input class="theme-preview-url" type="hidden" value="<?php echo esc_url(
       $theme->preview_url
   ); ?>" />
		</div>
		<?php
