...
function wp_ajax_ajax_tag_search()
{
     ...
     */
    $term_search_min_chars = (int) apply_filters(
     ...
     */
    if (
        0 === $term_search_min_chars ||
        strlen($search) < $term_search_min_chars
    ) {
        wp_die();
    }
...
function wp_ajax_wp_compression_test()
{
    ...
    if (isset($_GET["test"])) {
        ...
        if ("1" === $_GET["test"]) {
            ...
            if (
                false !==
                    stripos($_SERVER["HTTP_ACCEPT_ENCODING"], "deflate") &&
...
function wp_ajax_autocomplete_user()
{
     ...
     */
    if (
        isset($_REQUEST["autocomplete_type"]) &&
     ...
     */
    if (
        isset($_REQUEST["autocomplete_field"]) &&
    ...
    foreach ($users as $user) {
        $return[] = [
            ...
            "label" => sprintf(
                _x('%1$s (%2$s)', "user autocomplete result"),
                $user->user_login,
                $user->user_email
            ),
            "value" => $user->$field,
...
function wp_ajax_get_community_events()
{
    ...
    if (is_wp_error($events)) {
        ...
        if (empty($saved_location["ip"]) && !empty($events["location"]["ip"])) {
            ...
            isset($saved_location["ip"]) &&
            !empty($events["location"]["ip"]) &&
            $saved_location["ip"] !== $events["location"]["ip"]
        ) {
            $ip_changed = true;
        }
...
function wp_ajax_dashboard_widgets()
{
    ...
    if (
        "dashboard-user" === $pagenow ||
        "dashboard-network" === $pagenow ||
        "dashboard" === $pagenow
    ) {
...
function _wp_ajax_delete_comment_response($comment_id, $delta = -1)
{
    ...
    $x = new WP_Ajax_Response([
        ...
        "supplemental" => [
            ...
            "postId" => $comment ? $comment->comment_post_ID : "",
            /* translators: %s: Number of comments. */
            "total_items_i18n" => sprintf(
                _n("%s item", "%s items", $total),
                number_format_i18n($total)
            ),
            "total_pages" => (int) ceil($total / $per_page),
            "total_pages_i18n" => number_format_i18n(
...
function _wp_ajax_add_hierarchical_term()
{
    ...
    $parent = isset($_POST["new" . $taxonomy->name . "_parent"])
        ? (int) $_POST["new" . $taxonomy->name . "_parent"]
        : 0;
    ...
    if ("category" === $taxonomy->name) {
        $post_category = isset($_POST["post_category"])
            ? (array) $_POST["post_category"]
            : [];
    } else {
        $post_category =
            isset($_POST["tax_input"]) &&
            isset($_POST["tax_input"][$taxonomy->name])
                ? (array) $_POST["tax_input"][$taxonomy->name]
                : [];
    }
...
function wp_ajax_delete_meta()
{
    ...
    if (
        ...
        !current_user_can("delete_post_meta", $meta->post_id, $meta->meta_key)
    ) {
        wp_die(-1);
    }
...
function wp_ajax_add_link_category($action)
{
    ...
    foreach ($names as $cat_name) {
        ...
        $x->add([
            ...
            "id" => $cat_id,
            "data" =>
                "<li id='link-category-$cat_id'><label for='in-link-category-$cat_id' class='selectit'><input value='" .
                esc_attr($cat_id) .
                "' type='checkbox' checked='checked' name='link_category[]' id='in-link-category-$cat_id'/> $cat_name</label></li>",
            "position" => -1,
        ]);
    }
    $x->send();
...
function wp_ajax_replyto_comment($action)
{
    ...
    if (empty($post->post_status)) {
        ...
        in_array($post->post_status, ["draft", "pending", "trash"], true)
    ) {
    ...
    if ($user->exists()) {
        ...
        if (current_user_can("unfiltered_html")) {
            ...
            if (
                wp_create_nonce("unfiltered-html-comment") !==
                $_POST["_wp_unfiltered_html_comment"]
            ) {
    ...
    if (!empty($_POST["approve_parent"])) {
        ...
        if (
            ...
            (int) $parent->comment_post_ID === $comment_post_id
        ) {
    ...
    $position =
        isset($_POST["position"]) && (int) $_POST["position"]
            ? (int) $_POST["position"]
            : "-1";
...
function wp_ajax_add_menu_item()
{
    ...
    foreach ((array) $item_ids as $menu_item_id) {
        ...
        if (!empty($menu_obj->ID)) {
            ...
            $menu_obj->title = empty($menu_obj->title)
                ? __("Menu Item")
                : $menu_obj->title;
...
function wp_ajax_add_meta()
{
    ...
    if (isset($_POST["metakeyselect"]) || isset($_POST["metakeyinput"])) {
        ...
        if (
            isset($_POST["metakeyselect"]) &&
            "#NONE#" === $_POST["metakeyselect"] &&
            empty($_POST["metakeyinput"])
        ) {
            wp_die(1);
        }
...
function wp_ajax_menu_locations_save()
{
    ...
    set_theme_mod(
        "nav_menu_locations",
        array_map("absint", $_POST["menu-locations"])
    );
    wp_die(1);
}
...
function wp_ajax_meta_box_order()
{
    ...
    $page_columns = isset($_POST["page_columns"])
        ? $_POST["page_columns"]
        : "auto";
...
function wp_ajax_find_posts()
{
    ...
    foreach ($posts as $post) {
        ...
        $html .=
            ...
            esc_html($post_types[$post->post_type]->labels->singular_name) .
...
function wp_ajax_save_widget()
{
    ...
    $multi_number = !empty($_POST["multi_number"])
        ? (int) $_POST["multi_number"]
        : 0;
    $settings =
...
function wp_ajax_media_create_image_subsizes()
{
    ...
    if (!empty($_POST["_wp_upload_failed_cleanup"])) {
        ...
        if (
            ...
            if (
                $attachment &&
                time() - strtotime($attachment->post_date_gmt) < 600
            ) {
...
function wp_ajax_upload_attachment()
{
    ...
    $post_data = !empty($_REQUEST["post_data"])
        ? _wp_get_allowed_postdata(
            _wp_translate_postdata(false, (array) $_REQUEST["post_data"])
        )
        : [];
...
function wp_ajax_set_attachment_thumbnail()
{
    ...
    if (
        false ===
        check_ajax_referer("set-attachment-thumbnail", "_ajax_nonce", false)
    ) {
        wp_send_json_error();
    }
...
{
    wp_die(
        date_i18n(sanitize_option("date_format", wp_unslash($_POST["date"])))
    );
}
...
{
    wp_die(
        date_i18n(sanitize_option("time_format", wp_unslash($_POST["date"])))
    );
}
...
function wp_ajax_wp_remove_post_lock()
{
    ...
    $active_lock = array_map(
        "absint",
        explode(":", $_POST["active_post_lock"])
    );
...
function wp_ajax_query_attachments()
{
    ...
    if (
        current_user_can(
            get_post_type_object("attachment")->cap->read_private_posts
        )
    ) {
...
function wp_ajax_save_attachment()
{
    ...
    if (wp_attachment_is("audio", $post["ID"])) {
        ...
        foreach (
            wp_get_attachment_id3_keys((object) $post, "edit")
            as $key => $label
        ) {
            if (isset($changes[$key])) {
                $changed = true;
                $id3data[$key] = sanitize_text_field(
                    wp_unslash($changes[$key])
                );
            }
        }
...
function wp_ajax_save_attachment_compat()
{
    ...
    if (
        empty($_REQUEST["attachments"]) ||
        empty($_REQUEST["attachments"][$id])
    ) {
        wp_send_json_error();
    }
    ...
    foreach (get_attachment_taxonomies($post) as $taxonomy) {
        if (isset($attachment_data[$taxonomy])) {
            wp_set_object_terms(
                ...
                array_map(
                    "trim",
                    preg_split("/,+/", $attachment_data[$taxonomy])
                ),
                $taxonomy,
                false
            );
...
function wp_ajax_send_attachment_to_editor()
{
    ...
    if (str_starts_with($post->post_mime_type, "image")) {
        ...
        $size = isset($attachment["image-size"])
...
function wp_ajax_query_themes()
{
    ...
    foreach ($api->themes as &$theme) {
        ...
        $theme->block_theme =
            $is_theme_installed && wp_get_theme($theme->slug)->is_block_theme();
        ...
        if (
            ...
            $customize_url = $theme->block_theme
            ...
            $theme->customize_url = add_query_arg(
                [
                    "return" => urlencode(
                        network_admin_url("theme-install.php", "relative")
                    ),
                ],
                $customize_url
            );
        ...
        $theme->compatible_php = is_php_version_compatible(
            $theme->requires_php
        );
    }
...
function wp_ajax_parse_media_shortcode()
{
    ...
    $media_shortcodes = ["audio", "embed", "playlist", "video", "gallery"];
...
function wp_ajax_crop_image()
{
    ...
    switch ($context) {
        case "site-icon":
            ...
            $attachment = wp_copy_parent_attachment_properties(
                $cropped,
                $attachment_id,
                $context
            );
            ...
            $attachment_id = $wp_site_icon->insert_attachment(
                $attachment,
                $cropped
            );
...
function wp_ajax_save_wporg_username()
{
    ...
    wp_send_json_success(
        update_user_meta(get_current_user_id(), "wporg_favorites", $username)
    );
}
...
function wp_ajax_install_plugin()
{
    ...
    if (
        current_user_can("activate_plugin", $install_status["file"]) &&
        is_plugin_inactive($install_status["file"])
    ) {
        $status["activateUrl"] = add_query_arg(
            [
                "_wpnonce" => wp_create_nonce(
                    "activate-plugin_" . $install_status["file"]
                ),
                "action" => "activate",
...
function wp_ajax_delete_plugin()
{
    ...
    $url = wp_nonce_url(
        "plugins.php?action=delete-selected&verify-delete=1&checked[]=" .
            $plugin,
        "bulk-plugins"
    );
...
function wp_ajax_search_install_plugins()
{
    ...
    $pagenow = isset($_POST["pagenow"]) ? sanitize_key($_POST["pagenow"]) : "";
    if (
        "plugin-install-network" === $pagenow ||
...
function wp_ajax_wp_privacy_export_personal_data()
{
    ...
    if (!current_user_can("export_others_personal_data")) {
        wp_send_json_error(
            __("Sorry, you are not allowed to perform this action.")
        );
    }

    check_ajax_referer(
        "wp-privacy-export-personal-data-" . $request_id,
        "security"
    );
    ...
    $send_as_email = isset($_POST["sendAsEmail"])
    ...
    if (!is_array($exporters)) {
        wp_send_json_error(
            __("An exporter has improperly used the registration filter.")
        );
    }
...
function wp_ajax_wp_privacy_erase_personal_data()
{
    ...
    check_ajax_referer(
        "wp-privacy-erase-personal-data-" . $request_id,
        "security"
    );
    ...
    if (0 < count($erasers)) {
        ...
        if (!is_array($eraser)) {
            ...
            wp_send_json_error(
                sprintf(
                    __("Expected an array describing the eraser at index %d."),
                    $eraser_index
                )
            );
        }
...
function wp_ajax_health_check_get_sizes()
{
    ...
    if (
        isset($all_sizes["total_size"]["debug"]) &&
...
function wp_ajax_send_password_reset()
{
    ...
    if (!current_user_can("edit_user", $user_id)) {
        wp_send_json_error(
            __("Cannot send password reset, permission denied.")
        );
    }
    ...
    if (true === $results) {
        wp_send_json_success(
            /* translators: %s: User's display name. */
            sprintf(
                __("A password reset link was emailed to %s."),
                $user->display_name
            )
        );
    } else {
