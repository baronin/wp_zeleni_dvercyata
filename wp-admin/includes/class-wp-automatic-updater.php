...
class WP_Automatic_Updater
{
    ...
    public function should_update($type, $item, $context)
    {
        ...
        if ("core" === $type) {
            ...
            if (!$update && wp_is_auto_update_enabled_for_type($type)) {
                ...
                $auto_updates = (array) get_site_option(
                    "auto_update_{$type}s",
                    []
                );
        ...
        if (in_array($type, ["plugin", "theme"], true)) {
            if (
                !empty($item->requires_php) &&
                version_compare(PHP_VERSION, $item->requires_php, "<")
            ) {
                return false;
            }
        }
    ...
    public function update($type, $item)
    {
        ...
        if ("theme" === $type && $is_debug) {
            error_log(
                "    Upgrading theme " . var_export($item->theme, true) . "..."
            );
        }
        ...
        if ("plugin" === $type) {
            ...
            if ($was_active && !is_wp_error($upgrade_result)) {
                ...
                if ($this->has_fatal_error()) {
                    ...
                    $backup_restored = $upgrader->restore_temp_backup(
                        $temp_backup
                    );
                    if (is_wp_error($backup_restored)) {
                        ...
                        $backup_deleted = $upgrader->delete_temp_backup(
                            $temp_backup
                        );
    ...
    public function run()
    {
        ...
        if (isset($this->update_results["plugin"])) {
            foreach ($this->update_results["plugin"] as $upgrade) {
                $plugin_stats[$upgrade->item->plugin] =
                    true === $upgrade->result;
            }
        }
        ...
        if (!empty($this->update_results)) {
            ...
            if (!empty($this->update_results["core"])) {
                ...
                !empty($this->update_results["plugin"]) ||
                !empty($this->update_results["theme"])
            ) {
    ...
    protected function after_core_update($update_result)
    {
        ...
        if (
            ...
            str_contains($error_code, "__copy_dir")
        ) {
            $critical = true;
        } elseif (
            "rollback_was_required" === $error_code &&
            is_wp_error($result->get_error_data()->rollback)
        ) {
        ...
        if ($critical) {
            ...
            if (isset($rollback_result)) {
                $critical_data[
                    "rollback_code"
                ] = $rollback_result->get_error_code();
                $critical_data[
                    "rollback_data"
                ] = $rollback_result->get_error_data();
            }
        ...
        if (
            in_array($error_code, $transient_failures, true) &&
    ...
    protected function send_email($type, $core_update, $result = null)
    {
        ...
        if (
            "upgrade" === $next_user_core_update->response &&
            version_compare(
        ...
        $critical_support =
            "critical" === $type && !empty($core_update->support_email);
        if ($critical_support) {
        ...
        if (
            ...
            (get_plugin_updates() || get_theme_updates())
        ) {
            $body .=
                "\n\n" .
                __(
        ...
        if ("critical" === $type && is_wp_error($result)) {
            ...
            /* translators: %s: WordPress version. */
            $body .= sprintf(
                __("Your site was running version %s."),
            ...
            if ("rollback_was_required" === $result->get_error_code()) {
                $errors = [
                    $result,
                    $result->get_error_data()->update,
                    $result->get_error_data()->rollback,
                ];
            } else {
                $errors = [$result];
            }
    ...
    protected function after_plugin_theme_update($update_results)
    {
        ...
        if (!empty($update_results["plugin"])) {
             ...
             */
            $notifications_enabled = apply_filters(
     ...
     */
    protected function send_plugin_theme_email(
        $type,
        $successful_updates,
        $failed_updates
    ) {
        ...
        $past_failure_emails = get_option(
            ...
            foreach ($failed_updates as $update_type => $failures) {
                foreach ($failures as $failed_update) {
                    if (
                        !isset(
                            $past_failure_emails[
                                $failed_update->item->{$update_type}
                            ]
                        )
                    ) {
        ...
        if (in_array($type, ["fail", "mixed"], true)) {
            ...
            if (!empty($failed_updates["plugin"])) {
                ...
                foreach ($failed_updates["plugin"] as $item) {
                    ...
                    $past_failure_emails[$item->item->plugin] =
                        $item->item->new_version;
                }
            ...
            if (!empty($failed_updates["theme"])) {
                ...
                foreach ($failed_updates["theme"] as $item) {
                    ...
                    $past_failure_emails[$item->item->theme] =
                        $item->item->new_version;
                }
    ...
    protected function send_debug_email()
    {
        ...
        if (isset($this->update_results["core"])) {
            ...
            if ($result->result && !is_wp_error($result->result)) {
                /* translators: %s: WordPress version. */
                $body[] = sprintf(
                    __("SUCCESS: WordPress was successfully updated to %s"),
                    $result->name
                );
            } else {
                /* translators: %s: WordPress version. */
                $body[] = sprintf(
                    __("FAILED: WordPress failed to update to %s"),
                    $result->name
                );
                ++$failures;
            }
        ...
        foreach (["plugin", "theme", "translation"] as $type) {
            ...
            $success_items = wp_list_filter($this->update_results[$type], [
                "result" => true,
            ]);
...
=========='
    ...
    protected function has_fatal_error()
    {
        ...
        if (is_wp_error($response)) {
            if ($is_debug) {
                error_log(
                    "Loopback request failed: " . $response->get_error_message()
                );
            }
            return true;
        }
