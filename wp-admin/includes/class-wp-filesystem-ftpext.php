...
class WP_Filesystem_FTPext extends WP_Filesystem_Base
{
    ...
    public function __construct($opt = "")
    {
        ...
        if (!extension_loaded("ftp")) {
            $this->errors->add(
                "no_ftp_ext",
                __("The ftp PHP extension is not available")
            );
            return;
        }
    ...
    public function delete($file, $recursive = false, $type = false)
    {
        ...
        if (!empty($filelist)) {
            foreach ($filelist as $delete_file) {
                $this->delete(
                    trailingslashit($file) . $delete_file["name"],
                    $recursive,
                    $delete_file["type"]
                );
            }
        ...
        if (
            ...
            if ($lucifer) {
                ...
                if (8 === $lcount) {
                    ...
                    if (
                        preg_match("/([0-9]{2}):([0-9]{2})/", $lucifer[7], $l2)
                    ) {
                        $b["year"] = gmdate("Y");
                        $b["hour"] = $l2[1];
                        $b["minute"] = $l2[2];
