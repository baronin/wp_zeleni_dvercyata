...
function wp_credits($version = "", $locale = "")
{
    ...
    if (
        ...
        (isset($results["data"]["version"]) &&
            !str_starts_with($version, $results["data"]["version"]))
    ) {
...
{
    $data =
        '<a href="' . esc_url($data[1]) . '">' . esc_html($data[0]) . "</a>";
}
...
{
    $group_data = isset($credits["groups"][$slug])
        ? $credits["groups"][$slug]
        : [];
    ...
    switch ($group_data["type"]) {
        ...
        default:
            ...
            foreach ($group_data["data"] as $person_data) {
                ...
                echo '<a href="' .
                    esc_url(
                        sprintf($credits_data["profiles"], $person_data[2])
                    ) .
                    '" class="web">';
                $size = $compact ? 80 : 160;
                $data = get_avatar_data(
                    $person_data[1] . "@sha256.gravatar.com",
                    ["size" => $size]
                );
                $data2x = get_avatar_data(
                    $person_data[1] . "@sha256.gravatar.com",
                    ["size" => $size * 2]
                );
