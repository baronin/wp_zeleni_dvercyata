...
class WP_MS_Themes_List_Table extends WP_List_Table
{
    ...
    public function __construct($args = [])
    {
        ...
        $status = isset($_REQUEST["theme_status"])
        ...
        $this->is_site_themes =
            "site-themes-network" === $this->screen->id ? true : false;
    ...
    public function prepare_items()
    {
        ...
        $themes = [
            ...
            "disabled" => [],
            "upgrade" => [],
            "broken" => $this->is_site_themes
                ? []
                : wp_get_themes(["errors" => true]),
        ];
        ...
        foreach ((array) $themes["all"] as $key => $theme) {
            ...
            $filter = $theme->is_allowed($allowed_where, $this->site_id)
                ? "enabled"
                : "disabled";
            $themes[$filter][$key] = $themes["all"][$key];

            $theme_data = [
                "update_supported" => isset($theme->update_supported)
                    ? $theme->update_supported
                    : true,
            ];

            // Extra info if known. array_merge() ensures $theme_data has precedence if keys collide.
            if (isset($current->response[$key])) {
                $theme_data = array_merge(
                    (array) $current->response[$key],
                    $theme_data
                );
            } elseif (isset($current->no_update[$key])) {
                $theme_data = array_merge(
                    (array) $current->no_update[$key],
                    $theme_data
                );
            } else {
            ...
            $filter_payload = (object) array_merge(
                $filter_payload,
            ...
            if ($this->show_autoupdates) {
                $enabled =
                    in_array($key, $auto_updates, true) &&
                    $theme->update_supported;
    ...
    public function _search_callback($theme)
    {
        ...
        foreach (
            ["Name", "Description", "Author", "Author", "AuthorURI"]
            as $field
        ) {
            // Don't mark up; Do translate.
            if (
                false !== stripos($theme->display($field, false, true), $term)
            ) {
                return true;
            }
        }
    ...
    protected function get_sortable_columns()
    {
        return [
            "name" => [
                ...
                false,
                __("Theme"),
                __("Table ordered by Theme Name."),
                "asc",
            ],
        ];
    }
    ...
    protected function get_bulk_actions()
    {
        ...
        $actions = [];
        if ("enabled" !== $status) {
            $actions["enable-selected"] = $this->is_site_themes
                ? __("Enable")
                : __("Network Enable");
        }
        if ("disabled" !== $status) {
            $actions["disable-selected"] = $this->is_site_themes
                ? __("Disable")
                : __("Network Disable");
        }
        if (!$this->is_site_themes) {
    ...
    public function column_cb($item)
    {
        ...
        ?>
		<input type="checkbox" name="checked[]" value="<?php echo esc_attr(
    ...
    public function column_description($theme)
    {
            ...
            $pre =
                "broken" === $status
                    ? '<strong class="error-message">' .
                        ...
                        "</strong> "
                    : "";
            wp_admin_notice($pre . $theme->errors()->get_error_message(), [
                "type" => "error",
        ...
        echo "<div class='theme-description'><p>" .
                ...
                case "name":
                    ...
                    if (!empty($this->site_id)) {
                        ...
                        if ($item->get_template() === $template) {
                            $active_theme_label =
                                " &mdash; " . __("Active Theme");
                        }

                        /* In case this is a child theme, label it properly */
                        if (
                            $stylesheet !== $template &&
                            $item->get_stylesheet() === $stylesheet
                        ) {
                            $active_theme_label =
                                " &mdash; " . __("Active Child Theme");
                        }
                    }

                    echo "<td class='theme-title column-primary{$extra_classes}'><strong>" .
                        $item->display("Name") .
                        $active_theme_label .
                        "</strong>";
