...
function wp_insert_link($linkdata, $wp_error = false)
{
    ...
    $link_rating = !empty($parsed_args["link_rating"])
        ? $parsed_args["link_rating"]
        : 0;
    $link_image = !empty($parsed_args["link_image"])
        ? $parsed_args["link_image"]
        : "";
    $link_target = !empty($parsed_args["link_target"])
        ? $parsed_args["link_target"]
        : "";
    $link_visible = !empty($parsed_args["link_visible"])
        ? $parsed_args["link_visible"]
        : "Y";
    $link_owner = !empty($parsed_args["link_owner"])
        ? $parsed_args["link_owner"]
        : get_current_user_id();
    $link_notes = !empty($parsed_args["link_notes"])
        ? $parsed_args["link_notes"]
        : "";
    $link_description = !empty($parsed_args["link_description"])
        ? $parsed_args["link_description"]
        : "";
    $link_rss = !empty($parsed_args["link_rss"])
        ? $parsed_args["link_rss"]
        : "";
    $link_rel = !empty($parsed_args["link_rel"])
        ? $parsed_args["link_rel"]
        : "";
    $link_category = !empty($parsed_args["link_category"])
    ...
    if ($update) {
        if (
            ...
            if ($wp_error) {
                return new WP_Error(
                    "db_update_error",
                    __("Could not update link in the database."),
                    $wpdb->last_error
                );
            } else {
                return 0;
        ...
        if (empty($plugins["link-manager/link-manager.php"])) {
            if (current_user_can("install_plugins")) {
                $install_url = wp_nonce_url(
                    self_admin_url(
                        "update.php?action=install-plugin&plugin=link-manager"
                    ),
