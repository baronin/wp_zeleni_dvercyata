...
class WP_Comments_List_Table extends WP_List_Table
{
    ...
    public function prepare_items()
    {
        ...
        $comment_type = !empty($_REQUEST["comment_type"])
        ...
        $args = [
            "status" => isset($status_map[$comment_status])
                ? $status_map[$comment_status]
                : $comment_status,
            "search" => $search,
            "user_id" => $user_id,
            "offset" => $start,
        ...
        if (is_array($_comments)) {
            ...
            $_comment_post_ids = array_unique(
                wp_list_pluck($_comments, "comment_post_ID")
            );
    ...
    protected function get_views()
    {
        ...
        return apply_filters(
            ...
            $this->get_views_links($status_links)
        );
    }
    ...
    protected function get_bulk_actions()
    {
        ...
        if (!current_user_can("moderate_comments")) {
            return []; // Return an empty array if the user doesn't have permission
        }
        ...
        if (
            in_array($comment_status, ["trash", "spam"], true) ||
            !EMPTY_TRASH_DAYS
        ) {
    ...
    protected function extra_tablenav($which)
    {
        ...
        if (
            ...
            $title =
                "spam" === $comment_status
                    ? esc_attr__("Empty Spam")
                    : esc_attr__("Empty Trash");
    ...
    protected function get_sortable_columns()
    {
        return [
            "author" => [
                ...
                false,
                __("Author"),
                __("Table ordered by Comment Author."),
            ],
            "response" => [
                "comment_post_ID",
                false,
...
<table class="wp-list-table <?php echo implode(
  ...
  ?>
    ...
    public function single_row($item)
    {
        ...
        $this->user_can = current_user_can(
            "edit_comment",
            $comment->comment_ID
        );
    ...
    protected function handle_row_actions($item, $column_name, $primary)
    {
        ...
        $action_string =
            "comment.php?action=%s&c=" . $comment->comment_ID . "&%s";
        ...
        if ("spam" !== $the_comment_status && "trash" !== $the_comment_status) {
            ...
            $format =
                '<button type="button" data-comment-id="%d" data-post-id="%d" data-action="%s" class="%s button-link" aria-expanded="false" aria-label="%s">%s</button>';
        ...
        $output .=
            '<div class="' .
            ($always_visible ? "row-actions visible" : "row-actions") .
            '">';
   ...
   _e("Select comment"); ?>
    ...
    public function column_comment($comment)
    {
        ...
        if ($this->user_can) {
            ...
            $comment_content = apply_filters(
                "comment_edit_pre",
                $comment->comment_content
            ); ?>
		<div id="inline-<?php echo $comment->comment_ID; ?>" class="hidden">
			<textarea class="comment" rows="1" cols="1"><?php echo esc_textarea(
       $comment_content
   ); ?></textarea>
			<div class="author-email"><?php echo esc_html(
       $comment->comment_author_email
   ); ?></div>
			<div class="author"><?php echo esc_html($comment->comment_author); ?></div>
			<div class="author-url"><?php echo esc_url(
    ...
    public function column_author($comment)
    {
        ...
        $author_url_display = untrailingslashit(
            preg_replace("|^http(s)?://(www\.)?|i", "", $author_url)
        );

        if (strlen($author_url_display) > 50) {
            $author_url_display = wp_html_excerpt(
                $author_url_display,
                49,
                "&hellip;"
            );
