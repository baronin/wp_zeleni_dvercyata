...
function wp_terms_checklist($post_id = 0, $args = [])
{
    ...
    if (
        empty($parsed_args["walker"]) ||
        !($parsed_args["walker"] instanceof Walker)
    ) {
    ...
    if (is_array($parsed_args["selected_cats"])) {
        $args["selected_cats"] = array_map(
            "intval",
            $parsed_args["selected_cats"]
        );
    } elseif ($post_id) {
    ...
    if (is_array($parsed_args["popular_cats"])) {
        $args["popular_cats"] = array_map(
            "intval",
            $parsed_args["popular_cats"]
        );
    } else {
...
function wp_popular_terms_checklist(
    ...
    if ($post && $post->ID) {
        $checked_terms = wp_get_object_terms($post->ID, $taxonomy, [
            "fields" => "ids",
        ]);
    } else {
        $checked_terms = [];
    ...
    foreach ((array) $terms as $term) {
        ...
        $checked = in_array($term->term_id, $checked_terms, true)
            ? 'checked="checked"'
            : "";
        ?>
		...
		<li id="<?php echo $id; ?>" class="popular-category">
			<label class="selectit">
				<input id="in-<?php echo $id; ?>" type="checkbox" <?php echo $checked; ?> value="<?php echo (int) $term->term_id; ?>" <?php disabled(
...
function wp_link_category_checklist($link_id = 0)
{
    ...
    foreach ($categories as $category) {
        ...
        $checked = in_array($cat_id, $checked_categories, true)
            ? ' checked="checked"'
            : "";
        echo '<li id="link-category-',
            $cat_id,
        ...
        if ($taxonomy->hierarchical) {
            ...
            if (false === $terms) {
                ...
                wp_cache_add(
                    ...
                    $taxonomy_name . "_relationships"
                );
            }
            $term_ids = empty($terms) ? [] : wp_list_pluck($terms, "term_id");

            echo '<div class="post_category" id="' .
                $taxonomy_name .
                "_" .
                $post->ID .
                '">' .
...
<?php endif; ?>
 ...
 ?>
	...
	<div id="replysubmit" class="submit">
		...
		<?php wp_admin_notice('<p class="error"></p>', [
      "type" => "error",
	...
	<input type="hidden" name="checkbox" id="checkbox" value="<?php echo $checkbox
     ? 1
     : 0; ?>" />
	<input type="hidden" name="mode" id="mode" value="<?php echo esc_attr(
     $mode
 ); ?>" />
	<?php
...
<div class="hidden" id="trash-undo-holder">
	<div class="trash-undo-inside">
		<?php /* translators: %s: Comment author, filled by Ajax. */
...
<thead>
<tr>
<th class="left"><label for="metakeyselect"><?php _ex(
    "Name",
    "meta name"
); ?></label></th>
...
<option value="#NONE#"><?php _e("&mdash; Select &mdash;"); ?></option>
      ...
      if (
          ...
          !current_user_can("add_post_meta", $post->ID, $key)
      ) {
          continue;
      }
      echo "\n<option value='" .
...
<input class="hidden" type="text" id="metakeyinput" name="metakeyinput" value="" aria-label="<?php _e(
...
<div class="submit add-custom-field">
	<?php submit_button(__("Add Custom Field"), "", "addmeta", false, [
     "id" => "newmeta-submit",
...
function touch_time($edit = 1, $for_post = 1, $tab_index = 0, $multi = 0)
{
    ...
    for ($i = 1; $i < 13; $i = $i + 1) {
        ...
        $month .=
            sprintf(__('%1$s-%2$s'), $monthnum, $monthtext) . "</option>\n";
    }
    $month .= "</select></label>";
...
<p>
<a href="#edit_timestamp" class="save-timestamp hide-if-no-js button"><?php _e(
    "OK"
); ?></a>
<a href="#edit_timestamp" class="cancel-timestamp hide-if-no-js button-cancel"><?php _e(
    "Cancel"
); ?></a>
</p>
	<?php
...
function parent_dropdown(
    ...
    if ($items) {
        foreach ($items as $item) {
            ...
            echo "\n\t<option class='level-$level' value='$item->ID' $selected>$pad " .
...
function wp_import_upload_form($action)
{
    ...
    if (!empty($upload_dir["error"])):
        ...
        /* translators: %s: Maximum allowed file size. */
         ...
         ?>
<form enctype="multipart/form-data" id="import-upload-form" method="post" class="wp-upload-form" action="<?php echo esc_url(
    wp_nonce_url($action, "import-upload")
); ?>">
<p>
		<?php printf(
        ...
        foreach (["high", "core", "default", "low"] as $a_priority) {
            ...
            if ("core" === $priority) {
                ...
                if ("default" === $a_priority) {
                    $wp_meta_boxes[$page][$a_context]["core"][$id] =
            ...
            if (empty($priority)) {
                ...
                $callback_args =
                    $wp_meta_boxes[$page][$a_context][$a_priority][$id]["args"];
            }
    ...
    if (empty($plugins["classic-editor/classic-editor.php"])) {
        if (current_user_can("install_plugins")) {
            $install_url = wp_nonce_url(
                self_admin_url(
                    "plugin-install.php?tab=favorites&user=wordpressdotorg&save=0"
                ),
        ...
        if (current_user_can("activate_plugins")) {
            $activate_url = wp_nonce_url(
                self_admin_url(
                    "plugins.php?action=activate&plugin=classic-editor/classic-editor.php"
                ),
...
function do_meta_boxes($screen, $context, $data_object)
{
    ...
    printf(
        '<div id="%s-sortables" class="meta-box-sortables">',
        esc_attr($context)
    );
    ...
    if (isset($wp_meta_boxes[$page][$context])) {
        foreach (["high", "sorted", "core", "default", "low"] as $priority) {
            if (isset($wp_meta_boxes[$page][$context][$priority])) {
                foreach (
                    (array) $wp_meta_boxes[$page][$context][$priority]
                    as $box
                ) {
                    ...
                    if (is_array($box["args"])) {
                        ...
                        if (
                            ...
                            isset($box["args"]["__back_compat_meta_box"]) &&
                        ...
                        if (isset($box["args"]["__back_compat_meta_box"])) {
                            $block_compatible =
                                $block_compatible ||
                                (bool) $box["args"]["__back_compat_meta_box"];
                    ...
                    if ("dashboard_browser_nag" !== $box["id"]) {
                        ...
                        if (
                            is_array($box["args"]) &&
                            isset($box["args"]["__widget_basename"])
                        ) {
                        ...
                        echo '<button type="button" class="handle-order-higher" aria-disabled="false" aria-describedby="' .
                            $box["id"] .
                            '-handle-order-higher-description">';
                        echo '<span class="screen-reader-text">' .
                            /* translators: Hidden accessibility text. */
                            __("Move up") .
                            "</span>";
                        ...
                        echo '<button type="button" class="handle-order-lower" aria-disabled="false" aria-describedby="' .
                            $box["id"] .
                            '-handle-order-lower-description">';
                        echo '<span class="screen-reader-text">' .
                            /* translators: Hidden accessibility text. */
                            __("Move down") .
                            "</span>";
                    ...
                    if (
                        ...
                        if ($plugin) {
                            ...
                            wp_admin_notice($meta_box_not_compatible_message, [
     ...
     foreach (["high", "core", "default", "low"] as $priority) {
         if (isset($wp_meta_boxes[$page][$context][$priority])) {
             foreach ($wp_meta_boxes[$page][$context][$priority] as $box) {
                 ...
                 ++$i;
                 $hidden_class = in_array($box["id"], $hidden, true)
                     ? "hide-if-js"
                     : "";
                 ...
                 ?>
					<li class="control-section accordion-section <?php echo $hidden_class; ?> <?php echo $open_class; ?> <?php echo esc_attr(
     $box["id"]
 ); ?>" id="<?php echo esc_attr($box["id"]); ?>">
						<h3 class="accordion-section-title hndle">
							<button type="button" class="accordion-trigger" aria-expanded="<?php echo $aria_expanded; ?>" aria-controls="<?php echo esc_attr(
    $box["id"]
); ?>-content">
						...
						<div class="accordion-section-content <?php postbox_classes(
          $box["id"],
          $page
      ); ?>" id="<?php echo esc_attr($box["id"]); ?>-content">
							<div class="inside">
    ...
    _e("Close media attachment panel"); ?>
		...
		<div class="find-box-inside">
			<div class="find-box-search">
				<?php if ($found_action) { ?>
					<input type="hidden" name="found_action" value="<?php echo esc_attr(
         $found_action
     ); ?>" />
				<?php } ?>
     ...
     _e("Search"); ?>
				...
				<input type="button" id="find-posts-search" value="<?php esc_attr_e(
        "Search"
    ); ?>" class="button" />
				<div class="clear"></div>
			</div>
...
<title><?php bloginfo(
    "name"
); ?> &rsaquo; <?php echo $title; ?> &#8212; <?php _e("WordPress"); ?></title>
...
var ajaxurl = '<?php echo esc_js(admin_url("admin-ajax.php", "relative")); ?>',
	...
	thousandsSeparator = '<?php echo esc_js(
     $wp_locale->number_format["thousands_sep"]
 ); ?>',
	decimalPoint = '<?php echo esc_js(
 ...
 $admin_body_class .=
     " locale-" .
     sanitize_html_class(strtolower(str_replace("_", "-", get_user_locale())));
...
function get_post_states($post)
{
    ...
    if ((int) get_option("wp_page_for_privacy_policy") === $post->ID) {
        $post_states["page_for_privacy_policy"] = _x(
            "Privacy Policy Page",
            "page label"
        );
    }
...
function get_media_states($post)
{
    ...
    if (current_theme_supports("custom-header")) {
        ...
        if (is_random_header_image()) {
            ...
            if (
                $meta_header === $stylesheet &&
                in_array($post->ID, $header_images, true)
            ) {
        ...
        if (get_theme_support("custom-header", "video") && has_header_video()) {
            $mods = get_theme_mods();
            if (
                isset($mods["header_video"]) &&
                $post->ID === $mods["header_video"]
            ) {
...
function compression_test()
{
    ?>
	<script type="text/javascript">
	var compressionNonce = <?php echo wp_json_encode(
     wp_create_nonce("update_can_compress_scripts")
 ); ?>;
	var testCompression = {
		get : function(test) {
			var x;
...
function get_submit_button(
    ...
    foreach ($type as $t) {
        ...
        $classes[] = in_array($t, $button_shorthand, true)
            ? "button-" . $t
            : $t;
    }
...
function _local_storage_notice()
{
    ...
    wp_admin_notice($local_storage_message, [
