...
class Custom_Image_Header
{
    ...
    public function __construct(
        ...
        add_action("wp_ajax_custom-header-remove", [
            $this,
            "ajax_header_remove",
        ]);
    }
    ...
    public function step()
    {
        ...
        if (
            ...
            (2 === $step &&
                !wp_verify_nonce(
                    $_REQUEST["_wpnonce-custom-header-upload"],
                    "custom-header-upload"
                )) ||
            (3 === $step &&
                !wp_verify_nonce(
                    $_REQUEST["_wpnonce"],
                    "custom-header-crop-image"
                ))
        ) {
            return 1;
        }
    ...
    public function show_header_selector($type = "default")
    {
        ...
        foreach ($headers as $header_key => $header) {
            ...
            $header_alt_text = empty($header["alt_text"])
                ? ""
                : $header["alt_text"];

            echo '<div class="default-header">';
            echo '<label><input name="default-header" type="radio" value="' .
                esc_attr($header_key) .
                '" ' .
                checked($header_url, get_theme_mod("header_image"), false) .
                " />";
            $width = "";
            ...
            echo '<img src="' .
                esc_url(set_url_scheme($header_thumbnail)) .
                '" alt="' .
                esc_attr($header_alt_text) .
                '"' .
                $width .
                " /></label>";
            echo "</div>";
    ...
    public function js_1()
    {
        ...
        if (current_theme_supports("custom-header", "default-text-color")) {
            $default_color = get_theme_support(
                "custom-header",
                "default-text-color"
            );
...
<h1><?php _e("Custom Header"); ?></h1>
  ...
  if (!empty($this->updated)) {
      ...
      wp_admin_notice($updated_message, [
          "id" => "message",
...
<td>
	...
	<div id="headimg" style="<?php echo $header_image_style; ?>">
		...
		<h1><a id="name" class="displaying-header-text" <?php echo $style; ?> onclick="return false;" href="<?php bloginfo(
     "url"
 ); ?>" tabindex="-1"><?php bloginfo("name"); ?></a></h1>
		<div id="desc" class="displaying-header-text" <?php echo $style; ?>><?php bloginfo(
    "description"
); ?></div>
	</div>
			<?php
...
<td>
   ...
   ?>
	...
	<form enctype="multipart/form-data" id="upload-form" class="wp-upload-form" method="post" action="<?php echo esc_url(
     add_query_arg("step", 2)
 ); ?>">
	<p>
		<label for="upload"><?php _e(
      "Choose an image from your computer:"
       ...
       "_wpnonce-custom-header-upload"
			...
			<?php $modal_update_href = add_query_arg(
       [
           "page" => "custom-header",
           "step" => 2,
           "_wpnonce-custom-header-upload" => wp_create_nonce(
               "custom-header-upload"
           ),
       ],
	...
	<p>
		<label for="choose-from-library-link"><?php _e(
      "Or choose an image from your media library:"
		...
		<button id="choose-from-library-link" class="button"
			...
			data-update="<?php esc_attr_e("Set as header"); ?>"><?php _e(
    "Choose Image"
); ?></button>
	</p>
	</form>
...
<td>
	<p><?php _e(
     "You can choose one of your previously uploaded headers, or show a random one."
			...
			<?php $this->show_header_selector("uploaded"); ?>
</td>
</tr>
			<?php endif;
  if (!empty($this->default_headers)): ?>
...
<td>
	...
	<?php endif; ?>
			<?php $this->show_header_selector("default"); ?>
</td>
</tr>
			<?php endif;
  if (get_header_image()): ?>
...
<td>
	<p><?php _e(
     "This will restore the original header image. You will not be able to restore any customizations."
			...
			<?php submit_button(
       __("Restore Original Header Image"),
       "",
       "resetheader",
       false
   ); ?>
...
<td>
	<p>
	<label><input type="checkbox" name="display-header-text" id="display-header-text"<?php checked(
     display_header_text()
 ); ?> /> <?php _e("Show header text with your image."); ?></label>
	</p>
</td>
</tr>
...
<td>
   ...
   $default_color_attr = $default_color
       ? ' data-default-color="' . esc_attr($default_color) . '"'
       : "";
   ...
   echo '<input type="text" name="text-color" id="text-color" value="' .
   ...
   if ($default_color) {
       ...
       echo ' <span class="description hide-if-js">' .
           sprintf(_x("Default: %s", "color"), esc_html($default_color)) .
           "</span>";
   }
   ?>
	</p>
  ...
  ?>
    ...
    public function step_2()
    {
        ...
        if (current_theme_supports("custom-header", "max-width")) {
            $max_width = max(
                $max_width,
                get_theme_support("custom-header", "max-width")
            );
        }
        ...
        if (
            ...
            $this->set_header_image(
                compact("url", "attachment_id", "width", "height")
            );
...
<form method="post" action="<?php echo esc_url(add_query_arg("step", 3)); ?>">
	<p class="hide-if-no-js"><?php _e(
     "Choose the part of the image you want to use as your header."
 ); ?></p>
	<p class="hide-if-js"><strong><?php _e(
     "You need JavaScript to choose a part of the image."
 ); ?></strong></p>

	<div id="crop_image" style="position: relative">
		<img src="<?php echo esc_url($url); ?>" id="upload" width="<?php echo esc_attr(
    $width
); ?>" height="<?php echo esc_attr($height); ?>" alt="" />
	</div>

	<input type="hidden" name="x1" id="x1" value="0" />
	<input type="hidden" name="y1" id="y1" value="0" />
	<input type="hidden" name="width" id="width" value="<?php echo esc_attr(
     $width
 ); ?>" />
	<input type="hidden" name="height" id="height" value="<?php echo esc_attr(
     $height
 ); ?>" />
	<input type="hidden" name="attachment_id" id="attachment_id" value="<?php echo esc_attr(
     $attachment_id
 ); ?>" />
	<input type="hidden" name="oitar" id="oitar" value="<?php echo esc_attr(
     $oitar
 ); ?>" />
	...
	<p class="submit">
      ...
      submit_button(
          __("Skip Cropping, Publish Image as Is"),
          "",
          "skip-cropping",
          false
      );
    ...
    public function step_2_manage_upload()
    {
        ...
        if (!wp_match_mime_types("image", $wp_filetype["type"])) {
            wp_die(
                __("The uploaded file is not a valid image. Please try again.")
            );
        }
    ...
    public function step_3()
    {
        ...
        // Cleanup.
        $medium = str_replace(
            wp_basename($original),
            "midsize-" . wp_basename($original),
            $original
        );
        ...
        if (
            empty($_POST["create-new-attachment"]) &&
            empty($_POST["skip-cropping"])
        ) {
    ...
    final public function get_header_dimensions($dimensions)
    {
        ...
        $has_flex_height = current_theme_supports(
            "custom-header",
            "flex-height"
        );
     ...
     */
    final public function create_attachment_object(
        $cropped,
    ...
    {
        $parent_id = isset($attachment["post_parent"])
    ...
    {
        $header_image_data_setting = $wp_customize->get_setting(
            "header_image_data"
        );
    ...
    public function get_uploaded_header_images()
    {
        ...
        foreach ($header_images as &$header_image) {
            ...
            $header_image["timestamp"] = isset($header_meta[$timestamp_key])
                ? $header_meta[$timestamp_key]
                : "";
            $header_image["alt_text"] = isset($header_meta[$alt_text_key])
                ? $header_meta[$alt_text_key]
                : "";
        }
