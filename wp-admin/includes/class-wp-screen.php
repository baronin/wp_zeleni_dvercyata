...
final class WP_Screen
{
    ...
    public static function get($hook_name = "")
    {
        ...
        if ($hook_name && post_type_exists($hook_name)) {
            ...
            if (
                in_array(
                    $id,
                    ["post-new", "link-add", "media-new", "user-new"],
                    true
                )
            ) {
                $id = substr($id, 0, -4);
        ...
        if (!$hook_name) {
            if (isset($_REQUEST["post_type"])) {
                $post_type = post_type_exists($_REQUEST["post_type"])
                    ? $_REQUEST["post_type"]
                    : false;
            }
            if (isset($_REQUEST["taxonomy"])) {
                $taxonomy = taxonomy_exists($_REQUEST["taxonomy"])
                    ? $_REQUEST["taxonomy"]
                    : false;
            }
                ...
                case "post":
                    ...
                    if ($post_id) {
                        ...
                        if ($post) {
                            ...
                            if (!$replace_editor) {
                                $is_block_editor = use_block_editor_for_post(
                                    $post
                                );
                            }
                        }
        ...
        switch ($base) {
            case "post":
                ...
                if (empty($post_id)) {
                    $is_block_editor = use_block_editor_for_post_type(
                        $post_type
                    );
                }
    ...
    public function render_screen_meta()
    {
        ...
        $old_help = isset(self::$_old_compat_help[$this->id])
            ? self::$_old_compat_help[$this->id]
            : "";
		...
		<div id="screen-meta" class="metabox-prefs">
			...
			<div id="contextual-help-wrap" class="<?php echo esc_attr(
       $help_class
   ); ?>" tabindex="-1" aria-label="<?php esc_attr_e(
    "Contextual Help Tab"
); ?>">
  ...
  ?>
		...
		<?php if (!$this->get_help_tabs() && !$this->show_screen_options()) {
      return;
  } ?>
  ...
  if ($this->show_screen_options()): ?>
			...
			<button type="button" id="show-settings-link" class="button show-settings" aria-controls="screen-options-wrap" aria-expanded="false"><?php _e(
       "Screen Options"
   ); ?></button>
			</div>
			<?php endif;
  if ($this->get_help_tabs()): ?>
			...
			<button type="button" id="contextual-help-link" class="button show-settings" aria-controls="contextual-help-wrap" aria-expanded="false"><?php _e(
       "Help"
   ); ?></button>
			</div>
		<?php endif;?>
    ...
    public function show_screen_options()
    {
         ...
         */
        $this->_screen_settings = apply_filters(
            "screen_settings",
            $this->_screen_settings,
            $this
        );
         ...
         * @param WP_Screen $screen      Current WP_Screen instance.
         */
        $this->_show_screen_options = apply_filters(
  ...
  if (
      ...
      if (isset($_GET["welcome"])) {
          ...
          $welcome_checked = (int) get_user_meta(
              get_current_user_id(),
              "show_welcome_panel",
              true
          );
      ...
      echo '<input type="checkbox" id="wp_welcome_panel-hide"' .
          checked((bool) $welcome_checked, true, false) .
          " />";
  ...
  foreach ($columns as $column => $title) {
      ...
      $id = "$column-hide";
      echo "<label>";
      echo '<input class="hide-column-tog" name="' .
          $id .
          '" type="checkbox" id="' .
          $id .
          '" value="' .
          $column .
          '"' .
          checked(!in_array($column, $hidden, true), true, false) .
          " />";
      echo "$title</label>\n";
  }?>
		</fieldset>
    ...
    public function render_screen_layout()
    {
		...
		<?php for ($i = 1; $i <= $num; ++$i): ?>
			...
			<input type='radio' name='screen_columns' value='<?php echo esc_attr(
       $i
    ...
    public function render_per_page_options()
    {
        ...
        if ("edit_comments_per_page" === $option) {
            $comment_status = isset($_REQUEST["comment_status"])
        ...
        $option
    ); ?>"><?php echo $per_page_label; ?></label>
				<input type="number" step="1" min="1" max="999" class="screen-per-page" name="wp_screen_options[value]"
					id="<?php echo esc_attr($option); ?>"
					value="<?php echo esc_attr($per_page); ?>" />
			<?php endif; ?>
				<input type="hidden" name="wp_screen_options[option]" value="<?php echo esc_attr(
        $option
    ); ?>" />
		</fieldset>
		<?php
    ...
    public function render_view_mode()
    {
         ...
         */
        $view_mode_post_types = apply_filters(
        ...
        if (
            "edit" === $screen->base &&
            !in_array($this->post_type, $view_mode_post_types, true)
        ) {
            return;
        }
		...
		<fieldset class="metabox-prefs view-mode">
			...
			<label for="list-view-mode">
				<input id="list-view-mode" type="radio" name="mode" value="list" <?php checked(
        "list",
        $mode
    ); ?> />
				<?php _e("Compact view"); ?>
    ...
    public function render_screen_reader_content($key = "", $tag = "h2")
    {
        ...
        echo "<$tag class='screen-reader-text'>" .
            $this->_screen_reader_content[$key] .
            "</$tag>";
    }
}
