...
if (!current_user_can("view_site_health_checks")) {
    wp_die(
        __("Sorry, you are not allowed to access site health information."),
        "",
        403
    );
}
...
if ("update_https" === $action) {
    ...
    if (!current_user_can("update_https")) {
        wp_die(
            __("Sorry, you are not allowed to update this site to HTTPS."),
            403
        );
    }
    ...
    wp_redirect(
        add_query_arg("https_updated", (int) $result, wp_get_referer())
    );
    exit();
}
...
<div class="health-check-header">
	...
	<?php if (isset($_GET["https_updated"])) {
      ...
      "The site health check shows information about your WordPress configuration and items that may need your attention."
		...
		<div class="site-health-issues-wrapper hidden" id="health-check-issues-critical">
			<h3 class="site-health-issue-count-title">
				<?php /* translators: %s: Number of critical issues found. */
    printf(
        _n("%s critical issue", "%s critical issues", 0),
       ...
       "Critical issues are items that may have a high impact on your sites performance or security, and resolving these issues should be prioritized."
		...
		<div class="site-health-issues-wrapper hidden" id="health-check-issues-recommended">
			<h3 class="site-health-issue-count-title">
				<?php /* translators: %s: Number of recommended improvements. */
    printf(
        _n("%s recommended improvement", "%s recommended improvements", 0),
        '<span class="issue-count">0</span>'
    ); ?>
			</h3>
