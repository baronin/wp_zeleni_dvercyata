...
function list_core_update($update)
{
    ...
    echo "<p>";
    echo '<input name="version" value="' .
        esc_attr($update->current) .
        '" type="hidden" />';
    echo '<input name="locale" value="' .
        esc_attr($update->locale) .
        '" type="hidden" />';
    if ($show_buttons) {
        if ($first_pass) {
            submit_button(
                $submit,
                $current ? "" : "primary regular",
                "upgrade",
                false
            );
            $first_pass = false;
    ...
    if (
        "en_US" !== $update->locale &&
        (!isset($wp_local_package) || $wp_local_package !== $update->locale)
    ) {
        echo '<p class="hint">' .
            __(
...
function dismissed_updates()
{
    ...
    if ($dismissed) {
		...
		<?php
  echo '<p class="hide-if-no-js"><button type="button" class="button" id="show-dismissed" aria-expanded="false">' .
...
function core_upgrade_preamble()
{
    ...
    if (
        isset($updates[0]->version) &&
        version_compare($updates[0]->version, $wp_version, ">")
    ) {
    ...
    if (
        $updates &&
        (count($updates) > 1 || "latest" !== $updates[0]->response)
    ) {
        echo "<p>" .
            __(
...
function core_auto_updates_settings()
{
    ...
    // Defaults:
    $upgrade_dev =
        get_site_option("auto_update_core_dev", "enabled") === "enabled";
    $upgrade_minor =
        get_site_option("auto_update_core_minor", "enabled") === "enabled";
    $upgrade_major =
        get_site_option("auto_update_core_major", "unset") === "enabled";
    ...
    if (defined("WP_AUTO_UPDATE_CORE")) {
        if (false === WP_AUTO_UPDATE_CORE) {
            ...
            in_array(
                WP_AUTO_UPDATE_CORE,
                ["beta", "rc", "development", "branch-development"],
                true
            )
        ) {
    ...
    /** This filter is documented in wp-admin/includes/class-core-upgrader.php */
    $upgrade_minor = apply_filters(
        "allow_minor_auto_core_updates",
        $upgrade_minor
    );
    /** This filter is documented in wp-admin/includes/class-core-upgrader.php */
    $upgrade_major = apply_filters(
    ...
    $action_url = self_admin_url(
        "update-core.php?action=core-major-auto-updates-settings"
    );
    ?>

	<p class="auto-update-status">
		<?php if ($updater->is_vcs_checkout(ABSPATH)) {
      _e(
      ...
      if ($can_set_update_option) {
          ...
          printf(
              ...
              wp_nonce_url(
                  add_query_arg("value", "disable", $action_url),
      ...
      if ($can_set_update_option) {
          ...
          printf(
              ...
              wp_nonce_url(
                  add_query_arg("value", "enable", $action_url),
                  "core-major-auto-updates-nonce"
              ),
...
<p><input id="upgrade-plugins" class="button" type="submit" value="<?php esc_attr_e(
    "Update Plugins"
); ?>" name="upgrade" /></p>
<table class="widefat updates-table" id="update-plugins-table">
	...
	<tr>
		...
		<td class="manage-column"><label for="plugins-select-all"><?php _e(
      "Select All"
  ); ?></label></td>
	</tr>
	</thead>
 ...
 foreach ((array) $plugins as $plugin_file => $plugin_data) {
     ...
     $plugin_data = (object) _get_plugin_data_markup_translate(
         $plugin_file,
         (array) $plugin_data,
         false,
         true
     ...
     foreach ($preferred_icons as $preferred_icon) {
         if (!empty($plugin_data->update->icons[$preferred_icon])) {
             $icon =
                 '<img src="' .
                 esc_url($plugin_data->update->icons[$preferred_icon]) .
                 '" alt="" />';
             break;
         }
     }
     ...
     if (
         ...
         $compat =
             "<br />" .
             sprintf(
                 __("Compatibility with WordPress %s: Unknown"),
                 $cur_wp_version
             );
     }
     ...
     $requires_php = isset($plugin_data->update->requires_php)
         ? $plugin_data->update->requires_php
         : null;
     ...
     if (isset($plugin_data->update->upgrade_notice)) {
         $upgrade_notice =
             "<br />" . strip_tags($plugin_data->update->upgrade_notice);
     } else {
         $upgrade_notice = "";
     }
	...
	<tr>
		<td class="check-column">
			<?php if ($compatible_php): ?>
				<input type="checkbox" name="checked[]" id="<?php echo $checkbox_id; ?>" value="<?php echo esc_attr(
    $plugin_file
); ?>" />
				<label for="<?php echo $checkbox_id; ?>">
					<span class="screen-reader-text">
					<?php /* translators: Hidden accessibility text. %s: Plugin name. */
 ...
 ?>
	...
	<tr>
		...
		<td class="manage-column"><label for="plugins-select-all-2"><?php _e(
      "Select All"
  ); ?></label></td>
	</tr>
	</tfoot>
</table>
<p><input id="upgrade-plugins-2" class="button" type="submit" value="<?php esc_attr_e(
    "Update Plugins"
); ?>" name="upgrade" /></p>
</form>
	<?php
...
<form method="post" action="<?php echo esc_url(
	...
	<?php wp_nonce_field("upgrade-core"); ?>
<p><input id="upgrade-themes" class="button" type="submit" value="<?php esc_attr_e(
    "Update Themes"
); ?>" name="upgrade" /></p>
<table class="widefat updates-table" id="update-themes-table">
	...
	<tr>
		...
		<td class="manage-column"><label for="themes-select-all"><?php _e(
      "Select All"
  ); ?></label></td>
	</tr>
	</thead>
 ...
 foreach ($themes as $stylesheet => $theme) {
     ...
     $requires_wp = isset($theme->update["requires"])
         ? $theme->update["requires"]
         : null;
     $requires_php = isset($theme->update["requires_php"])
         ? $theme->update["requires_php"]
         : null;
    ...
    $stylesheet
     ...
     printf(__("Select %s"), $theme->display("Name")); ?>
       ...
       $theme->get_screenshot() . "?ver=" . $theme->version
 ...
 ?>
	...
	<tr>
		...
		<td class="manage-column"><label for="themes-select-all-2"><?php _e(
      "Select All"
  ); ?></label></td>
	</tr>
	</tfoot>
</table>
<p><input id="upgrade-themes-2" class="button" type="submit" value="<?php esc_attr_e(
    "Update Themes"
); ?>" name="upgrade" /></p>
</form>
	<?php
...
function list_translation_updates()
{
	...
	<form method="post" action="<?php echo esc_url(
     $form_action
		...
		<p><input class="button" type="submit" value="<?php esc_attr_e(
      "Update Translations"
  ); ?>" name="upgrade" /></p>
	</form>
	<?php
...
function do_core_upgrade($reinstall = false)
{
    ...
    $allow_relaxed_file_ownership =
        !$reinstall && isset($update->new_files) && !$update->new_files;
    ?>
	<div class="wrap">
...
function do_dismiss_core_update()
{
    ...
    wp_redirect(
        wp_nonce_url("update-core.php?action=upgrade-core", "upgrade-core")
    );
    exit();
}
...
$upgrade_error = false;
if (
    ("do-theme-upgrade" === $action ||
        ("do-plugin-upgrade" === $action && !isset($_GET["plugins"]))) &&
    !isset($_POST["checked"])
) {
...
if ("upgrade-core" === $action) {
 ...
 require_once ABSPATH . "wp-admin/admin-footer.php";
	...
	<div class="wrap">
		...
		<iframe src="<?php echo $url; ?>" style="width: 100%; height: 100%; min-height: 750px;" frameborder="0" title="<?php esc_attr_e(
    "Update progress"
); ?>"></iframe>
	</div>
	<?php
