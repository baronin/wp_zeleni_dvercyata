...
if (!current_user_can("export")) {
    wp_die(
        __("Sorry, you are not allowed to export the content of this site.")
    );
}
...
<p><label><input type="radio" name="content" value="all" checked="checked" aria-describedby="all-content-desc" /> <?php _e(
    "All content"
); ?></label></p>
<p class="description" id="all-content-desc"><?php _e(
    "This will contain all of your posts, pages, comments, custom fields, terms, navigation menus, and custom posts."
); ?></p>

<p><label><input type="radio" name="content" value="posts" /> <?php _ex(
    "Posts",
...
<ul id="post-filters" class="export-filters">
  ...
  $authors = $wpdb->get_col(
      "SELECT DISTINCT post_author FROM {$wpdb->posts} WHERE post_type = 'post'"
  );
  wp_dropdown_users([
      "include" => $authors,
      "name" => "post_author",
  ...
  ?>
   ...
   _e("Date range:"); ?>
		...
		<label for="post-start-date" class="label-responsive"><?php _e(
      "Start date:"
  ); ?></label>
		...
		<label for="post-end-date" class="label-responsive"><?php _e(
      "End date:"
  ); ?></label>
	...
	<li>
		<label for="post-status" class="label-responsive"><?php _e(
      "Status:"
  ); ?></label>
   ...
   foreach ($post_statuses as $status): ?>
			<option value="<?php echo esc_attr($status->name); ?>"><?php echo esc_html(
    $status->label
); ?></option>
			<?php endforeach;
   ?>
...
<p><label><input type="radio" name="content" value="pages" /> <?php _e(
    "Pages"
); ?></label></p>
<ul id="page-filters" class="export-filters">
	<li>
		...
		<?php
  $authors = $wpdb->get_col(
      "SELECT DISTINCT post_author FROM {$wpdb->posts} WHERE post_type = 'page'"
  );
  wp_dropdown_users([
      "include" => $authors,
      "name" => "page_author",
  ...
  ?>
   ...
   _e("Date range:"); ?>
		...
		<label for="page-start-date" class="label-responsive"><?php _e(
      "Start date:"
  ); ?></label>
		...
		<label for="page-end-date" class="label-responsive"><?php _e(
      "End date:"
  ); ?></label>
	...
	<li>
		<label for="page-status" class="label-responsive"><?php _e(
      "Status:"
  ); ?></label>
...
<p><label><input type="radio" name="content" value="<?php echo esc_attr(
    $post_type->name
); ?>" /> <?php echo esc_html($post_type->label); ?></label></p>
<?php endforeach; ?>

<p><label><input type="radio" name="content" value="attachment" /> <?php _e(
    "Media"
); ?></label></p>
<ul id="attachment-filters" class="export-filters">
   ...
   _e("Date range:"); ?>
		...
		<label for="attachment-start-date" class="label-responsive"><?php _e(
      "Start date:"
  ); ?></label>
		...
		<label for="attachment-end-date" class="label-responsive"><?php _e(
      "End date:"
  ); ?></label>
