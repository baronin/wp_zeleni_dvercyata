...
wp_localize_script("theme", "_wpThemeSettings", [
    ...
    "settings" => [
        ...
        "installURI" => current_user_can("install_themes")
...
<div class="wrap">
 ...
 if (!empty($tabs["upload"]) && current_user_can("upload_themes")) {
     echo ' <button type="button" class="upload-view-toggle page-title-action hide-if-no-js" aria-expanded="false">' .
         __("Upload Theme") .
         "</button>";
 }
 ?>
	...
	<?php wp_admin_notice(__("The Theme Installer screen requires JavaScript."), [
  ...
  _e("Filter themes list"); ?>
	...
	<div class="wp-filter hide-if-no-js">
		...
		<ul class="filter-links">
			...
			<li><a href="#" data-sort="block-themes"><?php _ex(
       "Block Themes",
       "themes"
   ); ?></a></li>
			<li><a href="#" data-sort="favorites"><?php _ex(
       "Favorites",
       "themes"
   ); ?></a></li>
		</ul>

		<button type="button" class="button drawer-toggle" aria-expanded="false"><?php _e(
      "Feature Filter"
  ); ?></button>
   ...
   if (
       isset($_GET["_wpnonce"]) &&
       wp_verify_nonce(wp_unslash($_GET["_wpnonce"]), $action)
   ) {
       $user = isset($_GET["user"])
           ? wp_unslash($_GET["user"])
   ...
   ?>
       ...
       "If you have marked themes as favorites on WordPress.org, you can browse them here."
			...
			<p class="favorites-username">
				<label for="wporg-username-input"><?php _e(
        "Your WordPress.org username:"
    ); ?></label>
				<input type="hidden" id="wporg-username-nonce" name="_wpnonce" value="<?php echo esc_attr(
        wp_create_nonce($action)
    ); ?>" />
				<input type="search" id="wporg-username-input" value="<?php echo esc_attr(
        $user
    ); ?>" />
				<input type="button" class="button favorites-form-submit" value="<?php esc_attr_e(
        "Get Favorites"
    ); ?>" />
			</p>
		</div>
		...
		<div class="filter-drawer">
			<div class="buttons">
				<button type="button" class="apply-filters button"><?php _e(
        "Apply Filters"
    ); ?><span></span></button>
				<button type="button" class="clear-filters button" aria-label="<?php esc_attr_e(
  ...
  foreach ($feature_list as $feature_group => $features) {
      ...
      foreach ($features as $feature => $feature_name) {
          ...
          echo '<input type="checkbox" id="filter-id-' .
              $feature .
              '" value="' .
              $feature .
              '" /> ';
          echo '<label for="filter-id-' .
              $feature .
              '">' .
  ...
  ?>
        ...
        "Clear current filters"
			...
			<div class="filtered-by">
				...
				<div class="tags"></div>
				<button type="button" class="button-link edit-filters"><?php _e(
        "Edit Filters"
    ); ?></button>
			</div>
		</div>
...
<script id="tmpl-theme" type="text/template">
	...
	<# if ( data.installed ) { #>
		<?php wp_admin_notice(_x("Installed", "theme"), [
      "type" => "success",
	...
	<span class="more-details"><?php _ex(
     "Details &amp; Preview",
     "theme"
 ); ?></span>
	<div class="theme-author">
		<?php /* translators: %s: Theme author name. */
	...
	<div class="theme-id-container">
     ...
     $aria_label = sprintf(_x("Activate %s", "theme"), "{{ data.name }}"); ?>
           ...
           $aria_label
						...
						<# } else { #>
							<button class="button button-primary disabled"><?php _ex(
           "Activated",
           "theme"
       ); ?></button>
						<# } #>
					...
					<# if ( data.customize_url ) { #>
						<# if ( ! data.active ) { #>
							<# if ( ! data.block_theme ) { #>
								<a class="button load-customize" href="{{ data.customize_url }}"><?php _e(
            "Live Preview"
        ); ?></a>
							<# } #>
						<# } else { #>
           ...
           "Customize"
						...
						<# } #>
					<# } else { #>
						<button class="button preview install-theme-preview"><?php _e(
          "Preview"
      ); ?></button>
					<# } #>
				<# } else { #>
					<?php /* translators: %s: Theme name. */
     $aria_label = sprintf(
         _x("Cannot Activate %s", "theme"),
         "{{ data.name }}"
     ); ?>
					<# if ( data.activate_url ) { #>
						<a class="button button-primary disabled" aria-label="<?php echo esc_attr(
          $aria_label
    ...
    _e("Next theme"); ?>
     ...
     $aria_label = sprintf(_x("Activate %s", "theme"), "{{ data.name }}"); ?>
          ...
          "theme"
					...
					<# } #>
				<# } else { #>
					<a class="button button-primary disabled" ><?php _ex(
         "Cannot Activate",
         "theme"
     ); ?></a>
				<# } #>
			<# } else { #>
				<# if ( data.compatible_wp && data.compatible_php ) { #>
					<a href="{{ data.install_url }}" class="button button-primary theme-install" data-name="{{ data.name }}" data-slug="{{ data.id }}"><?php _e(
         "Install"
     ); ?></a>
				<# } else { #>
					<a class="button button-primary disabled" ><?php _ex(
      ...
      printf(__("By %s"), "{{ data.author }}"); ?>
         ...
         printf(__("(%s ratings)"), "{{ data.num_ratings }}"); ?>
						...
						<# } else { #>
							<span class="no-rating"><?php _e(
           "This theme has not been rated yet."
       ); ?></span>
						<# } #>

						<div class="theme-version">
							<?php /* translators: %s: Theme version. */
       printf(__("Version: %s"), "{{ data.version }}"); ?>
         ...
         ?>
			...
			<div class="wp-full-overlay-footer">
				<button type="button" class="collapse-sidebar button" aria-expanded="true" aria-label="<?php esc_attr_e(
        "Collapse Sidebar"
    ); ?>">
		...
		<iframe src="{{ data.preview_url }}" title="<?php esc_attr_e(
      "Preview"
  ); ?>"></iframe>
	</div>
</script>
