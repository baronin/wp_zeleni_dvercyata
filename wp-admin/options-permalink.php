...
if ($iis7_permalinks) {
    if (
        (!file_exists($home_path . "web.config") &&
            win_is_writable($home_path)) ||
        win_is_writable($home_path . "web.config")
    ) {
        $writable = true;
    } else {
        $writable = false;
    ...
    if (
        (!file_exists($home_path . ".htaccess") && is_writable($home_path)) ||
        is_writable($home_path . ".htaccess")
    ) {
        $writable = true;
    } else {
        $writable = false;
        $existing_rules = array_filter(
            extract_from_markers($home_path . ".htaccess", "WordPress")
        );
        $new_rules = array_filter(
...
$default_structures = [
    ...
    [
        ...
        "example" =>
            ...
            gmdate("Y/m/d") .
            "/" .
            _x("sample-post", "sample permalink structure") .
            "/",
    ],
    [
        "id" => "month-name",
    ...
    [
        ...
        "value" =>
            $index_php_prefix .
            "/" .
            _x("archives", "sample permalink base") .
            "/%post_id%",
        "example" =>
            $url_base . "/" . _x("archives", "sample permalink base") . "/123",
    ],
    [
        "id" => "post-name",
        "label" => __("Post name"),
    ...
    _e("Permalink structure"); ?>
			...
			<div class="row">
				<input id="permalink-input-<?php echo esc_attr($input["id"]); ?>"
					name="selection" aria-describedby="permalink-<?php echo esc_attr(
         $input["id"]
     ); ?>"
			...
			<div class="row">
				<input id="custom_selection"
					...
					<?php checked(
         !in_array($permalink_structure, $default_structure_values, true)
     ); ?>
				/>
				<div>
	...
	<?php if ($iis7_permalinks):
     if (
        ...
        "<kbd>⌘ + A</kbd>"
          ...
          $wp_rewrite->iis7_url_rewrite_rules(true)
      ); ?></textarea>
					</p>
				</form>
				<p>
     ...
     ?>
	<?php
     /* translators: 1: .htaccess, 2: Documentation URL, 3: Ctrl + A, 4: ⌘ + A */ // End if ! $writable && $htaccess_update_required.
