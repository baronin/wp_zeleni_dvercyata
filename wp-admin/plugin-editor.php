...
if (empty($plugins)) {
	...
	<div class="wrap">
		...
		<?php wp_admin_notice(__("No plugins are currently available."), [
      "id" => "message",
...
if (empty($plugin)) {
    if ($file) {
        ...
        foreach (array_keys($plugins) as $plugin_candidate) {
            if (
                $plugin_candidate === $file ||
                ("." !== $file_dirname &&
                    dirname($plugin_candidate) === $file_dirname)
            ) {
...
if ("POST" === $_SERVER["REQUEST_METHOD"]) {
    ...
    if (is_wp_error($r)) {
        $edit_error = $r;
        if (
            check_ajax_referer("edit-plugin_" . $file, "nonce", false) &&
...
if (!is_file($real_file)) {
    ...
    if (preg_match('/\.([^.]+)$/', $real_file, $matches)) {
        ...
        if (!in_array($ext, $editable_extensions, true)) {
            wp_die(
                sprintf("<p>%s</p>", __("Files of this type are not editable."))
            );
        }
    }
}
...
<div class="alignright">
	<form action="plugin-editor.php" method="get">
		<label for="plugin" id="theme-plugin-editor-selector"><?php _e(
      "Select plugin to edit:"
  ); ?> </label>
		<select name="plugin" id="plugin">
		<?php foreach ($plugins as $plugin_key => $a_plugin) {
...
<div id="templateside">
 ...
 foreach ($plugin_files as $plugin_file) {
     if (
         ...
         in_array($matches[1], $editable_extensions, true)
     ) {
...
<form name="template" id="template" action="plugin-editor.php" method="post">
	...
	<?php if (is_writable($real_file)): ?>
		...
		<?php if (in_array($plugin, (array) get_option("active_plugins", []), true)) {
      wp_admin_notice(
          __(
