...
switch ($action) {
    case "post-quickdraft-save":
        ...
        $_POST["ping_status"] = get_default_comment_status(
            $post->post_type,
            "pingback"
        );
    ...
    case "editpost":
        ...
        if (
            isset($_COOKIE["wp-saving-post"]) &&
            $_COOKIE["wp-saving-post"] === $post_id . "-check"
        ) {
            setcookie(
                "wp-saving-post",
                $post_id . "-saved",
    ...
    case "trash":
        ...
        if (!current_user_can("delete_post", $post_id)) {
            wp_die(
                __("Sorry, you are not allowed to move this item to the Trash.")
            );
        }
