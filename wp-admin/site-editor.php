...
function _wp_get_site_editor_redirection_url()
{
    ...
    if (
        isset($_REQUEST["postType"]) &&
        "wp_navigation" === $_REQUEST["postType"] &&
        !empty($_REQUEST["postId"])
    ) {
        return add_query_arg(
            ["p" => "/wp_navigation/" . $_REQUEST["postId"]],
            remove_query_arg(["postType", "postId"])
        );
    }
    ...
    if (
        ...
        return add_query_arg(
            ["p" => "/navigation"],
            remove_query_arg("postType")
        );
    }

    if (isset($_REQUEST["path"]) && "/wp_global_styles" === $_REQUEST["path"]) {
        return add_query_arg(["p" => "/styles"], remove_query_arg("path"));
    }

    if (
        isset($_REQUEST["postType"]) &&
        "page" === $_REQUEST["postType"] &&
        (empty($_REQUEST["canvas"]) || empty($_REQUEST["postId"]))
    ) {
        return add_query_arg(["p" => "/page"], remove_query_arg("postType"));
    }
    ...
    if (
        isset($_REQUEST["postType"]) &&
        "wp_block" === $_REQUEST["postType"] &&
        (empty($_REQUEST["canvas"]) || empty($_REQUEST["postId"]))
    ) {
        return add_query_arg(["p" => "/pattern"], remove_query_arg("postType"));
    }
    ...
    if (
        isset($_REQUEST["postType"]) &&
        "wp_template_part" === $_REQUEST["postType"] &&
        (empty($_REQUEST["canvas"]) || empty($_REQUEST["postId"]))
    ) {
        return add_query_arg(["p" => "/pattern"]);
    }
    ...
    if (
        ...
        return add_query_arg(
            ["p" => "/wp_template_part/" . $_REQUEST["postId"]],
            remove_query_arg(["postType", "postId"])
        );
    }
    ...
    if (
        isset($_REQUEST["path"]) &&
        "/wp_template_part/all" === $_REQUEST["path"]
    ) {
        return add_query_arg(
            [
                "p" => "/pattern",
    ...
    if (isset($_REQUEST["path"]) && "/page" === $_REQUEST["path"]) {
        return add_query_arg(["p" => "/page"], remove_query_arg("path"));
    }

    if (isset($_REQUEST["path"]) && "/wp_template" === $_REQUEST["path"]) {
        return add_query_arg(["p" => "/template"], remove_query_arg("path"));
    }

    if (isset($_REQUEST["path"]) && "/patterns" === $_REQUEST["path"]) {
        return add_query_arg(["p" => "/pattern"], remove_query_arg("path"));
    }

    if (isset($_REQUEST["path"]) && "/navigation" === $_REQUEST["path"]) {
        return add_query_arg(["p" => "/navigation"], remove_query_arg("path"));
    }

    return add_query_arg(["p" => "/"]);
}
...
if (!empty($_GET["postId"]) && is_numeric($_GET["postId"])) {
    ...
    isset($_GET["p"]) &&
    preg_match('/^\/page\/(\d+)$/', $_GET["p"], $matches)
) {
...
] = WP_Block_Patterns_Registry::get_instance()->get_all_registered(true);
$custom_settings[
...
$preload_paths = [
    ...
    ["/wp/v2/global-styles/" . $active_global_styles_id, "OPTIONS"],
    ...
    if ($route_for_post) {
        ...
        if ("page" === $block_editor_context->post->post_type) {
            $preload_paths[] = add_query_arg(
                ...
                empty($block_editor_context->post->post_name)
                    ? "page"
