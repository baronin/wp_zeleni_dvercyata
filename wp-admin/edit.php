...
foreach (["p", "attachment_id", "page_id"] as $_redirect) {
    if (!empty($_REQUEST[$_redirect])) {
        wp_redirect(
            admin_url("edit-comments.php?p=" . absint($_REQUEST[$_redirect]))
        );
        exit();
    }
}
...
$bulk_messages["wp_block"] = [
    ...
    "locked" =>
        1 === $bulk_counts["locked"]
            ? __("1 pattern not updated, somebody is editing it.")
            : /* translators: %s: Number of patterns. */
            _n(
    ...
    if ("untrashed" === $message && isset($_REQUEST["ids"])) {
        ...
        if (1 === count($ids) && current_user_can("edit_post", $ids[0])) {
            $messages[] = sprintf(
                ...
                esc_html(
                    get_post_type_object(get_post_type($ids[0]))->labels
                        ->edit_item
                )
            );
        }
...
<?php $wp_list_table->search_box(
    $post_type_object->labels->search_items,
    "post"
); ?>

<input type="hidden" name="post_status" class="post_status_page" value="<?php echo !empty(
    $_REQUEST["post_status"]
)
    ? esc_attr($_REQUEST["post_status"])
    : "all"; ?>" />
<input type="hidden" name="post_type" class="post_type_page" value="<?php echo esc_attr(
    $post_type
); ?>" />

<?php if (!empty($_REQUEST["author"])) { ?>
<input type="hidden" name="author" value="<?php echo esc_attr(
    $_REQUEST["author"]
); ?>" />
<?php } ?>
