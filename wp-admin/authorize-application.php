...
if (
    isset($_POST["action"]) &&
    "authorize_application_password" === $_POST["action"]
) {
...
$success_url = !empty($_REQUEST["success_url"])
    ? $_REQUEST["success_url"]
    : null;
...
if (is_wp_error($is_valid)) {
    wp_die(
        __("The Authorize Application request is not allowed.") .
            " " .
...
if (wp_is_site_protected_by_basic_auth("front")) {
    wp_die(
        ...
        [
            ...
            "link_url" => $reject_url
                ? add_query_arg("error", "disabled", $reject_url)
                : admin_url(),
        ]
    );
}
     ...
     ?>
      ...
      /* translators: %s: The URL the user is being redirected to. */
       ...
       admin_url("authorize-application.php")
				...
				<input type="hidden" name="success_url" value="<?php echo esc_url(
        $success_url
    ); ?>" />
				<input type="hidden" name="reject_url" value="<?php echo esc_url(
        $reject_url
    ); ?>" />

				<div class="form-field">
					<label for="app_name"><?php _e("New Application Password Name"); ?></label>
					<input type="text" id="app_name" name="app_name" value="<?php echo esc_attr(
         $app_name
     ); ?>" required />
				</div>

				<?php do_action("wp_authorize_application_password_form", $request, $user); ?>
