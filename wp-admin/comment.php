...
switch ($action) {
    ...
    case "spam":
        ...
        if (
            str_replace("1", "approve", $comment->comment_approved) === $action
        ) {
        ...
        $nonce_action =
            "approve" === $action ? "approve-comment_" : "delete-comment_";
...
<tr>
	...
	<td><a href="<?php comment_author_url($comment); ?>"><?php comment_author_url(
    $comment
); ?></a></td>
</tr>
<?php } ?>
<tr>
	<th scope="row"><?php /* translators: Column name or table row header. */ _e(
     "In response to"
 ); ?></th>
	<td>
		<?php
...
<tr>
	<th scope="row"><?php /* translators: Field name in comment form. */ _ex(
     "Comment",
     "noun"
 ); ?></th>
	<td class="comment-content">
       ...
       admin_url("comment.php?action=editcomment&c={$comment->comment_ID}")
   ); ?>"><?php esc_html_e("Edit"); ?></a>
		</p>
	</td>
</tr>
</table>
...
<form action="comment.php" method="get" class="comment-ays-submit">
    ...
    case "unapprovecomment":
        ...
        if (
            ...
            in_array($action, ["approvecomment", "unapprovecomment"], true)
        ) {
            $redir = admin_url(
                "edit-comments.php?p=" . absint($comment->comment_post_ID)
            );
        } else {
    ...
    case "editedcomment":
        ...
        $location =
            (empty($_POST["referredby"])
                ? "edit-comments.php?p=$comment_post_id"
                : $_POST["referredby"]) .
            "#comment-" .
            $comment_id;
