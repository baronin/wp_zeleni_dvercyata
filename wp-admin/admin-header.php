...
$admin_body_class .=
    " branch-" . str_replace([".", ","], "-", (float) get_bloginfo("version"));
$admin_body_class .=
    " version-" .
    str_replace(
        ".",
...
if (is_child_theme()) {
    $admin_body_class .=
        " wp-child-theme-" . sanitize_html_class(get_stylesheet());
}
...
if (
    ...
    (E_NOTICE !== $error_get_last["type"] ||
        "wp-config.php" !== wp_basename($error_get_last["file"]))
) {
...
?>
<body class="wp-admin wp-core-ui no-js <?php echo esc_attr(
    $admin_body_classes
); ?>">
<script type="text/javascript">
...
<?php // Make sure the customize body classes are correct as early as possible.
