...
switch ($action) {
    case "add-menu-item":
        ...
        if (isset($_REQUEST["nav-menu-locations"])) {
            set_theme_mod(
                "nav_menu_locations",
                array_map("absint", $_REQUEST["menu-locations"])
            );
    ...
    case "move-down-menu-item":
        ...
        $menu_item_id = isset($_REQUEST["menu-item"])
            ? (int) $_REQUEST["menu-item"]
            : 0;

        if (is_nav_menu_item($menu_item_id)) {
            $menus = isset($_REQUEST["menu"])
                ? [(int) $_REQUEST["menu"]]
                : wp_get_object_terms($menu_item_id, "nav_menu", [
                    "fields" => "ids",
                ]);
            ...
            if (!is_wp_error($menus) && !empty($menus[0])) {
                ...
                $menu_item_data = (array) wp_setup_nav_menu_item(
                ...
                foreach (
                    (array) $ordered_menu_items
                    as $ordered_menu_item_object
                ) {
                ...
                if (
                    ...
                    $next_item_id =
                        $orders_to_dbids[$dbids_to_orders[$menu_item_id] + 1];
                    $next_item_data = (array) wp_setup_nav_menu_item(
                        get_post($next_item_id)
                    );
                    ...
                    if (
                        ...
                        (empty($next_item_data["menu_item_parent"]) ||
                            (int) $next_item_data["menu_item_parent"] !==
                        ...
                        $parent_object = wp_setup_nav_menu_item(
                            get_post($parent_db_id)
                        );

                        if (!is_wp_error($parent_object)) {
                            $parent_data = (array) $parent_object;
                            $menu_item_data["menu_item_parent"] =
                                $parent_data["menu_item_parent"];

                            // Reset invalid `menu_item_parent`.
                            $menu_item_data = _wp_reset_invalid_menu_item_parent(
                                $menu_item_data
                            );
                        ...
                        $next_item_data["menu_order"] =
                            $next_item_data["menu_order"] - 1;
                        $menu_item_data["menu_order"] =
                            $menu_item_data["menu_order"] + 1;
                    ...
                    $menu_item_data["menu_item_parent"] = (int) get_post_meta(
                        $menu_item_data["menu_item_parent"],
                        "_menu_item_menu_item_parent",
                        true
                    );
    ...
    case "move-up-menu-item":
        ...
        if (is_nav_menu_item($menu_item_id)) {
            ...
            if (!is_wp_error($menus) && !empty($menus[0])) {
                ...
                if (
                    !empty($dbids_to_orders[$menu_item_id]) &&
                    !empty(
                        $orders_to_dbids[$dbids_to_orders[$menu_item_id] - 1]
                    )
                ) {
                    ...
                    if (
                        ...
                        if (!is_wp_error($parent_object)) {
                            ...
                            if (
                                ...
                                !empty(
                                    $orders_to_dbids[
                                        $dbids_to_orders[$parent_db_id] - 1
                                    ]
                                ) &&
                                ...
                                $_possible_parent_id = (int) get_post_meta(
                                    $orders_to_dbids[
                                        $dbids_to_orders[$parent_db_id] - 1
                                    ],
                                    "_menu_item_menu_item_parent",
    ...
    case "update":
        ...
        if (0 === $nav_menu_selected_id) {
            ...
            if ($new_menu_title) {
                $_nav_menu_selected_id = wp_update_nav_menu_object(0, [
                    "menu-name" => $new_menu_title,
                ]);
                ...
                if (is_wp_error($_nav_menu_selected_id)) {
                    ...
                    $_menu_object = wp_get_nav_menu_object(
                    ...
                    if (isset($_REQUEST["menu-item"])) {
                        wp_save_nav_menu_items(
                            $nav_menu_selected_id,
                            absint($_REQUEST["menu-item"])
                        );
                    }

                    if (
                        isset($_REQUEST["zero-menu-state"]) ||
                        !empty($_POST["auto-add-pages"])
                    ) {
                    ...
                    wp_redirect(
                        admin_url(
                            "nav-menus.php?menu=" . $_nav_menu_selected_id
                        )
                    );
                    exit();
                }
            ...
            foreach ($locations as $location => $description) {
                if (
                    (empty($_POST["menu-locations"]) ||
                        empty($_POST["menu-locations"][$location])) &&
            ...
            if (!is_wp_error($_menu_object)) {
                $_nav_menu_selected_id = wp_update_nav_menu_object(
                    $nav_menu_selected_id,
            ...
            if (!is_wp_error($_menu_object)) {
                ...
                if ($nav_menu_selected_id !== $_nav_menu_selected_id) {
                    wp_redirect(
                        admin_url(
                            "nav-menus.php?menu=" . (int) $_nav_menu_selected_id
                        )
                    );
                    exit();
                }
    ...
    case "locations":
        ...
        if (isset($_POST["menu-locations"])) {
            ...
            $messages[] = wp_get_admin_notice(__("Menu locations updated."), [
                "id" => "message",
                "additional_classes" => ["updated"],
                "dismissible" => true,
            ]);
...
$add_new_screen =
    isset($_GET["menu"]) && 0 === (int) $_GET["menu"] ? true : false;

$locations_screen =
    isset($_GET["action"]) && "locations" === $_GET["action"] ? true : false;
 ...
 */
if (
    1 === count(get_registered_nav_menus()) &&
    !$add_new_screen &&
    empty($nav_menus) &&
...
if (
    $nav_menu_selected_id !== $recently_edited &&
    is_nav_menu($nav_menu_selected_id)
) {
    update_user_meta(
        $current_user->ID,
...
if (!$nav_menu_selected_title && is_nav_menu($nav_menu_selected_id)) {
    $_menu_object = wp_get_nav_menu_object($nav_menu_selected_id);
    $nav_menu_selected_title = !is_wp_error($_menu_object)
        ? $_menu_object->name
        : "";
}
...
foreach ((array) $nav_menus as $key => $_nav_menu) {
    $nav_menus[$key]->truncated_name = wp_html_excerpt(
        $_nav_menu->name,
        40,
        "&hellip;"
    );
...
<div class="wrap">
 ...
 if (
     !isset($_GET["action"]) ||
     (isset($_GET["action"]) && "locations" !== $_GET["action"])
 ) {
 ...
 ?>
	...
	<nav class="nav-tab-wrapper wp-clearfix" aria-label="<?php esc_attr_e(
     "Secondary menu"
 ); ?>">
		<a href="<?php echo esc_url(
      admin_url("nav-menus.php")
  ); ?>" class="nav-tab<?php echo $nav_tab_active_class; ?>"<?php echo $nav_aria_current; ?>><?php esc_html_e(
    "Edit Menus"
); ?></a>
   ...
   ); ?>" class="nav-tab<?php echo $active_tab_class; ?>"<?php echo $aria_current; ?>><?php esc_html_e(
    "Manage Locations"
); ?></a>
			<?php
  } ?>
 ...
 endforeach; ?>
	...
	<div id="menu-locations-wrap">
      ...
      add_query_arg(["action" => "locations"], admin_url("nav-menus.php"))
			...
			<table class="widefat fixed" id="menu-locations-table">
				<thead>
				<tr>
					<th scope="col" class="manage-column column-locations"><?php _e(
         "Theme Location"
     ); ?></th>
					<th scope="col" class="manage-column column-menus"><?php _e(
         "Assigned Menu"
     ); ?></th>
				</tr>
				</thead>
            ...
            $data_orig = "";
            $selected =
                isset($menu_locations[$_location]) &&
                $menu_locations[$_location] === $menu->term_id;

            if ($selected) {
                $data_orig = 'data-orig="true"';
            }
            ?>
									<option <?php echo $data_orig; ?> <?php selected(
     $selected
     ...
     ?>
			...
			<p class="button-controls wp-clearfix"><?php submit_button(
       __("Save Changes"),
       "primary left",
       "nav-menu-locations",
       false
			...
			<input type="hidden" name="menu" id="nav-menu-meta-object-id" value="<?php echo esc_attr(
       $nav_menu_selected_id
   ); ?>" />
		</form>
  ...
  if ($menu_count < 1): ?>
       ...
       esc_url(
           add_query_arg(
			...
			<span class="screen-reader-text">
				<?php _e("Click the Save Menu button to save your changes."); ?>
			</span>
		</span><!-- /add-edit-menu-action -->
		<?php else: ?>
			<form method="get" action="<?php echo esc_url(admin_url("nav-menus.php")); ?>">
			<input type="hidden" name="action" value="edit" />
			<label for="select-menu-to-edit" class="selected-menu"><?php _e(
       "Select a menu to edit:"
         ...
         "&mdash; Select &mdash;"
				...
				<?php foreach ((array) $nav_menus as $_nav_menu): ?>
					<option value="<?php echo esc_attr($_nav_menu->term_id); ?>" <?php selected(
    $_nav_menu->term_id,
    $nav_menu_selected_id
); ?>>
						<?php
      ...
      if (
          !empty($menu_locations) &&
          in_array($_nav_menu->term_id, $menu_locations, true)
      ) {
          $locations_assigned_to_this_menu = [];

          foreach (
              array_keys($menu_locations, $_nav_menu->term_id, true)
              as $menu_location_key
          ) {
              if (isset($locations[$menu_location_key])) {
                  $locations_assigned_to_this_menu[] =
                      $locations[$menu_location_key];
              }
          }

          $locations_listed_per_menu = absint(
              apply_filters("wp_nav_locations_listed_per_menu", 3)
          );

          $assigned_locations = array_slice(
              $locations_assigned_to_this_menu,
              0,
          ...
          if (!empty($assigned_locations)) {
              printf(
                  ...
                  count($locations_assigned_to_this_menu) >
                  count($assigned_locations)
                      ? " &hellip;"
                      : ""
              );
          }
      ...
      ?>
			...
			<span class="submit-btn"><input type="submit" class="button" value="<?php esc_attr_e(
       "Select"
   ); ?>"></span>
  ...
  ?>
	...
	<div id="menu-management-liquid">
     ...
     $menu_name_aria_desc = $add_new_screen
     ...
     ?>
					...
					<input type="hidden" name="menu" id="menu" value="<?php echo esc_attr(
         $nav_menu_selected_id
     ); ?>" />
					<div id="nav-menu-header">
						<div class="major-publishing-actions wp-clearfix">
							<label class="menu-name-label" for="menu-name"><?php _e("Menu Name"); ?></label>
							<input name="menu-name" id="menu-name" type="text" class="menu-name regular-text menu-item-textbox form-required" required="required" <?php echo $menu_name_val .
           $menu_name_aria_desc; ?> />
							<div class="publishing-action">
								<?php submit_button(
            empty($nav_menu_selected_id) ? __("Create Menu") : __("Save Menu"),
            "primary large menu-save",
            "save_menu",
            false,
            ["id" => "save_menu_header"]
        ); ?>
        ...
        ?>
								...
								<?php if (isset($edit_markup) && !is_wp_error($edit_markup)) {
            echo $edit_markup;
        } else {
             ?>
							...
							<?php
       if ($add_new_screen): ?>
								<p class="post-body-plain" id="menu-name-desc"><?php _e(
            "Give your menu a name, then click Create Menu."
        ); ?></p>
								<?php if (isset($_GET["use-location"])): ?>
									<input type="hidden" name="use-location" value="<?php echo esc_attr(
             $_GET["use-location"]
         ); ?>" />
								<?php endif; ?>
       ...
       ?>
							...
							<?php if (!$add_new_screen): ?>
								<div id="nav-menu-bulk-actions-bottom" class="bulk-actions" <?php echo $hide_style; ?>>
									...
									<input type="button" class="deletion menu-items-delete disabled" value="<?php esc_attr_e(
            ...
            if (!isset($auto_add["auto_add"])) {
                ...
                false !==
                array_search($nav_menu_selected_id, $auto_add["auto_add"], true)
            ) {
                $auto_add = true;
            } else {
                $auto_add = false;
								...
								<fieldset class="menu-settings-group auto-add-pages">
									<legend class="menu-settings-group-name howto"><?php _e(
             "Auto add pages"
         ); ?></legend>
									<div class="menu-settings-input checkbox-input">
										<input type="checkbox"<?php checked(
              $auto_add
          ); ?> name="auto-add-pages" id="auto-add-pages" value="1" /> <label for="auto-add-pages"><?php printf(
     ...
     esc_url(admin_url("edit.php?post_type=page"))
              ...
              ?>
											<div class="menu-settings-input checkbox-input">
												<input type="checkbox"<?php checked(
                $checked
            ); ?> name="menu-locations[<?php echo esc_attr(
     $location
 ); ?>]" id="locations-<?php echo esc_attr(
    $location
); ?>" value="<?php echo esc_attr($nav_menu_selected_id); ?>" />
												<label for="locations-<?php echo esc_attr(
                $location
            ); ?>"><?php echo $description; ?></label>
												<?php if (
                !empty($menu_locations[$location]) &&
                $menu_locations[$location] !== $nav_menu_selected_id
            ): ?>
