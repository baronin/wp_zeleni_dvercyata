...
if (isset($_REQUEST["action"]) && "add-site" === $_REQUEST["action"]) {
    ...
    if (!is_wp_error($id)) {
        if (
            ...
            !get_user_option("primary_blog", $user_id)
        ) {
            update_user_option($user_id, "primary_blog", $id, true);
        }

        wpmu_new_site_admin_notification($id, $user_id);
        wpmu_welcome_notification($id, $user_id, $password, $title, [
            "public" => 1,
        ]);
        wp_redirect(
            add_query_arg(
...
if (isset($_GET["update"])) {
    ...
    if ("added" === $_GET["update"]) {
        $messages[] = sprintf(
            ...
            __(
                'Site added. <a href="%1$s">Visit Dashboard</a> or <a href="%2$s">Edit Site</a>'
            ),
...
<form method="post" action="<?php echo esc_url(
    network_admin_url("site-new.php?action=add-site")
...
<?php wp_nonce_field("add-blog", "_wpnonce_add-blog"); ?>
	<table class="form-table" role="presentation">
     ...
     ?>
			...
			<?php
   if (is_subdomain_install()) { ?>
				<input name="blog[domain]" type="text" class="regular-text ltr" id="site-address" aria-describedby="site-address-desc" autocapitalize="none" autocorrect="off" required /><span class="no-break">.<?php echo preg_replace(
        "|^www\.|",
        "",
        get_network()->domain
    ); ?></span>
				<?php } else {echo get_network()->domain . get_network()->path; ?>
				...
				<?php }
   echo '<p class="description" id="site-address-desc">' .
  ...
  if (!empty($languages) || !empty($translations)): ?>
			<tr class="form-field form-required">
				<th scope="row"><label for="site-language"><?php _e(
        "Site Language"
    ); ?></label></th>
				<td>
					<?php
...
?>
     ...
     ?>
		...
		<tr class="form-field">
			<td colspan="2" class="td-full"><p id="site-admin-email"><?php _e(
