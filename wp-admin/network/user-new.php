...
if (isset($_REQUEST["action"]) && "add-user" === $_REQUEST["action"]) {
    ...
    if (
        ...
        $user_id = wpmu_create_user(
            esc_html(strtolower($user["username"])),
            $password,
...
?>
	<form action="<?php echo esc_url(
     network_admin_url("user-new.php?action=add-user")
 ); ?>" id="adduser" method="post" novalidate="novalidate">
		<p><?php echo wp_required_field_message(); ?></p>
		<table class="form-table" role="presentation">
			<tr class="form-field form-required">
				<th scope="row"><label for="username"><?php _e(
        "Username"
    ); ?> <?php echo wp_required_field_indicator(); ?></label></th>
        ...
        "Email"
			...
			<tr class="form-field">
				<td colspan="2" class="td-full"><?php _e(
