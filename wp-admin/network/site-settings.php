...
if (
    ...
    foreach ((array) $_POST["option"] as $key => $val) {
        ...
        if (
            0 === $key ||
            is_array($val) ||
            in_array($key, $skip_options, true)
        ) {
...
<form method="post" action="site-settings.php?action=update-site">
  ...
  foreach ($options as $option) {
      ...
      if (str_contains($option->option_value, "\n")) { ?>
				<tr class="form-field">
					<th scope="row"><label for="<?php echo esc_attr(
         $option->option_name
     ); ?>" class="code"><?php echo esc_html(
    $option->option_name
); ?></label></th>
					<td><textarea class="<?php echo $class; ?>" rows="5" cols="40" name="option[<?php echo esc_attr(
    $option->option_name
); ?>]" id="<?php echo esc_attr($option->option_name); ?>"<?php disabled(
    $disabled
); ?>><?php echo esc_textarea($option->option_value); ?></textarea></td>
				</tr>
				<?php } else { ?>
				<tr class="form-field">
					<th scope="row"><label for="<?php echo esc_attr(
    ...
    $option->option_name
					...
					<?php if (
         $is_main_site &&
         in_array($option->option_name, ["siteurl", "home"], true)
     ) { ?>
					<td><code><?php echo esc_html($option->option_value); ?></code></td>
					<?php } else { ?>
					<td><input class="<?php echo $class; ?>" name="option[<?php echo esc_attr(
    $option->option_name
); ?>]" type="text" id="<?php echo esc_attr(
    $option->option_name
