...
if (isset($_REQUEST["action"]) && "update-site" === $_REQUEST["action"]) {
    ...
    if ($is_main_site) {
        ...
        if (!$new_url_scheme) {
            $blog_data["url"] = esc_url(
                $parsed_scheme . "://" . $blog_data["url"]
            );
        }
    ...
    $old_home_host =
        $old_home_parsed["host"] .
        (isset($old_home_parsed["port"]) ? ":" . $old_home_parsed["port"] : "");

    if (
        $old_home_host === $existing_details->domain &&
        $old_home_parsed["path"] === $existing_details->path
    ) {
        $new_home_url = untrailingslashit(
            sanitize_url(
                $blog_data["scheme"] .
                    "://" .
                    $new_details->domain .
    ...
    $old_site_host =
        $old_site_parsed["host"] .
        (isset($old_site_parsed["port"]) ? ":" . $old_site_parsed["port"] : "");

    if (
        $old_site_host === $existing_details->domain &&
        $old_site_parsed["path"] === $existing_details->path
    ) {
        $new_site_url = untrailingslashit(
            sanitize_url(
                $blog_data["scheme"] .
                    "://" .
                    $new_details->domain .
...
<p class="edit-site-actions"><a href="<?php echo esc_url(
    get_home_url($id, "/")
); ?>"><?php _e("Visit"); ?></a> | <a href="<?php echo esc_url(
    get_admin_url($id)
); ?>"><?php _e("Dashboard"); ?></a></p>
<?php
...
<form method="post" action="site-info.php?action=update-site">
	...
	<table class="form-table" role="presentation">
		<?php // The main site of the network should not be updated on this page.
  if ($is_main_site): ?>
		<tr class="form-field">
			<th scope="row"><?php _e("Site Address (URL)"); ?></th>
			<td><?php echo esc_url(
       $parsed_scheme . "://" . $details->domain . $details->path
   ); ?></td>
		</tr>
			<?php
      // For any other site, the scheme, domain, and path can all be changed.
       ...
       "://" .
       esc_attr($details->domain) .
       esc_attr($details->path); ?>" /></td>
		</tr>
		<?php endif; ?>

		<tr class="form-field">
			<th scope="row"><label for="blog_registered"><?php _ex(
       "Registered",
       "site"
   ); ?></label></th>
			<td><input name="blog[registered]" type="text" id="blog_registered" value="<?php echo esc_attr(
       $details->registered
   ); ?>" /></td>
		</tr>
		<tr class="form-field">
			<th scope="row"><label for="blog_last_updated"><?php _e(
       "Last Updated"
   ); ?></label></th>
			<td><input name="blog[last_updated]" type="text" id="blog_last_updated" value="<?php echo esc_attr(
  ...
  ?>
    ...
    _e("Set site attributes"); ?>
			...
			<?php foreach ($attribute_fields as $field_key => $field_label): ?>
				<label><input type="checkbox" name="blog[<?php echo $field_key; ?>]" value="1" <?php checked(
    (bool) $details->$field_key,
    true
