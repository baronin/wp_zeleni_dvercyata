...
if (!empty($_GET["network_admin_hash"])) {
    ...
    if (
        ...
        hash_equals($new_admin_details["hash"], $_GET["network_admin_hash"]) &&
    ...
    !empty($_GET["dismiss"]) &&
    "new_network_admin_email" === $_GET["dismiss"]
) {
...
get_current_screen()->set_help_sidebar(
    "<p><strong>" .
        ...
        "<p>" .
        __(
            '<a href="https://developer.wordpress.org/advanced-administration/multisite/admin/settings/">Documentation on Network Settings</a>'
        ) .
        "</p>" .
        "<p>" .
        __(
...
if ($_POST) {
    ...
    wp_redirect(
        add_query_arg("updated", "true", network_admin_url("settings.php"))
    );
    exit();
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php" novalidate="novalidate">
		...
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><label for="site_name"><?php _e(
        "Network Title"
    ); ?></label></th>
				<td>
					<input name="site_name" type="text" id="site_name" class="regular-text" value="<?php echo esc_attr(
         get_network()->site_name
			...
			<tr>
				<th scope="row"><label for="admin_email"><?php _e(
        "Network Admin Email"
    ); ?></label></th>
				<td>
					<input name="new_admin_email" type="email" id="admin_email" aria-describedby="admin-email-desc" class="regular-text" value="<?php echo esc_attr(
     ...
     if (
         $new_admin_email &&
         get_site_option("admin_email") !== $new_admin_email
     ):
         ...
         $notice_message .= sprintf(
             ...
             esc_url(
                 wp_nonce_url(
                     network_admin_url(
                         "settings.php?dismiss=new_network_admin_email"
                     ),
    ...
    ?>
      ...
      _e("New registrations settings"); ?>
					...
					<label><input name="registration" type="radio" id="registration1" value="none"<?php checked(
         $reg,
         "none"
     ); ?> /> <?php _e("Registration is disabled"); ?></label><br />
					<label><input name="registration" type="radio" id="registration2" value="user"<?php checked(
         $reg,
         "user"
     ); ?> /> <?php _e("User accounts may be registered"); ?></label><br />
					<label><input name="registration" type="radio" id="registration3" value="blog"<?php checked(
         $reg,
         "blog"
     ); ?> /> <?php _e(
     ...
     "Both sites and user accounts can be registered"
         ...
         printf(
             ...
             __(
                 'If registration is disabled, please set %1$s in %2$s to a URL you will redirect visitors to if they visit a non-existent site.'
             ),
         ...
         echo "</p>";
			...
			<tr>
				...
				<?php if (!get_site_option("registrationnotification")) {
        update_site_option("registrationnotification", "yes");
    } ?>
				<td>
					<label><input name="registrationnotification" type="checkbox" id="registrationnotification" value="yes"<?php checked(
     ...
     "Send the network admin an email notification every time someone registers a site or user account"
			...
			<tr id="addnewusers">
				...
				<td>
					<label><input name="add_new_users" type="checkbox" id="add_new_users" value="1"<?php checked(
         get_site_option("add_new_users")
     ); ?> /> <?php _e(
     'Allow site administrators to add new users to their site via the "Users &rarr; Add User" page'
 ); ?></label>
				</td>
			</tr>

			<tr>
				<th scope="row"><label for="illegal_names"><?php _e(
        "Banned Names"
    ); ?></label></th>
				<td>
					<?php
     ...
     ?>
					<input name="illegal_names" type="text" id="illegal_names" aria-describedby="illegal-names-desc" class="large-text" value="<?php echo esc_attr(
         $illegal_names
     ); ?>" size="45" />
          ...
          "Users are not allowed to register these sites. Separate names by spaces."
			...
			<tr>
				<th scope="row"><label for="limited_email_domains"><?php _e(
...
<?php echo esc_textarea($limited_email_domains); ?></textarea>
          ...
          "If you want to limit site registrations to certain domains. One domain per line."
			...
			<tr>
				<th scope="row"><label for="banned_email_domains"><?php _e(
...
<?php echo esc_textarea($banned_email_domains); ?></textarea>
		...
		<table class="form-table" role="presentation">
			...
			<tr>
				<th scope="row"><label for="welcome_email"><?php _e(
        "Welcome Email"
    ); ?></label></th>
				<td>
...
<?php echo esc_textarea(get_site_option("welcome_email")); ?></textarea>
			...
			<tr>
				<th scope="row"><label for="welcome_user_email"><?php _e(
        "Welcome User Email"
    ); ?></label></th>
				<td>
...
<?php echo esc_textarea(get_site_option("first_page")); ?></textarea>
			...
			<tr>
				<th scope="row"><label for="first_comment"><?php _e(
        "First Comment"
    ); ?></label></th>
				<td>
...
<?php echo esc_textarea(get_site_option("first_comment")); ?></textarea>
			...
			<tr>
				<th scope="row"><label for="first_comment_author"><?php _e(
        "First Comment Author"
    ); ?></label></th>
				<td>
					<input type="text" size="40" name="first_comment_author" id="first_comment_author" aria-describedby="first-comment-author-desc" value="<?php echo esc_attr(
         get_site_option("first_comment_author")
     ); ?>" />
			...
			<tr>
				<th scope="row"><label for="first_comment_email"><?php _e(
        "First Comment Email"
    ); ?></label></th>
				<td>
					<input type="text" size="40" name="first_comment_email" id="first_comment_email" aria-describedby="first-comment-email-desc" value="<?php echo esc_attr(
         get_site_option("first_comment_email")
     ); ?>" />
			...
			<tr>
				<th scope="row"><label for="first_comment_url"><?php _e(
        "First Comment URL"
    ); ?></label></th>
				<td>
					<input type="text" size="40" name="first_comment_url" id="first_comment_url" aria-describedby="first-comment-url-desc" value="<?php echo esc_attr(
		...
		<table class="form-table" role="presentation">
			<tr>
				...
				<td>
					<label><input type="checkbox" id="upload_space_check_disabled" name="upload_space_check_disabled" value="0"<?php checked(
         ...
         false
          ...
          '</label><label><input name="blog_upload_space" type="number" min="0" style="width: 100px" id="blog_upload_space" aria-describedby="blog-upload-space-desc" value="' .
              esc_attr(get_site_option("blog_upload_space", 100)) .
              '" />'
      ); ?>
					</label><br />
      ...
      _e("Size in megabytes"); ?>
			...
			<tr>
				<th scope="row"><label for="upload_filetypes"><?php _e(
        "Upload file types"
    ); ?></label></th>
				<td>
					<input name="upload_filetypes" type="text" id="upload_filetypes" aria-describedby="upload-filetypes-desc" class="large-text" value="<?php echo esc_attr(
         get_site_option("upload_filetypes", "jpg jpeg png gif")
     ); ?>" size="45" />
			...
			<tr>
				<th scope="row"><label for="fileupload_maxk"><?php _e(
        "Max upload file size"
         ...
         '<input name="fileupload_maxk" type="number" min="0" style="width: 100px" id="fileupload_maxk" aria-describedby="fileupload-maxk-desc" value="' .
             esc_attr(get_site_option("fileupload_maxk", 300)) .
             '" />'
     ); ?>
  ...
  if ($menu_items): ?>
      ...
      foreach ((array) $menu_items as $key => $val) {
          echo "<label><input type='checkbox' name='menu_items[" .
              $key .
              "]' value='1'" .
              (isset($menu_perms[$key])
                  ? checked($menu_perms[$key], "1", false)
                  : "") .
              " /> " .
              esc_html($val) .
              "</label><br/>";
