...
get_current_screen()->add_help_tab([
    ...
    "content" =>
        "<p>" .
        __(
            "Add Site takes you to the screen for adding a new site to the network. You can search for a site by Name, ID number, or IP address. Screen Options allows you to choose how many sites to display on one page."
        ) .
        "</p>" .
        "<p>" .
        __(
        ...
        "<li>" .
        __(
            "Delete which is a permanent action after the confirmation screen."
        ) .
        "</li>" .
        "<li>" .
...
if (isset($_GET["action"])) {
    ...
    if ("confirm" === $_GET["action"]) {
        ...
        if (is_main_site($id)) {
            wp_die(
                __("Sorry, you are not allowed to change the current site.")
            );
        }

        $site_details = get_site($id);
        $site_address = untrailingslashit(
            $site_details->domain . $site_details->path
        );
        ...
        ?>
			<div class="wrap">
				...
				<form action="sites.php?action=<?php echo esc_attr(
        $site_action
    ); ?>" method="post">
         ...
         wp_get_referer()
					...
					<?php if ("deleteblog" === $site_action) {
         $submit = __("Delete this site permanently"); ?>
						<div class="notice notice-warning inline">
							<p><?php _e(
           "Deleting a site is a permanent action that cannot be undone. This will delete the entire site and its uploads directory."
       ); ?>
						</div>
						<?php
     } else {
  ...
  exit();
    ...
    switch ($_GET["action"]) {
        case "deleteblog":
            ...
            if (
                0 !== $id &&
                !is_main_site($id) &&
                current_user_can("delete_site", $id)
            ) {
                ...
                if (!current_user_can("delete_site", $site_id)) {
                    ...
                    wp_die(
                        sprintf(
                            /* translators: %s: Site URL. */
                            __(
                                "Sorry, you are not allowed to delete the site %s."
                            ),
                            $site_address
                        ),
                        403
              ...
              wp_get_referer()
               ...
               ?>
												<li>
													...
													<input type="hidden" name="site_ids[]" value="<?php echo esc_attr(
                 $site_id
             ); ?>" />
												</li>
											<?php
        ...
        break;
                        ...
                        wp_die(
                            __(
                                "Sorry, you are not allowed to change the current site."
                            )
                        );
                    }
                }
        ...
        case "matureblog":
            update_blog_status(
                $id,
                "mature",
                "matureblog" === $_GET["action"] ? "1" : "0"
            );
            break;
    }

    if (
        empty($updated_action) &&
    ...
    if (!empty($updated_action)) {
        wp_safe_redirect(
            add_query_arg(["updated" => $updated_action], wp_get_referer())
        );
        exit();
    }
}
...
<?php if (current_user_can("create_sites")): ?>
 ...
 ); ?>" class="page-title-action"><?php echo esc_html__("Add Site"); ?></a>
<?php endif; ?>
