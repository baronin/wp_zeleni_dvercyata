...
if (!empty($_GET["trashed"]) && absint($_GET["trashed"])) {
    ...
    $message .= sprintf(
        ...
        esc_url(
            wp_nonce_url(
                "upload.php?doaction=undo&action=untrash&ids=" .
                    (isset($_GET["ids"]) ? $_GET["ids"] : ""),
                "bulk-media"
            )
        ),
        __("Undo")
...
$messages[4] =
    __("Media file moved to the Trash.") .
    sprintf(
        ' <a href="%1$s">%2$s</a>',
        esc_url(
            wp_nonce_url(
...
if ("grid" === $mode) {
   ...
   ); ?>" class="page-title-action aria-button-if-js"><?php echo esc_html__(
    "Add Media File"
); ?></a>
			<?php } ?>
...
if (current_user_can("upload_files")) { ?>
	<a href="<?php echo esc_url(
     admin_url("media-new.php")
 ); ?>" class="page-title-action"><?php echo esc_html__(
    "Add Media File"
); ?></a>
						<?php }
