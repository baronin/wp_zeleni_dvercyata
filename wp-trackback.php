...
if ($charset) {
    ...
    if (
        ...
        !in_array($charset, mb_list_encodings(), true)
    ) {
        $charset = "";
    }
}
...
if (!empty($trackback_url) && !empty($title)) {
    ...
    if (!pings_open($post_id)) {
        trackback_response(
            1,
            __("Sorry, trackbacks are closed for this item.")
        );
    }
    ...
    if ($dupe) {
        trackback_response(
            1,
            __("There is already a ping from that URL for this post.")
        );
    }
