...
function login_header($title = null, $message = "", $wp_error = null)
{
 ...
 $classes[] =
     " locale-" .
     sanitize_html_class(strtolower(str_replace("_", "-", get_locale())));
 ...
 do_action("login_header"); ?>
	<?php if ("confirm_admin_email" !== $action && !empty($title)): ?>
		...
		<?php endif; ?>
	<div id="login">
		<h1 role="presentation" class="wp-login-logo"><a href="<?php echo esc_url(
      $login_header_url
	...
	<?php if (
     ...
     ?>
					...
					<?php if (isset($_GET["redirect_to"]) && "" !== $_GET["redirect_to"]) { ?>
						<input type="hidden" name="redirect_to" value="<?php echo sanitize_url(
          $_GET["redirect_to"]
      ); ?>" />
					<?php } ?>
...
if (
    !in_array($action, $default_actions, true) &&
...
if (defined("RELOCATE") && RELOCATE) {
    ...
    $url = dirname(
        set_url_scheme("http://" . $_SERVER["HTTP_HOST"] . $_SERVER["PHP_SELF"])
    );
...
switch ($action) {
    case "confirm_admin_email":
        ...
        if (!empty($_POST["correct-admin-email"])) {
            ...
            if ($admin_email_check_interval > 0) {
                update_option(
                    "admin_email_lifespan",
                    time() + $admin_email_check_interval
                );
            }
      ...
      site_url("wp-login.php?action=confirm_admin_email", "login_post")
  ); ?>" method="post">
			<?php
   /**
   ...
   ?>
    ...
    ?>
     ...
     ?>
      ...
      ?>
						<a href="<?php echo esc_url($remind_me_link); ?>"><?php _e(
    "Remind me later"
); ?></a>
					</div>
				<?php endif; ?>
  ...
  break;
   ...
   do_action("lostpassword_form"); ?>
       ...
       $redirect_to
		...
		<p id="nav">
			<a class="wp-login-log-in" href="<?php echo esc_url(
       wp_login_url()
   ); ?>"><?php _e("Log in"); ?></a>
  ...
  break;
    ...
    case "rp":
        ...
        if (
            ...
            0 < strpos($_COOKIE[$rp_cookie], ":")
        ) {
            list($rp_login, $rp_key) = explode(
                ":",
                wp_unslash($_COOKIE[$rp_cookie]),
                2
            );
        ...
        if (!empty($_POST["pass1"])) {
            ...
            if (empty($_POST["pass1"])) {
                $errors->add(
                    ...
                    __("The password cannot be a space or all spaces.")
                );
            }
        }
        ...
        if (
            !empty($_POST["pass1"]) &&
            trim($_POST["pass2"]) !== $_POST["pass1"]
        ) {
            $errors->add(
                "password_reset_mismatch",
                __("<strong>Error:</strong> The passwords do not match.")
            );
        }
      ...
      network_site_url("wp-login.php?action=resetpass", "login_post")
       ...
       $rp_login
			...
			<div class="user-pass1-wrap">
				...
				<div class="wp-pwd">
					<input type="password" name="pass1" id="pass1" class="input password-input" size="24" value="" autocomplete="new-password" spellcheck="false" data-reveal="1" data-pw="<?php echo esc_attr(
   ...
   do_action("resetpass_form", $user); ?>
			...
			<p class="submit reset-pass-submit">
				<button type="button" class="button wp-generate-pw hide-if-no-js skip-aria-expanded"><?php _e(
        "Generate Password"
    ); ?></button>
				<input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e(
        "Save Password"
  ...
  break;
    ...
    case "register":
        ...
        login_header(
            ...
            wp_get_admin_notice(__("Register For This Site"), [
                "type" => "info",
      ...
      site_url("wp-login.php?action=register", "login_post")
        ...
        $user_email
    ); ?>" size="25" autocomplete="email" required="required" />
			</p>
			<?php /**
  ...
  break;
    ...
    ?>
        ...
        if (
            empty($_POST) &&
            $errors->get_error_codes() === ["empty_username", "empty_password"]
        ) {
            $errors = new WP_Error("", "");
        }
        ...
        if ($interim_login) {
            ...
            if (isset($_GET["loggedout"]) && $_GET["loggedout"]) {
                ...
                if (!empty($query["app_name"])) {
                    ...
                    $message = sprintf(
                        'Please log in to %1$s to authorize %2$s to connect to your account.',
                        get_bloginfo("name", "display"),
                        "<strong>" . esc_html($query["app_name"]) . "</strong>"
                    );
                } else {
      ...
      site_url("wp-login.php", "login_post")
			...
			<p>
				<label for="user_login"><?php _e("Username or Email Address"); ?></label>
				<input type="text" name="log" id="user_login"<?php echo $aria_describedby; ?> class="input" value="<?php echo esc_attr(
     $user_login
 ); ?>" size="20" autocapitalize="off" autocomplete="username" required="required" />
			</p>
   ...
   do_action("login_form"); ?>
       ...
       $rememberme
   ); ?> /> <label for="rememberme"><?php esc_html_e(
     "Remember Me"
 ); ?></label></p>
			<p class="submit">
				<input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e(
