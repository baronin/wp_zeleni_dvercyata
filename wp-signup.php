...
if (
    is_array(get_site_option("illegal_names")) &&
    isset($_GET["new"]) &&
    in_array($_GET["new"], get_site_option("illegal_names"), true)
) {
...
function show_blog_form($blogname = "", $blog_title = "", $errors = "")
{
    ...
    if (!is_subdomain_install()) {
        echo '<div class="wp-signup-blogname"><span class="prefix_address" id="prefix-address">' .
        ...
        echo '<div class="wp-signup-blogname"><input name="blogname" type="text" id="blogname" value="' .
            esc_attr($blogname) .
            '" maxlength="60" autocomplete="off" required="required" aria-describedby="' .
            $errmsg_blogname_aria .
 ...
 if ($errmsg_blog_title) {
     ...
     echo '<p class="error" id="wp-signup-blog-title-error">' .
         $errmsg_blog_title .
         "</p>";
 }
 echo '<input name="blog_title" type="text" id="blog_title" value="' .
     esc_attr($blog_title) .
...
function show_user_form($user_name = "", $user_email = "", $errors = "")
{
    ...
    if ($errmsg_username) {
        ...
        echo '<p class="error" id="wp-signup-username-error">' .
            $errmsg_username .
            "</p>";
    }
    ?>
	<input name="user_name" type="text" id="user_name" value="<?php echo esc_attr(
     $user_name
 ); ?>" autocapitalize="none" autocorrect="off" maxlength="60" autocomplete="username" required="required" aria-describedby="<?php echo $errmsg_username_aria; ?>wp-signup-username-description" />
	<p id="wp-signup-username-description"><?php _e(
 ...
 if ($errmsg_email) {
     ...
     echo '<p class="error" id="wp-signup-email-error">' .
         $errmsg_email .
         "</p>";
 }
 ?>
	<input name="user_email" type="email" id="user_email" value="<?php echo esc_attr(
     $user_email
 ); ?>" maxlength="200" autocomplete="email" required="required" aria-describedby="<?php echo $errmsg_email_aria; ?>wp-signup-email-description" />
	<p id="wp-signup-email-description"><?php _e(
 ...
 if ($errmsg_generic) {
     echo '<p class="error" id="wp-signup-generic-error">' .
         $errmsg_generic .
         "</p>";
 }
 /**
...
function signup_another_blog($blogname = "", $blog_title = "", $errors = "")
{
 ...
 ?>
  ...
  do_action("signup_hidden_fields", "create-another-site"); ?>
		...
		<p class="submit"><input type="submit" name="submit" class="submit" value="<?php esc_attr_e(
      "Create Site"
  ); ?>" /></p>
	</form>
	<?php
...
function validate_another_blog_signup()
{
     ...
     */
    $meta_defaults = apply_filters_deprecated(
...
function signup_user($user_name = "", $user_email = "", $errors = "")
{
    ...
    $signup_for = isset($_POST["signup_for"])
	...
	<h2>
	<?php /* translators: %s: Name of the network. */
 printf(__("Get your own %s account in seconds"), get_network()->site_name); ?>
	</h2>
	<form id="setupform" method="post" action="wp-signup.php" novalidate="novalidate">
		<input type="hidden" name="stage" value="validate-user-signup" />
		<?php /** This action is documented in wp-signup.php */
  do_action("signup_hidden_fields", "validate-user"); ?>
		...
		<?php else: ?>
			<fieldset class="signup-options">
				...
				<p class="wp-signup-radio-buttons">
					<span class="wp-signup-radio-button">
						<input id="signupblog" type="radio" name="signup_for" value="blog" <?php checked(
          $signup_for,
          "blog"
      ); ?> />
					...
					<span class="wp-signup-radio-button">
						<input id="signupuser" type="radio" name="signup_for" value="user" <?php checked(
          $signup_for,
          "user"
      ); ?> />
						<label class="checkbox" for="signupuser"><?php _e(
...
function confirm_user_signup($user_name, $user_email)
{
 ...
 printf(__("%s is your new username"), $user_name); ?>
	...
	<p>
	<?php /* translators: %s: The user email address. */
 printf(
     __("Check your inbox at %s and click on the given link."),
...
function signup_blog(
	...
	<form id="setupform" method="post" action="wp-signup.php">
		...
		<input type="hidden" name="user_name" value="<?php echo esc_attr(
      $user_name
  ); ?>" />
		<input type="hidden" name="user_email" value="<?php echo esc_attr(
      $user_email
  ); ?>" />
		<?php /** This action is documented in wp-signup.php */
...
function confirm_blog_signup(
    ...
    ?>
	<h2>
	<?php /* translators: %s: Site address. */
 printf(
...
$newblogname = isset($_GET["new"])
    ? strtolower(preg_replace('/^-|-$|[^-a-zA-Z0-9]/', "", $_GET["new"]))
    : null;
...
if ("none" === $active_signup) {
    ...
    switch ($stage) {
        ...
        default:
            ...
            if (
                is_user_logged_in() &&
                ("all" === $active_signup || "blog" === $active_signup)
            ) {
                signup_another_blog($newblogname);
            } elseif (
                !is_user_logged_in() &&
                ("all" === $active_signup || "user" === $active_signup)
            ) {
