...
if (
    null === $result ||
    (is_wp_error($result) && "invalid_key" === $result->get_error_code())
) {
    status_header(404);
...
<div id="signup-content" class="widecolumn">
	...
	<?php if (!$key) { ?>
		...
		<form name="activateform" id="activateform" method="post" action="<?php echo esc_url(
      network_site_url($blog_details->path . "wp-activate.php")
  ); ?>">
			<p>
			...
			<p class="submit">
				<input id="submit" type="submit" name="Submit" class="submit" value="<?php esc_attr_e(
        "Activate"
    ); ?>" />
			</p>
		</form>
		...
		<?php } else {if (
         is_wp_error($result) &&
         in_array($result->get_error_code(), $valid_error_codes, true)
     ) {
   ...
   if ("" === $signup->domain . $signup->path) {
       ...
       printf(
           ...
           sprintf(
               '<a href="http://%1$s">%1$s</a>',
               esc_url($signup->domain . $blog_details->path)
           ),
           esc_html($signup->user_login),
           esc_html($signup->user_email),
           esc_url(wp_lostpassword_url())
   ...
   echo "</p>";
         ...
         $url = isset($result["blog_id"])
             ? esc_url(get_home_url((int) $result["blog_id"]))
             : "";
         ...
         ?>
			...
			<p><span class="h3"><?php _e("Username:"); ?></span> <?php echo esc_html(
    $user->user_login
); ?></p>
			<p><span class="h3"><?php _e("Password:"); ?></span> <?php echo esc_html(
    $result["password"]
); ?></p>
			</div>

			<?php if ($url && network_home_url("", "http") !== $url):
       ...
       ?>
				<p class="view">
				<?php /* translators: 1: Site URL, 2: Login URL. */
    printf(
        __(
